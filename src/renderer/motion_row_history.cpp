...
bool finite(const SubmittedMatrix& rows) noexcept {
    for (float v : rows)
        if (!std::isfinite(v) || std::fabs(v) > 1e15f) return false;
    return true;
}
} // namespace
...
bool MotionRowHistory::less(const RigidDrawKey& a, const RigidDrawKey& b) noexcept {
    return fields(a) < fields(b);
}
bool MotionRowHistory::equal(const RigidDrawKey& a, const RigidDrawKey& b) noexcept {
    return fields(a) == fields(b);
}
...
bool MotionRowHistory::key_valid(const RigidDrawKey& k) noexcept {
    ...
    if (!k.object_lifetime || !k.camera_lifetime || !k.draw_domain || !k.node || !k.camera || !k.vertex_buffer ||
        !k.declaration || !k.position_program || !k.stride || !k.primitives || !k.pass)
        return false;
...
void MotionRowHistory::begin_frame(const MotionRowFrame& frame) noexcept {
    ...
    if (!frame.generation || !frame.width || !frame.height || frame.generation != previous_frame_.generation ||
...
MotionRowHistory::Entry* MotionRowHistory::find_previous(const RigidDrawKey& key) noexcept {
    auto it = std::lower_bound(
        previous_.begin(), previous_.end(), key,
...
unsigned MotionRowHistory::classify_miss(const RigidDrawKey& key) const noexcept {
    ...
    const auto prefix = [](const RigidDrawKey& k) {
        return std::tie(k.object_lifetime, k.camera_lifetime, k.draw_domain, k.node);
    };
...
bool MotionRowHistory::commit(bool frame_succeeded) noexcept {
    if (!ready_ || !collecting_ || !frame_succeeded || overflow_) {
        invalidate();
        return false;
    }
    ...
    for (std::size_t i = 1; i < current_.size(); ++i)
        if (equal(current_[i - 1].key, current_[i].key)) current_[i - 1].poisoned = current_[i].poisoned = true;
