...
constexpr unsigned mov = 1, add = 2, mad = 4, mul = 5, dp4 = 9, minimum = 10, maximum = 11, dcl = 31, power = 32,
                   texld = 66, def = 81, cmp = 88;
...
unsigned type(Word value) noexcept {
    return ((value >> 28) & 7) | ((value >> 8) & 24);
}
unsigned index(Word value) noexcept {
    return value & 0x7ff;
}
Word reg(unsigned kind, unsigned number) noexcept {
    return 0x80000000u | ((kind & 7) << 28) | ((kind & 24) << 8) | number;
}
Word dst(unsigned kind, unsigned number, unsigned mask = 7) noexcept {
    return reg(kind, number) | (mask << 16);
}
...
Word lane(unsigned kind, unsigned number, unsigned component) noexcept {
    return src(kind, number, component * 0x55);
}
...
struct Shape {
    unsigned arithmetic = 0, texture = 0, outputs = 0;
    std::size_t first = 0;
};
...
bool structure(const Word* code, std::size_t count, Shape& result, PixelModel model) noexcept {
    ...
    if (count < 2 || (model != PixelModel::Sm20 && model != PixelModel::Sm2x) || code[0] != static_cast<Word>(model))
        return false;
    bool executable = false, sampler_declared = false;
    for (std::size_t at = 1; at < count;) {
        const auto token = code[at], op = token & 0xffffu;
        if (token == end_token)
            return at == count - 1 && result.first && result.arithmetic <= 64 && result.texture <= 32;
        ...
        if (op == def) {
            if (executable || size != 5 || code[at + 1] != dst(constant, index(code[at + 1]), 15) ||
                index(code[at + 1]) >= 32)
                return false;
        } else if (op == dcl) {
            ...
            if (type(declared) == sampler) {
                if (sampler_declared || declared != dst(sampler, 0, 15) || code[at + 1] != (0x80000000u | (2u << 27)))
                    return false;
                sampler_declared = true;
            } else if (code[at + 1] != 0x80000000u ||
                       (declared != (dst(coordinate, 0, 3) | pp) && declared != dst(color, 0, 1)))
                return false;
        } else {
            executable = true;
            const unsigned expected = (op == mov ? 2 : op == cmp ? 4 : 3);
            if (size != expected || (op != mov && op != mul && op != dp4 && op != minimum && op != maximum &&
                                     op != power && op != texld && op != cmp))
                return false;
            const auto target = code[at + 1];
            const unsigned target_type = type(target), target_index = index(target), mask = (target >> 16) & 15;
            if (!mask || (target != (dst(target_type, target_index, mask)) &&
                          target != (dst(target_type, target_index, mask) | pp)))
                return false;
            if (target_type == temporary) {
                if (target_index >= 12) return false;
            } else if (target_type == output) {
                if (target_index > 2 || op != mov || mask != 15 || (result.outputs & (1u << target_index)) ||
                    (target_index != 0 && target != dst(output, target_index, 15)))
                    return false;
            ...
            if (op == texld) {
                if (!sampler_declared || code[at + 2] != src(coordinate, 0) || code[at + 3] != src(sampler, 0))
                    return false;
                ++result.texture;
            } else {
                if (op == power) {
                    ...
                    if (target_type != temporary ||
                        (type(code[at + 3]) == temporary && index(code[at + 3]) == target_index))
                        return false;
...
bool original_shape(const Word* code, const Profile& profile, const Shape& shape) noexcept {
    ...
    if (profile.affine &&
        !expect(code, profile.words, at, def, {dst(constant, 3, 15), bits(1), bits(0), bits(0), bits(0)}))
        return false;
    if (at != profile.declaration || !expect(code, profile.words, at, dcl, {0x80000000u, dst(coordinate, 0, 3) | pp}))
        return false;
    ...
    if (!expect(code, profile.words, at, dcl, {0x80000000u | (2u << 27), dst(sampler, 0, 15)}) ||
        !expect(code, profile.words, at, texld, {dst(temporary, 0, 15) | pp, src(coordinate, 0), src(sampler, 0)}))
        return false;
    if (profile.affine) {
        if (!expect(code, profile.words, at, mov, {dst(temporary, 1) | pp, src(temporary, 0)}) ||
            !expect(code, profile.words, at, mov, {dst(temporary, 1, 8), lane(constant, 3, 0)}))
            return false;
        for (unsigned component = 0; component < 3; ++component)
            if (!expect(code, profile.words, at, dp4,
                        {dst(temporary, 0, 1u << component) | pp, src(temporary, 1), src(constant, component)}))
                return false;
    }
    if (at != profile.copy_before) return false;
    if (profile.fade &&
        !expect(code, profile.words, at, mul, {dst(temporary, 0) | pp, src(temporary, 0), lane(color, 0, 0)}))
        return false;
...
struct HullProfile {
    std::uint64_t pixel;
    unsigned words, definition, emission;
    bool modulated;
};
// clang-format off
...
// clang-format on
static_assert(sizeof(hull_profiles) / sizeof(hull_profiles[0]) == linear_emission_hull_program_count,
              "twelve reviewed hull programs");
...
unsigned slot_cost(unsigned op) noexcept {
    ...
    case 18:
    case 24:
    case 33: return 2; // lrp, m3x2, crs
    case 21:
    case 23:
    case power:
    case 36: return 3; // m4x3, m3x3, pow, nrm
    case 20:
    case 22: return 4; // m4x4, m3x4
    case 37: return 8; // sincos
    default: return 1;
    }
}
bool texture_op(unsigned op) noexcept {
    return op == 65 || op == texld || op == 93 || op == 95;
}
bool hull_structure(const Word* code, std::size_t count, std::size_t& first_declaration, unsigned& instructions,
                    bool original = true) noexcept {
...
SourceGainBlend linear_emission_source_gain_blend(std::uint32_t blend_enable, std::uint32_t srgb_write,
    ...
    constexpr std::uint32_t blend_one = 2, blend_inv_src_color = 4, op_add = 1; // D3DBLEND_ONE, D3DBLEND_INVSRCCOLOR,
                                                                                // D3DBLENDOP_ADD
...
LinearEmissionResult linear_emission_pixel_variant(const Word* original, std::size_t count,
    ...
    for (const auto& profile : profiles)
        if (profile.pixel == hash && profile.words == count) {
            selected = &profile;
            break;
        }
    ...
    if (!structure(original, count, original_structure, profile.model) ||
        !original_shape(original, profile, original_structure))
    ...
    try {
        ...
        emit(result, def,
             {dst(constant, 31, 15), bits(config.gain == 0 ? 0 : config.gain), bits(config.coverage ? 1.0f : 0.0f),
              bits(0), bits(0)});
        ...
        Shape transformed;
        if (!structure(result.data(), result.size(), transformed, profile.model) ||
            transformed.outputs != (config.coverage ? 7u : 3u) || transformed.texture != 1 ||
            transformed.arithmetic !=
                original_structure.arithmetic + (profile.fade ? 22u : 21u) + (config.coverage ? 2u : 0u))
...
SourceGainBlend linear_emission_hull_source_gain_blend(std::uint32_t blend_enable, std::uint32_t srgb_write,
    ...
    if (!blend_enable || srgb_write || src != blend_one || dst != blend_one || op != op_add)
        return SourceGainBlend::Blend;
    return SourceGainBlend::Admit;
}
LinearEmissionResult linear_emission_hull_source_gain_variant(const Word* original, std::size_t count, float gain,
    ...
    if (!hull_structure(original, count, first_declaration, instructions) || first_declaration != profile.definition ||
        !hull_tail(original, profile))
        return LinearEmissionResult::ProfileMismatch;
    try {
        Words result;
        if (gain == 1) {
