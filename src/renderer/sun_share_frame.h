...
enum class SunUntrackedReason : std::uint8_t {
    ...
    NoZWrite = 5,     // gate 4: z test or z write disabled (or state unreadable)
    Blended = 6,      // gate 4: alpha blending enabled
    State = 7,        // gate 4: sRGB write, alpha test/color mask outside the opaque and cutout arms
...
struct SunShareFrame {
    ...
    // Returns true when the draw was counted as an untracked writer.
