...
constexpr const char* sun_untracked_reason_name(unsigned reason) noexcept {
    ...
    case 4: return "pair";
    case 5: return "no_zwrite";
    case 6: return "blended";
    case 7: return "state";
    case 8: return "rows";
    case 9: return "geometry";
    case 10: return "no_depth";
    case 11: return "fade_arm";
    case 12: return "apply_failed";
    case 13: return "scope";
    case 14: return "history";
    case 15: return "read_failed";
...
inline bool pixel_program_writes_depth(const std::uint32_t* code, std::size_t words) noexcept {
    ...
    std::size_t at = 1;
    while (at < words) {
        const std::uint32_t token = code[at];
        const unsigned op = token & 0xffffu;
        if (op == 0xffffu) return false;
        if (op == 0xfffeu) {
            at += 1 + ((token >> 16) & 0x7fffu);
            continue;
        }
        if (major >= 2) {
            const unsigned length = (token >> 24) & 15u;
            if (at + length >= words) return true;
            if (op != 0x51u && op != 0x30u && op != 0x2fu && op != 0x1fu) // def, defi, defb, dcl carry no register
                                                                          // writes
                for (unsigned i = 1; i <= length; ++i) {
                    const std::uint32_t operand = code[at + i];
                    if ((operand & 0x80000000u) && (((operand >> 28) & 7u) | ((operand >> 8) & 0x18u)) == 9u)
                        return true;
                }
            at += 1 + length;
        } else {
