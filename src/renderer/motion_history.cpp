...
auto fields(const RigidDrawKey& k) noexcept {
    return std::tie(k.object_lifetime, k.camera_lifetime, k.draw_domain, k.node, k.camera, k.mesh, k.node_handle,
                    k.camera_handle, k.model, k.lod, k.vertex_buffer, k.vertex_revision, k.index_buffer,
                    k.index_revision, k.declaration, k.position_program, k.stream_offset, k.stride, k.position_offset,
                    k.position_type, k.topology, k.first, k.primitives, k.base_vertex, k.min_vertex, k.vertex_count,
                    k.index_format, k.indexed, k.pass);
}
} // namespace
...
MotionHistory::MotionHistory(std::size_t capacity, MotionHistoryPurpose purpose) noexcept
    ...
    , purpose_(purpose) {}
bool MotionHistory::less(const RigidDrawKey& a, const RigidDrawKey& b) noexcept {
    return fields(a) < fields(b);
}
bool MotionHistory::equal(const RigidDrawKey& a, const RigidDrawKey& b) noexcept {
    return fields(a) == fields(b);
}
...
Correspondence MotionHistory::validate(const RigidObservation& o) noexcept {
    ...
    if (!k.object_lifetime || !k.camera_lifetime || !k.draw_domain || !k.node || !k.camera || !k.mesh ||
        !k.vertex_buffer || !k.declaration || !k.position_program || !position_bytes || !k.stride ||
        k.position_offset > k.stride || k.stride - k.position_offset < position_bytes || !k.primitives ||
        (k.topology != 4 && k.topology != 5) ||
        (k.indexed && (!k.index_buffer || !k.vertex_count || (k.index_format != 101 && k.index_format != 102))) ||
        (!k.indexed &&
         (k.index_buffer || k.index_revision || k.index_format || k.base_vertex || k.min_vertex || k.vertex_count)))
...
bool MotionHistory::begin_frame(MotionFrame frame) noexcept {
    ...
    if (!capacity_ || !frame.frame || !frame.epoch || !frame.width || !frame.height ||
        ...
        (frame.continuity != MotionContinuity::Unknown && frame.continuity != MotionContinuity::Discontinuity &&
    ...
    if (!previous_valid_ || previous_frame_.frame == std::numeric_limits<std::uint64_t>::max() ||
        ...
        (purpose_ == MotionHistoryPurpose::TemporalAccumulation && frame.continuity != MotionContinuity::Continuous)) {
        previous_.clear();
        previous_valid_ = false;
    }
    current_.clear();
...
RigidMotionPair MotionHistory::lookup(const RigidDrawKey& key) const noexcept {
    ...
    if (!now) {
        result.status = Correspondence::MissingCurrent;
        return result;
    }
    if (now->status != Correspondence::Matched) {
        result.status = now->status;
        return result;
    }
    if (!previous_valid_) {
        result.status = Correspondence::NoPreviousFrame;
        return result;
    }
    const auto* old = find(previous_, key);
    if (!old) {
        result.status = Correspondence::MissingPrevious;
        return result;
    }
    if (old->status != Correspondence::Matched) {
        result.status = old->status;
        return result;
    }
    result.status = Correspondence::Matched;
    result.continuity = frame_.continuity;
    result.current = now->matrix;
    result.previous = old->matrix;
    return result;
}
bool MotionHistory::commit(bool frame_succeeded) noexcept {
    if (!frame_succeeded || phase_ != Phase::Sealed) {
        invalidate();
        return false;
    }
