...
constexpr unsigned composition_policy_bit(LinearCompositionPolicy p) noexcept {
    return unsigned(p);
}
...
struct LinearEmissionBoundary {
    ...
    IDirect3DPixelShader9* augmented = nullptr; // cached, qualified three-output PS
...
public:
    ...
    HRESULT attach(
        IDirect3DDevice9*, void* const* native, const D3DCAPS9&, D3DFORMAT adapter_format, D3DFORMAT depth_format,
    ...
    void fixture_separate_copy(bool enabled) noexcept {
        if (!impl_) fixture_separate_copy_ = enabled;
    }
    ...
    void fixture_source_over(const DWORD* words) noexcept {
        if (!impl_) fixture_source_over_ = words;
    }
#endif
