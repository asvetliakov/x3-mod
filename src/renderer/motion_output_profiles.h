...
inline constexpr std::size_t motion_output_profile_count = sizeof motion_output_profiles /
                                                           sizeof motion_output_profiles[0];
...
constexpr bool motion_output_depth_valid(const MotionOutputProfile& row) noexcept {
    const bool vertex_ok = row.vertex_depth_output_register == motion_output_depth_none ||
                           (row.vertex_depth_output_register < 12 &&
                            row.vertex_depth_output_register != row.vertex_output_register);
    ...
    const bool pixel_ok = row.pixel_depth_input_register == motion_output_depth_none ||
                          (row.pixel_depth_input_register < 10 &&
                           row.pixel_depth_input_register != row.pixel_input_register);
    // A pixel program can only read an interpolator its vertex program writes.
    const bool output_ok = !row.depth_output || (motion_output_vertex_exports_depth(row) &&
                                                 row.pixel_depth_input_register != motion_output_depth_none);
...
constexpr bool motion_output_profile_valid(const MotionOutputProfile& row) noexcept {
    ...
    bool ok = class_valid && row.vertex_version == 0xfffe0300u && row.pixel_version == 0xffff0300u &&
              row.vertex_dword_count > 2 && row.pixel_dword_count > 2 && row.vertex_output_register < 12 &&
              row.texcoord_index < 16 && row.pixel_input_register < 10 && row.pixel_temporary_base + 3 <= 32 &&
              row.pixel_output_register < 4 && row.pixel_output_register != 0 && row.vertex_constant_base + 4 <= 256 &&
              row.pixel_constant_base + 5 <= 224 && row.matrix_register + 4 <= row.vertex_constant_base &&
              row.position_temporary < 32 && row.vertex_declaration_insert_dword < row.vertex_arithmetic_insert_dword &&
              ...
              row.position_dp4_dwords[3] + 4 == row.vertex_arithmetic_insert_dword && motion_output_depth_valid(row);
...
constexpr bool motion_output_vertex_sides_agree(const MotionOutputProfile& a, const MotionOutputProfile& b) noexcept {
    bool ok = a.vertex_dword_count == b.vertex_dword_count && a.vertex_version == b.vertex_version &&
              ...
              a.depth_texcoord_index == b.depth_texcoord_index && a.vertex_constant_base == b.vertex_constant_base &&
...
constexpr bool motion_output_pixel_sides_agree(const MotionOutputProfile& a, const MotionOutputProfile& b) noexcept {
    return a.pixel_dword_count == b.pixel_dword_count && a.pixel_version == b.pixel_version &&
           ...
           a.pixel_input_register == b.pixel_input_register && a.pixel_temporary_base == b.pixel_temporary_base &&
           a.pixel_output_register == b.pixel_output_register && a.pixel_constant_base == b.pixel_constant_base &&
...
constexpr bool motion_output_profiles_consistent() noexcept {
    for (std::size_t i = 0; i < motion_output_profile_count; ++i) {
        ...
        for (std::size_t j = 0; j < i; ++j) {
            ...
            if (a.vertex_fingerprint == b.vertex_fingerprint && !motion_output_vertex_sides_agree(a, b)) return false;
            if (a.pixel_fingerprint == b.pixel_fingerprint && !motion_output_pixel_sides_agree(a, b)) return false;
        }
    }
