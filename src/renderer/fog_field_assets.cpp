...
float f32(const std::uint8_t* p) noexcept { float v; const auto bits = u32(p); std::memcpy(&v, &bits, 4); return v; }
bool unit(float v) noexcept { return std::isfinite(v) && v >= 0.f && v <= 1.f; }
...
struct FamilyFile {
    ...
    explicit FamilyFile(const wchar_t* path) noexcept {
        ...
        else if (GetFileType(handle) != FILE_TYPE_DISK) { CloseHandle(handle); handle = INVALID_HANDLE_VALUE; error = ERROR_ACCESS_DENIED; }
    }
    ~FamilyFile() { if (handle != INVALID_HANDLE_VALUE) CloseHandle(handle); }
    ...
    bool size(std::uint64_t& out) const noexcept {
        ...
        out = std::uint64_t(value.QuadPart); return true;
    }
    ...
    explicit FamilyFile(const char* path) noexcept {
        struct stat info {};
        if (::stat(path, &info) != 0) { absent = true; return; }
        if (S_ISREG(info.st_mode)) handle = std::fopen(path, "rb");
    }
    ~FamilyFile() { if (handle) std::fclose(handle); }
    FamilyFile(const FamilyFile&) = delete;
    FamilyFile& operator=(const FamilyFile&) = delete;
    ...
    bool size(std::uint64_t& out) const noexcept {
        ...
        out = std::uint64_t(end); return true;
    }
    bool read(std::uint64_t offset, void* out, std::size_t bytes) const noexcept {
        if (offset > std::uint64_t(std::numeric_limits<off_t>::max()) || fseeko(handle, off_t(offset), SEEK_SET) != 0) return false;
...
bool compiled_name(const char* name) noexcept {
    for (const auto& family : family_profiles) if (!std::strcmp(name, family.family)) return true;
    return false;
}
...
bool valid_name(const std::uint8_t* p) noexcept {
    ...
    for (std::size_t i = length; i < 32; ++i) if (p[i]) return false;
    return true;
}
void reject(FamilyTable& table, const char* reason) noexcept {
    table.status = FamilyFileStatus::Rejected; table.reason = reason;
...
void load_family_file(const family_path_char* path, FamilyTable& table) noexcept {
    table.families = table.packets = table.rows_disabled = 0; table.file_bytes = 0;
    if (!path || !*path) { table.status = FamilyFileStatus::Absent; table.reason = "absent"; return; }
    FamilyFile file(path);
    if (!file.open()) {
        if (file.missing()) { table.status = FamilyFileStatus::Absent; table.reason = "absent"; }
        else reject(table, "open_failed");
        return;
    }
    ...
    if (table_bytes != families * family_row_bytes + packets * family_packet_row_bytes) return reject(table, "table_bytes");
    ...
    table.families = families; table.packets = packets;
    ...
    for (std::uint32_t j = 0; j < packets; ++j) {
        ...
        packet.offset = u64(p); packet.size = u64(p + 8);
        packet.width = u32(p + 16); packet.height = u32(p + 20); packet.texel_bytes = u32(p + 24);
        packet.decoded_bytes = u32(p + 28); packet.decoded_fnv1a = u64(p + 32); packet.profile = u32(p + 40);
    }
    for (std::uint32_t i = 0; i < families; ++i) {
        ...
        row.profile = u32(r + 32); row.packet = u32(r + 36);
        row.base_sigma = f32(r + 40); row.occupancy = f32(r + 44);
        for (unsigned c = 0; c < 3; ++c) row.chroma[c] = f32(r + 48 + 4 * c);
        for (unsigned k = 0; k < 12; ++k) row.colours[k / 3][k % 3] = f32(r + 60 + 4 * k);
        row.flags = u32(r + 108);
        const char* reason = nullptr;
        if (!valid_name(r)) { row.name[0] = 0; reason = "name"; }
        else if (compiled_name(row.name)) reason = "name_compiled";
        else if (row.profile != family_name_id(row.name)) reason = "profile_id";
        else if (row.packet >= packets) reason = "packet_index";
        else if (!std::isfinite(row.base_sigma) || !(row.base_sigma > 0.f) || row.base_sigma > 1e-4f) reason = "sigma";
        else if (!std::isfinite(row.occupancy) || row.occupancy < .01f || row.occupancy > .5f) reason = "occupancy";
        else if (!unit(row.chroma[0]) || !unit(row.chroma[1]) || !unit(row.chroma[2])) reason = "chroma";
        else if (row.flags & ~(family_flag_background_palette | family_flag_override)) reason = "flags";
        else for (unsigned k = 0; k < 12 && !reason; ++k) if (!unit(row.colours[k / 3][k % 3])) reason = "colour";
        ...
        for (std::uint32_t e = 0; !reason && e < i; ++e) {
            if (!std::strcmp(table.rows[e].name, row.name)) reason = "name_duplicate";
            else if (table.rows[e].profile == row.profile) reason = "profile_id_duplicate";
        }
    ...
    for (std::uint32_t j = 0; j < packets; ++j) {
        ...
        if (packet.width != atlas_width || packet.height != atlas_height || packet.texel_bytes != atlas_texel ||
            packet.decoded_bytes != atlas_decoded) reason = "packet_dimensions";
        else if (packet.decoded_fnv1a == 0) reason = "packet_checksum_zero";
        else if (!owner || !is_file_profile(static_cast<Profile>(packet.profile)) || u32(p + 44) != 0) reason = "packet_profile";
        else if (packet.size <= header_size || packet.size > max_packet_bytes) reason = "packet_size";
        else if (packet.offset < table_end || packet.offset > file_size || packet.size > file_size - packet.offset) reason = "packet_offset";
        else {
            ...
            if (!file.read(packet.offset, head, header_size)) reason = "read_failed";
                     ...
                     u32(head + 24) != packet.width || u32(head + 28) != packet.height || u32(head + 32) != packet.texel_bytes ||
                     u32(head + 36) != packet.decoded_bytes || u64(head + 44) != packet.decoded_fnv1a || u32(head + 52) != 0)
                reason = "packet_header";
        }
        if (reason) disable_packet(table, j, reason);
    }
    table.status = FamilyFileStatus::Loaded; table.reason = "ok";
}
...
bool family_profile_info(const FamilyTable& table, Profile profile, ProfileInfo& info) noexcept {
    ...
    info = ProfileInfo{static_cast<Profile>(packet.profile), qualified_recipe_id, packet.width, packet.height,
                       packet.texel_bytes, packet.decoded_bytes, row->base_sigma, packet.decoded_fnv1a, 0};
    return true;
}
...
Result decode_family_packet(FamilyTable& table, const family_path_char* path, Profile profile,
    ...
    Result result{};
    {
        ...
        if (!allocated) result = fail(Status::AllocationFailed, output);
        else {
            FamilyFile file(path);
            std::uint64_t size = 0;
            if (!file.open() || !file.size(size) || size != table.file_bytes || !file.read(packet.offset, bytes.data(), bytes.size()))
                result = fail(Status::ResourceLoadFailed, output);
            else result = decode_packet(bytes.data(), bytes.size(), info, output);
        }
    ...
    if (!result && (result.status != Status::AllocationFailed || packet.allocation_failures.fetch_add(1, std::memory_order_relaxed) > 0))
...
bool load_family_table() noexcept {
    ...
    if (env && env < 8 && (!lstrcmpW(value, L"0") || !lstrcmpiW(value, L"none"))) {
        ...
        if (!buffer) reject(global_table, "path");
        else {
            global_path = buffer;
            if (!WideCharToMultiByte(CP_UTF8, 0, buffer, -1, global_path_utf8, int(sizeof(global_path_utf8)), nullptr, nullptr))
    ...
    if (value && (!std::strcmp(value, "0") || !std::strcmp(value, "none") || !std::strcmp(value, "NONE"))) {
        ...
        load_family_file(global_path, global_table);
    } else load_family_file(nullptr, global_table); // host builds have no module directory: absent
#endif
...
const char* family_table_path() noexcept { return global_path_utf8; }
