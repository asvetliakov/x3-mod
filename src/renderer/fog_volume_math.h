...
inline bool fog_world_inverse(const double rotation[9], double inv[9]) noexcept {
    ...
    for (unsigned i = 0; i < 3; ++i)
        for (unsigned j = 0; j < 3; ++j) {
            double dot = 0;
            for (unsigned k = 0; k < 3; ++k) dot += rotation[3 * i + k] * rotation[3 * j + k];
...
inline bool fog_world_basis(const double rotation[9], const double translation[3], const double sun_view[3],
                            FogWorldBasis& output) noexcept {
    output = {};
    if (!rotation || !translation || !sun_view) return false;
    for (unsigned i = 0; i < 3; ++i)
        if (!std::isfinite(translation[i]) || !std::isfinite(sun_view[i])) return false;
    double inv[9];
    if (!fog_world_inverse(rotation, inv)) return false;
    FogWorldBasis result{};
    double sun[3]{}, length2 = 0;
    for (unsigned col = 0; col < 3; ++col) {
        double origin = 0;
        ...
        double mod = std::fmod(origin, fog_volume_period);
        if (mod < 0) mod += fog_volume_period;
...
inline void fog_phase_constants(float g, float gamma, const float radiance[3], float phase[4],
                                float light[4]) noexcept {
    // Preserve the qualified default coefficients' rounding, while other g uses HG.
    phase[0] = g == .3f ? 1.09f : 1.f + g * g;
    phase[1] = g == .3f ? .6f : 2.f * g;
    phase[2] = g == .3f ? .91f : 1.f - g * g;
    phase[3] = gamma;
