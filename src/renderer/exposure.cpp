...
float meter_level0(const float rgb_engine[3], ExposureDecode mode, const ExposureParams& p) noexcept {
    const float L = luma(decode_channel(rgb_engine[0], mode), decode_channel(rgb_engine[1], mode),
                         decode_channel(rgb_engine[2], mode));
...
bool meter_clipped(const float rgb_engine[3], ExposureDecode mode, const ExposureParams& p) noexcept {
    return luma(decode_channel(rgb_engine[0], mode), decode_channel(rgb_engine[1], mode),
                decode_channel(rgb_engine[2], mode)) > p.meter_clip;
}
...
bool reduce_chain(float* mean, float* max, float* scratch, unsigned& width, unsigned& height, unsigned factor,
                  unsigned tile_max) noexcept {
...
MeterStatistics meter_statistics(const float* mean_log, const float* max_log, const float* weights, unsigned tiles,
    ...
    for (unsigned i = 0; i < tiles; ++i) {
        ...
        if (v >= bg_log) {
            ...
            scratch[m.lit++] = TileSample{v, w};
            sum_lit += double(v) * double(w);
            weight_lit += w;
        }
    }
    ...
    std::nth_element(scratch, scratch + p99, scratch + tiles,
                     [](const TileSample& a, const TileSample& b) { return a.value < b.value; });
...
float ev_target(const MeterStatistics& m, const ExposureParams& p) noexcept {
    return exposure_target(m, p).ev_target;
}
...
float taa_k(float exposure) noexcept {
    return std::isfinite(exposure) && exposure >= 0.f ? exposure : 0.f;
}
float luma_weight(float luma_value, float k) noexcept {
    return 1.f / (1.f + k * luma_value);
}
...
ExposureState::ExposureState(const ExposureParams& params, ExposureMode mode, float ev_manual,
                             float ev_initial) noexcept
    : params_(params)
    , mode_(mode)
    , ev_manual_(ev_manual)
    , ev_initial_(ev_initial)
    , ev_adapted_(ev_initial) {}
void ExposureState::configure(const ExposureParams& params, ExposureMode mode, float ev_manual) noexcept {
    params_ = params;
    mode_ = mode;
    ev_manual_ = std::isfinite(ev_manual) ? clampf(ev_manual, params.ev_min, params.ev_max) : 0.f;
}
void ExposureState::reset() noexcept {
    ev_adapted_ = ev_initial_;
    ev_target_ = 0.f;
    target_ = ExposureTarget{};
    meter_ = MeterStatistics{};
    dt_ = 0.f;
    steps_ = 0;
}
float ExposureState::step(const MeterStatistics& meter, float dt) noexcept {
    meter_ = meter;
    dt_ = clamp_dt(dt, params_);
