...
// wrapped shaders (per pixel program, and per vertex / pixel pair) and (diagnostic / fixture only) one 1x1 system-memory surface. Device and
...
enum class LensVerdict : std::uint8_t { Applied, NotReady, NoShader, Unhashed, Blend, Variant, CacheFull, Device, Body };
