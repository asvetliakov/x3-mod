...
// clip w), and f from a 3x3 PCF of the cascade replay maps
// (src/temporal/sun_shadow_cascade_apply_ps.hlsl). The pass owns only its
// program pair, the quad declaration and a D3DSBT_ALL block; the caller
// (MotionOutput's scene-end hook) supplies RT2, the maps, each map's view ->
// sun rows, the jittered projection latch and the target, and serializes
// rendering, Reset and teardown. Documented D3D9 only; the format gates are
// CheckDeviceFormat queries. The single-map program (sun_shadow_apply_ps.hlsl,
// execute) was removed on 2026-09-25 (docs/architecture/directional-shadows.md,
// "Single map removed").
#include <cstdint>
#include <d3d9.h>
...
public:
    ...
    // the cascade program's slot count against MaxPixelShader30InstructionSlots,
    // ZERO/SRCCOLOR blend caps, A32B32G32R32F/R32F render-target textures and
    ...
    // result.skipped and touches no device state ("absent" when no cascade is
