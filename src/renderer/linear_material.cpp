...
struct PaletteUse {
    unsigned operand, source_constant, swizzle;
};
...
constexpr Word palette_color_bits[2][6][3] = {
    {{0x3e189899u, 0x3edededfu, 0x3eeaeaebu},
     {0x3f3ebebfu, 0x3ecececfu, 0x3dc8c8c9u},
     {0x3f088889u, 0x3f0d8d8eu, 0x3eeaeaebu},
     {0x3ea2a2a3u, 0x3f7dfdfeu, 0x3f70f0f1u},
     {0x3f179798u, 0x3f3bbbbcu, 0x3e949495u},
     {0x3ee0e0e1u, 0x3f24a4a5u, 0x3f37b7b8u}},
    {{0x3f09898au, 0x3f179798u, 0x3f31b1b2u},
     {0x3e929293u, 0x3ec2c2c3u, 0x3ecececfu},
     {0x3edcdcddu, 0x3e6cecedu, 0x3dc8c8c9u},
     {0x00000000u, 0x00000000u, 0x00000000u},
     {0x3ed0d0d1u, 0x3f008081u, 0x3f24a4a5u},
     {0x00000000u, 0x00000000u, 0x00000000u}},
};
constexpr Word palette_linear_bits[2][6][3] = {
    {{0x3c78a190u, 0x3e244b4eu, 0x3e3877beu},
     {0x3f06005fu, 0x3e0b5d6bu, 0x3bc5f02bu},
     {0x3e806e22u, 0x3e8b0c33u, 0x3e3877beu},
     {0x3da449ffu, 0x3f7b9a80u, 0x3f600904u},
     {0x3ea1aba8u, 0x3f016406u, 0x3d86a9f9u},
     {0x3e27916bu, 0x3ec1e1dbu, 0x3ef6c272u}},
    {{0x3e828455u, 0x3ea1aba8u, 0x3ee54f54u},
     {0x3d82b15fu, 0x3df440b1u, 0x3e0b5d6bu},
     {0x3e210e33u, 0x3d239fa4u, 0x3bc5f02bu},
     {0x00000000u, 0x00000000u, 0x00000000u},
     {0x3e0e5be7u, 0x3e60c9c8u, 0x3ec1e1dbu},
     {0x00000000u, 0x00000000u, 0x00000000u}},
};
const PaletteProgram* palette_program(std::uint64_t hash) noexcept {
    for (const auto& p : palette_programs)
...
constexpr bool bump_pixel(std::uint64_t hash) noexcept {
    for (const auto& pixel : pixels)
        if (pixel.hash == hash) return pixel.bump;
    return false;
}
constexpr LinearMaterialPairContract pair_contract(std::uint64_t hash, std::uint32_t mask) noexcept {
    ...
    for (const auto& p : pixels)
        if (p.hash == hash && p.bump && p.palette_style) {
            const auto source = static_cast<std::uint8_t>(p.palette_style == 3 ? 7 : 6);
            result.scalar_transport[0] = {source, 0, 1, 3};
            result.scalar_transport_count = 1;
            if (p.palette_style == 1) {
                result.scalar_transport[1] = {source, 1, 2, 3};
                result.scalar_transport_count = 2;
            }
        }
    return result;
}
...
struct FamilyAbi {
    unsigned vertex_rgb, pixel_rgb, pixel_scratch;
...
FamilyAbi family_abi(const Pixel& pixel) noexcept {
    ...
    if (pixel.palette_style)
        return pixel.bump ? FamilyAbi{8, 7, 10, 9, 8, pixel.palette_style == 3 ? 5u : 7u, 10, 9, 8}
...
bool fade_eligible(const Pixel& pixel) noexcept {
    return pixel.asteroid_layout != 0 || pixel.hash == station_fade_ps;
}
bool fade_eligible(const Vertex& vertex) noexcept {
    return vertex.asteroid_layout != 0 || vertex.hash == station_fade_vs;
}
unsigned point_site(const Vertex& vertex) noexcept {
    return (vertex.asteroid_layout || vertex.palette_style || vertex.glass_fresnel)
               ? vertex.point
               : (vertex.loop ? 428u : 389u) + (vertex.bump ? 9u : 0u);
}
unsigned emissive_site(const Vertex& vertex) noexcept {
    return (vertex.asteroid_layout || vertex.palette_style || vertex.glass_fresnel)
               ? vertex.emissive
               : (vertex.loop ? 443u : 397u) + (vertex.bump ? 9u : 0u);
}
unsigned kind(Word token) noexcept {
    return ((token >> 28) & 7) | ((token >> 8) & 24);
}
unsigned index(Word token) noexcept {
    return token & 0x7ff;
}
unsigned mask(Word token) noexcept {
    return (token >> 16) & 15;
}
unsigned length(Word token) noexcept {
    return (token & 0xffff) == 0xfffe ? (token >> 16) & 0x7fff : (token >> 24) & 15;
}
Word reg(unsigned type, unsigned number) noexcept {
    return 0x80000000u | ((type & 7u) << 28) | ((type & 24u) << 8) | number;
}
Word dst(unsigned type, unsigned number, unsigned lanes = xyz) noexcept {
...
Word lane(unsigned type, unsigned number, unsigned component) noexcept {
    return src(type, number, component * 0x55);
}
Word bits(float value) noexcept {
    Word result;
    std::memcpy(&result, &value, sizeof result);
    return result;
}
float canonical_gain(float value) noexcept {
...
void definitions(Words& out, bool vertex, const LinearMaterialConfig& config) {
    ...
    emit(out, def,
         {dst(constant, base + 1, xyzw), bits(canonical_gain(config.direct_gain)),
...
void sanitize(Words& out, bool vertex, unsigned target, Source source, bool selective_output = false) {
    ...
    if (!vertex && kind(source.value) == constant) {
        if (source.value & relative)
            ...
            emit(out, mov, {dst(temp, target), source.value});
    ...
    if (source.value & relative)
        ...
        emit(out, max_op, {dst(temp, target), source.value, lane(constant, base, 1)});
    emit(out, min_op,
         {dst(temp, target), src(temp, target),
          lane(constant, selective_output ? 222 : base, selective_output ? 1 : 2)});
}
void transfer(Words& out, bool vertex, unsigned target, Source source, bool encode = false, unsigned pixel_scratch = 9,
              bool selective_output = false) {
    ...
    for (unsigned component = 0; component < 3; ++component)
        emit(out, pow_op,
             {dst(temp, scratch, 1u << component), lane(temp, scratch, component),
              lane(constant, encode ? base + 1 : base, encode ? 1 : 0)});
    if (vertex)
        emit(out, mul, {dst(temp, target), src(temp, scratch), src(temp, 9)});
    else
        emit(out, cmp,
             {dst(temp, target), src(temp, target, identity, 1), lane(constant, base, 1), src(temp, scratch)});
}
...
bool body_shape(unsigned op, unsigned& operands, unsigned& slots, bool& destination) noexcept {
    ...
    case 15:
    case abs_op:
    case 46: operands = 2; return true;
    case add:
    case mul:
    case 8:
    case 9:
    case min_op:
    case max_op:
    case slt: operands = 3; return true;
    case mad:
    case cmp: operands = 4; return true;
    case pow_op:
        operands = 3;
        slots = 3;
        return true;
...
bool structure(const Word* code, std::size_t words, bool vertex, Structure& result, bool original, const FamilyAbi& abi,
               unsigned original_temp_count, bool palette = false, bool relocated_rgb = false, bool xt = false) {
    ...
    for (std::size_t at = 1; at < words;) {
        ...
        if (op == dcl) {
            ...
            if (type == 10) {
                ...
                if (vertex || number >= samplers.size() || samplers[number] || (dimension != 2 && dimension != 3))
                    return false;
            ...
            if (original && type == (vertex ? output_reg : input) &&
                (reserved_varying(number, vertex, abi, relocated_rgb) ||
                 (usage == 5 && (semantic == abi.motion_texcoord || semantic == abi.depth_texcoord)) ||
                 (!relocated_rgb && usage == LinearMaterialAbi::rgb_usage &&
                  semantic == LinearMaterialAbi::rgb_usage_index)))
                return false;
            ...
            unsigned expected = 0, cost = 0;
            bool destination = false;
            if (!body_shape(op, expected, cost, destination) ||
                (!vertex && !destination && !(xt && (op == 40 || op == 41 || op == 42 || op == 43))) ||
                (vertex && (op == cmp || op == texld || op == 90)) ||
                ((vertex || original) && (op == dsx || op == dsy || op == texldd)))
                return false;
            ...
            for (unsigned offset = 1; offset <= n; ++offset) {
                ...
                if ((type == temp && number >= 32) || (type == constant && number >= (vertex ? 256u : 224u)) ||
                    ...
                    (type == color_output && number >= 4))
                    return false;
                if (original &&
                    ((type == temp && number >= original_temp_count) ||
                     (type == constant &&
                      ((number >= (vertex ? 248u : 212u) && number <= (vertex ? 249u : 213u)) ||
                       (palette && number >= (vertex ? 240u : 204u) && number <= (vertex ? 245u : 209u)))) ||
                     (type == (vertex ? output_reg : input) && reserved_varying(number, vertex, abi, relocated_rgb))))
                    return false;
                ++parameters;
                if (parameter & relative) {
                    if ((destination && offset == 1) || ++offset > n || !vertex || kind(parameter) != constant ||
                        index(parameter) > 2 || code[at + offset] != src(3, 0, 255))
                        return false;
                }
            }
            if (parameters != expected) return false;
            if (op == texld || op == texldd) {
                if (n != (op == texld ? 3u : 5u) || kind(code[at + 3]) != 10 || index(code[at + 3]) >= samplers.size())
                    return false;
                const auto dimension = samplers[index(code[at + 3])];
                if (!dimension) return false;
                if (op == texldd) {
                    if (dimension != 2) return false;
                } // explicit gradients: the 2D light map only
                else
...
bool exact(const Word* code, const Structure& s, unsigned at, unsigned opcode, Word destination,
           std::initializer_list<Word> sources) noexcept {
    if (at >= s.boundary.size() || !s.boundary[at] || code[at] != ((sources.size() + 1) << 24 | opcode) ||
        code[at + 1] != destination)
        return false;
    return std::equal(sources.begin(), sources.end(), code + at + 2);
}
bool no_write(const Word* code, const Structure& s, unsigned number, unsigned lanes, unsigned begin,
    ...
    for (const auto& instruction : s.instructions)
        if (instruction.at > begin && instruction.at < end && instruction.count && instruction.opcode != dcl &&
            ...
            index(code[instruction.at + 1]) == number && (mask(code[instruction.at + 1]) & lanes))
            return false;
    return true;
}
bool vertex_sites(const Word* code, const Structure& s, const Vertex& vertex) noexcept {
    ...
    const unsigned alpha = (vertex.asteroid_layout || vertex.palette_style || vertex.glass_fresnel)
                               ? vertex.alpha
                               : (loop ? 500 : 455) + (vertex.bump ? 27 : 0);
    const unsigned point_temp = (vertex.asteroid_layout || vertex.palette_style || vertex.glass_fresnel)
                                    ? vertex.point_temporary
                                : vertex.bump ? (loop ? 1 : 0)
                                              : (loop ? 5 : 1);
    ...
    if (loop) {
        if (!exact(code, s, point, mul, dst(temp, point_temp),
                   {lane(temp, response, 3), src(constant, 1) | relative, src(3, 0, 255)}) ||
            !exact(code, s, emissive, add, dst(output_reg, 1), {src(temp, accumulator), src(constant, 40)}))
            return false;
    } else if (!exact(code, s, point, mul, dst(temp, point_temp), {lane(temp, response, 2), src(constant, 5)}) ||
               !exact(code, s, emissive, mad, dst(output_reg, 1),
                      {src(temp, point_temp), lane(temp, response, 3), src(constant, 19)}))
        return false;
    if (!exact(code, s, alpha, mul, dst(output_reg, 1, 8),
               {lane(temp, alpha_temp, 3), lane(constant, loop ? 39 : 18, 0)}) ||
        !exact(code, s, alpha + 5, mov, dst(output_reg, 1, 8), {lane(constant, loop ? 39 : 18, 0)}))
        return false;
    unsigned writes = 0;
    for (const auto& instruction : s.instructions)
        if (instruction.count && instruction.opcode != dcl && kind(code[instruction.at + 1]) == output_reg &&
            index(code[instruction.at + 1]) == 1) {
...
bool glass_color_sites(const Word* code, const Structure& s, bool vertex, unsigned fresnel, unsigned clamp,
    ...
    for (const auto& ins : s.instructions) {
        ...
        if (ins.opcode == dcl && kind(code[at + 2]) == (vertex ? output_reg : input)) {
            ...
            if (n == (vertex ? 1u : 0u)) {
                if (code[at + 1] != (0x80000000u | 10u) ||
                    code[at + 2] != (dst(vertex ? output_reg : input, n, xyzw) | (vertex ? 0 : pp)))
                    return false;
                ++color0;
            } else if (n == (vertex ? 6u : 5u)) {
                if (code[at + 1] != (0x80000000u | 10u | (1u << 16)) ||
                    code[at + 2] != (dst(vertex ? output_reg : input, n, 1) | (vertex ? 0 : pp)))
                    return false;
                ++color1;
            }
            continue;
...
bool pixel_sites(const Word* code, const Structure& s, const Pixel& p) noexcept {
    if (p.asteroid_layout) {
        ...
        for (unsigned sampler = 0; sampler <= detail; ++sampler)
            if (!exact(code, s, p.texture[sampler], texld, dst(temp, 0, xyzw) | pp,
                       {sampler == detail ? detail_coordinate : src(input, 1), src(10, sampler)}))
                return false;
        if (!exact(code, s, p.clamp, mov, dst(temp, p.clamp_temporary) | pp | sat, {src(input, 0)}) ||
            ...
            !exact(code, s, p.final_rgb - 4, mul, dst(color_output, 0, 8) | pp,
                   {lane(temp, 0, 3), lane(input, 0, 3)}) ||
            !exact(code, s, p.texture[detail] + 4, mul, dst(temp, 2) | pp,
                   {src(temp, 0), lane(constant, p.light1 ? 4 : 2, 0)}) ||
            !exact(code, s, p.texture[0] + 4, mad, dst(temp, 0) | pp,
                   {lane(constant, p.light1 ? 5 : 3, 0), src(temp, 0), src(temp, 2)}) ||
            !no_write(code, s, 0, 8, p.texture[0], p.final_rgb - 4) ||
            !no_write(code, s, 2, xyz, p.texture[detail] + 4, p.texture[0] + 4))
            return false;
        ...
        for (const auto& instruction : s.instructions) {
            ...
            if (std::find(p.rgb.begin(), p.rgb.end(), at) != p.rgb.end() &&
                (mask(code[at + 1]) != xyz || !(code[at + 1] & pp) ||
                 (instruction.opcode != mov && instruction.opcode != mul && instruction.opcode != add &&
                  instruction.opcode != mad)))
                return false;
            ...
            for (unsigned operand = 2; operand <= instruction.count; ++operand) {
                ...
                if (found == p.color_source.end() || value != src(constant, index(value)) ||
                    index(value) != (found - p.color_source.begin() < 2 ? p.light0 : p.light1))
                    return false;
                ++directional;
            }
        }
        for (unsigned at : p.rgb)
            if (at && (at >= s.boundary.size() || !s.boundary[at])) return false;
        ...
        return p.texture[specular] != 0 && outputs == 2 && textures == detail + 1 &&
               directional == (p.light1 ? 4u : 1u);
    }
    if (p.glass_fresnel) {
        for (unsigned sampler = 0; sampler < 3; ++sampler)
            if (!exact(code, s, p.texture[sampler], texld, dst(temp, 0, xyzw) | pp,
                       {src(input, sampler == 2 ? 4 : 1), src(10, sampler)}))
                return false;
        if (!exact(code, s, p.clamp, mov, dst(temp, 1) | pp | sat, {src(input, 0)}) ||
            ...
            !exact(code, s, p.final_rgb + 5, mul, dst(color_output, 0, 8) | pp,
                   {lane(temp, 0, 3), lane(input, 0, 3)}) ||
            !no_write(code, s, 0, 8, p.texture[0], p.final_rgb + 5))
            return false;
        ...
        for (const auto& ins : s.instructions) {
            ...
            if (std::find(p.rgb.begin(), p.rgb.end(), at) != p.rgb.end() &&
                (mask(code[at + 1]) != xyz || !(code[at + 1] & pp) ||
                 (ins.opcode != mov && ins.opcode != add && ins.opcode != mul && ins.opcode != mad)))
                return false;
    ...
    for (unsigned sampler = 0; sampler < (p.bump ? 5u : 4u); ++sampler) {
        ...
        const Word coordinate = p.bump && sampler == cube ? src(temp, 0)
                                                          : src(input, !p.bump && sampler == cube ? 4 : 1);
        if (!exact(code, s, p.texture[sampler], texld, dst(temp, target, xyzw) | pp, {coordinate, src(10, sampler)}))
            return false;
    }
    if (!exact(code, s, p.clamp, mov, dst(temp, p.clamp_temporary) | pp | sat, {src(input, 0)}) ||
        ...
        !exact(code, s, p.texture[lightmap] + 4, 18, dst(temp, 2, 8) | pp,
               {lane(constant, p.affine_end ? 3 : 0, 0), lane(temp, 0, 3), lane(temp, 1, 3)}))
        return false;
    if (!no_write(code, s, 1, 8, p.texture[0], p.texture[lightmap] + 4) ||
        !no_write(code, s, 0, 8, p.texture[lightmap], p.texture[lightmap] + 4) ||
        !no_write(code, s, 2, 8, p.texture[lightmap] + 4, p.final_rgb + 4))
        return false;
    if (p.affine_end)
        for (unsigned lane_index = 0; lane_index < 3; ++lane_index)
            if (!exact(code, s, p.affine_end - 8 + lane_index * 4, 9, dst(temp, albedo, 1u << lane_index) | pp,
                       {src(temp, affine_source), src(constant, lane_index)}))
                return false;
    unsigned outputs = 0, textures = 0, directional = 0;
    for (const auto& instruction : s.instructions) {
...
void palette_definitions(Words& out, const PaletteProgram& p, bool vertex, unsigned style) {
    ...
    for (unsigned role = 0; role < p.sources.size(); ++role)
        if (p.sources[role].operand)
            emit(out, def,
                 {dst(constant, (vertex ? 240u : 204u) + role, xyzw), colors[role][0], colors[role][1], colors[role][2],
                  0});
}
bool palette_sites(const Word* code, const Structure& s, const PaletteProgram& p, bool vertex, bool bump,
                   unsigned style) noexcept {
    const auto& colors = palette_color_bits[style == 3 ? 1 : 0];
    for (unsigned role = 0; role < p.sources.size(); ++role) {
        const auto& use = p.sources[role];
        if (!use.operand) continue;
        ...
        for (const auto& ins : s.instructions) {
            ...
            if ((ins.opcode == mul || ins.opcode == mad) && use.operand >= ins.at + 2 &&
                use.operand <= ins.at + ins.count) {
                if (mask(code[ins.at + 1]) != xyz ||
                    code[use.operand] != src(constant, use.source_constant, use.swizzle))
                    return false;
                source = true;
            }
        }
    ...
    unsigned scalar_declarations = 0, view_declarations = 0, normal_declarations = 0, palette_declarations = 0,
             scalar_uses = 0;
    for (const auto& ins : s.instructions) {
        if (ins.opcode == dcl && kind(code[ins.at + 2]) == (vertex ? output_reg : input)) {
            ...
            if (bump && number == (vertex ? 8u : 7u)) {
                if (code[ins.at + 1] != (0x80000005u | ((style == 3 ? 7u : 6u) << 16)) ||
                    code[ins.at + 2] !=
                        (dst(vertex ? output_reg : input, number, style == 1 ? 3u : 1u) | (vertex ? 0 : pp)))
                    return false;
                ++scalar_declarations;
            }
            if (bump && (number == (vertex ? 3u : 2u) || (style == 1 && number == (vertex ? 4u : 3u)))) {
                ...
                if (code[ins.at + 1] != (0x80000005u | (semantic << 16)) ||
                    code[ins.at + 2] != (dst(vertex ? output_reg : input, number) | (vertex ? 0 : pp)))
                    return false;
                if (semantic == 1)
                    ++view_declarations;
                else
            ...
            if (!vertex && style == 2 && number == (bump ? 6u : 5u)) {
                if (code[ins.at + 1] != (0x80000005u | ((bump ? 5u : 4u) << 16)) ||
                    code[ins.at + 2] != (dst(input, number) | pp))
                    return false;
            ...
            for (unsigned operand = start; operand <= stop; ++operand) {
                ...
                if (kind(code[at]) == (vertex ? output_reg : input) && index(code[at]) == (vertex ? 8u : 7u)) {
                    ...
                    if (vertex) {
                        if (scalar == 1) {
                            if (!exact(code, s, static_cast<unsigned>(ins.at), 14, dst(output_reg, 8, 2),
                                       {lane(temp, 2, 2)}))
                                return false;
                        } else if (style != 3) {
                            if (!exact(code, s, static_cast<unsigned>(ins.at), add, dst(output_reg, 8, 1),
                                       {lane(temp, 2, 3), lane(temp, 2, 3)}))
                                return false;
                        } else if (!exact(code, s, static_cast<unsigned>(ins.at), add, dst(output_reg, 8, 1),
                                          {lane(temp, 2, 3), lane(constant, 43, 0)}) &&
                                   !exact(code, s, static_cast<unsigned>(ins.at), add, dst(output_reg, 8, 1),
                                          {lane(temp, 2, 3), lane(constant, 21, 3)}))
                            return false;
                    } else if (code[at] != lane(input, 7, scalar) || mask(code[ins.at + 1]) != xyz ||
                               (ins.opcode != mul && ins.opcode != mad))
                        return false;
                    ++scalar_uses;
                }
            }
    ...
    return (!bump || (scalar_declarations == 1 && view_declarations == 1 &&
                      normal_declarations == (style == 1 ? 1u : 0u) && scalar_uses == (style == 1 ? 2u : 1u))) &&
...
void constant_uses(const Word* code, const Structure& s, unsigned number, unsigned& definitions,
                   unsigned& reads) noexcept {
    definitions = 0;
    reads = 0;
    for (const auto& instruction : s.instructions) {
        ...
        for (unsigned operand = 1; operand <= instruction.count; ++operand)
            if (kind(code[instruction.at + operand]) == constant && index(code[instruction.at + operand]) == number)
                ++reads;
    }
}
...
bool fill_constant_free(const Word* code, const Structure& s) noexcept {
    return constant_free(code, s, fill_constant);
}
...
bool reads_rgb(Word operand, unsigned reg) noexcept {
    ...
    for (unsigned c = 0; c < 4; ++c)
        if (((swizzle >> (2 * c)) & 3) < 3) return true;
    return false;
}
...
//   mad_sat rU.w, rT.x, c210.x, -c203.y       (1)  g = saturate(rho^2 / K - 1) = saturate(K tau^2 - 1): the
//   minification
...
constexpr unsigned lightmap_widen_words = 117, lightmap_widen_fetch_offset = 41, lightmap_widen_before = 11,
                   lightmap_widen_after = 16;
constexpr unsigned lightmap_widen_constant = 210, lightmap_widen_luma_constant = 211,
                   lightmap_widen_gate_constant = 203;
constexpr float lightmap_widen_floor_scale = 32.0f; // b = saturate(32 L_k - 1)
constexpr unsigned swizzle_xyxy = 0x44, swizzle_xyyy = 0x54, swizzle_zwww = 0xfe, swizzle_yzyz = 0x99,
                   swizzle_xzxx = 0x08, swizzle_ywxx = 0x0d;
constexpr float lightmap_widen_epsilon = 0x1p-8f, lightmap_widen_gate_scale = 1.0f / 0.15f,
                lightmap_widen_gate_offset = 9.0f;
Word lightmap_widen_operand() noexcept {
    return src(constant, lightmap_dynamic_constant, swizzle_yzyz);
}
...
bool lightmap_widen_coordinate(Word operand) noexcept {
    return kind(operand) == input && ((operand >> 16) & 0xff) == identity && ((operand >> 24) & 0xf) == 0 &&
           !(operand & relative);
}
void lightmap_widen_definitions(Words& out, const HullLightmapWiden& w) {
    emit(out, def,
         {dst(constant, lightmap_widen_constant, xyzw), bits(1.0f / w.k), bits(lightmap_widen_epsilon),
          bits(w.b - 1.0f), bits(lightmap_widen_gate_scale)});
    emit(out, def,
         {dst(constant, lightmap_widen_luma_constant, xyzw), bits(0.2126f), bits(0.7152f), bits(0.0722f),
          bits(lightmap_widen_gate_offset)});
    emit(out, def,
         {dst(constant, lightmap_widen_gate_constant, xyzw), bits(lightmap_widen_floor_scale), bits(1.0f), bits(0.0f),
          bits(0.0f)});
}
...
void lightmap_widen_fetch(Words& out, const Word* fetch, unsigned g) {
    ...
    emit(out, mad,
         {dst(temp, u, 8u) | sat, lane(temp, t, 0), lane(constant, lightmap_widen_constant, 0),
          src(constant, lightmap_widen_gate_constant, 0x55, 1)});
    ...
    emit(out, mad,
         {dst(temp, t, 2u) | sat, lane(temp, t, 0), lane(constant, lightmap_widen_gate_constant, 0),
          src(constant, lightmap_widen_gate_constant, 0x55, 1)});
    ...
    emit(out, mad,
         {dst(temp, t, 1u) | sat, lane(temp, t, 0), lane(constant, lightmap_widen_constant, 3),
          src(constant, lightmap_widen_luma_constant, 0xff, 1)});
...
void lightmap_widen_patch(Word* block, unsigned g) {
    ...
    const Word fetch_words[4] = {0, block[lightmap_widen_fetch_offset + 1], block[lightmap_widen_fetch_offset + 2],
                                 block[lightmap_widen_fetch_offset + 3]};
    Words expected;
...
int lightmap_widen_site(const Word* code, const Structure& s, std::size_t i, unsigned stage) noexcept {
    ...
    if (!lightmap_widen_coordinate(coordinate) || kind(sampler) != 10 || index(sampler) != stage ||
        (sampler & relative))
        return -1;
    ...
    if (first.at != fetch.at - lightmap_widen_fetch_offset ||
        last.at + last.count + 1 != first.at + lightmap_widen_words)
        return -1;
    ...
    lightmap_widen_fetch(expected, fetch_words, g);
    for (unsigned q = 0; q < lightmap_widen_words; ++q)
        if (block[q] != expected[q]) return -1;
    return int(g);
}
...
int lightmap_term(const Word* code, const Structure& s, std::size_t site, std::size_t final_rgb, unsigned stage,
    ...
    if (widen) {
        const int g = lightmap_widen_site(code, s, i, stage);
        if (g < 0) return -1;
        *widened = g;
    } else if (fetch.opcode != texld || fetch.count != 3)
        return -1;
    if (kind(target) != temp || (target & ~pp) != dst(temp, index(target), xyzw) || kind(code[fetch.at + 3]) != 10 ||
        index(code[fetch.at + 3]) != stage || (code[fetch.at + 3] & relative))
        return -1;
    const unsigned reg = index(target);
    if (widen && (unsigned(*widened) == reg || unsigned(*widened) + 1 == reg || unsigned(*widened) + 2 == reg))
        return -1;
    ...
    for (const auto& in : s.instructions)
        if (in.opcode == dcl && kind(code[in.at + 2]) == 10 && index(code[in.at + 2]) == stage)
            two_dimensional = ((code[in.at + 1] >> 27) & 15) == 2;
    ...
    if (gained) {
        ...
        if (gain.opcode != mul || gain.count != 3 || code[gain.at + 1] != dst(temp, reg) ||
            code[gain.at + 2] != src(temp, reg) || code[gain.at + 3] != lightmap_gain_operand(dynamic))
            return -1;
    }
    ...
    for (++i; i < s.instructions.size(); ++i) {
        ...
        if (final) {
            for (unsigned q = 1; q <= in.count; ++q)
                if (reads_rgb(code[in.at + q], reg)) return -1;
            continue;
        }
        ...
        if (in.at == final_rgb) {
            if ((in.opcode != add && in.opcode != mad) || (code[in.at + 1] & ~pp) != final_destination || reads != 1)
                return -1;
            bool operand = false;
            for (unsigned q = 2; q <= in.count; ++q)
                if (code[in.at + q] == src(temp, reg)) operand = true;
            if (!operand) return -1;
            final = true;
            continue;
        }
        if (reads) {
            if (terra || in.opcode != mad || !exact || reads != 1 || (code[in.at + 1] & ~pp) != dst(temp, reg))
                return -1;
            terra = true;
            continue;
        }
...
bool motion_insertions(const Word* original, std::size_t words, const Words& motion, const MotionOutputProfile& row,
                       bool vertex, bool depth, const Structure& s, std::vector<Insertion>& insertions) {
    ...
    if (vertex) {
        ...
        const std::size_t declarations = 3 + (depth ? 3 : 0),
                          definitions_count = material_motion_pixel_definition_words(depth);
        ...
        insertions.push_back({row.pixel_declaration_insert_dword,
                              row.pixel_declaration_insert_dword + definitions_count,
    ...
    for (const auto& insertion : insertions) {
        if (insertion.at >= words || !s.boundary[insertion.at] || insertion.at < original_at ||
            insertion.begin != motion_at + insertion.at - original_at || insertion.end <= insertion.begin ||
            insertion.end > motion.size())
            return false;
        if (!std::equal(original + original_at, original + insertion.at, motion.begin() + motion_at)) return false;
        original_at = insertion.at;
        motion_at = insertion.end;
    }
...
const Pixel* pixel_for(std::uint64_t hash, std::size_t words) noexcept {
    for (const auto& p : pixels)
        if (p.hash == hash && p.words == words) return &p;
    return nullptr;
}
const Vertex* vertex_for(std::uint64_t hash, std::size_t words) noexcept {
    for (const auto& v : vertices)
        if (v.hash == hash && v.words == words) return &v;
    return nullptr;
}
const MotionOutputProfile* selected_row(bool vertex, std::uint64_t hash) noexcept {
    if (vertex) {
        for (const auto& pixel : pixels)
            if (linear_material_pair_reviewed(hash, pixel.hash)) return material_motion_profile(hash, pixel.hash);
    } else {
        for (const auto& v : vertices)
            if (linear_material_pair_reviewed(v.hash, hash)) return material_motion_profile(v.hash, hash);
    }
    return nullptr;
}
...
LinearMaterialResult transform(const Word* original, std::size_t words, const LinearMaterialConfig& config,
                               Words& output, bool current_depth, bool vertex, bool fade = false,
                               bool* fill_applied = nullptr, bool* sun_extracted = nullptr) noexcept {
    ...
    if (fade && !(vertex ? fade_eligible(*v) : fade_eligible(*p))) return LinearMaterialResult::UnsupportedShader;
    ...
    if (!row || !row_pixel || row_pixel->bump != bump ||
        (vertex && (row_pixel->asteroid_layout != v->asteroid_layout || row_pixel->palette_style != v->palette_style ||
                    bool(row_pixel->glass_fresnel) != bool(v->glass_fresnel))))
    ...
    if (row->transformation_class != ((bump || row_pixel->palette_style || row_pixel->glass_fresnel)
                                          ? MotionOutputClass::RelocatedRegisters
                                          : MotionOutputClass::ReferenceRegisters) ||
        row->vertex_output_register != abi.vertex_motion || row->pixel_input_register != abi.pixel_motion ||
        row->pixel_temporary_base != temporal_temporary_base(*row_pixel) || row->vertex_constant_base != 252 ||
        row->pixel_constant_base != 216 || row->pixel_output_register != 1 ||
    ...
    try {
        ...
        if (!structure(original, words, vertex, original_structure, true, abi, original_temp_count,
                       row_pixel->palette_style != 0,
                       (bump && row_pixel->palette_style != 0) || row_pixel->glass_fresnel != 0) ||
            !(vertex ? vertex_sites(original, original_structure, *v)
                     : pixel_sites(original, original_structure, *p)) ||
            (row_pixel->glass_fresnel &&
             !glass_color_sites(original, original_structure, vertex, vertex ? v->glass_fresnel : p->glass_fresnel,
                                vertex ? 0 : p->clamp, vertex ? 0 : p->final_rgb + 5)) ||
            (row_pixel->palette_style &&
             !palette_sites(original, original_structure, *palette, vertex, bump, row_pixel->palette_style)))
            return LinearMaterialResult::ProfileMismatch;
        ...
        unsigned fill_sum = 0, fill_at = 0;
        bool fill = false;
        if (!vertex && config.fill > 0.0f) {
            const unsigned albedo = (p->asteroid_layout || p->glass_fresnel) ? 0u
                                    : p->palette_style ? (p->affine_end ? (p->bump ? 1u : 4u) : 1u)
                                                       : (p->affine_end ? (p->bump ? 4u : 3u) : 1u);
        ...
        if (config.selective_exposure && !vertex && config.fill > 0.f && !fill)
            return LinearMaterialResult::ProfileMismatch;
        Words motion;
        const auto motion_result = vertex ? material_motion_vertex_variant_for(*row, original, words, motion,
                                                                               current_depth)
                                          : material_motion_pixel_variant_for(*row, original, words, motion,
                                                                              current_depth);
        if (motion_result != MaterialMotionResult::Applied)
            return motion_result == MaterialMotionResult::AllocationFailure ? LinearMaterialResult::AllocationFailure
                                                                            : LinearMaterialResult::ProfileMismatch;
        if (config.selective_exposure && !exposure_motion_reservations(motion, vertex))
            return LinearMaterialResult::ProfileMismatch;
        const bool depth = vertex ? material_motion_vertex_exports_depth(*row, current_depth)
                                  : material_motion_pixel_writes_depth(*row, current_depth);
        std::vector<Insertion> insertions;
        if (!motion_insertions(original, words, motion, *row, vertex, depth, original_structure, insertions))
            return LinearMaterialResult::ProfileMismatch;
        ...
        for (std::size_t at = 1; at < words;) {
            ...
            const auto declaration_at = vertex ? row->vertex_declaration_insert_dword
                                               : row->pixel_declaration_insert_dword;
            if (at == declaration_at) {
                ...
                if (!row_pixel->glass_fresnel)
                    emit(combined, dcl,
                         {0x80000000u | LinearMaterialAbi::rgb_usage | (LinearMaterialAbi::rgb_usage_index << 16),
                          dst(vertex ? output_reg : input, vertex ? abi.vertex_rgb : abi.pixel_rgb)});
                if (!vertex) {
                    transfer(combined, false, 12, {src(constant, p->light0)}, false, abi.pixel_scratch);
                    gain(combined, false, 12, 0);
                    if (p->light1) {
                        transfer(combined, false, 13, {src(constant, p->light1)}, false, abi.pixel_scratch);
                        gain(combined, false, 13, 0);
                    }
                }
            }
            if (original[at] == end_token) {
                if (config.selective_exposure && !vertex && !fade) exposure_invalid_sun(combined);
                combined.push_back(end_token);
                ++at;
                continue;
            }
            const unsigned op = original[at] & 0xffff, n = length(original[at]);
            if (vertex && at == point_site(*v)) {
                transfer(combined, true, 7, {original[at + 3], v->loop ? original[at + 4] : 0});
                gain(combined, true, 7, 0);
            }
            if (vertex && at == emissive_site(*v)) {
                ...
                emit(combined, abs_op, {dst(temp, 7), src(temp, 7)});
                gain(combined, true, 7, 1);
            }
            ...
            if (palette && bump && op == dcl && kind(original[at + 2]) == (vertex ? output_reg : input) &&
                index(original[at + 2]) == (vertex ? 8u : 7u)) {
                at += n + 1;
                continue;
            }
            ...
            if (palette && op != 0xfffe) {
                if (op == dcl && kind(original[at + 2]) == (vertex ? output_reg : input)) {
                    ...
                    if (bump && (number == (vertex ? 3u : 2u) ||
                                 (row_pixel->palette_style == 1 && number == (vertex ? 4u : 3u))))
                    ...
                    if (!vertex && row_pixel->palette_style == 2 && number == (bump ? 6u : 5u))
                        combined[copied + 2] &= ~pp;
                }
                for (unsigned role = 0; role < palette->sources.size(); ++role) {
                    ...
                    if (operand > at && operand <= at + n)
                        combined[copied + operand - at] = src(constant, (vertex ? 240u : 204u) + role);
                }
                for (unsigned scalar = 0; scalar < 2; ++scalar) {
                    const auto operand = palette->scalar_operand[scalar];
                    if (operand > at && operand <= at + n)
                        combined[copied + operand - at] = vertex ? dst(output_reg, 3 + scalar, 8)
                                                                 : lane(input, 2 + scalar, 3);
                }
            }
            ...
            if (vertex) {
                ...
                if (at == point_site(*v)) {
                    ...
                    if (v->loop) {
                        combined.erase(combined.begin() + copied + 4);
                        combined[copied] = (3u << 24) | mul;
                    }
                }
                ...
                if (p->glass_fresnel) {
                    ...
                    if (at == (p->affine_end ? p->affine_end : p->texture[0])) {
                        const unsigned albedo = p->palette_style ? (p->affine_end ? (p->bump ? 1 : 4) : 1)
                                                : p->affine_end  ? (p->bump ? 4 : 3)
                                                                 : 1;
                        transfer(combined, false, albedo, {src(temp, albedo)}, false, abi.pixel_scratch);
                    }
                    if (at == p->texture[p->bump ? 4 : 3])
                        transfer(combined, false, 0, {src(temp, 0)}, false, abi.pixel_scratch);
                    if (at == p->texture[p->bump ? 3 : 2]) {
                        transfer(combined, false, 0, {src(temp, 0)}, false, abi.pixel_scratch);
                        gain(combined, false, 0, 2);
                    }
                }
                if (at == p->final_rgb) {
        ...
        if (!structure(combined.data(), combined.size(), vertex, final_structure, false, abi, original_temp_count))
        ...
        if (sun_extracted && !config.selective_exposure) {
            ...
            if (!sun_share_variant(original, original_structure, combined, final_structure, sun_sites, seeds,
                                   p->final_rgb, sun_proved) ||
        ...
        if (fade) {
            ...
            for (const auto& instruction : original_structure.instructions) {
                ...
                if (!vertex && kind(original[at + 1]) == color_output && (mask(original[at + 1]) & 8)) {
                    ...
                    if (op != mul || mask(original[at + 1]) != 8 || index(original[at + 1]) != 0 ||
                        !(original[at + 1] & pp))
                        return LinearMaterialResult::ProfileMismatch;
        ...
        return LinearMaterialResult::AllocationFailure;
    }
}
} // namespace
...
bool linear_material_fill_sum(const Word* code, std::size_t words, unsigned albedo_register, unsigned& sum_register,
                              unsigned& instruction_dword) noexcept {
    ...
    const auto whole = [&](Word operand) { return kind(operand) == temp && operand == src(temp, index(operand)); };
    ...
    for (std::size_t at = 1; at < words;) {
        ...
        if (op != 0xfffeu && (op == mul || op == mad) && n >= 3 &&
            ...
            sum_register = index(code[at + 2]);
            instruction_dword = static_cast<unsigned>(at);
            ++found;
        }
        at += n + 1;
    }
...
LinearMaterialPairContract linear_material_pair_contract(std::uint64_t vertex, std::uint64_t pixel) noexcept {
    if (const auto* p = xt_pixel(pixel); p && vertex == (p->bump ? xt_bump_vs : xt_default_vs)) {
        ...
        if (p->bump) {
            result.scalar_transport[0] = {6, 0, 1, 3};
            result.scalar_transport[1] = {6, 1, 2, 3};
            result.scalar_transport_count = 2;
        }
        return result;
    }
...
bool linear_material_asteroid_pair(std::uint64_t vertex, std::uint64_t pixel) noexcept {
    bool listed = false;
    for (const auto& pair : pairs)
        if (pair.vertex == vertex && pair.pixel == pixel) {
            listed = true;
            break;
        }
    if (!listed) return false;
    for (const auto& p : pixels)
        if (p.hash == pixel) return p.asteroid_layout != 0;
    return false;
}
LinearMaterialResult linear_material_vertex_variant(const Word* original, std::size_t words,
                                                    const LinearMaterialConfig& config, Words& output,
                                                    bool current_depth) noexcept {
    if (original && words == 768 && material_motion_fingerprint(original, words) == xt_bump_vs)
        return xt_transform(original, words, config, output, current_depth, true, *xt_pixel(0x5f82ecacd39529cdull),
                            true);
...
LinearMaterialResult linear_material_pixel_variant_fill(const Word* original, std::size_t words,
    ...
    fill_applied = false;
    if (original && words >= 1561 && words <= 1791 &&
        std::any_of(std::begin(xt_pixels), std::end(xt_pixels), [&](const XtPixel& p) { return p.words == words; }))
        if (const auto* p = xt_pixel(material_motion_fingerprint(original, words)))
...
LinearMaterialResult linear_material_pixel_variant_sun_share(const Word* original, std::size_t words,
                                                             const LinearMaterialConfig& config, Words& output,
                                                             bool current_depth, bool& extraction_applied) noexcept {
    extraction_applied = false;
    if (original && words >= 1561 && words <= 1791)
        if (const auto* p = xt_pixel(material_motion_fingerprint(original, words)))
            return xt_transform(original, words, config, output, current_depth, false, *p, true, nullptr,
                                &extraction_applied);
...
bool linear_material_xt_default_pair(std::uint64_t vertex, std::uint64_t pixel) noexcept {
    const auto* p = xt_pixel(pixel);
    return vertex == xt_default_vs && p && !p->bump;
}
LinearMaterialResult linear_material_xt_default_vertex_variant(const Word* original, std::size_t words,
                                                               const LinearMaterialConfig& config, Words& output,
                                                               bool current_depth, bool linear) noexcept {
...
LinearMaterialResult linear_material_xt_default_pixel_variant(const Word* original, std::size_t words,
    ...
    return !p || p->bump ? LinearMaterialResult::UnsupportedShader
                         : xt_transform(original, words, config, output, current_depth, false, *p, linear);
}
    ...
    for (const auto& i : s.instructions) {
        ...
        for (unsigned n = 1; n <= last; ++n) {
            ...
            if ((t == temp && r >= original_share_total && r <= sun_final) ||
                (t == constant && (r == original_share_domain || r == sun_constant)))
                return false;
        }
    }
    return true;
...
bool original_sun_plan(const Word* original, const Structure& source, const std::array<unsigned, 2>& seeds,
                       unsigned final_rgb, unsigned fill_site, unsigned fill_sum, std::vector<SunEdit>& edits,
                       Word& final_destination) {
    SunState state{};
    std::vector<SunBranch> branches;
    unsigned seeded = 0, expected = 0, finals = 0;
    bool initialized = false, fill_tracked = fill_site == 0;
    ...
    for (const auto& i : source.instructions) {
        ...
        SunEdit edit{i.at, {}};
        if (!initialized) {
            for (unsigned r = sun_base; r <= sun_final; ++r)
                emit(edit.words, mov, {dst(temp, r), lane(constant, original_share_domain, 1)});
            initialized = true;
        ...
        if (!destination) {
            for (unsigned q = 1; q <= i.count; ++q)
                if (sun_read(original[i.at + q], xyzw, state)) return false;
        ...
        const unsigned consumed = (i.opcode == mov || i.opcode == add || i.opcode == mul || i.opcode == mad) ? lanes
                                                                                                             : xyzw;
        ...
        for (const auto seed : seeds)
            if (seed > i.at && seed <= i.at + i.count) {
                if (seed_operand) return false;
                seed_operand = static_cast<unsigned>(seed - i.at);
                ++seeded;
            }
        ...
        if (active) {
            ...
            if (lanes != xyz || (od & sat) ||
                ...
                (i.opcode != mov && i.opcode != add && i.opcode != mul && i.opcode != mad))
                return false;
            std::array<Word, 3> parallel{lane(constant, original_share_domain, 1),
                                         lane(constant, original_share_domain, 1),
                                         lane(constant, original_share_domain, 1)};
            for (unsigned q = 2; q <= i.count; ++q)
                if (dep[q - 2]) {
            ...
            if (seed_operand) {
                ...
                if (i.opcode != mad || (seed_operand != 2 && seed_operand != 3) || dep[0] || dep[1] ||
                    kind(sun) != constant || (sun & relative) || index(sun) >= LinearMaterialAbi::pixel_definition_base)
                    return false;
                if (dep[2])
                    emit(edit.words, mad, {output, original[i.at + 2], original[i.at + 3], parallel[2]});
                else
                    emit(edit.words, mul, {output, original[i.at + 2], original[i.at + 3]});
            } else if (i.opcode == mov)
                emit(edit.words, mov, {output, parallel[0]});
            else if (i.opcode == add) {
                if (dep[0] && dep[1])
                    emit(edit.words, add, {output, parallel[0], parallel[1]});
                else
                    emit(edit.words, mov, {output, parallel[dep[0] ? 0 : 1]});
            } else {
                if (dep[0] && dep[1]) return false; // nonlinear in sun
                if (dep[0] || dep[1]) {
                    const Word a = dep[0] ? parallel[0] : original[i.at + 2],
                               b = dep[1] ? parallel[1] : original[i.at + 3];
                    if (i.opcode == mad && dep[2])
                        emit(edit.words, mad, {output, a, b, parallel[2]});
                    else
                        emit(edit.words, mul, {output, a, b});
                } else
                    emit(edit.words, mov, {output, parallel[2]});
            }
            if (!final) state[target] = xyz;
        } else if (kind(od) == temp && target < state.size()) {
            const unsigned killed = state[target] & lanes;
            if (killed)
                emit(edit.words, mov, {dst(temp, sun_base + target, killed), lane(constant, original_share_domain, 1)});
            state[target] &= ~lanes;
        }
        if (!edit.words.empty()) edits.push_back(std::move(edit));
        if (final) {
...
// gained program.
LinearMaterialResult original_fill_transform(const Word* original, std::size_t words, float fill, Words& output,
    ...
    if (!std::isfinite(lightmap_gain) || lightmap_gain < 1.0f || lightmap_gain > 8.0f)
        return LinearMaterialResult::InvalidConfig;
    ...
    if (words >= 1561) {
        xt = xt_pixel(hash);
        if (xt && xt->words != words) xt = nullptr;
    }
    const Pixel* p = xt ? nullptr : pixel_for(hash, words);
    if (!xt && !p) return LinearMaterialResult::UnsupportedShader;
    const MotionOutputProfile* row = xt ? material_motion_profile(xt->bump ? xt_bump_vs : xt_default_vs, xt->hash)
                                        : selected_row(false, hash);
    if (!row || row->pixel_fingerprint != hash || row->pixel_dword_count != words || row->pixel_constant_base != 216 ||
        row->pixel_output_register != 1 || !row->depth_output)
        return LinearMaterialResult::ProfileMismatch;
    ...
    try {
        Structure s;
        const bool proved = xt ? structure(original, words, false, s, true, abi, temp_count, false, xt->bump, true) &&
                                     xt_pixel_sites(original, s, *xt)
                               : structure(original, words, false, s, true, abi, temp_count, p->palette_style != 0,
                                           (p->bump && p->palette_style != 0) || p->glass_fresnel != 0) &&
                                     pixel_sites(original, s, *p);
        if (!proved) return LinearMaterialResult::ProfileMismatch;
        unsigned sum = 0, site = 0, light = 0;
        bool fill_site = false;
        if (xt) {
            ...
            const unsigned albedo = (p->asteroid_layout || p->glass_fresnel) ? 0u
                                    : p->palette_style ? (p->affine_end ? (p->bump ? 1u : 4u) : 1u)
                                                       : (p->affine_end ? (p->bump ? 4u : 3u) : 1u);
            fill_site = linear_material_fill_sum(original, words, albedo, sum, site) && sum < temp_count &&
                        sum != albedo &&
        ...
        unsigned lightmap_stage = 0, lightmap_site = 0, lightmap_reg = 0;
        bool lightmap_on = false;
        if (lightmap_gain != 1.0f) {
            ...
            const int reg = lightmap_term(original, s, lightmap_site, final_rgb, lightmap_stage, false,
                                          dst(color_output, 0));
            ...
            if (lightmap_on && widen &&
                !(lightmap_widen_coordinate(original[lightmap_site + 2]) &&
                  constant_free(original, s, lightmap_dynamic_constant) &&
                  constant_free(original, s, lightmap_widen_constant) &&
                  constant_free(original, s, lightmap_widen_luma_constant) &&
        ...
        if (mr != MaterialMotionResult::Applied)
            return mr == MaterialMotionResult::AllocationFailure ? LinearMaterialResult::AllocationFailure
                                                                 : LinearMaterialResult::ProfileMismatch;
        const bool depth = material_motion_pixel_writes_depth(*row, current_depth);
        std::vector<Insertion> insertions;
        if (!motion_insertions(original, words, motion, *row, false, depth, s, insertions))
            return LinearMaterialResult::ProfileMismatch;
        if (share)
        ...
        const auto flush_edits = [&](std::size_t at) {
            while (edited < edits.size() && edits[edited].at == at) {
                ...
                combined.insert(combined.end(), e.words.begin(), e.words.end());
            }
        };
        for (std::size_t at = 1; at < words;) {
            ...
            while (inserted < insertions.size() && insertions[inserted].at == at) {
                const auto& i = insertions[inserted++];
                ...
                if (share) {
                    emit(combined, def,
                         {dst(constant, original_share_domain, xyzw), bits(2.2f), bits(0.0f), bits(65504.0f),
                          bits(original_share_slack)});
                    emit(combined, def,
                         {dst(constant, sun_constant, xyzw), bits(0.2126f), bits(0.7152f), bits(0.0722f),
                          bits(0x1p-20f)});
                }
                if (lightmap_on && !lightmap_dynamic)
                    emit(combined, def,
                         {dst(constant, lightmap_gain_constant, xyzw), bits(lightmap_gain), bits(0.0f), bits(0.0f),
                          bits(0.0f)});
            ...
            if (widen_on && at == lightmap_site) {
                ...
                widen_at = copy;
                lightmap_widen_fetch(combined, original + at, 0u);
        ...
        if (widen_on) {
            ...
            for (std::size_t at = 1; at < combined.size();) {
                const Word token = combined[at];
                const unsigned op = token & 0xffff, n = length(token);
                if (token == end_token) break;
                if (op == 0xfffe || n > combined.size() - at - 1) {
                    at += n + 1;
                    continue;
                }
                if (op != dcl && op != def && !(at >= widen_at && at < widen_at + lightmap_widen_words))
                    for (unsigned q = 1; q <= n; ++q)
                        if (kind(combined[at + q]) == temp) highest = std::max(highest, index(combined[at + q]) + 1u);
                at += n + 1;
            }
        ...
        if (widen_on) {
            ...
            Words block;
            const Word fetch_words[4] = {0, combined[combined_site + 1], combined[combined_site + 2],
                                         combined[combined_site + 3]};
            ...
            for (std::size_t at = 0; at < block.size();) {
                ...
                for (unsigned q = 1; q <= n; ++q)
                    if (kind(block[at + q]) == temp) {
                        const unsigned r = index(block[at + q]);
                        if (r == widen_reg) ++block_g;
                        if (r == widen_reg + 1) ++block_t;
                        if (r == widen_reg + 2) ++block_u;
                    }
                at += n + 1;
            }
            unsigned references_g = 0, references_t = 0, references_u = 0, lane_reads = 0, other_lanes = 0,
            ...
            for (const auto& in : final_structure.instructions) {
                ...
                for (unsigned q = 1; q <= in.count; ++q) {
                    ...
                    if (q >= 2 && kind(operand) == constant && index(operand) == lightmap_dynamic_constant &&
                        !(operand & relative)) {
                        const unsigned sw = (operand >> 16) & 0xff;
                        if (operand == lightmap_widen_operand())
                            ++lane_reads;
                        else if (sw != 0x00 && sw != 0xff)
                            ++other_lanes; // anything but a replicated .x or .w
                    }
                }
            }
            constant_uses(combined.data(), final_structure, lightmap_dynamic_constant, definitions, reads);
            if (references_g != block_g || references_t != block_t || references_u != block_u || lane_reads != 1 ||
                other_lanes != 0 || definitions != 0)
                return LinearMaterialResult::ProfileMismatch;
...
int linear_material_flow_control_depth(const Word* code, std::size_t words, std::size_t site) noexcept {
    ...
    for (std::size_t at = 1; at < words;) {
        ...
        if (op == 25 || op == 26 || op == 30 || op == 28)
            return -2; // call, callnz, label, ret: a subroutine before the fetch (its body is not walked)
        if (op == 27 || op == 38 || op == 40 || op == 41)
            ++depth; // loop, rep, if, ifc
        else if (op == 29 || op == 39 || op == 43) {
            if (--depth < 0) return -1;
        } // endloop, endrep, endif
        at += n + 1;
    }
    return -1;
}
LinearMaterialResult linear_material_original_fill_pixel_variant(const Word* original, std::size_t words, float fill,
                                                                 Words& output, bool current_depth,
...
LinearMaterialResult linear_material_original_sun_share_pixel_variant(
    const Word* original, std::size_t words, float fill, Words& output, bool current_depth, bool& share_applied,
    float lightmap_gain, bool* lightmap_gain_applied, bool lightmap_dynamic, const HullLightmapWiden* widen,
    ...
    return original_fill_transform(original, words, fill, output, current_depth, fill_applied, &share_applied,
                                   lightmap_gain, lightmap_gain_applied, lightmap_dynamic, widen, widen_applied);
}
LinearMaterialResult linear_material_hull_lightmap_gain_pixel_variant(const Word* original, std::size_t words,
                                                                      float fill, float gain, Words& output,
                                                                      bool current_depth, bool& fill_applied,
                                                                      bool& gain_applied, bool dynamic,
                                                                      const HullLightmapWiden* widen,
                                                                      bool* widen_applied) noexcept {
...
unsigned linear_material_hull_lightmap_stage(std::uint64_t pixel, std::size_t words) noexcept {
    if (words >= 1561) {
        if (const XtPixel* xt = xt_pixel(pixel); xt && xt->words == words) return xt->bump ? 3u : 2u;
    }
    if (const Pixel* p = pixel_for(pixel, words))
        return (p->asteroid_layout || p->glass_fresnel) ? 0u : p->bump ? 3u : 2u;
    return 0u;
}
} // namespace x3m::renderer
