...
constexpr D3DSAMPLERSTATETYPE touched_samplers[] = {D3DSAMP_MINFILTER,   D3DSAMP_MAGFILTER,    D3DSAMP_MIPFILTER,
...
constexpr DWORD sampler_values[] = {
    D3DTEXF_POINT, D3DTEXF_POINT, D3DTEXF_NONE, D3DTADDRESS_CLAMP, D3DTADDRESS_CLAMP, FALSE, 0, 0};
...
const char* hdr_tonemap_name(HdrTonemap tonemap) noexcept {
    return tonemap == HdrTonemap::Agx ? "agx" : "identity";
}
...
const char* hdr_decode_name(x3::temporal::AgxDecode decode) noexcept {
    return decode == x3::temporal::AgxDecode::srgb   ? "srgb"
           : decode == x3::temporal::AgxDecode::none ? "none"
                                                     : "gamma2.2";
}
const char* hdr_exposure_name(ExposureMode mode) noexcept {
    return mode == ExposureMode::Manual ? "manual" : "auto";
}
...
void HdrPass::shutdown() noexcept {
    ...
    drop(sharpen_shader_);
    drop(tonemap_sharpen_shader_);
    drop(quad_vs_);
...
void HdrPass::release_chain() noexcept {
    for (unsigned i = 0; i < chain_max_levels; ++i) {
        drop(chain_[i]);
        chain_width_[i] = chain_height_[i] = 0;
    }
    chain_count_ = 0;
    tile_width_ = tile_height_ = 0;
...
std::uint64_t HdrPass::chain_bytes() const noexcept {
    ...
    for (unsigned i = 0; i < chain_count_; ++i)
        bytes += std::uint64_t(chain_width_[i]) * chain_height_[i] * chain_texel_bytes_;
...
static unsigned chain_geometry(UINT width, UINT height, UINT levels_w[], UINT levels_h[], unsigned max_levels,
                               UINT* tile_w, UINT* tile_h) noexcept {
    UINT w = width, h = height;
    unsigned count = 0;
    for (;;) {
        const UINT nw = (w + 3) / 4, nh = (h + 3) / 4;
        if (nw <= kMeterTileMax && nh <= kMeterTileMax) {
            *tile_w = nw;
            *tile_h = nh;
            return count;
        }
...
HRESULT HdrPass::ensure_chain(UINT width, UINT height) noexcept {
    ...
    if (chain_ring_[0] && chain_ring_[1] && chain_readback_[0] && chain_readback_[1]) {
        ...
        for (unsigned i = 0; same && i < count; ++i)
            same = chain_[i] && chain_width_[i] == levels_w[i] && chain_height_[i] == levels_h[i];
        if (same) return S_OK;
    }
    release_chain();
    ...
    auto level = [&](UINT w, UINT h, IDirect3DSurface9** out) {
        ...
        HRESULT r = call<CreateTextureFn>(CreateTexture)(device_, w, h, 1, D3DUSAGE_RENDERTARGET, caps_.chain_format,
                                                         D3DPOOL_DEFAULT, &texture, nullptr);
        if (SUCCEEDED(r) && texture)
            r = texture->GetSurfaceLevel(0, out);
        else if (SUCCEEDED(r))
            r = E_FAIL;
        drop(texture);
        return r;
    ...
    for (unsigned i = 0; SUCCEEDED(hr) && i < count; ++i) {
        ...
        if (SUCCEEDED(hr)) {
            chain_width_[chain_count_] = levels_w[i];
            chain_height_[chain_count_] = levels_h[i];
            ++chain_count_;
        }
    }
    for (unsigned i = 0; SUCCEEDED(hr) && i < 2; ++i) {
        ...
        if (SUCCEEDED(hr))
            hr = call<CreateOffscreenFn>(CreateOffscreenPlainSurface)(device_, tile_w, tile_h, caps_.chain_format,
                                                                      D3DPOOL_SYSTEMMEM, &chain_readback_[i], nullptr);
    }
    if (SUCCEEDED(hr)) {
        tile_width_ = tile_w;
        tile_height_ = tile_h;
        ...
        if (tiles > tile_capacity_) {
            tile_mean_.reset(new (std::nothrow) float[tiles]);
            tile_max_.reset(new (std::nothrow) float[tiles]);
            tile_weight_.reset(new (std::nothrow) float[tiles]);
            tile_scratch_.reset(new (std::nothrow) TileSample[tiles]);
            if (!tile_mean_ || !tile_max_ || !tile_weight_ || !tile_scratch_) {
                tile_mean_.reset();
                tile_max_.reset();
                tile_weight_.reset();
                tile_scratch_.reset();
...
HRESULT HdrPass::ensure_target(UINT width, UINT height) noexcept {
    ...
    IDirect3DTexture9* texture = nullptr;
    HRESULT hr = fault(HdrFault::TargetCreate)
                     ? E_OUTOFMEMORY
                     : call<CreateTextureFn>(CreateTexture)(device_, width, height, 1, D3DUSAGE_RENDERTARGET,
                                                            D3DFMT_A16B16G16R16F, D3DPOOL_DEFAULT, &texture, nullptr);
    if (SUCCEEDED(hr) && texture)
        hr = texture->GetSurfaceLevel(0, &target_);
    ...
    width_ = last_width_ = width;
    height_ = last_height_ = height;
...
HRESULT HdrPass::exchange_target(IDirect3DSurface9*& candidate) noexcept {
    if (!device_ || !native_ || !enabled() || !target_ || !width_ || !height_ || !candidate) return E_INVALIDARG;
    bool equal = false;
    ...
    const auto compatible = [&](const D3DSURFACE_DESC& d) noexcept {
        return d.Type == D3DRTYPE_SURFACE && d.Format == D3DFMT_A16B16G16R16F && d.Usage == D3DUSAGE_RENDERTARGET &&
               d.Pool == D3DPOOL_DEFAULT && d.MultiSampleType == D3DMULTISAMPLE_NONE && !d.MultiSampleQuality &&
...
HRESULT HdrPass::save(SavedState& saved) noexcept {
    ...
    if (FAILED(hr = call<GetStreamFn>(GetStreamSource)(device_, 0, &saved.stream, &saved.offset, &saved.stride)))
        return hr;
    ...
    for (unsigned i = 0; i < sampler_count; ++i)
        if (FAILED(hr = call<GetSamplerFn>(GetSamplerState)(device_, 0, touched_samplers[i], &saved.samplers[i])))
            return hr;
    for (unsigned i = 0; i < 2; ++i)
    ...
    if (saved.constants_saved &&
        FAILED(hr = call<GetPsConstFn>(GetPixelShaderConstantF)(device_, 0, &saved.constants[0][0], constant_count)))
        return hr;
    return S_OK;
}
...
HRESULT HdrPass::restore(const SavedState& saved, IDirect3DSurface9* rt0) noexcept {
    ...
    if (saved.fvf)
        step(call<SetFvfFn>(SetFVF)(device_, saved.fvf));
    else
        step(call<SetDeclarationFn>(SetVertexDeclaration)(device_, saved.declaration));
    ...
    if (saved.constants_saved)
        step(call<SetPsConstFn>(SetPixelShaderConstantF)(device_, 0, &saved.constants[0][0], constant_count));
    return first;
}
...
HRESULT HdrPass::copy_draw(IDirect3DSurface9* source_target, IDirect3DTexture9* source_texture,
    ...
    *restoration = S_OK;
    ...
    for (unsigned i = 0; i < touched_count; ++i)
        step(call<SetRsFn>(SetRenderState)(device_, touched_states[i], touched_values[i]));
    ...
    if (program && program->constants)
        step(call<SetPsConstFn>(SetPixelShaderConstantF)(device_, x3::temporal::kAgxFirstRegister, program->constants,
    ...
    if (SUCCEEDED(op)) {
        ...
        if (FAILED(injected_draw))
            op = injected_draw; // fixture seam: the draw "failed" and nothing was written
        else
...
HRESULT HdrPass::meter_chain(IDirect3DTexture9* scene_texture, UINT width, UINT height, HRESULT injected) noexcept {
    if (!chain_ring_[chain_slot_] || !chain_readback_[chain_slot_] || !meter_level0_shader_ || !meter_reduce_shader_)
    ...
    for (unsigned i = 0; SUCCEEDED(op) && i <= chain_count_; ++i) {
        ...
        c.source[0] = float(sw);
        c.source[1] = float(sh);
        c.source[2] = 1.f / float(sw);
        c.source[3] = 1.f / float(sh);
        for (unsigned k = 0; k < 4; ++k) c.decode[k] = agx_.decode[k];
        c.meter[0] = config_.params.meter_floor;
        c.meter[1] = config_.params.meter_clip;
        c.output[0] = float(dw);
        c.output[1] = float(dh);
...
HRESULT HdrPass::quad(UINT width, UINT height) noexcept {
    ...
    if (quad_fvf_)
        quad_vertices_xyzrhw(width, height, vertices);
    else
...
HRESULT HdrPass::mrt_draw(IDirect3DSurface9* rt0, IDirect3DSurface9* rt1, IDirect3DSurface9* rt2,
    ...
    *restoration = S_OK;
    ...
    step(call<SetRtFn>(SetRenderTarget)(device_, 0, rt0));
    for (unsigned i = 1; i < saved.target_count; ++i)
        if (i == 1 ? rt1 != nullptr : i == 2 && rt2 != nullptr)
            step(call<SetRtFn>(SetRenderTarget)(device_, i, i == 1 ? rt1 : rt2));
    step(call<SetDepthFn>(SetDepthStencilSurface)(device_, nullptr));
    const D3DVIEWPORT9 viewport{0, 0, width, height, 0.f, 1.f};
    step(call<SetViewportFn>(SetViewport)(device_, &viewport));
    step(bind_quad_program());
...
bool HdrPass::self_test(bool with_depth, bool scene_open, char* detail, std::size_t detail_size) noexcept {
    ...
    IDirect3DSurface9* scene_copy = nullptr;
    IDirect3DSurface9* motion_copy = nullptr;
    IDirect3DSurface9* depth_copy = nullptr;
    IDirect3DSurface9* color_copy = nullptr;
    IDirect3DPixelShader9* mrt_shader = nullptr;
    IDirect3DPixelShader9* single_shader = nullptr;
    bool ok = false;
    HRESULT hr = S_OK, restore_hr = S_OK, draw = S_OK, blend = S_OK, copy = S_OK, copy_restore = S_OK, scene_hr = S_OK,
            stretch = S_FALSE;
    ...
    unsigned scene_errors = 0, sum_errors = 0, motion_errors = 0, depth_errors = 0, copy_errors = 0, stretch_errors = 0,
             tonemap_errors = 0, meter_errors = 0;
    float meter_value = 0.f, meter_max = 0.f, meter_expected = 0.f;
    const char* stage = "create";
    bool own_scene = false;
    do {
        if (FAILED(hr = call<CreateTextureFn>(CreateTexture)(device_, 4, 4, 1, D3DUSAGE_RENDERTARGET,
                                                             D3DFMT_A16B16G16R16F, D3DPOOL_DEFAULT, &scene, nullptr)))
            break;
        if (FAILED(hr = scene->GetSurfaceLevel(0, &scene_surface))) break;
        if (FAILED(hr = call<CreateTextureFn>(CreateTexture)(device_, 4, 4, 1, D3DUSAGE_RENDERTARGET,
                                                             D3DFMT_A32B32G32R32F, D3DPOOL_DEFAULT, &motion, nullptr)))
            break;
        if (FAILED(hr = motion->GetSurfaceLevel(0, &motion_surface))) break;
        if (with_depth) {
            if (FAILED(hr = call<CreateTextureFn>(CreateTexture)(device_, 4, 4, 1, D3DUSAGE_RENDERTARGET, D3DFMT_R32F,
                                                                 D3DPOOL_DEFAULT, &depth, nullptr)))
                break;
            if (FAILED(hr = depth->GetSurfaceLevel(0, &depth_surface))) break;
            if (FAILED(hr = call<CreateOffscreenFn>(CreateOffscreenPlainSurface)(
                           device_, 4, 4, D3DFMT_R32F, D3DPOOL_SYSTEMMEM, &depth_copy, nullptr)))
                break;
        }
        if (FAILED(hr = call<CreateRtFn>(CreateRenderTarget)(device_, 4, 4, D3DFMT_A8R8G8B8, D3DMULTISAMPLE_NONE, 0,
                                                             FALSE, &color, nullptr)))
            break;
        if (FAILED(hr = call<CreateOffscreenFn>(CreateOffscreenPlainSurface)(device_, 4, 4, D3DFMT_A16B16G16R16F,
                                                                             D3DPOOL_SYSTEMMEM, &scene_copy, nullptr)))
            break;
        if (FAILED(hr = call<CreateOffscreenFn>(CreateOffscreenPlainSurface)(device_, 4, 4, D3DFMT_A32B32G32R32F,
                                                                             D3DPOOL_SYSTEMMEM, &motion_copy, nullptr)))
            break;
        if (FAILED(hr = call<CreateOffscreenFn>(CreateOffscreenPlainSurface)(device_, 4, 4, D3DFMT_A8R8G8B8,
                                                                             D3DPOOL_SYSTEMMEM, &color_copy, nullptr)))
            break;
        if (FAILED(hr = call<CreatePsFn>(CreatePixelShader)(
                       device_, with_depth ? self_test_mrt_program : self_test_pair_program, &mrt_shader)))
            break;
        if (FAILED(hr = call<CreatePsFn>(CreatePixelShader)(device_, self_test_single_program, &single_shader))) break;
        stage = "scene";
        if (!scene_open) {
            if (FAILED(scene_hr = call<SceneFn>(BeginScene)(device_))) {
                hr = scene_hr;
                break;
            }
            own_scene = true;
        ...
        if (with_depth && FAILED(hr = call<GetRtDataFn>(GetRenderTargetData)(device_, depth_surface, depth_copy)))
            break;
        D3DLOCKED_RECT lock{};
        ...
        for (unsigned y = 0; y < 4; ++y)
            for (unsigned x = 0; x < 4; ++x) {
                ...
                std::memcpy(value, static_cast<const char*>(lock.pBits) + y * lock.Pitch + x * 8, 8);
        ...
        stage = "copy";
        {
            ...
            if (FAILED(hr = call<GetRtFn>(GetRenderTarget)(device_, 0, &current)) || !current) {
        ...
        if (caps_.tonemap && tonemap_shader_) {
            ...
            if (FAILED(tonemap) || tonemap_errors) {
                caps_.tonemap = false;
                caps_.tonemap_reason = "self_test";
            }
        }
        if (caps_.meter && meter_level0_shader_) {
            stage = "meter";
            IDirect3DTexture9* one = nullptr;
            IDirect3DSurface9* one_surface = nullptr;
            IDirect3DSurface9* one_copy = nullptr;
            IDirect3DSurface9* saved_ring = chain_ring_[chain_slot_];
            IDirect3DSurface9* saved_readback = chain_readback_[chain_slot_];
            IDirect3DSurface9* saved_levels[chain_max_levels];
            const unsigned saved_count = chain_count_;
            ...
            meter = call<CreateTextureFn>(CreateTexture)(device_, 1, 1, 1, D3DUSAGE_RENDERTARGET, caps_.chain_format,
                                                         D3DPOOL_DEFAULT, &one, nullptr);
            if (SUCCEEDED(meter) && one) meter = one->GetSurfaceLevel(0, &one_surface);
            if (SUCCEEDED(meter))
                meter = call<CreateOffscreenFn>(CreateOffscreenPlainSurface)(device_, 1, 1, caps_.chain_format,
                                                                             D3DPOOL_SYSTEMMEM, &one_copy, nullptr);
            if (SUCCEEDED(meter)) {
                ...
                chain_ring_[chain_slot_] = one_surface;
                chain_readback_[chain_slot_] = one_copy;
                chain_count_ = 0;
                tile_width_ = tile_height_ = 1;
                IDirect3DSurface9* current = nullptr;
                if (FAILED(meter = call<GetRtFn>(GetRenderTarget)(device_, 0, &current)) || !current) {
                    if (SUCCEEDED(meter)) meter = E_FAIL;
                } else {
                    Program program;
                    program.shader = shader_;
                    program.meter = true;
                    HRESULT meter_restore = S_OK;
                    meter = copy_draw(scene_surface, scene, color, 4, 4, current, &meter_restore, S_OK, false,
                                      &program);
                    if (SUCCEEDED(meter)) meter = program.meter_result;
                    if (SUCCEEDED(meter) && FAILED(meter_restore)) meter = meter_restore;
                    drop(current);
                }
                chain_ring_[chain_slot_] = saved_ring;
                chain_readback_[chain_slot_] = saved_readback;
                chain_count_ = saved_count;
                for (unsigned i = 0; i < chain_max_levels; ++i) chain_[i] = saved_levels[i];
                chain_pending_[chain_slot_] = saved_pending;
                tile_width_ = saved_tile_w;
                tile_height_ = saved_tile_h;
            ...
            drop(one_surface);
            drop(one);
            if (FAILED(meter) || meter_errors) {
                caps_.meter = false;
                caps_.meter_reason = "self_test";
            }
        }
        ...
        stage = "compare";
        ok = !scene_errors && !sum_errors && !motion_errors && !depth_errors && !copy_errors &&
             !fault(HdrFault::SelfTest);
    } while (false);
    if (own_scene) {
        const HRESULT end = call<SceneFn>(EndScene)(device_);
        if (FAILED(end) && SUCCEEDED(hr)) {
            hr = end;
            ok = false;
            stage = "end_scene";
        }
    ...
    std::snprintf(
        detail, detail_size,
        "stage=%s result=%08lx draw=%08lx blend=%08lx copy=%08lx restore=%08lx copy_restore=%08lx scene=%08lx stretch=%08lx scene_errors=%u sum_errors=%u motion_errors=%u depth_errors=%u copy_errors=%u stretch_errors=%u targets=%u tonemap=%08lx tonemap_errors=%u meter=%08lx meter_errors=%u meter_value=%.5f meter_max=%.5f meter_expected=%.5f",
        stage, hr, draw, blend, copy, restore_hr, copy_restore, scene_hr, stretch, scene_errors, sum_errors,
        motion_errors, depth_errors, copy_errors, stretch_errors, with_depth ? 3u : 2u, tonemap, tonemap_errors, meter,
        meter_errors, double(meter_value), double(meter_max), double(meter_expected));
    return ok;
}

bool HdrPass::recheck(bool scene_open, char* detail, std::size_t detail_size) noexcept {
    if (!caps_.enabled) {
        std::snprintf(detail, detail_size, "disabled");
        return false;
    }
    return self_test(with_depth_, scene_open, detail, detail_size);
}

void HdrPass::attach(IDirect3DDevice9* device, void* const* native, const D3DCAPS9& caps, D3DFORMAT main_format,
                     bool with_depth) noexcept {
    shutdown();
    device_ = device;
    native_ = native;
    caps9_ = caps;
    main_format_ = main_format;
    with_depth_ = with_depth;
    caps_ = HdrCaps{};
    const char* reason = "ok";
    if (!device || !native)
        reason = "device";
    else if (D3DSHADER_VERSION_MAJOR(caps.PixelShaderVersion) < 3)
        reason = "ps_version";
    else if (caps.NumSimultaneousRTs < 2)
        reason = "mrt_count";
        ...
        D3DDISPLAYMODE mode{};
        if (FAILED(call<GetDirect3DFn>(GetDirect3D)(device_, &factory)) || !factory)
            reason = "factory";
        else if (FAILED(call<GetCreationFn>(GetCreationParameters)(device_, &creation)) ||
                 FAILED(call<GetDisplayModeFn>(GetDisplayMode)(device_, 0, &mode)))
            reason = "adapter_query";
        else {
            const UINT adapter = creation.AdapterOrdinal;
            const D3DDEVTYPE type = creation.DeviceType;
            caps_.fp16_target = factory->CheckDeviceFormat(adapter, type, mode.Format, D3DUSAGE_RENDERTARGET,
                                                           D3DRTYPE_TEXTURE, D3DFMT_A16B16G16R16F);
            caps_.fp16_blending = factory->CheckDeviceFormat(
                adapter, type, mode.Format, D3DUSAGE_RENDERTARGET | D3DUSAGE_QUERY_POSTPIXELSHADER_BLENDING,
                D3DRTYPE_TEXTURE, D3DFMT_A16B16G16R16F);
            caps_.fp16_filter = factory->CheckDeviceFormat(adapter, type, mode.Format, D3DUSAGE_QUERY_FILTER,
                                                           D3DRTYPE_TEXTURE, D3DFMT_A16B16G16R16F);
            caps_.fp16_sampling = factory->CheckDeviceFormat(adapter, type, mode.Format, 0, D3DRTYPE_TEXTURE,
                                                             D3DFMT_A16B16G16R16F);
            caps_.stretch_conversion = factory->CheckDeviceFormatConversion(adapter, type, D3DFMT_A16B16G16R16F,
                                                                            main_format);
            ...
            if (FAILED(caps_.fp16_target))
                reason = "fp16_target";
            else if (FAILED(caps_.fp16_blending))
                reason = "fp16_blending";
            else if (FAILED(caps_.fp16_sampling))
                reason = "fp16_sampling";
        }
        drop(factory);
    }
    ...
    if (!std::strcmp(reason, "ok")) {
        ...
        HRESULT hr = call<CreateVsFn>(CreateVertexShader)(
            device_, reinterpret_cast<const DWORD*>(quad_vertex_program()), &quad_vs_);
        if (SUCCEEDED(hr))
            hr = call<CreateDeclarationFn>(CreateVertexDeclaration)(device_, quad_declaration, &quad_declaration_);
        if (FAILED(hr) || !quad_vs_ || !quad_declaration_) {
            reason = "quad_shader";
            std::snprintf(caps_.self_test_detail, sizeof caps_.self_test_detail, "create=%08lx", hr);
            drop(quad_vs_);
            drop(quad_declaration_);
        }
    }
    if (!std::strcmp(reason, "ok")) {
        const HRESULT hr = call<CreatePsFn>(CreatePixelShader)(
            device_, reinterpret_cast<const DWORD*>(hdr_writeback_program()), &shader_);
        if (FAILED(hr) || !shader_) {
            reason = "shader";
            std::snprintf(caps_.self_test_detail, sizeof caps_.self_test_detail, "create=%08lx", hr);
            drop(shader_);
        }
    ...
    if (!std::strcmp(reason, "ok") && config_.tonemap == HdrTonemap::Agx) {
        caps_.tonemap_shader = fault(HdrFault::TonemapShader)
                                   ? E_FAIL
                                   : call<CreatePsFn>(CreatePixelShader)(
                                         device_, reinterpret_cast<const DWORD*>(hdr_tonemap_program()),
                                         &tonemap_shader_);
        if (FAILED(caps_.tonemap_shader) || !tonemap_shader_) {
            drop(tonemap_shader_);
            caps_.tonemap = false;
            caps_.tonemap_reason = "shader";
        } else {
            caps_.tonemap = true;
            caps_.tonemap_reason = "ok";
        }
        if (caps_.tonemap && config_.meter_requested()) {
            ...
            if (SUCCEEDED(call<GetDirect3DFn>(GetDirect3D)(device_, &factory)) && factory &&
                SUCCEEDED(call<GetCreationFn>(GetCreationParameters)(device_, &creation)) &&
                SUCCEEDED(call<GetDisplayModeFn>(GetDisplayMode)(device_, 0, &mode))) {
                const D3DFORMAT candidates[2] = {D3DFMT_G32R32F, D3DFMT_A32B32G32R32F};
                const char* names[2] = {"G32R32F", "A32B32G32R32F"};
                for (unsigned i = 0; i < 2; ++i) {
                    caps_.chain_target = factory->CheckDeviceFormat(creation.AdapterOrdinal, creation.DeviceType,
                                                                    mode.Format, D3DUSAGE_RENDERTARGET,
                                                                    D3DRTYPE_TEXTURE, candidates[i]);
                    caps_.chain_sampling = SUCCEEDED(caps_.chain_target)
                                               ? factory->CheckDeviceFormat(creation.AdapterOrdinal,
                                                                            creation.DeviceType, mode.Format, 0,
                                                                            D3DRTYPE_TEXTURE, candidates[i])
                                               : S_FALSE;
                    if (SUCCEEDED(caps_.chain_target) && SUCCEEDED(caps_.chain_sampling)) {
                        caps_.chain_format = candidates[i];
                        caps_.chain_format_name = names[i];
                        break;
                    }
                }
            } else {
                caps_.chain_target = E_FAIL;
                caps_.chain_sampling = E_FAIL;
            }
            drop(factory);
            chain_texel_bytes_ = caps_.chain_format == D3DFMT_G32R32F ? 8u : 16u;
            if (FAILED(caps_.chain_target))
                caps_.meter_reason = "chain_target";
            else if (FAILED(caps_.chain_sampling))
                caps_.meter_reason = "chain_sampling";
            else {
                caps_.meter_shader = call<CreatePsFn>(CreatePixelShader)(
                    device_, reinterpret_cast<const DWORD*>(hdr_meter_level0_program()), &meter_level0_shader_);
                if (SUCCEEDED(caps_.meter_shader))
                    caps_.meter_shader = call<CreatePsFn>(CreatePixelShader)(
                        device_, reinterpret_cast<const DWORD*>(hdr_meter_reduce_program()), &meter_reduce_shader_);
                if (FAILED(caps_.meter_shader) || !meter_level0_shader_ || !meter_reduce_shader_) {
                    drop(meter_level0_shader_);
                    drop(meter_reduce_shader_);
                    caps_.meter_reason = "shader";
                } else {
                    caps_.meter = true;
                    caps_.meter_reason = "ok";
                }
            }
        }
    }
    ...
    if (!std::strcmp(reason, "ok") && config_.sharpen > 0.f) {
        caps_.sharpen_shader = call<CreatePsFn>(CreatePixelShader)(
            device_, reinterpret_cast<const DWORD*>(taa_sharpen_program()), &sharpen_shader_);
        if (SUCCEEDED(caps_.sharpen_shader) && caps_.tonemap)
            caps_.sharpen_shader = call<CreatePsFn>(CreatePixelShader)(
                device_, reinterpret_cast<const DWORD*>(hdr_tonemap_sharpen_program()), &tonemap_sharpen_shader_);
        if (FAILED(caps_.sharpen_shader) || !sharpen_shader_ || (caps_.tonemap && !tonemap_sharpen_shader_)) {
            drop(sharpen_shader_);
            drop(tonemap_sharpen_shader_);
            caps_.sharpen = false;
            caps_.sharpen_reason = "shader";
        } else {
            caps_.sharpen = true;
            caps_.sharpen_reason = "ok";
        }
    }
    ...
    if (!std::strcmp(reason, "ok") && config_.dither) {
        caps_.dither = true;
        caps_.dither_shader = call<CreatePsFn>(CreatePixelShader)(
            device_, reinterpret_cast<const DWORD*>(hdr_writeback_dither_program()), &writeback_dither_shader_);
        if (FAILED(caps_.dither_shader) || !writeback_dither_shader_) {
            drop(writeback_dither_shader_);
            caps_.dither_reason = "shader";
        } else
    ...
    if (!caps_.tonemap) {
        drop(tonemap_shader_);
        caps_.meter = false;
        if (config_.tonemap == HdrTonemap::Agx && config_.meter_requested() &&
            (!std::strcmp(caps_.meter_reason, "ok") || !std::strcmp(caps_.meter_reason, "off")))
            caps_.meter_reason = "tonemap";
    }
    if (!caps_.meter) {
        drop(meter_level0_shader_);
        drop(meter_reduce_shader_);
    ...
    if (!caps_.enabled) {
        ...
        drop(quad_vs_);
        drop(quad_declaration_);
        caps_.tonemap = caps_.meter = caps_.sharpen = caps_.dither = false;
    }
}
        ...
        if (SUCCEEDED(r.readback)) {
            ...
            for (UINT y = 0; y < tile_height_; ++y) {
                ...
                for (UINT x = 0; x < tile_width_; ++x) {
                    float texel[2];
                    std::memcpy(texel, row + std::size_t(x) * chain_texel_bytes_, 8);
                    tile_mean_[std::size_t(y) * tile_width_ + x] = texel[0];
                    tile_max_[std::size_t(y) * tile_width_ + x] = texel[1];
                }
            }
...
HdrWriteback HdrPass::write_back(IDirect3DSurface9* main, IDirect3DSurface9* final_rt0, bool scene_open, bool write,
                                 bool timing, IDirect3DTexture9* source, HdrDisplaySnapshot* display) noexcept {
    if (display) *display = {};
    HdrWriteback result{};
    if (!main || !final_rt0) {
        result.unwind = true;
        result.unwind_reason = "arguments";
        return result;
    }
    ...
    if (write && target_ && shader_) {
        injected_draw = fault(HdrFault::Draw)      ? E_FAIL
                        : fault(HdrFault::Lost)    ? D3DERR_DEVICELOST
                        : fault(HdrFault::Stretch) ? E_ABORT
                                                   : S_OK;
        ...
        Program identity;
        identity.shader = identity_shader();
        if (SUCCEEDED(hr)) {
            Program program;
            if (use_tonemap || sharpen) {
                ...
                program.meter = use_tonemap && meter_active() && chain_ring_[0] != nullptr;
                program.timing = timing;
                const HRESULT injected_tonemap = fault(HdrFault::TonemapDraw) ? E_FAIL : injected_draw;
                hr = copy_draw(target_, texture, main, width_, height_, final_rt0, &result.restore, injected_tonemap,
                               injected_restore, &program);
                result.tonemap = use_tonemap;
                result.sharpened = sharpen;
                result.tonemap_draw = use_tonemap ? hr : S_FALSE;
                result.meter = program.meter_result;
                result.ticks_meter = program.ticks_meter;
                if (sharpen && FAILED(hr) && !lost(hr) && SUCCEEDED(result.restore) && SUCCEEDED(injected_draw)) {
                    ...
                    ++sharpen_failures_;
                    result.sharpened = false;
                    result.sharpen_fallback = true;
                    program.shader = use_tonemap ? tonemap_shader_ : identity.shader;
                    program.sharpen = nullptr;
                    program.meter = false;
                    hr = copy_draw(target_, texture, main, width_, height_, final_rt0, &result.restore, injected_draw,
                                   injected_restore, &program);
                    if (use_tonemap) result.tonemap_draw = hr;
                }
                if (use_tonemap && FAILED(hr) && !lost(hr) && SUCCEEDED(result.restore) && SUCCEEDED(injected_draw)) {
                    ++tonemap_failures_;
                    result.tonemap = false;
                    result.fallback = true;
                    hr = copy_draw(target_, texture, main, width_, height_, final_rt0, &result.restore, injected_draw,
                                   injected_restore, &identity);
                }
            } else
                hr = copy_draw(target_, texture, main, width_, height_, final_rt0, &result.restore, injected_draw,
                               injected_restore, &identity);
        ...
        if (own_scene) {
            const HRESULT end = call<SceneFn>(EndScene)(device_);
            if (FAILED(end) && SUCCEEDED(result.draw)) result.draw = end;
        }
        drop(texture);
        ...
        if (SUCCEEDED(result.draw) && SUCCEEDED(result.restore)) {
            ...
            if (display && result.tonemap) {
                ...
                display->decode = agx_.decode[2] == 1.f   ? x3::temporal::AgxDecode::none
                                  : agx_.decode[1] == 1.f ? x3::temporal::AgxDecode::srgb
                                                          : x3::temporal::AgxDecode::gamma22;
                if (result.sharpened) {
        ...
        result.unwind = true;
        if (SUCCEEDED(result.draw)) {
            result.source = HdrWritebackSource::Shader;
            result.unwind_reason = "restore";
        } else {
            result.unwind_reason = lost(result.draw) ? "lost" : "draw";
            const std::uint64_t stretch_begin = stamp(timing);
            if (injected_draw == E_ABORT)
                result.stretch = E_FAIL; // fixture seam: both copy rungs fail
            else if (lost(result.draw))
                result.stretch = result.draw;
            else if (FAILED(caps_.stretch_conversion))
                result.stretch = D3DERR_NOTAVAILABLE;
            else
                result.stretch = call<StretchFn>(StretchRect)(device_, target_, nullptr, main, nullptr, D3DTEXF_POINT);
            result.ticks_stretch = stamp(timing) - stretch_begin;
            if (SUCCEEDED(result.stretch))
                result.source = HdrWritebackSource::Stretch;
            else {
                result.source = HdrWritebackSource::Restore;
                if (!lost(result.draw)) result.unwind_reason = "stretch";
            }
        }
    } else if (write) {
        result.unwind = true;
        result.unwind_reason = "resources";
    }
    ...
    if (FAILED(result.bind)) {
        result.unwind = true;
        if (!std::strcmp(result.unwind_reason, "none")) result.unwind_reason = "bind";
    }
    return result;
}
...
HRESULT HdrPass::fixture_readback(float* out, std::size_t floats, UINT* width, UINT* height) noexcept {
    ...
    HRESULT hr = call<CreateOffscreenFn>(CreateOffscreenPlainSurface)(device_, width_, height_, D3DFMT_A16B16G16R16F,
    ...
    if (SUCCEEDED(hr)) {
        for (UINT y = 0; y < height_; ++y) {
            const unsigned short* row = reinterpret_cast<const unsigned short*>(static_cast<const char*>(lock.pBits) +
                                                                                y * lock.Pitch);
