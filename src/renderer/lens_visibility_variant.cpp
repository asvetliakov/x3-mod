...
constexpr float clip_tap_weight = 1.f / 9.f; // lens_visibility_variant.h, "Clip pair": the fragment and the eight
                                             // knight moves, one ninth each
// What the wraps add, counted from the emitted words (host test): the step-1 wrap mov / texld / mul / mov, the clip
// pair the f fetch, the three uv instructions, nine taps of texld + cmp (+ add for eight of them) and the three tail
// instructions.
constexpr unsigned wrap_arithmetic_slots = 3, wrap_texture_slots = 1, clip_arithmetic_slots = 32,
                   clip_texture_slots = 10;
unsigned register_type(std::uint32_t token) noexcept {
    return ((token >> 28) & 7u) | ((token >> 8) & 0x18u);
}
unsigned register_number(std::uint32_t token) noexcept {
    return token & 0x7ffu;
}
...
unsigned arithmetic_slots(std::uint32_t opcode) noexcept {
    ...
    case 23:
    case 21: return 3; // m3x3, m4x3
    case 22:
    case 20: return 4; // m3x4, m4x4
    case 36:
    case 32: return 3; // nrm, pow
    case 37: return 8; // sincos
    case 86:
    case 87: return 2; // dsx, dsy
    case If:
    case Ifc:
    case Loop:
    case Rep:
    case BreakC: return 3;
    case EndLoop:
    case EndRep: return 2;
    case Nop:
    case Dcl:
    case Def:
    case DefI:
...
unsigned texture_slots(std::uint32_t opcode) noexcept {
    return opcode == TexLd || opcode == 65 ? 1u : opcode == 93 ? 3u : 0u;
} // texld / texldb / texldp, texkill, texldd
...
unsigned highest_free(const std::uint32_t* bits, unsigned limit, unsigned skip = ~0u, unsigned skip2 = ~0u) noexcept {
    for (unsigned i = limit; i-- > 0;)
        if (i != skip && i != skip2 && !(bits[i >> 5] >> (i & 31) & 1u)) return i;
    return limit;
}
...
LensVisibilityResult scan_pixel(const std::uint32_t* original, std::size_t words, PixelScan& scan) noexcept {
    ...
    scan.temporaries = scan.sm3 ? 32u : 12u;
    scan.constants = scan.sm3 ? 224u : 32u;
    scan.samplers = 16u;
    ...
    while (at < words) {
        ...
        auto note = [&](std::uint32_t parameter) {
            ...
            if (type == Temp) {
                ...
                scan.used_temp[0] |= 1u << number;
            } else if (type == Const) {
                if (number >= 224) return false;
                scan.used_const[number >> 5] |= 1u << (number & 31);
            } else if (type == Sampler) {
                if (number >= scan.samplers) return false;
                scan.used_sampler[0] |= 1u << number;
            } else if (type == Texture && !scan.sm3) {
                if (number >= 8) return false;
                scan.used_texcoord[0] |= 1u << number;
            } else if (type == Const2 || type == Const3 || type == Const4)
                return false;
            return true;
        };
        switch (opcode) {
        ...
        case CallNz:
        case Ret:
        case Label: return LensVisibilityResult::UnsupportedShader;
        case Def:
            if (length != 5 || !note(p[0])) return LensVisibilityResult::UnsupportedShader;
            break;
        case DefI:
        case DefB: break; // integer / boolean registers: never chosen below
        case Dcl:
            if (length != 2 || !note(p[1])) return LensVisibilityResult::UnsupportedShader;
            break;
        default: {
            if (texture_slots(opcode))
                scan.texture_slots += texture_slots(opcode);
            else
                scan.arithmetic_slots += arithmetic_slots(opcode);
            for (std::size_t i = 0; i < length; ++i)
                if (!note(p[i])) return LensVisibilityResult::UnsupportedShader;
            ...
            if (opcode == If || opcode == Ifc || opcode == Loop || opcode == Rep)
                ++depth;
            else if (opcode == EndIf || opcode == EndLoop || opcode == EndRep) {
                if (!depth) return LensVisibilityResult::InvalidInput;
                --depth;
            }
        }
        }
    ...
    scan.end = at;
    scan.body = body ? body : 1;
    return LensVisibilityResult::Applied;
}
// The body with every oC0 destination (always the first parameter) redirected to rO.
void copy_body_redirected(const std::uint32_t* original, std::size_t body, std::size_t end, std::uint32_t O,
    ...
    for (std::size_t i = body; i < end;) {
        ...
        for (std::size_t k = 0; k < length; ++k) {
            ...
            if (k == 0 && !comment && opcode != Def && opcode != DefI && opcode != DefB && opcode != Dcl &&
                has_destination(opcode) && register_type(parameter) == ColorOut && register_number(parameter) == 0)
...
constexpr std::uint32_t dst_temp(unsigned n, unsigned mask) noexcept {
    return 0x80000000u | (mask << 16) | n;
}
constexpr std::uint32_t src_temp(unsigned n, unsigned swizzle = 0xe4) noexcept {
    return 0x80000000u | (swizzle << 16) | n;
}
constexpr std::uint32_t src_const(unsigned n, unsigned swizzle = 0xe4, bool negate = false) noexcept {
    return 0xa0000000u | (negate ? 1u << 24 : 0u) | (swizzle << 16) | n;
}
constexpr std::uint32_t src_texcoord(unsigned n, unsigned swizzle = 0xe4) noexcept {
    return 0xb0000000u | (swizzle << 16) | n;
}
constexpr std::uint32_t src_sampler(unsigned n) noexcept {
    return 0xa0e40800u | n;
}
constexpr unsigned swz(unsigned x, unsigned y, unsigned z, unsigned w) noexcept {
    return x | (y << 2) | (z << 4) | (w << 6);
}
LensVisibilityResult build_pixel(const std::uint32_t* original, std::size_t words, LensVisibilityScale scale, bool clip,
                                 unsigned texcoord, float dx_u, float dy_v, bool core_f,
    ...
    const unsigned mask = scale == LensVisibilityScale::Rgb     ? 0x7u
                          : scale == LensVisibilityScale::Alpha ? 0x8u
                          : scale == LensVisibilityScale::Both  ? 0xfu
                                                                : 0u;
    if (!mask) return LensVisibilityResult::InvalidInput;
    if (clip) {
        if (scan.sm3)
            return LensVisibilityResult::UnsupportedVersion; // the clip pair is vs_2_x / ps_2_x (the lens scene's
                                                             // model)
    ...
    chosen.sampler = highest_free(scan.used_sampler, scan.samplers);
    chosen.constant = highest_free(scan.used_const, scan.constants);
    ...
    if (chosen.sampler == scan.samplers || chosen.constant == scan.constants ||
        chosen.output_temporary == scan.temporaries || chosen.fetch_temporary == scan.temporaries)
    ...
    if (!scan.sm3 && (scan.arithmetic_slots + (clip ? clip_arithmetic_slots : wrap_arithmetic_slots) > 64 ||
                      scan.texture_slots + (clip ? clip_texture_slots : wrap_texture_slots) > 32))
        return LensVisibilityResult::ResourceLimit;
    ...
    if (clip) {
        ...
        C = highest_free(scan.used_const, scan.constants, chosen.constant);
        Dc = highest_free(scan.used_const, scan.constants, chosen.constant, C);
        std::uint32_t taken_const[7];
        std::memcpy(taken_const, scan.used_const, sizeof taken_const);
        for (unsigned c : {chosen.constant, C, Dc})
            if (c < scan.constants) taken_const[c >> 5] |= 1u << (c & 31);
        ...
        std::uint32_t taken[1] = {scan.used_temp[0] | (1u << chosen.output_temporary) | (1u << chosen.fetch_temporary) |
                                  (1u << Q)};
        S = highest_free(taken, scan.temporaries);
        if (S < scan.temporaries) taken[0] |= 1u << S;
        T2 = highest_free(taken, scan.temporaries);
        if (T2 < scan.temporaries) taken[0] |= 1u << T2;
        E = highest_free(taken, scan.temporaries);
        if (chosen.depth_sampler == scan.samplers || C == scan.constants || Dc == scan.constants ||
            Wc == scan.constants || Q == scan.temporaries || S == scan.temporaries || T2 == scan.temporaries ||
            E == scan.temporaries)
            return LensVisibilityResult::ResourceLimit;
        chosen.constants[1] = C;
        chosen.constants[2] = Dc;
        chosen.constants[3] = Wc;
    }
    const std::uint32_t K = chosen.constant, Sm = chosen.sampler, O = chosen.output_temporary,
                        T = chosen.fetch_temporary;
    std::vector<std::uint32_t> result;
    try {
        result.reserve(words + 9 + 15 + (clip ? 80 : 0));
    } catch (const std::bad_alloc&) {
        return LensVisibilityResult::AllocationFailure;
    }
...
const char* lens_visibility_result_name(LensVisibilityResult r) noexcept {
    switch (r) {
    case LensVisibilityResult::Applied: return "applied";
    case LensVisibilityResult::InvalidInput: return "invalid_input";
    case LensVisibilityResult::UnsupportedVersion: return "unsupported_version";
    case LensVisibilityResult::UnsupportedShader: return "unsupported_shader";
    case LensVisibilityResult::NoOutput: return "no_output";
    case LensVisibilityResult::ResourceLimit: return "resource_limit";
...
LensVisibilityResult lens_visibility_vertex_variant(const std::uint32_t* original, std::size_t words, unsigned texcoord,
    ...
    float defs[256][4]{};
    std::uint32_t defined[8]{}; // def constants
    ...
    while (at < words) {
        ...
        case Def:
            if (length == 5 && register_type(p[0]) == Const && register_number(p[0]) < 256) {
                const unsigned n = register_number(p[0]);
                defined[n >> 5] |= 1u << (n & 31);
                std::memcpy(defs[n], p + 1, 16);
            }
            break;
        case Dcl:
            if (length == 2 && (p[0] & 0x1fu) == 0 && ((p[0] >> 16) & 0xfu) == 0 &&
                register_type(p[1]) == 1 /* input */)
                position_input = register_number(p[1]);
            break;
        case DefI:
        case DefB: break;
        default:
            for (std::size_t i = 0; i < length; ++i) {
                ...
                const unsigned type = register_type(parameter), number = register_number(parameter);
                if (type == Temp) {
                    if (number >= 32) return LensVisibilityResult::UnsupportedShader;
                    used_temp[0] |= 1u << number;
                } else if (type == TexCrdOut && i == 0) {
                    if (number >= 8) return LensVisibilityResult::UnsupportedShader;
                    used_texcoord[0] |= 1u << number;
                }
            }
            // A write to a temporary: does it leave (position.xyz, 1)? Any write to the dp4 source after the first dp4
            // spoils it.
            if (length && has_destination(opcode) && register_type(p[0]) == Temp && !(p[0] & relative)) {
                ...
                bool known = false;
                auto plain = [&](std::uint32_t src, unsigned type) {
                    return register_type(src) == type && !(src & relative) && !((src >> 24) & 0xfu);
                };
                auto lane = [](std::uint32_t src, unsigned k) { return (src >> (16 + 2 * k)) & 3u; };
                if (mask == 0xfu && depth == 0 && !((p[0] >> 20) & 0xfu)) {
                    if (opcode == Mov && length == 2 && plain(p[1], 1) && register_number(p[1]) == position_input &&
                        ((p[1] >> 16) & 0xffu) == 0xe4u)
                        known = true;
                    else if (opcode == 4 /* mad */ && length == 4 && plain(p[1], 1) &&
                             register_number(p[1]) == position_input && plain(p[2], Const) && plain(p[3], Const)) {
                        const unsigned a = register_number(p[2]), b = register_number(p[3]);
                        if (a < 256 && b < 256 && (defined[a >> 5] >> (a & 31) & 1u) &&
                            (defined[b >> 5] >> (b & 31) & 1u)) {
                            known = true;
                            for (unsigned k = 0; k < 4; ++k) {
                                const float mul = defs[a][lane(p[2], k)], add = defs[b][lane(p[3], k)];
                                if (k < 3)
                                    known = known && lane(p[1], k) == k && mul == 1.f && add == 0.f;
                                else
                                    known = known && mul == 0.f && add == 1.f;
                            }
                        }
                    }
                }
            ...
            if (length && has_destination(opcode) && register_type(p[0]) == RastOut && register_number(p[0]) == 0) {
                ...
                const unsigned mask = (p[0] >> 16) & 0xfu, lane = mask == 1   ? 0
                                                                  : mask == 2 ? 1
                                                                  : mask == 4 ? 2
                                                                  : mask == 8 ? 3
                                                                              : 4;
                if (opcode != Dp4 || length != 3 || depth || lane == 4 || (lanes & mask) || (p[0] & relative) ||
                    ((p[0] >> 20) & 0xfu))
                    return LensVisibilityResult::UnsupportedShader;
                if (register_type(p[1]) != Temp || ((p[1] >> 16) & 0xffu) != 0xe4u || ((p[1] >> 24) & 0xfu) ||
                    (p[1] & relative))
                    return LensVisibilityResult::UnsupportedShader;
                if (register_type(p[2]) != Const || ((p[2] >> 16) & 0xffu) != 0xe4u || ((p[2] >> 24) & 0xfu) ||
                    (p[2] & relative))
                    return LensVisibilityResult::UnsupportedShader;
                ...
                if (source_temp == ~0u) {
                    source_temp = temp;
                    matrix = constant - lane;
                    origin = origin_of[temp] == 1;
                } else if (temp != source_temp || constant - lane != matrix)
                    return LensVisibilityResult::UnsupportedShader;
                lanes |= mask;
    ...
    for (std::size_t i = body; i < end;) {
        ...
        for (std::size_t k = 0; k < length; ++k) {
            std::uint32_t parameter = original[i + 1 + k];
            if (k == 0 && !comment && opcode == Dp4 && register_type(parameter) == RastOut &&
                register_number(parameter) == 0)
    ...
    for (std::uint32_t w :
         {0x02000001u, 0xc00f0000u, src_temp(P), 0x02000001u, 0xe00f0000u | texcoord, src_temp(P), end_token})
        result.push_back(w);
    output.swap(result);
...
unsigned lens_visibility_free_texcoord(const std::uint32_t* vertex, std::size_t vertex_words,
    ...
    if (!vertex || vertex_words < 2 || vertex_words > (1u << 16) ||
        (vertex[0] != 0xfffe0200u && vertex[0] != 0xfffe0201u))
        return 8;
    ...
    for (std::size_t at = 1; at < vertex_words;) {
        ...
        if (opcode != comment_opcode && opcode != Def && opcode != DefI && opcode != DefB && opcode != Dcl && length &&
            has_destination(opcode)) {
