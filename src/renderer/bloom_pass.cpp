...
using Stretch = HRESULT(WINAPI*)(D, IDirect3DSurface9*, const RECT*, IDirect3DSurface9*, const RECT*,
...
void first_failure(HRESULT& first, HRESULT hr) noexcept {
...
bool main_descriptor(const D3DSURFACE_DESC& d) noexcept {
    return d.Format == D3DFMT_A8R8G8B8 && d.Type == D3DRTYPE_SURFACE && (d.Usage & D3DUSAGE_RENDERTARGET) &&
...
bool bytecode_valid(BloomBytecode b, DWORD version) noexcept {
    ...
    if (!b.words || b.count < 2 || b.count > 8192 || b.words[0] != version || b.words[b.count - 1] != 0x0000ffffu)
        return false;
    std::size_t i = 1;
    while (i < b.count - 1) {
...
void geometry(x3::temporal::BloomConstants& c, UINT sw, UINT sh, UINT dw, UINT dh) noexcept {
    ...
    c.source[2] = 1.f / sw;
    c.source[3] = 1.f / sh;
    c.destination[0] = float(dw);
    c.destination[1] = float(dh);
    c.destination[2] = 1.f / dw;
    c.destination[3] = 1.f / dh;
}
} // namespace
...
struct BloomPass::TextureViews {
    ...
    explicit TextureViews(BloomPass& p) noexcept
        : pass(p) {}
    ...
    ~TextureViews() {
        const bool was_releasing = pass.releasing_;
        pass.releasing_ = true;
        auto release = [&](IDirect3DTexture9*& p) {
            if (p) --pass.transient_views_;
            drop(p);
        };
...
void BloomPass::revoke() noexcept {
    pending_ = {};
...
std::uint64_t BloomPass::resource_bytes() const noexcept {
    return bytes(resources_);
}
unsigned BloomPass::references() const noexcept {
    unsigned n = (vertex_ ? 1u : 0u) + (declaration_ ? 1u : 0u) + (down_ ? 1u : 0u) + (up_ ? 1u : 0u) +
                 (candidate_ ? 1u : 0u) + (sharpen_ ? 1u : 0u) + (copy_ ? 1u : 0u);
    ...
    return n + (resources_.stage.surface ? 1u : 0u) + (resources_.candidate.surface ? 1u : 0u) +
           (resources_.recovery.surface ? 1u : 0u);
}
void BloomPass::before_reset() noexcept {
    revoke();
    if (++epoch_ == 0) ++epoch_;
    const bool was_releasing = releasing_;
    releasing_ = true;
    release(resources_);
...
HRESULT BloomPass::attach(IDirect3DDevice9* device, void* const* native, const D3DCAPS9& dc,
    ...
    auto fail = [&](HRESULT hr, const char* reason) {
        ...
        shutdown();
        caps_.reason = reason;
        caps_.formats = formats;
        caps_.programs = programs;
        return hr;
    };
    if (dc.PixelShaderVersion < D3DPS_VERSION(3, 0) || dc.VertexShaderVersion < D3DVS_VERSION(3, 0) ||
        ...
        !(dc.TextureAddressCaps & D3DPTADDRESSCAPS_CLAMP) || !(dc.TextureFilterCaps & D3DPTFILTERCAPS_MINFLINEAR) ||
        !(dc.TextureFilterCaps & D3DPTFILTERCAPS_MAGFLINEAR))
        return fail(D3DERR_NOTAVAILABLE, "caps");
    D3DDEVICE_CREATION_PARAMETERS creation{};
    D3DDISPLAYMODE mode{};
    HRESULT hr = call<GetCreation>(GetCreationParameters)(device_, &creation);
    if (FAILED(hr)) return fail(hr, "creation");
    if (creation.BehaviorFlags & (D3DCREATE_PUREDEVICE | D3DCREATE_SOFTWARE_VERTEXPROCESSING))
        return fail(D3DERR_NOTAVAILABLE, "vertex_processing");
    caps_.mixed_vertex_processing = (creation.BehaviorFlags & D3DCREATE_MIXED_VERTEXPROCESSING) != 0;
    if (!(creation.BehaviorFlags & (D3DCREATE_MIXED_VERTEXPROCESSING | D3DCREATE_HARDWARE_VERTEXPROCESSING)))
        return fail(D3DERR_NOTAVAILABLE, "vertex_processing");
    ...
    if (FAILED(hr) || !factory) {
        drop(factory);
        return fail(FAILED(hr) ? hr : E_FAIL, "factory");
    }
    caps_.formats = S_OK;
    for (D3DFORMAT format : {D3DFMT_A16B16G16R16F, D3DFMT_A8R8G8B8}) {
        first_failure(caps_.formats,
                      factory->CheckDeviceFormat(creation.AdapterOrdinal, creation.DeviceType, mode.Format,
    ...
    const BloomBytecode ps[] = {bundle.extract[0], bundle.extract[1], bundle.extract[2], bundle.extract[3],
                                bundle.extract[4], bundle.extract[5], bundle.down,       bundle.up,
                                bundle.candidate,  bundle.sharpen,    bundle.copy};
    for (const auto& b : ps)
        if (!bytecode_valid(b, D3DPS_VERSION(3, 0))) return fail(E_INVALIDARG, "pixel_bytecode");
    ...
    if (SUCCEEDED(caps_.programs))
        caps_.programs = call<CreateDecl>(CreateVertexDeclaration)(device_, quad_declaration, &declaration_);
    ...
    IDirect3DPixelShader9** outputs[] = {&extract_[0], &extract_[1], &extract_[2], &extract_[3],
                                         &extract_[4], &extract_[5], &down_,       &up_,
                                         &candidate_,  &sharpen_,    &copy_};
...
HRESULT BloomPass::create(Image& out, UINT width, UINT height, D3DFORMAT format) noexcept {
    ...
    HRESULT hr = call<CreateTex>(CreateTexture)(device_, width, height, 1, D3DUSAGE_RENDERTARGET, format,
...
HRESULT BloomPass::ensure(UINT width, UINT height, unsigned levels, bool sharpen) noexcept {
    ...
    if (!x3::temporal::prepare_bloom_layout(layout, {width, height}, levels) || width > device_caps_.MaxTextureWidth ||
        height > device_caps_.MaxTextureHeight)
        return E_INVALIDARG;
    auto admissible = [&](UINT w, UINT h) {
        ...
        if ((device_caps_.TextureCaps & D3DPTEXTURECAPS_POW2) &&
            !(device_caps_.TextureCaps & D3DPTEXTURECAPS_NONPOW2CONDITIONAL) && ((w & (w - 1)) || (h & (h - 1))))
            return false;
        const UINT aspect = device_caps_.MaxTextureAspectRatio;
        return !aspect ||
               (std::uint64_t(w) <= std::uint64_t(h) * aspect && std::uint64_t(h) <= std::uint64_t(w) * aspect);
    };
    ...
    if (resources_.width == width && resources_.height == height && resources_.layout.count == layout.count &&
        resources_.sharpen == sharpen)
        return S_OK;
    Resources next{};
    next.width = width;
    next.height = height;
    next.layout = layout;
    next.sharpen = sharpen;
...
HRESULT BloomPass::restore(const SavedState& s, bool inject_partial_failure) noexcept {
    ...
    for (unsigned i = 1; i < s.target_count; ++i) first_failure(hr, call<SetRT>(SetRenderTarget)(device_, i, s.rt[i]));
    ...
    if (caps_.mixed_vertex_processing) first_failure(hr, call<SetSW>(SetSoftwareVertexProcessing)(device_, s.software));
    first_failure(hr, call<SetNPatch>(SetNPatchMode)(device_, s.npatch));
    if (s.fvf)
        first_failure(hr, call<SetFvf>(SetFVF)(device_, s.fvf));
    else
        first_failure(hr, call<SetDecl>(SetVertexDeclaration)(device_, s.declaration));
    ...
    for (unsigned i = 0; i < state_count; ++i) {
        ...
        if (inject_partial_failure && states[i] == D3DRS_COLORWRITEENABLE)
            first_failure(hr, E_FAIL);
        else
            first_failure(hr, call<SetRS>(SetRenderState)(device_, states[i], s.rs[i]));
    }
    return hr;
}
    ...
    for (unsigned stage = 0; stage < 2; ++stage)
        for (unsigned i = 0; i < sampler_count; ++i) {
            ...
            first_failure(hr, call<SetSampler>(SetSamplerState)(device_, stage, samplers[i],
                                                                linear ? DWORD(D3DTEXF_LINEAR) : sampler_values[i]));
        }
    return hr;
}
...
HRESULT BloomPass::draw(const Image& output, IDirect3DPixelShader9* shader, IDirect3DTexture9* s0,
    ...
    if (constants)
        first_failure(hr, call<SetConstants>(SetPixelShaderConstantF)(device_, x3::temporal::kBloomFirstRegister,
                                                                      constants->source,
...
bool BloomPass::owned(IDirect3DSurface9* t) const noexcept {
    if (!t) return false;
    if (t == resources_.candidate.surface || t == resources_.recovery.surface || t == resources_.stage.surface)
        return true;
...
HRESULT BloomPass::validate_inputs(const BloomPrepare& p) const noexcept {
    ...
    if (!enabled() || !p.scene || !b.admitted || !b.main || owned(b.main) || b.thread != GetCurrentThreadId() ||
        !main_descriptor(b.main_desc) || !x3::temporal::valid_bloom_params(p.filter) ||
        !x3::temporal::valid_sharpen(p.sharpen))
        return E_INVALIDARG;
    if (p.exact_sharpen && p.sharpen > 0) {
        const auto& c = p.sharpen_constants.values;
        for (float v : c)
            if (!std::isfinite(v)) return E_INVALIDARG;
        // Preserve the supplied gain exactly; only validate its supported
        // range and that its texel steps describe this same image.
        if (c[0] < 0.25f || c[0] > 1.f || c[3] != 0.f || c[1] != 1.f / float(b.main_desc.Width) ||
            c[2] != 1.f / float(b.main_desc.Height))
            return E_INVALIDARG;
    }
    const auto mode = p.decode;
    if (mode != x3::temporal::AgxDecode::gamma22 && mode != x3::temporal::AgxDecode::srgb &&
        mode != x3::temporal::AgxDecode::none)
        return E_INVALIDARG;
    ...
    for (unsigned i = 0; i < sizeof(p.agx) / sizeof(float); ++i)
        if (!std::isfinite(floats[i])) return E_INVALIDARG;
    x3::temporal::AgxConstants expected{};
    x3::temporal::set_decode(expected, mode);
    for (unsigned i = 0; i < 4; ++i)
        if (p.agx.decode[i] != expected.decode[i]) return E_INVALIDARG;
    if (p.agx.exposure[0] <= 0 || p.agx.exposure[0] > x3::temporal::kAgxClampOff || p.agx.exposure[1] <= 0 ||
        p.agx.exposure[1] > x3::temporal::kAgxClampOff)
        return E_INVALIDARG;
    ...
    if (scene.Format != D3DFMT_A16B16G16R16F || scene.Pool != D3DPOOL_DEFAULT || scene.Width != b.main_desc.Width ||
        scene.Height != b.main_desc.Height || scene.MultiSampleType != D3DMULTISAMPLE_NONE || scene.MultiSampleQuality)
        return E_INVALIDARG;
...
HRESULT BloomPass::validate_boundary(const BloomBoundary& b, const SavedState& s, bool full_viewport) const noexcept {
    if (!b.admitted || b.thread != GetCurrentThreadId() || !b.main || s.rt[0] != b.main || s.depth != b.depth ||
        !main_descriptor(b.main_desc))
        return E_INVALIDARG;
    ...
    if (b.depth) {
        ...
        if (!descriptor_equal(actual, b.depth_desc) || !(actual.Usage & D3DUSAGE_DEPTHSTENCIL) ||
            ...
            actual.MultiSampleQuality || actual.Width < b.main_desc.Width || actual.Height < b.main_desc.Height)
            return E_INVALIDARG;
    }
    if (full_viewport && (s.viewport.X || s.viewport.Y || s.viewport.Width != b.main_desc.Width ||
                          s.viewport.Height != b.main_desc.Height))
        return E_INVALIDARG;
    return S_OK;
}
...
BloomPreparation BloomPass::prepare(const BloomPrepare& p) noexcept {
    ...
    if (FAILED(result.operation)) {
        if (lost(result.operation)) disabled_ = true;
        return result;
    }
    ...
    if (FAILED(result.allocation)) {
        result.reason = "allocation";
        if (lost(result.allocation)) disabled_ = true;
        return result;
    }
    SavedState saved;
    ...
    if (FAILED(result.saved)) {
        result.reason = "save";
        if (lost(result.saved)) disabled_ = true;
        return result;
    }
    ...
    c.filter[0] = p.filter.threshold;
    c.filter[1] = p.filter.knee;
    c.filter[2] = p.filter.scatter;
    c.filter[3] = p.filter.strength;
    ...
    for (unsigned i = resources_.layout.count - 1; i > 0 && SUCCEEDED(result.operation); --i) {
        ...
        geometry(c, sw, sh, output.width, output.height);
        result.operation = fault(BloomFault::PrepareDraw) ? E_FAIL : draw(output, up_, views.down[i - 1], source, &c);
        source = views.up[i - 1];
        sw = output.width;
        sh = output.height;
    }
    ...
    if (SUCCEEDED(result.operation) && p.sharpen > 0) {
        ...
        if (p.exact_sharpen)
            sharp = p.sharpen_constants;
        else if (!x3::temporal::prepare_sharpen(sharp, p.sharpen, width, height))
            result.operation = E_INVALIDARG;
        sharp.values[3] = p.agx.exposure[2]; // display dither of the 8-bit candidate write (taa_sharpen_ps.hlsl)
        if (SUCCEEDED(result.operation))
            result.operation = call<SetConstants>(SetPixelShaderConstantF)(device_, x3::temporal::kSharpenRegister,
                                                                           sharp.values, 1);
        if (SUCCEEDED(result.operation))
            result.operation = fault(BloomFault::PrepareDraw)
                                   ? E_FAIL
    ...
    pending_ = {this,
                ...
                p.boundary.reset,
                p.boundary.thread,
                resources_.candidate.surface,
                p.boundary.main,
                p.boundary.depth};
    result.candidate = pending_;
    result.ready = true;
    result.reason = "ok";
    return result;
}
...
bool BloomPass::valid(const BloomCandidate& token) const noexcept {
    return enabled() && pending_.owner == this && token.owner == this && token.epoch == epoch_ &&
           token.epoch == pending_.epoch && token.serial == serial_ && token.serial == pending_.serial &&
           token.frame == pending_.frame && token.reset == pending_.reset && token.thread == pending_.thread &&
           token.thread == GetCurrentThreadId() && token.surface == resources_.candidate.surface &&
           token.surface == pending_.surface && token.main == pending_.main && token.depth == pending_.depth;
}
...
BloomCommit BloomPass::commit(const BloomCandidate& token, const BloomBoundary& boundary) noexcept {
    ...
    if (FAILED(result.saved)) {
        result.reason = "save";
        result.requalification_required = lost(result.saved);
    ...
    if (FAILED(result.operation) || boundary.main_desc.Width != resources_.width ||
        ...
        result.reason = "boundary";
        result.requalification_required = lost(result.operation);
        if (result.requalification_required) disabled_ = true;
        return result;
    }
    TextureViews views(*this);
    ...
    result.backup = fault(BloomFault::Backup)
                        ? E_FAIL
                        : call<Stretch>(StretchRect)(device_, boundary.main, nullptr, resources_.recovery.surface,
                                                     nullptr, D3DTEXF_NONE);
    if (FAILED(result.backup)) {
        result.reason = "backup";
        result.requalification_required = lost(result.backup);
        if (result.requalification_required) disabled_ = true;
        return result;
    }
    ...
    if (SUCCEEDED(result.operation)) {
        ...
        if (result.write_attempted && fault(BloomFault::CommitDrawAfterWrite) && SUCCEEDED(result.operation))
            result.operation = E_FAIL;
    }
    ...
    result.recovery = FAILED(unbind)     ? unbind
                      : !recovery_issued ? E_FAIL
                                         : call<Stretch>(StretchRect)(device_, resources_.recovery.surface, nullptr,
                                                                      boundary.main, nullptr, D3DTEXF_NONE);
    ...
    result.recovery_restore = restore(saved, fault(BloomFault::RecoveryRestore));
    result.state_preserved = SUCCEEDED(result.recovery_restore);
    result.requalification_required = !result.original_preserved || !result.state_preserved || FAILED(result.restore) ||
                                      lost(result.operation) || lost(result.recovery);
    if (result.requalification_required) disabled_ = true;
