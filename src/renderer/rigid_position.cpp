...
template <class T, std::size_t N> constexpr std::size_t maximum_words(const T (&table)[N]) noexcept {
    std::size_t result = 0;
    for (const auto& p : table)
        if (p.word_count > result) result = p.word_count;
    return result;
}
template <class T, std::size_t N> constexpr bool strictly_sorted(const T (&table)[N]) noexcept {
    for (std::size_t i = 1; i < N; ++i)
        if (table[i - 1].hash >= table[i].hash) return false;
    return true;
}
...
template <class T, std::size_t N> bool length_known(const T (&table)[N], std::size_t count) noexcept {
    for (const auto& p : table)
        if (p.word_count == count) return true;
    return false;
}
...
template <class T, std::size_t N> const T* lookup(const T (&table)[N], std::uint64_t hash, std::size_t count) noexcept {
    std::size_t first = 0, last = N;
    while (first < last) {
        const auto middle = first + (last - first) / 2;
        if (table[middle].hash < hash)
            first = middle + 1;
        else
            last = middle;
    }
    return first < N && table[first].hash == hash && table[first].word_count == count ? &table[first] : nullptr;
}
}
...
VertexPositionPath classify_vertex_position(const std::uint32_t* words, std::size_t word_count) noexcept {
    if (!words || word_count > kMaxReviewedVertexShaderWords ||
        (!length_known(profiles, word_count) && !length_known(exceptions, word_count)))
...
const PixelCoverageProfile* find_pixel_coverage(const std::uint32_t* words, std::size_t word_count) noexcept {
    if (!words || word_count > kMaxReviewedPixelShaderWords || !length_known(pixel_profiles, word_count))
        return nullptr;
