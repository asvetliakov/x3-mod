...
template <std::size_t N> constexpr std::array<DWORD, N> bloom_native_words(const std::uint32_t (&source)[N]) noexcept {
...
template <std::size_t N> constexpr BloomBytecode bloom_code(const DWORD (&words)[N]) noexcept {
    return {words, N};
}
template <std::size_t N> constexpr BloomBytecode bloom_code(const std::array<DWORD, N>& words) noexcept {
    return {words.data(), N};
}
} // namespace detail
...
inline constexpr BloomPrograms bloom_programs() noexcept {
    ...
    return {bloom_code(bloom_quad_words),
            ...
            bloom_code(bloom_down_words),
            bloom_code(bloom_up_words),
            bloom_code(bloom_agx_words),
            bloom_code(bloom_sharpen_words),
            bloom_code(bloom_copy_words)};
}
} // namespace x3m::renderer
