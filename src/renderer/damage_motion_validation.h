...
inline bool damage_pixel_structure(const MotionOutputProfile& row, const std::uint32_t* words,
    ...
    const auto instruction = [&](std::size_t at, std::uint32_t token, std::size_t length) {
        ...
        if (at < row.pixel_declaration_insert_dword) {
            ...
            if (at == 1313) {
                if (token != 0x05000051u || words[at + 1] != 0xa00f0019u || words[at + 2] != 0x3f800000u ||
                    words[at + 3] != (two_sided ? 0xbf800000u : 0u) ||
                    words[at + 4] != (two_sided ? 0u : 0x40000000u) ||
                    words[at + 5] != (two_sided ? 0x40000000u : 0xbf800000u))
                    return false;
                literal = true;
            }
        ...
        if (next_flow < 7 && at == flow[next_flow]) {
            ...
            if (op == 40) {
                if (depth || token != 0x01000028u || words[at + 1] != (0xe0e40800u | (next_flow == 4 ? 1u : 0u)))
                    return false;
                depth = 1;
            } else if (op == 41) {
                // if_ne r1.y, -r1.y; full-precision mov r1.z, c25.<zero>.
                if (depth || token != 0x02050029u || words[at + 1] != 0x80550001u || words[at + 2] != 0x81550001u)
                    return false;
                depth = 1;
            } else {
        ...
        case 6:
        case 7:
        case 36: arity = 2; break; // mov rcp rsq nrm
        case 2:
        case 5:
        case 8:
        case 9:
        case 11:
        case 32:
        case 66: arity = 3; break;
        case 4:
        case 18:
        case 88:
        case 90: arity = 4; break;
        ...
        if (at > flow[2] && at < flow[3]) {
            if (at != flow[2] + 3 || token != 0x02000001u || words[at + 1] != 0x80040001u || words[at + 2] != zero)
                return false;
            ++body;
        }
        if (at == mad_at) {
            if (depth || token != 0x04000004u || words[at + 1] != 0x80240001u || words[at + 2] != two ||
                words[at + 3] != 0x81000000u || words[at + 4] != 0xa0000019u)
                return false;
            ++initializer_sites;
        }
        if (at == cmp_at) {
            if (depth || token != 0x04000058u || words[at + 1] != 0x80020001u || words[at + 2] != 0x80aa0001u ||
                words[at + 3] != zero || words[at + 4] != 0xa0000019u)
                return false;
            ++initializer_sites;
        }
        ...
        if (at > mad_at && at < flow[2] && at != cmp_at && register_type(destination) == 0 &&
            (destination & 0x7ffu) == 1 && (destination & 0x00060000u))
            return false;
