...
constexpr unsigned mov=1,mul=5,dp3=8,minimum=10,maximum=11,dcl=31,power=32,tex=66,def=81,cmp=88;
constexpr Word end=0xffff,comment=0xfffe,coissue=0x40000000u,pp=0x200000u;
struct Profile { std::uint64_t hash; unsigned count; bool scalar; };
...
Word reg(unsigned kind,unsigned n) noexcept { return 0x80000000u|((kind&7)<<28)|((kind&24)<<8)|n; }
Word dst(unsigned kind,unsigned n,unsigned mask=7) noexcept { return reg(kind,n)|(mask<<16); }
Word src(unsigned kind,unsigned n,unsigned swizzle=0xe4,unsigned modifier=0) noexcept { return reg(kind,n)|(swizzle<<16)|(modifier<<24); }
Word lane(unsigned kind,unsigned n,unsigned c) noexcept { return src(kind,n,c*0x55); }
Word bits(float f) noexcept { Word v;std::memcpy(&v,&f,4);return v; }
unsigned kind(Word v) noexcept { return ((v>>28)&7)|((v>>8)&24); }
unsigned number(Word v) noexcept { return v&0x7ff; }
std::uint64_t fingerprint(const Word* code,std::size_t count) noexcept {
    std::uint64_t h=0xcbf29ce484222325ull;
    for(std::size_t i=0;i<count;++i) for(unsigned shift=0;shift<32;shift+=8) { h^=(code[i]>>shift)&255;h*=0x100000001b3ull; }
    return h;
}
bool expect(const Word* code,std::size_t count,std::size_t& at,Word token,
    ...
    at+=operands.size()+1;return true;
}
...
bool original_shape(const Word* code,std::size_t count,bool scalar) noexcept {
    ...
    if(scalar && !expect(code,count,at,def,{dst(constant,0,15),bits(1),bits(0),bits(0),bits(0)})) return false;
    ...
    if(!expect(code,count,at,mul,{dst(temporary,0),src(coordinate,0),scalar?src(temporary,0):lane(color,0,3)}) ||
       !expect(code,count,at,mov|coissue,{dst(temporary,0,8),lane(coordinate,0,3)})) return false;
...
bool generated_shape(const Words& words,Budget& budget,unsigned outputs) noexcept {
    ...
    for(std::size_t at=1;at<words.size();) {
        ...
        if(token==end) return at==words.size()-1 && budget.outputs==((1u<<outputs)-1) &&
            budget.textures==1 && budget.arithmetic<=64 && decls==7;
        unsigned size=op==comment?(token>>16)&0x7fff:(token>>24)&15;
        if(size>words.size()-at-1) return false;
        if(op==comment) { if(executable || defs || decls || token!=(comment|(size<<16))) return false;at+=size+1;continue; }
        if(token!=((size<<24)|op)) return false;
        if(op==def) {
            if(executable || size!=5) return false;
            const auto d=words[at+1];const unsigned n=number(d);
            if((n!=0 && n!=30 && n!=31) || d!=dst(constant,n,15) || (defs&(1u<<n))) return false;
            defs|=1u<<n;
        } else if(op==dcl) {
            if(executable || size!=2) return false;
            const auto d=words[at+2];unsigned bit=0;
            ...
            for(unsigned j=2;j<=size;++j) {
                const Word s=words[at+j];const unsigned sk=kind(s),sn=number(s),sw=(s>>16)&255,mod=(s>>24)&15;
                if(mod>1 || s!=src(sk,sn,sw,mod)) return false;
                if(sk==temporary) {
                    if(sn>=4) return false;
                    unsigned read=0;
                    for(unsigned c=0;c<4;++c) if((op==dp3?7u:mask)&(1u<<c)) read|=1u<<((sw>>(2*c))&3);
                    ...
                    if(sk==color) for(unsigned c=0;c<4;++c)
                        if(((op==dp3?7u:mask)&(1u<<c)) && !(color_mask&(1u<<((sw>>(2*c))&3)))) return false;
                } else return false;
            }
            if(k==output) {
            ...
            if(op==tex) {
                if(k!=temporary || words[at+2]!=src(coordinate,0) || words[at+3]!=src(sampler,0)) return false;
                ++budget.textures;
            } else {
                if(op==power) for(unsigned j=2;j<=3;++j) {
                    const auto s=words[at+j];const unsigned sw=(s>>16)&255;
