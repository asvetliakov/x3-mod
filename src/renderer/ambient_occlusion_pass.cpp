...
HRESULT AmbientOcclusionPass::execute(const AmbientOcclusionFrame& in, AmbientOcclusionResult* out) noexcept {
    ...
    *out = {};
    ...
    if (SUCCEEDED(hr) && (desc.Width != in.width || desc.Height != in.height || (desc.Format != D3DFMT_R32F && desc.Format != D3DFMT_G32R32F && desc.Format != D3DFMT_A32B32G32R32F) || desc.MultiSampleType != D3DMULTISAMPLE_NONE))
        hr = E_INVALIDARG;
