...
constexpr std::uint32_t row(unsigned n) {
    return 0xa0e40000u | n;
}
...
const RigidPositionProfile* find_rigid_replay_profile(const std::uint32_t* words, std::size_t count) noexcept {
    const auto* p = find_rigid_position(words, count);
    return p && p->shader_version == 0xfffe0300u && p->position_write_order == PositionWriteOrder::XYZW &&
