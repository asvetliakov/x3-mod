#pragma once
// Conservative recognizer for the observed X3 0.3 scene/bloom/clear sequence.
...
struct SceneSignatures {
    ShaderPair background[3] = {
        ...
        {0xbe199829a9bb78dbull, 0xcd6d6eb4b3d99443ull}}; // Required haze marker is slot 2.
    ShaderPair bloom[4] = {
...
enum class EventKind { Clear, Draw, SetRenderTarget, SetDepth, Copy, Unsupported };
struct Event {
...
public:
    ...
    Selection observe(const Event& event) {
        ...
        if (event.sequence != sequence_ + 1) { reject(BoundaryRejection::Sequence); return {}; }
...
private:
    ...
    std::uint64_t device_ = 0, generation_ = 0, frame_ = 0, sequence_ = 0, epoch_ = 0;
    Surface main_, depth_, copied_, a_, b_, bound_rt_;
    bool background_draw_ = false, background_haze_ = false, scene_writer_ = false;
    unsigned bloom_ = 0;
    void reject(BoundaryRejection why) { state_ = BoundaryState::Rejected; rejection_ = why; }
    ...
    bool haze(const Event& e) const { return matches(e, signatures_.background[2]); }
    ...
    bool advance(const Event& e) {
        ...
        case BoundaryState::Background:
            if (e.kind == EventKind::Draw && scene_draw(e) && background_pair(e)) {
                background_draw_ = true; background_haze_ |= haze(e); return true;
            }
            if (e.kind != EventKind::Clear || !background_draw_ || !background_haze_ || !depth_clear(e)) return false;
