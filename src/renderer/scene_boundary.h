...
public:
    ...
    void begin_frame(std::uint64_t device, std::uint64_t generation, std::uint64_t frame) {
        *this = SceneBoundarySelector{signatures_};
        state_ = BoundaryState::AwaitInitialClear;
        rejection_ = BoundaryRejection::None;
        device_ = device;
    ...
    Selection before_clear(const Event& pending) const {
        if (state_ != BoundaryState::AwaitFinalClear || pending.sequence != sequence_ + 1 ||
            pending.kind != EventKind::Clear || !depth_clear(pending))
            return {};
    ...
    Selection observe(const Event& event) {
        ...
        if (event.kind == EventKind::Unsupported ||
            ...
            reject(BoundaryRejection::Pattern);
            return {};
        }
...
private:
    ...
    static bool full(const Event& e) {
        return e.only_rt0 && e.viewport.known && e.viewport.x == 0 && e.viewport.y == 0 &&
               e.viewport.width == e.rt.width && e.viewport.height == e.rt.height && e.viewport.min_z == 0 &&
               e.viewport.max_z == 1;
    }
    bool depth_clear(const Event& e) const {
        return e.clear_flags == 2 && e.rect_count == 0 && e.clear_z == 1 && full(e) && same(e.rt, main_) &&
               same(e.depth, depth_);
    }
    ...
    static bool depth_writer(const Event& e) { return e.draw_state_known && e.z_enable == 1 && e.z_write == 1; }
    ...
    bool half_target(const Surface& s) const {
        return color(s) && s.container && s.identity != main_.identity && s.identity != copied_.identity &&
               s.container != copied_.container && std::uint64_t(s.width) * 2 == main_.width &&
               std::uint64_t(s.height) * 2 == main_.height;
    }
    bool advance(const Event& e) {
        switch (state_) {
        case BoundaryState::AwaitInitialClear:
            if (e.kind != EventKind::Clear || e.clear_flags != 3 || e.rect_count || e.clear_z != 1 || !full(e) ||
                !color(e.rt) || !e.depth.known || !e.depth.identity || e.depth.format != 77 || e.depth.msaa ||
                e.depth.width != e.rt.width || e.depth.height != e.rt.height || e.depth.identity == e.rt.identity)
                return false;
            main_ = e.rt;
            depth_ = e.depth;
            epoch_ = 1;
            state_ = BoundaryState::Background;
            return true;
        ...
        case BoundaryState::Scene:
            ...
            state_ = BoundaryState::AwaitCopy;
            return true;
        case BoundaryState::AwaitCopy:
            ...
            if (e.kind != EventKind::Copy || !same(e.source, main_) || !color(e.destination) ||
                ...
                e.destination.width != main_.width || e.destination.height != main_.height || !e.source_rect_null ||
                !e.destination_rect_null)
                return false;
            copied_ = e.destination;
            state_ = BoundaryState::AwaitBloomTarget;
            return true;
        case BoundaryState::AwaitBloomTarget:
            if (e.kind != EventKind::SetRenderTarget || e.rt_index) return false;
            if (bloom_ == 0) {
                if (!half_target(e.rt)) return false;
                a_ = e.rt;
            } else if (bloom_ == 1) {
                if (!half_target(e.rt) || e.rt.identity == a_.identity || e.rt.container == a_.container) return false;
                b_ = e.rt;
            } else if (!same(e.rt, bloom_ == 2 ? a_ : main_))
                return false;
            bound_rt_ = e.rt;
            state_ = BoundaryState::AwaitBloomDraw;
            return true;
        case BoundaryState::AwaitBloomDraw: {
            const auto input = bloom_ == 0 ? copied_.container : (bloom_ == 2 ? b_.container : a_.container);
            if (e.kind != EventKind::Draw || !e.draw_state_known || !same(e.rt, bound_rt_) || !e.depth.known ||
                e.depth.identity || !full(e) || e.topology != 5 || e.primitives != 2 || e.z_enable || e.z_write ||
                !matches(e, signatures_.bloom[bloom_]) || e.texture0 != input)
                return false;
            ++bloom_;
            state_ = bloom_ == 4 ? BoundaryState::AwaitDepthRebind : BoundaryState::AwaitBloomTarget;
            return true;
        }
        case BoundaryState::AwaitDepthRebind:
            if (e.kind != EventKind::SetDepth || !same(e.depth, depth_)) return false;
            state_ = BoundaryState::AwaitFinalClear;
            return true;
        case BoundaryState::AwaitFinalClear:
            if (e.kind != EventKind::Clear || !depth_clear(e)) return false;
            state_ = BoundaryState::Selected;
            return true;
