...
HRESULT GpuSyncTiming::attach(IDirect3DDevice9* device, void* const* native, unsigned window) noexcept {
    ...
    if (!device || !native) {
        reason_ = "no_device";
        return create_result_ = E_INVALIDARG;
    }
    LARGE_INTEGER f{};
    if (!QueryPerformanceFrequency(&f) || f.QuadPart <= 0) {
        reason_ = "no_performance_counter";
        return create_result_ = E_FAIL;
    }
...
HRESULT GpuSyncTiming::create() noexcept {
    ...
    if (tripped_) {
        reason_ = "sync_timeouts";
        return create_result_ = D3DERR_NOTAVAILABLE;
    ...
    if (FAILED(hr)) {
        ...
        return create_result_ = hr;
    } // partial creation rolled back
    // The optional census queries: unsupported or refused leaves the timing (and the other census) available without
    // them.
    if (create_query(device_, D3DQUERYTYPE_OCCLUSION, nullptr) == S_OK)
        for (IDirect3DQuery9*& query : census_)
            if (FAILED(create_query(device_, D3DQUERYTYPE_OCCLUSION, &query)) || !query) query = nullptr;
    add_ref(device_);
    const ULONG after = release_fn(device_);
...
void GpuSyncTiming::release() noexcept {
    ...
    for (IDirect3DQuery9*& query : queries_) {
        IDirect3DQuery9* old = query;
        query = nullptr;
        if (old) old->Release();
    }
    for (IDirect3DQuery9*& query : census_) {
...
void GpuSyncTiming::before_reset() noexcept {
    if (device_) {
        release();
...
void GpuSyncTiming::after_reset(HRESULT reset) noexcept {
    ...
    if (FAILED(reset)) {
        reason_ = "reset_failed";
        return;
    } // still released; the next successful Reset recreates
    create();
}
void GpuSyncTiming::detach() noexcept {
    release();
    device_ = nullptr;
    native_ = nullptr;
    reason_ = "detached";
    create_result_ = S_FALSE;
}
...
bool GpuSyncTiming::sync(unsigned boundary, std::uint64_t* after, std::uint64_t* wait) noexcept {
    ...
    if (FAILED(hr)) {
        ++stats_.issue_failures;
        stats_.last_failure = hr;
        return false;
    }
    ...
    for (;;) {
        ...
        if (hr != S_FALSE) {
            ++stats_.data_failures;
            stats_.last_failure = hr;
            return false;
        } // D3DERR_DEVICELOST and any other refusal
        if (now - start > limit) {
            ++stats_.timeouts;
            stats_.last_failure = S_FALSE;
            return false;
        }
    }
}
void GpuSyncTiming::mark(unsigned pass, bool begin) noexcept {
    ...
    if (begin && pass == gpu_sync_timing::Scene) {
        ...
        if (cooperative != D3D_OK) {
            ++stats_.not_cooperative;
            ++stats_.dropped_frames;
            tracker_.abandon_frame();
            return;
        }
    }
    ...
    if (!sync(2u * pass + (begin ? 0u : 1u), &after, &wait)) {
        ...
        if (++consecutive_failures_ >= spin_failure_limit || timeouts) {
            ...
            create_result_ = stats_.last_failure;
    ...
    consecutive_failures_ = 0;
    if (begin)
        tracker_.begin(pass, after);
    else
        tracker_.end(pass, after, wait);
}
...
void GpuSyncTiming::census_close(unsigned slot, std::uint32_t area, std::uint32_t tag) noexcept {
    if (census_state_[slot] != Census::Open || !census_[slot])
        return; // closes a begun bracket even after the cut-off switched the boundaries off
    PreserveCpuState guard;
    census_state_[slot] = SUCCEEDED(census_[slot]->Issue(D3DISSUE_END)) ? Census::Issued : Census::Done;
    census_area_[slot] = area;
    census_tag_[slot] = tag;
}
bool GpuSyncTiming::frame(std::uint64_t frame, gpu_sync_timing::Report* report) noexcept {
    ...
    for (unsigned slot = 0; slot < 2; ++slot) {
        if (census_state_[slot] == Census::Issued && !tracker_.abandoned()) {
            ...
            const gpu_sync_timing::CensusRead read = hr == S_OK                ? gpu_sync_timing::CensusOk
                                                     : hr == S_FALSE           ? gpu_sync_timing::CensusNotReady
                                                     : hr == D3DERR_DEVICELOST ? gpu_sync_timing::CensusLost
                                                                               : gpu_sync_timing::CensusFailed;
            if (slot == 0)
                tracker_.census(hr == S_OK ? std::uint32_t(pixels) : 0u, census_area_[0], read);
            else
                tracker_.needs(hr == S_OK ? std::uint32_t(pixels) : 0u, census_area_[1], census_tag_[1], read);
        }
