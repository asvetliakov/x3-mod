...
bool depth_format(D3DFORMAT f) noexcept {
...
constexpr D3DSAMPLERSTATETYPE lens_states[6] = {D3DSAMP_ADDRESSU,  D3DSAMP_ADDRESSV,  D3DSAMP_MAGFILTER,
                                                D3DSAMP_MINFILTER, D3DSAMP_MIPFILTER, D3DSAMP_SRGBTEXTURE};
constexpr DWORD lens_values[6] = {D3DTADDRESS_CLAMP, D3DTADDRESS_CLAMP, D3DTEXF_POINT,
                                  D3DTEXF_POINT,     D3DTEXF_NONE,      FALSE};
// Step 2: the core clip's tap step in RT2 pixels (the fragment and the eight knight moves (+-2, +-1) / (+-1, +-2)
// steps, every one on a texel centre: lens_visibility_variant.h, "Clip pair").
...
const char* lens_verdict_name(LensVerdict v) noexcept {
    switch (v) {
    case LensVerdict::Applied: return "applied";
    case LensVerdict::NotReady: return "not_ready";
    case LensVerdict::NoShader: return "no_shader";
    case LensVerdict::Unhashed: return "unhashed";
    case LensVerdict::Blend: return "blend";
    case LensVerdict::Variant: return "variant";
    case LensVerdict::CacheFull: return "cache_full";
    case LensVerdict::Device: return "device";
    case LensVerdict::Body: return "body";
    }
    return "invalid";
}
...
struct SunOcclusionPass::SavedState {
    ...
    DWORD fvf = 0;
    UINT count;
    SavedState(SunOcclusionPass& p, UINT n)
        : pass(p)
        , count(n > 4 ? 4 : n) {}
    ~SavedState() {
    ...
    HRESULT restore() noexcept {
        ...
        D d = pass.device_;
        for (UINT i = 0; i < 2; ++i)
            if (!attempt(pass.call<SetTextureFn>(SetTexture)(d, i, nullptr)))
                return first; // RT2 leaves the sampler before it is a target again
        ...
        ++pass.device_calls_;
        if (!attempt(pass.saved_->Apply()))
            return first; // after the targets: SetRenderTarget(0) rewrote viewport and scissor, the block holds the
                          // caller's
        if (declaration)
            attempt(pass.call<SetDeclarationFn>(SetVertexDeclaration)(d, declaration));
        else if (fvf)
            attempt(pass.call<SetFvfFn>(SetFVF)(d, fvf));
        else
            attempt(pass.call<SetDeclarationFn>(SetVertexDeclaration)(d, nullptr));
        return first;
    }
};
SunOcclusionPass::~SunOcclusionPass() {
    detach();
}
unsigned SunOcclusionPass::references() const noexcept {
    unsigned n = 0;
    for (const void* p : {static_cast<const void*>(normal_), static_cast<const void*>(saved_),
                          static_cast<const void*>(program_), static_cast<const void*>(quad_vs_),
                          static_cast<const void*>(quad_declaration_), static_cast<const void*>(textures_[0]),
                          static_cast<const void*>(textures_[1]), static_cast<const void*>(surfaces_[0]),
                          static_cast<const void*>(surfaces_[1]), static_cast<const void*>(readback_)})
        n += p != nullptr;
    for (unsigned i = 0; i < variant_count_; ++i)
        for (auto* s : variant_[i].shader) n += s != nullptr;
    for (unsigned i = 0; i < pair_count_; ++i) {
        n += pair_[i].vertex != nullptr;
        for (auto* s : pair_[i].pixel) n += s != nullptr;
    }
...
void SunOcclusionPass::before_reset() noexcept {
    drop(normal_);
    drop(saved_);
    drop(readback_); // every state block is a DEFAULT-pool citizen: gone before Reset
    for (auto& b : blocks_) {
        drop(b.saved);
        drop(b.ours);
        b = Blocks{};
...
void SunOcclusionPass::after_reset(HRESULT result) noexcept {
    if (SUCCEEDED(result)) reset_pending_ = false;
}
void SunOcclusionPass::detach() noexcept {
    before_reset();
    for (unsigned i = 0; i < variant_count_; ++i) {
        for (auto*& s : variant_[i].shader) drop(s);
        variant_[i] = Variant{};
    }
    for (unsigned i = 0; i < pair_count_; ++i) {
        drop(pair_[i].vertex);
        for (auto*& s : pair_[i].pixel) drop(s);
        pair_[i] = Pair{};
    }
    variant_count_ = 0;
    pair_count_ = 0;
...
HRESULT SunOcclusionPass::attach(IDirect3DDevice9* d, void* const* native, const D3DCAPS9& caps,
                                 D3DFORMAT adapter_format) noexcept {
    detach();
    if (!d) {
        caps_.reason = "device";
        return E_INVALIDARG;
    ...
    caps_.program_slots = ps3_program_slots(reinterpret_cast<const std::uint32_t*>(visibility_words),
                                            std::size(visibility_words));
    if (caps_.program_slots == 0 || caps.MaxPixelShader30InstructionSlots < caps_.program_slots)
        return refuse("ps_slots", D3DERR_NOTAVAILABLE);
    ...
    if (SUCCEEDED(hr) && FAILED(api->CheckDeviceFormat(creation.AdapterOrdinal, creation.DeviceType, adapter_format,
                                                       D3DUSAGE_RENDERTARGET, D3DRTYPE_TEXTURE, fraction_format)))
        gate = "a16b16g16r16f";
    drop(api);
    caps_.formats = hr;
    if (FAILED(hr)) return refuse("format_query", hr);
    if (gate) {
        caps_.formats = D3DERR_NOTAVAILABLE;
        return refuse(gate, D3DERR_NOTAVAILABLE);
    }
    render_targets_ = caps.NumSimultaneousRTs ? caps.NumSimultaneousRTs : 1;
    streams_ = caps.MaxStreams;
    try {
        words_.reserve(4096);
        wrapped_.reserve(4096 + 96);
        vertex_words_.reserve(4096);
    } catch (const std::bad_alloc&) {
        return refuse("memory", E_OUTOFMEMORY);
    }
    ...
    caps_.programs = hr;
    if (FAILED(hr)) {
        const SunOcclusionCaps kept = caps_;
        detach();
        caps_ = kept;
...
HRESULT SunOcclusionPass::ensure_targets() noexcept {
    ...
    for (unsigned i = 0; i < 2 && SUCCEEDED(hr); ++i) {
        hr = call<CreateTextureFn>(CreateTexture)(device_, 1, 1, 1, D3DUSAGE_RENDERTARGET, fraction_format,
                                                  D3DPOOL_DEFAULT, &textures_[i], nullptr);
        if (SUCCEEDED(hr)) hr = textures_[i]->GetSurfaceLevel(0, &surfaces_[i]);
        if (SUCCEEDED(hr))
            hr = call<ColorFillFn>(ColorFill)(device_, surfaces_[i], nullptr, D3DCOLOR_ARGB(255, 255, 255, 255));
    }
    if (FAILED(hr)) {
        for (auto& s : surfaces_) drop(s);
        for (auto& t : textures_) drop(t);
...
template <class Sets> HRESULT SunOcclusionPass::record(IDirect3DStateBlock9** out, Sets&& sets) noexcept {
    ...
    if (FAILED(body) || FAILED(hr) || !block) {
        drop(block);
        return FAILED(body) ? body : FAILED(hr) ? hr : E_FAIL;
    }
    *out = block;
    return S_OK;
}
...
HRESULT SunOcclusionPass::execute(const SunVisibilityFrame& in, SunVisibilityResult* out) noexcept {
    ...
    *out = {};
    ...
    for (float v : {in.u, in.v, in.radius_u, in.radius_v, in.alpha, in.curve, in.jitter_u, in.jitter_v})
        if (!std::isfinite(v)) return skip("params");
    if (!(in.radius_u > 0.f) || !(in.radius_v > 0.f) || in.radius_u > 1.f || in.radius_v > 1.f || in.alpha < 0.f ||
        in.alpha > 1.f || in.curve < .25f || in.curve > 4.f || in.u < -4.f || in.u > 5.f || in.v < -4.f || in.v > 5.f ||
        in.jitter_u < -jitter_limit || in.jitter_u > jitter_limit || in.jitter_v < -jitter_limit ||
        in.jitter_v > jitter_limit)
        return skip("params");
    D3DSURFACE_DESC desc{};
    if (FAILED(in.depth->GetLevelDesc(0, &desc)) || !depth_format(desc.Format) ||
        desc.MultiSampleType != D3DMULTISAMPLE_NONE)
        return skip("format");
    if (FAILED(same_device(device_, in.depth))) return skip("device");
    auto fail = [&](SunOcclusionStage stage, HRESULT hr) {
        valid_ = false;
        out->failed = stage;
        out->operation = hr;
        return hr;
    ...
    auto step = [&](SunOcclusionStage s, HRESULT value) {
        stage = s;
        hr = value;
        return SUCCEEDED(hr);
    };
    ...
    for (UINT i = 1; i < render_targets_ && i < 4 && SUCCEEDED(hr); ++i)
        hr = call<SetRtFn>(SetRenderTarget)(d, i, nullptr);
    ...
    if (SUCCEEDED(hr) && !in.caller_scene_open)
        own_scene = step(SunOcclusionStage::Scene, call<SceneFn>(BeginScene)(d));
    // c0 the disc, c1 the smoothing, c2.xy the jitter (sun_visibility_ps.hlsl); the recorded set covers all three.
    const float block[3][4] = {{in.u, in.v, in.radius_u, in.radius_v},
                               {in.alpha, seed ? 1.f : 0.f, in.curve, 0.f},
                               {in.jitter_u, in.jitter_v, 0.f, 0.f}};
    if (SUCCEEDED(hr))
        step(SunOcclusionStage::Constants, call<SetPsConstantsFn>(SetPixelShaderConstantF)(d, 0, &block[0][0], 3));
    ...
    if (SUCCEEDED(hr)) {
        ...
        drew = step(SunOcclusionStage::Draw,
                    call<DrawUpFn>(DrawPrimitiveUP)(d, D3DPT_TRIANGLESTRIP, 2, vertices, sizeof(QuadVertex)));
    }
    if (own_scene && !lost(hr)) {
        const HRESULT end = call<SceneFn>(EndScene)(d);
        if (SUCCEEDED(hr) || lost(end)) {
            if (FAILED(end)) stage = SunOcclusionStage::EndScene;
            hr = end;
        }
    }
    out->operation = hr;
...
SunOcclusionPass::Variant* SunOcclusionPass::find(std::uint64_t hash) noexcept {
    for (unsigned i = 0; i < variant_count_; ++i)
        if (variant_[i].hash == hash) return &variant_[i];
...
bool SunOcclusionPass::read_function(IDirect3DVertexShader9* vs, IDirect3DPixelShader9* ps,
                                     std::vector<std::uint32_t>& out) noexcept {
    UINT bytes = 0;
    const HRESULT size = vs ? vs->GetFunction(nullptr, &bytes) : ps->GetFunction(nullptr, &bytes);
    if (FAILED(size) || bytes < 8 || (bytes & 3u) || bytes > (1u << 18)) {
        variant_refusal_ = "function_size";
        return false;
    }
    try {
        out.resize(bytes / 4);
    } catch (const std::bad_alloc&) {
        variant_refusal_ = "memory";
        return false;
    }
    const HRESULT read = vs ? vs->GetFunction(out.data(), &bytes) : ps->GetFunction(out.data(), &bytes);
    if (FAILED(read)) {
        variant_refusal_ = "function_read";
        return false;
    }
    return true;
}
...
bool SunOcclusionPass::build(Variant& v, unsigned mode, IDirect3DPixelShader9* original) noexcept {
    ...
    const LensVisibilityResult result = lens_visibility_pixel_variant(words_.data(), words_.size(),
                                                                      LensVisibilityScale(mode + 1), wrapped_, &layout);
    if (result != LensVisibilityResult::Applied) {
        variant_refusal_ = lens_visibility_result_name(result);
        return false;
    }
    IDirect3DPixelShader9* created = nullptr;
    if (FAILED(
            call<CreatePsFn>(CreatePixelShader)(device_, reinterpret_cast<const DWORD*>(wrapped_.data()), &created)) ||
        !created) {
        drop(created);
        variant_refusal_ = "create";
        return false;
    }
    v.shader[mode] = created;
    v.sampler[mode] = std::uint8_t(layout.sampler);
    v.constant[mode] = std::uint8_t(layout.constant);
    v.state[mode] = 1;
    return true;
}
SunOcclusionPass::Pair* SunOcclusionPass::find_pair(std::uint64_t vertex_hash, std::uint64_t pixel_hash) noexcept {
    for (unsigned i = 0; i < pair_count_; ++i)
        if (pair_[i].vertex_hash == vertex_hash && pair_[i].pixel_hash == pixel_hash) return &pair_[i];
    if (pair_count_ == pair_capacity) return nullptr;
    Pair& fresh = pair_[pair_count_++];
    fresh = Pair{};
    fresh.vertex_hash = vertex_hash;
    fresh.pixel_hash = pixel_hash;
    return &fresh;
}
...
bool SunOcclusionPass::scan_pair(Pair& p, const LensState& state) noexcept {
    p.state = 2;
    if (!read_function(state.vertex_shader, nullptr, vertex_words_) || !read_function(nullptr, state.shader, words_))
        return false;
    const unsigned texcoord = lens_visibility_free_texcoord(vertex_words_.data(), vertex_words_.size(), words_.data(),
                                                            words_.size());
    if (texcoord >= 8) {
        variant_refusal_ = "no_free_texcoord";
        return false;
    }
    unsigned matrix = 0;
    bool origin = false;
    const LensVisibilityResult result = lens_visibility_vertex_variant(vertex_words_.data(), vertex_words_.size(),
                                                                       texcoord, wrapped_, &matrix, &origin);
    if (result != LensVisibilityResult::Applied) {
        variant_refusal_ = lens_visibility_result_name(result);
        return false;
    }
    p.texcoord = std::uint8_t(texcoord);
    p.matrix_register = matrix;
    p.origin_known = origin;
    p.state = 1;
    return true;
}
...
bool SunOcclusionPass::build_pair_vertex(Pair& p, const LensState& state) noexcept {
    ...
    bool origin = false;
    const LensVisibilityResult result = lens_visibility_vertex_variant(vertex_words_.data(), vertex_words_.size(),
                                                                       p.texcoord, wrapped_, &matrix, &origin);
    if (result != LensVisibilityResult::Applied || matrix != p.matrix_register) {
        variant_refusal_ = result != LensVisibilityResult::Applied ? lens_visibility_result_name(result)
                                                                   : "vertex_changed";
        return false;
    }
    IDirect3DVertexShader9* created = nullptr;
    if (FAILED(
            call<CreateVsFn>(CreateVertexShader)(device_, reinterpret_cast<const DWORD*>(wrapped_.data()), &created)) ||
        !created) {
        drop(created);
        variant_refusal_ = "create_vertex";
        return false;
    }
    p.vertex = created;
    p.vertex_state = 1;
...
bool SunOcclusionPass::build_pair_pixel(Pair& p, unsigned mode, const LensState& state) noexcept {
    if (p.width != state.depth_width || p.height != state.depth_height || p.core_f != core_f_) { // another RT2 size or
                                                                                                 // product mode: the
                                                                                                 // pixel wraps are
                                                                                                 // rebuilt
        ...
        p.width = state.depth_width;
        p.height = state.depth_height;
        p.core_f = core_f_;
    }
    p.pixel_state[mode] = 2;
    if (!p.width || !p.height) {
        variant_refusal_ = "depth_size";
        return false;
    }
    if (!read_function(nullptr, state.shader, words_)) return false;
    LensVisibilityLayout layout{};
    const LensVisibilityResult result = lens_visibility_pixel_clip_variant(
        words_.data(), words_.size(), LensVisibilityScale(mode + 1), p.texcoord, soft_edge_pixels / float(p.width),
        soft_edge_pixels / float(p.height), core_f_, wrapped_, &layout);
    if (result != LensVisibilityResult::Applied) {
        variant_refusal_ = lens_visibility_result_name(result);
        return false;
    }
    ...
    if (FAILED(
        ...
        !created) {
        drop(created);
        variant_refusal_ = "create_clip";
        return false;
    }
    p.pixel[mode] = created;
    p.sampler[mode] = std::uint8_t(layout.sampler);
    p.depth_sampler[mode] = std::uint8_t(layout.depth_sampler);
    p.pixel_state[mode] = 1;
...
HRESULT SunOcclusionPass::ensure_blocks(unsigned sampler, unsigned depth_sampler, const std::uint8_t constants[4],
                                        unsigned* index) noexcept {
    Blocks* slot = nullptr;
    for (unsigned i = 0; i < block_capacity; ++i) {
        Blocks& b = blocks_[i];
        if (b.saved && b.ours && b.sampler == sampler && b.depth_sampler == depth_sampler &&
            std::memcmp(b.constants, constants, 4) == 0) {
            *index = i;
            return S_OK;
        }
    ...
    auto states = [&]() -> HRESULT {
        for (unsigned s : {sampler, depth_sampler}) {
            ...
            for (unsigned i = 0; i < 6; ++i) {
                const HRESULT hr = call<SetSamplerFn>(SetSamplerState)(d, s, lens_states[i], lens_values[i]);
                if (FAILED(hr)) return hr;
            }
        }
        return S_OK;
    ...
    if (SUCCEEDED(hr))
        hr = record(&slot->saved, [&]() -> HRESULT {
            ...
            const float zero[4]{};
            for (unsigned i = 0; i < 4 && SUCCEEDED(inner); ++i)
                if (constants[i] != 255)
                    inner = call<SetPsConstantsFn>(SetPixelShaderConstantF)(d, constants[i], zero, 1);
    ...
    slot->sampler = std::uint8_t(sampler);
    slot->depth_sampler = std::uint8_t(depth_sampler);
    std::memcpy(slot->constants, constants, 4);
...
LensVerdict SunOcclusionPass::lens_prepare(const LensState& state, Prepared* out) noexcept {
    ...
    if (out) {
        out->matrix_register = p->matrix_register;
        out->origin_known = p->origin_known;
    }
...
LensVerdict SunOcclusionPass::lens_begin(const LensState& state, LensDraw& out) noexcept {
    ...
    if (state.body == core::Body::Ghost) {
        ...
        out.sampler = std::uint8_t(sampler);
        out.block = std::uint8_t(block);
        out.applied = true;
        ++device_calls_;
        ...
        if (FAILED(hr)) {
            lens_end(out);
            return LensVerdict::Device;
        } // everything back; the draw goes out unwrapped
        return LensVerdict::Applied;
    }
    // Core: the clip pair.
    ...
    if (p->state != 1 || !p->origin_known)
        return LensVerdict::Variant; // a core body needs a classifiable origin: the caller never sends one otherwise
    if (p->vertex_state == 0) build_pair_vertex(*p, state);
    if (p->vertex_state != 1) return LensVerdict::Variant;
    if (p->pixel_state[mode] == 0 || p->width != state.depth_width || p->height != state.depth_height ||
        p->core_f != core_f_)
        build_pair_pixel(*p, mode, state);
    ...
    if (FAILED(blocks_[block].saved->Capture())) return LensVerdict::Device; // nothing changed yet
    out.sampler = std::uint8_t(sampler);
    out.depth_sampler = std::uint8_t(depth_sampler);
    out.block = std::uint8_t(block);
    out.clipped = true;
    out.applied = true;
    ++device_calls_;
...
HRESULT SunOcclusionPass::lens_end(LensDraw& draw) noexcept {
    ...
    ++device_calls_;
    return blocks_[block].saved->Apply(); // the application's shaders, textures, sampler states and the wrap's def
                                          // registers, as captured for this draw
}
HRESULT SunOcclusionPass::readback(float out[4]) noexcept {
    ...
    HRESULT hr = S_OK;
    if (!readback_)
        hr = call<CreateOffscreenFn>(CreateOffscreenPlainSurface)(device_, 1, 1, fraction_format, D3DPOOL_SYSTEMMEM,
                                                                  &readback_, nullptr);
    ...
    for (unsigned i = 0; i < 4; ++i) { // binary16 -> float, integer and SSE only
        ...
        float v = e == 0    ? float(m) * (1.f / 16777216.f)
                  : e == 31 ? 65504.f
                            : float(1024u + m) * (1.f / 1024.f) * float(1u << e) * (1.f / 32768.f);
