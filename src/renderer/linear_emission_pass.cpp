...
HRESULT object_identity(IUnknown* a, IUnknown* b, bool& equal) noexcept {
    ...
    HRESULT hr = FAILED(ha) ? ha : FAILED(hb) ? hb : (!aa || !bb) ? E_NOINTERFACE : S_OK;
    if (SUCCEEDED(hr)) equal = aa == bb;
    drop(aa);
    drop(bb);
    return hr;
}
...
constexpr DWORD fullscreen[] = {FALSE,
                                ...
                                D3DBLEND_ONE,
                                D3DBLEND_ZERO};
constexpr D3DSAMPLERSTATETYPE samplers[] = {D3DSAMP_MINFILTER,   D3DSAMP_MAGFILTER,    D3DSAMP_MIPFILTER,
                                            D3DSAMP_ADDRESSU,    D3DSAMP_ADDRESSV,     D3DSAMP_SRGBTEXTURE,
                                            D3DSAMP_MAXMIPLEVEL, D3DSAMP_MIPMAPLODBIAS};
constexpr DWORD sample_values[] = {
    D3DTEXF_POINT, D3DTEXF_POINT, D3DTEXF_NONE, D3DTADDRESS_CLAMP, D3DTADDRESS_CLAMP, FALSE, 0, 0};
static_assert(std::size(states) == std::size(fullscreen) && std::size(samplers) == std::size(sample_values));
...
bool patched_packed_composite(float gain, DWORD (&out)[std::size(packed_composite_words)]) noexcept {
    ...
    for (unsigned i = 0; i + 5 < std::size(packed_composite_words); ++i)
        if (packed_composite_words[i] == def_c1_token && packed_composite_words[i + 1] == def_c1_register) {
            ++found;
            at = i;
        }
...
struct LinearEmissionPass::Impl {
    ...
    bool has_frame = false, mask_valid = false, blocked = false, recovered = false;
    IDirect3DSurface9** selected = nullptr;
    IDirect3DSurface9* selected_surface = nullptr;
    IDirect3DSurface9* input_scene = nullptr; // exact borrowed caller ownership slot value
    ...
    bool fault(LinearEmissionPassFault f) noexcept {
        if (f != fault_kind || !fault_count) return false;
        --fault_count;
        return true;
    }
#else
    ...
    bool owned(IDirect3DSurface9* p) const noexcept {
        return p &&
               (same_object(p, b) || same_object(p, e) || same_object(p, c) || same_object(p, m) || same_object(p, pb));
    }
    bool supported_state(D3DRENDERSTATETYPE state) const noexcept {
        ...
        if (state == D3DRS_BLENDOPALPHA || state == D3DRS_SRCBLENDALPHA || state == D3DRS_DESTBLENDALPHA)
            return source_over;
        if (state == D3DRS_COLORWRITEENABLE) return (caps9.PrimitiveMiscCaps & D3DPMISCCAPS_COLORWRITEENABLE) != 0;
        ...
        unsigned slot = state == D3DRS_COLORWRITEENABLE1   ? 1
                        ...
                        : state == D3DRS_COLORWRITEENABLE3 ? 3
                                                           : 0;
        return !slot || (slot < rt_count && (caps9.PrimitiveMiscCaps & D3DPMISCCAPS_INDEPENDENTWRITEMASKS));
    }
    bool distinct_from_pool(IDirect3DSurface9* surface) noexcept {
        for (auto* target : {b, e, c, m, pb}) {
            bool equal = false;
            if (FAILED(object_identity(surface, target, equal)) || equal) return false;
        }
        return true;
    }
    HRESULT save() noexcept {
        saved.release();
        if (fault(LinearEmissionPassFault::Save)) return E_FAIL;
        HRESULT hr = S_OK;
        for (unsigned i = 0; i < rt_count; ++i) {
            hr = call(GetRt, DWORD(i), &saved.rt[i]);
            if (FAILED(hr) && !(i && hr == D3DERR_NOTFOUND && !saved.rt[i])) return hr;
        }
        if (!saved.rt[0]) return E_FAIL;
...
#undef GET
    ...
    HRESULT restore(IDirect3DSurface9* target) noexcept {
        ...
        HRESULT first = S_OK;
        auto step = [&](HRESULT h) {
            if (SUCCEEDED(first) && FAILED(h)) first = h;
        };
        for (unsigned i = 0; i < stages; ++i)
            step(call(SetTexture, DWORD(i), static_cast<IDirect3DBaseTexture9*>(nullptr)));
        for (unsigned i = 1; i < rt_count; ++i) step(call(SetRt, DWORD(i), static_cast<IDirect3DSurface9*>(nullptr)));
        step(call(SetRt, DWORD(0), target));
        for (unsigned i = 1; i < rt_count; ++i) step(call(SetRt, DWORD(i), saved.rt[i]));
        ...
        for (unsigned i = 0; i < std::size(states); ++i)
            if (supported_state(states[i])) step(call(SetRs, states[i], saved.rs[i]));
        return first;
    }
    HRESULT target(IDirect3DSurface9* surface) noexcept {
        HRESULT hr;
        for (unsigned i = 0; i < stages; ++i)
            if (FAILED(hr = call(SetTexture, DWORD(i), static_cast<IDirect3DBaseTexture9*>(nullptr)))) return hr;
        for (unsigned i = 1; i < rt_count; ++i)
            if (FAILED(hr = call(SetRt, DWORD(i), static_cast<IDirect3DSurface9*>(nullptr)))) return hr;
        if (FAILED(hr = call(SetDepth, static_cast<IDirect3DSurface9*>(nullptr)))) return hr;
    ...
    HRESULT full_state() noexcept {
        ...
        if (FAILED(hr = call(SetViewport, &viewport)) || FAILED(hr = call(SetScissor, &scissor)) ||
            FAILED(hr = call(SetVs, vs)) || FAILED(hr = call(SetDecl, declaration)) ||
            FAILED(hr = call(SetFreq, UINT(0), UINT(1))))
            return hr;
        for (unsigned i = 0; i < std::size(states); ++i)
            if (supported_state(states[i]) && FAILED(hr = call(SetRs, states[i], fullscreen[i]))) return hr;
        for (unsigned i = 0; i < stages; ++i)
            for (unsigned j = 0; j < std::size(samplers); ++j)
                if (FAILED(hr = call(SetSampler, DWORD(i), samplers[j], sample_values[j]))) return hr;
        return S_OK;
    }
    ...
    HRESULT plane_masks() noexcept {
        ...
        for (unsigned i = 1; i < 4 && SUCCEEDED(hr); ++i)
            hr = call(SetRs,
                      D3DRENDERSTATETYPE(i == 1   ? D3DRS_COLORWRITEENABLE1
                                         : i == 2 ? D3DRS_COLORWRITEENABLE2
                                                  : D3DRS_COLORWRITEENABLE3),
                      DWORD(7));
        return hr;
    }
    ...
    HRESULT clear(IDirect3DSurface9* surface) noexcept {
        ...
        D3DVIEWPORT9 viewport{0, 0, width, height, 0, 1};
        RECT scissor{0, 0, LONG(width), LONG(height)};
        if (SUCCEEDED(hr)) hr = call(SetViewport, &viewport);
        if (SUCCEEDED(hr)) hr = call(SetScissor, &scissor);
        if (SUCCEEDED(hr)) hr = call(SetRs, D3DRS_SCISSORTESTENABLE, DWORD(FALSE));
        if (SUCCEEDED(hr))
            hr = call(Clear, DWORD(0), static_cast<const D3DRECT*>(nullptr), DWORD(D3DCLEAR_TARGET), D3DCOLOR(0), 1.f,
                      DWORD(0));
        return hr;
    }
    ...
    HRESULT draw(Program kind, IDirect3DSurface9* destination, IDirect3DSurface9* a,
        ...
        case Program::Composite:
            ...
            program = source_over ? source_over_composite : composite;
            break;
        ...
        case Program::PackedComposite:
            ...
            sources[4] = a;
            bound = 5;
            program = packed_composite;
            break;
        }
        HRESULT hr = S_OK;
        for (unsigned i = 0; i < bound; ++i) {
            hr = sources[i]->GetContainer(IID_IDirect3DTexture9, reinterpret_cast<void**>(&views[i]));
            if (FAILED(hr) || !views[i]) {
                if (SUCCEEDED(hr)) hr = E_NOINTERFACE;
                break;
            }
        }
        if (SUCCEEDED(hr)) hr = target(destination);
        ...
        if (SUCCEEDED(hr) && kind == Program::Copy && fused_copy) hr = call(SetRt, DWORD(1), e);
        if (SUCCEEDED(hr) && kind == Program::PlaneInit) hr = attach_planes();
        if (SUCCEEDED(hr)) hr = full_state();
        if (SUCCEEDED(hr) && kind == Program::PlaneInit) hr = plane_masks();
        if (SUCCEEDED(hr) && region) {
            const bool composite = kind == Program::Composite || kind == Program::PackedComposite;
            if (fault(composite ? LinearEmissionPassFault::CompositeScissor : LinearEmissionPassFault::RegionScissor))
                hr = E_FAIL;
        ...
        for (unsigned i = 0; i < bound && SUCCEEDED(hr); ++i)
            hr = call(SetTexture, DWORD(i), static_cast<IDirect3DBaseTexture9*>(views[i]));
        if (SUCCEEDED(hr)) hr = call(SetPs, program);
        if (SUCCEEDED(hr) && kind == Program::PlaneInit && fault(LinearEmissionPassFault::PlaneInit)) hr = E_FAIL;
        if (SUCCEEDED(hr)) {
            QuadVertex vertices[4];
            quad_vertices(width, height, vertices);
            hr = call(DrawUp, D3DPT_TRIANGLESTRIP, UINT(2), static_cast<const void*>(vertices),
                      UINT(sizeof(QuadVertex)));
        }
        for (auto& p : views) drop(p);
        return hr;
    }
    bool source_ok(const LinearEmissionBoundary& boundary) noexcept {
        if (!same_object(saved.rt[0], boundary.scene) || !saved.depth || !distinct_from_pool(boundary.scene))
            return false;
        for (unsigned i = 1; i < rt_count; ++i)
            if (saved.rt[i]) return false;
        D3DSURFACE_DESC a{}, depth{};
        if (FAILED(boundary.scene->GetDesc(&a)) || FAILED(saved.depth->GetDesc(&depth))) return false;
        if (a.Format != D3DFMT_A16B16G16R16F || a.Pool != D3DPOOL_DEFAULT || a.Usage != D3DUSAGE_RENDERTARGET ||
            a.Width != width || a.Height != height || a.MultiSampleType != D3DMULTISAMPLE_NONE || a.MultiSampleQuality)
            return false;
        if (depth.Format != depth_format || depth.Width < width || depth.Height < height ||
            depth.MultiSampleType != D3DMULTISAMPLE_NONE || depth.MultiSampleQuality)
            return false;
        if (packed) {
            ...
            return saved.state(D3DRS_ALPHABLENDENABLE) && saved.state(D3DRS_BLENDOP) == D3DBLENDOP_ADD &&
                   saved.state(D3DRS_SRCBLEND) == D3DBLEND_ONE &&
                   saved.state(D3DRS_DESTBLEND) == D3DBLEND_INVSRCCOLOR && saved.state(D3DRS_COLORWRITEENABLE) == 15 &&
                   !saved.state(D3DRS_SEPARATEALPHABLENDENABLE) && !saved.state(D3DRS_ZWRITEENABLE) &&
                   !saved.state(D3DRS_STENCILENABLE) && !saved.state(D3DRS_FOGENABLE) &&
                   !saved.state(D3DRS_DITHERENABLE) && !saved.state(D3DRS_SRGBWRITEENABLE) && !saved.ss[0][5];
        }
        if (source_over) {
            return boundary.augmented_vertex && saved.state(D3DRS_ALPHABLENDENABLE) &&
                   saved.state(D3DRS_BLENDOP) == D3DBLENDOP_ADD && saved.state(D3DRS_SRCBLEND) == D3DBLEND_SRCALPHA &&
                   saved.state(D3DRS_DESTBLEND) == D3DBLEND_INVSRCALPHA && saved.state(D3DRS_COLORWRITEENABLE) == 7 &&
                   !saved.state(D3DRS_SEPARATEALPHABLENDENABLE) && saved.state(D3DRS_ZENABLE) == D3DZB_TRUE &&
                   !saved.state(D3DRS_ALPHATESTENABLE) && !saved.state(D3DRS_ZWRITEENABLE) &&
                   !saved.state(D3DRS_STENCILENABLE) && !saved.state(D3DRS_FOGENABLE) &&
                   !saved.state(D3DRS_DITHERENABLE) && !saved.state(D3DRS_SRGBWRITEENABLE) && !saved.ss[0][5];
        }
        return saved.state(D3DRS_ALPHABLENDENABLE) && saved.state(D3DRS_BLENDOP) == D3DBLENDOP_ADD &&
               saved.state(D3DRS_SRCBLEND) == D3DBLEND_ONE && saved.state(D3DRS_DESTBLEND) == D3DBLEND_ONE &&
               (!supported_state(D3DRS_COLORWRITEENABLE) || saved.state(D3DRS_COLORWRITEENABLE) == 15) &&
               saved.state(D3DRS_ZENABLE) == D3DZB_TRUE && !saved.state(D3DRS_ALPHATESTENABLE) &&
               !saved.state(D3DRS_ZWRITEENABLE) && !saved.state(D3DRS_STENCILENABLE) && !saved.state(D3DRS_FOGENABLE) &&
               !saved.state(D3DRS_DITHERENABLE) && !saved.state(D3DRS_SRGBWRITEENABLE) && !saved.ss[0][5];
    }
    ...
    RECT select_region(const LinearEmissionBoundary& boundary) const noexcept {
        ...
        if (!boundary.region_known) return whole;
        ...
        if (r.right <= r.left || r.bottom <= r.top) return whole;
        return r;
    }
        ...
        if (device && native && (b || e || c || m || pb)) {
            for (unsigned i = 0; i < stages; ++i) {
                ...
                if (SUCCEEDED(hr) && texture)
                    hr = texture->QueryInterface(IID_IDirect3DTexture9, reinterpret_cast<void**>(&tex));
                if (SUCCEEDED(hr) && tex) hr = tex->GetSurfaceLevel(0, &level);
                if (SUCCEEDED(hr) && owned(level))
                    call(SetTexture, DWORD(i), static_cast<IDirect3DBaseTexture9*>(nullptr));
            ...
            for (unsigned remaining = rt_count; remaining > 0; --remaining) {
                ...
                if (SUCCEEDED(call(GetRt, DWORD(i), &rt)) && owned(rt)) {
                    if (i)
                        ...
                        if (SUCCEEDED(call(GetBackBuffer, UINT(0), UINT(0), D3DBACKBUFFER_TYPE_MONO, &back)) && back)
...
LinearEmissionPass::~LinearEmissionPass() {
    detach();
}
...
HRESULT LinearEmissionPass::attach(IDirect3DDevice9* device, void* const* native, const D3DCAPS9& caps9,
                                   D3DFORMAT format, D3DFORMAT depth, unsigned requested_policies) noexcept {
    detach();
#ifdef X3M_LINEAR_DISTANCE_FADE_FIXTURE
    if (fixture_source_over_) requested_policies = 6;
#endif
    if (!device || !native || !requested_policies || (requested_policies & ~15u)) return E_INVALIDARG;
    ...
    p.caps.reason = "caps";
    if (caps9.NumSimultaneousRTs < 3 || caps9.PixelShaderVersion < D3DPS_VERSION(3, 0) ||
    ...
    constexpr DWORD fade_caps = D3DPMISCCAPS_SEPARATEALPHABLEND | D3DPMISCCAPS_INDEPENDENTWRITEMASKS |
    ...
    if (caps9.NumSimultaneousRTs < 4 || (caps9.PrimitiveMiscCaps & packed_caps) != packed_caps ||
        !(caps9.RasterCaps & D3DPRASTERCAPS_SCISSORTEST) || !(caps9.SrcBlendCaps & D3DPBLENDCAPS_ONE) ||
        !(caps9.DestBlendCaps & D3DPBLENDCAPS_INVSRCALPHA))
        supported &= ~8u;
    if (!supported) {
    ...
    if (SUCCEEDED(hr))
        hr = factory->CheckDeviceFormat(creation.AdapterOrdinal, creation.DeviceType, format, 0, D3DRTYPE_TEXTURE,
                                        D3DFMT_A16B16G16R16F);
    if (SUCCEEDED(hr))
        hr = factory->CheckDepthStencilMatch(creation.AdapterOrdinal, creation.DeviceType, format, D3DFMT_A16B16G16R16F,
                                             depth);
    drop(factory);
    p.caps.formats = hr;
    if (FAILED(hr)) {
    ...
    if (base) {
        for (unsigned policy : {1u, 2u}) {
            ...
            if (SUCCEEDED(hr))
                p.caps.available_policies |= policy == 1 ? 1u : (supported & 6u);
            else {
                drop(program);
    ...
    if (base && (supported & 8u)) {
        ...
        if (SUCCEEDED(hr))
            p.caps.available_policies |= 8u;
        else {
            drop(p.plane_init);
            drop(p.packed_composite);
            if (SUCCEEDED(first)) first = hr;
        }
    }
...
HRESULT LinearEmissionPass::ensure_targets(UINT width, UINT height) noexcept {
    if (!impl_ || !impl_->caps.enabled || !width || !height) return E_INVALIDARG;
    auto& p = *impl_;
    if (p.phase != Impl::Phase::Idle) return D3DERR_INVALIDCALL;
    const unsigned count = p.caps.supports(LinearCompositionPolicy::PackedScreenInPlace) ? 5u : 4u;
    if (width == p.width && height == p.height && p.b && p.e && p.c && p.m && (count < 5 || p.pb)) return S_OK;
    if (width > p.caps9.MaxTextureWidth || height > p.caps9.MaxTextureHeight) return D3DERR_NOTAVAILABLE;
    if (p.fault(LinearEmissionPassFault::Allocation)) return E_OUTOFMEMORY;
    IDirect3DSurface9* surfaces[5]{};
    HRESULT hr = S_OK;
    for (unsigned i = 0; i < count; ++i) {
        auto& surface = surfaces[i];
        IDirect3DTexture9* texture = nullptr;
        hr = p.call(CreateTexture, width, height, UINT(1), DWORD(D3DUSAGE_RENDERTARGET), D3DFMT_A16B16G16R16F,
        ...
        if (SUCCEEDED(hr) && !surface) hr = E_FAIL;
        drop(texture);
        if (FAILED(hr)) break;
    }
    if (SUCCEEDED(hr))
        for (unsigned i = 0; i < count; ++i)
            for (unsigned j = 0; j < i; ++j) {
                ...
                if (FAILED(identity) || equal) hr = FAILED(identity) ? identity : E_INVALIDARG;
            }
    if (SUCCEEDED(hr)) {
        p.reset_targets();
        ...
        for (auto& surface : surfaces) drop(surface);
    return hr;
}
LinearEmissionPreparation LinearEmissionPass::begin_frame(std::uint64_t frame) noexcept {
    LinearEmissionPreparation out;
    if (!impl_ || !impl_->m) return out;
    auto& p = *impl_;
    if (p.phase != Impl::Phase::Idle || (p.has_frame && p.frame == frame)) return out;
    p.mask_valid = false;
    p.blocked = true;
    p.source_over = false;
    p.in_place = false;
    ...
    out.operation = p.fault(LinearEmissionPassFault::FrameClear) ? E_FAIL : p.clear(p.m);
...
LinearEmissionPreparation LinearEmissionPass::prepare(const LinearEmissionBoundary& boundary) noexcept {
    LinearEmissionPreparation out;
    if (!impl_) return out;
    auto& p = *impl_;
    if (!p.caps.enabled || !p.b || !p.has_frame || p.blocked || p.phase != Impl::Phase::Idle || !boundary.admitted ||
        !boundary.scene || !boundary.augmented || boundary.frame != p.frame)
        return out;
    if (boundary.policy != LinearCompositionPolicy::AdditiveEmission &&
    ...
    const auto policy = p.packed        ? LinearCompositionPolicy::PackedScreenInPlace
                        ...
                        : p.source_over ? LinearCompositionPolicy::DistanceFade
                                        : LinearCompositionPolicy::AdditiveEmission;
    if (!p.caps.supports(policy)) {
        out.operation = D3DERR_NOTAVAILABLE;
        return out;
    }
    ...
    if (SUCCEEDED(out.operation)) {
        ...
        if (p.fault(LinearEmissionPassFault::EmissionClear))
            out.operation = E_FAIL;
        else if (!p.fused_copy)
            out.operation = p.clear(p.e);
    }
    if (SUCCEEDED(out.operation) && p.packed) {
        ...
        if (SUCCEEDED(out.operation)) out.operation = p.restore(p.in_place ? boundary.scene : p.b);
        if (SUCCEEDED(out.operation)) out.operation = p.call(SetRt, DWORD(1), p.e);
        if (SUCCEEDED(out.operation)) out.operation = p.call(SetRt, DWORD(2), p.m);
    ...
    if (SUCCEEDED(out.operation)) out.operation = p.call(SetViewport, &p.saved.viewport);
    if (SUCCEEDED(out.operation)) out.operation = p.call(SetScissor, &p.saved.scissor);
    if (SUCCEEDED(out.operation)) {
        if (p.fault(LinearEmissionPassFault::SourceBind))
            out.operation = E_FAIL;
        else if (p.source_over)
            out.operation = p.call(SetVs, boundary.augmented_vertex);
    ...
    if (FAILED(out.operation)) {
        ...
        if (p.fault(LinearEmissionPassFault::Restore)) out.restore = E_FAIL;
        out.state_preserved = SUCCEEDED(out.restore);
...
LinearEmissionCompletion LinearEmissionPass::finish(HRESULT source) noexcept {
    if (!impl_ || impl_->phase != Impl::Phase::Prepared) return {};
    auto& p = *impl_;
    p.completion = {};
    ...
    if (p.in_place) {
        ...
        if (FAILED(source)) {
            ...
            p.completion.composition = p.fault(LinearEmissionPassFault::Composite)
                                           ? E_FAIL
                                           : p.draw(p.packed ? Impl::Program::PackedComposite
                                                             : Impl::Program::Composite,
        ...
        if (p.fault(LinearEmissionPassFault::Restore)) p.completion.restore = E_FAIL;
        if (FAILED(p.completion.restore)) p.completion.image = LinearEmissionImage::Incomplete;
        if (recover) {
    ...
    if (FAILED(source)) {
        ...
        p.completion.image = SUCCEEDED(p.completion.composition) ? LinearEmissionImage::Linear
...
IDirect3DSurface9** LinearEmissionPass::owning_candidate() noexcept {
    return impl_ && impl_->phase == Impl::Phase::Pending && impl_->completion.candidate_bound ? impl_->selected
                                                                                              : nullptr;
}
HRESULT LinearEmissionPass::acknowledge_exchange(bool exchanged) noexcept {
    if (!impl_ || impl_->phase != Impl::Phase::Pending || !impl_->selected) return D3DERR_INVALIDCALL;
    auto& p = *impl_;
    if (!exchanged) return *p.selected == p.selected_surface ? S_FALSE : E_INVALIDARG;
    if (!p.completion.candidate_bound || *p.selected != p.input_scene || *p.selected == p.selected_surface)
        return E_INVALIDARG;
    p.saved.release();
    p.phase = Impl::Phase::Idle;
    p.selected = nullptr;
...
LinearEmissionCompletion LinearEmissionPass::recover_native() noexcept {
    if (!impl_ || impl_->phase != Impl::Phase::Pending || impl_->recovered) return {};
    auto& p = *impl_;
    if (!p.selected || *p.selected != p.selected_surface) return {}; // ownership already exchanged
    p.recovered = true;
    p.selected = &p.b;
    p.selected_surface = p.b;
    p.completion.restore = p.restore(p.b);
    if (p.fault(LinearEmissionPassFault::RecoveryRestore)) p.completion.restore = E_FAIL;
    p.completion.candidate_bound = SUCCEEDED(p.completion.restore);
    p.completion.image = p.completion.candidate_bound && SUCCEEDED(p.completion.source)
                             ? LinearEmissionImage::Native
...
bool LinearEmissionPass::coverage_valid() const noexcept {
    return impl_ && impl_->phase == Impl::Phase::Idle && impl_->mask_valid && !impl_->blocked;
}
...
unsigned LinearEmissionPass::references() const noexcept {
    if (!impl_) return 0;
    const auto& p = *impl_;
    return !!p.b + !!p.e + !!p.c + !!p.m + !!p.pb + !!p.vs + !!p.copy + !!p.composite + !!p.declaration +
           !!p.source_over_composite + !!p.plane_init + !!p.packed_composite;
}
void LinearEmissionPass::before_reset() noexcept {
    if (impl_) impl_->reset_targets();
}
...
#ifdef X3M_LINEAR_EMISSION_PASS_FIXTURE
void LinearEmissionPass::inject(LinearEmissionPassFault f, unsigned count) noexcept {
    if (impl_) {
        impl_->fault_kind = f;
        impl_->fault_count = count;
    }
}
LinearEmissionCompletion LinearEmissionPass::fixture_completion() const noexcept {
