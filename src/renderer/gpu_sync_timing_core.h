...
enum Pass : unsigned {
    ...
    // Sub-passes inside Taa (TemporalPass::run), appended so the indices above stay: each pair adds its sync floor to
    // Taa.
    TaaCopy, // input copies: colour scratch / staging, depth StretchRect or copy draw (none when folded into
             // taa_mask_tests), depth decode, supplemental snapshot
    TaaMask, // the stabiliser mask draws (far / thin region, one or three full-screen draws; nests the three taa_mask_*
             // below)
    ...
    // Sub-passes inside FogRoute (FogPass::execute, stored path), appended likewise: each pair adds its sync floor to
    // FogRoute.
    ...
    // The three mask draws inside TaaMask (TemporalPass::run, one Span per draw), appended likewise: each pair adds its
    // sync floor to TaaMask.
    TaaMaskTests, // the per-pixel tests draw (thin region) or the far-only draw; with a two- or four-channel depth it
...
inline const char* pass_name(unsigned pass) noexcept {
    static constexpr const char* names[pass_count] = {
        "scene",         "engine",       "shadow_depth",   "sun_apply",   "retention",
...
struct Marks {
    ...
    // count at the frame's end; `area` is the draw's full pixel count. The first bracket per frame counts. Default:
    // nothing.
    ...
    virtual void needs_end(std::uint32_t area, std::uint32_t scale) noexcept {
        (void)area;
        (void)scale;
    }
...
class Span {
public:
    Span(Marks* marks, unsigned pass) noexcept
        : marks_(marks)
        , pass_(pass) {
        if (marks_) marks_->begin(pass_);
    }
    ~Span() {
        if (marks_) marks_->end(pass_);
    }
...
class CensusSpan {
public:
    CensusSpan(Marks* marks, std::uint32_t area) noexcept
        : marks_(marks)
        , area_(area) {
        if (marks_) marks_->census_begin();
    }
    ~CensusSpan() {
        if (marks_) marks_->census_end(area_);
    }
    CensusSpan(const CensusSpan&) = delete;
    CensusSpan& operator=(const CensusSpan&) = delete;
...
class NeedsSpan {
public:
    NeedsSpan(Marks* marks, std::uint32_t area, std::uint32_t scale) noexcept
        : marks_(marks)
        , area_(area)
        , scale_(scale) {
        if (marks_) marks_->needs_begin();
    }
    ~NeedsSpan() {
        if (marks_) marks_->needs_end(area_, scale_);
    }
    NeedsSpan(const NeedsSpan&) = delete;
    NeedsSpan& operator=(const NeedsSpan&) = delete;

private:
    Marks* marks_;
    std::uint32_t area_, scale_;
};
...
constexpr unsigned histogram_linear = 64, histogram_sub = 16,
                   histogram_buckets = histogram_linear + (32 - 6) * histogram_sub; // 480
...
inline std::uint32_t bucket_low(unsigned bucket) noexcept {
    ...
    const unsigned e = (bucket - histogram_linear) / histogram_sub + 6u,
                   sub = (bucket - histogram_linear) % histogram_sub;
...
struct Stat {
    std::uint32_t n = 0, median = 0, p90 = 0;
};
struct PassReport {
    Stat window, session;
    std::uint32_t wait_median = 0;
};
...
struct CensusReport {
    Stat ppm, px;
    std::uint32_t max_ppm = 0, max_px = 0, area = 0, pixels = 0, tag = 0, unread = 0, lost = 0, failed = 0;
};
...
class CensusWindow {
public:
    void pending(std::uint32_t pixels, std::uint32_t area, std::uint32_t tag, CensusRead read) noexcept {
        read_ = read;
        pixels_ = pixels;
        area_ = area;
        tag_ = tag;
    ...
    void file() noexcept {
        if (read_ == CensusNotReady)
            ++unread_;
        else if (read_ == CensusLost)
            ++lost_;
        else if (read_ == CensusFailed)
            ++failed_;
            ...
            max_px_ = std::max(max_px_, pixels_);
            last_area_ = area_;
            last_pixels_ = pixels_;
            last_tag_ = tag_;
        }
        read_ = CensusNone;
    }
    ...
    template <class Exact> CensusReport take(Exact exact) noexcept {
        CensusReport r;
        r.ppm = exact(ppm_, count_);
        r.px = exact(px_, count_);
        r.max_ppm = max_ppm_;
        r.max_px = max_px_;
        r.area = last_area_;
        r.pixels = last_pixels_;
        r.tag = last_tag_;
        r.unread = unread_;
        r.lost = lost_;
        r.failed = failed_;
...
private:
    ...
    std::uint32_t ppm_[window_frames_max]{}, px_[window_frames_max]{}, count_ = 0, max_ppm_ = 0, max_px_ = 0,
                                                                       last_area_ = 0, last_pixels_ = 0, last_tag_ = 0;
    std::uint32_t unread_ = 0, lost_ = 0, failed_ = 0;
};
struct Report {
    std::uint64_t window = 0, first_frame = 0, last_frame = 0;
    std::uint32_t frames = 0, dropped = 0, unclosed = 0; // frames in the window; frames dropped by a failed sync;
                                                         // passes open at a frame end
...
class Tracker {
public:
    void configure(unsigned window) noexcept {
        window_ = window < 1 ? 1u : window > window_frames_max ? window_frames_max : window;
    }
    ...
    void census(std::uint32_t pixels, std::uint32_t area, CensusRead read) noexcept {
        census_.pending(pixels, area, 0, read);
    }
    // The frame's needs-repair count at march spacing `scale`, filed like census().
    void needs(std::uint32_t pixels, std::uint32_t area, std::uint32_t scale, CensusRead read) noexcept {
        needs_.pending(pixels, area, scale, read);
    }
    ...
    bool frame(std::uint64_t frame_index, std::uint64_t stamp, std::uint64_t frequency) noexcept {
        ...
        if (abandoned_)
            ...
            for (unsigned pass = 0; pass < pass_count; ++pass) {
                ...
                samples_[pass][count_[pass]] = us;
                waits_[pass][count_[pass]] = ticks_to_us(wait_[pass], frequency);
                ++count_[pass];
                ++hist_[pass][bucket_of(us)];
                ++session_n_[pass];
            }
            census_.file();
            needs_.file();
    ...
    Report report() noexcept {
        Report r = summary();
        r.window = ++windows_;
        r.first_frame = first_frame_;
        r.last_frame = last_frame_;
        r.frames = frames_;
        r.dropped = dropped_;
        r.unclosed = unclosed_;
        for (unsigned pass = 0; pass < pass_count; ++pass) {
        ...
        r.census = census_.take(&Tracker::exact);
        r.needs = needs_.take(&Tracker::exact);
    ...
    Report summary() const noexcept {
        ...
        r.last_frame = last_frame_;
        for (unsigned pass = 0; pass < pass_count; ++pass)
            r.pass[pass].session = histogram(hist_[pass], session_n_[pass]);
...
private:
    void clear_marks() noexcept {
        open_ = begun_ = closed_ = 0;
        abandoned_ = false;
        census_.drop_pending();
        needs_.drop_pending();
    ...
    static Stat exact(std::uint32_t* values, std::uint32_t n) noexcept {
        ...
        s.median = values[rank_of(n, 50) - 1];
        s.p90 = values[rank_of(n, 90) - 1];
        return s;
    }
    static Stat histogram(const std::uint32_t* buckets, std::uint32_t n) noexcept {
        ...
        for (unsigned b = 0; b < histogram_buckets; ++b) {
            seen += buckets[b];
            if (!median && seen >= r50) {
                s.median = bucket_value(b);
                median = true;
            }
            if (seen >= r90) {
                s.p90 = bucket_value(b);
                break;
            }
        }
        return s;
    ...
    std::uint32_t samples_[pass_count][window_frames_max]{}, waits_[pass_count][window_frames_max]{},
        count_[pass_count]{};
