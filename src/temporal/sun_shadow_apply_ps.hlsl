...
// the same frame's cascade-0 depth replay map. Per pixel: RT2 (A32B32G32R32F:
// .r = device z/w with the -1 sentinel, .g = share, .b = the interpolated
// clip w) point-sampled at the pixel; the view depth is z = .b itself
// (docs/architecture/shadow-receiver-depth.md; the z/w law on .r is gone,
// one fp32 ULP of z/w is 4-85 u at 20-90 km); NDC from the quad
...
float4 terms : register(c1);    // x = m22, y = m32 (uploaded, unread: the receiver depth is .b), z = exponent, w = constant bias (normalized sun depth)
...
float4 limits : register(c6);   // x = receiver-plane bias clamp and non-planar fallback, y = relative view-depth step that voids the plane fit, z, w = 0 (reserved)
