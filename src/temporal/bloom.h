...
inline bool valid_bloom_params(const BloomParams& p) noexcept {
    return p.levels >= 1 && p.levels <= kBloomMaxLevels && std::isfinite(p.strength) && p.strength >= 0.f &&
           p.strength <= kBloomMaxStrength && std::isfinite(p.threshold) && p.threshold >= 0.f &&
           p.threshold <= kAgxClampOff && std::isfinite(p.knee) && p.knee >= 0.f && p.knee <= 1.f &&
           std::isfinite(p.scatter) && p.scatter >= 0.f && p.scatter <= 1.f && std::isfinite(p.authored_glow_gain) &&
           p.authored_glow_gain >= 0.f && p.authored_glow_gain <= kBloomMaxAuthoredGlowGain &&
...
inline bool select_bloom_extract(BloomExtractShader& out, BloomSize size, AgxDecode decode) noexcept {
    if (!valid_bloom_size(size) ||
        (decode != AgxDecode::gamma22 && decode != AgxDecode::srgb && decode != AgxDecode::none))
        return false;
...
inline bool prepare_bloom(BloomConstants& out, BloomSize source, BloomSize destination, const BloomParams& p,
                          float exposure, float clamp_max, AgxDecode decode) noexcept {
    ...
    BloomConstants next{};
    next.source[0] = float(source.width);
    next.source[1] = float(source.height);
    next.source[2] = 1.f / source.width;
    next.source[3] = 1.f / source.height;
    next.destination[0] = float(destination.width);
    next.destination[1] = float(destination.height);
    next.destination[2] = 1.f / destination.width;
    next.destination[3] = 1.f / destination.height;
    next.filter[0] = p.threshold;
    next.filter[1] = p.knee;
    next.filter[2] = p.scatter;
    next.filter[3] = p.strength;
