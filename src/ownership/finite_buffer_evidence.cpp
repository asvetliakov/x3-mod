...
void add(std::uint64_t& counter,std::uint64_t amount) noexcept {
    counter=amount>std::numeric_limits<std::uint64_t>::max()-counter
        ?std::numeric_limits<std::uint64_t>::max():counter+amount;
}
...
bool same(const PositionEvidenceRange& a,const PositionEvidenceRange& b) noexcept {
    return a.stream_offset==b.stream_offset&&a.stride==b.stride&&
        a.position_offset==b.position_offset&&a.first_vertex==b.first_vertex&&
        a.vertex_count==b.vertex_count&&a.storage==b.storage;
}
}
bool FiniteBufferEvidence::required_payload(EvidenceBufferKind kind,
        std::uint64_t bytes,std::size_t* result) noexcept {
    if(result)*result=0;
    ...
    *result=static_cast<std::size_t>(payload);return true;
}
bool FiniteBufferEvidence::initialize(EvidenceBufferKind kind,std::uint64_t bytes,
                                     std::size_t budget) noexcept {
    reset();std::size_t payload=0;
    ...
    payload_bytes_=payload;byte_size_=bytes;kind_=kind;return true;
}
void FiniteBufferEvidence::reset() noexcept {
    atlas_.reset();payload_bytes_=0;byte_size_=0;kind_=EvidenceBufferKind::Unknown;
    revision_=high_revision_=0;pending_=staged_=false;pending_revision_=offset_=length_=0;
    indices_known_=staged_indices_known_=false;index_minimum_=index_maximum_=0;
    cache_valid_=false;cache_revision_=0;cache_range_={};cache_result_=FiniteStatus::Unknown;
    counters_={};
}
void FiniteBufferEvidence::invalidate(std::uint64_t revision) noexcept {
    ...
    revision_=0;pending_=staged_=false;pending_revision_=offset_=length_=0;
    indices_known_=staged_indices_known_=false;index_minimum_=index_maximum_=0;
    cache_valid_=false;
}
...
bool FiniteBufferEvidence::begin_write(std::uint64_t revision,std::uint64_t offset,
        std::uint64_t length,EvidenceWriteMode mode) noexcept {
    if(pending_||kind_==EvidenceBufferKind::Unknown||mode!=EvidenceWriteMode::Preserving||
       !revision||revision<=high_revision_||offset>byte_size_||
       (!length&&offset)||length>byte_size_-offset){invalidate(revision);return false;}
    if(!length)length=byte_size_; // Only (0,0) reaches this normalization.
    if(!length){invalidate(revision);return false;}
    ...
    high_revision_=revision;pending_=true;staged_=false;
    pending_revision_=revision;offset_=offset;length_=length;
    cache_valid_=false;indices_known_=staged_indices_known_=false;
...
bool FiniteBufferEvidence::stage_mapped(std::uint64_t revision,const void* bytes,
    ...
    if(!pending_||staged_||revision!=pending_revision_||!bytes||
       mapped_length!=length_||mapped_length>std::numeric_limits<std::size_t>::max()){
        invalidate(revision);return false;
    }
    ...
    if(kind_==EvidenceBufferKind::Vertex){
        ...
        index_minimum_=minimum;index_maximum_=maximum;staged_indices_known_=true;
...
bool FiniteBufferEvidence::finish_write(std::uint64_t revision,bool success) noexcept {
    if(!success||!pending_||!staged_||revision!=pending_revision_){invalidate(revision);return false;}
    revision_=revision;indices_known_=staged_indices_known_;pending_=staged_=false;
    pending_revision_=offset_=length_=0;staged_indices_known_=false;return true;
}
FiniteStatus FiniteBufferEvidence::query_positions(std::uint64_t revision,
    ...
    if(cache_valid_&&cache_revision_==revision&&same(cache_range_,range)){
        add(counters_.cache_hits,1);return cache_result_;
    }
    const std::uint32_t width=range.storage==PositionStorage::Float3?12:
                              range.storage==PositionStorage::Half4?8:0;
    ...
    if(!width||!range.vertex_count||range.first_vertex<0||range.stride<width||
       ...
       range.position_offset>byte_size_-range.stream_offset)return FiniteStatus::Unknown;
    const auto base=range.stream_offset+range.position_offset;
    if(base%alignment||range.stride%alignment||width>byte_size_-base)
        return FiniteStatus::Unknown;
    ...
    bool unknown=false;FiniteStatus result=FiniteStatus::Finite;
    for(std::uint64_t vertex=0;vertex<range.vertex_count;++vertex){
        ...
        for(unsigned component=0;component<3;++component){
            ...
            const auto bits=cell(at/4);add(counters_.position_components,1);
            if(!(bits&known))unknown=true;
            else if(!(bits&(range.storage==PositionStorage::Float3?float_finite:
                            at%4==0?half_low:half_high))){result=FiniteStatus::NonFinite;break;}
        }
        if(result==FiniteStatus::NonFinite)break;
    }
    if(result!=FiniteStatus::NonFinite&&unknown)result=FiniteStatus::Unknown;
    cache_valid_=true;cache_revision_=revision;cache_range_=range;cache_result_=result;
    return result;
}
IndexEvidenceBounds FiniteBufferEvidence::query_indices(std::uint64_t revision,
        std::uint64_t first,std::uint64_t count) const noexcept {
    if((kind_!=EvidenceBufferKind::Index16&&kind_!=EvidenceBufferKind::Index32)||
       pending_||!revision||revision!=revision_||!indices_known_||!count)return {};
...
bool FiniteBufferEvidence::indexed_vertex_range(const IndexEvidenceBounds& bounds,
        std::int64_t base,std::uint64_t minimum,std::uint64_t count,
        std::int64_t* first) noexcept {
    if(first)*first=0;
    if(!first||!bounds.known||bounds.minimum>bounds.maximum||!count||
       minimum>bounds.minimum||count-1>std::numeric_limits<std::uint64_t>::max()-minimum||
       bounds.maximum>minimum+count-1)return false;
