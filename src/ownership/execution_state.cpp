...
void ObservedExecutionState::initialize(bool enabled) noexcept {
    ...
    if (initialized_) { poison(ExecutionReason::NativeBypass, true); return; }
...
ExecutionView ObservedExecutionState::view() const noexcept {
    ...
    std::lock_guard<std::mutex> lock(mutex_);
    ExecutionView v;
    v.requested = requested_; v.known = requested_ && healthy_ && !resetting_ && !query_tainted_ && in_flight_ == 0;
    v.scene_open = scene_; v.stateblock_recording = recording_;
    v.queries_idle = v.known && active_ == 0;
    v.generation = generation_; v.active_queries = active_; v.in_flight = in_flight_;
...
void ObservedExecutionState::observe_result_locked(std::int32_t result) noexcept {
    if (device_lost(result)) {
        if (loss_epoch_ == std::numeric_limits<std::uint64_t>::max()) poison(ExecutionReason::CounterOverflow, true);
        else ++loss_epoch_;
...
void ObservedExecutionState::begin_scene(std::int32_t result) noexcept { if (!requested_.load(std::memory_order_relaxed)) return; std::lock_guard<std::mutex> lock(mutex_); transition(scene_, true, result); }
void ObservedExecutionState::end_scene(std::int32_t result) noexcept { if (!requested_.load(std::memory_order_relaxed)) return; std::lock_guard<std::mutex> lock(mutex_); transition(scene_, false, result); }
void ObservedExecutionState::begin_stateblock(std::int32_t result) noexcept { if (!requested_.load(std::memory_order_relaxed)) return; std::lock_guard<std::mutex> lock(mutex_); transition(recording_, true, result); }
void ObservedExecutionState::end_stateblock(std::int32_t result) noexcept { if (!requested_.load(std::memory_order_relaxed)) return; std::lock_guard<std::mutex> lock(mutex_); transition(recording_, false, result); }
void ObservedExecutionState::query_created(ExecutionQuery& q, std::uint32_t type) noexcept {
    ...
    if (q.owner_.load() != this || q.registered_) { poison(ExecutionReason::UnknownQuery, true); return; }
    q.generation_ = generation_; q.registered_ = true;
...
void ObservedExecutionState::before_reset() noexcept {
    ...
    resetting_ = true; healthy_ = false; reset_loss_epoch_ = loss_epoch_;
...
void ObservedExecutionState::after_reset(std::int32_t result) noexcept {
    ...
    if (loss_epoch_ != reset_loss_epoch_) { poison(ExecutionReason::DeviceLost, true); return; }
...
void ObservedExecutionState::unknown_native_execution() noexcept { if (!requested_.load(std::memory_order_relaxed)) return; std::lock_guard<std::mutex> lock(mutex_); poison(ExecutionReason::NativeBypass, true); }
ExecutionObservation::ExecutionObservation(ObservedExecutionState& owner) noexcept : owner_(&owner) { owner_->start_native(*this); }
ExecutionObservation::~ExecutionObservation() { owner_->finish_native(*this); }
void ExecutionObservation::complete() noexcept { owner_->complete_native(*this); }
ExecutionObservation ObservedExecutionState::begin_native() noexcept { return ExecutionObservation(*this); }
...
void ObservedExecutionState::finish_native(ExecutionObservation& call) noexcept {
    ...
    if (!in_flight_) { poison(ExecutionReason::OverlappingNativeCalls, true); return; }
    --in_flight_;
}
void ObservedExecutionState::complete_native(ExecutionObservation& call) noexcept {
    ...
    std::lock_guard<std::mutex> lock(mutex_);
    if (!requested_) return;
    if (!call.active_ || call.completed_) { poison(ExecutionReason::NativeBypass, true); return; }
