...
struct BufferLockObservation {
    ...
    template<class T> void increment(T& value) noexcept {
        if(value==std::numeric_limits<T>::max())saturated=true;
        else ++value;
    }
    void begin_lock(std::uint32_t offset,std::uint32_t size,std::uint32_t flags,
                    std::uint32_t thread) noexcept {
        increment(attempt_serial);increment(in_flight_locks);
        ...
        last_offset=offset;last_size=size;last_flags=flags;last_thread=thread;
    }
    void begin_unlock(std::uint32_t thread=0) noexcept {
        increment(unlock_serial);increment(in_flight_unlocks);last_unlock_thread=thread;
    }
    void complete_lock(bool success) noexcept {
        if(!success)increment(failed_locks);
        if(in_flight_locks)--in_flight_locks;else ambiguous=true;
    }
    void complete_unlock(bool success) noexcept {
        if(!success)increment(failed_unlocks);
        if(in_flight_unlocks)--in_flight_unlocks;else ambiguous=true;
    }
    bool quiet() const noexcept {
        return allocation_id&&!saturated&&!ambiguous&&!pending_locks&&
            !in_flight_locks&&!in_flight_unlocks;
    }
};
} // namespace x3m::ownership
