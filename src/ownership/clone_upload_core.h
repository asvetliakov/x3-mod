...
struct Shape {
    std::uint32_t vertices, faces, stride;
};
...
constexpr std::uint32_t bytes_for(unsigned slot, Buffer kind) noexcept {
    return kind == Buffer::Vertex ? shapes[slot].vertices * shapes[slot].stride : shapes[slot].faces * 6u;
}
struct Identity {
...
class Store final {
    ...
    bool device(const DeviceGuard& g) const noexcept {
        return g.owner && g.generation && g.owner == beginning_.owner && g.generation == beginning_.generation &&
               g.tracking && !g.resetting && !g.lost && !g.retiring;
    }
    bool current(std::uint64_t invocation, const DeviceGuard& g) noexcept {
        ...
        if (!device(g)) {
            refuse(Refusal::Device);
            return false;
        }
        if (!g.thread || g.thread != beginning_.thread) {
            refuse(Refusal::Thread);
            return false;
        }
        return true;
    }
...
public:
    ...
    ~Store() {
        for (unsigned s = 0; s < pair_count; ++s) wipe(s);
    }
    std::uint64_t begin(unsigned slot, const DeviceGuard& g, Identity vertex_source, Identity index_source) noexcept {
        if (active_) {
            refuse(Refusal::Busy);
            return 0;
        }
        ...
        if (pairs_[slot].record.producer_payload_valid || duplicate_[slot]) {
            wipe(slot);
            duplicate_[slot] = true;
            reason_ = Refusal::Busy;
            ++stats_.refusals;
            return 0;
        }
        if (serial_ == std::numeric_limits<std::uint64_t>::max()) {
            reason_ = Refusal::Exhausted;
            ++stats_.refusals;
            return 0;
        }
        wipe(slot);
    ...
    bool created(std::uint64_t invocation, Buffer kind, const DeviceGuard& g, const Creation& c) noexcept {
        if (!current(invocation, g)) return false;
        auto& p = parts_[index(kind)];
        const auto id = c.identity.allocation;
        if (p.created || !id || c.identity.revision || id == sources_[0].allocation || id == sources_[1].allocation ||
            (parts_[1 - index(kind)].created && id == parts_[1 - index(kind)].creation.identity.allocation)) {
            refuse(Refusal::Allocation);
            return false;
        }
        ...
        if (!c.managed || (c.actual_usage & (8u | 0x200u)) || !c.own_dispatch || c.bytes != bytes_for(slot_, kind) ||
            (kind == Buffer::Index && !c.index16)) {
            refuse(Refusal::Contract);
            return false;
        }
        p.creation = c;
        p.created = true;
    ...
    bool source_lock(std::uint64_t invocation, Buffer kind, const DeviceGuard& g, Identity source, std::uint32_t offset,
                     std::uint32_t size, std::uint32_t flags, bool success) noexcept {
        if (!current(invocation, g)) return false;
        auto& p = parts_[index(kind)];
        if (!success || p.source_lock || !(source == sources_[index(kind)]) || offset || size || flags != 0x810u) {
            refuse(Refusal::Source);
            return false;
        }
        p.source_lock = true;
    ...
    bool source_unlock(std::uint64_t invocation, Buffer kind, const DeviceGuard& g, Identity source,
        ...
        p.source_closed = true;
        return true;
    }
    bool mapped(std::uint64_t invocation, Buffer kind, const DeviceGuard& g, Identity id, const void* pointer,
                std::uint64_t lock_serial, std::uint32_t offset, std::uint32_t size, std::uint32_t flags,
                bool success) noexcept {
        if (!current(invocation, g)) return false;
        auto& p = parts_[index(kind)];
        if (!success || !p.created || p.mapped || !pointer || !lock_serial ||
            id.allocation != p.creation.identity.allocation || id.revision != 1 || offset || size || flags != 0x800u) {
            refuse(Refusal::Mapping);
            return false;
        }
        p.written = id;
        p.mapping = pointer;
        p.lock_serial = lock_serial;
        p.mapped = true;
        return true;
    }
    bool stage(std::uint64_t invocation, Buffer kind, const MapGuard& g) noexcept {
        ...
        if (!p.mapped || p.staged || p.closed || !g.authenticated || !g.own_dispatch || g.ambiguous ||
            !(g.identity == p.written) || g.pointer != p.mapping || g.lock_serial != p.lock_serial || g.pending != 1 ||
            g.in_flight_locks || g.in_flight_unlocks != 1) {
            refuse(Refusal::Mapping);
            return false;
        }
        ...
        if (used > pair_capacity) {
            refuse(Refusal::Exhausted);
            return false;
        }
        ...
        stats_.staged_bytes += n;
        p.staged = true;
        return true;
    }
    bool unlocked(std::uint64_t invocation, Buffer kind, const DeviceGuard& g, Identity id, bool exact_s_ok,
                  bool quiet) noexcept {
        if (!current(invocation, g)) return false;
        auto& p = parts_[index(kind)];
        p.mapping = nullptr;
        if (!exact_s_ok || !quiet || !p.staged || p.closed || !(id == p.written)) {
            refuse(Refusal::Unlock);
            return false;
        }
        p.closed = true;
        return true;
    ...
    bool finish(std::uint64_t invocation, bool clone_succeeded, const FinalGuard& g) noexcept {
        ...
        if (accepted && !clone_succeeded) {
            refuse(Refusal::Clone);
            accepted = false;
        }
        if (accepted && (!g.authenticated || !g.quiet || !g.own_dispatch || !parts_[0].closed || !parts_[1].closed ||
                         !parts_[0].source_closed || !parts_[1].source_closed || !(g.buffers[0] == parts_[0].written) ||
                         !(g.buffers[1] == parts_[1].written))) {
            refuse(Refusal::Linkage);
            accepted = false;
        }
        if (accepted) {
            auto& r = pairs_[slot_].record;
            r.invocation = active_;
            r.owner = beginning_.owner;
            r.generation = beginning_.generation;
            for (unsigned i = 0; i < 2; ++i) {
                r.buffers[i] = parts_[i].written;
                r.bytes[i] = parts_[i].creation.bytes;
            }
            r.producer_payload_valid = true;
            ++stats_.publications;
        } else
            wipe(slot_);
        for (auto& p : parts_) p.mapping = nullptr;
    ...
    void invalidate(std::uint64_t allocation) noexcept {
        for (unsigned s = 0; s < pair_count; ++s)
            if (pairs_[s].record.buffers[0].allocation == allocation ||
                pairs_[s].record.buffers[1].allocation == allocation)
                wipe(s);
        if (active_ && (parts_[0].creation.identity.allocation == allocation ||
                        parts_[1].creation.identity.allocation == allocation))
            refuse(Refusal::Mutation);
    }
    void reset() noexcept {
    ...
    Record record(unsigned slot) const noexcept { return slot < pair_count ? pairs_[slot].record : Record{}; }
    bool copy_retained(unsigned slot, Buffer kind, const FinalGuard& g, void* output,
                       std::size_t capacity) const noexcept {
        if (slot >= pair_count || !output) return false;
        const auto& r = pairs_[slot].record;
        if (!r.producer_payload_valid || !g.authenticated || !g.quiet || !g.own_dispatch || !g.device.tracking ||
            g.device.resetting || g.device.lost || g.device.retiring || g.device.owner != r.owner ||
            g.device.generation != r.generation || !(g.buffers[0] == r.buffers[0]) || !(g.buffers[1] == r.buffers[1]) ||
            capacity < r.bytes[index(kind)])
            return false;
        std::memcpy(output, arena_.data() + offset(slot, kind), r.bytes[index(kind)]);
        return true;
    }
    ...
    bool copy_pair(unsigned slot, const FinalGuard& g, void* vertex, std::size_t vertex_capacity, void* indices,
                   std::size_t index_capacity, Record& result) const noexcept {
        if (slot >= pair_count || !vertex || !indices) return false;
        const auto& r = pairs_[slot].record;
        if (!r.producer_payload_valid || !g.authenticated || !g.quiet || !g.own_dispatch || !g.device.tracking ||
            g.device.resetting || g.device.lost || g.device.retiring || g.device.owner != r.owner ||
            g.device.generation != r.generation || !(g.buffers[0] == r.buffers[0]) || !(g.buffers[1] == r.buffers[1]) ||
            vertex_capacity < r.bytes[0] || index_capacity < r.bytes[1])
            return false;
