...
public:
    ...
    bool initialize(EvidenceBufferKind kind, std::uint64_t byte_size, std::size_t payload_budget) noexcept;
    ...
    static bool required_payload(EvidenceBufferKind kind, std::uint64_t byte_size, std::size_t* bytes) noexcept;
    ...
    bool begin_write(std::uint64_t revision, std::uint64_t offset, std::uint64_t length,
    ...
    bool stage_mapped(std::uint64_t revision, const void* bytes, std::uint64_t mapped_length) noexcept;
    ...
    FiniteStatus query_positions(std::uint64_t revision, const PositionEvidenceRange& range) noexcept;
    IndexEvidenceBounds query_indices(std::uint64_t revision, std::uint64_t first_index,
    ...
    static bool indexed_vertex_range(const IndexEvidenceBounds& bounds, std::int64_t base_vertex,
                                     std::uint64_t minimum_vertex, std::uint64_t vertex_count,
