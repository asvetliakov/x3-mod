#pragma once
// The x3m.ini grammar (docs/architecture/config-file.md, section 1): whole-line ';' / '#' comments, [section] lines that
// only group, `key = value` with key [A-Za-z0-9_-]+ (lower-cased, '-' read as '_', the environment name without X3M_),
// the value trimmed with one matching pair of double quotes removed, no escapes and no trailing comments. Bounds: 64 KiB
// per file (the caller refuses a larger one), 512 bytes per line (a longer line is refused alone); a line holding a NUL
// byte is refused alone (problem=nul), never cut at it. The value is checked against the generated schema (type, the
// site's accepted intervals, per-position element ranges, list counts, choices); a bool also takes on/off/true/false/yes/no and is
// normalised to 1/0, a list loses the blanks around its commas; every other value reaches the read site as written. An
// empty value is invalid except for a string or path and a key whose default is empty.
// Pure C++17 with no Windows header, no allocation and no CRT number parsing (numbers are read here with plain double
// arithmetic, SSE2 in the DLL): shared by the DLL's resolver (src/proxy/config.cpp) and the host test
// (verification/analysis/test_config_schema.py). The text is parsed in place: keys and values are NUL-terminated inside it.
...
struct Result {
    const char* value[schema::entry_count] = {}; // the checked value (NUL-terminated inside the text), nullptr = not set
    unsigned line[schema::entry_count] = {};
    unsigned keys = 0;                            // entries the file sets
    unsigned unknown = 0, invalid = 0, duplicate = 0, env_only = 0, renamed = 0; // invalid includes too_long, syntax and nul
    unsigned lines = 0;
...
template <class Char>
inline int compare(const Char* a, const char* b) noexcept {
    for (;; ++a, ++b) {
...
template <class Char>
inline int find(const Char* env) noexcept {
    if (!env) return -1;
...
inline char lower(char c) noexcept { return c >= 'A' && c <= 'Z' ? char(c - 'A' + 'a') : c; }
inline bool equals_folded(const char* a, const char* word) noexcept { // case-insensitive, word lower-case
    for (; *word; ++a, ++word) if (lower(*a) != *word) return false;
    return *a == 0;
}
...
// [+-]digits[.digits][(e|E)[+-]digits] (at least one mantissa digit; integer: digits only after the sign). Returns the end
// of the number or nullptr.
inline const char* number(const char* p, bool integer, double* out) noexcept {
    ...
    if (!integer && *p == '.') {
        double scale = 0.1;
        for (++p; *p >= '0' && *p <= '9'; ++p, ++digits) { value += double(*p - '0') * scale; scale *= 0.1; }
    }
    ...
    if (!integer && (*p == 'e' || *p == 'E')) {
        ...
        for (; *p >= '0' && *p <= '9'; ++p, ++exponent_digits) if (exponent < 1000) exponent = exponent * 10 + unsigned(*p - '0');
...
inline bool in_range(const schema::Entry& e, double v) noexcept { return in_intervals(e.range_first, e.range_count, v); }
...
inline bool check(const schema::Entry& e, char* value) noexcept {
    ...
    switch (e.type) {
    case Type::Bool: {
        const bool on = !value[1] && value[0] == '1' ? true : equals_folded(value, "on") || equals_folded(value, "true") || equals_folded(value, "yes");
        const bool off = !value[1] && value[0] == '0' ? true : equals_folded(value, "off") || equals_folded(value, "false") || equals_folded(value, "no");
        if (!on && !off) return false;
        value[0] = on ? '1' : '0'; value[1] = 0;
        return true;
    }
    case Type::Int:
    case Type::Float: {
...
inline void note(Result& r, unsigned line, Problem problem, const char* key, const char* value, int entry) noexcept {
    if (r.issue_count < issue_limit) r.issues[r.issue_count++] = Issue{line, problem, key, value, entry};
    else ++r.more;
}
inline bool blank(char c) noexcept { return c == ' ' || c == '\t' || c == '\r' || c == '\v' || c == '\f'; }
inline bool key_char(char c) noexcept { return (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || (c >= '0' && c <= '9') || c == '_' || c == '-'; }
...
inline int find_key(const char* key, bool* renamed) noexcept {
    ...
    *renamed = false;
    ...
    for (unsigned a = 0; a < schema::alias_count; ++a)
        if (schema::aliases[a].key && compare(key, schema::aliases[a].key) == 0) { *renamed = true; return int(schema::aliases[a].entry); }
    return -1;
}
// Where an entry's value comes from when the environment does not set it (config::get, docs/architecture/config-file.md
// section 3): the file, else the profile's base (the default, or the off value under X3M_CONFIG=bare). A launcher default
// marker (marker_of >= 0) takes its default only while the key it marks itself comes from the defaults: when the file or
// the environment sets that key the marker is absent. file_sets(i) and in_environment(i) answer for entry i.
...
inline const char* base_value(int i, bool bare) noexcept { return bare ? schema::entries[i].off_value : schema::entries[i].default_value; }
...
inline void parse(char* text, unsigned size, Result& r) noexcept {
    ...
    char* at = text;
    if (size >= 3 && static_cast<unsigned char>(at[0]) == 0xef && static_cast<unsigned char>(at[1]) == 0xbb && static_cast<unsigned char>(at[2]) == 0xbf) at += 3;
    for (unsigned n = 1; at < limit; ++n) {
        ...
        *end = 0;
        r.lines = n;
        if (unsigned(end - line) > line_limit) { ++r.invalid; note(r, n, Problem::too_long, "", "", -1); continue; }
        {   // A NUL inside the line (end is where this line's '\n' was): the line is refused, never read up to the NUL.
            const char* scan = line;
            while (scan < end && *scan) ++scan;
            if (scan < end) { ++r.invalid; note(r, n, Problem::nul, "", "", -1); continue; }
        }
        ...
        *stop = 0;
        ...
        if (*line == '[') {
            if (stop[-1] != ']') { ++r.invalid; note(r, n, Problem::syntax, "", line, -1); }
            continue;
        }
        char* equals = line;
        ...
        if (!key_ok) { ++r.invalid; note(r, n, Problem::syntax, "", line, -1); continue; }
        *key_end = 0;
        ...
        if (value_end - value >= 2 && *value == '"' && value_end[-1] == '"') { ++value; *--value_end = 0; }
        bool renamed = false;
        const int i = find_key(line, &renamed);
        if (i < 0) { ++r.unknown; note(r, n, Problem::unknown, line, value, -1); }
        else if (schema::entries[i].flags & schema::env_only) { ++r.env_only; note(r, n, Problem::env_only, line, value, i); }
        else if (!check(schema::entries[i], value)) { ++r.invalid; note(r, n, Problem::invalid, line, value, i); }
        else {
            if (renamed) { ++r.renamed; note(r, n, Problem::renamed, line, schema::entries[i].key, i); }
            if (r.value[i]) { ++r.duplicate; note(r, n, Problem::duplicate, line, value, i); }
            else ++r.keys;
            r.value[i] = value;
            r.line[i] = n;
        }
