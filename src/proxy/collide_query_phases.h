...
bool shutdown();
namespace detail {
void begin(core::Kind kind);
void end();
void invalidate();
void foreign();
}
// Off: one predictable flag test, no function call and no clock read.
inline void begin(core::Kind kind = core::Miss) {
    if (enabled) detail::begin(kind);
}
inline void end() {
    if (enabled) detail::end();
}
inline void invalidate() {
    if (enabled) detail::invalidate();
}
inline void foreign() {
    if (enabled) detail::foreign();
}
