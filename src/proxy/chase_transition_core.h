...
struct State {
    ...
    Lifetime* find(std::uintptr_t cockpit) noexcept {
        ...
        for (auto& l : lives)
            if (l.cockpit == cockpit) return &l;
        return nullptr;
    }
    void revoke(std::uintptr_t cockpit) noexcept {
        for (auto& t : threads)
            if (t.cockpit == cockpit) {
                t.cockpit = 0;
                t.frame = 0;
                t.token = {};
            }
    ...
    std::uint64_t construct(std::uintptr_t cockpit, std::uint32_t thread) noexcept {
        ...
        *l = {cockpit, ++next_generation, thread, false};
        return l->generation;
    }
    ...
    void destroy(std::uintptr_t cockpit) noexcept {
        revoke(cockpit);
        if (auto* l = find(cockpit)) *l = {};
    }
    std::uint64_t generation(std::uintptr_t cockpit) noexcept {
        auto* l = find(cockpit);
        return l && l->complete ? l->generation : 0;
    }
    ...
    Update current(std::uintptr_t cockpit, std::uint32_t id) noexcept {
        for (auto& t : threads)
            if (t.id == id && t.cockpit == cockpit && t.token.generation && t.token.generation == generation(cockpit))
                return t.token;
        return {};
    }
};
...
void provenance(std::uint32_t ebp, Origin& o, Reader bytes, CodeRange code_address,
    ...
    auto code_offset = [&](std::uint32_t code, std::uint32_t offset) {
        std::uintptr_t at = 0;
        unsigned char op = 0;
        return code_address(code, offset, 1, at) && field(at, 0, op);
    };
    std::uint32_t vm = 0, code = 0, command = 0, return_pc = 0;
    if (expected_command != 0x30 && expected_command != 1) {
        o.flags |= 1;
        return;
    }
    if (!field(ebp, 4, return_pc) || return_pc != 0x4a3909 || !field(ebp, 0xc, o.task) || !field(ebp, 0x10, command) ||
        command != expected_command || !field(vm_root, 0, vm) || !field(vm, 8, code) || !field(o.task, 0x1c, o.pc) ||
        !field(o.task, 0x3c, o.context)) {
        o.flags |= 1;
        return;
    }
    o.valid |= 1;
    std::uintptr_t at = 0;
    unsigned char op[5]{};
    if (o.pc < 5 || !code_address(code, o.pc - 5, 5, at) || !bytes(at, 0, op, 5)) {
        o.flags |= 1;
        return;
    }
    ...
    if (op[0] != 0x82 || cmd != expected_command || group > (0x1454 / 24) - 3 ||
        !field(vm, (group + 2) * 24, dispatch) || dispatch != 0x42d340) {
        o.flags |= 1;
        return;
    }
    o.valid |= 2;
    ...
    if (o.context && !(o.context & 3) && field(o.context, 0, o.context_word0))
        o.valid |= 4;
    else
        o.flags |= 1;
    std::uint32_t base = 0, capacity = 0;
    std::int32_t index = 0;
    if (!field(o.task, 0x14, base) || !field(o.task, 0x10, capacity) || !field(o.task, 0x18, index) || index > 0 ||
        capacity > 1048576 || index < -std::int32_t(capacity)) {
        o.flags |= 1;
        return;
    }
    ...
    if (cells && !bytes(base - used * 5, 0, stack, cells * 5)) {
        o.flags |= 1;
        return;
    }
    o.valid |= 8;
    if (used > 64) o.flags |= 2;
    for (unsigned i = 0; i + 1 < cells; ++i)
        if (stack[i * 5] == 10 && stack[(i + 1) * 5] == 3) {
            ...
            if ((context && ((context & 3) || !field(context, 0, word0))) || !code_offset(code, ret)) {
                o.flags |= 1;
                continue;
            }
            if (o.count == 6) {
            ...
            o.contexts[o.count] = context;
            o.returns[o.count++] = ret;
        }
}
