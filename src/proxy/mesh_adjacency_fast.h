...
struct Policy {
    ...
    bool normal_selection = true; // Among several reverse edges prefer the most parallel face normal (first found on
                                  // ties).
    bool weld_refusal = true;     // A vertex is not welded to a representative when a face contains both.
    bool heap_order = true; // Sweep vertices in D3DX's heapsort permutation of the byte-0 key (false: stable descending
                            // order).
    bool retire_own_entry = false; // true: hide the querying edge even when its lookup found nothing (the pre-review-26
                                   // module).
    bool unlink_refused = true; // The selected entry leaves the table even when the single-adjacency check then refuses
                                // it.
    bool later_slot_check = false; // true: the single-adjacency check also looks at the face's later slots (D3DX:
                                   // earlier slots only).
};
struct Report {
    ...
    uint32_t degenerate_faces = 0, welded_degenerate_faces = 0, refused_welds = 0, multi_candidates = 0,
             normal_selected = 0, repeated_neighbours = 0, unmatched = 0;
};
