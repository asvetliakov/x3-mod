...
inline bool position_type_supported(std::uint32_t type) noexcept { return type == 2 || type == 3 || type == 16; }
inline std::uint32_t position_bytes(std::uint32_t type) noexcept { return type == 2 ? 12u : type == 3 ? 16u : type == 16 ? 8u : 0u; }
...
inline void read_position(const unsigned char* p, std::uint32_t type, float out[3]) noexcept {
    if (type == 16) { std::uint16_t h[3]; std::memcpy(h, p, 6); for (unsigned i = 0; i < 3; ++i) out[i] = half_to_float(h[i]); }
    else std::memcpy(out, p, 12);
}
inline bool finite(float x) noexcept { return x == x && x <= 3.4028235e38f && x >= -3.4028235e38f; }
...
inline int exponent_of(float x) noexcept {
    ...
    if (e) return e - 127;
    std::uint32_t m = bits & 0x7FFFFFu; int k = -1; // a subnormal is m * 2^-149: floor(log2) = highest set bit - 149
    while (m) { m >>= 1; ++k; }
    return k - 149;
}
...
inline bool measure(const unsigned char* vertices, std::uint32_t vertex_window, std::uint32_t stride, std::uint32_t position_offset,
                    std::uint32_t position_type, const void* indices, bool index32, std::uint32_t index_bias,
                    std::uint32_t triangles, Histogram& out) noexcept {
    out = Histogram{};
    ...
    for (std::uint32_t t = 0; t < triangles; t += step) {
        ...
        for (unsigned k = 0; k < 3; ++k) {
            ...
            std::uint32_t i;
            if (!indices) i = at;
            else if (index32) { std::memcpy(&i, static_cast<const unsigned char*>(indices) + std::size_t(at) * 4, 4); }
            else { std::uint16_t s; std::memcpy(&s, static_cast<const unsigned char*>(indices) + std::size_t(at) * 2, 2); i = s; }
            if (indices) { if (i < index_bias) return false; i -= index_bias; }
        ...
        const float la = a[0] * a[0] + a[1] * a[1] + a[2] * a[2], lb = b[0] * b[0] + b[1] * b[1] + b[2] * b[2], lc = c[0] * c[0] + c[1] * c[1] + c[2] * c[2];
        float longest = la > lb ? la : lb; longest = longest > lc ? longest : lc;
        const float area2 = n[0] * n[0] + n[1] * n[1] + n[2] * n[2]; // (2 area)^2
        if (!(longest > 0.f) || !(area2 > 0.f) || !finite(longest) || !finite(area2)) continue; // degenerate: not counted
        const float h = scalar::sqrt(area2 / longest); // 2 area / longest edge
        if (!(h > 0.f) || !finite(h)) continue;
        int e = exponent_of(h);
        if (e < lowest) e = lowest; else if (e >= lowest + span) e = lowest + span - 1;
        ++counts[e - lowest];
        if (e > top) top = e;
        ++measured;
    }
    ...
    out.e0 = e0; out.total = measured;
    std::uint32_t running = 0;
    out.cumulative[0] = 0.f;
    for (unsigned b = 0; b < bins; ++b) { running += folded[b]; out.cumulative[b + 1] = float(std::int32_t(running)) / float(std::int32_t(measured)); } // counts below 2^31: signed conversions (cvtsi2ss)
...
inline float rt2_alpha(float fraction) noexcept { return fraction >= vote_fraction ? 1.f - fraction : 1.f; }
...
struct Key {
    ...
    bool operator==(const Key& o) const noexcept {
        return vb == o.vb && ib == o.ib && stream_offset == o.stream_offset && stride == o.stride && position_offset == o.position_offset
            && position_type == o.position_type && first == o.first && primitives == o.primitives && min_vertex == o.min_vertex
            && vertex_count == o.vertex_count && base_vertex == o.base_vertex;
    }
...
struct IdentityIndex {
    ...
    struct Slot { std::uintptr_t id = 0; std::uint8_t count = 0; bool overflow = false; std::uint16_t entries[per_id]{}; };
    Slot slots[capacity]{};
    unsigned live = 0;
    bool full = false; // an identity could not be indexed: every invalidation scans the cache until clear()
    static unsigned home(std::uintptr_t id) noexcept { return unsigned((std::uint64_t(id) * 0x9E3779B97F4A7C15ull) >> 40) & (capacity - 1); }
    ...
    void add(std::uintptr_t id, std::uint16_t entry) noexcept {
        ...
        Slot* s = find(id);
        if (!s) {
            if (live >= capacity / 4 * 3) { full = true; return; }
            unsigned i = home(id);
            while (slots[i].id) i = (i + 1) & (capacity - 1);
            s = &slots[i]; s->id = id; ++live;
        }
        for (unsigned k = 0; k < s->count; ++k) if (s->entries[k] == entry) return;
        if (s->count < per_id) s->entries[s->count++] = entry; else s->overflow = true;
    }
    void remove(std::uintptr_t id, std::uint16_t entry) noexcept {
        if (!id) return;
        Slot* s = find(id);
        if (!s) return;
        for (unsigned k = 0; k < s->count; ++k) if (s->entries[k] == entry) { s->entries[k] = s->entries[--s->count]; break; }
    ...
    void erase(unsigned i) noexcept {
        ...
        for (;;) {
            ...
            if (!stays) { slots[i] = slots[j]; i = j; }
        }
        slots[i] = Slot{}; --live;
    }
    void clear() noexcept { for (auto& s : slots) s = Slot{}; live = 0; full = false; }
};
...
struct Volatility {
    ...
    static unsigned slot(std::uintptr_t id) noexcept { return unsigned((std::uint64_t(id) * 0x9E3779B97F4A7C15ull) >> 40) & (size - 1); }
    unsigned bump(std::uintptr_t id) noexcept {
        const unsigned i = slot(id);
        if (ids[i] != id) { ids[i] = id; counts[i] = 0; }
        if (counts[i] < 255) ++counts[i];
        return counts[i];
    }
    bool is_volatile(std::uintptr_t id) const noexcept { const unsigned i = slot(id); return id && ids[i] == id && counts[i] >= volatile_after; }
    void forget(std::uintptr_t id) noexcept { const unsigned i = slot(id); if (ids[i] == id) { ids[i] = 0; counts[i] = 0; } }
    void clear() noexcept { for (auto& i : ids) i = 0; for (auto& c : counts) c = 0; }
};
struct Cache {
    ...
    void begin_frame() noexcept { if (++stamp == 0) { stamp = 1; for (auto& e : entries) e.used = 0; } }
    ...
    Entry* find(const Key& key) noexcept {
        ...
        for (unsigned w = 0; w < ways; ++w) {
            ...
            if (e.key == key) { e.used = stamp; return &e; }
        }
        return nullptr;
    }
    Entry* way_for(const Key& key) noexcept {
        Entry* set = set_of(key); Entry* empty = nullptr; Entry* oldest = nullptr;
        for (unsigned w = 0; w < ways; ++w) {
            Entry& e = set[w];
            if (e.state == State::Empty) { if (!empty) empty = &e; continue; }
    ...
    void drop(std::size_t i) noexcept {
        Entry& e = entries[i];
        index.remove(e.vb_identity, std::uint16_t(i)); index.remove(e.ib_identity, std::uint16_t(i));
        e = Entry{}; tags[i] = 0;
    }
    // vb_identity / ib_identity: the wrappers the histogram (or the refusal) was read through, 0 when not watched.
    bool store(const Key& key, const Histogram* histogram, std::uintptr_t vb_identity = 0, std::uintptr_t ib_identity = 0) noexcept {
        Entry* e = way_for(key);
        if (!e) { ++refused; return false; }
        const std::size_t i = std::size_t(e - entries);
        drop(i);
        e->key = key; e->state = histogram ? State::Known : State::Unreadable; e->attempts = 0; e->used = stamp;
        e->histogram = histogram ? *histogram : Histogram{};
        e->vb_identity = vb_identity; e->ib_identity = ib_identity;
        index.add(vb_identity, std::uint16_t(i)); index.add(ib_identity, std::uint16_t(i));
    ...
    void retry(const Key& key) noexcept {
        ...
        const std::size_t i = std::size_t(e - entries);
        drop(i);
        e->key = key; e->attempts = std::uint8_t(attempts); e->used = stamp;
    ...
    unsigned invalidate(std::uintptr_t id) noexcept {
        ...
        if (!index.full && s && !s->overflow) {
            ...
            for (unsigned k = 0; k < n; ++k) if (entries[victims[k]].state != State::Empty) { drop(victims[k]); ++dropped; }
            return dropped;
        }
        if (!index.full && !s) return 0;
        for (std::size_t i = 0; i < size; ++i) {
            const Entry& e = entries[i];
            if (e.state != State::Empty && (e.vb_identity == id || e.ib_identity == id)) { drop(i); ++dropped; }
        }
    ...
    void clear() noexcept { for (auto& e : entries) e = Entry{}; for (auto& t : tags) t = 0; index.clear(); stamp = 1; refused = 0; }
};
