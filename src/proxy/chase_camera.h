...
struct FirstApplied { // the static inferences of the first applied frame, logged once (review 31 O2/O3)
    ...
    std::uint32_t fov298 = 0, plane_w = 0, plane_h = 0, default_plane_h = 0, view_mode = 0, connect_mode = 0,
                  flags_1a0 = 0, tracking_mode = 0, aim_gun = 0;
...
struct Stats {
    ...
    std::uint64_t frames = 0, applied = 0, refused = 0, inactive = 0, unbound = 0, snaps = 0, coalesced = 0, clamps = 0,
                  write_refused = 0, locked_frames = 0, scene_fixed = 0;
    unsigned cockpits_seen = 0; // distinct cockpit pointers reaching the site since the last report
    std::uint32_t last_verdict = 0, last_snap_reason = 0, view_mode = 0, connect_mode = 0, flags_1a0 = 0,
                  tracking_mode = 0;
    bool target_locked = false;
    double lag_deg = 0, pos_lag = 0, distance = 0, dt_ms = 0, half_vfov_tan = 0, boom_local[3] = {0, 0, 0},
           scene_fix_deg = 0;
    ...
    // slower drift accumulates until it crosses that hysteresis), the handler frame of that change and the change
    // count.
