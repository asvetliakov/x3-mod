...
inline const char* name(Status s) {
    ...
    case Status::ForeignExecutable: return "foreign_executable";
    ...
    case Status::NoCockpit: return "no_cockpit";
    case Status::NoSector: return "no_sector";
    case Status::Loading: return "loading";
    case Status::BadClass: return "bad_class";
    case Status::BadCount: return "bad_count";
    case Status::BadIndex: return "bad_index";
    case Status::BadDust: return "bad_dust";
    case Status::BadFog: return "bad_fog";
    case Status::CameraMismatch: return "camera_mismatch";
    case Status::AnchorMismatch: return "anchor_mismatch";
    }
    return "invalid";
}
...
inline const char* name(AnchorWalk s) {
    switch (s) {
    case AnchorWalk::None: return "none";
    case AnchorWalk::Found: return "found";
    case AnchorWalk::OtherSector: return "other_sector";
    case AnchorWalk::Bound: return "bound";
...
inline const char* name(Check s) {
    switch (s) {
    case Check::Unavailable: return "unavailable";
    case Check::Inactive: return "inactive";
    case Check::Match: return "match";
    case Check::Mismatch: return "mismatch";
    }
    return "invalid";
}
...
template <class Read>
AnchorWalk walk_anchor(Read& read, std::uint32_t sector, std::uint32_t object, unsigned limit, unsigned& depth,
                       std::uint32_t& last) {
    depth = 0;
    last = object;
    for (;;) {
...
inline bool anchor_refused(const Sample& s) {
    return s.anchor_check == Check::Mismatch && s.anchor_walk != AnchorWalk::Found;
}
...
template <class Read> bool read_record(Read& read, Sample& out) {
    ...
    out.name_pointer = row[0];
    out.dust = std::int32_t(row[0xf0 / 4]);
    out.fog_near = std::int32_t(row[0x104 / 4]);
    out.fog_far = std::int32_t(row[0x108 / 4]);
    out.stardust = std::int32_t(row[0x10c / 4]);
    for (unsigned i = 0; i < 8; ++i) out.rates[i] = std::int32_t(row[0xd0 / 4 + i]);
    if (out.dust < 0 || out.dust > 64) {
        out.status = Status::BadDust;
        return false;
    }
    if (!((out.fog_near == 0 && out.fog_far == 0) ||
          (out.fog_near > 0 && out.fog_near <= out.fog_far && out.fog_far <= 2000000000))) {
        out.status = Status::BadFog;
        return false;
    }
    out.row_valid = true;
    char raw[32]{};
...
template <class Read> Sample sample(Read& read, unsigned walk = 0) {
    Sample out;
    std::uint32_t header = 0, bucket[2]{}, link = 0;
    if (!field(read, 0x608504, 0, out.registry)) return out;
    if (!out.registry) {
        out.status = Status::NoCockpit;
        return out;
    }
    if (out.registry & 3) {
    ...
    if (!n || n > 65536 || (n & (n - 1)) || !bucket[0] || (bucket[0] & 3)) {
        out.status = Status::Malformed;
        return out;
    }
    ...
    if (field(read, out.cockpit, 0xc, out.ref_object) && out.ref_object &&
        ...
        if (out.anchor_check == Check::Mismatch && walk && out.sector && !(out.sector & 3)) {
            ...
            out.anchor_walk = walk_anchor(read, out.sector, out.ref_sector,
                                          walk < anchor_walk_limit ? walk : anchor_walk_limit, out.anchor_depth,
                                          out.anchor_last);
        }
    }
    if (!out.sector) {
        out.status = Status::NoSector;
        return out;
    }
    if (out.sector & 3) {
        out.status = Status::Malformed;
    ...
    if (out.class48 != 1) {
        out.status = Status::BadClass;
        return out;
    }
    if (!field(read, 0x607040, 0, out.count) || !field(read, 0x606fc0, 0, out.table)) return out;
    if (!out.table || !out.count) {
        out.status = Status::Loading;
        return out;
    }
    if (out.count < 0 || out.count > 4096) {
        out.status = Status::BadCount;
        return out;
    }
    if (out.table & 3) {
        out.status = Status::Malformed;
    ...
    if (out.index < 0 || out.index >= out.count) {
        out.status = Status::BadIndex;
        return out;
    }
    ...
    if (field(read, out.cockpit, 0x58, out.camera) && out.camera && !(out.camera & 3)) {
        ...
        if (field(read, out.camera, 0x270, out.flags270) && field(read, out.camera, 0x36c, fog)) {
            out.camera_valid = true;
            out.cam_near = fog[0];
            out.cam_far = fog[1];
            out.camera_check = (out.flags270 & 0x10000)
                                   ? (out.cam_near == out.fog_near && out.cam_far == out.fog_far ? Check::Match
                                                                                                 : Check::Mismatch)
                                   : Check::Inactive;
        }
    }
    ...
    out.status = out.camera_check == Check::Mismatch ? Status::CameraMismatch
                 : anchor_refused(out)               ? Status::AnchorMismatch
                                                     : Status::Ready;
    return out;
}
...
struct AnchorSpan {
    ...
    bool update(const Sample& s) {
        ...
        const bool report = next && next != state;
        state = next;
        return report;
    }
};
