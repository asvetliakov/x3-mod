...
// X3M_CULL_SMALL_PARTS_PX as printable ASCII (anything else is logged as "?").
bool read_setting(char* out, unsigned capacity) {
    wchar_t text[32]{};
    const DWORD length = GetEnvironmentVariableW(L"X3M_CULL_SMALL_PARTS_PX", text, 32);
...
std::uintptr_t emit_stub(std::uint32_t cull_target, void*** slot_out) {
    ...
    core::encode_stub(std::uint32_t(at), std::uint32_t(reinterpret_cast<std::uintptr_t>(&x3m_cull_small_parts_threshold)),
                      std::uint32_t(reinterpret_cast<std::uintptr_t>(&x3m_cull_small_parts_culled)), cull_target, std::uint32_t(slot), code);
...
bool install_at(std::uintptr_t site, std::uintptr_t cull_target) {
    ...
    if (!reason && !(stub = emit_stub(std::uint32_t(cull_target), &slot))) reason = "arena_full";
    if (!reason) {
        ...
        if (!engine_patch::claim(site_, spec)) {
            ...
            patched_ = true; stub_ = stub; reason = "ok";
        }
    }
    state_ = reason;
...
bool initialize() {
    ...
    char setting[32]{};
    if (!read_setting(setting, sizeof setting)) { state_ = "disabled"; SetLastError(error); return false; }
    double px = 0;
    ...
    else { px_ = px; applied = install_at(core::site_va, core::cull_va); }
    log("cull_small_parts requested=%s px=%.4g patched=%u reason=%s site=0x%08lx cull=0x%08lx write=%s stub=0x%08lx camera=%s",
        ...
        site_.patched_in ? (site_.atomic_write ? "atomic" : "plain") : "none", static_cast<unsigned long>(stub_), camera_state::status());
    SetLastError(error);
    return applied;
}
bool shutdown() {
...
std::int32_t publish(float m00, unsigned width) {
    ...
    cull_census::note_small_threshold(threshold);
...
void present(unsigned long long device, unsigned long long frame, bool captured) {
    ...
    if (captured) {
        const DWORD error = GetLastError();
        log("cull_small_parts_frame device=%llu frame=%llu px=%.4g threshold=%ld culled=%lu m00=%.9g width=%u",
            device, frame, px_, static_cast<long>(x3m_cull_small_parts_threshold), static_cast<unsigned long>(x3m_cull_small_parts_culled), static_cast<double>(last_m00_), width_);
        SetLastError(error);
    }
    x3m_cull_small_parts_culled = 0;
}
