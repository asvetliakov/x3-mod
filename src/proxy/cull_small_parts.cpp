...
bool bytes_match(std::uintptr_t at, const unsigned char* expected, unsigned length) {
    ...
    return length <= core::window_length && engine_patch::read_code(at, actual, length) &&
...
bool read_setting(const wchar_t* name, char* out, unsigned capacity) {
    ...
    if (length >= 32 || length + 1 > capacity) {
        out[0] = '?';
        out[1] = 0;
        return true;
    }
...
bool initialize() {
    ...
    char setting[32]{};
    if (!read_setting(L"X3M_CULL_SMALL_PARTS_PX", setting, sizeof setting)) {
        state_ = "disabled";
        SetLastError(error);
        return false;
    }
    char projectiles_text[32]{};
    read_setting(L"X3M_CULL_SMALL_PARTS_PROJECTILES", projectiles_text, sizeof projectiles_text); // unset = the
                                                                                                  // default, on
    bool exempt = true;
    ...
    bool applied = false;
    if (parsed && px == 0.0)
        state_ = "disabled"; // an explicit 0 is the documented off
    else if (!parsed || !core::valid_px(px))
        state_ = "invalid_px";
        ...
        if (exempt && !(bytes_match(core::marker_store_va, core::marker_store, core::marker_store_length) &&
                        bytes_match(core::marker_or_va, core::marker_or, core::marker_or_length))) {
            exempt = false;
...
double requested_px() {
    return patched_ ? px_ : 0.0;
}
bool set_px(double px) {
    if (!core::valid_px(px)) return false;
    px_ = px;
    return true;
}
namespace {
...
void note_scene_projection(float m00, float m11) {
    if (patched_) scene_.note(m00, m11);
}
void begin_frame() {
    if (!patched_) return;
    const DWORD error = GetLastError();
...
Stats stats() {
    ...
    s.width = width_;
    s.focus = last_focus_;
    s.scene = last_source_ == core::Source::scene;
    s.fallback = unsigned(last_fallback_);
    return s;
}
}
