...
constexpr ULONGLONG kExceptionDrainMs = 500; // the crash row waits this long at most (GetTickCount64) for the writer's drain
...
struct AppendStats { std::uint64_t rows = 0, bytes = 0, ticks = 0, max_ticks = 0; };
AppendStats append_{};
...
struct WriteStats { std::uint64_t writes = 0, bytes = 0, ticks = 0, max_ticks = 0, failures = 0, buckets[6] = {}; };
WriteStats write_{};
...
struct Named { std::uintptr_t base; char name[40]; };
Named named_[4] = {};
...
struct Line {
    char text[384];
    unsigned used = 0;
    void put(char c) noexcept { if (used < sizeof text - 1) text[used++] = c; }
    void str(const char* s) noexcept { while (*s) put(*s++); }
    void hex8(std::uint32_t v) noexcept { for (int i = 7; i >= 0; --i) put("0123456789abcdef"[(v >> (i * 4)) & 15u]); }
    void dec(std::uint32_t v) noexcept { char d[10]; int n = 0; do { d[n++] = char('0' + v % 10u); v /= 10u; } while (v); while (n) put(d[--n]); }
    void dec64(std::uint64_t v) noexcept { char d[20]; int n = 0; do { d[n++] = char('0' + unsigned(v % 10u)); v /= 10u; } while (v); while (n) put(d[--n]); }
    unsigned end_row() noexcept { text[used] = '\n'; return used + 1; } // put() stops one short, so the newline fits
};
...
bool reserve(unsigned n, bool& wake) noexcept {
    ...
    handed_[active_] = true; active_ = other; wake = true;
    return true;
}
void put_locked(const char* text, unsigned n) noexcept { std::memcpy(buffer_[active_] + used_[active_], text, n); used_[active_] += n; appended_ += LONG(n); }
bool drops_row_locked(bool& wake) noexcept {
    if (!pending_drops_) return true;
    Line line; line.str("log_dropped n="); line.dec(pending_drops_);
    const unsigned n = line.end_row();
    if (!reserve(n, wake)) return false;
    put_locked(line.text, n); pending_drops_ = 0;
    return true;
}
void drop_locked() noexcept { ++pending_drops_; InterlockedIncrement(&dropped_total_); }
std::uint64_t now_ticks() noexcept { LARGE_INTEGER t{}; QueryPerformanceCounter(&t); return std::uint64_t(t.QuadPart); }
double as_double(std::uint64_t v) noexcept { return double(std::uint32_t(v >> 32)) * 4294967296.0 + double(std::uint32_t(v)); } // no x87 u64 conversion
...
void write_half(unsigned h) noexcept {
    ...
    while (offset < size) {
        ...
        if (timing_) {
            ++write_.writes; write_.bytes += part; write_.ticks += ticks; if (ticks > write_.max_ticks) write_.max_ticks = ticks;
            unsigned bucket = 0;
            for (std::uint64_t edge = frequency_ / 100000u; bucket < 5 && ticks > edge; edge *= 10u) ++bucket; // 10 us, 100 us, ... 100 ms
...
void drain_all() noexcept {
    for (;;) {
        ...
        const unsigned other = active_ ^ 1u;
        if (handed_[other]) h = other;                                           // the older rows first
        else if (used_[active_]) { h = active_; handed_[h] = true; active_ = other; } // the other half is empty here
        ReleaseSRWLockExclusive(&lock_);
        if (h == 2) return;
        write_half(h);
        AcquireSRWLockExclusive(&lock_);
        used_[h] = 0; handed_[h] = false; InterlockedExchange(&written_[h], 0);
        ReleaseSRWLockExclusive(&lock_);
    }
}
...
void summary(const char* reason) noexcept {
    AcquireSRWLockExclusive(&lock_);
    const AppendStats a = append_; append_ = {};
    const WriteStats w = write_; write_ = {};
    const LONG frames = frames_; const LONG window_frames = frames - summary_frames_; summary_frames_ = frames;
    ReleaseSRWLockExclusive(&lock_);
    const double us = frequency_ ? 1e6 / as_double(frequency_) : 0.;
    x3m::log("log_writer reason=%s frames=%ld rows=%llu bytes=%llu dropped_total=%ld format_us=%.1f format_max_us=%.2f format_us_per_frame=%.2f"
             ...
             reason, long(window_frames), static_cast<unsigned long long>(a.rows), static_cast<unsigned long long>(a.bytes), long(dropped_total_),
             as_double(a.ticks) * us, as_double(a.max_ticks) * us, window_frames > 0 ? as_double(a.ticks) * us / double(window_frames) : 0.,
             static_cast<unsigned long long>(w.writes), static_cast<unsigned long long>(w.bytes), as_double(w.ticks) * us, as_double(w.max_ticks) * us,
             static_cast<unsigned long long>(w.failures), static_cast<unsigned long long>(w.buckets[0]), static_cast<unsigned long long>(w.buckets[1]),
             static_cast<unsigned long long>(w.buckets[2]), static_cast<unsigned long long>(w.buckets[3]), static_cast<unsigned long long>(w.buckets[4]),
             static_cast<unsigned long long>(w.buckets[5]), 2u * kHalf, static_cast<unsigned long>(kPeriodMs));
}
DWORD WINAPI writer(void*) {
    ...
    for (;;) {
        ...
        drain_all();
        if (timing_ && LONG(GetTickCount() - next_summary) >= 0) { summary("period"); next_summary += kSummaryMs; drain_all(); }
        if (stopping) break;
    }
...
bool writer_running() noexcept { return thread_ && WaitForSingleObject(thread_, 0) == WAIT_TIMEOUT; }
...
void drain_direct() noexcept {
    const unsigned other = active_ ^ 1u;
    if (handed_[other] && used_[other] > unsigned(written_[other])) write_direct(buffer_[other] + written_[other], used_[other] - unsigned(written_[other]));
    if (used_[active_] > unsigned(written_[active_])) write_direct(buffer_[active_] + written_[active_], used_[active_] - unsigned(written_[active_]));
    used_[0] = used_[1] = 0; handed_[0] = handed_[1] = false; written_[0] = 0; written_[1] = 0;
    flushed_ = appended_;
}
void wide_digits(std::wstring& out, unsigned value, unsigned width) {
    wchar_t d[10]; unsigned n = 0;
    do { d[n++] = wchar_t(L'0' + value % 10u); value /= 10u; } while (value && n < 10);
    while (n < width && n < 10) d[n++] = L'0';
    while (n) out += d[--n];
}
void narrow_digits(char*& at, const char* end, unsigned value, unsigned width) {
    char d[10]; unsigned n = 0;
    do { d[n++] = char('0' + value % 10u); value /= 10u; } while (value && n < 10);
...
char fold(char c) noexcept { return c >= 'A' && c <= 'Z' ? char(c - 'A' + 'a') : c; }
bool separator(char c) noexcept { return c == '\\' || c == '/'; }
...
bool match_at(const std::string& text, std::size_t at, const std::string& needle) noexcept {
    ...
    for (std::size_t i = 0; i < needle.size(); ++i) if (fold(text[at + i]) != fold(needle[i])) return false;
    const std::size_t end = at + needle.size();
    return end == text.size() || separator(text[end]) || text[end] == ':' || text[end] == ';' || text[end] == ' ' || text[end] == '"';
}
std::string replace_all(const std::string& text, const std::string& needle, const char* token) {
    ...
    for (std::size_t i = 0; i < text.size();) {
        if (match_at(text, i, needle)) { out += token; i += needle.size(); }
        else out += text[i++];
    }
    return out;
}
HANDLE create(const std::wstring& path) {
    ...
    return CreateFileW(path.c_str(), GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
}
...
unsigned remove_stale(const std::wstring& base) {
    ...
    do {
        ...
        while (name[i] >= L'0' && name[i] <= L'9') { ++i; ++digits; }
        const bool pattern = !_wcsnicmp(name, L"x3m-", 4) && digits > 0 && !_wcsicmp(name + i, L".log") && !(found.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY);
        if (pattern && CompareFileTime(&found.ftLastWriteTime, &prev.ftLastWriteTime) < 0 && DeleteFileW((base + L"\\" + name).c_str())) ++removed;
...
HANDLE open_in(const std::wstring& base, const char*& previous, std::wstring& opened, unsigned& stale) {
    ...
    if (MoveFileExW(log.c_str(), prev.c_str(), MOVEFILE_REPLACE_EXISTING)) previous = "renamed";
    else {
        const DWORD error = GetLastError();
        if (error == ERROR_FILE_NOT_FOUND || error == ERROR_PATH_NOT_FOUND) previous = "absent";
        else { previous = "busy"; target = &pid; }
    }
    HANDLE file = create(*target);
    if (file == INVALID_HANDLE_VALUE && target == &log) { previous = "busy"; target = &pid; file = create(pid); }
    if (file != INVALID_HANDLE_VALUE) { opened = *target; stale = remove_stale(base); }
    return file;
}
...
std::wstring final_path(HANDLE handle, const std::wstring& fallback) {
    ...
    const FinalPathFn fn = reinterpret_cast<FinalPathFn>(reinterpret_cast<void*>(proc)); // via void*: -Wcast-function-type
    std::wstring path(1024, L'\0');
    DWORD length = fn(handle, &path[0], DWORD(path.size()), 0 /* FILE_NAME_NORMALIZED | VOLUME_NAME_DOS */);
    if (length >= path.size()) { path.assign(length + 1, L'\0'); length = fn(handle, &path[0], DWORD(path.size()), 0); }
    if (!length || length >= path.size()) return fallback;
    path.resize(length);
    if (path.compare(0, 8, L"\\\\?\\UNC\\") == 0) path = L"\\\\" + path.substr(8);
    else if (path.compare(0, 4, L"\\\\?\\") == 0) path = path.substr(4);
    return path;
}
bool buffer_empty() noexcept { return !handed_[0] && !handed_[1] && !used_[active_]; } // racy read: a wait condition only
bool flushed_past(LONG target) noexcept { return LONG(flushed_ - target) >= 0; } // wrap-safe; racy read of an aligned word
}
...
extern "C" void x3m_session_log_open(HMODULE module, Opened* out) {
    ...
    {   // Session name: local time and process id, as the former session-*.log name carried.
        ...
        char* at = o.session; const char* end = o.session + sizeof o.session - 1;
        narrow_digits(at, end, now.wYear, 4); narrow_digits(at, end, now.wMonth, 2); narrow_digits(at, end, now.wDay, 2);
        if (at < end) *at++ = '-';
        narrow_digits(at, end, now.wHour, 2); narrow_digits(at, end, now.wMinute, 2); narrow_digits(at, end, now.wSecond, 2);
        if (at < end) *at++ = '-';
    ...
    if (game_known) { o.captures = game + L"\\x3-modern-captures"; CreateDirectoryW(o.captures.c_str(), nullptr); }
    std::wstring opened;
    const std::wstring override_path = environment(L"X3M_LOG_FILE");
    if (!override_path.empty()) {
        o.file = create(override_path);
        if (o.file != INVALID_HANDLE_VALUE) { o.source = "override"; o.previous = "none"; opened = override_path; }
        else o.override_failed = true;
    }
    ...
    }
    {
        ...
        if (!base.empty() && (o.file == INVALID_HANDLE_VALUE || o.captures.empty())) {
            ...
            const std::wstring captures = base + L"\\captures"; CreateDirectoryW(captures.c_str(), nullptr);
            if (o.file == INVALID_HANDLE_VALUE) {
                o.file = open_in(base, o.previous, opened, o.stale_removed);
                if (o.file != INVALID_HANDLE_VALUE) { o.source = "localappdata"; o.captures_source = "localappdata"; o.captures = captures; }
            }
            if (o.captures.empty()) { o.captures = captures; o.captures_source = "localappdata"; } // an override with no known game directory
        }
    }
...
extern "C" LONG WINAPI x3m_exception_witness(EXCEPTION_POINTERS* info) {
    ...
    if (record->ExceptionCode != EXCEPTION_STACK_OVERFLOW) {
        ...
        if (!flushed_past(target) && !other_writer && TryAcquireSRWLockExclusive(&lock_)) { drain_direct(); ReleaseSRWLockExclusive(&lock_); }
    }
    const auto address = std::uint32_t(reinterpret_cast<std::uintptr_t>(record->ExceptionAddress));
    const CONTEXT* c = info->ContextRecord;
    std::uintptr_t base = 0;
    MEMORY_BASIC_INFORMATION region{};
    if (VirtualQuery(record->ExceptionAddress, &region, sizeof region) == sizeof region) base = reinterpret_cast<std::uintptr_t>(region.AllocationBase);
    const char* name = base ? "other" : "none";
    const LONG named = named_count_;
    for (LONG i = 0; i < named && i < 4; ++i) if (base && named_[i].base == base) name = named_[i].name;
    Line line;
    line.str("exception code="); line.hex8(std::uint32_t(record->ExceptionCode));
    line.str(" address="); line.hex8(address);
    line.str(" eip="); line.hex8(c ? std::uint32_t(c->Eip) : 0u);
    line.str(" esp="); line.hex8(c ? std::uint32_t(c->Esp) : 0u);
    line.str(" thread="); line.dec(std::uint32_t(GetCurrentThreadId()));
    line.str(" frame="); line.dec(std::uint32_t(frames_));
    line.str(" module="); line.str(name);
    line.str(" base="); line.hex8(std::uint32_t(base));
    line.str(" offset="); line.hex8(address - std::uint32_t(base));
    line.str(" access_kind="); line.dec(record->NumberParameters >= 1 ? std::uint32_t(record->ExceptionInformation[0]) : 0u);
    line.str(" access="); line.hex8(record->NumberParameters >= 2 ? std::uint32_t(record->ExceptionInformation[1]) : 0u);
...
extern "C" void x3m_session_end() {
    ...
    const std::uint64_t ticks = std::uint64_t(now.QuadPart) > x3m::dll_load_qpc ? std::uint64_t(now.QuadPart) - x3m::dll_load_qpc : 0;
    const std::uint64_t elapsed_ms = frequency_ ? ticks / frequency_ * 1000u + ticks % frequency_ * 1000u / frequency_ : 0;
    Line line;
    line.str("session_end frames="); line.dec(std::uint32_t(frames_));
    line.str(" elapsed_ms="); line.dec64(elapsed_ms);
    line.str(" devices="); line.dec(std::uint32_t(devices_));
    line.str(" resets="); line.dec(std::uint32_t(resets_));
    line.str(" exception="); line.dec(exception_latch_ ? 1u : 0u);
    line.str(" dropped="); line.dec(std::uint32_t(dropped_total_));
    line.str(" filter="); line.str(filter_state_);
    write_direct(line.text, line.end_row());
    SetLastError(error);
}

namespace x3m::session_log {
Opened open(HMODULE module) { Opened opened; x3m_session_log_open(module, &opened); return opened; }
void start_writer(bool timing) noexcept {
    if (file_ == INVALID_HANDLE_VALUE) return;
    if (thread_) {
        if (writer_running()) {
            if (!stop_) return; // running
            if (WaitForSingleObject(thread_, kJoinMs) != WAIT_OBJECT_0) return; // a parked writer still draining: never two
        }
        CloseHandle(thread_); thread_ = nullptr; writer_id_ = 0;
    }
    if (!started_) { timing_ = timing; started_ = true; }
    ...
    if (!wake_ || !GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS, reinterpret_cast<LPCWSTR>(&writer), &module_ref_)) return;
    thread_ = CreateThread(nullptr, 64u << 10, &writer, nullptr, STACK_SIZE_PARAM_IS_A_RESERVATION, &writer_id_);
    if (!thread_) { FreeLibrary(module_ref_); module_ref_ = nullptr; }
}
void park_writer(const char* reason) noexcept {
    ...
    if (exited) { CloseHandle(thread_); thread_ = nullptr; writer_id_ = 0; }
    x3m::log("log_writer_parked reason=%s exited=%u", reason, unsigned(exited)); // buffered: the next writer or detach writes it
}
void vlog(const char* format, va_list args) noexcept {
    ...
    bool wake = false;
    unsigned bytes = 0;
    if (!closing_) AcquireSRWLockExclusive(&lock_);
    else if (!TryAcquireSRWLockExclusive(&lock_)) { InterlockedIncrement(&dropped_total_); va_end(copy); return; } // DllMain: never wait
    if (n > 0 && unsigned(n) < kScratch - 1 && scratch[n - 1] == '\n') --n; // a caller's own line end (former direct writers)
    if (n >= 0 && unsigned(n) < kScratch - 1) {
        scratch[n] = '\n'; bytes = unsigned(n) + 1;
        if (drops_row_locked(wake) && reserve(bytes, wake)) put_locked(scratch, bytes);
        else { drop_locked(); bytes = 0; }
    } else if (n >= 0) {
        ...
        if (drops_row_locked(wake) && reserve(want, wake)) {
            ...
            if (m > 0 && unsigned(m) > cap) { std::memcpy(at + length, " truncated=1", 12); length += 12; }
            at[length++] = '\n'; used_[active_] += length; appended_ += LONG(length); bytes = length;
        } else drop_locked();
    }
    if (timed) {
        const std::uint64_t ticks = now_ticks() - begin;
        ++append_.rows; append_.bytes += bytes; append_.ticks += ticks;
...
void request_drain() noexcept { if (wake_) SetEvent(wake_); }
void report(const char* reason) noexcept { if (timing_ && file_ != INVALID_HANDLE_VALUE) summary(reason); }
void closing() noexcept { InterlockedExchange(&closing_, 1); }
...
HANDLE handle() noexcept { return file_; }
unsigned dropped_total() noexcept { return unsigned(dropped_total_); }
std::string redact_wide_path(const wchar_t* path) { return path ? redact_path(utf8(std::wstring(path))) : std::string(); }
...
void note_frame() noexcept { InterlockedIncrement(&frames_); }
void note_device() noexcept { InterlockedIncrement(&devices_); }
void note_reset() noexcept { InterlockedIncrement(&resets_); }
void name_module(HMODULE module, const char* name) noexcept {
    ...
    unsigned i = 0;
    for (; name[i] && i < sizeof n.name - 1; ++i) { const char ch = name[i]; n.name[i] = ch > 0x20 && ch < 0x7f ? ch : '_'; }
    n.name[i] = 0;
...
void arm_exception_witness(HMODULE proxy) noexcept {
    ...
    if (length && length < MAX_PATH) {
        ...
        unsigned n = 0;
        for (unsigned i = start; i < length && n < sizeof name - 1; ++i) name[n++] = path[i] > 0x20 && path[i] < 0x7f ? char(path[i]) : '_';
        name[n] = 0;
    }
    name_module(exe, name);
    name_module(proxy, "proxy");
    if (filter_armed_) return;
    previous_filter_ = SetUnhandledExceptionFilter(&x3m_exception_witness); // chained: ours first, then the one it replaced
    filter_armed_ = true; filter_state_ = "ours";
}
void detach(bool process_exit) noexcept {
    ...
    if (filter_armed_) {
        ...
        if (current != &x3m_exception_witness) { SetUnhandledExceptionFilter(current); filter_state_ = "replaced"; }
        filter_armed_ = false;
    }
    ...
    if (process_exit) drain_direct();
    else if (TryAcquireSRWLockExclusive(&lock_)) { drain_direct(); ReleaseSRWLockExclusive(&lock_); }
    else return;
    x3m_session_end();
}
}
