...
static_assert(x3m::submit_phases::detail::site_count == x3m::submit_phases::sites::Count,
              "role table and site table are one list");
static_assert(x3m::submit_phases::detail::sort_enter_site == x3m::submit_phases::sites::SortEnter &&
                  ...
                  x3m::submit_phases::detail::end_begin_site == x3m::submit_phases::sites::EndBegin,
              "special sites keep their table positions");
...
inline std::uint32_t word(std::uint32_t address) noexcept {
...
std::uint32_t queue_length() noexcept {
    const std::uint32_t root = *view_root_global;
    if (!root) return 0;
    const std::uint32_t sentinel = root + queue_sentinel_offset;
    std::uint32_t count = 0;
    for (std::uint32_t entry = word(root + queue_head_offset); entry && entry != sentinel && count < chase_cap;
         entry = word(entry))
        ++count;
    return count;
}
...
std::uint32_t walk_length(std::uint32_t view, std::uint32_t hit) noexcept {
    ...
    for (std::uint32_t record = word(view + cache_head_offset); record && count < chase_cap;) {
        const std::uint32_t next = word(record);
        if (!next) break;
        ++count;
        if (record == hit) break;
        record = next;
...
void emit_window() {
    ...
    log("submit_phases qpc=%llu frame=%llu frames=%u stamps_p50=%llu stamps_p95=%llu self_p50_us=%llu dispatch_cost_ns=%llu "
        ...
        emitted, s.frame, s.frames, s.stamps_p50, s.stamps_p95, s.self_p50, dispatch_cost_ns, s.calls_p50[Sort],
        s.interval_p50[Sort], s.interval_p95[Sort], s.sort_nodes_p50, s.sort_nodes_max, s.calls_p50[Walk],
        s.interval_p50[Walk], s.interval_p95[Walk], s.walk_misses_p50, s.walk_iterations_p50, s.walk_iterations_p95,
        walk_sample_period, s.calls_p50[Technique], s.interval_p50[Technique], s.interval_p95[Technique],
        s.calls_p50[End], s.interval_p50[End], s.interval_p95[End], s.calls_p50[Block], s.interval_p50[Block],
        s.interval_p95[Block], s.block_net_p50, s.calls_p50[InverseWorld], s.interval_p50[InverseWorld],
        s.interval_p95[InverseWorld], s.calls_p50[InverseView], s.interval_p50[InverseView],
        s.interval_p95[InverseView], s.calls_p50[Material], s.interval_p50[Material], s.interval_p95[Material],
        s.material_net_p50, s.calls_p50[World], s.interval_p50[World], s.interval_p95[World], accumulator.block_skipped,
        accumulator.reopened, accumulator.idle, accumulator.clock_errors, accumulator.clock_failures,
...
extern "C" __attribute__((force_align_arg_pointer)) void __cdecl x3m_submit_phase_enter(unsigned index,
    ...
    const bool count_walk = (index == sites::WalkMiss || index == sites::WalkJoin) &&
                            accumulator.open_clock[detail::Walk] && accumulator.walk_sampled();
    LARGE_INTEGER v{};
    const std::uint64_t now = QueryPerformanceCounter(&v) && v.QuadPart > 0 ? std::uint64_t(v.QuadPart) : 0;
    accumulator.stamp(index, now);
    if (count_walk)
        accumulator.walked(walk_length(saved[x3m::lean_stub::SavedEdi],
                                       index == sites::WalkJoin ? saved[x3m::lean_stub::SavedEsi] : 0));
}
...
bool initialize() {
    ...
    if (telemetry::enabled()) {
        ...
        if (!frequency)
            ...
            status = "frame_phases_off"; // the frame boundary and the owner thread come from the frame group
    ...
    log("submit_phase_mode requested=1 enabled=%u status=%s sites=%u window=%u owner=present_thread dispatch_cost_ns=%llu walk_sample_period=%u qpc_frequency=%llu",
        unsigned(active.load()), status, sites::Count, detail::window_frames, detail::dispatch_cost_ns,
        detail::walk_sample_period, frequency);
    for (unsigned i = 0; i < sites::Count; ++i)
        log("submit_phase_site index=%u address=%08lx length=%u patched=%u status=%s", i,
