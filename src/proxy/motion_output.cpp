...
void MotionOutput::configure_hull_lightmap_gain(float gain) noexcept {
    if (device_) return; // Creation-time shader variant: immutable after attach.
    // Gain 1 keeps the fill/motion variants (no variant, no selection); the
...
// Overlay arm (asteroid-fog-temporal.md "Run 130"): any other reviewed pair
// (gate 3 passed) drawn in the same exact fade-band state right after a
// routed draw of the same node is that node's source-over sub-mesh (the
// hull pair's glass/window layer: own vertex buffer and textures, the node's
// rows and cube map); it is admitted at permille 1000 without the fraction
// (its alpha is the material's, not a distance fade) or the hysteresis,
// counted as overlay_routed/overlay_refused. "Right after" is exact: the
// very next draw index of the same frame, the same node identity here and
// the same lifetime serial once sample_scope has read it (the scope gate
// turns a mismatch back into the gate-4 refusal before anything is applied
// or recorded). Original shading only: with linear materials requested a
// blended reviewed pair may belong to the composition or glass bracket,
// whose colour path no fixture has verified through this arm. Cost: a
// refused reviewed-pair draw with z on and z write off pays up to eight
...
bool MotionOutput::ensure_taa() noexcept {
    ...
    // Camera-relative gate of the thin region (section 32.1): its three programs are created by configure_far above; without
    // them, or beside the line filter (the mask's r channel carries the second gate), the thin region keeps its screen-speed gate.
    if (SUCCEEDED(hr) && taa_thin_camera_gate_ && (taa_thin_weight_ <= 0.f || taa_line_filter_ > 0.f || !taa_->camera_gate_available())) {
        log("motion_output_taa_thin_region device=%llu camera_gate_unavailable=1 reason=%s thin_region=%.4f line_filter=%.3f", id_, taa_thin_weight_ <= 0.f ? "thin_region_off" : taa_line_filter_ > 0.f ? "line_filter" : "camera_program", double(taa_thin_weight_), double(taa_line_filter_));
...
void MotionOutput::set_pixel_constants_f(UINT start, const float* data, UINT count) noexcept {
    ...
    // Caster counter: c0..c31 as written, for the bound program's own
    // LightDir_Dir0 register at the draw (shadow_replay_sun.h).
    if (candidates_requested_ && start < shadow_replay::sun_register_limit) {
        const UINT hi = end < shadow_replay::sun_register_limit ? end : shadow_replay::sun_register_limit;
...
// The shadowed application value (the SetSamplerState hook; read once here
// when unknown, and per widened draw with the state hooks off) is restored by
// undo; only a stage not already ANISOTROPIC is written. Documented sampler
// state, direct_call (never through the hook, so the shadow keeps the
// application's value).
bool MotionOutput::ensure_widen_filter(MotionRoute& route) noexcept {
    const unsigned stage = shadow_.hull_lightmap_stage;
    auto& s = samplers_[stage];
    if (!state_hooks_) s.minfilter_known = false; // hooks off: the application may have written it since
...
HRESULT MotionOutput::bind_variant_pair(MotionRoute& route, bool material) noexcept {
    ...
    // for every gain draw whose light-map size is known (evaluate_draw's
    // uploaded footprint lanes; the block itself holds k = 1 near), that is
    // not alpha tested (the opaque chain already refused blending; rL.w is
    // widened too) and whose light-map stage holds a mip chain (the shadow's
    // level count; a single-level placeholder would sample identically). No
    // getter.
#ifdef X3M_MOTION_OUTPUT_FIXTURE
    const bool widen_suppressed = fixture_configured_ && fixture_.suppress_lightmap_widen != 0;
#else
...
void MotionOutput::evaluate_draw(const MotionDrawCall& call, MotionRoute& route) noexcept {
    ...
    // computes k = clamp(K . texels per pixel, 1, K) from them). Zero with the
    // option off, without a gain pair or with an unknown size (k = 1 then).
    lightmap_widen_draw_scale_[0] = lightmap_widen_draw_scale_[1] = 0.f;
...
void MotionOutput::readback() noexcept {
    ...
    if (depth_cascades_on() && depth_replay_ && depth_replay_->caps().readable) {
        // One readback and one basis line per cascade (shadow-cascades.md,
...
// record_capacity arrays unless a cascade asks for more records, then arrays
// of the largest per-cascade records, allocated here once (kept across
// re-attaches while large enough) and never on the draw path. The importance
// scratch and the classification ring exist only while their option is on.
// Called with no lease outstanding (attach: release_depth_leases ran first).
bool MotionOutput::attach_candidate_storage() noexcept {
    ...
    const bool ext = capacity > shadow_replay::record_capacity;
    if (!ext) candidate_capacity_ = shadow_replay::record_capacity;
...
// The per-cascade policies of the live set (attach and every ladder commit): the static-only
// mask, the back-face mask (ShadowCascadeSet::backface_mask: the option's index law or the
// texel law on the live extents and sizes) and each cascade's classification eps.
void MotionOutput::refresh_cascade_policy() noexcept {
    const auto& set = depth_cascades_;
...
// cascade i's eps (depth_cascade_class_eps_). The retention store answers the
// cascades it is informative about (Node::static_mask / moved_mask: promoted
// at that tier, or verified moved beyond it since); a node it does not know
// or has not verified yet (fresh, or its streak accruing) goes to the ring
// for the open cascades: one drift of this draw's world rows from its anchor
// (re-anchored beyond the base eps), compared against each open cascade's
// eps. `miss`: the ring had no anchor (first sighting, evicted, no serial or
...
std::uint8_t MotionOutput::classify_candidate_static(const MotionRoute& route, const float* rows, const float* lo, const float* hi, std::uint8_t wanted, bool& miss) noexcept {
    ...
    for (unsigned i = 0; i < depth_cascades_.count; ++i) {
        ...
        const double e = depth_cascade_class_eps_[i];
        if (d2 <= e * e) statics |= std::uint8_t(1u << i);
    }
    return statics;
}
void MotionOutput::note_candidate_draw(const MotionRoute& route) noexcept {
    ...
    // (run116) diagnosis", cause 4). A known origin nearer than the near bound stays refused.
    ...
    if (cascades && (cascade_mask & depth_cascade_static_mask_) && zwrite && shadow_ok && managed && !route.alpha_tested) {
        ...
        if (refused) {
            ...
            for (unsigned i = 0; i < depth_cascades_.count; ++i) {
                if (!(refused & (1u << i))) continue;
                ++(large ? candidates_.counts.large_admitted : candidates_.counts.static_only_refused)[i];
                if (miss && !large) ++candidates_.counts.class_miss[i];
            }
            if (!large) {
                cascade_mask = std::uint8_t(cascade_mask & ~refused);
                const bool was_admitted = admitted;
                admitted = cascade_mask != 0;
                // A draw the static gate refused from every cascade it met is no candidate (no record,
...
void MotionOutput::publish_shadow_replay_candidates() noexcept {
    ...
    // Importance drop order (shadow-cascade-extents.md, "Caster pool control"):
    // the draw path recorded every admitted caster; each cascade over its cap
    // now keeps its largest, the records left without a cascade go (their
    ...
    {
        const auto samples = sun_latch_.frame;
        sun_verdict_ = sun_latch_.end_frame();
        if (sun_verdict_ == shadow_replay::SunVerdict::Relatched && depth_replay_) depth_replay_->invalidate_retained();
        const float* sun = sun_latch_.frame_sun();
...
// replay of the same frame, before the fog and the resolve. Every precondition is
...
void MotionOutput::run_sun_shadow_apply() noexcept {
    ...
    if (!skip && depth_cascades_on()) {
        ...
        // camera motion). Valid: replayed this frame, or, for the far cascade of
        // a set, on the previous frame (the alternate-frame budget); anything
