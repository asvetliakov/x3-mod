...
constexpr engine_patch::SiteSpec site_spec = {
    "cockpit_update_pose", site_va, {0x83, 0x7b, 0x54, 0x00, 0x0f, 0x84, 0x09, 0x02, 0x00, 0x00}, 10, 0, 6};
...
constexpr unsigned cockpit_scene_camera = 0x08, cockpit_ref_object = 0x0c, cockpit_view_object = 0x10,
                   cockpit_camera = 0x58, cockpit_view_rel = 0xf0;
constexpr unsigned cockpit_view_mode = 0x150, cockpit_flags = 0x1a0, cockpit_connect_mode = 0x1c0,
                   cockpit_aim_gun = 0x1d8, cockpit_tracked = 0x1e0, cockpit_tracking_mode = 0x1e4,
                   cockpit_sector = 0x1fc;
constexpr unsigned camera_position = 0x30, camera_basis = 0x40, camera_fov = 0x298, camera_plane_w = 0x300,
                   camera_plane_h = 0x304;
...
bool writable(uintptr_t address, size_t size) {
    ...
    if (!(info.Protect & (PAGE_READWRITE | PAGE_EXECUTE_READWRITE | PAGE_WRITECOPY | PAGE_EXECUTE_WRITECOPY)))
        return false;
...
void note_cockpit_seen(uintptr_t cockpit) { // caller holds stats_lock
    for (unsigned i = 0; i < stats_.cockpits_seen && i < max_cockpits_seen; ++i)
        if (cockpits_seen[i] == cockpit) return;
...
double fix_scene_camera(uintptr_t cockpit_scene, const chase::Mat3& view_rel_now, const chase::Mat3& view_rel_vanilla) {
    ...
    if (!cockpit_scene || (cockpit_scene & 3) || !engine_memory::read(cockpit_scene, bytes, scene_camera_block))
        return -1.0;
    int32_t rows[12];
    std::memcpy(rows, bytes + camera_basis, 48);
    ...
    int32_t p[3];
    std::memcpy(p, bytes + camera_position, 12);
    const chase::Vec3 position = chase::mul(
        chase::mul(chase::Vec3{double(p[0]), double(p[1]), double(p[2])}, chase::transpose(basis)), fixed);
    int32_t out_position[3];
    if (!chase::to_fixed(fixed, rows) || !chase::to_int(position, out_position) ||
        !writable(cockpit_scene, scene_camera_block))
        return -1.0;
    std::memcpy(reinterpret_cast<void*>(cockpit_scene + camera_position), out_position, 12);
    for (int r = 0; r < 3; ++r)
        std::memcpy(reinterpret_cast<void*>(cockpit_scene + camera_basis + 16 * r), rows + 4 * r, 12);
...
void handle(uint32_t* regs) {
    ...
    auto u32 = [](const unsigned char* p, unsigned off) {
        uint32_t v;
        std::memcpy(&v, p + off, 4);
        return v;
    };
    auto i32 = [](const unsigned char* p, unsigned off) {
        int32_t v;
        std::memcpy(&v, p + off, 4);
        return v;
    };
    ...
    const uintptr_t ref_object = u32(cockpit_bytes, cockpit_ref_object),
                    view_object = u32(cockpit_bytes, cockpit_view_object);
    ...
    const bool active = admits_pose(ref_object, view_object,
                                    view_object == 0 && chase_transition::active_control_cockpit(cockpit));
    AcquireSRWLockExclusive(&stats_lock);
    note_cockpit_seen(cockpit);
    if (!active)
        ++stats_.inactive;
    ...
    if (cockpit != last_cockpit) {
        last_cockpit = cockpit;
        chase::note_gap(pipeline);
    }
    ...
    if (ok) {
        ...
        if (engine_memory::read(default_view_plane_slot, &defaults, 4) && defaults)
            engine_memory::read(defaults + 0x28, plane, 8);
    }
    LARGE_INTEGER stamp{};
    QueryPerformanceCounter(&stamp);
    const uint64_t now = uint64_t(stamp.QuadPart);
    const double dt = qpc_last && now > qpc_last && qpc_frequency ? double(now - qpc_last) / double(qpc_frequency)
                                                                  : 0.0;
    qpc_last = now;
    if (!ok) {
        chase::note_gap(pipeline);
    ...
    int32_t fixed[12];
    rows(camera_bytes, camera_basis, fixed);
    in.vanilla_cam = chase::from_fixed(fixed);
    rows(cockpit_bytes, cockpit_view_rel, fixed);
    in.view_rel = chase::from_fixed(fixed);
    in.vanilla_pos = {double(i32(camera_bytes, camera_position)), double(i32(camera_bytes, camera_position + 4)),
                      double(i32(camera_bytes, camera_position + 8))};
    ...
    in.view_mode = u32(cockpit_bytes, cockpit_view_mode);
    in.connect_mode = u32(cockpit_bytes, cockpit_connect_mode);
    in.flags_1a0 = u32(cockpit_bytes, cockpit_flags);
    in.ref_object = ref_object;
    in.sector = u32(cockpit_bytes, cockpit_sector);
    ...
    const chase::Vec3 boom_local = derived_ok ? chase::mul(in.vanilla_pos - in.ship_pos, chase::transpose(derived))
                                              : chase::Vec3{};
    ...
    double scene_fix_deg = -1.0;
    if (result.verdict == chase::Verdict::Applied) {
        int32_t position[3], basis_rows[12], rel_rows[12];
        rows(camera_bytes, camera_basis, basis_rows);
        rows(cockpit_bytes, cockpit_view_rel, rel_rows); // keep the 4th words
        if (chase::to_int(pose.pos, position) && chase::to_fixed(pose.basis, basis_rows) &&
            chase::to_fixed(pose.view_rel, rel_rows) &&
            ...
            written = true;
            if (scene_fix_enabled)
                scene_fix_deg = fix_scene_camera(u32(cockpit_bytes, cockpit_scene_camera), pose.view_rel, in.view_rel);
        } else {
            write_refused = true;
            chase::note_gap(pipeline);
        }
    ...
    if (scene_fix_deg >= 0) {
        ++stats_.scene_fixed;
        stats_.scene_fix_deg = scene_fix_deg;
    }
    if (written && std::fabs(result.fov_factor - stats_.fov_factor) > 1e-4) {
        stats_.fov_factor = result.fov_factor;
        stats_.fov_half_vfov_tan = in.half_vfov_tan;
        stats_.fov_change_frame = stats_.frames;
        ++stats_.fov_changes;
    }
    stats_.snaps = pipeline.snaps;
    stats_.coalesced = pipeline.coalesced;
    stats_.clamps = pipeline.clamps;
    ...
    stats_.last_verdict = unsigned(result.verdict);
    if (result.snapped || result.coalesced) stats_.last_snap_reason = result.snap_reason;
    stats_.view_mode = in.view_mode;
    stats_.connect_mode = in.connect_mode;
    stats_.flags_1a0 = in.flags_1a0;
    stats_.tracking_mode = tracking_mode;
    stats_.target_locked = in.target_locked;
    stats_.lag_deg = result.lag_deg;
    stats_.pos_lag = result.pos_lag;
    stats_.distance = result.distance;
    stats_.dt_ms = dt * 1000.0;
    stats_.half_vfov_tan = in.half_vfov_tan;
    stats_.boom_local[0] = boom_local.x;
    stats_.boom_local[1] = boom_local.y;
    stats_.boom_local[2] = boom_local.z;
    if (written) {
        basis_deviation_deg = anchor.basis_valid && derived_ok && chase::orthonormality_error(anchor.basis) < 0.05
                                  ? angle_between_deg(derived, anchor.basis)
                                  : -1.0;
        stats_.native_base_domain = anchor.base_domain;
        stats_.domain_delta = anchor.render_position_valid ? chase::length(anchor.position - anchor.render_position)
                                                           : -1.0;
        stats_.render_basis_deviation = anchor.render_basis_valid && derived_ok &&
                                                chase::orthonormality_error(anchor.render_basis) < 0.05
                                            ? angle_between_deg(derived, anchor.render_basis)
                                            : -1.0;
        if (anchor.render_position_valid) {
            if (!stats_.window_domain_samples)
                stats_.domain_delta_min = stats_.domain_delta_max = stats_.domain_delta;
            else {
        ...
        if (!stats_.first.captured) {
            ...
            f.captured = true;
            f.handler_frame = stats_.frames;
            f.half_vfov_tan = in.half_vfov_tan;
            f.boom_local[0] = boom_local.x;
            f.boom_local[1] = boom_local.y;
            f.boom_local[2] = boom_local.z;
            f.fov298 = fov298;
            f.plane_w = uint32_t(w);
            f.plane_h = uint32_t(h);
            f.default_plane_h = plane[1];
            f.view_mode = in.view_mode;
            f.connect_mode = in.connect_mode;
            f.flags_1a0 = in.flags_1a0;
            f.tracking_mode = tracking_mode;
            f.aim_gun = u32(cockpit_bytes, cockpit_aim_gun);
            f.ref_object = ref_object;
            f.view_object = view_object;
            f.tracked_object = tracked;
            f.target_locked = in.target_locked;
            f.native_base_domain = anchor.base_domain;
            f.domain_delta = stats_.domain_delta;
        }
    }
    ...
    chase_fire::camera_context(cockpit, ref_object, camera, in.view_mode, in.connect_mode, in.flags_1a0, context_ok,
                               now);
...
void* emit_stub(void*** next_out) {
    ...
    for (unsigned i = 0; i < 8; ++i) {
        ...
        e.byte(0x24);
        e.byte(static_cast<unsigned char>(16 * i));
    }
    e.byte(0x8d);
    e.byte(0x84);
    e.byte(0x24);
    ...
    e.byte(0x50);
    e.byte(0xe8);
    e.rel32(reinterpret_cast<const void*>(&x3m_chase_camera_enter));
    e.byte(0x83);
    e.byte(0xc4);
    e.byte(0x04);
    for (unsigned i = 0; i < 8; ++i) {
    ...
    while (e.ok() && reinterpret_cast<uintptr_t>(e.here()) < next)
        e.byte(0xcc); // e.ok(): an exhausted emitter reports here() == nullptr
...
bool initialize() {
    ...
    qpc_frequency = f.QuadPart > 0 ? uint64_t(f.QuadPart) : 0;
    const char* refusal = nullptr;
    if (!tunables_ok)
        refusal = "invalid_tunables";
    else if (!qpc_frequency)
        refusal = "no_qpc";
    else if (!object_trace::executable_verified())
        refusal = "executable_mismatch";
    else if (!engine_patch::install_window_open())
        refusal = "late_claim";
    if (refusal) {
        state = refusal;
        log("chase_camera requested=1 installed=0 status=%s window_closed_by=%s", refusal,
            engine_patch::install_window_reason() ? engine_patch::install_window_reason() : "-");
        SetLastError(error);
        return false;
    }
    ...
    if (okay) {
        void** next = nullptr;
        void* stub = emit_stub(&next);
        if (!stub || !next) {
            site.status = "stub_failed";
            okay = false;
        } else if (!engine_patch::store_pointer(next, *site.entry)) {
            site.status = "chain_write_failed";
            okay = false;
        } else if (!engine_patch::push_front(site, stub)) {
            site.status = "chain_failed";
            okay = false;
        }
        if (!okay && site.patched_in)
            engine_patch::restore(site); // the plain tail would be harmless, but keep the vanilla bytes
    }
    ...
    log("chase_camera requested=1 installed=%u status=%s site=0x%08lx length=%u rel32_offset=%u atomic_write=%u arena_used=%u "
        ...
        unsigned(okay), state.load(), static_cast<unsigned long>(site_va), site_spec.length, site_spec.rel32_offset,
        unsigned(site.atomic_write), engine_patch::arena_used(), tunables.rot_tau, tunables.pos_tau, tunables.offset_y,
        tunables.pitch_down_deg, tunables.distance_scale, unsigned(tunables.fov_compensate), tunables.lag_clamp_deg,
        tunables.pos_lag_clamp, tunables.combat_tightness,
        tunables.combat_tightness > 0 ? "tracking_1e4_unverified" : "off", tunables.max_dt,
        tunables.snap_coalesce_frames, unsigned(scene_fix_enabled), unsigned(timing_enabled));
...
Stats stats() {
    ...
    s.installed = site.patched_in;
    s.status = state.load();
    s.atomic_write = unsigned(site.atomic_write);
    return s;
}
std::uint32_t snap_generation() {
    return snap_epoch.load(std::memory_order_relaxed);
}
std::uint32_t pose_write_count() {
    return pose_writes.load(std::memory_order_relaxed);
}
bool pose_applied_since(std::uint32_t mark) {
    return pose_gate_open(site.patched_in, pose_written.load(std::memory_order_relaxed),
                          pose_writes.load(std::memory_order_relaxed), mark);
}
void report(std::uint64_t frame) {
    ...
    if (s.first.captured && !s.first.logged) {
        ...
        log("chase_camera first_applied frame=%llu handler_frame=%llu half_vfov_tan=%.4f fov298=0x%lx plane_w=0x%lx plane_h=0x%lx default_plane_h=0x%lx mode=%lu connect=%lu flags_1a0=0x%lx "
            ...
            frame, f.handler_frame, f.half_vfov_tan, static_cast<unsigned long>(f.fov298),
            static_cast<unsigned long>(f.plane_w), static_cast<unsigned long>(f.plane_h),
            static_cast<unsigned long>(f.default_plane_h), static_cast<unsigned long>(f.view_mode),
            static_cast<unsigned long>(f.connect_mode), static_cast<unsigned long>(f.flags_1a0),
            static_cast<unsigned long>(f.tracking_mode), static_cast<unsigned long>(f.aim_gun),
            static_cast<unsigned long>(f.tracked_object), unsigned(f.target_locked),
            static_cast<unsigned long>(f.ref_object), static_cast<unsigned long>(f.view_object), f.boom_local[0],
            f.boom_local[1], f.boom_local[2], s.cockpits_seen, f.native_base_domain ? "base" : "render",
            f.domain_delta);
    }
    if (s.fov_changes != fov_changes_logged && fov_rows < max_fov_rows) {
        ...
        log("chase_fov_compensate frame=%llu handler_frame=%llu factor=%.4f half_vfov_tan=%.4f enabled=%u distance_scale=%.3f boom_scale=%.4f changes=%llu row=%u/%u",
            frame, s.fov_change_frame, s.fov_factor, s.fov_half_vfov_tan, unsigned(tunables.fov_compensate),
            tunables.distance_scale, tunables.distance_scale * s.fov_factor, s.fov_changes - fov_changes_logged,
            fov_rows, max_fov_rows);
    ...
    if (timing_enabled)
        log("chase_camera_window frame=%llu applied=%llu native_base=%llu native_render=%llu native_branch=%s domain_delta=%.1f domain_samples=%llu domain_delta_min=%.1f domain_delta_max=%.1f "
            ...
            frame, s.window_applied, s.window_base_domain, s.window_applied - s.window_base_domain,
            s.native_base_domain ? "base" : "render", s.domain_delta, s.window_domain_samples,
            s.window_domain_samples ? s.domain_delta_min : 0.0, s.window_domain_samples ? s.domain_delta_max : 0.0,
    ...
    log("chase_camera frame=%llu status=%s frames=%llu applied=%llu refused=%llu refused_inactive=%llu admitted_unbound=%llu cockpits_seen=%u snaps=%llu coalesced=%llu clamps=%llu write_refused=%llu "
        ...
        frame, s.status, s.frames, s.applied, s.refused, s.inactive, s.unbound, s.cockpits_seen, s.snaps, s.coalesced,
        s.clamps, s.write_refused, static_cast<unsigned long>(s.last_verdict),
        static_cast<unsigned long>(s.last_snap_reason), static_cast<unsigned long>(s.view_mode),
        static_cast<unsigned long>(s.connect_mode), static_cast<unsigned long>(s.flags_1a0),
        static_cast<unsigned long>(s.tracking_mode), unsigned(s.target_locked), s.locked_frames, s.lag_deg, s.pos_lag,
        s.distance, s.dt_ms, deviation, s.half_vfov_tan, s.boom_local[0], s.boom_local[1], s.boom_local[2],
        s.scene_fixed, s.scene_fix_deg);
}
