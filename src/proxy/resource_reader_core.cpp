...
const char* reason_name(unsigned reason) noexcept {
    static const char* const names[] = {"none",   "unopened", "gz_handle",  "progress", "tell",    "state",
                                        "length", "scratch",  "short_read", "not_gzip", "method",  "reserved",
                                        "header", "empty",    "alloc",      "init",     "inflate", "size"};
...
Result decode(const Environment& env, FileObject* object, bool game_buffer) noexcept {
    ...
    if (catalogue) {
        ...
        if (object->cursor != 0 || object->length < 0 || object->offset < 0 || position != object->offset)
            return fail(r, Reason::State);
        begin_offset = object->offset;
        ...
        if (length < 0) {
            env.fseek(file, position, seek_set);
            return fail(r, Reason::Tell);
        }
    }
    ...
    const uint64_t t_scan = tick();
    r.read_ticks = t_scan - t_read;
    ...
    if (scratch[0] == 0x1f && scratch[1] == 0x8b) {
        ...
        if (((scratch[1] ^ key) & 0xff) != 0x1f || ((scratch[2] ^ key) & 0xff) != 0x8b)
            return fail(r, Reason::NotGzip, &guard, scratch);
        start = 1;
        r.scrambled = true;
        xor_bytes(scratch + 1, size_t(length) - 1, key);
    }
    const unsigned char* gz = scratch + start;
    const uint32_t gzlen = uint32_t(length) - start;
    ...
    uint32_t p = 10;
    const uint32_t limit = gzlen - 8;
    if (flg & 4) {
        if (p + 2 > limit) return fail(r, Reason::Header, &guard, scratch);
        const uint32_t xlen = uint32_t(gz[p]) | uint32_t(gz[p + 1]) << 8;
        p += 2 + xlen;
    }
    if (flg & 8) {
        while (p < limit && gz[p]) ++p;
        ++p;
    }
    if (flg & 0x10) {
        while (p < limit && gz[p]) ++p;
    ...
    const uint64_t t_alloc = tick();
    r.scan_ticks = t_alloc - t_scan;
    unsigned char* out = static_cast<unsigned char*>(game_buffer ? env.malloc(isize)
                                                                 : HeapAlloc(GetProcessHeap(), 0, isize));
    const uint64_t t_inflate = tick();
    r.alloc_ticks = t_inflate - t_alloc;
    ...
    ZStream z{};
    if (env.inflateInit2_(&z, -15, "1.2.3", int(sizeof z)) != z_ok) {
        release();
        return fail(r, Reason::Init, &guard, scratch);
    }
    z.next_in = gz + p;
    z.avail_in = gzlen - p;
    z.next_out = out;
    ...
    const uint32_t consumed = gzlen - p - z.avail_in; // deflate bytes zlib needed (exact: inflate hands back whole
                                                      // unused bytes)
    env.inflateEnd(&z);
    r.inflate_ticks = tick() - t_inflate;
    if (status != z_stream_end) {
        release();
        return fail(r, Reason::Inflate, &guard, scratch);
    }
    if (produced != isize) {
        release();
        return fail(r, Reason::Size, &guard, scratch);
    }
    ...
    r.outcome = Outcome::Handled;
    r.buffer = out;
    r.size = isize;
    if (game_buffer) {
        // Exactly the original's bookkeeping: three byte counters, the allocation count, the two size globals, the
        // record cursor.
        for (unsigned i = 0; i < 3; ++i)
            if (env.counters[i]) *env.counters[i] += isize;
        if (env.counters[3]) *env.counters[3] += 1;
        for (unsigned i = 0; i < 2; ++i)
            if (env.size_globals[i]) *env.size_globals[i] = isize;
...
inline void account(const Result& r) noexcept { // a handled file: counts, bytes, total and phase ticks
    add64(&stats.handled, 1);
    add64(&stats.bytes_in, r.extent);
    add64(&stats.bytes_out, r.size);
    add64(&stats.ticks, r.ticks);
    max64(&stats.max_ticks, r.ticks);
    add64(&stats.read_ticks, r.read_ticks);
    add64(&stats.scan_ticks, r.scan_ticks);
    add64(&stats.alloc_ticks, r.alloc_ticks);
    add64(&stats.inflate_ticks, r.inflate_ticks);
...
Statistics statistics() {
    Statistics s;
    s.calls = load64(&stats.calls);
    s.handled = load64(&stats.handled);
    s.fallbacks = load64(&stats.fallbacks);
    s.bytes_in = load64(&stats.bytes_in);
    s.bytes_out = load64(&stats.bytes_out);
    s.ticks = load64(&stats.ticks);
    s.max_ticks = load64(&stats.max_ticks);
    s.verify_files = load64(&stats.verify_files);
    s.verify_equal = load64(&stats.verify_equal);
    s.verify_mismatched = load64(&stats.verify_mismatched);
    s.verify_original_null = load64(&stats.verify_original_null);
    s.original_ticks = load64(&stats.original_ticks);
    s.catalogue = load64(&stats.catalogue);
    s.scrambled = load64(&stats.scrambled);
    s.cursor_short = load64(&stats.cursor_short);
    s.read_ticks = load64(&stats.read_ticks);
    s.scan_ticks = load64(&stats.scan_ticks);
    s.alloc_ticks = load64(&stats.alloc_ticks);
    s.inflate_ticks = load64(&stats.inflate_ticks);
...
extern "C" uint32_t __cdecl x3m_resource_read_entry(uint32_t* regs) {
    ...
    if (bound_mode == Mode::Fast) {
        ...
        add64(&stats.fallbacks, 1);
        add64(&stats.reasons[static_cast<unsigned>(r.reason)], 1);
        SetLastError(error);
        return 0;
    }
    if (bound_mode == Mode::Verify && continuation && *continuation) {
        ...
        add64(&stats.original_ticks, t1 - t0);
        add64(&stats.verify_files, 1);
        VerifyEvent e{};
        e.size = r.size;
        e.catalogue = r.catalogue;
        e.scrambled = r.scrambled;
        e.our_ticks = r.ticks;
        e.original_ticks = t1 - t0;
        e.original_null = original == nullptr;
        e.original_size = bound_env.size_globals[0] ? *bound_env.size_globals[0] : 0;
        e.globals_ok = bound_env.size_globals[0] && bound_env.size_globals[1]
                           ? (*bound_env.size_globals[0] == r.size && *bound_env.size_globals[1] == r.size)
                           : true;
        e.counters_ok = true;
        for (unsigned i = 0; i < 3; ++i)
            if (bound_env.counters[i] && *bound_env.counters[i] - counters_before[i] != r.size) e.counters_ok = false;
        if (bound_env.counters[3] && *bound_env.counters[3] - counters_before[3] != 1) e.counters_ok = false;
        e.cursor = object->cursor;
        e.expected_cursor = r.expected_cursor;
        e.cursor_ok = !r.catalogue || object->cursor == r.expected_cursor;
        e.position = bound_env.ftell(object->file);
        e.expected_position = r.expected_position;
        e.position_ok = e.position == r.expected_position;
        e.mismatches = 0;
        e.first_mismatch = 0;
        if (original) {
            const auto* a = static_cast<const unsigned char*>(r.buffer);
            const auto* b = static_cast<const unsigned char*>(original);
            const uint32_t n = e.original_size < r.size ? e.original_size : r.size;
            for (uint32_t i = 0; i < n; ++i)
                if (a[i] != b[i]) {
                    if (!e.mismatches) e.first_mismatch = i;
                    ++e.mismatches;
                }
            if (e.original_size != r.size) {
                if (!e.mismatches) e.first_mismatch = n;
                e.mismatches += e.original_size > r.size ? e.original_size - r.size : r.size - e.original_size;
            }
        } else
            add64(&stats.verify_original_null, 1);
        e.equal = !e.original_null && !e.mismatches && e.globals_ok && e.counters_ok && e.cursor_ok && e.position_ok;
        if (e.equal)
            add64(&stats.verify_equal, 1);
        else
            add64(&stats.verify_mismatched, 1);
        (void)globals_before;
...
struct PoolEntry {
    void* file;
    unsigned state;
    char path[pool_path_length];
}; // state 0 free slot, 1 in use, 2 kept
...
inline bool same_path(const char* a, const char* b) noexcept {
    while (*a && *a == *b) {
        ++a;
        ++b;
    }
...
inline bool stream_error(void* file) noexcept {
    return (*reinterpret_cast<const uint32_t*>(static_cast<const char*>(file) + pool_env.flag_offset) &
            pool_env.error_flag) != 0;
}
}
void pool_bind(const PoolEnvironment& env) noexcept {
    pool_env = env;
    pool_bound = env.fopen && env.fclose;
}
extern "C" void* __cdecl x3m_pool_fopen(const char* path, const char* mode) {
    ...
    if (read_binary) {
        ...
        for (auto& e : pool)
            if (e.state == 2 && same_path(e.path, path)) {
                e.state = 1;
                void* f = e.file;
                --pool_stats.held;
                ReleaseSRWLockExclusive(&pool_lock);
                add64(&pool_stats.reused, 1);
                return f;
            }
        ReleaseSRWLockExclusive(&pool_lock);
    }
...
extern "C" int __cdecl x3m_pool_fclose(void* file) {
    ...
    AcquireSRWLockExclusive(&pool_lock);
    for (auto& e : pool)
        if (e.state == 1 && e.file == file) {
            if (stream_error(file)) {
                e.state = 0;
                e.file = nullptr;
                ReleaseSRWLockExclusive(&pool_lock);
                add64(&pool_stats.errors, 1);
...
PoolStatistics pool_statistics() noexcept {
    PoolStatistics s;
    s.opens = load64(&pool_stats.opens);
    s.reused = load64(&pool_stats.reused);
    s.real_opens = load64(&pool_stats.real_opens);
    s.closes = load64(&pool_stats.closes);
    s.kept = load64(&pool_stats.kept);
    s.real_closes = load64(&pool_stats.real_closes);
    s.errors = load64(&pool_stats.errors);
    s.full = load64(&pool_stats.full);
    AcquireSRWLockExclusive(&pool_lock);
    s.held = pool_stats.held;
    ReleaseSRWLockExclusive(&pool_lock); // held is maintained under the lock
    return s;
}
