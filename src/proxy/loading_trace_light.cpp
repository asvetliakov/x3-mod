...
template <typename T> T original(unsigned op) noexcept {
    return reinterpret_cast<T>(originals[op]);
}
...
void record_path(unsigned op, LPCSTR path) noexcept {
    ...
    auto& record = path_records[index];
    record.op = op;
    unsigned i = 0;
    for (; i < path_record_length - 1 && path[i]; ++i) record.path[i] = path[i];
    record.path[i] = 0;
}
}
...
bool nesting_available() noexcept {
    return span_slot != TLS_OUT_OF_INDEXES;
}
void set_original(unsigned op, PVOID value) noexcept {
    if (op < row_count) originals[op] = value;
}
PVOID get_original(unsigned op) noexcept {
    return op < row_count ? originals[op] : nullptr;
}
...
void take(unsigned op, Sample& out) noexcept {
    ...
    auto& r = rows[op];
    out.count = exchange64(&r.calls, 0);
    out.failures = exchange64(&r.failures, 0);
    out.pending = exchange64(&r.pending, 0);
    out.ambiguous = exchange64(&r.ambiguous, 0);
    out.bytes = exchange64(&r.bytes, 0);
    out.inclusive_ticks = exchange64(&r.inclusive, 0);
    out.exclusive_ticks = exchange64(&r.exclusive, 0);
    out.maximum_ticks = exchange64(&r.maximum, 0);
    out.overhead_ticks = exchange64(&r.overhead, 0);
}
...
void Span::begin(unsigned operation) noexcept {
    caller_error = GetLastError();
    op = operation;
    children = 0;
    begin_ticks = tick();
    if (span_slot != TLS_OUT_OF_INDEXES) {
        parent = static_cast<Span*>(TlsGetValue(span_slot));
        TlsSetValue(span_slot, this);
    } else
        parent = nullptr;
}
void Span::before_call() const noexcept {
...
BOOL WINAPI file_read(HANDLE file, LPVOID buffer, DWORD requested, LPDWORD read, LPOVERLAPPED overlapped) {
    ...
    span.finish(!result && !pending, result && read ? *read : 0, pending);
    return result;
}
DWORD WINAPI file_seek(HANDLE file, LONG distance, PLONG high, DWORD method) {
    Span span;
    span.begin(index(Operation::FileSeek));
    span.before_call();
    ...
    span.finish(false, 0, false, result == INVALID_SET_FILE_POINTER && error != NO_ERROR);
    return result;
}
namespace {
...
template <typename T> void store_relaxed(T& field, T value) noexcept {
    __atomic_store_n(&field, value, __ATOMIC_RELAXED);
}
template <typename T> T load_relaxed(const T& field) noexcept {
...
void cursor_record(uint32_t op, uint32_t a, uint32_t b, uint32_t result) noexcept {
    const DWORD error = GetLastError();
    if (op == 0)
        InterlockedIncrement(&cursor_set_calls);
    else
        InterlockedIncrement(&cursor_pos_calls);
    while (InterlockedCompareExchange(&cursor_lock, 1, 0) != 0) YieldProcessor();
    bool changed;
    if (op == 0) {
        changed = !cursor_have_set || a != cursor_last_handle;
        cursor_have_set = true;
        cursor_last_handle = a;
    } else {
        changed = !cursor_have_pos || a != cursor_last_x || b != cursor_last_y || result != cursor_last_result;
        cursor_have_pos = true;
        cursor_last_x = a;
        cursor_last_y = b;
        cursor_last_result = result;
    }
    if (changed) {
        const uint32_t tick = GetTickCount(), thread = GetCurrentThreadId(); // only for a recorded change: the game
                                                                             // repeats SetCursor(NULL) per WM_SETCURSOR
        ...
        store_relaxed(slot.op, op);
        store_relaxed(slot.a, a);
        store_relaxed(slot.b, b);
        store_relaxed(slot.result, result);
        store_relaxed(slot.tick, tick);
        store_relaxed(slot.thread, thread);
        __atomic_thread_fence(__ATOMIC_RELEASE);
...
void cursor_observe(bool on) noexcept {
    InterlockedExchange(&cursor_observing, on ? 1 : 0);
}
unsigned cursor_drain(uint32_t after, window_trace::core::CursorEvent* out, unsigned capacity, uint32_t* newest,
                      window_trace::core::CursorCounts* counts) noexcept {
    ...
    if (last >= window && first < last - window + 1) {
        dropped = last - window + 1 - first;
        first = last - window + 1;
    } // more than 64 changes since the previous drain
    unsigned copied = 0;
    for (uint32_t sequence = first; sequence <= last && sequence != 0; ++sequence) {
        const auto& slot = cursor_ring[(sequence - 1) & (window - 1)];
        if (copied >= capacity || __atomic_load_n(&slot.seq, __ATOMIC_ACQUIRE) != sequence) {
            ++dropped;
            continue;
        } // overwritten (or being written) since `last`
        auto& e = out[copied];
        e.op = load_relaxed(slot.op);
        e.a = load_relaxed(slot.a);
        e.b = load_relaxed(slot.b);
        e.result = load_relaxed(slot.result);
        e.tick = load_relaxed(slot.tick);
        e.thread = load_relaxed(slot.thread);
        __atomic_thread_fence(__ATOMIC_ACQUIRE);
        if (__atomic_load_n(&slot.seq, __ATOMIC_RELAXED) != sequence) {
            ++dropped;
            continue;
        } // overwritten while copied
        e.seq = sequence;
        ++copied;
    }
    if (counts) {
        counts->set = uint32_t(InterlockedExchange(&cursor_set_calls, 0));
        counts->pos = uint32_t(InterlockedExchange(&cursor_pos_calls, 0));
...
HCURSOR WINAPI cursor_set(HCURSOR value) {
    Span span;
    span.begin(index(Operation::CursorSet));
    span.before_call();
    HCURSOR result = original<decltype(&SetCursor)>(span.op)(value);
    span.finish();
    if (cursor_observing)
        cursor_record(0, uint32_t(reinterpret_cast<uintptr_t>(value)), uint32_t(reinterpret_cast<uintptr_t>(result)),
                      0);
    return result;
}
BOOL WINAPI cursor_position(int x, int y) {
    Span span;
    span.begin(index(Operation::CursorPosition));
    span.before_call();
...
void* __cdecl gz_open_traced(const char* path, const char* mode) {
    Span span;
    span.begin(index(Operation::GzOpen));
    span.before_call();
...
int __cdecl gz_close_traced(void* file) {
    Span span;
    span.begin(index(Operation::GzClose));
    span.before_call();
...
int __cdecl gz_write(void* file, const void* data, unsigned size) {
    Span span;
    span.begin(index(Operation::GzWrite));
    span.before_call();
    int result = original<GzWriteFn>(span.op)(file, data, size);
    span.finish(result <= 0 && size > 0, result > 0 ? unsigned(result) : 0);
    return result;
}
int __cdecl inflate_stream(void* stream, int flush) {
    Span span;
    span.begin(index(Operation::Inflate));
    span.before_call();
...
int __cdecl inflate_end(void* stream) {
    Span span;
    span.begin(index(Operation::InflateEnd));
    span.before_call();
...
BOOL WINAPI crypt_acquire_context(HCRYPTPROV* provider, LPCSTR container, LPCSTR name, DWORD type, DWORD flags) {
    Span span;
    span.begin(index(Operation::CryptAcquire));
    span.before_call();
...
BOOL WINAPI crypt_hash_data(HCRYPTHASH hash, const BYTE* data, DWORD length, DWORD flags) {
    Span span;
    span.begin(index(Operation::CryptHash));
    span.before_call();
    BOOL result = original<decltype(&CryptHashData)>(span.op)(hash, data, length, flags);
    span.finish(!result, length);
    return result;
}
BOOL WINAPI crypt_verify_signature(HCRYPTHASH hash, const BYTE* signature, DWORD length, HCRYPTKEY key,
                                   LPCSTR description, DWORD flags) {
    Span span;
    span.begin(index(Operation::CryptVerify));
    span.before_call();
...
BOOL WINAPI crypt_get_hash_param(HCRYPTHASH hash, DWORD parameter, BYTE* data, DWORD* length, DWORD flags) {
    Span span;
    span.begin(index(Operation::CryptHashParam));
    span.before_call();
...
BOOL WINAPI crypt_destroy_hash(HCRYPTHASH hash) {
    Span span;
    span.begin(index(Operation::CryptHashDestroy));
    span.before_call();
...
BOOL WINAPI crypt_destroy_key(HCRYPTKEY key) {
    Span span;
    span.begin(index(Operation::CryptKeyDestroy));
    span.before_call();
...
BOOL WINAPI create_directory(LPCSTR path, LPSECURITY_ATTRIBUTES security) {
    Span span;
    span.begin(index(Operation::DirectoryCreate));
    record_path(span.op, path);
    span.before_call();
...
BOOL WINAPI delete_file(LPCSTR path) {
    Span span;
    span.begin(index(Operation::FileDelete));
    record_path(span.op, path);
    span.before_call();
...
BOOL WINAPI move_file(LPCSTR from, LPCSTR to) {
    Span span;
    span.begin(index(Operation::FileMove));
    record_path(span.op, to);
    span.before_call();
...
BOOL WINAPI move_file_ex(LPCSTR from, LPCSTR to, DWORD flags) {
    Span span;
    span.begin(index(Operation::FileMoveEx));
    record_path(span.op, to);
    span.before_call();
...
BOOL WINAPI write_file(HANDLE file, LPCVOID data, DWORD size, LPDWORD written, LPOVERLAPPED overlapped) {
    Span span;
    span.begin(index(Operation::FileWrite));
    span.before_call();
    BOOL result = original<decltype(&WriteFile)>(span.op)(file, data, size, written, overlapped);
    const DWORD error = GetLastError();
    const bool pending = !result && error == ERROR_IO_PENDING;
    span.finish(!result && !pending, result && written ? *written : 0, pending);
    return result;
}
DWORD WINAPI get_file_type(HANDLE file) {
    Span span;
    span.begin(index(Operation::FileType));
    span.before_call();
    DWORD result = original<decltype(&GetFileType)>(span.op)(file);
    const DWORD error = GetLastError();
    span.finish(result == FILE_TYPE_UNKNOWN && error != NO_ERROR);
    return result;
}
BOOL WINAPI close_handle(HANDLE handle) {
    Span span;
    span.begin(index(Operation::HandleClose));
    span.before_call();
...
namespace {
struct ShadowEntry {
    uint32_t site, slot, ret, ctx;
    uint64_t begin;
};
struct Shadow {
    unsigned depth;
    ShadowEntry entries[probe_shadow_depth];
};
...
Shadow* shadow() noexcept {
    ...
    if (!s) {
        ...
        if (!s || !TlsSetValue(shadow_slot, s)) {
...
inline bool plausible(uint32_t pointer) noexcept {
    return pointer >= 0x10000u && pointer < 0x7fff0000u && (pointer & 3) == 0;
}
inline bool object_flags(uint32_t object, uint32_t& flags) noexcept {
    return plausible(object) && probe_read32(object + 4, flags);
}
void note_caller(ProbeRow& row, uint32_t address) noexcept {
    for (unsigned i = 0; i < probe_caller_limit; ++i) {
        if (row.caller_address[i] == address) {
            add64(&row.caller_calls[i], 1);
            return;
        }
        if (!row.caller_address[i]) {
            if (InterlockedCompareExchange(reinterpret_cast<volatile LONG*>(&row.caller_address[i]), LONG(address),
                                           0) == 0 ||
                row.caller_address[i] == address) {
                add64(&row.caller_calls[i], 1);
                return;
            }
        }
...
unsigned shadow_blocks() noexcept {
    return unsigned(shadow_block_count);
}
unsigned shadow_block_bytes() noexcept {
    return unsigned(sizeof(Shadow));
}
bool probe_read32(uint32_t address, uint32_t& out) noexcept {
    ...
    for (unsigned i = 0; i < readable_pages && !trusted; ++i) {
        const uint32_t entry_page = readable[i].page;
        const DWORD entry_tick = readable[i].tick; // two 32-bit reads, no 64-bit copy
    ...
    if (!trusted) {
        ...
        readable[slot].tick = now;
        readable[slot].page = page; // page last: a reader that sees it also sees a tick no older than now
    }
    out = *reinterpret_cast<const volatile uint32_t*>(address);
    return true;
}
void probe_configure(unsigned site, const ProbeConfig& config) noexcept {
    if (site < probe_site_limit) probe_configs[site] = config;
}
void probe_set_exit_stub(const void* stub) noexcept {
    exit_stub = stub;
}
void probe_set_size_global(const uint32_t* address) noexcept {
    size_global = address;
}
void probe_take(unsigned site, ProbeRow& out) noexcept {
    ...
    out.calls = exchange64(&r.calls, 0);
    out.exits = exchange64(&r.exits, 0);
    out.inclusive = exchange64(&r.inclusive, 0);
    out.maximum = exchange64(&r.maximum, 0);
    out.overflow = exchange64(&r.overflow, 0);
    out.desync = exchange64(&r.desync, 0);
    out.bytes = exchange64(&r.bytes, 0);
    for (unsigned i = 0; i < probe_extra_count; ++i) out.extra[i] = exchange64(&r.extra[i], 0);
    for (unsigned i = 0; i < probe_caller_limit; ++i) {
        out.caller_address[i] = r.caller_address[i];
        out.caller_calls[i] = exchange64(&r.caller_calls[i], 0);
    }
}
extern "C" void __cdecl x3m_probe_enter(unsigned site, uint32_t* regs) {
    if (site >= probe_site_limit) return;
    const DWORD error = GetLastError();
    auto& row = probe_rows[site];
    const auto& config = probe_configs[site];
    add64(&row.calls, 1);
    uint32_t ctx = 0;
    switch (config.kind) {
    ...
    case ProbeKind::ResourceRead: {                     // EAX = file object; branch by its flags
        ctx = regs[7];
        uint32_t flags = 0;
        if (object_flags(ctx, flags))
            add64(&row.extra[!(flags & 1) ? 2 : (flags & 3) == 3 ? 1 : (flags & 5) == 5 ? 2 : 0], 1);
        break;
    }
    case ProbeKind::ReadDispatch: { // ECX = element size, EAX = count, ESI = object
        add64(&row.bytes, uint64_t(regs[6]) * regs[7]);
        uint32_t flags = 0;
        if (object_flags(regs[1], flags))
            add64(&row.extra[!(flags & 1) ? 3 : (flags & 3) == 3 ? 1 : (flags & 5) == 5 ? 2 : 0], 1);
        break;
    }
    case ProbeKind::FindWrapper: note_caller(row, regs[9]); break; // bucket by return address
    default: break;
    }
    if (config.kind != ProbeKind::CountOnly && config.kind != ProbeKind::ReadDispatch && exit_stub) { // the dispatcher
                                                                                                      // is counted at
                                                                                                      // entry only
                                                                                                      // (calls, bytes,
                                                                                                      // branch)
        Shadow* s = shadow();
        if (!s || s->depth >= probe_shadow_depth)
            add64(&row.overflow, 1);
        else {
            auto& e = s->entries[s->depth++];
            e.site = site;
            e.slot = uint32_t(reinterpret_cast<uintptr_t>(&regs[9]));
            e.ret = regs[9];
            e.ctx = ctx;
            e.begin = tick();
...
extern "C" uint32_t __cdecl x3m_probe_exit(uint32_t* regs) {
    ...
    if (s && s->depth) {
        ...
        while (s->depth > 1) {
            ...
            if (top.slot < expected) {
                add64(&probe_rows[top.site < probe_site_limit ? top.site : 0].desync, 1);
                --s->depth;
            } else
                break;
        }
        ...
        add64(&row.exits, 1);
        add64(&row.inclusive, elapsed);
        max64(&row.maximum, elapsed);
        ...
        case ProbeKind::ResourceOpen: { // AL = success; +0x04 flags after return
            if (!(regs[7] & 0xff))
                ...
                uint32_t flags = 0;
                if (object_flags(e.ctx, flags)) add64(&row.extra[(flags & 3) == 3 ? 1 : 0], 1);
            }
            break;
        }
