...
struct Module {
    ...
    Kind kind = KindOther;
    char name[64]{};
    bool used = false, loaded = false, seen = false, pinned = false;
};
struct SortedRange {
    uintptr_t begin, end;
    unsigned module;
...
struct LeafKey {
    ...
    uint16_t module;
    uint16_t slot;
}; // slot stored +1: zero means empty
struct FrameKey {
    uint32_t rva;
    uint32_t slot;
};
...
bool same(const LeafKey& a, const LeafKey& b) {
    return a.rva == b.rva && a.module == b.module && a.slot == b.slot;
}
bool same(const FrameKey& a, const FrameKey& b) {
    return a.rva == b.rva && a.slot == b.slot;
}
bool same(const PairKey& a, const PairKey& b) {
    return a.rva == b.rva && a.caller == b.caller && a.slot == b.slot;
}
...
uint32_t hash(const LeafKey& k) {
    return mix(k.rva * 0x9e3779b1u ^ (uint32_t(k.module) << 20) ^ (uint32_t(k.slot) * 0x85ebca77u));
}
uint32_t hash(const FrameKey& k) {
    return mix(k.rva * 0x9e3779b1u ^ (k.slot * 0x85ebca77u));
}
uint32_t hash(const PairKey& k) {
    return mix(k.rva * 0x9e3779b1u ^ (k.caller * 0xc2b2ae3du) ^ (k.slot * 0x85ebca77u));
}
template <typename Key, unsigned Capacity> struct Table {
    static_assert((Capacity & (Capacity - 1)) == 0);
    Key keys[Capacity];
    uint32_t counts[Capacity];
    unsigned used = 0;
    uint64_t dropped = 0;
...
uint64_t delta_ticks = 0, delta_tick_total = 0, delta_tick_max = 0, delta_samples = 0, delta_refresh = 0,
         delta_dropped = 0;
...
unsigned module_of(uintptr_t address) {
    for (unsigned i = 0; i < max_modules; ++i) {
        ...
        if (m.used && m.loaded && address >= m.base && address - m.base < m.size) return i;
    }
    return ~0u;
}
const SortedRange* range_of(uintptr_t address) {
    ...
    while (lo < hi) {
        ...
        if (sorted[mid].begin <= address)
            lo = mid + 1;
        else
            hi = mid;
    }
    if (!lo) return nullptr;
    const auto& r = sorted[lo - 1];
    return address < r.end ? &r : nullptr;
}
...
bool call_precedes(uintptr_t address, const SortedRange& r) {
    ...
    if (b[-2] == 0xff) {
        const unsigned m = b[-1];
        if ((m & 0xf8) == 0xd0) return true;
        if ((m & 0xf8) == 0x10 && (m & 7) != 4 && (m & 7) != 5) return true;
    }
    if (b[-3] == 0xff) {
        const unsigned m = b[-2];
        if ((m & 0xf8) == 0x50 && (m & 7) != 4) return true;
        if (m == 0x14) return true;
    }
    if (b[-4] == 0xff && b[-3] == 0x54) return true;
    if (b[-6] == 0xff) {
        const unsigned m = b[-5];
        if ((m & 0xf8) == 0x90 && (m & 7) != 4) return true;
        if (m == 0x15) return true;
    }
...
struct Walk {
    uintptr_t eip = 0;
    unsigned leaf_module = ~0u;
    bool stack_known = false;
    unsigned count = 0;
    struct Entry {
        uintptr_t slot, address;
    } merged[merged_limit];
};
...
bool capture(const Thread& t, Walk& w) {
    ...
    if (!(limit < base && base - limit <= stack_limit_bytes && limit % 4 == 0 && base % 4 == 0 && esp >= limit &&
          esp < base && esp % 4 == 0))
        return true;
    w.stack_known = true;
    Walk::Entry chain[chain_limit];
...
void aggregate(unsigned slot, Thread& t, const Walk& w) {
    ...
    uint32_t frame_rva = no_frame, caller_rva = no_frame;
    unsigned i = 0;
    const uintptr_t main_size = main_module == ~0u ? 0 : modules[main_module].size;
    const auto in_main = [&](uintptr_t address) {
        return main_size && address >= main_base && address - main_base < main_size;
    };
    if (main_size && w.leaf_module == main_module)
        frame_rva = leaf_rva;
    else
        for (; i < w.count; ++i) {
            const auto& e = w.merged[i];
            if (in_main(e.address)) {
                frame_rva = uint32_t(e.address - main_base);
                ++i;
                break;
            }
        }
    if (frame_rva != no_frame)
        for (; i < w.count; ++i) {
            const auto& e = w.merged[i];
            if (in_main(e.address)) {
                caller_rva = uint32_t(e.address - main_base);
                break;
            }
        }
...
Kind classify(const MODULEENTRY32W& entry, uintptr_t base) {
    ...
    if (windows_directory_length && !_wcsnicmp(entry.szExePath, windows_directory, windows_directory_length))
        return KindWine;
    return KindOther;
}
void refresh_modules() {
    ...
    if (snapshot == INVALID_HANDLE_VALUE) return;
    {
        ...
        for (BOOL more = Module32FirstW(snapshot, &entry); more; more = Module32NextW(snapshot, &entry)) {
            ...
            for (unsigned i = 0; i < max_modules; ++i) {
                if (modules[i].used && modules[i].base == base && modules[i].size == entry.modBaseSize) {
                    index = i;
                    break;
                }
            ...
            if (fresh) {
                ...
                m.kind = classify(entry, base);
                m.used = true;
                unsigned n = 0;
                for (; n < sizeof m.name - 1 && entry.szModule[n]; ++n)
                    m.name[n] = entry.szModule[n] < 0x80 ? char(entry.szModule[n]) : '?';
                m.name[n] = 0;
            }
            if (!m.loaded && !m.pinned) {
                ...
                m.pinned = GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_PIN,
                                              reinterpret_cast<LPCWSTR>(base), &pinned) &&
                           reinterpret_cast<uintptr_t>(pinned) == base;
                if (!m.pinned && fresh)
                    log("profile_module index=%u name=%s base=0x%08lx pinned=0 error=%lu", index, m.name,
                        static_cast<unsigned long>(base), GetLastError());
            }
            if (m.pinned && !m.loaded) {
                ...
                while (cursor < base + m.size) {
                    ...
                    const uintptr_t begin = reinterpret_cast<uintptr_t>(info.BaseAddress),
                                    end = std::min<uintptr_t>(begin + info.RegionSize, base + uintptr_t(m.size));
                    const bool executable = info.State == MEM_COMMIT &&
                                            (info.Protect & (PAGE_EXECUTE | PAGE_EXECUTE_READ | PAGE_EXECUTE_READWRITE |
                                                             PAGE_EXECUTE_WRITECOPY)) &&
                                            !(info.Protect & PAGE_GUARD);
                    if (executable) {
                        if (m.range_count && m.ranges[m.range_count - 1].end == begin)
                            m.ranges[m.range_count - 1].end = end;
                        else if (m.range_count < max_ranges)
                            m.ranges[m.range_count++] = {begin, end};
                    }
                    if (end <= cursor) break;
                    cursor = end;
                }
                m.loaded = true;
                uintptr_t text_begin = m.range_count ? m.ranges[0].begin : base,
                          text_end = m.range_count ? m.ranges[m.range_count - 1].end : base;
                log("profile_module index=%u name=%s kind=%s base=0x%08lx size=0x%lx text_rva=0x%lx text_size=0x%lx ranges=%u pinned=1",
                    index, m.name, kind_names[m.kind], static_cast<unsigned long>(base),
                    static_cast<unsigned long>(m.size), static_cast<unsigned long>(text_begin - base),
                    static_cast<unsigned long>(text_end - text_begin), m.range_count);
    ...
    for (unsigned i = 0; i < max_modules; ++i) {
        auto& m = modules[i];
        if (m.used && !m.seen && m.loaded) {
            m.loaded = false;
            m.range_count = 0;
            log("profile_module index=%u unloaded=1", i);
        }
    }
    sorted_count = 0;
    unsigned count = 0;
    for (unsigned i = 0; i < max_modules; ++i) {
        const auto& m = modules[i];
        if (!m.used || !m.loaded) continue;
        ++count;
        for (unsigned r = 0; r < m.range_count; ++r) sorted[sorted_count++] = {m.ranges[r].begin, m.ranges[r].end, i};
    }
    std::sort(sorted, sorted + sorted_count,
              [](const SortedRange& a, const SortedRange& b) { return a.begin < b.begin; });
...
void refresh_threads() {
    ...
    if (snapshot == INVALID_HANDLE_VALUE) return; // keep the known threads; retry next refresh
    {
        ...
        for (BOOL more = Thread32First(snapshot, &entry); more; more = Thread32Next(snapshot, &entry)) {
            ...
            bool known = false;
            for (auto& t : threads)
                if (t.used && !t.retired && t.tid == entry.th32ThreadID) {
                    t.seen = true;
                    known = true;
                    break;
                }
    ...
    for (unsigned c = 0; c < candidate_count; ++c) {
        ...
        if (GetThreadTimes(h, &creation, &exit, &kernel, &user))
            candidates[c].creation = (uint64_t(creation.dwHighDateTime) << 32) | creation.dwLowDateTime;
        CloseHandle(h);
    }
    ...
    for (unsigned c = 0; c < candidate_count; ++c) {
        ...
        t = Thread{};
        t.handle = h;
        t.tid = candidates[c].tid;
        t.creation = candidates[c].creation;
        t.used = t.seen = true;
        if (query_thread) {
            struct {
                LONG status;
            ...
            if (query_thread(h, 0 /*ThreadBasicInformation*/, &basic, sizeof basic, nullptr) >= 0)
                t.teb = reinterpret_cast<uintptr_t>(basic.teb);
            PVOID start = nullptr;
            if (query_thread(h, 9 /*ThreadQuerySetWin32StartAddress*/, &start, sizeof start, nullptr) >= 0 && start) {
                ...
                t.start_module = module_of(address);
                t.start_rva = uint32_t(t.start_module == ~0u ? address : address - modules[t.start_module].base);
            }
        }
        log("profile_thread_seen slot=%u tid=%lu teb=0x%08lx start_module=%u start_rva=0x%lx creation=%llu", slot,
            t.tid, static_cast<unsigned long>(t.teb), t.start_module, static_cast<unsigned long>(t.start_rva),
            static_cast<unsigned long long>(t.creation));
    }
    unsigned live = 0;
    for (const auto& t : threads) live += t.used && !t.retired;
    thread_count.store(live, std::memory_order_relaxed);
    unsampled_count.store(unsampled, std::memory_order_relaxed);
}
void refresh() {
    ...
    delta_refresh += cost;
    refresh_total.fetch_add(cost, std::memory_order_relaxed);
}
...
void tick() {
    ...
    for (unsigned slot = 0; slot < max_threads; ++slot) {
        ...
        if (ok)
            ...
            ++t.context_failures;
            ++t.context_failures_delta;
        }
    }
    const uint64_t cost = qpc() - begin;
    ++delta_ticks;
    delta_tick_total += cost;
    delta_tick_max = std::max(delta_tick_max, cost);
    tick_count.fetch_add(1, std::memory_order_relaxed);
    tick_total.fetch_add(cost, std::memory_order_relaxed);
    uint64_t old = tick_maximum.load(std::memory_order_relaxed);
    while (old < cost && !tick_maximum.compare_exchange_weak(old, cost, std::memory_order_relaxed)) {}
}

template <typename Key, unsigned Capacity> unsigned rank(const Table<Key, Capacity>& table, unsigned top) {
    unsigned n = 0;
    for (unsigned i = 0; i < Capacity; ++i)
        if (table.keys[i].slot) ranked[n++] = {i, table.counts[i]};
    const unsigned keep = std::min(n, top);
    std::partial_sort(ranked, ranked + keep, ranked + n, [](const Ranked& a, const Ranked& b) {
...
template <typename Key, unsigned Capacity> void merge(Table<Key, Capacity>& into, const Table<Key, Capacity>& from) {
    for (unsigned i = 0; i < Capacity; ++i)
        if (from.keys[i].slot) into.add(from.keys[i], from.counts[i]);
}
void report_tables(const char* scope, uint64_t stamp, const Table<LeafKey, leaf_capacity>& leaves,
                   const Table<FrameKey, frame_capacity>& frames, const Table<PairKey, pair_capacity>& pairs,
                   unsigned tops, unsigned pair_tops) {
    unsigned n = rank(leaves, tops);
    for (unsigned i = 0; i < n; ++i) {
        const auto& k = leaves.keys[ranked[i].index];
        log("profile_leaf scope=%s qpc=%llu slot=%u module=%u name=%s rva=0x%lx count=%lu", scope,
            static_cast<unsigned long long>(stamp), unsigned(k.slot - 1), unsigned(k.module),
            k.module == 0xffff ? "-" : modules[k.module].name, static_cast<unsigned long>(k.rva),
            static_cast<unsigned long>(ranked[i].count));
    }
    n = rank(frames, tops);
    for (unsigned i = 0; i < n; ++i) {
        const auto& k = frames.keys[ranked[i].index];
        log("profile_frame scope=%s qpc=%llu slot=%u rva=0x%lx count=%lu", scope,
            static_cast<unsigned long long>(stamp), k.slot - 1, static_cast<unsigned long>(k.rva),
            static_cast<unsigned long>(ranked[i].count));
    }
    n = rank(pairs, pair_tops);
    for (unsigned i = 0; i < n; ++i) {
        const auto& k = pairs.keys[ranked[i].index];
        log("profile_pair scope=%s qpc=%llu slot=%u rva=0x%lx caller=0x%lx count=%lu", scope,
            static_cast<unsigned long long>(stamp), k.slot - 1, static_cast<unsigned long>(k.rva),
            static_cast<unsigned long>(k.caller), static_cast<unsigned long>(ranked[i].count));
    }
}
void report_threads(const char* scope, uint64_t stamp, bool cumulative) {
    for (unsigned slot = 0; slot < max_threads; ++slot) {
        const auto& t = threads[slot];
        if (!t.used) continue;
        const uint64_t* leaf = cumulative ? t.leaf_total : t.leaf;
        const uint64_t samples = cumulative ? t.total : t.samples;
        ...
        log("profile_thread scope=%s qpc=%llu slot=%u tid=%lu samples=%llu total=%llu leaf_x3ap=%llu leaf_ntdll=%llu leaf_wine=%llu leaf_d3dx=%llu leaf_zlib=%llu leaf_xml=%llu leaf_proxy=%llu leaf_other=%llu stack_unknown=%llu suspend_failures=%llu context_failures=%llu start_module=%u start_rva=0x%lx retired=%u suspend_failures_delta=%llu context_failures_delta=%llu",
            scope, static_cast<unsigned long long>(stamp), slot, t.tid, static_cast<unsigned long long>(samples),
            static_cast<unsigned long long>(t.total), static_cast<unsigned long long>(leaf[KindX3ap]),
            static_cast<unsigned long long>(leaf[KindNtdll]), static_cast<unsigned long long>(leaf[KindWine]),
            static_cast<unsigned long long>(leaf[KindD3dx]), static_cast<unsigned long long>(leaf[KindZlib]),
            static_cast<unsigned long long>(leaf[KindXml]), static_cast<unsigned long long>(leaf[KindProxy]),
            static_cast<unsigned long long>(leaf[KindOther]), static_cast<unsigned long long>(t.stack_unknown),
            static_cast<unsigned long long>(t.suspend_failures), static_cast<unsigned long long>(t.context_failures),
            t.start_module, static_cast<unsigned long>(t.start_rva), t.retired,
            static_cast<unsigned long long>(t.suspend_failures_delta),
            static_cast<unsigned long long>(t.context_failures_delta));
    }
}
...
void report(bool final) {
    const uint64_t begin = qpc();
    const uint64_t dropped = leaf_delta.dropped + frame_delta.dropped + pair_delta.dropped;
    ...
    log("profile_report scope=delta qpc=%llu frequency=%llu since_start_us=%.3f elapsed_us=%.3f interval_us=%u samples=%llu threads=%u threads_unsampled=%u modules=%u ticks=%llu tick_us_mean=%.3f tick_us_max=%.3f refresh_us=%.3f dropped=%llu table_used=%u,%u,%u",
        static_cast<unsigned long long>(begin), static_cast<unsigned long long>(frequency), us(begin - startup),
        us(begin - last_report), settings.interval_us, static_cast<unsigned long long>(delta_samples), live,
        unsampled_count.load(std::memory_order_relaxed), module_count.load(std::memory_order_relaxed),
        static_cast<unsigned long long>(delta_ticks), delta_ticks ? us(delta_tick_total) / double(delta_ticks) : 0.0,
        us(delta_tick_max), us(delta_refresh), static_cast<unsigned long long>(dropped), leaf_delta.used,
        frame_delta.used, pair_delta.used);
    report_threads("delta", begin, false);
    report_tables("delta", begin, leaf_delta, frame_delta, pair_delta, delta_tops, delta_pair_tops);
    merge(leaf_total, leaf_delta);
    merge(frame_total, frame_delta);
    merge(pair_total, pair_delta);
    leaf_delta.clear();
    frame_delta.clear();
    pair_delta.clear();
    leaf_delta.dropped = frame_delta.dropped = pair_delta.dropped = 0;
    sample_count.fetch_add(delta_samples, std::memory_order_relaxed);
    drop_count.fetch_add(dropped, std::memory_order_relaxed);
    const uint64_t reports = report_count.fetch_add(1, std::memory_order_relaxed) + 1;
    for (auto& t : threads) {
        t.samples = 0;
        std::memset(t.leaf, 0, sizeof t.leaf);
        t.suspend_failures_delta = t.context_failures_delta = 0;
        if (t.retired) {
            t.used = false;
            t.retired = false;
        }
    ...
    if (final || reports % cumulative_every == 0) {
        log("profile_report scope=cumulative qpc=%llu frequency=%llu since_start_us=%.3f samples=%llu ticks=%llu tick_us_mean=%.3f tick_us_max=%.3f refresh_us=%.3f report_us=%.3f dropped=%llu cumulative_dropped=%llu table_used=%u,%u,%u final=%u",
            static_cast<unsigned long long>(begin), static_cast<unsigned long long>(frequency), us(begin - startup),
            static_cast<unsigned long long>(sample_count.load(std::memory_order_relaxed)),
            static_cast<unsigned long long>(tick_count.load(std::memory_order_relaxed)),
            tick_count.load() ? us(tick_total.load()) / double(tick_count.load()) : 0.0, us(tick_maximum.load()),
            us(refresh_total.load()), us(report_total.load()),
            static_cast<unsigned long long>(drop_count.load(std::memory_order_relaxed)),
            static_cast<unsigned long long>(leaf_total.dropped + frame_total.dropped + pair_total.dropped),
            leaf_total.used, frame_total.used, pair_total.used, final);
    ...
    const uint64_t cost = qpc() - begin;
    report_total.fetch_add(cost, std::memory_order_relaxed);
...
DWORD WINAPI sampler_main(LPVOID) {
    ...
    last_report = last_refresh = qpc();
    refresh();
    while (!stop_requested.load(std::memory_order_acquire)) {
        const uint64_t begin = qpc();
        if (begin - last_refresh >= frequency) {
            refresh();
            last_refresh = begin;
        }
        tick();
...
unsigned env_number(const wchar_t* name, unsigned fallback, unsigned lo, unsigned hi) {
    wchar_t value[32]{};
    if (x3m::config::get(name, value, 32) == 0) return fallback;
    const unsigned long n = wcstoul(value, nullptr, 10);
    return n >= lo && n <= hi ? unsigned(n) : fallback;
}
}
...
bool initialize() {
    ...
    if (!QueryPerformanceFrequency(&f) || f.QuadPart <= 0) {
        log("profile_start enabled=0 reason=no_qpc");
        return false;
    }
    ...
    interval_ticks = frequency * settings.interval_us / 1000000;
    report_interval_ticks = frequency * settings.report_s;
    ...
    if (GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_PIN,
        ...
        proxy_base = reinterpret_cast<uintptr_t>(self);
    if (HMODULE ntdll = GetModuleHandleW(L"ntdll.dll"))
        query_thread = reinterpret_cast<QueryThreadFn>(
    ...
    if (!stop_event) {
        log("profile_start enabled=0 reason=no_event error=%lu", GetLastError());
        return false;
    }
    startup = qpc();
    last_report = startup;
    ...
    if (!sampler) {
        log("profile_start enabled=0 reason=no_thread error=%lu", GetLastError());
    ...
    log("profile_start schema=1 enabled=1 qpc=%llu frequency=%llu interval_us=%u report_s=%u sampler_tid=%lu init_tid=%lu main_base=0x%08lx proxy_base=0x%08lx query_thread=%u chain_limit=%u scan_dwords=%u threads_max=%u modules_max=%u tables=%u,%u,%u",
        static_cast<unsigned long long>(startup), static_cast<unsigned long long>(frequency), settings.interval_us,
        settings.report_s, sampler_tid, init_tid, static_cast<unsigned long>(main_base),
        static_cast<unsigned long>(proxy_base), query_thread != nullptr, chain_limit, scan_dwords, max_threads,
        max_modules, leaf_capacity, frame_capacity, pair_capacity);
    ResumeThread(sampler);
    return true;
}
bool active() {
    return sampler != nullptr && !finished;
}
void shutdown() {
    ...
    if (wait != WAIT_OBJECT_0) {
        log("profile_stop joined=0 wait=%lu", wait);
        return;
    } // Tables stay owned by the live thread.
    report(true);
    unsigned closed = 0;
    for (auto& t : threads) {
        if (t.handle) {
    ...
    CloseHandle(stop_event);
    stop_event = nullptr;
    log("profile_stop joined=1 qpc=%llu stop_us=%.3f handles_closed=%u", static_cast<unsigned long long>(qpc()),
        us(qpc() - begin), closed);
}
Status status() {
    Status s;
    s.active = active();
    s.ticks = tick_count.load(std::memory_order_relaxed);
    s.samples = sample_count.load(std::memory_order_relaxed);
    s.dropped = drop_count.load(std::memory_order_relaxed);
    s.reports = report_count.load(std::memory_order_relaxed);
    s.tick_ticks = tick_total.load(std::memory_order_relaxed);
    s.tick_max_ticks = tick_maximum.load(std::memory_order_relaxed);
    s.report_ticks = report_total.load(std::memory_order_relaxed);
    s.refresh_ticks = refresh_total.load(std::memory_order_relaxed);
    s.threads = thread_count.load(std::memory_order_relaxed);
    s.modules = module_count.load(std::memory_order_relaxed);
    s.threads_unsampled = unsampled_count.load(std::memory_order_relaxed);
    return s;
}
}
