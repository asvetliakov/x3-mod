...
detail::Addresses addresses{sites::kQueryReturn,  sites::kSavegameReturn, sites::kScriptReturn,
                            sites::kSpeechReturn, sites::kHelperReturn,   sites::kSelectorReturn,
                            sites::kSelectorKind, sites::kVideoBlitBegin, sites::kVideoBlitEnd};
detail::Gate gate;
...
std::atomic<std::uint64_t> video_foreign{0}, video_early{0}; // in-range enters dropped: another thread / before the
                                                             // owner is admitted
...
std::uint64_t to_us(std::uint64_t ticks) noexcept {
...
void reset_state() {
    ...
    window.reset();
    video = {};
    video_foreign.store(0, std::memory_order_relaxed);
    video_early.store(0, std::memory_order_relaxed);
    attempts_frame = 0;
    attempts_total = refused_total = 0;
...
const char* kind_text(const detail::Entry& e, char (&buffer)[16]) {
    if (e.caller == detail::selector || e.caller == detail::other)
        std::snprintf(buffer, sizeof buffer, "0x%lx", static_cast<unsigned long>(e.kind));
    else
...
void write_enter_line(const detail::Entry& e) {
    ...
    if (handle == INVALID_HANDLE_VALUE || !handle) {
        ++enter_limit.suppressed;
        return;
    }
    // kind_text formats with the CRT (x87 code): behind call_preserved like the row itself.
    x3m::call_preserved([&] {
        char kind[16];
        log("media_cue_enter frame=%llu qpc=%llu id=%lu kind=%s caller=%s flags=0x%lx attempt=%lu", e.frame, e.qpc,
            static_cast<unsigned long>(e.id), kind_text(e, kind), detail::caller_names[e.caller],
            static_cast<unsigned long>(e.flags), static_cast<unsigned long>(e.attempt));
    });
}
...
void write_video_line(const ownership::SurfaceLockEvent& e, const char* stage) {
    ...
    x3m::call_preserved([&] { // the result text is CRT-formatted (x87 code)
        ...
        if (e.phase == ownership::SurfaceLockPhase::LockEnter || e.phase == ownership::SurfaceLockPhase::UnlockEnter)
            std::snprintf(result, sizeof result, "pending");
        else
            std::snprintf(result, sizeof result, "0x%08lx", static_cast<unsigned long>(e.result));
        log("media_video_blit frame=%llu qpc=%llu texture=%p width=%u height=%u format=%u flags=0x%lx result=%s stage=%s blits=%llu unlocks=%llu",
            current_frame.load(std::memory_order_relaxed), qpc(), static_cast<void*>(e.surface), unsigned(desc.Width),
            unsigned(desc.Height), unsigned(desc.Format), static_cast<unsigned long>(e.flags), result, stage,
            video.locks_total, video.unlocks);
    });
}
...
void video_lock_observe(const ownership::SurfaceLockEvent& e) {
    ...
    if (!detail::video_blit_caller(std::uint32_t(reinterpret_cast<std::uintptr_t>(e.return_address)), addresses))
        return;
    ...
    switch (e.phase) {
    case ownership::SurfaceLockPhase::LockEnter:
        if (video.lock_enter()) write_video_line(e, "lock_enter");
        break;
    case ownership::SurfaceLockPhase::LockResult:
        if (video.lock_result(FAILED(e.result))) write_video_line(e, "lock");
        break;
    case ownership::SurfaceLockPhase::UnlockEnter:
        if (video.unlock_enter()) write_video_line(e, "unlock_enter");
        break;
    case ownership::SurfaceLockPhase::UnlockResult:
        if (video.unlock_result(FAILED(e.result))) write_video_line(e, "unlock");
        break;
    }
}
void emit_entry(const detail::Entry& e) {
    ...
    log("media_cue frame=%llu qpc=%llu id=%lu kind=%s caller=%s flags=0x%lx result=%s us=%llu attempts_frame=%lu cached=%u",
        e.frame, e.qpc, static_cast<unsigned long>(e.id), kind, detail::caller_names[e.caller],
        static_cast<unsigned long>(e.flags), result_text(e, result),
        e.outcome == detail::observed ? to_us(e.duration_ticks) : 0ull, static_cast<unsigned long>(e.attempt),
        unsigned(e.outcome == detail::refused));
}
void emit_window() {
    detail::Summary s;
    ...
    for (unsigned i = 0; i < s.id_count; ++i) {
        const int n = std::snprintf(ids + used, sizeof ids - used, "%s%lu:%lu", i ? "," : "",
                                    static_cast<unsigned long>(s.ids[i].id),
                                    static_cast<unsigned long>(s.ids[i].count));
    ...
    log("media_cue_window qpc=%llu frame=%llu frames=%u attempts=%llu failures=%llu successes=%llu refused=%llu unobserved=%llu attempts_frame_p50=%llu attempts_frame_max=%lu ids=%s id_overflow=%llu cache_used=%u evictions=%llu depth_max=%u overflow=%llu stale=%llu mismatched=%llu lost=%llu suppressed=%llu enter_suppressed=%llu dropped=%llu early=%u foreign=%u video_blits=%llu video_unlocks=%llu video_failures=%llu video_suppressed=%llu video_reentries=%llu video_foreign=%llu video_early=%llu",
        qpc(), s.frame, s.frames, s.attempts, s.failures, s.successes, s.refused, s.unobserved, s.attempts_frame_p50,
        static_cast<unsigned long>(s.attempts_frame_max), s.id_count ? ids : "none", s.id_overflow, cache.used,
        cache.evictions, pending.max_depth, pending.overflow, pending.stale, pending.mismatched, pending.lost,
        limit.suppressed, enter_limit.suppressed, ring.dropped, gate.early.exchange(0, std::memory_order_relaxed),
        gate.foreign.exchange(0, std::memory_order_relaxed), video.locks, video.unlocks, video.failures,
        video.suppressed, video.reentries, video_foreign.exchange(0, std::memory_order_relaxed),
        video_early.exchange(0, std::memory_order_relaxed));
    limit.suppressed = 0;
    enter_limit.suppressed = 0;
    ring.dropped = 0;
    video.close();
    if (pending.lost && !lost_reported) {
        lost_reported = true;
        log("media_cue_lost count=%llu last_return=%08lx note=return_matched_no_pending_entry_fail_safe_used",
            pending.lost, static_cast<unsigned long>(pending.last_return));
    }
}
}
}
...
extern "C" __attribute__((force_align_arg_pointer)) unsigned __cdecl x3m_media_cue_enter(
    x3m::media_cue::EnterFrame* f) {
    ...
    detail::Entry e;
    e.qpc = now;
    e.frame = current_frame.load(std::memory_order_relaxed);
    e.id = f->id;
    e.kind = f->slot_10;
    e.flags = f->eax;
    e.attempt = attempts_frame;
    e.caller = caller;
    e.scoped = scoped;
    if (cache_on && scoped && cache.refuses(f->id, now, retry_ticks)) {
        ++refused_total;
        e.outcome = detail::refused;
        ring.push(e);
        return 0;
    }
    detail::Pending p;
    p.ret = f->ret;
    p.esp = std::uint32_t(reinterpret_cast<std::uintptr_t>(&f->ret));
    p.id = f->id;
    p.kind = f->slot_10;
    p.flags = f->eax;
    p.attempt = attempts_frame;
    p.qpc = now;
    p.frame = e.frame;
    p.caller = caller;
    p.scoped = scoped;
    if (pending.push(p))
        f->ret = std::uint32_t(reinterpret_cast<std::uintptr_t>(return_trampoline));
    else
        ring.push(e); // depth exhausted: proceed unobserved
...
extern "C" __attribute__((force_align_arg_pointer)) std::uint32_t __cdecl x3m_media_cue_return(
    x3m::media_cue::ReturnFrame* f) {
    using namespace x3m::media_cue;
    ...
    if (f->eax)
        cache.success(p.id);
    else if (cache_on && p.scoped)
        cache.fail(p.id, now);
    detail::Entry e;
    e.qpc = p.qpc;
    e.frame = p.frame;
    e.duration_ticks = now >= p.qpc ? now - p.qpc : 0;
    e.id = p.id;
    e.kind = p.kind;
    e.flags = p.flags;
    e.result = f->eax;
...
void* emit(unsigned, void*** next_out) {
    return emit_gate(reinterpret_cast<const void*>(&x3m_media_cue_enter),
                     reinterpret_cast<const void*>(&x3m_media_cue_return), next_out, &return_trampoline);
}
}
...
bool initialize() {
    ...
    if (x3m::config::get(L"X3M_MEDIA_CUE_RETRY_S", value, 16) > 0) {
        const unsigned long n = wcstoul(value, nullptr, 10);
        if (n >= 1 && n <= max_retry_s) retry_s = unsigned(n);
    }
    ...
    log("media_cue_mode trace_requested=%u cache_requested=%u trace=%u cache=%u enabled=%u id2_video_skip=%u status=%s retry_s=%u cache_entries=%u pending_depth=%u ring=%u lines_per_second=%u window=%u owner=present_thread sector_change=interval_only qpc_frequency=%llu return_trampoline=%08lx",
        unsigned(trace_wanted), unsigned(cache_wanted), unsigned(trace_on && active.load()),
        unsigned(cache_on && active.load()), unsigned(active.load()), unsigned(installed.load()), status, retry_s,
        detail::cache_entries, detail::pending_depth, detail::ring_entries, detail::lines_per_second,
        detail::window_frames, frequency, reinterpret_cast<unsigned long>(return_trampoline));
    for (unsigned i = 0; i < sites::Count; ++i)
        log("media_cue_site index=%u address=%08lx length=%u rel32=%u patched=%u status=%s", i,
...
void frame_impl(std::uint64_t frame) noexcept {
    ...
    window.frame(frame, attempts_frame);
    attempts_frame = 0;
    Entry e;
    while (ring.pop(e)) {
        window.entry(e);
...
bool fixture_install(const engine_patch::SiteSpec* specs, const detail::Addresses& fixture_addresses, bool trace,
                     bool cache_enabled, std::uint64_t ticks, const char** status) {
...
bool fixture_pop_trace(detail::Entry* out) {
    ...
    detail::Entry e;
    if (!ring.pop(e)) return false;
    window.entry(e);
    *out = e;
    return true;
}
std::uint32_t fixture_attempts_frame() {
    return attempts_frame;
}
std::uint64_t fixture_refused() {
    return refused_total;
}
const char* fixture_site_status() {
    return patches[0].status;
}
void fixture_drop_pending() {
    pending.depth = 0;
} // models a return whose entry vanished: the next return is `lost`
const detail::RateLimit* fixture_enter_limit() {
    return &enter_limit;
}
void fixture_reset_enter_limit() {
    enter_limit = {};
}
const detail::VideoBlit* fixture_video() {
    return &video;
}
std::uint64_t fixture_video_dropped(bool foreign) {
    return (foreign ? video_foreign : video_early).load(std::memory_order_relaxed);
}
void fixture_reset_video() {
    video = {};
    video_foreign.store(0);
    video_early.store(0);
}
#endif
}
