...
void emit_entry(const detail::Entry& e) {
    char result[16];char kind[16];
    if(e.caller==detail::selector||e.caller==detail::other)std::snprintf(kind,sizeof kind,"0x%lx",static_cast<unsigned long>(e.kind));
    else std::snprintf(kind,sizeof kind,"none");
...
void emit_window() {
    ...
    log("media_cue_window qpc=%llu frame=%llu frames=%u attempts=%llu failures=%llu successes=%llu refused=%llu unobserved=%llu attempts_frame_p50=%llu attempts_frame_max=%lu ids=%s id_overflow=%llu cache_used=%u evictions=%llu depth_max=%u overflow=%llu stale=%llu mismatched=%llu lost=%llu suppressed=%llu dropped=%llu early=%u foreign=%u",
...
// execute no x87 opcode, never log and never allocate. A call before the
// owner is admitted or from another thread passes unobserved.
