...
constexpr unsigned char lens_context[] = {0x56, 0xe8, 0x4a, 0xc2, 0x00, 0x00, 0x8b, 0x0d, 0x18, 0x85, 0x60,
                                          0x00, 0x8b, 0x81, 0x04, 0x63, 0x00, 0x00, 0x83, 0xc4, 0x04};
...
inline bool eligible(std::uintptr_t record_owner, std::uintptr_t view, std::uintptr_t main_view,
                     std::uint32_t owner_flags) noexcept {
    return main_view != 0 && view == main_view && record_owner != 0 && record_owner != view &&
           (owner_flags & view_flag_background) != 0;
}
...
struct Ready {
    ...
    bool blocked = false;  // sticky for the process, across reset(): a lens draw that can never carry the fraction was
                           // seen
    bool answered = false; // the override answered in this frame
    void begin_frame() noexcept {
        previous_record = record;
        previous_records = records;
        previous_pass_ok = pass_ok;
        record = 0;
        records = 0;
        pass_ok = false;
        answered = false;
    ...
    void reset() noexcept {
        const bool keep = blocked;
        *this = Ready{};
        blocked = keep;
    } // device Reset: the refusal that blocked is a property of the program or the material, not of the device
      // generation
    // Registers the probe call; true when the override may answer for it.
    bool probe(std::uintptr_t rec, bool enabled) noexcept {
        if (rec != record) {
            ++records;
            if (records == 1) record = rec;
        }
        return enabled && !blocked && records == 1 && rec == record && previous_records == 1 &&
               previous_record == rec && previous_pass_ok;
    }
...
struct Hold {
    ...
    bool step(bool ran, bool holdable) noexcept {
        ...
        if (holdable && !exhausted && frames < hold_limit) {
            ++frames;
            return true;
        }
        exhausted = true;
...
struct Footprint {
    ...
    bool radius_known = false, saturated = false, valid = false;
};
...
inline Footprint footprint(const Latch& in, unsigned width, unsigned height, float radius_scale) noexcept {
    ...
    out.u = .5f + float(in.x) / 65536.f;
    out.v = .5f - float(in.y) / 65536.f;
    ...
    if (out.saturated || in.accumulator <= 0 || in.accumulator > 200 || in.size <= 0 || !(avg > 0.f) || !(wn > 0.f) ||
        !(rect_w > 0.f) || half == 0 || half >= 0x4000u)
        return out;
    ...
    out.radius_u = r;
    out.radius_v = r * float(width) / float(height);
    out.radius_known = true;
    return out;
}
inline void apply_radius(Footprint& f, float radius_u, unsigned width, unsigned height) noexcept {
    ...
    f.radius_u = r;
    f.radius_v = height ? r * float(width) / float(height) : r;
}
...
inline JitterUv jitter_uv(bool active, float jx_px, float jy_px, unsigned width, unsigned height) noexcept {
    ...
    j.u = jx_px / float(width);
    j.v = jy_px / float(height);
    return j;
}
...
constexpr std::uint32_t blend_one = 2, blend_srccolor = 3, blend_invsrccolor = 4, blend_srcalpha = 5,
                        blend_invsrcalpha = 6;
...
struct BlendState {
    std::uint32_t enable = 0, src = 0, dst = 0, op = 0, srgb_write = 0, alpha_test = 0, alpha_ref = 0, alpha_func = 0,
                  fog = 0;
};
inline Scale classify_blend(const BlendState& s) noexcept {
    ...
    if (s.src == blend_srcalpha && (s.dst == blend_one || s.dst == blend_invsrcalpha))
        scale = Scale::Alpha;
    else if (s.src == blend_one && (s.dst == blend_one || s.dst == blend_invsrccolor))
        scale = Scale::Rgb;
    else if (s.src == blend_one && s.dst == blend_invsrcalpha)
        scale = Scale::Both;
...
constexpr float body_tolerance_u = .005f; // 6.4 px at 1280: covers the record -> uv mapping error the next flight
                                          // measures
struct BodyCentre {
    float u = 0.f, v = 0.f, distance_u = 0.f;
    Body body = Body::Unknown;
};
inline BodyCentre classify_body(const float rows[16], bool rows_known, float sun_u, float sun_v,
                                float aspect_w_over_h) noexcept {
    BodyCentre out;
    ...
    if (out.distance_u <= body_tolerance_u) {
        out.body = Body::Core;
        return out;
    }
    const float sx = sun_u - .5f, sy = (sun_v - .5f) / aspect_w_over_h, bx = out.u - .5f,
                by = (out.v - .5f) / aspect_w_over_h;
    const float s = sqrt_no_x87(sx * sx + sy * sy);
    if (s > body_tolerance_u) {
        const float line = (bx * sy - by * sx) / s; // perpendicular distance of the centre from the
                                                    // sun-through-screen-centre line
        out.body = (line < 0.f ? -line : line) <= body_tolerance_u ? Body::Ghost : Body::Other;
    } else
        out.body = Body::Other; // the sun on the screen centre: every ghost coincides with the core and was classed
                                // above
    return out;
}
...
template <class Read> std::uintptr_t main_view(Read& read) noexcept {
    ...
    if (!word(cockpit, 0x58, camera) || !word(camera, view_flags, flags) || !(flags & view_flag_sector_camera))
        return 0;
    return camera;
}
