...
constexpr engine_patch::SiteSpec spec = {"chase_cursor_admission", 0x00445a41,
    {0x0f,0x84,0x64,0x02,0x00,0x00}, 6,0,2};
...
bool active_cockpit(std::uintptr_t& cockpit) {
    ...
    for(unsigned i=0;link && i<32;++i) {
        ...
        if(row[1]==handle){cockpit=row[2];return cockpit && !(cockpit&3);}
        link=row[0];
    }
    return false;
}
Result decide(const Context& witness,std::uint32_t ship,std::uint64_t stamp) {
    if(!witness.qpc || !stamp || stamp<witness.qpc || stamp-witness.qpc>frequency*2 ||
       witness.thread!=GetCurrentThreadId())return Result::Stale;
    ...
    if(!field(cockpit,0xc,refs) || !field(cockpit,0x58,camera) || !field(ship,8,id) ||
       !field(cockpit,0x150,mode) || !field(cockpit,0x1c0,connect) || !field(cockpit,0x1a0,flags))return Result::Read;
    if(refs[0]!=ship || refs[1]!=ship || camera!=witness.camera || id!=witness.ship_id)return Result::Identity;
    ...
    if(!field(0x00607ce8,0,cursor) || !field(camera,0x288,viewport) ||
       !field(0x00606f38,0,defaults) || !field(defaults,0,screen) || !field(screen,4,dimensions))return Result::Read;
    if(cursor[0]!=0 || dimensions[0]<=0 || dimensions[1]<=0 ||
       viewport[0]<0 || viewport[0]>=viewport[1] || viewport[1]>65536 ||
       viewport[2]<0 || viewport[2]>=viewport[3] || viewport[3]>65536 ||
       cursor[1]<0 || cursor[1]>=dimensions[0] || cursor[2]<0 || cursor[2]>=dimensions[1])return Result::Cursor;
...
void handle(std::uint32_t* regs) {
    ...
    if(timing) {
        ...
        ++counts.timed;counts.ticks+=ticks;if(ticks>counts.max_ticks)counts.max_ticks=ticks;
    }
...
x3m_chase_fire_enter(std::uint32_t* regs) {
...
void* emit(void*** next_out) {
    ...
    e.byte(0xe8);e.rel32(reinterpret_cast<const void*>(&x3m_chase_fire_enter));
...
bool initialize() {
    ...
    initialized=true;
    if(!chase_camera::installed() || !chase_camera::wanted()){SetLastError(error);return false;}
    LARGE_INTEGER f{};
    bool okay=object_trace::executable_verified() && engine_patch::install_window_open() && QueryPerformanceFrequency(&f) && f.QuadPart>0;
    status=okay?"installing":"prerequisite_failed";
    frequency=okay?std::uint64_t(f.QuadPart):0;
    timing=telemetry::enabled();
    if(okay){okay=engine_patch::claim(site,spec);status=site.status;}
    if(okay) {
        void** next=nullptr;void* stub=emit(&next);
        if(!stub || !next){okay=false;status="stub_failed";}
        else if(!engine_patch::store_pointer(next,*site.entry)){okay=false;status="chain_pointer_failed";}
        else if(!engine_patch::push_front(site,stub)){okay=false;status="chain_failed";}
    }
...
void invalidate_camera(std::uintptr_t cockpit) {
    ...
    AcquireSRWLockExclusive(&lock);
    if(context.cockpit==cockpit){context={};player.store(0,std::memory_order_relaxed);}
    ReleaseSRWLockExclusive(&lock);
}
void camera_context(std::uintptr_t cockpit,std::uintptr_t ship,std::uintptr_t camera,
                    std::uint32_t mode,std::uint32_t connect,std::uint32_t flags,bool applied,std::uint64_t stamp) {
    if(!enabled.load(std::memory_order_acquire))return;
    Context next;
    if(applied && mode==258 && connect==0 && !(flags&4) && stamp && cockpit && ship && camera &&
       !((cockpit|ship|camera)&3) && field(ship,8,next.ship_id)) {
        next.cockpit=cockpit;next.ship=ship;next.camera=camera;next.qpc=stamp;next.thread=GetCurrentThreadId();
    }
    AcquireSRWLockExclusive(&lock);
    context=next;player.store(std::uint32_t(next.ship),std::memory_order_relaxed);
    ReleaseSRWLockExclusive(&lock);
}
void report(std::uint64_t frame) {
    if(!enabled.load(std::memory_order_acquire))return;
    Counts c;
    AcquireSRWLockExclusive(&lock);c=counts;counts={};ReleaseSRWLockExclusive(&lock);
...
void shutdown() {
    enabled.store(false,std::memory_order_release);player.store(0,std::memory_order_relaxed);
    if(site.patched_in){const bool restored=engine_patch::restore(site);log("chase_fire_shutdown restored=%u status=%s",unsigned(restored),site.status);}
}
}
