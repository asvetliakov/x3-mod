...
struct Role { unsigned char interval; Action action; };
...
inline constexpr unsigned sort_enter_site = 0, walk_begin_site = 4, walk_miss_site = 5, walk_join_site = 6, end_begin_site = 9;
...
struct Accumulator {
    ...
    void close(unsigned interval, std::uint64_t now) noexcept {
        ...
        if (!opened) { ++idle; return; }
        open_clock[interval] = 0;
        if (now >= opened) { ticks[interval] += now - opened; ++calls[interval]; } else ++clock_errors;
    }
    void stamp(unsigned index, std::uint64_t now) noexcept {
        ...
        if (!now) { ++clock_failures; open_clock[role.interval] = 0; return; }
        if (index == end_begin_site && open_clock[Block]) { close(Block, now); ++block_skipped; } // the geometry guard skipped the pass-loop guard
        if (role.action == Open) {
    ...
    void sorted(std::uint32_t nodes) noexcept { sort_nodes += nodes; if (nodes > sort_nodes_max) sort_nodes_max = nodes; }
    void walked(std::uint32_t iterations) noexcept { walk_sampled_iterations += iterations; }
    void discard() noexcept {
        for (unsigned i = 0; i < interval_count; ++i) { open_clock[i] = 0; ticks[i] = 0; calls[i] = 0; }
    ...
    void take(std::uint64_t frame, std::uint64_t frequency, Sample& out) noexcept {
        out = Sample{};
        out.frame = frame; out.stamps = stamps;
        ...
        out.sort_nodes = sort_nodes; out.sort_nodes_max = sort_nodes_max; out.walk_misses = walk_misses;
...
struct Summary {
    ...
    std::uint64_t sort_nodes_p50 = 0, sort_nodes_max = 0, walk_misses_p50 = 0, walk_iterations_p50 = 0, walk_iterations_p95 = 0;
};
...
public:
    ...
    void add(const Sample& s) noexcept {
        if (count_ < window_frames) {
            for (unsigned i = 0; i < interval_count; ++i) { calls_[i][count_] = s.calls[i]; interval_[i][count_] = s.interval_us[i]; }
            stamps_[count_] = s.stamps; self_[count_] = s.self_us;
            net_[0][count_] = s.material_net_us; net_[1][count_] = s.block_net_us;
            sort_nodes_[count_] = s.sort_nodes; walk_misses_[count_] = s.walk_misses; walk_iterations_[count_] = s.walk_iterations;
    ...
    bool close(Summary& out) noexcept {
        ...
        out.stamps_p50 = percentile(stamps_, 50); out.stamps_p95 = percentile(stamps_, 95); out.self_p50 = percentile(self_, 50);
        for (unsigned i = 0; i < interval_count; ++i) {
        ...
        out.material_net_p50 = percentile(net_[0], 50); out.block_net_p50 = percentile(net_[1], 50);
        out.sort_nodes_p50 = percentile(sort_nodes_, 50); out.sort_nodes_max = sort_nodes_max_;
        out.walk_misses_p50 = percentile(walk_misses_, 50);
        out.walk_iterations_p50 = percentile(walk_iterations_, 50); out.walk_iterations_p95 = percentile(walk_iterations_, 95);
        reset();
        return true;
    }
    void reset() noexcept { count_ = 0; last_frame_ = 0; sort_nodes_max_ = 0; }
