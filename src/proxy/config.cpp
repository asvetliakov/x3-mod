...
// initialize_log (load_backend's INIT_ONCE, before any device); everything it fills is static storage written before
// the resolver pointer is published and never written again, so config::get takes no lock and allocates nothing.
#include "config.h"
...
parse::Layer layer_of(int i) noexcept {
    return parse::below_environment(
        i, profile_ == Profile::bare, [](int k) { return file_value_[k] != nullptr; }, entry_in_environment);
}
void below_environment(int i, const wchar_t** wide, const char** narrow) noexcept {
    ...
    *narrow = nullptr;
    ...
    if (layer == parse::Layer::file)
        ...
        *narrow = parse::base_value(i, profile_ == Profile::bare);
}
template <class Char> DWORD copy_out(const Char* value, wchar_t* out, DWORD capacity, DWORD saved) noexcept {
    DWORD length = 0;
...
bool equals(const wchar_t* a, const wchar_t* b) noexcept {
    return std::wcscmp(a, b) == 0;
}
bool absolute(const wchar_t* p) noexcept {
    return p[0] == L'\\' || p[0] == L'/' ||
           (((p[0] >= L'A' && p[0] <= L'Z') || (p[0] >= L'a' && p[0] <= L'z')) && p[1] == L':');
}
...
bool beside_module(HMODULE module, const wchar_t* name) noexcept {
    ...
    if (cut + name_length + 1 > kPathCapacity) {
        path_[0] = 0;
        return false;
    }
...
void read_file(bool override_path) noexcept {
    const HANDLE file = CreateFileW(path_, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
                                    nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
    if (file == INVALID_HANDLE_VALUE) {
        open_error_ = GetLastError();
        if (open_error_ == ERROR_FILE_NOT_FOUND || open_error_ == ERROR_PATH_NOT_FOUND)
            source_ = "missing";
        else {
            source_ = "refused";
            reason_ = "open";
    ...
    if (size.QuadPart > LONGLONG(parse::file_limit)) {
        source_ = "refused";
        reason_ = "size";
        bytes_ = size.QuadPart > 0xffffffffll ? 0xffffffffu : DWORD(size.QuadPart);
        CloseHandle(file);
        return;
    }
    DWORD total = 0;
    while (total < DWORD(size.QuadPart)) {
        DWORD got = 0;
        if (!ReadFile(file, text_ + total, DWORD(size.QuadPart) - total, &got, nullptr)) {
            source_ = "refused";
            reason_ = "read";
            break;
        }
    ...
    for (unsigned i = 0; i < schema::entry_count; ++i) {
        ...
        if (!v) continue;
        const int n = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, v, -1, pool_ + used,
                                          int(kPoolCapacity - used));
        if (n <= 0) {
            ++bad_utf8_;
            ++result_.invalid;
            --result_.keys;
...
std::string shown_value(int entry, const char* value) {
    if (entry >= 0 && schema::entries[entry].type == schema::Type::Path)
        return printable(session_log::redact_path(value));
...
void load(HMODULE module) noexcept {
    ...
    if (n >= kSettingCapacity) {
        source_ = "refused";
        reason_ = "config_too_long";
    } else if (n && equals(setting, L"none")) {
        profile_ = Profile::none;
        source_ = "none";
    } else if (n && equals(setting, L"bare")) {
        profile_ = Profile::bare;
        source_ = "bare";
    } else if (!n) {
        if (beside_module(module, L"x3m.ini"))
            read_file(false);
        else {
            source_ = "missing";
            reason_ = "module_path";
        ...
        read_file(true);
    } else if (beside_module(module, setting))
        read_file(true);
    else {
        source_ = "refused";
        reason_ = "path_too_long";
    }
    QueryPerformanceCounter(&end);
    QueryPerformanceFrequency(&frequency);
    us_ = frequency.QuadPart > 0 && end.QuadPart > begin.QuadPart
              ? static_cast<unsigned long long>((end.QuadPart - begin.QuadPart) * 1000000ll / frequency.QuadPart)
              : 0ull;
...
std::string effective_below_environment() {
    ...
    for (unsigned i = 0; i < schema::entry_count; ++i) {
        ...
        const char* value = layer == parse::Layer::file ? result_.value[i]
                                                        : parse::base_value(int(i), profile_ == Profile::bare);
        line += ' ';
        line += schema::entries[i].env;
        line += '=';
...
void log_rows() noexcept {
    ...
    try {
        ...
        log("config_open file=%s source=%s keys=%u unknown=%u invalid=%u duplicate=%u env_only=%u renamed=%u bytes=%lu lines=%u us=%llu%s",
            file.c_str(), source_, result_.keys, result_.unknown, result_.invalid, result_.duplicate, result_.env_only,
            result_.renamed, static_cast<unsigned long>(bytes_), result_.lines, us_, extra.c_str());
        for (unsigned k = 0; k < result_.issue_count; ++k) {
            const parse::Issue& issue = result_.issues[k];
            log("config_key key=%s problem=%s line=%u value=%s", printable(issue.key, 64).c_str(),
                parse::problem_name(issue.problem), issue.line, shown_value(issue.entry, issue.value).c_str());
        }
        if (result_.more) log("config_more=%u", result_.more);
        if (!std::strcmp(source_, "game") || !std::strcmp(source_, "override")) {
            // The keys the file set, as the resolver hands them out; those the environment overrides are named after
            // them.
            ...
            for (unsigned i = 0; i < schema::entry_count; ++i) {
                ...
                wchar_t name[96];
                wide_name(schema::entries[i].env, name, 96);
                if (in_environment(name)) {
                    overridden += overridden.empty() ? "" : ",";
                    overridden += schema::entries[i].key;
                    continue;
                }
                line += ' ';
                line += schema::entries[i].key;
                line += '=';
                line += shown_value(int(i), result_.value[i]);
            }
