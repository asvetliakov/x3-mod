...
bool install_at(std::uintptr_t window_address, std::uint32_t key) {
    ...
    if (!window_address)
        reason = "invalid_site";
    else if (!key_valid(key))
        reason = "bad_key";
    else if (!engine_patch::install_window_open())
        reason = "late_claim";
    else if (!engine_patch::read_code(window_address, current, window_length))
        reason = "bytes_mismatch";
    else
        reason = plan(current, key, replacement);
    if (!reason) {
        ...
        if (!protect(code, PAGE_EXECUTE_READWRITE, &site_protection))
            ...
            patched_ = true;
            site_ = site;
            key_ = key; // ownership published before the mutation, so a failed rollback stays registered
            bool atomic = false;
            const bool flushed = swap(code, replacement, &atomic);
            write_ = atomic ? "atomic" : "plain";
            unsigned char readback[site_length]{};
            const bool verified = flushed && engine_patch::read_code(site, readback, site_length) &&
                                  !std::memcmp(readback, replacement, site_length);
            DWORD unused = 0;
            ...
            if (!verified || !protected_again) {
                ...
                if (engine_patch::read_code(site, back, site_length) && !std::memcmp(back, original, site_length)) {
                    ...
                    patched_ = false;
                    key_ = 0;
                    reason = reprotected ? "patch_rolled_back" : "rollback_unprotected"; // the latter: original bytes,
                                                                                         // page left writable
                } else
...
bool initialize() {
    ...
    if (!requested)
        ...
        applied = install_at(window_va, key);
    log("pause_key_only patched=%u key=0x%lx reason=%s requested=%u site=0x%08lx write=%s", patched_ ? 1u : 0u,
        static_cast<unsigned long>(key), state_, requested ? 1u : 0u, static_cast<unsigned long>(site_va), write_);
    SetLastError(error);
    return applied;
}
bool shutdown() {
