...
struct Result {
    ...
    void* buffer = nullptr;
    uint32_t size = 0;   // output (Handled)
    uint32_t extent = 0; // compressed bytes read (Handled or later fallback)
    bool catalogue = false, scrambled = false;
    int32_t expected_cursor = 0; // catalogue: the cursor the original's chunk loop leaves (see resource-reader.md
                                 // "Record cursor")
    ...
    uint64_t read_ticks = 0, scan_ticks = 0, alloc_ticks = 0, inflate_ticks = 0; // phases: fseek+fread,
                                                                                 // XOR+magic+header, malloc,
...
struct Statistics {
    ...
    uint64_t read_ticks = 0, scan_ticks = 0, alloc_ticks = 0, inflate_ticks = 0; // phase decomposition of `ticks` over
                                                                                 // handled files
};
...
struct VerifyEvent {
    ...
    bool equal = false, original_null = false, globals_ok = false, counters_ok = false, cursor_ok = false,
         position_ok = false, catalogue = false, scrambled = false;
    int32_t cursor = 0, expected_cursor = 0;
    long position = 0, expected_position = 0; // after the original ran vs the chunk-loop prediction
...
struct PoolStatistics {
    uint64_t opens = 0, reused = 0, real_opens = 0, closes = 0, kept = 0, real_closes = 0, errors = 0, full = 0;
    unsigned held = 0;
};
