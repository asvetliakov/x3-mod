...
template <class Encode> std::uintptr_t emit_stub(Encode&& encode, void*** slot_out) {
...
bool install_at(std::uintptr_t p1, std::uintptr_t p2, bool counters) {
    ...
    if (!p1 || !p2)
        ...
        reason = "late_claim";
    else if (!windows_match(p1, p2))
        reason = "bytes_mismatch";
    else if (!pin_self())
        reason = "pin_failed";
    std::uintptr_t p1_stub = 0, p2_stub = 0;
    void** p1_slot = nullptr;
    void** p2_slot = nullptr;
    if (!reason) {
        const std::uint32_t enabled = std::uint32_t(reinterpret_cast<std::uintptr_t>(&x3m_collide_box_cull_enabled));
        const auto slot_address = [](unsigned i) {
            return std::uint32_t(reinterpret_cast<std::uintptr_t>(&x3m_collide_box_cull_counters[i]));
        };
        ...
        p1_stub = emit_stub(
            [&](std::uint32_t at, std::uint32_t slot, unsigned char* out) {
                return encode_p1_stub(at, enabled, c1, slot, std::uint32_t(p1 + p1_continue_offset), out);
            },
            &p1_slot);
        p2_stub = emit_stub(
            [&](std::uint32_t at, std::uint32_t slot, unsigned char* out) {
                return encode_p2_stub(at, enabled, c2, slot, std::uint32_t(p2 + p2_continue_offset), out);
            },
            &p2_slot);
    ...
    if (!reason) {
        ...
        if (!hook_site(p1_site_, spec_for("collide_box_cull_p1", p1, core::p1_site), p1_stub, p1_slot, &reason)) {
            if (p1_site_.patched_in) {
                patched_ = true;
                state_ = "p1_rollback_failed";
                return false;
            } // registered for shutdown()
        } else if (!hook_site(p2_site_, spec_for("collide_box_cull_p2", p2, core::p2_site), p2_stub, p2_slot,
                              &reason)) {
            // Partial install: P1 goes back; a live but unrestorable P2 keeps both registered so shutdown() tries
            // again.
            if (p2_site_.patched_in) {
                patched_ = true;
                state_ = "p2_rollback_failed";
                return false;
            }
            if (!engine_patch::restore(p1_site_)) {
                patched_ = true;
                state_ = "p1_rollback_failed";
                return false;
            }
...
bool initialize() {
    ...
    else if (!bytes_match(sqrt_helper_va, sqrt_helper, sqrt_helper_length) ||
             !bytes_match(ftol_helper_va, ftol_helper, ftol_helper_length) ||
             !call_targets(p1_sqrt_call_va, sqrt_helper_va) || !call_targets(p1_ftol_call_va, ftol_helper_va) ||
             !call_targets(p2_sqrt_call_va, sqrt_helper_va) || !call_targets(p2_ftol_call_va, ftol_helper_va))
        state_ = "helper_mismatch";
    else
        applied = install_at(p1_site_va, p2_site_va, true);
    log("collide_box_cull requested=%u patched=%u reason=%s p1_site=0x%08lx p2_site=0x%08lx write_p1=%s write_p2=%s stub_p1=0x%08lx stub_p2=0x%08lx counters=%u enabled=%u",
        requested ? 1u : 0u, patched_ ? 1u : 0u, state_, static_cast<unsigned long>(p1_site_va),
        static_cast<unsigned long>(p2_site_va),
        p1_site_.patched_in ? (p1_site_.atomic_write ? "atomic" : "plain") : "none",
        p2_site_.patched_in ? (p2_site_.atomic_write ? "atomic" : "plain") : "none",
        static_cast<unsigned long>(p1_stub_), static_cast<unsigned long>(p2_stub_), counters_ ? 1u : 0u,
        x3m_collide_box_cull_enabled ? 1u : 0u);
    SetLastError(error);
    return applied;
}
bool shutdown() {
    ...
    patched_ = false;
    counters_ = false;
    p1_stub_ = p2_stub_ = 0; // the stubs stay in the arena (a thread may still be inside them)
...
std::uintptr_t p1_stub_address() {
    return patched_ ? p1_stub_ : 0;
}
std::uintptr_t p2_stub_address() {
    return patched_ ? p2_stub_ : 0;
}
void set_enabled(bool enabled) {
    x3m_collide_box_cull_enabled = patched_ && enabled ? 1 : 0;
}
bool enabled() {
    return x3m_collide_box_cull_enabled != 0;
}
void take_counters(std::uint32_t out[4]) {
    for (unsigned i = 0; i < 4; ++i)
        out[i] = static_cast<std::uint32_t>(
            InterlockedExchange(reinterpret_cast<volatile LONG*>(&x3m_collide_box_cull_counters[i]), 0));
}
void present(unsigned long long device, unsigned long long frame, bool captured) {
    ...
    if (captured)
        log("collide_census_frame device=%llu frame=%llu p1_pairs=%lu p1_rejected=%lu p2_cands=%lu p2_rejected=%lu counters=%u enabled=%u",
            device, frame, (unsigned long)values[0], (unsigned long)values[1], (unsigned long)values[2],
            (unsigned long)values[3], counters_ ? 1u : 0u, x3m_collide_box_cull_enabled ? 1u : 0u);
    if (window_.full()) {
        WindowSummary s;
        if (window_.close(s) && windows_)
            log("collide_census frame=%llu frames=%u p1_pairs_p50=%llu p1_pairs_max=%llu p1_pairs_sum=%llu p1_rejected_p50=%llu p1_rejected_max=%llu p1_rejected_sum=%llu "
                ...
                s.sum[2], s.p50[3], s.max[3], s.sum[3], counters_ ? 1u : 0u, x3m_collide_box_cull_enabled ? 1u : 0u);
    }
    SetLastError(error);
}
}
