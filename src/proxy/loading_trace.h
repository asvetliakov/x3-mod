...
#include <array>
#include <cstdint>
struct ID3DXMesh; // d3dx9mesh.h interface; declared here so translation units without D3DX headers can include this
                  // file
...
enum class Operation : unsigned {
    ...
    FindFirst,
    FindNext,
    FindClose, // resource resolver directory enumeration (loading-orchestration.md, section 2)
    GzGetc,
    GzTell,
    GzClose,
    GzWrite, // savegame stream rows (savegame-gz-stream.md); routed through gz_buffer when X3M_GZ_BUFFER=1
...
struct AdjacencyStatistics {
    ...
    std::array<uint64_t, 9> fallback_reasons{}; // input, gate, declaration, size, lock, module, math_table,
                                                // competing_normals, fp_domain
