...
constexpr GUID kCLSID_CWMADecMediaObject = {
    0x2eeb4adf, 0x4578, 0x4d10, {0xbc, 0xa7, 0xbb, 0x95, 0x5f, 0x56, 0x32, 0x0a}};
constexpr GUID kDMOCATEGORY_AUDIO_DECODER = {
...
struct IDMOWrapperFilterLocal {
    const IDMOWrapperFilterVtbl* vtbl;
};
...
constexpr engine_patch::SiteSpec kSite = {
    "voice_dmo_init_after", 0x004cfd46, {0x8b, 0xf0, 0x81, 0xfe, 0x0e, 0x00, 0x07, 0x80}, 8, 0, 0};
...
#endif
struct Record {
    std::uint32_t object, filter, qi_hr, init_hr;
};
...
struct Fault {
    std::uint32_t code, address, access_kind, access, thread, arena, eip, esp, eax, ebx, esi, edi, hits, activations;
};
Fault fault{};
...
void handle(std::uint32_t* regs) noexcept {
    ...
    if (r.object > UINT32_MAX - kDecoderSlot - 4 ||
        !engine_memory::read(r.object + kDecoderSlot, &filter, sizeof filter) || !filter) {
    ...
    if (FAILED(HRESULT(r.qi_hr)) || !view)
        skipped.fetch_add(1, std::memory_order_relaxed);
    else {
        r.init_hr = std::uint32_t(view->vtbl->Init(view, kCLSID_CWMADecMediaObject, kDMOCATEGORY_AUDIO_DECODER));
        view->vtbl->Release(view);
        if (SUCCEEDED(HRESULT(r.init_hr))) {
            retries_ok.fetch_add(1, std::memory_order_relaxed);
            regs[7] = r.init_hr;
        } else
            retries_failed.fetch_add(1, std::memory_order_relaxed); // EAX untouched: the game retries Init as it does
                                                                    // today
    }
...
extern "C" __attribute__((force_align_arg_pointer)) void __cdecl x3m_voice_dmo_fallback_enter(std::uint32_t* regs) {
...
void* emit(void*** next_out) {
    ...
    e.byte(0x50);
    e.byte(0xe8);
    e.rel32(reinterpret_cast<const void*>(&x3m_voice_dmo_fallback_enter));
    e.byte(0x83);
    e.byte(0xc4);
    e.byte(4);
    for (unsigned i = 0; i < 8; ++i) {
...
int format_fault(char* out, unsigned size, const Fault& f, std::uint32_t faults, std::uint32_t other) noexcept {
    ...
    return std::snprintf(
        out, size,
        "voice_dmo_fallback_fault code=%08lx address=%08lx access_kind=%lu access=%08lx thread=%lu arena=%08lx arena_offset=%s%lx eip=%08lx esp=%08lx eax=%08lx ebx=%08lx esi=%08lx edi=%08lx hits=%lu activations=%lu faults=%lu other_first_chance=%lu\n",
        static_cast<unsigned long>(f.code), static_cast<unsigned long>(f.address),
        static_cast<unsigned long>(f.access_kind), static_cast<unsigned long>(f.access),
        static_cast<unsigned long>(f.thread), static_cast<unsigned long>(f.arena),
        in_arena ? "" : "outside:", static_cast<unsigned long>(in_arena ? f.address - f.arena : f.address),
        static_cast<unsigned long>(f.eip), static_cast<unsigned long>(f.esp), static_cast<unsigned long>(f.eax),
        static_cast<unsigned long>(f.ebx), static_cast<unsigned long>(f.esi), static_cast<unsigned long>(f.edi),
        static_cast<unsigned long>(f.hits), static_cast<unsigned long>(f.activations),
        static_cast<unsigned long>(faults), static_cast<unsigned long>(other));
}
LONG CALLBACK fault_witness(EXCEPTION_POINTERS* info) {
    ...
    default:
        return EXCEPTION_CONTINUE_SEARCH; // C++ throws, breakpoints, guard pages, stack overflow (no room to format):
                                          // not ours
    }
    ...
    const bool execute = record->ExceptionCode == EXCEPTION_ACCESS_VIOLATION && record->NumberParameters >= 2 &&
                         (record->ExceptionInformation[0] == 8 ||
                          (c && std::uintptr_t(c->Eip) == std::uintptr_t(record->ExceptionInformation[1])) ||
                          std::uintptr_t(record->ExceptionInformation[1]) == at);
    if (!execute) {
        other_first_chance.fetch_add(1, std::memory_order_relaxed);
        return EXCEPTION_CONTINUE_SEARCH;
    }
    if (fault_count.fetch_add(1, std::memory_order_acq_rel) != 0)
        return EXCEPTION_CONTINUE_SEARCH; // first execute fault only
    Fault f{};
    f.code = std::uint32_t(record->ExceptionCode);
    f.address = std::uint32_t(reinterpret_cast<std::uintptr_t>(record->ExceptionAddress));
    ...
    f.arena = std::uint32_t(reinterpret_cast<std::uintptr_t>(engine_patch::arena_base()));
    if (c) {
        f.eip = c->Eip;
        f.esp = c->Esp;
        f.eax = c->Eax;
    ...
    f.hits = hits.load(std::memory_order_relaxed);
    f.activations = activations.load(std::memory_order_relaxed);
    fault = f;
    fault_seq.store(1, std::memory_order_release);
    char line[352];
    const int n = format_fault(line, sizeof line, f, 1, other_first_chance.load(std::memory_order_relaxed));
    ...
    if (n > 0 && handle != INVALID_HANDLE_VALUE && handle)
        WriteFile(handle, line, DWORD(n), &written_bytes, nullptr); // unbuffered, no lock; may precede buffered lines
    return EXCEPTION_CONTINUE_SEARCH;
}
bool install(const char*& status) {
    ...
    if (!engine_patch::verify_bytes(spec.address, spec.expected, spec.length)) {
        status = "preflight_bytes";
        return false;
    }
    if (!engine_patch::claim(patch, spec)) {
        status = patch.status;
        return false;
    }
    void** next = nullptr;
    void* stub = emit(&next);
    if (stub && next && engine_patch::store_pointer(next, *patch.entry) && engine_patch::push_front(patch, stub)) {
        stub_entry = stub;
        status = "ok";
        return true;
    }
...
bool initialize() {
    ...
    if (installed)
        fault_handler = AddVectoredExceptionHandler(1, &fault_witness); // armed with the hook only; nothing per frame
    ...
    log("voice_dmo_fallback requested=1 installed=%u status=%s site=%08lx length=%u patched=%u site_status=%s condition=%08lx retry_clsid=2eeb4adf-4578-4d10-bca7-bb955f56320a"
        ...
        unsigned(installed), status, static_cast<unsigned long>(site_spec().address), kSite.length,
        unsigned(patch.patched_in), patch.status, static_cast<unsigned long>(kClassNotRegistered),
        reinterpret_cast<unsigned long>(engine_patch::arena_base()), engine_patch::arena_capacity(),
        engine_patch::arena_used(), reinterpret_cast<unsigned long>(stub_entry),
        reinterpret_cast<unsigned long>(patch.tail), reinterpret_cast<unsigned long>(patch.dispatcher),
        reinterpret_cast<unsigned long>(patch.entry), reinterpret_cast<unsigned long>(&x3m_voice_dmo_fallback_enter),
        unsigned(fault_handler != nullptr));
    return installed;
}
#ifdef X3M_VOICE_DMO_FIXTURE
bool fixture_site(std::uintptr_t address) {
...
void report() {
    ...
    if (fault_seq.load(std::memory_order_acquire) != fault_reported) {
        ErrorGuard error;
        fault_reported = fault_seq.load(std::memory_order_acquire);
        char line[352];
        const int n = format_fault(line, sizeof line, fault, fault_count.load(std::memory_order_relaxed),
                                   other_first_chance.load(std::memory_order_relaxed));
        if (n > 0) {
            line[n - 1] = 0;
            log("%s", line);
        } // the preformatted line without its newline
    }
    const std::uint32_t n = written.load(std::memory_order_acquire);
    if (n == reported) return;
    ErrorGuard error;
    if (n - reported > kRing) {
        log("voice_dmo_fallback_overflow lost=%u", n - reported - kRing);
        reported = n - kRing;
    }
    for (; reported != n; ++reported) {
        ...
        log("voice_dmo_fallback activation=%u object=%08lx filter=%08lx qi_hr=%08lx init_hr=%08lx hits=%u activations=%u retries_ok=%u retries_failed=%u skipped=%u",
            reported + 1, static_cast<unsigned long>(r.object), static_cast<unsigned long>(r.filter),
            static_cast<unsigned long>(r.qi_hr), static_cast<unsigned long>(r.init_hr),
            hits.load(std::memory_order_relaxed), activations.load(std::memory_order_relaxed),
            retries_ok.load(std::memory_order_relaxed), retries_failed.load(std::memory_order_relaxed),
