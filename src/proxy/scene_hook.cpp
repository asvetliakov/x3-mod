...
constexpr unsigned char expected_boundary[] = {0x83, 0xbe, 0x9c, 0x02, 0x00, 0x00, 0x11, 0x7e, 0x0a,
                                               0xe8, 0x9a, 0x25, 0x05, 0x00, 0xc6, 0x44, 0x24, 0x13,
                                               0x01, 0x8b, 0x86, 0x70, 0x02, 0x00, 0x00};
bool read_memory(std::uintptr_t address, void* out, std::size_t size) {
    SIZE_T copied = 0;
    return address && ReadProcessMemory(GetCurrentProcess(), reinterpret_cast<void*>(address), out, size, &copied) &&
           copied == size;
}
...
bool patch(void* site, void* target) {
    ...
    if (!read_memory(reinterpret_cast<std::uintptr_t>(site), code.data(), code.size()) || code[0] != 0xe8) {
        state = "callsite_mismatch";
        return false;
    }
    std::uint32_t displacement = 0;
    std::memcpy(&displacement, code.data() + 1, 4);
    if (reinterpret_cast<std::uintptr_t>(site) + 5 + displacement != reinterpret_cast<std::uintptr_t>(target)) {
    ...
    const std::uint32_t redirected = reinterpret_cast<std::uintptr_t>(entry) -
    ...
    if (!flushed || !protected_again) {
        DWORD writable = 0;
        if (!VirtualProtect(site, 5, PAGE_EXECUTE_READWRITE, &writable)) {
...
bool initialize(Listener fn, const X3mCompositorBinding* callbacks) {
    ...
    if (!read_memory(callsite_va, call, 5) || std::memcmp(call, expected_bytes, 5)) {
        state = "callsite_mismatch";
        SetLastError(error);
        return false;
    }
...
bool active() {
    return installed_ && !std::strcmp(state.load(), "active");
}
bool compositor_active() {
    return bridge_bound && active();
}
std::uintptr_t compositor_caller_pc() {
    return compositor_active() ? reinterpret_cast<std::uintptr_t>(patched_site) + 5 : 0;
}
const char* status() {
    return state.load();
}
unsigned long long signals() {
    return signal_count.load(std::memory_order_relaxed);
}
bool shutdown() {
    const DWORD error = GetLastError();
    if (!installed_) {
        const bool result = unbind_bridge();
        SetLastError(error);
        return result;
    }
    ...
    if (bridge_bound && x3m_compositor_bridge_active()) {
        SetLastError(error);
        return false;
    }
    ...
    if (!read_memory(reinterpret_cast<std::uintptr_t>(patched_site), code, 5) ||
        ...
        state = "shutdown_not_owned";
        SetLastError(error);
        return false;
    }
    listener.store(nullptr);
    if (!VirtualProtect(patched_site, 5, PAGE_EXECUTE_READWRITE, &protection)) {
    ...
    if (flush && protect) {
        installed_ = false;
        patched_site = nullptr;
        unbound = unbind_bridge();
    }
    SetLastError(error);
    return flush && protect && unbound;
}
