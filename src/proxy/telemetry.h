...
public:
    Scope(State& state, Metric metric)
        : state_(enabled(metric) ? &state : nullptr)
        , metric_(metric)
        , begin_(state_ ? now() : 0) {}
    ~Scope() {
        if (state_) record(*state_, metric_, now() - begin_);
    }
};
}
