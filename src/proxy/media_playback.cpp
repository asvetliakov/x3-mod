...
Transition Adapter::loop_seek(SessionHandle h, std::int32_t start, std::int32_t end, bool* backpressure) noexcept {
    if (backpressure) *backpressure = false;
    if (!find(h)) return {};
    return transition(runtime_.loop_seek(h, start, end, backpressure), {});
}
