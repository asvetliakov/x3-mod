...
struct SiteDefinition { engine_patch::SiteSpec spec; ProbeKind kind; const char* extra[4]; };
const SiteDefinition definitions[site_count]={
    {{"resource_load",  0x004e8e10,{0x55,0x8b,0xec,0x83,0xe4,0xf8},6,0},ProbeKind::Plain,{"-","-","-","-"}},
    {{"resource_open",  0x004e8780,{0x53,0x8b,0x5c,0x24,0x08},5,4},ProbeKind::ResourceOpen,{"loose","catalogue","failed","-"}},
    {{"name_resolve",   0x004e7590,{0x6a,0xff,0x68,0x1d,0xfa,0x52,0x00},7,12},ProbeKind::Resolve,{"hit","miss","-","-"}},
    {{"resource_read",  0x004e8880,{0x81,0xec,0x54,0x04,0x00,0x00},6,0},ProbeKind::ResourceRead,{"plain","catalogue","gzhandle_or_unopened","null_result"}},
    {{"read_dispatch",  0x004e9210,{0xf6,0x46,0x04,0x01,0x53},5,4},ProbeKind::ReadDispatch,{"plain","catalogue","gzhandle","unopened"}},
    {{"crt_fgetc",      0x0050fff5,{0x6a,0x0c,0x68,0x40,0xd0,0x56,0x00},7,0},ProbeKind::CountOnly,{"-","-","-","-"}},
    {{"sopen_helper",   0x00527869,{0x55,0x8b,0xec,0x83,0xec,0x34},6,0},ProbeKind::Plain,{"-","-","-","-"}},
    {{"find_wrapper",   0x004d2950,{0x53,0x55,0x56,0x57,0xbd,0x38,0x01,0x00,0x00},9,0},ProbeKind::FindWrapper,{"-","-","-","-"}},
    {{"signature_check",0x004cabc0,{0x83,0xec,0x1c,0x53,0x8b,0x5c,0x24,0x34},8,0},ProbeKind::Plain,{"-","-","-","-"}},
    {{"texture_body",   0x004dd2c0,{0x6a,0xff,0x68,0x50,0xfd,0x52,0x00},7,0},ProbeKind::Plain,{"-","-","-","-"}},
    {{"texture_loader", 0x004dc540,{0x6a,0xff,0x68,0x70,0xfd,0x52,0x00},7,0},ProbeKind::Plain,{"-","-","-","-"}},
    {{"mesh_body",      0x004bc680,{0x83,0xec,0x10,0x53,0x55},5,0},ProbeKind::Plain,{"-","-","-","-"}},
};
...
const char* kind_name(ProbeKind kind){
    switch(kind){case ProbeKind::ResourceOpen:return "resource_open";case ProbeKind::Resolve:return "resolve";case ProbeKind::ResourceRead:return "resource_read";
    case ProbeKind::ReadDispatch:return "read_dispatch";case ProbeKind::FindWrapper:return "find_wrapper";case ProbeKind::CountOnly:return "count_only";default:return "plain";}
}
...
void* emit_exit_stub() {
    ...
    e.byte(0x83);e.byte(0xec);e.byte(0x04);e.byte(0x9c);e.byte(0x60);e.byte(0x89);e.byte(0xe0);e.byte(0x50);
    e.byte(0xe8);e.rel32(reinterpret_cast<const void*>(&x3m_probe_exit));
    e.byte(0x83);e.byte(0xc4);e.byte(0x04);e.byte(0x89);e.byte(0x44);e.byte(0x24);e.byte(0x24);e.byte(0x61);e.byte(0x9d);e.byte(0xc3);
    return e.finish();
}
...
void* emit_entry_stub(unsigned index,void*** next_out) {
    ...
    if(!e.ok())return nullptr;
    void* start=e.here();
    e.byte(0x9c);e.byte(0x60);e.byte(0x89);e.byte(0xe0);e.byte(0x50);e.byte(0x6a);e.byte(static_cast<unsigned char>(index));
    e.byte(0xe8);e.rel32(reinterpret_cast<const void*>(&x3m_probe_enter));
    e.byte(0x83);e.byte(0xc4);e.byte(0x08);e.byte(0x61);e.byte(0x9d);
...
bool install_site(unsigned i,const engine_patch::SiteSpec& spec,ProbeKind kind,const char* const extra[4]) {
    auto& site=sites[i];auto& report=reports[i];
    report={spec.name,spec.address,spec.length,kind!=ProbeKind::CountOnly&&kind!=ProbeKind::ReadDispatch,"pending",{extra[0],extra[1],extra[2],extra[3]}};
    probe_configure(i,ProbeConfig{kind,spec.ret_pop});
    if(!engine_patch::claim(site,spec)){report.status=site.status;return false;}
    void** next=nullptr;void* stub=emit_entry_stub(i,&next);
    if(!stub||!next){report.status="stub_failed";return false;}
    // Continuation first, then the chain head: a thread entering the site
    // between the two stores still lands on a complete stub.
    if(!engine_patch::store_pointer(next,*site.entry)){report.status="chain_write_failed";return false;}
    if(!engine_patch::push_front(site,stub)){report.status="chain_failed";return false;}
    report.status="active";++installed_;
    return true;
}
bool install_all(const engine_patch::SiteSpec* specs,const unsigned* kinds,const char* const (*extras)[4],const uint32_t* size_global) {
...
unsigned installed_sites(){return installed_;}
const SiteReport& site(unsigned index){return reports[index<site_count?index:0];}
engine_patch::Site* resource_read_site(){return sites[resource_read_index].patched_in?&sites[resource_read_index]:nullptr;}
bool initialize() {
    ...
    log("loading_probes requested=1 installed=%u sites=%u exit_stub=%u nesting=%u arena_used=%u scope=entry_counting_trampolines behaviour_change=0",
        installed_,site_count,exit_stub!=nullptr,unsigned(loading_trace::light::nesting_available()),engine_patch::arena_used());
    SetLastError(error);
    return okay;
}
void report() {
    ...
    for(unsigned i=0;i<site_count;++i){
        ...
        log("loading_probe site=%s va=0x%08lx qpc=%llu calls=%llu exits=%llu inclusive_ticks=%llu max_ticks=%llu total_us=%.3f max_us=%.3f overflow=%llu desync=%llu bytes=%llu x0=%llu x1=%llu x2=%llu x3=%llu",
            ...
            double(row.inclusive)*1e6/double(frequency),double(row.maximum)*1e6/double(frequency),row.overflow,row.desync,row.bytes,row.extra[0],row.extra[1],row.extra[2],row.extra[3]);
        for(unsigned k=0;k<probe_caller_limit;++k)if(row.caller_address[k]&&row.caller_calls[k])
            log("loading_probe_caller site=%s caller=0x%08lx calls=%llu",r.name,static_cast<unsigned long>(row.caller_address[k]),row.caller_calls[k]);
    }
    ...
    for(unsigned i=0;i<n;++i){
        const unsigned op=paths[i].op;const unsigned base=static_cast<unsigned>(loading_trace::Operation::DirectoryCreate);
...
bool fixture_install(const engine_patch::SiteSpec* specs,unsigned count,const unsigned* kinds,const uint32_t* size_global) {
    if(count!=site_count)return false;
    static const char* extra_storage[site_count][4];
    for(unsigned i=0;i<site_count;++i)for(unsigned k=0;k<4;++k)extra_storage[i][k]=definitions[i].extra[k];
    initialized_=true;requested_=true;
    return install_all(specs,kinds,extra_storage,size_global);
}
void fixture_shutdown(){shutdown();}
#endif
}
