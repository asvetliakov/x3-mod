...
struct Request {
    std::uint32_t cockpit = 0, target = 0, mode = 0;
};
struct Pump {
    std::uint32_t active = 0, flags = 0;
    bool valid = false;
};
struct Segment {
    Stamp begin{}, end{};
...
template <class T> struct Window {
    ...
    void add(const T& v) noexcept {
        ...
        if (first_used < 4)
            ...
            recent[next] = v;
            next = (next + 1) % 4;
            if (recent_used < 4) ++recent_used;
        }
    }
    void clear() noexcept {
        first_used = recent_used = next = 0;
        count = 0;
    }
};
struct Metric {
    ...
    void add(const Stamp& a, const Stamp& b) noexcept {
        ...
        ++count;
        const auto elapsed = b.qpc - a.qpc;
        total += elapsed;
        if (elapsed > maximum) {
            maximum = elapsed;
            begin = a.qpc;
        ...
        if (a.cpu && b.cpu && b.user >= a.user && b.kernel >= a.kernel) {
            ++cpu_valid;
            user += b.user - a.user;
...
struct Core {
    std::uint64_t frequency = 0, loop = 0, invalidated = 0, unmatched = 0, overflow = 0, order_errors = 0,
                  clock_errors = 0;
    std::uint64_t frame_count = 0, frame_max = 0, frame_max_begin = 0, frame_max_end = 0, frame_max_id = 0,
                  frame_max_device = 0;
    ...
    void invalidate() noexcept {
        ...
        pump = {};
        input_part = 0;
        input_ticks = input_last = 0;
        input_valid = false;
        for (auto& t : stack) {
            t.raw = 0;
            t.bridged = t.detail = false;
            t.endpoint = {};
        }
    }
    ...
    void segment(const Stamp& at) noexcept {
        ...
        if (phase == 6) {
            input_parts[input_part].add(phase_begin, at);
            input_ticks += at.qpc - phase_begin.qpc;
        }
        if (anchor_valid && at.qpc > phase_begin.qpc) {
            if (used < tape_capacity)
                tape[used++] = {phase_begin, at, loop, phase, input_part, phase_request, request, pump};
            else {
                ++overflow;
                tape_overflow = true;
            }
    ...
    void bridge(const Present& p, Stamp sample = {}) noexcept {
        if (!depth || stack[depth - 1].kind != 3 || stack[depth - 1].raw != p.raw || !p.raw || !p.device || !p.qpc) {
            ++unmatched;
            return;
        }
        auto& t = stack[depth - 1];
        if (t.bridged || p.qpc < t.begin.qpc || (anchor_valid && p.qpc < anchor.qpc)) {
            ++order_errors;
            invalidate();
            return;
        }
        ...
        if (!phase_live) return;
        if (anchor_valid) {
            if (p.frame != anchor.frame + 1) {
                ++order_errors;
                used = 0;
                tape_overflow = true;
            } else {
                const auto elapsed = p.qpc - anchor.qpc;
                ++frame_count;
                if (elapsed > frame_max) {
                    frame_max = elapsed;
                    frame_max_begin = anchor.qpc;
                    frame_max_end = p.qpc;
                    frame_max_id = p.frame;
                    frame_max_device = p.device;
                }
                if (frequency && elapsed >= (frame_threshold ? frame_threshold : frequency / 20)) {
                    ...
                    f.current = p;
                    f.used = used;
                    f.overflow = tape_overflow;
                    f.dispatch_begin = t.begin.qpc;
                    f.dispatch_end = 0;
                    f.covered = 0;
                    for (unsigned i = 0; i < used; ++i) {
                        f.segments[i] = tape[i];
                        f.covered += tape[i].end.qpc - tape[i].begin.qpc;
                    }
                    t.detail = true;
                }
            }
    ...
    void end(unsigned kind, const Stamp& at, std::uint32_t result = 0) noexcept {
        if (!valid(at)) return;
        if (!depth || stack[depth - 1].kind != kind) {
            ++unmatched;
            invalidate();
            return;
        }
        auto& t = stack[depth - 1];
        if (at.qpc < t.begin.qpc) {
            ++clock_errors;
            invalidate();
            return;
        }
        ...
        const Call record{kind,      t.begin, at,         t.request, t.endpoint, result,
                          t.witness, t.loop,  t.children, t.phase,   false};
        if (first) {
            first_mask |= 1u << kind;
            pending_first |= 1u << kind;
            first_calls[kind - 4] = record;
            first_calls[kind - 4].first = true;
        }
        if (frequency && at.qpc - t.begin.qpc >= frequency / 100) slow_calls.add(record);
        if (depth > 1) stack[depth - 2].children += at.qpc - t.begin.qpc;
        if (t.detail) {
            t.staged.dispatch_end = at.qpc;
            slow_frames.add(t.staged);
        }
        t.raw = 0;
        t.bridged = t.detail = false;
        --depth;
    ...
    void target_end(unsigned kind, const Stamp& at, std::uintptr_t esp, std::uint32_t result) noexcept {
        ...
        if (!depth || stack[depth - 1].kind != kind || stack[depth - 1].witness.end_esp != esp) {
            ++ignored_joins;
            return;
        }
        end(kind, at, result);
    ...
    void clear_window() noexcept {
        ...
        slow_frames.clear();
        slow_calls.clear();
        frame_count = frame_max = frame_max_begin = frame_max_end = frame_max_id = frame_max_device = 0;
...
struct LoadingPhases {
    ...
    struct Marker {
        unsigned name = 0;
        std::uint64_t frame = 0, qpc = 0, stall = 0;
    };
    ...
    unsigned present(std::uint64_t device, std::uint64_t reset, std::uint64_t frame, std::uint64_t qpc,
                     Marker* out) noexcept {
        unsigned n = 0;
        if (have_last && device == last_device && reset == last_reset && qpc >= last_qpc && stall_ticks &&
            qpc - last_qpc >= stall_ticks) {
            const auto gap = qpc - last_qpc;
            if (!(emitted & (1u << MenuShown))) {
                out[n++] = {MenuShown, frame, qpc, gap};
                emitted |= 1u << MenuShown;
