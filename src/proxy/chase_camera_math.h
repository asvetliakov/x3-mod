...
inline Vec3 row(const Mat3& a, int i) {
    return {a.m[i][0], a.m[i][1], a.m[i][2]};
}
...
inline Vec3 mul(Vec3 v, const Mat3& b) {
    return {v.x * b.m[0][0] + v.y * b.m[1][0] + v.z * b.m[2][0], v.x * b.m[0][1] + v.y * b.m[1][1] + v.z * b.m[2][1],
            v.x * b.m[0][2] + v.y * b.m[1][2] + v.z * b.m[2][2]};
}
...
inline bool finite(const Mat3& a) {
    for (auto& r : a.m)
        for (double v : r)
...
inline double determinant(const Mat3& a) {
    return dot(row(a, 0), cross(row(a, 1), row(a, 2)));
}
...
inline bool orthonormalize(Mat3& a) {
    ...
    const double lf = length(f);
    if (!(lf > 1e-9)) return false;
    f = f * (1.0 / lf);
    u = u - f * dot(u, f);
    const double lu = length(u);
    if (!(lu > 1e-9)) return false;
    u = u * (1.0 / lu);
...
inline double orthonormality_error(const Mat3& a) {
    ...
    for (int i = 0; i < 3; ++i)
        for (int j = 0; j < 3; ++j) e = std::fmax(e, std::fabs(dot(row(a, i), row(a, j)) - (i == j ? 1.0 : 0.0)));
    return e;
}
...
inline Mat3 exp_rotation(Vec3 r) {
    ...
    w.m[0][0] = t * k.x * k.x + c;
    w.m[0][1] = t * k.x * k.y + s * k.z;
    w.m[0][2] = t * k.x * k.z - s * k.y;
    w.m[1][0] = t * k.x * k.y - s * k.z;
    w.m[1][1] = t * k.y * k.y + c;
    w.m[1][2] = t * k.y * k.z + s * k.x;
    w.m[2][0] = t * k.x * k.z + s * k.y;
    w.m[2][1] = t * k.y * k.z - s * k.x;
    w.m[2][2] = t * k.z * k.z + c;
    return w;
}
...
inline Vec3 log_rotation(const Mat3& w) {
    ...
    if (s < 1e-9) {
        ...
        int i = 0;
        if (w.m[1][1] > w.m[i][i]) i = 1;
        if (w.m[2][2] > w.m[i][i]) i = 2;
        Vec3 k;
        double* kk[3] = {&k.x, &k.y, &k.z};
        *kk[i] = std::sqrt(std::fmax(0.0, (w.m[i][i] + 1.0) * 0.5));
        for (int j = 0; j < 3; ++j)
            if (j != i) *kk[j] = (w.m[i][j] + w.m[j][i]) * 0.25 / (*kk[i] > 1e-9 ? *kk[i] : 1.0);
        const double lk = length(k);
        if (!(lk > 1e-9)) return {};
        return k * (angle / lk);
    }
    return axis * (angle / s);
...
inline Mat3 from_fixed(const std::int32_t* rows /* 12 ints, stride 4 per row */) {
    Mat3 a;
    for (int i = 0; i < 3; ++i)
        for (int j = 0; j < 3; ++j) a.m[i][j] = rows[i * 4 + j] / fixed_one;
    return a;
}
...
inline bool to_int(Vec3 v, std::int32_t* out) {
    ...
    for (double x : c)
        if (!(x >= -2147483648.0 && x <= 2147483647.0)) return false;
    out[0] = static_cast<std::int32_t>(c[0]);
    out[1] = static_cast<std::int32_t>(c[1]);
    out[2] = static_cast<std::int32_t>(c[2]);
    return true;
}
...
struct Tunables {
    ...
    double pitch_down_deg = 0.5;   // near-parallel elevated framing (user default 2026-09-16); 0 keeps legacy framing;
                                   // (0,30] sets ship-relative downward look
    double distance_scale = 1.05;  // multiplies the vanilla boom offset (X3M_CHASE_DISTANCE_SCALE; 0.90 before the
                                   // 2026-09-23 user decision)
    bool fov_compensate = true;    // multiply the boom by fov_compensation(): same ship screen size at any vertical FOV
                                   // (X3M_CHASE_FOV_COMPENSATE)
    ...
    double combat_tightness = 0.0; // 0..1: while a target is locked (Input::target_locked) both time constants are
                                   // scaled by (1 - tightness); 1 = rigid follow (X3M_CHASE_COMBAT_TIGHTNESS)
    ...
    unsigned snap_coalesce_frames = 3;      // a sector-only snap within this many applied frames of the previous snap
                                       // re-seats the springs but raises no second cut (A4: gate jump = teleport, then
                                       // the sector follows a frame later)
};
inline bool valid(const Tunables& t) {
    return t.rot_tau > 0 && t.rot_tau <= 10 && t.pos_tau > 0 && t.pos_tau <= 10 && std::isfinite(t.offset_y) &&
           std::fabs(t.offset_y) <= 1 && t.pitch_down_deg >= 0 && t.pitch_down_deg <= 30 && t.distance_scale > 0 &&
           t.distance_scale <= 10 && t.lag_clamp_deg >= 0 && t.lag_clamp_deg <= 90 && t.pos_lag_clamp >= 0 &&
           t.pos_lag_clamp <= 1 && t.combat_tightness >= 0 && t.combat_tightness <= 1 && t.max_dt > 0 &&
           t.max_dt <= 5 && t.snap_ratio >= 1 && t.max_orthonormality_error > 0 && t.snap_coalesce_frames <= 60;
}
...
enum class Verdict : std::uint32_t {
    ...
    SpecialConnect = 3, // connect mode other than 0 (4/5/6/8/9 scripted/cinematic; 1/2/7 without study semantics pass
                        // through too)
    ...
    VerbatimBasis = 7,  // connect mode 3 or +0x1a0 & 4: the engine wrote camera.basis = +0xf0 verbatim, so the derived
                        // ship basis would be the identity (A2)
};
...
inline Step step(State& s, const Input& in, double dt, const Tunables& t, Pose* out) {
    Step r;
    r.dt = dt;
    auto refuse = [&](Verdict v) {
        r.verdict = v;
        ++s.refused;
        reset(s);
        return r;
    };
    if (!valid(t) || !finite(in.ship_pos) || !finite(in.vanilla_pos) || !finite(in.vanilla_cam) ||
        !finite(in.view_rel) || !std::isfinite(in.half_vfov_tan) || !(in.half_vfov_tan > 0) || !std::isfinite(dt))
        return refuse(Verdict::InvalidInput);
    if (orthonormality_error(in.vanilla_cam) > t.max_orthonormality_error ||
        orthonormality_error(in.view_rel) > t.max_orthonormality_error)
        return refuse(Verdict::InvalidInput);
    ...
    s.last_ref = in.ref_object;
    s.last_sector = in.sector;
    s.last_mode = in.view_mode;
    s.last_connect = in.connect_mode;
    s.last_ship_pos = in.ship_pos;
    ...
    if (snap) {
        s.basis = target;
        s.offset = target_boom;
        s.rot = {};
        s.pos = {};
        s.tracking = true;
        ++s.snaps;
        r.snap_reason = snap;
        s.applied_since_snap = 0;
        if (coalesce) {
            r.coalesced = true;
            ++s.coalesced;
        } else
        ...
        if (clamp_spring(s.rot, t.lag_clamp_deg * pi / 180.0)) {
            ++s.clamps;
            ++s.rotation_clamps;
        }
        s.basis = mul(target, exp_rotation(s.rot.x));
        if (!orthonormalize(s.basis)) {
            r.snap_reason = 32;
            ++s.snaps;
            return refuse(Verdict::NumericFailure);
        }
        ...
        if (clamp_spring(s.pos, t.pos_lag_clamp * target_length)) {
            ++s.clamps;
            ++s.position_clamps;
        }
        s.offset = target_boom + s.pos.x;
    }
    if (!finite(s.basis) || !finite(s.offset) || !finite(s.rot.x) || !finite(s.rot.v) || !finite(s.pos.x) ||
        !finite(s.pos.v)) {
        r.snap_reason = 32;
        ++s.snaps;
        return refuse(Verdict::NumericFailure);
