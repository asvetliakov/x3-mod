...
static_assert(unsigned(renderer::fog_field::Profile::Bluewell) == 1 &&
                  unsigned(renderer::fog_field::Profile::Foggreenoutlands) == 2,
              "embedded fog profile IDs");
struct FogSectorFrame {
    ...
    bool same_key(const FogSectorFrame& b) const noexcept {
        return generation == b.generation && field_generation == b.field_generation && sector == b.sector &&
               table == b.table && record == b.record && index == b.index && profile == b.profile &&
               recipe == b.recipe && status == b.status;
    }
...
inline FogSectorPlacement fog_sector_placement(const FogSectorFrame& f) noexcept {
    ...
    out.key = fog_sector_mix(fog_sector_mix(fog_sector_mix(std::uint64_t(std::uint32_t(f.index))) ^ f.profile) ^
                             f.recipe);
...
inline FogSectorFrame fog_sector_frame(const sector_background::Sample& s, std::uint64_t frame,
    ...
    if (s.status != sector_background::Status::Ready)
        out.reason = sector_background::name(s.status);
    else if (!s.row_valid)
        out.reason = "row_invalid";
    else if (!s.name_valid)
        out.reason = "name_invalid";
    else if (s.camera_check == sector_background::Check::Mismatch || sector_background::anchor_refused(s))
        out.reason = "sample_mismatch";
    else if (s.dust == 0)
        out.reason = "clear";
    else if (s.dust < 0)
        out.reason = "dust_invalid";
    else {
        ...
        if (!out.profile && families) {
            if (const auto* row = families->find(s.family)) {
