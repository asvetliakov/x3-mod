...
enum class Verdict : unsigned char { kept = 0, culled_size, culled_min, culled_other, no_exit };
constexpr unsigned verdict_count = 5;
inline const char* verdict_name(Verdict v) {
    static const char* const names[verdict_count] = {"kept", "culled_size", "culled_min", "culled_other", "no_exit"};
...
// measure < 1 (bit 0x4000000 clear), otherwise by a later step (the
// env-map view's < 20 test with the bit set, or the last-LOD fade).
inline Verdict classify(const Entry& e) {
