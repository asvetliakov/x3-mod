...
constexpr unsigned char exit_window[exit_window_length] = {0x83, 0xf9, 0x03, 0x7c, 0x0a, 0x81, 0x8f, 0x30, 0x01,
                                                           0x00, 0x00, 0x00, 0x00, 0x10, 0x00, 0x8b, 0x7f, 0x0c,
                                                           0x83, 0x3f, 0x00, 0x74, 0x18, 0x8b, 0x74, 0x24, 0x18};
...
constexpr unsigned parent_offset = 0x18, radius_offset = 0xa0, flags12c_offset = 0x12c, flags130_offset = 0x130,
                   model_offset = 0x140, lod_offset = 0x14c, threshold_1d8_offset = 0x1d8, threshold_1dc_offset = 0x1dc;
...
constexpr unsigned model_records_offset = 0x0c, model_lod_count_offset = 0x10, record_threshold_offset = 0x34,
                   ladder_cap = 8;
...
struct Entry {
    ...
    std::uint32_t model_ptr; // the model pointer at the exit site (exit_model_pointer), 0 when the pass had none for
                             // this node
    std::uint32_t flags130;  // node+0x130 at the measure site (projectile_flag: the small-parts stub's exemption)
    std::uint32_t flag31;    // bit 31 of the root's +0x12c (Terran-station LOD branch bit), flag31_unknown when not
                             // resolved
};
...
inline bool format_ladder(char* out, unsigned size, bool known, std::int32_t count, unsigned thresholds,
...
constexpr unsigned body_name_cap = 63, body_name_scan = 256; // printed / scanned for the NUL (names <= 255 by the save
                                                             // format)
...
inline bool body_slot(std::int32_t id, std::int32_t fixed, std::int32_t dynamic, std::uint32_t* slot) {
    const std::int64_t s = id < 1000    ? std::int64_t(id)
                           : id < 20000 ? std::int64_t(id) - 9000
                                        : std::int64_t(fixed) + id - 20000;
...
struct TrackSlot {
    std::uint32_t node, view, model, seen;
    std::int32_t lod;
}; // seen 0 = never used
...
inline Observed track_observe(TrackSlot* table, std::uint32_t node, std::uint32_t view, std::uint32_t model,
                              std::int32_t lod, std::uint32_t frame, std::int32_t* from) {
    ...
    for (unsigned p = 0, i = track_hash(node, view); p < track_probe; ++p, i = (i + 1) & (track_size - 1)) {
        ...
        if (!t.seen) {
            if (!reuse) reuse = &t;
            break;
        } // slots are never emptied: the key is not further on
...
template <class Char> inline bool parse_lod_switch_cap(const Char* text, unsigned length, unsigned* cap) {
...
inline void encode_measure_stub(std::uint32_t at, std::uint32_t enabled, std::uint32_t handler, std::uint32_t next_slot,
                                unsigned char out[measure_stub_length]) {
    out[0] = 0x80;
    out[1] = 0x3d;
    std::memcpy(out + 2, &enabled, 4);
    out[6] = 0x00;
    out[7] = 0x74;
    out[8] = static_cast<unsigned char>(measure_stub_continue - 9);
    out[9] = 0x50;
    out[10] = 0x51;
    out[11] = 0x52;
    out[12] = 0xff;
    ...
    out[25] = 0x57;
    out[26] = 0xe8;
    const std::uint32_t rel = handler - (at + 31);
    std::memcpy(out + 27, &rel, 4);
    out[31] = 0x83;
    out[32] = 0xc4;
    out[33] = 0x14;
    out[34] = 0x5a;
...
//   33  ff 25 abs32      JMP  [next]                ; the tail: displaced MOV + CMP (flags regenerated), jump back to
//   the JE
...
inline void encode_exit_stub(std::uint32_t at, std::uint32_t enabled, std::uint32_t handler, std::uint32_t next_slot,
                             unsigned char out[exit_stub_length]) {
    out[0] = 0x80;
    out[1] = 0x3d;
    std::memcpy(out + 2, &enabled, 4);
    out[6] = 0x00;
    out[7] = 0x74;
    out[8] = static_cast<unsigned char>(exit_stub_continue - 9);
    out[9] = 0x50;
    out[10] = 0x51;
    out[11] = 0x52;
    out[12] = 0xff;
    out[13] = 0x74;
    out[14] = 0x24;
    out[15] = static_cast<unsigned char>(d_slot_offset + 12);
    out[16] = 0xff;
    out[17] = 0x74;
    out[18] = 0x24;
    out[19] = static_cast<unsigned char>(model_slot_offset + 16);
    out[20] = 0x53;
    out[21] = 0x57;
    out[22] = 0xe8;
    const std::uint32_t rel = handler - (at + 27);
    std::memcpy(out + 23, &rel, 4);
    out[27] = 0x83;
    out[28] = 0xc4;
    out[29] = 0x10;
    out[30] = 0x5a;
