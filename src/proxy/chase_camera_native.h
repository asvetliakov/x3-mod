...
inline bool native_read(Read& read, std::uintptr_t base, unsigned offset, void* out, std::size_t bytes) {
    ...
    return base && !(base & 3) && base <= last && offset <= last - base &&
        bytes <= last - base - offset && read(base + offset, out, bytes);
}
template<class Read>
inline bool read_native_anchor(std::uintptr_t reference, Read read, NativeAnchor* out) {
    unsigned char object[0x74];
    auto u32 = [](const unsigned char* bytes, unsigned off) {
        std::uint32_t value; std::memcpy(&value, bytes + off, 4); return value;
    };
    ...
    auto position = [&](unsigned off, Vec3* value) {
        ...
        *value = Vec3{double(p[0]), double(p[1]), double(p[2])};return true;
    };
    ...
    if (sample.base_domain) sample.render_position_valid = position(0xb0, &sample.render_position);
    else { sample.render_position = sample.position; sample.render_position_valid = true; }
    std::int32_t rows[12];
    ...
    if (!sample.base_domain) { sample.basis = sample.render_basis; sample.basis_valid = sample.render_basis_valid; }
    else {
        ...
        } else sample.basis_valid = native_read(read, sample.node, 0x40, rows, sizeof rows);
