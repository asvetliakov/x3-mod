...
bool install_group(const engine_patch::SiteSpec* specs, const char*& status) {
    ...
    for (unsigned i = 0; i < sites::Count; ++i)
        if (!engine_patch::verify_bytes(specs[i].address, specs[i].expected, specs[i].length)) {
    ...
    for (unsigned i = 0; i < sites::Count; ++i) {
        if (engine_patch::claim(patches[i], specs[i])) {
            void** next = nullptr;
            void* stub = game_phases::emit_stub(game_phases::sites::Count + i, &next);
            ...
            status = patches[i].status; // the claim's own reason (bytes_mismatch, late_claim, arena_full, ...)
        bool restored = true;
...
void emit_window() {
    ...
    PhaseText phases;
    for (unsigned i = 0; i < detail::phase_count; ++i)
        phases.add(" %s_p50_us=%llu %s_p95_us=%llu", detail::phase_names[i], s.phase_p50[i], detail::phase_names[i],
                   s.phase_p95[i]);
    ...
    log("frame_phases qpc=%llu frame=%llu frames=%u incomplete=%u dt_p50_us=%llu dt_p95_us=%llu%s views_p50=%llu order_errors=%llu clock_errors=%llu unmatched=%llu dropped=%llu early=%u foreign=%u",
        emitted, s.frame, s.frames, s.incomplete, s.dt_p50, s.dt_p95, phases.finish(), s.views_p50,
        tracker.order_errors, tracker.clock_errors, tracker.unmatched, tracker.dropped,
    ...
    for (unsigned i = 0; i < s.slow_frames_count; ++i) {
        ...
        PhaseText slow;
        for (unsigned p = 0; p < detail::phase_count; ++p)
            slow.add(" %s_us=%llu", detail::phase_names[p], f.phase_us[p]);
...
bool initialize() {
    ...
    initialized = true;
    const bool wanted = log_tier::perf_flag(L"X3M_FRAME_PHASES") || log_tier::debug() ||
                        log_tier::draw_trace(); // X3M_FRAME_PHASES=1, X3M_PERF=1, X3M_DEBUG=1 or X3M_DRAW_TRACE=1 (the
                                                // stamp families need the frame boundary)
    ...
    for (unsigned i = 0; i < sites::Count; ++i)
        log("frame_phase_site index=%u address=%08lx length=%u rel32=%u patched=%u status=%s", i,
...
void frame_impl(std::uint64_t frame) noexcept {
    ...
    residual_phases::frame(frame, taken, taken ? last_sample.phase_us[detail::views_phase] : 0,
                           taken ? last_sample.view_setup_us : 0, taken ? last_sample.view_submit_us : 0,
                           taken ? last_sample.views : 0); // X3M_RESIDUAL_PHASES only: same guard, ahead of the pass
                                                           // group so its accumulator is still open
    pass_phases::frame(frame, taken, taken ? last_sample.view_submit_us : 0); // X3M_PASS_PHASES only: closes the
                                                                              // frame's pass accumulators under this
                                                                              // guard
    light_phases::frame(frame, taken);
    submit_phases::frame(frame, taken); // X3M_SUBMIT_PHASES only: same guard, closes the frame's view_submit candidate
                                        // pairs
    loop_phases::frame(frame, taken, taken ? last_sample.dt_us : 0,
                       taken ? last_sample.phase_us[detail::pre_render] : 0); // X3M_LOOP_PHASES only: same guard, joins
                                                                              // dt and pre_render
    if (!taken) return;
