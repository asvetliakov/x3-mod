...
//           writer); zeroes [ECX+0x180..0x190] and [EAX+0x180..0x190]; CALL 0x0048a890; ADD ESP,0x18; POP EDI/ESI/ECX; RET
//
...
//
// 004e2530  a1 4c 85 60 00        MOV  EAX,[0x0060854c]         <- site 7 (function entry; five inbound calls, all to the entry)
// 004e2535  83 ec 40              SUB  ESP,0x40                 ; first flag writer
...
constexpr unsigned char n6_pre_window[n6_pre_length] = {0xd9,0xee, 0x51, 0xd9,0x1c,0x24, 0x8b,0xce, 0x6a,0x01, 0x6a,0x02, 0x50, 0x57, 0x33,0xff, 0x8b,0xc3};
constexpr unsigned char n6_post_window[n6_post_length] = {0x83,0xc4,0x14, 0x85,0xc0, 0x0f,0x84,0x91,0x00,0x00,0x00};
// Site 7: the first 35 bytes of 0x004e2530 with the two abs32 operands (hits at +1, mode flag at +10) supplied by the caller.
...
constexpr unsigned char n7_window[n7_window_length] = {
    0xa1,0x4c,0x85,0x60,0x00, 0x83,0xec,0x40, 0x83,0x3d,0x34,0x69,0x59,0x00,0x00, 0x53,0x55,0x56,0x57, 0x74,0x0e, 0x85,0xc0, 0x7e,0x0a,
    0x33,0xc0, 0x5f,0x5e,0x5d,0x5b, 0x83,0xc4,0x40, 0xc3};
...
constexpr unsigned char n6_callee[n6_callee_length] = {
    0x8b,0x54,0x24,0x04, 0x8b,0x52,0x5c, 0x83,0xec,0x64, 0x85,0xd2, 0x56, 0x8b,0x74,0x24,0x70, 0x8b,0x76,0x5c, 0x75,0x07};
...
constexpr unsigned flags40_offset = 0x40, flags44_offset = 0x44, class_offset = 0x48, subtype_offset = 0x4a, physics_offset = 0x70, radius_offset = 0xa4;
constexpr unsigned position_offset = 0x30, saved_offset = 0xb0, saved_words = 4, xform_offset = 0xc0, xform_words = 11, node_flags_offset = 0x12c, model_offset = 0x140;
...
enum : std::uint8_t { had_previous = 1, same_position = 2, same_xform = 4, same_saved = 8, unchanged = 16, memo_hit = 32, memo_unsafe = 64, visits_differ = 128 };
struct Entry {
    ObjectKey a, b;
    std::int32_t result;
...
inline std::uint32_t hash_words(const unsigned char* p, unsigned words, std::uint32_t h) {
    for (unsigned i = 0; i < words; ++i) { h = (h ^ load32(p + 4 * i)) * 0x01000193u; h ^= h >> 15; }
    return h;
}
// `object` and `physics` are the two blocks' bytes; the addresses are recorded as given.
inline ObjectKey read_key(std::uint32_t object_va, const unsigned char* object, std::uint32_t physics_va, const unsigned char* physics) {
    ObjectKey k{};
    k.object = object_va; k.physics = physics_va;
    std::memcpy(&k.cls, object + class_offset, 2); std::memcpy(&k.subtype, object + subtype_offset, 2);
    k.flags40 = load32(object + flags40_offset); k.flags44 = load32(object + flags44_offset);
    ...
    k.model = load32(physics + model_offset); k.node_flags = load32(physics + node_flags_offset);
    ...
    h = hash_words(physics + model_offset, 1, h); k.xform_hash = hash_words(physics + node_flags_offset, 1, h);
...
inline std::uint8_t compare_key(const ObjectKey& now, const ObjectKey& then) {
    ...
    if (now.xform_hash == then.xform_hash && now.model == then.model && now.node_flags == then.node_flags) f |= same_xform;
    if (now.saved_hash == then.saved_hash) f |= same_saved;
    return f;
}
struct MemoSummary { std::uint32_t with_previous = 0, unchanged = 0, memo_hits = 0, memo_hit_visits = 0, memo_unsafe = 0, visits_differ = 0, changed_position = 0, changed_xform = 0, changed_saved = 0; };
...
inline MemoSummary annotate(Entry* now, unsigned count, const Entry* then, unsigned then_count) {
    MemoSummary s; unsigned cursor = 0;
    for (unsigned i = 0; i < count; ++i) {
        Entry& e = now[i]; e.flags = 0;
        for (unsigned step = 0; step < then_count; ++step) {
            unsigned j = cursor + step; if (j >= then_count) j -= then_count;
            const Entry& p = then[j];
            if (p.a.object != e.a.object || p.b.object != e.b.object || p.a.physics != e.a.physics || p.b.physics != e.b.physics) continue;
            ...
            if (both == (same_position | same_xform | same_saved)) {
                e.flags |= unchanged; ++s.unchanged;
                if (p.result <= 0) {   // the memo would have answered "no contact"
                    if (e.result <= 0) { e.flags |= memo_hit; ++s.memo_hits; s.memo_hit_visits += e.visits; if (e.visits != p.visits) { e.flags |= visits_differ; ++s.visits_differ; } }
                    else { e.flags |= memo_unsafe; ++s.memo_unsafe; }   // same key, the engine now reports a contact: the key misses an input
                }
            }
            break;
        }
...
inline void order_by_visits(const Entry* entries, unsigned count, std::uint16_t* order) {
    for (unsigned i = 0; i < count; ++i) {
        unsigned j = i;
        while (j && entries[order[j - 1]].visits < entries[i].visits) { order[j] = order[j - 1]; --j; }
...
class StubWriter {
public:
    explicit StubWriter(unsigned char* out, std::uint32_t at) : out_(out), at_(at) {}
    ...
    void raw(std::initializer_list<unsigned char> list) { for (unsigned char b : list) byte(b); }
    void dword(std::uint32_t v) { byte(v & 0xff); byte((v >> 8) & 0xff); byte((v >> 16) & 0xff); byte((v >> 24) & 0xff); }
    void rel32(unsigned char opcode, std::uint32_t target) { byte(opcode); dword(target - (at_ + n_ + 4)); }
    void patch32(unsigned at, std::uint32_t v) { for (unsigned i = 0; i < 4; ++i) if (at + i < stub_capacity) out_[at + i] = static_cast<unsigned char>(v >> (8 * i)); }
private:
    unsigned char* out_; std::uint32_t at_; unsigned n_ = 0;
};
...
inline void emit_save(StubWriter& w) {
    ...
    for (unsigned i = 0; i < 8; ++i) w.raw({0x0f, 0x11, static_cast<unsigned char>(0x44 | (i << 3)), 0x24, static_cast<unsigned char>(i * 16)});
}
inline void emit_restore(StubWriter& w) {
    for (unsigned i = 0; i < 8; ++i) w.raw({0x0f, 0x10, static_cast<unsigned char>(0x44 | (i << 3)), 0x24, static_cast<unsigned char>(i * 16)});
    w.raw({0x81,0xc4,0x80,0x00,0x00,0x00, 0x61, 0x9d});
}
struct N5Operands { std::uint32_t return_va, target, pre_handler, post_handler, busy, nested, foreign; };
...
//   call target                                      (pushes ours into the same slot: the callee sees the engine's exact stack)
//   <save> ; push eax ; call post ; add esp,4 ; <restore>
//   mov byte [busy],0 ; jmp return_va                (ESP as after the callee's `ret`; every register, EFLAGS, x87 as the callee left them)
...
inline unsigned encode_n5_stub(std::uint32_t at, const N5Operands& o, unsigned char out[stub_capacity]) {
    StubWriter w(out, at);
    w.raw({0x81,0x3c,0x24}); w.dword(o.return_va); w.raw({0x0f,0x85}); const unsigned fix_foreign = w.length(); w.dword(0);
    w.raw({0x80,0x3d}); w.dword(o.busy); w.raw({0x00, 0x0f,0x85}); const unsigned fix_nested = w.length(); w.dword(0);
    w.raw({0xc6,0x05}); w.dword(o.busy); w.byte(0x01);
    emit_save(w); w.raw({0x56, 0x53}); w.rel32(0xe8, o.pre_handler); w.raw({0x83,0xc4,0x08}); emit_restore(w);
    w.raw({0x8d,0x64,0x24,0x04});
    w.rel32(0xe8, o.target);
    emit_save(w); w.byte(0x50); w.rel32(0xe8, o.post_handler); w.raw({0x83,0xc4,0x04}); emit_restore(w);
    w.raw({0xc6,0x05}); w.dword(o.busy); w.byte(0x00);
    w.rel32(0xe9, o.return_va);
    const unsigned nested = w.length();
    w.raw({0xff,0x05}); w.dword(o.nested); w.rel32(0xe9, o.target);
    const unsigned foreign = w.length();
    w.raw({0xff,0x05}); w.dword(o.foreign); w.rel32(0xe9, o.target);
    if (!w.ok()) return 0;
...
inline unsigned encode_n6_stub(std::uint32_t at, std::uint32_t counter, std::uint32_t target, unsigned char out[stub_capacity]) {
    StubWriter w(out, at);
    w.raw({0xff,0x05}); w.dword(counter); w.rel32(0xe9, target);
    return w.length();
}
// Site-7 stub: inc dword [counter] ; mov eax,[hits] (the displaced instruction) ; jmp site+5. No clock, no call.
inline unsigned encode_n7_stub(std::uint32_t at, std::uint32_t counter, std::uint32_t hits, std::uint32_t next, unsigned char out[stub_capacity]) {
    StubWriter w(out, at);
