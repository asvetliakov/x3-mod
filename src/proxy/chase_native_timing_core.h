...
inline bool same(const Token& a, const Token& b) {
    return a.valid() && b.valid() && a.generation == b.generation && a.update == b.update && a.thread == b.thread &&
           a.cockpit == b.cockpit;
}
struct Start {
...
struct Slot {
    ...
    Start starts[phase_count]{};
};
struct Metric {
    std::uint64_t calls = 0, ticks = 0, maximum = 0, failures = 0;
};
struct Sample {
    Token token{};
...
struct State {
    ...
    void begin(const Token& token, unsigned phase, std::uint32_t frame, std::uint64_t stamp) {
        ...
        for (auto& start : found->starts)
            if (start.stamp && !same(start.token, token)) {
                ++window.abandoned;
                start = {};
            }
    ...
    void end(const Token& token, unsigned phase, std::uint32_t frame, std::uint64_t stamp, std::uint32_t result,
             std::uint64_t slow_threshold) {
        ...
        if (!found || !found->starts[phase].stamp) {
            ++window.unmatched;
            return;
        }
        auto start = found->starts[phase];
        ...
        ++window.slow;
        if (window.first_used < 4)
            window.first[window.first_used++] = sample;
        else {
