...
struct Machine {
    ...
    std::uint32_t armed_by = 0;         // the arming message
...
// Observer step (the WH_CALLWNDPROC hook, window thread). True when this message armed an idle machine.
inline bool observe(Machine& m, std::uint32_t message, std::uint32_t wparam) {
    if (!arming_message(message, wparam) || m.state != State::idle) return false;
