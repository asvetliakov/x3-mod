...
inline bool perf() noexcept {
    return env_flag(L"X3M_PERF");
}
...
inline bool draw_trace() noexcept {
    return env_flag(L"X3M_DRAW_TRACE");
}
...
inline void init() noexcept {
    cached_debug = debug();
    cached_perf = perf();
    cached_draw_trace = draw_trace();
}
// X3M_TELEMETRY=1 or either group: the counters, the 1 Hz summaries, the
// loading-trace hook set and the family block's gate.
inline bool telemetry() noexcept {
    return env_flag(L"X3M_TELEMETRY") || perf() || debug();
}
// A boolean switch of a group (OR semantics: an explicit "0" does not defeat
// the group).
inline bool debug_flag(const wchar_t* name) noexcept {
    return env_flag(name) || debug();
}
inline bool perf_flag(const wchar_t* name) noexcept {
    return env_flag(name) || perf();
}
inline bool draw_trace_flag(const wchar_t* name) noexcept {
    return env_flag(name) || draw_trace();
}
