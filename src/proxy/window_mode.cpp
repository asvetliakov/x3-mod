...
namespace {
bool enabled_ = false;
core::Rect rect_of(const RECT& r) {
    core::Rect c;
    c.left = r.left;
    c.top = r.top;
    c.right = r.right;
...
Result apply(const char* phase, HWND focus_window, HWND device_window, bool windowed, UINT backbuffer_width,
    ...
    in.backbuffer_width = backbuffer_width;
    in.backbuffer_height = backbuffer_height;
    DWORD thread = 0;
    if (in.window_valid) {
        ...
        if (in.monitor_known) {
            in.monitor = rect_of(mi.rcMonitor);
            in.work = rect_of(mi.rcWork);
        }
    }
    ...
    if (result.decision.action == core::Action::move) {
        ...
        result.set_result = SetWindowPos(hwnd, HWND_TOP, in.monitor.left, in.monitor.top, int(core::width(in.monitor)),
                                         int(core::height(in.monitor)),
                                         SWP_NOZORDER | SWP_NOACTIVATE | SWP_NOOWNERZORDER);
        if (!result.set_result) {
            result.set_error = GetLastError();
            action = "move_failed";
        }
        RECT wr{};
    ...
    log("window_mode phase=%s hwnd=%p style=%08lx exstyle=%08lx before=%ld,%ld,%ld,%ld after=%ld,%ld,%ld,%ld monitor=%ld,%ld,%ld,%ld work=%ld,%ld,%ld,%ld "
        ...
        long(in.work.left), long(in.work.top), long(in.work.right), long(in.work.bottom), backbuffer_width,
        backbuffer_height, unsigned(windowed), unsigned(in.single_window), unsigned(in.top_level),
        static_cast<unsigned long>(thread), static_cast<unsigned long>(GetCurrentThreadId()), action,
        result.decision.reason, int(result.set_result), static_cast<unsigned long>(result.set_error));
