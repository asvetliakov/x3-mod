...
bool lifetime_current(const object_lifetime::Snapshot& before, std::uintptr_t registry,
    ...
    return before.known &&
           object_lifetime::current(registry, key.node, key.node_handle, key.camera, key.camera_handle, &after) &&
...
MotionCapture::MotionCapture() noexcept
    : history_(ownership::geometry_leases_per_frame, renderer::MotionHistoryPurpose::DiagnosticStorageCorrespondence) {}
MotionCapture::~MotionCapture() {
    release_geometry();
}
bool MotionCapture::same(const Domain& a, const Domain& b) noexcept {
    return std::tie(a.device, a.generation, a.color, a.depth, a.finite, a.observer, a.load, a.registry, a.camera,
                    a.width, a.height) == std::tie(b.device, b.generation, b.color, b.depth, b.finite, b.observer,
                                                   b.load, b.registry, b.camera, b.width, b.height);
}
...
void MotionCapture::begin_frame(IDirect3DDevice9* application, std::uint64_t frame, bool requested) noexcept {
    ...
    release_geometry();
    records_.clear();
    sealed_ = produced_ = confirmed_ = false;
    frame_ = frame;
    if (!requested || !frame || ownership::begin_geometry_frame(application, &geometry_) != S_OK) {
        invalidate();
        return;
    }
    active_ = true;
...
void MotionCapture::observe(const DrawInput& input, std::uintptr_t registry,
    ...
    if (records_.size() >= ownership::geometry_leases_per_frame) {
        invalidate();
        return;
    }
    try {
        records_.push_back({input, registry, lifetime});
    } catch (...) {
        invalidate();
    }
}
MotionCaptureDiagnostics MotionCapture::before_clear(IDirect3DDevice9* application,
                                                     const renderer::Selection& selection, DiagnosticConsumer consumer,
    ...
    stats.observations = records_.size();
    ...
    try {
        ...
        if (ownership::get_execution_view(application, &execution) != S_OK || !execution.known ||
            execution.stateblock_recording || !execution.queries_idle)
            return fail(S_FALSE);
        ...
        if (FAILED(application->GetDepthStencilSurface(&app_depth.p)) ||
            ...
            resource_id(app_depth.p) != selection.depth.identity ||
            resource_id(app_color.p) != selection.color.identity ||
            FAILED(native->GetDepthStencilSurface(&native_depth.p)))
        ...
        for (std::size_t n = 0; n < records_.size(); ++n) {
            auto& record = records_[n];
            auto& in = record.input;
            auto& observation = in.observation;
            ...
            bool eligible = !in.blockers && observation.proofs == renderer::AllRigidProofs &&
                            ...
                            in.color_target == selection.color.identity &&
                            in.depth_target == selection.depth.identity && in.width == selection.color.width &&
                            in.height == selection.color.height && in.geometry_lease.value &&
                            lifetime_current(record.lifetime, record.registry, key);
            if (eligible)
                eligible = ownership::inspect_geometry_lease(geometry_, in.geometry_lease, &views[n]) == S_OK &&
            ...
            const Domain candidate{selection.device,
                                   selection.generation,
                                   selection.color.identity,
                                   selection.depth.identity,
                                   in.finite_positions.generation,
                                   life.observer_epoch,
                                   life.load_epoch,
                                   life.registry_epoch,
                                   key.camera_lifetime,
        ...
        for (const auto& record : records_)
            if (!history_.observe(record.input.observation)) return fail(E_FAIL);
        if (!history_.seal()) return fail(E_FAIL);
        sealed_ = true;
        std::vector<renderer::RigidMotionDraw> draws;
        draws.reserve(records_.size());
        for (std::size_t n = 0; n < records_.size(); ++n) {
            const auto& in = records_[n].input;
            const auto& key = in.observation.key;
            ...
            draw.source_program = in.replay_source;
            draw.finite_positions_attested = true;
            draw.vertices = views[n].vertex_buffer;
            draw.indices = views[n].index_buffer;
            draw.stream_offset = key.stream_offset;
            draw.stride = key.stride;
            draw.position_offset = key.position_offset;
            draw.position_type = static_cast<D3DDECLTYPE>(key.position_type);
            draw.topology = static_cast<D3DPRIMITIVETYPE>(key.topology);
            draw.primitive_count = key.primitives;
            draw.start_vertex = key.indexed ? 0 : key.first;
            draw.minimum_vertex = key.min_vertex;
            draw.vertex_count = key.vertex_count;
            draw.start_index = key.indexed ? key.first : 0;
            draw.base_vertex = key.base_vertex;
            draw.cull = in.cull;
            draw.correspondence_attested = true;
        ...
        hr = pass.initialize(native);
        if (FAILED(hr)) return fail(hr);
        renderer::RigidMotionInputs inputs{};
        inputs.motion = motion.p;
        inputs.scene_depth = native_depth.p;
        inputs.width = current.width;
        inputs.height = current.height;
        inputs.draws = draws.data();
        inputs.draw_count = draws.size();
        inputs.scene_depth_current = true;
        inputs.caller_scene_open = execution.scene_open;
        inputs.caller_stateblock_recording = execution.stateblock_recording;
        inputs.caller_queries_idle = execution.queries_idle;
        renderer::RigidMotionOutput output{};
        hr = pass.run(inputs, &output);
        stats.operation = hr;
        stats.restoration = pass.diagnostics().restoration;
        stats.completed = pass.diagnostics().completed_draws;
        if (hr != S_OK) {
            ownership::invalidate_execution_state(application);
            return fail(hr);
        }
