...
constexpr float sun_agreement_cos = .999657f; // 1.5 degrees: twice the measured 0.753-degree per-node spread, far below
                                              // any other light
...
constexpr const char* sun_verdict_name(SunVerdict v) noexcept {
    switch (v) {
    case SunVerdict::Sampled: return "sampled";
    case SunVerdict::Retained: return "retained";
    case SunVerdict::Relatched: return "relatched";
    case SunVerdict::Changing: return "changing";
    default: return "none";
    }
}
constexpr bool sun_verdict_usable(SunVerdict v) noexcept {
    return v == SunVerdict::Sampled || v == SunVerdict::Retained;
}
struct SunLatch {
    ...
    struct Counts {
        std::uint32_t samples = 0, agree = 0, disagree = 0, invalid = 0, no_register = 0, unlatched = 0;
    } frame{};
    ...
    static bool unit(const float v[4]) noexcept {
        ...
        for (unsigned i = 0; i < 3; ++i) {
            if (!(v[i] == v[i]) || v[i] > 2.f || v[i] < -2.f) return false;
            n += v[i] * v[i];
        }
        constexpr float lo = (1.f - sun_unit_tolerance) * (1.f - sun_unit_tolerance),
                        hi = (1.f + sun_unit_tolerance) * (1.f + sun_unit_tolerance);
    ...
    bool sample(const float v[4], int reg, std::uint64_t program = 0) noexcept {
        ...
        if (!valid) {
            if (pending_valid && agrees(pending, v)) { // the second draw agrees: the first one's value is the sun
                ...
                valid = true;
                source_register = pending_register;
                source_program = pending_program;
                candidate_frames = 0;
                pending_valid = false;
                ++frame.agree;
                return true;
        ...
        if (!candidate_this_frame) {
            ...
            if (!candidate_frames || !agrees(candidate, v)) {
                for (unsigned i = 0; i < 4; ++i) candidate[i] = v[i];
                candidate_frames = 0;
            }
    ...
    SunVerdict end_frame() noexcept {
        ...
        if (++candidate_frames >= sun_relatch_frames) {
            ...
            source_register = candidate_register;
            source_program = candidate_program;
            candidate_frames = 0;
