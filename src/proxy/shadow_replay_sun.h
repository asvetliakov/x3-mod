...
// the value per mesh part as normalize(light - node) with the sun about 1.6e9
// units away (docs/reverse-engineering/camera-and-lights.md, "Directional
// lights: source, space and count"), so samples of one sector spread by up to
// 0.6 degrees. This latch turns them into ONE sun per sector, shared by the
// bounds test, every cascade's replay and the apply:
//  * a sample counts only when it is unit length within 1e-3 with w = 0;
//  * the first valid sample ever becomes the latched sun, and it stays
//    exactly that value while samples agree within sun_agreement_cos (1.5
//    degrees): the basis never follows whichever draw came first (no swim);
...
//    everything retained);
//  * before any valid sample there is no sun (None): the frame is refused.
...
constexpr float sun_agreement_cos = .999657f; // 1.5 degrees: above the 0.6-degree per-node spread, far below any other light
...
struct SunLatch {
    ...
    struct Counts { std::uint32_t samples = 0, agree = 0, disagree = 0, invalid = 0, no_register = 0; } frame{};
    ...
    bool sample(const float v[4], int reg, std::uint64_t program = 0) noexcept {
        ...
        if (!valid) {
            for (unsigned i = 0; i < 4; ++i) sun[i] = v[i];
            valid = true; source_register = reg; source_program = program; candidate_frames = 0; ++frame.agree;
            return true;
        }
