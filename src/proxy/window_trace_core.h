...
constexpr std::uint32_t wm_move = 0x0003, wm_size = 0x0005, wm_activate = 0x0006, wm_setfocus = 0x0007,
                        wm_killfocus = 0x0008, wm_activateapp = 0x001c, wm_cancelmode = 0x001f, wm_setcursor = 0x0020,
                        wm_mouseactivate = 0x0021, wm_windowposchanged = 0x0047, wm_displaychange = 0x007e,
                        wm_ncmousemove = 0x00a0, wm_syscommand = 0x0112, wm_mousemove = 0x0200,
...
inline bool transition_message(std::uint32_t m) {
    ...
    case wm_displaychange: return true;
...
struct SetCursorSummary {
    bool any = false;
    std::uint32_t last_row_ms = 0, result = 0, hit = 0, trigger = 0, suppressed = 0;
};
inline bool setcursor_row(SetCursorSummary& s, std::uint32_t now_ms, std::uint32_t result, std::uint32_t hit,
                          std::uint32_t trigger) {
    const bool row = !s.any || now_ms - s.last_row_ms >= setcursor_interval_ms || result != s.result || hit != s.hit ||
                     trigger != s.trigger;
    if (row) {
        s.any = true;
        s.last_row_ms = now_ms;
        s.result = result;
        s.hit = hit;
        s.trigger = trigger;
    } else
...
struct CursorCounts {
    std::uint32_t set = 0, pos = 0, dropped = 0;
};
}
