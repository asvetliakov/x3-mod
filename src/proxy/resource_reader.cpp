...
constexpr uintptr_t fread_va = 0x00512213, fseek_va = 0x00510343, ftell_va = 0x005128e6, malloc_va = 0x005112c4,
                    free_va = 0x0050e1b0;
...
constexpr uintptr_t pool_fopen_site = 0x004e87ff, pool_fclose_site = 0x004e9392, fopen_va = 0x00512a18,
                    fclose_va = 0x0050f8d4;
...
const char* mode_name(Mode m) {
    return m == Mode::Fast ? "fast" : m == Mode::Verify ? "verify" : "native";
}
void verify_sink(const VerifyEvent& e) {
    ...
    log("resource_reader verify equal=%u size=%lu original_size=%lu mismatches=%lu first=%lu original_null=%u globals_ok=%u counters_ok=%u cursor_ok=%u position_ok=%u cursor=%ld expected_cursor=%ld position=%ld expected_position=%ld catalogue=%u scrambled=%u our_us=%.3f original_us=%.3f",
        unsigned(e.equal), static_cast<unsigned long>(e.size), static_cast<unsigned long>(e.original_size),
        static_cast<unsigned long>(e.mismatches), static_cast<unsigned long>(e.first_mismatch),
        unsigned(e.original_null), unsigned(e.globals_ok), unsigned(e.counters_ok), unsigned(e.cursor_ok),
        unsigned(e.position_ok), long(e.cursor), long(e.expected_cursor), e.position, e.expected_position,
        unsigned(e.catalogue), unsigned(e.scrambled), double(e.our_ticks) * 1e6 / double(frequency),
        double(e.original_ticks) * 1e6 / double(frequency));
}
// Reader entry stub:
...
void* emit_reader_stub(void*** next_out) {
    ...
    e.byte(0x50);
    e.byte(0xe8);
    e.rel32(reinterpret_cast<const void*>(&x3m_resource_read_entry));
    e.byte(0x83);
    e.byte(0xc4);
    e.byte(0x04);
    e.byte(0x85);
...
bool install_stub(engine_patch::Site& target, const Environment& env, Mode mode) {
    void** next = nullptr;
    void* stub = emit_reader_stub(&next);
    if (!stub || !next) {
        status_ = "stub_failed";
        return false;
    }
    if (!engine_patch::store_pointer(next, *target.entry)) {
        status_ = "chain_write_failed";
        return false;
    }
    core_bind(env, mode, next, &verify_sink);
    if (!engine_patch::push_front(target, stub)) {
        status_ = "chain_failed";
        return false;
    }
    next_slot = next;
    site = &target;
...
Environment game_environment() {
    Environment env;
    env.fread = reinterpret_cast<decltype(env.fread)>(fread_va);
    env.fseek = reinterpret_cast<decltype(env.fseek)>(fseek_va);
    env.ftell = reinterpret_cast<decltype(env.ftell)>(ftell_va);
    env.malloc = reinterpret_cast<decltype(env.malloc)>(malloc_va);
...
bool pool_requested() {
    return pool_requested_;
}
bool pool_installed() {
    return pool_installed_;
}
bool initialize() {
    ...
    if (length && length < 16) {
        if (!lstrcmpiW(setting, L"fast"))
            mode_ = Mode::Fast;
        else if (!lstrcmpiW(setting, L"verify"))
            mode_ = Mode::Verify;
    }
    wchar_t pool_setting[8]{};
    pool_requested_ = x3m::config::get(L"X3M_DAT_HANDLES", pool_setting, 8) == 1 && pool_setting[0] == L'1';
    if (mode_ == Mode::Native && !pool_requested_) {
        status_ = "disabled";
        SetLastError(error);
        return false;
    }
    if (!object_trace::executable_verified()) {
        status_ = "executable_mismatch";
        log("resource_reader mode=%s installed=0 status=%s", mode_name(mode_), status_);
        SetLastError(error);
        return false;
    }
    if (mode_ != Mode::Native) {
        ...
        if (!callees_ok)
            status_ = "crt_bytes_mismatch";
        else if (!env.inflateInit2_ || !env.inflate || !env.inflateEnd)
            status_ = "zlib_exports_missing";
        else {
            engine_patch::Site* target = loading_probes::resource_read_site();
            if (!target) {
                engine_patch::SiteSpec spec{"resource_read", reader_va, {}, 6, 0, 0};
                std::memcpy(spec.expected, reader_bytes, 6);
                if (engine_patch::claim(own_site, spec))
                    target = &own_site;
                else
                    status_ = own_site.status;
            }
        ...
        log("resource_reader mode=%s installed=%u status=%s site=0x%08lx chained_after_probe=%u scope=whole_extent_fread,word_xor,single_inflate,game_malloc,no_memset fallback=original arena_used=%u",
            mode_name(mode_), unsigned(installed_), status_, static_cast<unsigned long>(reader_va),
            unsigned(loading_probes::resource_read_site() != nullptr), engine_patch::arena_used());
    }
    if (pool_requested_) {
        PoolEnvironment pool;
        pool.fopen = reinterpret_cast<decltype(pool.fopen)>(fopen_va);
        pool.fclose = reinterpret_cast<decltype(pool.fclose)>(fclose_va);
        pool_bind(pool);
        const bool open_ok = engine_patch::claim_call(pool_open_site, pool_fopen_site, fopen_va,
                                                      reinterpret_cast<void*>(&x3m_pool_fopen));
        const bool close_ok = open_ok && engine_patch::claim_call(pool_close_site, pool_fclose_site, fclose_va,
                                                                  reinterpret_cast<void*>(&x3m_pool_fclose));
        ...
        log("dat_handle_pool requested=1 installed=%u open_site=0x%08lx open_status=%s close_site=0x%08lx close_status=%s entries=32 scope=catalogue_fopen_reuse",
            unsigned(pool_installed_), static_cast<unsigned long>(pool_fopen_site), pool_open_site.status,
            static_cast<unsigned long>(pool_fclose_site), pool_close_site.status);
    }
    SetLastError(error);
    ...
    if (installed_) {
        ...
        if (s.calls != last_report_calls) {
            ...
            log("resource_reader_metric cumulative=1 mode=%s calls=%llu handled=%llu fallbacks=%llu bytes_in=%llu bytes_out=%llu fast_us=%.3f fast_max_us=%.3f original_us=%.3f verify_files=%llu verify_equal=%llu verify_mismatched=%llu verify_original_null=%llu catalogue=%llu scrambled=%llu cursor_short=%llu "
                ...
                mode_name(mode_), s.calls, s.handled, s.fallbacks, s.bytes_in, s.bytes_out,
                double(s.ticks) * 1e6 / double(frequency), double(s.max_ticks) * 1e6 / double(frequency),
                double(s.original_ticks) * 1e6 / double(frequency), s.verify_files, s.verify_equal, s.verify_mismatched,
                s.verify_original_null, s.catalogue, s.scrambled, s.cursor_short,
                double(s.read_ticks) * 1e6 / double(frequency), double(s.scan_ticks) * 1e6 / double(frequency),
                double(s.alloc_ticks) * 1e6 / double(frequency), double(s.inflate_ticks) * 1e6 / double(frequency),
                s.reasons[1], s.reasons[2], s.reasons[3], s.reasons[4], s.reasons[5], s.reasons[6], s.reasons[7],
                s.reasons[8], s.reasons[9], s.reasons[10], s.reasons[11], s.reasons[12], s.reasons[13], s.reasons[14],
                s.reasons[15], s.reasons[16], s.reasons[17]);
        }
    }
    if (pool_installed_) {
        ...
        if (p.opens != last_pool_opens) {
            ...
            log("dat_handle_pool_metric cumulative=1 opens=%llu reused=%llu real_opens=%llu closes=%llu kept=%llu real_closes=%llu errors=%llu full=%llu held=%u",
                p.opens, p.reused, p.real_opens, p.closes, p.kept, p.real_closes, p.errors, p.full, p.held);
        }
    }
    SetLastError(error);
}
void shutdown() {
    report();
    if (pool_installed_) {
        engine_patch::restore_call(pool_open_site);
        engine_patch::restore_call(pool_close_site);
        pool_drain();
        pool_installed_ = false;
    }
    if (site == &own_site && own_site.patched_in) engine_patch::restore(own_site);
    installed_ = false;
}
#ifdef X3M_RESOURCE_READER_FIXTURE
void fixture_bind(const Environment& env, Mode mode) {
    core_bind(env, mode, next_slot, &verify_sink);
    mode_ = mode;
    initialized_ = true;
}
...
bool fixture_install(uintptr_t site_address, const unsigned char* expected, unsigned length) {
    ...
    QueryPerformanceFrequency(&f);
    frequency = f.QuadPart > 0 ? uint64_t(f.QuadPart) : 1;
    engine_patch::SiteSpec spec{"resource_read", site_address, {}, length, 0, 0};
    std::memcpy(spec.expected, expected, length);
    engine_patch::Site* target = loading_probes::resource_read_site();
    if (!target) {
        if (!engine_patch::claim(own_site, spec)) {
            status_ = own_site.status;
    ...
    Environment env;
    return install_stub(*target, env, mode_);
}
