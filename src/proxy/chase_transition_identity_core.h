...
struct Identity {
    enum : std::uint32_t { vm_bit=1,native_bit=2,player_bit=4,controller_bit=8,
        monitor_bit=16,mode_bit=32,ref_bit=64,warp_bit=128,killed_bit=256,
        player_live_bit=512,controller_live_bit=1024,native_live_bit=2048 };
...
template<class Reader> struct IdentityReader {
    ...
    bool descriptor(std::uint32_t p,std::uint32_t (&row)[14]){
        return p>=classes && (p-classes)%0x38==0 && (p-classes)/0x38<count &&
            field(p,0,row) && row[0]<=INT32_MAX && row[2]==p && row[7]<=65536;
    }
    bool static_class(std::uint32_t id,std::uint32_t& p){
        ...
        for(unsigned step=0;lo<hi && step<13;++step){
            const auto mid=lo+(hi-lo)/2;std::uint32_t found=0;
            const auto at=classes+mid*0x38;
            if(!field(at,0,found)||found>INT32_MAX)return false;
            if(found==id){p=at;return true;}
            if(found<id)lo=mid+1;else hi=mid;
        }
        return false;
    }
    bool hash(std::uint32_t table,std::uint32_t key,std::uint32_t& value){
        std::uint32_t bucket[2]{},link=0,found=0;bool matched=false;
        if(!pointer(table)||!field(table,0,bucket)||!pointer(bucket[0])||!bucket[1]||
            bucket[1]>65536||(bucket[1]&(bucket[1]-1))||
            !field(bucket[0],4*(key&(bucket[1]-1)),link))return false;
        for(unsigned i=0;link && i<32;++i){
            std::uint32_t row[3]{};
            if(!pointer(link)||!field(link,0,row))return false;
            if(row[1]==key){if(matched)return false;matched=true;found=row[2];}
            link=row[0];
        }
    ...
    bool context(std::uint32_t id,std::uint32_t& p,std::uint32_t (&desc)[14]){
        if(id<=INT32_MAX){if(!static_class(id,p))return false;}
        else {std::uint32_t table=0;if(!field(vm,0x12d0,table)||!hash(table,~id,p))return false;}
        std::uint32_t head[3]{};
        return pointer(p)&&field(p,0,head)&&head[0]==id&&descriptor(head[2],desc);
    }
    bool cell(std::uint32_t context,const std::uint32_t (&desc)[14],unsigned index,
        ...
        tag=raw[0];std::memcpy(&value,raw+1,4);
    ...
    void capture(std::uint32_t ship,std::uint32_t monitor_context,Identity& out){
        ...
        const bool rooted=root();out.vm=vm;mark(I::vm_bit,rooted);
        if(!rooted){out.refused|=I::player_bit|I::controller_bit|I::warp_bit|I::killed_bit;
