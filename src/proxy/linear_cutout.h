...
inline constexpr std::uint64_t pair_hashes[] = {0x53a0a641107ed76cull, 0x63f96eba9eea7880ull, 0x4944d81dfe531b37ull,
                                                0x5e0a10fe752b6140ull};
...
constexpr bool state(const std::array<std::uint32_t, 8>& actual) noexcept {
    for (unsigned i = 0; i < values.size(); ++i)
        if (actual[i] != values[i]) return false;
    return true;
}
...
constexpr bool missed(bool candidate, bool submitted, bool success, bool routed, bool test_known, std::uint32_t test,
                      bool color_known, std::uint32_t color, bool alpha_known, std::uint32_t alpha, bool z_known,
                      std::uint32_t z, bool zfunc_known, std::uint32_t zfunc, bool source_over = false) noexcept {
    return candidate && submitted && success && !routed && (!test_known || test != 0) &&
           (!color_known || (color & 7u) != 0) && (!alpha_known || alpha != 1) &&
           (!z_known || !z || !zfunc_known || zfunc != 1) && !source_over;
}
// The observed source-over triple, all three states known.
constexpr bool source_over(bool blend_known, std::uint32_t blend, bool src_known, std::uint32_t src, bool dst_known,
                           std::uint32_t dst) noexcept {
    return blend_known && blend != 0 && src_known && src == 5 && dst_known && dst == 6; // D3DBLEND_SRCALPHA /
                                                                                        // D3DBLEND_INVSRCALPHA
}
// A valid supplemental fade mask cannot repair missing cutout correspondence.
constexpr bool unavailable(bool missed_cutout, bool composition_required, bool composition_ready) noexcept {
