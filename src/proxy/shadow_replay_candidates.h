...
constexpr unsigned record_capacity = 1024; // inline storage (fixed arrays in MotionOutput, no allocation); the
                                           // per-frame cap is at most this unless a cascade set asks for more records
constexpr unsigned record_capacity_max = 4096; // X3M_SHADOW_CASCADE_RECORDS: the largest per-cascade record capacity;
                                               // storage beyond record_capacity is allocated once at attach
constexpr unsigned default_cap = 512;          // the counter's per-frame record cap without cascades (the former
                                      // X3M_SHADOW_REPLAY_CAP default, removed 2026-09-25); a cascade set brings its
                                      // own caps
constexpr unsigned cascade_capacity = 5;  // = renderer::shadow_cascade_max (docs/architecture/shadow-cascades.md); this
                                          // header stays D3D- and renderer-free
...
constexpr std::uint64_t extent_read_bytes_per_frame = 1u << 20; // soft byte budget of those reads: the read that
                                                                // crosses it is the frame's last
constexpr unsigned extent_read_attempts = 8; // scene ends a range may be found locked or fail to Lock before it is
                                             // given up
...
inline std::uint64_t caster_key(std::uint64_t serial, std::uint64_t vb, std::uint32_t first, std::int32_t base_vertex,
...
struct FrameCounts {
    ...
    std::uint32_t bounds = 0, origin = 0, fallback = 0; // slice0 = bounds (extent meets the map box) + fallback (no
                                                        // extent yet: origin rule); origin: the origin rule alone
    std::uint32_t capped = 0, reads = 0; // managed candidates beyond the per-frame cap; extent reads performed at the
                                         // scene end
    ...
    std::uint32_t large_admitted[cascade_capacity]{}; // ... and moving draws admitted to cascade i by their extent
                                                      // (X3M_SHADOW_CASCADE_LARGE_MIN)
    ...
    std::uint32_t class_store = 0, class_ring = 0; // draws classified by the retention store's verdict; by the ring's
                                                   // anchor
    std::uint32_t class_miss[cascade_capacity]{}; // draws refused from cascade i with no anchor at all (first sighting,
                                                  // evicted, no serial or rows): the ring's limit, per cascade
...
struct Record {
    ...
    std::uint64_t vb_generation = 0, ib_generation = 0; // BufferLockView::generation at the draw (changes before every
                                                        // Reset)
...
constexpr const char* verdict_source_name(std::uint8_t v) noexcept {
    return v == 1 ? "bounds" : v == 2 ? "retained" : v == 3 ? "inflated" : "origin";
}
struct Witness {
...
inline BufferVerdict compare(const ownership::BufferLockObservation& at_draw,
    ...
    v.witness.flags = at_end.last_flags;
    v.witness.offset = at_end.last_offset;
    v.witness.size = at_end.last_size;
...
constexpr float importance_hysteresis = .8f; // a caster kept last frame stays kept while its size >= this x the
                                             // cascade's cutoff
...
constexpr std::uint32_t flip_stale_frames = 2; // an entry unseen this long is reusable (its bits have already been
                                               // reported leaving)
struct FlipTable {
    FlipEntry* entries = nullptr;
    unsigned slots = 0;
    ...
    void update(const Record* records, unsigned count, unsigned cascades, unsigned active, std::uint64_t frame_number,
        ...
        for (unsigned r = 0; r < count; ++r) {
            ...
            FlipEntry* found = nullptr;
            FlipEntry* reusable = nullptr;
            for (unsigned probe = 0, i = unsigned(key % slots); probe < flip_probe_limit;
                 ++probe, i = i + 1 == slots ? 0 : i + 1) {
                ...
                if (!reusable && frame - e.stamp > flip_stale_frames && frame - e.pending_stamp > flip_stale_frames)
                    reusable = &e;
            }
            if (!found && !reusable) {
                ++untracked;
                continue;
            } // chain full: this caster is not tracked this frame
            if (!found) {
                *reusable = FlipEntry{};
                reusable->key = key;
                found = reusable;
        ...
        for (unsigned i = 0; i < slots; ++i) {
            ...
            const bool fresh = e.stamp + 1 == frame; // the entry describes the previous frame: its mask and flips carry
                                                     // over
...
struct Frame {
    ...
    KeptEntry* kept_last = nullptr;
    unsigned kept_slots = 0; // the owner's table (importance order on); persists across frames
    void attach_kept(KeptEntry* table, unsigned slots) noexcept {
        kept_last = table;
        kept_slots = slots;
        if (table)
            for (unsigned i = 0; i < slots; ++i) table[i] = KeptEntry{};
    }
    std::uint8_t kept_mask(std::uint64_t key) const noexcept {
        if (!kept_last || !kept_slots) return 0;
        for (unsigned probe = 0, i = unsigned(key % kept_slots); probe < kept_slots;
             ++probe, i = i + 1 == kept_slots ? 0 : i + 1) {
    ...
    void remember_kept() noexcept {
        ...
        for (unsigned r = 0; r < record_count; ++r) {
            ...
            for (unsigned probe = 0, i = unsigned(rec.key % kept_slots); probe < kept_slots;
                 ++probe, i = i + 1 == kept_slots ? 0 : i + 1) {
                if (!kept_last[i].key || kept_last[i].key == rec.key) {
                    kept_last[i].key = rec.key;
                    kept_last[i].mask |= rec.cascades;
                    break;
                }
            }
        }
    ...
    bool draw(bool zwrite, bool admitted, bool by_bounds, bool origin_rule, bool excluded, bool shadow_ok, PoolClass vb,
              PoolClass ib, bool indexed, unsigned cap, std::uint8_t* cascade_mask = nullptr,
              const unsigned* cascade_caps = nullptr) noexcept {
        ++counts.routed;
        ...
        if (!shadow_ok) {
            ++counts.shadow_mismatch;
            return false;
        }
        const PoolClass second = indexed ? ib : PoolClass::Managed;
        if (vb == PoolClass::Dynamic || second == PoolClass::Dynamic) {
            ++counts.dynamic;
            return false;
        }
        if (vb == PoolClass::Unknown || second == PoolClass::Unknown) {
            ++counts.unknown;
            return false;
        }
        if (vb != PoolClass::Managed || second != PoolClass::Managed) {
            ++counts.default_pool;
            return false;
        }
        ++counts.managed;
        ...
        if (cascade_mask && cascade_caps) {
            ...
            for (unsigned i = 0; i < cascade_capacity; ++i) {
                ...
                if (counts.cascade[i] >= cascade_caps[i])
                    ++counts.cascade_capped[i];
                else
                    kept |= std::uint8_t(1u << i);
            }
            *cascade_mask = kept;
            if (!kept) {
                ++counts.capped;
    ...
    void count_cascades(std::uint8_t cascade_mask) noexcept {
        for (unsigned i = 0; i < cascade_capacity; ++i)
            if (cascade_mask & (1u << i)) ++counts.cascade[i];
    }
    ...
    void select_cascades(const unsigned* caps, unsigned cascades, std::uint16_t* scratch) noexcept {
        ...
        for (unsigned k = 0; k < cascades && k < cascade_capacity; ++k) {
            ...
            for (unsigned i = 0; i < record_count; ++i)
                if (records[i].cascades & (1u << k)) scratch[n++] = std::uint16_t(i);
            ...
            if (kept_last && kept_slots) {
                ...
                for (unsigned q = 1; q < keep; ++q)
                    if (r[scratch[q]].size < boundary) boundary = r[scratch[q]].size;
                const float cutoff = boundary * importance_hysteresis;
                const std::uint8_t bit = std::uint8_t(1u << k);
                const auto sticky = [this, r, cutoff, bit](std::uint16_t a) noexcept {
                    return r[a].size >= cutoff && (kept_mask(r[a].key) & bit) != 0;
                };
                bool any = false;
                for (unsigned q = keep; q < n && !any; ++q)
                    any = sticky(scratch[q]); // a dropped caster that was kept: re-rank with the priority
                if (any)
...
inline AlphaCaster alpha_caster(std::uint32_t func, std::uint32_t ref, float& threshold) noexcept {
    ...
    case 7:
        if (!code) return AlphaCaster::Opaque;
        threshold = float(code) / 255.f;
        return AlphaCaster::Tested;                                              // GREATEREQUAL
...
struct AlphaCasterCounts {
    std::uint32_t seen = 0, tested = 0, opaque = 0; // reached the decision; admitted with the alpha program; admitted
                                                    // as opaque
    std::uint32_t state = 0, function = 0, uv = 0, texture = 0, pool = 0; // refused: state read failed; the comparison;
                                                                          // no stream-0 TEXCOORD0; no 2D stage-0
                                                                          // texture; texture not D3DPOOL_MANAGED
};
...
struct ExtentEntry {
    ExtentKey key{};
    float lo[3]{}, hi[3]{};
    ExtentState state = ExtentState::Empty;
...
struct ExtentCache {
    ...
    const ExtentEntry* set_of(const ExtentKey& key) const noexcept {
        return entries + std::size_t(key.hash() % sets) * ways;
    }
    ...
    const ExtentEntry* find(const ExtentKey& key, const ExtentEntry** stale = nullptr) noexcept {
        ...
        for (unsigned w = 0; w < ways; ++w) {
            ...
            if (stale && e.state == ExtentState::Known) {
                if (e.pending_revision != key.revision) {
                    e.pending_revision = key.revision;
                    e.attempts = 0;
                    e.stale_since = stamp;
                }
                *stale = &e;
    ...
    ExtentEntry* way_for(const ExtentKey& key) noexcept {
        ExtentEntry* set = set_of(key);
        ExtentEntry* empty = nullptr;
        ExtentEntry* oldest = nullptr;
        for (unsigned w = 0; w < ways; ++w) {
            ExtentEntry& e = set[w];
            if (e.state == ExtentState::Empty) {
                if (!empty) empty = &e;
                continue;
            }
    ...
    void retry(const ExtentKey& key) noexcept {
        ...
        const unsigned attempts = same && e->state == ExtentState::Retry && e->key.revision == key.revision
                                      ? e->attempts + 1u
                                      : 1u;
    ...
    void clear() noexcept {
        for (auto& e : entries) {
            e.state = ExtentState::Empty;
            e.used = 0;
            e.stale_since = 0;
            e.pending_revision = 0;
        }
        stamp = 1;
        refused = refused_frame = 0;
    }
};
...
inline std::uint32_t vertices_of(std::uint32_t topology, std::uint32_t primitives) noexcept {
    ...
    case 5:
    case 6: return primitives ? primitives + 2 : 0; // TRIANGLESTRIP, TRIANGLEFAN
    default: return 0;
    }
}
// Position element decoders (documented D3DDECLTYPE values): FLOAT3 2, FLOAT4 3, FLOAT16_4 16.
inline bool extent_type_supported(std::uint32_t type) noexcept {
    return type == 2 || type == 3 || type == 16;
}
inline std::uint32_t extent_type_bytes(std::uint32_t type) noexcept {
...
inline bool extent_of(const unsigned char* bytes, std::uint32_t stride, std::uint32_t position_offset,
                      std::uint32_t type, std::uint32_t count, float lo[3], float hi[3]) noexcept {
    if (!bytes || !count || !extent_type_supported(type)) return false;
    lo[0] = lo[1] = lo[2] = 3.4028235e38f;
    hi[0] = hi[1] = hi[2] = -3.4028235e38f;
    for (std::uint32_t v = 0; v < count; ++v) {
        ...
        if (type == 16) {
            std::uint16_t h[3];
            std::memcpy(h, p, 6);
            for (unsigned i = 0; i < 3; ++i) c[i] = half_to_float(h[i]);
        } else
            std::memcpy(c, p, 12);
        for (unsigned i = 0; i < 3; ++i) {
