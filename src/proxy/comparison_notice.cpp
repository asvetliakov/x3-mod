...
const unsigned char* glyph(char c) noexcept {
    ...
    // clang-format on
    static constexpr unsigned char digits[][7] = {{14, 17, 19, 21, 25, 17, 14}, {4, 12, 4, 4, 4, 4, 14},
                                                  {14, 17, 1, 2, 4, 8, 31},     {30, 1, 1, 14, 1, 1, 30},
                                                  {2, 6, 10, 18, 31, 2, 2},     {31, 16, 16, 30, 1, 1, 30},
                                                  {14, 16, 16, 30, 17, 17, 14}, {31, 1, 2, 4, 8, 8, 8},
                                                  {14, 17, 17, 14, 17, 17, 14}, {14, 17, 17, 15, 1, 1, 14}};
    static constexpr unsigned char blank[7]{}, dot[7]{0, 0, 0, 0, 0, 12, 12}, plus[7]{0, 4, 4, 31, 4, 4, 0},
        minus[7]{0, 0, 0, 31, 0, 0, 0}, slash[7]{1, 2, 2, 4, 8, 8, 16};
...
void first_failure(HRESULT& first, HRESULT value) noexcept {
    if (SUCCEEDED(first) && FAILED(value)) first = value;
}
...
void ComparisonNotice::text(const char* first, const char* second) noexcept {
    ...
    std::strncpy(next[0], first, columns);
    std::strncpy(next[1], second, columns);
    if (!std::memcmp(next, lines_, sizeof next)) return;
    std::memcpy(lines_, next, sizeof next);
    count_ = 0;
    width_ = 0;
    clipped_width_ = clipped_height_ = 0; // the cached clip is stale
...
ComparisonNoticeResult ComparisonNotice::draw(IDirect3DDevice9* device, void* const* native,
    ...
    const auto get_rt = reinterpret_cast<GetRT>(native[38]);
    const auto set_rt = reinterpret_cast<SetRT>(native[37]);
    const auto get_ds = reinterpret_cast<GetDS>(native[40]);
    const auto set_ds = reinterpret_cast<SetDS>(native[39]);
    const auto get_vp = reinterpret_cast<GetVP>(native[48]);
    const auto set_vp = reinterpret_cast<SetVP>(native[47]);
    IDirect3DSurface9 *rt[4]{}, *depth = nullptr, *back = nullptr;
    D3DVIEWPORT9 viewport{};
    D3DSURFACE_DESC desc{};
    unsigned attempted_rt = 0;
    bool attempted_depth = false, attempted_viewport = false;
    auto release = [&] {
        for (auto& surface : rt) drop(surface);
        drop(depth);
        drop(back);
    };
    HRESULT hr = S_OK;
    ...
    for (unsigned i = 0; i < target_count; ++i) {
        ...
        if ((FAILED(hr) && !(i && hr == D3DERR_NOTFOUND && !rt[i])) || (!i && !rt[i])) {
            result.operation = FAILED(hr) ? hr : E_FAIL;
            release();
            return result;
        }
    }
    hr = get_ds(device, &depth);
    if (FAILED(hr) && !(hr == D3DERR_NOTFOUND && !depth)) {
        result.operation = hr;
        release();
        return result;
    }
    ...
    if (clipped_width_ != desc.Width || clipped_height_ != desc.Height) {
        ...
        for (unsigned i = 0; i < count_; ++i) {
            auto rect = pixels_[i];
            rect.x2 = std::min(rect.x2, LONG(desc.Width));
            rect.y2 = std::min(rect.y2, LONG(desc.Height));
        ...
        clipped_width_ = desc.Width;
        clipped_height_ = desc.Height;
    }
    const unsigned clipped_count = clipped_count_;
    const D3DRECT panel = panel_;
    if (!clipped_count || panel.x2 <= panel.x1 || panel.y2 <= panel.y1) {
        release();
        return result;
    }
    attempted_depth = true;
    hr = set_ds(device, nullptr);
    ...
    for (unsigned i = 1; i < target_count && SUCCEEDED(hr); ++i) {
        attempted_rt |= 1u << i;
        attempted_viewport = true;
        hr = set_rt(device, i, nullptr);
    }
    if (SUCCEEDED(hr)) {
        attempted_rt |= 1;
        attempted_viewport = true;
        hr = set_rt(device, 0, back);
    }
    const D3DVIEWPORT9 full{0, 0, desc.Width, desc.Height, 0.f, 1.f};
    if (SUCCEEDED(hr)) {
        attempted_viewport = true;
        hr = set_vp(device, &full);
    }
    ...
    for (unsigned i = 0; i < target_count; ++i)
        if (attempted_rt & (1u << i)) first_failure(result.restore, set_rt(device, i, rt[i]));
