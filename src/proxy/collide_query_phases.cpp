...
constexpr unsigned char pre[]={0x33,0xc0,0xd9,0x1c,0x24,0x51,0x52,0x53,0x57,0xa3,0x44,0x85,0x60,0x00,0xa3,0x48,0x85,0x60,0x00,0xa3,0x4c,0x85,0x60,0x00};
...
struct ErrorGuard {DWORD error=GetLastError();~ErrorGuard(){SetLastError(error);}};
...
#endif
    LARGE_INTEGER t{};return QueryPerformanceCounter(&t)&&t.QuadPart>0?std::uint64_t(t.QuadPart):0;}
bool ours(){return owner.load(std::memory_order_relaxed)==GetCurrentThreadId();}
std::uint64_t us(std::uint64_t ticks){return ticks/frequency*1000000+(ticks%frequency)*1000000/frequency;}
bool matches(std::uintptr_t address,const unsigned char* expected,unsigned size){
    unsigned char actual[32]{};return engine_patch::read_code(address,actual,size)&&!std::memcmp(actual,expected,size);
}
}
void detail::begin(core::Kind kind){
    ...
    if(generation!=seen_generation){accumulator.reset();seen_generation=generation;}
    const auto early=static_cast<std::uint32_t>(InterlockedExchange(&early_cpu_modes,0));
    accumulator.frame.invalid+=early;accumulator.frame.cpu_mode+=early;
...
void detail::end(){
    ...
    const LONG generation=InterlockedCompareExchange(&reset_generation,0,0);
    if(pending_generation!=generation){accumulator.invalidate(core::Reset);seen_generation=generation;}
    ...
    accumulator.end(now,eligible?read(collide_memo::core::visits_va):0,
        eligible?read(collide_memo::core::triangles_va):0,eligible?read(collide_memo::core::contacts_va):0);
}
void detail::invalidate(){if(ours())accumulator.invalidate();}
void detail::foreign(){InterlockedIncrement(&foreign_generation);}
void device_reset(){if(enabled)InterlockedIncrement(&reset_generation);}
bool initialize(bool memo_ready,bool verifying){
    ...
    if(memo_ready){
        LARGE_INTEGER f{};
        if(!QueryPerformanceFrequency(&f)||f.QuadPart<=0)reason="clock_unavailable";
        else if(!object_trace::executable_verified())reason="executable_mismatch";
        else if(!matches(site_va-sizeof pre,pre,sizeof pre)||!matches(site_va+5,post,sizeof post))reason="bytes_mismatch";
        else {
            ...
            for(auto& t:cost){const auto a=clock_tick(),b=clock_tick();t=a&&b>=a?b-a:0;}
            std::sort(cost,cost+129);clock_self_ns=cost[64]*1000000000/frequency;
            if(engine_patch::claim_call(patch,site_va,target_va,reinterpret_cast<void*>(&x3m_collide_query_descent_thunk))) {
                accumulator={};window={};descent_busy=false;owner.store(0,std::memory_order_relaxed);
                seen_generation=InterlockedCompareExchange(&reset_generation,0,0);
                verify_mode=verifying;enabled=true;reason="ok";
            }else {reason=patch.status;if(patch.patched_in&&!engine_patch::restore_call(patch))reason="rollback_failed";}
        }
    }
    log("collide_query_phase_mode requested=1 enabled=%u reason=%s site=0x004e2956 target=0x004e2530 frames=300 clock_self_ns=%llu qpc_frequency=%llu engine_query=dispatch_root_setup_descent non_descent=same_sample_difference",unsigned(enabled),reason,clock_self_ns,frequency);
    return enabled;
}
bool shutdown(){
    ErrorGuard error;enabled=false;
#ifdef X3M_COLLIDE_QUERY_FIXTURE
...
void present(unsigned long long device,unsigned long long frame){
    ...
    const auto quantile=[&](std::uint64_t core::Sample::*field,unsigned p){return us(window.quantile(field,p));};
    log("collide_query_phases qpc=%llu device=%llu frame=%llu first_frame=%llu frames=300 valid_frames=%u invalid_frames=%u verify=%u engine_query_p50_us=%llu engine_query_p95_us=%llu descent_p50_us=%llu descent_p95_us=%llu non_descent_p50_us=%llu non_descent_p95_us=%llu engine_query_sum_us=%llu descent_sum_us=%llu non_descent_sum_us=%llu queries=%llu descents=%llu misses=%llu verifies=%llu ineligible=%llu visits=%llu triangles=%llu contacts=%llu invalid=%llu discard_clock=%llu discard_reentry=%llu discard_foreign=%llu discard_unwind=%llu discard_reset=%llu discard_cpu_mode=%llu clock_self_ns=%llu",
        clock_tick(),device,frame,first_frame,window.valid_frames(),window.size-window.valid_frames(),unsigned(verify_mode),quantile(&core::Sample::query,50),quantile(&core::Sample::query,95),quantile(&core::Sample::descent,50),quantile(&core::Sample::descent,95),quantile(&core::Sample::non_descent,50),quantile(&core::Sample::non_descent,95),us(totals.query),us(totals.descent),us(totals.non_descent),totals.queries,totals.descents,totals.misses,totals.verifies,totals.ineligible,totals.visits,totals.triangles,totals.contacts,totals.invalid,totals.clock,totals.reentry,totals.foreign,totals.unwind,totals.reset,totals.cpu_mode,clock_self_ns);
    window={};
}
#ifdef X3M_COLLIDE_QUERY_FIXTURE
void fixture_clock(std::uint64_t (*clock)()){clock_override=clock;}
core::Sample fixture_sample(){return accumulator.frame;}
unsigned fixture_window_size(){return window.size;}
void fixture_fail_restore(bool fail){fail_restore=fail;}
void fixture_clear(){accumulator={};window={};seen_generation=InterlockedCompareExchange(&reset_generation,0,0);}
#endif
}
...
extern "C" int __cdecl x3m_collide_query_lookup(std::uint32_t flags,std::uint32_t cap,const x3m_collide_memo_args* args){
    const DWORD error=GetLastError();const int result=x3m_collide_memo_lookup(flags,cap,args);SetLastError(error);return result;
}
extern "C" void __cdecl x3m_collide_query_store(){
    const DWORD error=GetLastError();x3m_collide_memo_store();SetLastError(error);
}
extern "C" int __cdecl x3m_collide_query_enter(){
    ...
    if(descent_busy){accumulator.invalidate(core::Reentry);return 0;}
    descent_busy=true; // before QPC: even callback reentry must not claim the return slot
    const DWORD error=GetLastError();accumulator.enter(clock_tick());SetLastError(error);return 1;
}
extern "C" void __cdecl x3m_collide_query_leave(){
    using namespace x3m::collide_query_phases;
    const DWORD error=GetLastError();if(enabled&&ours()&&accumulator.pending){accumulator.leave(clock_tick());descent_busy=false;}SetLastError(error);
}
extern "C" void __cdecl x3m_collide_query_mode_discard(){
    ...
    if(enabled){
        ...
        if(!thread)InterlockedIncrement(&early_cpu_modes);
        else if(thread==GetCurrentThreadId())accumulator.reject_mode();
        else detail::foreign();
    }
}
extern "C" void __cdecl x3m_collide_query_mode_abandon(){
    x3m_collide_query_mode_discard();x3m_collide_memo_abandon();
}
extern "C" void __cdecl x3m_collide_query_descent_abandon(){
    using namespace x3m::collide_query_phases;
    if(ours())descent_busy=false;
}
extern "C" { std::uint32_t x3m_collide_query_descent_target=0x004e2530, x3m_collide_query_descent_return=0; }
