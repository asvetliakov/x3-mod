...
const char* take_back(const char* reason) {
    if (engine_patch::restore(site_)) { site_ = engine_patch::Site{}; site_at_ = 0; return reason; }
    patched_ = true; site_.patched_in = true;  // our jump may still be there: shutdown() tries again
...
bool install_at(std::uintptr_t window) {
    ...
    void** slot = nullptr;
    write_ = "none";
    if (!window) reason = "invalid_site";
    else if (!engine_patch::install_window_open()) reason = "late_claim";
    else if (!engine_patch::read_code(window, current, sites::window_length)) reason = "unreadable";
    else if ((reason = sites::plan(current)) != nullptr) {}
    else if (!(stub = emit_stub(&slot))) reason = "arena_full";
    if (!reason) {
        engine_patch::SiteSpec spec{};
        spec.name = sites::claim_spec.name; spec.address = window + sites::site_offset; spec.length = sites::site_length;
        spec.ret_pop = sites::claim_spec.ret_pop; spec.rel32_offset = sites::claim_spec.rel32_offset;
        ...
        if (!engine_patch::claim(site_, spec)) {
            reason = site_.status;
            if (!std::strcmp(reason, "patch_rolled_back") || !std::strcmp(reason, "rollback_failed")) write_ = site_.atomic_write ? "atomic" : "plain";
            if (site_.patched_in) { patched_ = true; state_ = "rollback_failed"; return false; }  // registered for shutdown()
            site_ = engine_patch::Site{}; site_at_ = 0;
        } else {
            ...
            if (!engine_patch::store_pointer(slot, *site_.entry) || !engine_patch::push_front(site_, reinterpret_cast<void*>(stub))) reason = take_back("chain_failed");
            else if (!read_back(stub)) reason = take_back("readback_mismatch");
            else { patched_ = true; stub_ = stub; reason = "ok"; }
        }
    }
    state_ = reason;
...
bool initialize() {
    ...
    if (patched_) { SetLastError(error); return stub_ != 0; }  // live only after ok; a registered rollback_failed stays a failure
    ...
    write_ = "none";
    if (length >= sites::setting_capacity) { state_ = "too_long"; parsed = false; }
    else if (length && !sites::parse_mode(text, &mode)) { state_ = "invalid_setting"; parsed = false; }
    else if (mode == sites::Mode::off) state_ = "off";
    else if (!object_trace::executable_verified()) state_ = "executable_mismatch";
    else applied = install_at(sites::window_va);
    // rollback_failed leaves the site registered with a jump that may still be live: not reported as a refusal.
    const char* status = applied ? "patched" : patched_ ? "patched_unverified" : parsed && mode == sites::Mode::off ? "off" : "refused";
    log("sun_flare_fix site=%08lx status=%s reason=%s mode=%s setting=%s write=%s stub=%08lx",
        static_cast<unsigned long>(sites::site_va), status, state_, parsed ? sites::mode_name(mode) : "-", setting, write_,
        static_cast<unsigned long>(stub_));
    SetLastError(error);
    return applied;
}
bool shutdown() {
    ...
    const char* reason;
    if (found_read && !std::memcmp(found, sites::expected_site, sites::site_length)) reason = "restored";          // already the engine's bytes: no write
    else if (!found_read || std::memcmp(found, site_.patched, 5)) reason = "restore_not_owned";                     // someone else's bytes: no write
    else {
        engine_patch::restore(site_);
        unsigned char back[sites::site_length]{};
        const bool back_ok = engine_patch::read_code(site_at_, back, sites::site_length) && !std::memcmp(back, sites::expected_site, sites::site_length);
    ...
    if (!std::strcmp(reason, "restored")) { patched_ = false; stub_ = 0; site_ = engine_patch::Site{}; site_at_ = 0; }  // the stub and tail stay in the arena
    state_ = reason;
    ...
    if (handle && handle != INVALID_HANDLE_VALUE) {
        ...
        if (found_read) std::snprintf(bytes, sizeof bytes, "%02x%02x%02x%02x%02x%02x", found[0], found[1], found[2], found[3], found[4], found[5]);
        const int n = std::snprintf(line, sizeof line, "sun_flare_fix_restore site=%08lx status=%s found=%s registered=%u\n",
