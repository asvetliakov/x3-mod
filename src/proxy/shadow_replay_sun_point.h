...
enum class PointSunReason : unsigned {
    Point = 0,
    Off = 1,
    Unavailable = 2,
    NoLight = 3,
...
constexpr const char* point_sun_reason_name(PointSunReason r) noexcept {
    switch (r) {
    case PointSunReason::Point: return "point";
    case PointSunReason::Off: return "off";
    case PointSunReason::Unavailable: return "unavailable";
    case PointSunReason::NoLight: return "no_light";
    case PointSunReason::Camera: return "camera";
    case PointSunReason::Near: return "near";
    case PointSunReason::Unchecked: return "unchecked";
    case PointSunReason::Disagrees: return "disagrees";
    case PointSunReason::Cooldown: return "cooldown";
    default: return "?";
    }
}
...
struct PointSun {
    ...
    std::int32_t validated_native[3]{};
    double validated_light[3]{};
    ...
    void begin_frame() noexcept {
        polled = light_valid = false;
        poll_reason = PointSunReason::Off;
        checks = disagreements = 0;
        worst_sin2 = 0.;
        worst_opposed = false;
        decided = 0;
        reason = PointSunReason::Off;
        distance = 0.;
        rederived = 0;
        rederived_mask = 0;
        carried_frame = false;
    ...
    void set_poll(const std::int32_t* position, PointSunReason why) noexcept {
        polled = true;
        poll_reason = why;
        light_valid = position != nullptr && why == PointSunReason::Point;
        if (!light_valid) return;
        for (unsigned i = 0; i < 3; ++i) {
            native[i] = position[i];
            light[i] = double(position[i]) * point_sun_context_scale;
        }
    }
    ...
    bool check(const float constant[4], const double origin[3]) noexcept {
        ...
        if (opposed ? !worst_opposed || sin2 < worst_sin2 : !worst_opposed && sin2 > worst_sin2) {
            worst_sin2 = sin2;
            worst_opposed = opposed;
        }
    ...
    // The world position the origin (0, 0, 0, 1) of a draw's clip rows maps to (renderer::shadow_replay_light_rows'
    // law).
    static bool draw_origin(const renderer::CameraState& camera, const float rows[16], double out[3]) noexcept {
        ...
        const double v[3] = {double(rows[3]) / camera.m00 - double(camera.t[0]),
                             double(rows[7]) / camera.m11 - double(camera.t[1]),
                             double(rows[15]) - double(camera.t[2])};
        double scratch[9];
    ...
    bool decide(bool enabled, const renderer::CameraState& camera, const renderer::ShadowCascadeSet& set) noexcept {
        ...
        const bool fresh = polled && light_valid &&
                           (checks || (validated && validated_native[0] == native[0] &&
                                       validated_native[1] == native[1] && validated_native[2] == native[2]));
        const bool carry = !fresh && validated && carried < point_sun_carry_frames; // hysteresis: the last validated
                                                                                    // position
        if (!enabled || !set.count)
            reason = PointSunReason::Off;
        else if (polled && !light_valid)
            reason = poll_reason;
        else if (cooldown)
            reason = PointSunReason::Cooldown;
        else if (disagreements)
            reason = PointSunReason::Disagrees;
        else if (!camera.valid)
            reason = PointSunReason::Camera;
        else if (!fresh && !carry)
            reason = polled ? PointSunReason::Unchecked : PointSunReason::Unavailable;
        else {
            carried_frame = !fresh;
            ...
            for (unsigned i = 0; i < 3; ++i) {
                ...
                to_light[i] = used[i] - position[i];
                d2 += to_light[i] * to_light[i];
            }
            distance = renderer::sqrt_sd(d2);
            const auto& last = set.cascades[set.count - 1];
            // The light must lie outside every cascade's volume (an orthographic map of a light inside it means
            // nothing).
            if (!(distance > double(last.depth_toward_light) + double(last.half_extent)) || !(distance < 1e15))
                reason = PointSunReason::Near;
            else {
                reason = PointSunReason::Point;
                decided = 1;
                for (unsigned k = 0; k < set.count; ++k) {
                    const auto& cascade = set.cascades[k];
                    double ideal[3], centre[3], n2 = 0.;
                    for (unsigned i = 0; i < 3; ++i) {
                        centre[i] = position[i] + forward[i] * double(cascade.forward_offset);
                        ideal[i] = used[i] - centre[i];
                        n2 += ideal[i] * ideal[i];
                    }
                    ...
                    const bool keep = held_valid[k] &&
                                      renderer::shadow_replay_basis(camera, suns + k * 4, cascade, old, anchor[k]);
                    ...
                    for (unsigned i = 0; i < 3; ++i) {
                        held[k][i] = adopt ? held[k - 1][i] : ideal[i];
                        suns[k * 4 + i] = adopt ? suns[(k - 1) * 4 + i] : float(ideal[i]);
                    }
                    suns[k * 4 + 3] = 0.f;
                    held_valid[k] = true;
                    ++rederived;
        ...
        if (decided < 0 && reason != PointSunReason::Camera)
            for (bool& v : held_valid)
                v = false; // a real switch: the next point frame derives afresh (a frame without a camera is refused
                           // anyway)
        return decided > 0;
    }
    ...
    void end_frame() noexcept {
        ...
        if (disagreements) {
            validated = false;
            cooldown = point_sun_cooldown_frames;
            carried = 0;
        } else if (checks && light_valid) {
            validated = true;
            carried = 0;
            for (unsigned i = 0; i < 3; ++i) {
                validated_native[i] = native[i];
                validated_light[i] = light[i];
            }
        } else if (carried_frame)
            ++carried;
        if (decided > 0)
            ++frames_point;
        else
            ++frames_latch[unsigned(reason) < unsigned(PointSunReason::Count) ? unsigned(reason) : 0u];
    }
    const float* sun(unsigned cascade) const noexcept {
        return decided > 0 && cascade < renderer::shadow_cascade_max ? suns + cascade * 4 : nullptr;
    }
    const double* grid_anchor(unsigned cascade) const noexcept {
        return decided > 0 && cascade < renderer::shadow_cascade_max ? anchor[cascade] : nullptr;
    }
