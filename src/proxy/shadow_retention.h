...
struct ShadowRetentionLifetime {
    ...
    object_lifetime::JournalDrain (*journal_drain)(object_lifetime::JournalCursor&, object_lifetime::JournalEntry*,
                                                   std::uint32_t) = nullptr;
    bool (*current)(std::uintptr_t registry, std::uintptr_t node, std::uint32_t node_handle, std::uintptr_t camera,
                    std::uint32_t camera_handle, object_lifetime::Snapshot*) = nullptr;
};
struct ShadowRetention {
    ...
    std::uintptr_t registry = 0, camera = 0;
    std::uint32_t camera_handle = 0;
    std::uint64_t mutation_revision = 0;
    bool mutation_known = false;
    ...
    bool sun_none = false;     // the previous frame had no validated sun (a fresh device starts without one: nothing to
                               // void yet)
    ...
    // Cost: the scene-end work of the frame; with X3M_SHADOW_RETENTION_TIMING=1 also the draw path (one counter pair
    // per recorded draw).
    bool timing = false;
    std::int64_t draw_ticks = 0;
    std::uint32_t draw_calls = 0;
    std::int64_t gate_ticks = 0;
    std::uint32_t gate_calls = 0; // note_refused_sighting (timing on): the static gate's refused-draw sightings
