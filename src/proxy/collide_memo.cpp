...
// Everything the enumeration of a no-contact query's writes rests on (section 14.2), outside the windows install_at
// compares.
bool bodies_match() {
    return body_matches(caller_va, caller_length, caller_fnv1a, false, false) &&
           body_matches(memo_target_va, target_length, target_fnv1a, false, false) &&
           body_matches(query_va, query_length, query_fnv1a, false, false) &&
           body_matches(descent_va, descent_length, descent_fnv1a, true, true) &&
           body_matches(leaf_va, leaf_length, leaf_fnv1a, true, false) &&
           body_matches(triangle_va, triangle_length, triangle_fnv1a, false, false) &&
           body_matches(sat_va, sat_length, sat_fnv1a, false, false) &&
           body_matches(matrix_helpers_va, matrix_helpers_length, matrix_helpers_fnv1a, false, false) &&
           body_matches(vector_helpers_va, vector_helpers_length, vector_helpers_fnv1a, false, false) &&
           body_matches(ftol_va, ftol_length, ftol_fnv1a, false, false);
}
...
bool build_key(Key& key, std::uint32_t flags, std::uint32_t cap, const x3m_collide_memo_args& a) {
    if (a.model_a == nullptr || a.model_b == nullptr)
        return false; // the caller tests both before the site; never dereferenced on trust
    if (a.model_a[model_state_word] != model_built || a.model_b[model_state_word] != model_built || a.model_a[0] == 0 ||
        a.model_b[0] == 0)
        return false;
    std::uint32_t* w = key.words;
    std::memcpy(w, a.R1, 36);
    std::memcpy(w + 9, a.T1, 12);
    w[12] = a.s1;
    std::memcpy(w + 13, a.R2, 36);
    std::memcpy(w + 22, a.T2, 12);
    w[25] = a.s2;
    w[26] = a.tolerance;
    w[27] = flags;
    w[28] = cap;
    w[29] = a.minimum != nullptr;
    w[30] = a.minimum != nullptr ? *a.minimum : 0u;
    w[31] = reinterpret_cast<std::uintptr_t>(a.model_a);
    w[32] = reinterpret_cast<std::uintptr_t>(a.model_b);
    std::memcpy(w + 33, a.model_a, 4 * header_words);
    std::memcpy(w + 33 + header_words, a.model_b, 4 * header_words);
...
int __cdecl x3m_collide_memo_lookup(std::uint32_t flags, std::uint32_t cap, const x3m_collide_memo_args* args) {
    ...
    if (owner != thread) {
        ...
        x3m::collide_query_phases::foreign();
        return 2;
    }
    if (busy_) {
        ++counters_.reentered;
        x3m::collide_query_phases::invalidate();
        return 2;
    }
    busy_ = true;
    const std::uint32_t frame = frame_;
    if (frame != seen_frame_) {
        seen_frame_ = frame;
        queries_since_tick_ = 0;
    }
    if (InterlockedExchange(&clear_requested_, 0) != 0 || ++queries_since_tick_ > queries_without_tick_limit) {
        table_.clear();
        queries_since_tick_ = 0;
        ++counters_.clears;
    }
    ...
    if (!pending_eligible_) {
        ++counters_.ineligible;
        x3m::collide_query_phases::begin(x3m::collide_query_phases::core::Ineligible);
        return 0;
    }
    Entry* const entry = table_.find(pending_key_, frame);
    if (entry == nullptr) {
        ++counters_.misses;
        pending_miss_ = table_.classify(pending_key_);
        ++counters_.miss_count[pending_miss_];
        x3m::collide_query_phases::begin();
        return 0;
    }
    if (verify_) {
        pending_verify_ = entry;
        x3m::collide_query_phases::begin(x3m::collide_query_phases::core::Verify);
        return 0;
    } // the engine runs too; store() compares
    entry->frame = frame;
    // Exactly what the query would have written (collide_memo_core.h); the contact record and *minimum stay as they
    // are.
    ...
    ++counters_.hits;
    if (entry->key.words[minimum_value_word] != pending_key_.words[minimum_value_word])
        ++counters_.min_relaxed_hits; // answered although the running minimum moved: no leaf was reached
    counters_.skipped_visits += entry->outputs.visits;
    counters_.skipped_triangles += entry->outputs.triangles;
    busy_ = false;
    return 1;
}
void __cdecl x3m_collide_memo_abandon() {
    busy_ = false;
...
void __cdecl x3m_collide_memo_store() { // owner thread only: reached on lookup() == 0 alone
    ...
    pending_eligible_ = false;
    Outputs now;
    now.visits = engine<std::uint32_t>(visits_va);
    now.triangles = engine<std::uint32_t>(triangles_va);
    now.tolerance_integer = engine<std::uint32_t>(tolerance_va);
    ...
    if (Entry* const expected = pending_verify_) {
        pending_verify_ = nullptr;
        if (!contact && !std::memcmp(&now, &expected->outputs, sizeof now)) {
            ++counters_.verified;
            expected->frame = frame_;
        } else {
            ++counters_.verify_mismatches;
    ...
    if (contact) {
        ++counters_.contacts;
        return;
    } // never stored: a contact is always computed
    ++counters_.stored;
...
bool install_at(const Addresses& a, bool verify_mode) {
    ...
    if (!bytes_match(a.site - memo_pre_length, memo_pre_window, memo_pre_length) ||
        !bytes_match(a.site + call_length, memo_post_window, memo_post_length))
    ...
    void* const thunk = phases ? reinterpret_cast<void*>(&x3m_collide_query_memo_thunk)
                               : reinterpret_cast<void*>(&x3m_collide_memo_thunk);
...
bool initialize() {
    ...
    else if (!bodies_match())
        state_ = "body_mismatch";
    else
        applied = install_at(Addresses{memo_site_va, memo_target_va}, verify_mode);
    log("collide_memo requested=%u patched=%u verify=%u reason=%s site=0x%08lx target=0x%08lx write=%s handler=0x%08lx entries=%u",
        requested ? 1u : 0u, patched_ ? 1u : 0u, patched_ && verify_ ? 1u : 0u, state_,
        static_cast<unsigned long>(memo_site_va), static_cast<unsigned long>(memo_target_va),
        site_.patched_in ? (site_.atomic_write ? "atomic" : "plain") : "none",
        static_cast<unsigned long>(reinterpret_cast<std::uintptr_t>(
            x3m::collide_query_phases::enabled ? &x3m_collide_query_memo_thunk : &x3m_collide_memo_thunk)),
        ways * sets);
    SetLastError(error);
    return applied;
}
...
core::Counters counters() {
    core::Counters c = counters_;
    c.foreign_thread = static_cast<std::uint32_t>(foreign_thread_);
    return c;
}
void device_reset() {
    if (patched_) {
        InterlockedExchange(&clear_requested_, 1);
        x3m::collide_query_phases::device_reset();
    }
}
void present(unsigned long long device, unsigned long long frame, bool) {
    ...
    if (busy_ && static_cast<LONG>(GetCurrentThreadId()) == owner_thread_) {
        busy_ = false;
        ++counters_.stuck_busy;
        InterlockedExchange(&clear_requested_, 1);
    }
    if (now % 300u != 0 || !windows_) return;
    const DWORD error = GetLastError();
    const Counters c = counters(); // written by the owner thread; a torn read costs one window's accuracy, never a
                                   // decision
    const auto d = [](std::uint32_t a, std::uint32_t b) { return static_cast<unsigned long>(a - b); };
    const std::uint32_t queries = (c.hits - logged_.hits) + (c.misses - logged_.misses) +
                                  (c.ineligible - logged_.ineligible) + (c.verified - logged_.verified) +
                                  (c.verify_mismatches - logged_.verify_mismatches);
    log("collide_memo device=%llu frame=%llu frames=300 verify=%u queries=%lu hits=%lu misses=%lu stored=%lu contacts=%lu ineligible=%lu evictions=%lu skipped_visits=%lu "
        ...
        device, frame, verify_ ? 1u : 0u, static_cast<unsigned long>(queries), d(c.hits, logged_.hits),
        d(c.misses, logged_.misses), d(c.stored, logged_.stored), d(c.contacts, logged_.contacts),
        d(c.ineligible, logged_.ineligible), d(c.evictions, logged_.evictions),
        d(c.skipped_visits, logged_.skipped_visits), d(c.skipped_triangles, logged_.skipped_triangles),
        d(c.verified, logged_.verified), d(c.verify_mismatches, logged_.verify_mismatches),
        d(c.foreign_thread, logged_.foreign_thread), d(c.reentered, logged_.reentered), d(c.clears, logged_.clears),
        d(c.stuck_busy, logged_.stuck_busy), d(c.min_relaxed_hits, logged_.min_relaxed_hits),
        d(c.miss_count[0], logged_.miss_count[0]), d(c.miss_visits[0], logged_.miss_visits[0]),
        d(c.miss_count[1], logged_.miss_count[1]), d(c.miss_visits[1], logged_.miss_visits[1]),
        d(c.miss_count[2], logged_.miss_count[2]), d(c.miss_visits[2], logged_.miss_visits[2]),
        d(c.miss_count[3], logged_.miss_count[3]), d(c.miss_visits[3], logged_.miss_visits[3]),
        d(c.miss_count[4], logged_.miss_count[4]), d(c.miss_visits[4], logged_.miss_visits[4]),
        d(c.miss_count[5], logged_.miss_count[5]), d(c.miss_visits[5], logged_.miss_visits[5]),
        d(c.miss_count[6], logged_.miss_count[6]), d(c.miss_visits[6], logged_.miss_visits[6]),
        d(c.miss_count[7], logged_.miss_count[7]), d(c.miss_visits[7], logged_.miss_visits[7]));
    SetLastError(error);
    logged_ = c;
}
}
