...
HRESULT query_resource_id(IDirect3DResource9* resource, uint64_t* id) noexcept {
    ...
    *id = 0;
    ...
    if (FAILED(hr) || bytes != sizeof(*id) || !*id) {
        *id = 0;
...
void capture_constants(IDirect3DDevice9* d, bool vertex, const D3DCAPS9& caps) {
    ...
    if (SUCCEEDED(floats)) {
        for (UINT i = 0; i < count; ++i) {
            uint32_t bits[4];
            std::memcpy(bits, values + i * 4, sizeof bits);
            if (bits[0] || bits[1] || bits[2] || bits[3])
                log("constant kind=%s type=f reg=%u bits=%08x,%08x,%08x,%08x", kind, i, bits[0], bits[1], bits[2],
                    bits[3]);
        }
    }
    int integers[16 * 4]{};
    ...
    if (SUCCEEDED(ints))
        for (UINT i = 0; i < 16; ++i)
            log("constant kind=%s type=i reg=%u values=%d,%d,%d,%d", kind, i, integers[i * 4], integers[i * 4 + 1],
                integers[i * 4 + 2], integers[i * 4 + 3]);
    BOOL booleans[16]{};
    ...
    if (SUCCEEDED(bools))
        for (UINT i = 0; i < 16; ++i) log("constant kind=%s type=b reg=%u values=%d", kind, i, booleans[i]);
}
void capture_geometry(IDirect3DDevice9* d, bool user_memory, const D3DCAPS9& caps) {
    ...
    for (UINT i = 0; i < streams; ++i) {
        ...
        if (FAILED(hr)) {
            log("stream slot=%u result=%08lx", i, hr);
            continue;
        }
        if (!buffer) {
            log("stream slot=%u result=00000000 identity=0", i);
            continue;
        }
        const auto id = resource_id(buffer);
        HRESULT freq = d->GetStreamSourceFreq(i, &frequency);
        log("stream slot=%u result=%08lx identity=%llu offset=%u stride=%u frequency=%u frequency_result=%08lx", i, hr,
            id, offset, stride, frequency, freq);
        D3DVERTEXBUFFER_DESC desc{};
        hr = buffer->GetDesc(&desc);
        if (SUCCEEDED(hr))
            log("vertex_buffer identity=%llu bytes=%u usage=%lu pool=%u fvf=%lu", id, desc.Size, desc.Usage, desc.Pool,
                desc.FVF);
    ...
    if (indices) {
        ...
        if (SUCCEEDED(indices->GetDesc(&desc)))
            log("index_buffer identity=%llu bytes=%u usage=%lu pool=%u format=%u", id, desc.Size, desc.Usage, desc.Pool,
                desc.Format);
