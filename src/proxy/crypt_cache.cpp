...
struct ProviderEntry {
    ...
    HCRYPTPROV handle=0;
    unsigned phase=Absent; DWORD owner_thread=0;
};
struct KeyEntry {
    bool used=false;
...
struct RetiredKeys {HCRYPTKEY handles[key_slots]{};unsigned count=0;
    void destroy() noexcept {for(unsigned i=0;i<count;++i)real.destroy_key(handles[i]);count=0;}
};
void evict(ProviderEntry& e,RetiredKeys& retired) noexcept {
    for(auto& k:keys)if(k.used&&k.provider==e.handle){if(!k.outstanding)retired.handles[retired.count++]=k.key;k.used=false;k.key=0;k.outstanding=0;--totals.keys_cached;}
    e.handle=0;e.phase=Absent;e.flags=0;e.owner_thread=0;
...
BOOL WINAPI acquire(HCRYPTPROV* out,LPCSTR container,LPCSTR provider,DWORD type,DWORD flags) {
    ...
    if(!out||type!=PROV_RSA_FULL||!same_name(allowed_container,false,container)||
       !same_name(allowed_provider,false,provider)||
       (flags!=CRYPT_DELETEKEYSET&&flags!=CRYPT_NEWKEYSET))return real.acquire(out,container,provider,type,flags);
    ...
    }
    {
        ...
        ProviderEntry* e=find_provider(container,provider,type);
        if(e&&e->handle){
            if(e->phase==Absent&&e->flags==flags&&out){*out=e->handle;e->phase=InUse;e->owner_thread=GetCurrentThreadId();++totals.hits;SetLastError(caller_error);return TRUE;}
    ...
    const DWORD result_error=GetLastError();
    {
        ...
        if(!result)++totals.failed_passthrough;
            ...
            if(out&&*out&&mutation_generation==generation){
                ...
                if(e&&!e->handle){e->handle=*out;e->flags=flags;e->phase=InUse;e->owner_thread=GetCurrentThreadId();++totals.providers_cached;}
            }
        }
    }
    SetLastError(result_error);
...
BOOL WINAPI release(HCRYPTPROV provider,DWORD flags) {
    ...
    RetiredKeys retired;
    {
        ...
        if(e)for(const auto& k:keys)key_outstanding|=k.used&&k.provider==provider&&k.outstanding;
        if(e&&e->phase==InUse&&!key_outstanding&&flags==0&&e->owner_thread==GetCurrentThreadId()){e->phase=Released;e->owner_thread=0;++totals.releases_suppressed;SetLastError(caller_error);return TRUE;}
...
BOOL WINAPI import_key(HCRYPTPROV provider,const BYTE* data,DWORD length,HCRYPTKEY public_key,DWORD flags,HCRYPTKEY* out) {
    ...
    uint64_t generation=0;
    const bool cacheable=data&&out&&length==276&&public_key==0&&flags==0&&
        data[0]==PUBLICKEYBLOB&&data[1]==CUR_BLOB_VERSION&&data[2]==0&&data[3]==0&&
        data[4]==0&&(data[5]==BYTE(CALG_RSA_SIGN>>8)||data[5]==BYTE(CALG_RSA_KEYX>>8))&&data[6]==0&&data[7]==0&&
        data[8]=='R'&&data[9]=='S'&&data[10]=='A'&&data[11]=='1'&&
        data[12]==0&&data[13]==8&&data[14]==0&&data[15]==0;
    {
        Lock lock;
        generation=++mutation_generation;
        ++totals.imports;
        ...
        if(e&&e->phase==InUse&&e->owner_thread==GetCurrentThreadId()){
            if(KeyEntry* k=find_key(provider,flags,data,length);k&&!k->outstanding){*out=k->key;++k->outstanding;++totals.import_hits;SetLastError(caller_error);return TRUE;}
        }
    ...
    if(result&&cacheable&&*out){
        ...
        if(e&&e->phase==InUse&&e->owner_thread==GetCurrentThreadId()&&!find_key(provider,flags,data,length)){
            for(auto& k:keys){
                if(k.used)continue;
...
BOOL WINAPI destroy_key(HCRYPTKEY key) {
    ...
    const DWORD caller_error=GetLastError();
    {
        ...
        ++mutation_generation;
        ++totals.destroys;
        if(KeyEntry* k=find_key(key)){const auto* e=find_provider(k->provider);
            if(k->outstanding&&e&&e->phase==InUse&&e->owner_thread==GetCurrentThreadId()){--k->outstanding;++totals.destroys_suppressed;SetLastError(caller_error);return TRUE;}
            k->used=false;k->key=0;--totals.keys_cached; // duplicate destroy goes to the real API
        }
    }
    SetLastError(caller_error);
...
bool shutdown() noexcept {
    ...
    HCRYPTKEY retired_keys[key_slots]{};ProviderEntry retired_providers[provider_slots];
    unsigned key_count=0,provider_count=0;
    {
        Lock lock(4096);
        if(!lock.held)return false;
        if(!is_enabled)return true;
        for(const auto& e:providers)if(e.used&&e.phase==InUse)return false;
        for(const auto& k:keys)if(k.used&&k.outstanding)return false;
        ++mutation_generation;
        InterlockedExchange(&is_enabled,0);
        for(auto& k:keys)if(k.used){retired_keys[key_count++]=k.key;k.used=false;k.key=0;--totals.keys_cached;}
        for(auto& e:providers)if(e.used){retired_providers[provider_count++]=e;if(e.handle)--totals.providers_cached;e.used=false;e.handle=0;}
    }
    ...
    for(unsigned i=0;i<provider_count;++i){
        ...
        if(e.phase==Absent){HCRYPTPROV scratch=0;real.acquire(&scratch,e.container,e.provider,e.type,CRYPT_DELETEKEYSET);}
    }
    return true;
}
