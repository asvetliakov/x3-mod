...
struct Row {
    uint64_t calls, failures, pending, ambiguous, bytes, inclusive, exclusive, maximum, overhead;
};
...
struct Span {
    uint64_t begin_ticks = 0, children = 0;
    Span* parent = nullptr;
    DWORD caller_error = 0;
    unsigned op = 0;
...
struct ProbeRow {
    ...
    uint32_t caller_address[probe_caller_limit];
    uint64_t caller_calls[probe_caller_limit];
};
