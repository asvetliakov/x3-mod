...
// 70 -> 0x2768, 100 -> 0x3b6f). Two sites (fov_sites.h), both or neither:
...
// Nothing per frame, saves unchanged. Written on the backend-load path inside
// the engine_patch install window (a late call is refused, late_claim) after
// the structural executable check, the reader contract, the INS_SetFocus case
// window and callee prefix, and the constructor's 28-byte window compare (fail
// closed). A failed second site rolls the first back; a rollback that cannot
// be completed keeps the site registered (patched_unverified). When the
...
// unload restores the INS_SetFocus bytes (only over our jmp) and 00 40 00 00
...
// and table stay in the never-freed arena.
namespace x3m::fov {
...
const char* setfocus_state();  // none (not attempted or restored), active, or the INS_SetFocus failure reason
bool patched();                // either site registered (the constructor's immediate or the INS_SetFocus claim)
