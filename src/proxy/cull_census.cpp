...
Ladder read_ladder(std::uint32_t model) {
    ...
    std::int16_t count = 0;
    std::memcpy(&records, head, 4);
    std::memcpy(&count, head + (model_lod_count_offset - model_records_offset), 2);
    l.known = true;
    l.count = count;
    ...
    for (unsigned i = 0; i < n; ++i)
        if (!pointers[i] || !engine_memory::read(std::uintptr_t(pointers[i]) + record_threshold_offset, &thr[i], 4))
            return l;
    std::memcpy(l.thr, thr, sizeof thr);
    l.thresholds = n;
    return l;
}
...
const BodyTable& body_table() {
    ...
    std::uint32_t g = 0;
    std::int32_t head[3]{}; // +0xb4 fixed, +0xb8 dynamic, +0xbc slots
    static_assert(body_dynamic_count_offset == body_fixed_count_offset + 4 &&
                      body_slots_offset == body_fixed_count_offset + 8,
                  "one 12-byte header read");
    if (!engine_memory::read(body_global_, &g, 4) || !g ||
        !engine_memory::read(std::uintptr_t(g) + body_fixed_count_offset, head, sizeof head))
        return body_table_;
    body_table_.fixed = head[0];
    body_table_.dynamic = head[1];
    body_table_.slots = std::uint32_t(head[2]);
...
const char* body_of(std::uint32_t id) {
    ...
    if (t.valid && body_slot(std::int32_t(id), t.fixed, t.dynamic, &index) &&
        (entry = std::uint64_t(t.slots) + std::uint64_t(index) * body_slot_stride + body_slot_name_offset) <=
            0xfffffffcu &&
        engine_memory::read(std::uintptr_t(entry), &p, 4)) {
        if (!p) {
            body_default_name(std::int32_t(id), name);
            shown = name;
        } else if (read_name(p, name))
            shown = name;
    }
...
void scan_lod_switches(unsigned long long frame, std::uint32_t entries, bool fresh) {
    if (++track_frame_ == 0) {
        clear_tracks();
        track_frame_ = 1;
    } // wrap: re-seed everything
    std::uint32_t switches = 0, nodes = 0;
    for (std::uint32_t i = 0; i < entries; ++i) {
        const Entry& e = ring_[i];
        if (!e.exited || !(e.flags_out & 2u))
            continue; // only nodes the pass kept: a culled node's +0x14c is the entry 0
        ++nodes;
        std::int32_t from = 0;
        ...
        x3m::log("lod_switch frame=%llu node=%08lx%s from=%ld to=%ld s=%ld D=%ld T_pad=%s%s view=%08lx", frame,
                 (unsigned long)e.node, body_of(e.model), (long)from, (long)e.lod, (long)e.s, (long)e.d, pad,
                 flag31_suffix(e.flag31), (unsigned long)e.view);
    }
    if (switches > lod_switch_cap_)
        x3m::log("lod_switch_overflow frame=%llu dropped=%lu cap=%u", frame,
                 (unsigned long)(switches - lod_switch_cap_), lod_switch_cap_);
    if (switches)
        x3m::log("lod_switch_frame frame=%llu switches=%lu nodes=%lu", frame, (unsigned long)switches,
                 (unsigned long)nodes);
}

bool bytes_match(std::uintptr_t at, const unsigned char* expected, unsigned length) {
    unsigned char actual[measure_window_length]{};
    return length <= measure_window_length && engine_patch::read_code(at, actual, length) &&
...
engine_patch::SiteSpec spec_for(const char* name, std::uintptr_t address, const unsigned char* bytes) {
    ...
    spec.length = site_length;
    spec.ret_pop = ret_pop;
    spec.rel32_offset = 0;
...
template <unsigned Length, class Encode> std::uintptr_t emit_stub(Encode&& encode, void*** slot_out) {
...
bool hook_site(engine_patch::Site& site, const engine_patch::SiteSpec& spec, std::uintptr_t stub, void** slot,
               const char** reason) {
    ...
    if (!stub || !slot || !engine_patch::store_pointer(slot, *site.entry) ||
        !engine_patch::push_front(site, reinterpret_cast<void*>(stub))) {
...
extern "C" void x3m_cull_census_measure(std::uint32_t node, std::int32_t measure, std::int32_t s, std::int32_t d,
    ...
    Entry& e = ring_[index];
    e.node = node;
    e.model = n[model_offset / 4];
    e.view = view;
    e.s = s;
    e.measure = measure;
    e.d = d;
    e.radius = std::int32_t(n[radius_offset / 4]);
    e.thr_1dc = std::int32_t(n[threshold_1dc_offset / 4]);
    e.thr_1d8 = std::int32_t(n[threshold_1d8_offset / 4]);
    const std::uint32_t parent = n[parent_offset / 4];
    e.parent = parent;
    e.flag31 = ancestors_.resolve(parent, n[flags12c_offset / 4]);
    e.limit = size_limit(e.thr_1d8, parent != 0,
                         parent ? std::int32_t(reinterpret_cast<const std::uint32_t*>(parent)[threshold_1d8_offset / 4])
                                : 0);
    e.flags_in = n[flags12c_offset / 4];
    e.flags130 = n[flags130_offset / 4];
    e.flags_out = 0;
    e.lod = 0;
    e.exited = 0;
    e.model_ptr = 0;
...
extern "C" void x3m_cull_census_exit(std::uint32_t node, std::uint32_t ebx, std::uint32_t model_slot,
    ...
    if (pending_node_ != node) {
        unmeasured_.fetch_add(1, std::memory_order_relaxed);
        return;
    }
    pending_node_ = 0;
    ...
    const auto* n = reinterpret_cast<const std::uint32_t*>(node);
    Entry& e = ring_[index];
    e.flags_out = n[flags12c_offset / 4];
    e.lod = std::int32_t(n[lod_offset / 4]);
    e.exited = 1;
...
bool install_at(std::uintptr_t measure_site, std::uintptr_t exit_site) {
    ...
    if (!measure_site || !exit_site || measure_site < measure_site_offset || exit_site < exit_site_offset)
        reason = "invalid_site";
        ...
        reason = "late_claim";
             ...
             !bytes_match(exit_site - exit_site_offset, exit_window, exit_window_length))
        reason = "bytes_mismatch";
    else if (!pin_self())
        reason = "pin_failed";
    else if (!ring_ && !(ring_ = static_cast<Entry*>(VirtualAlloc(nullptr, sizeof(Entry) * ring_size,
                                                                  MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE))))
        reason = "ring_alloc_failed";
    std::uintptr_t measure_stub = 0, exit_stub = 0;
    void** measure_slot = nullptr;
    void** exit_slot = nullptr;
    if (!reason) {
        const std::uint32_t enabled = std::uint32_t(reinterpret_cast<std::uintptr_t>(&x3m_cull_census_enabled));
        measure_stub = emit_stub<measure_stub_length>(
            [&](std::uint32_t at, std::uint32_t slot, unsigned char* out) {
                encode_measure_stub(
                    at, enabled, std::uint32_t(reinterpret_cast<std::uintptr_t>(&x3m_cull_census_measure)), slot, out);
            },
            &measure_slot);
        exit_stub = emit_stub<exit_stub_length>(
            [&](std::uint32_t at, std::uint32_t slot, unsigned char* out) {
                encode_exit_stub(at, enabled, std::uint32_t(reinterpret_cast<std::uintptr_t>(&x3m_cull_census_exit)),
                                 slot, out);
            },
            &exit_slot);
    ...
    if (!reason) {
        ...
        if (!hook_site(measure_site_, spec_for("cull_census_measure", measure_site, core::measure_site), measure_stub,
                       measure_slot, &reason)) {
            if (measure_site_.patched_in) {
                patched_ = true;
                state_ = "measure_rollback_failed";
                return false;
            } // registered for shutdown()
        } else if (!hook_site(exit_site_, spec_for("cull_census_exit", exit_site, core::exit_site), exit_stub,
                              exit_slot, &reason)) {
            ...
            if (exit_site_.patched_in) {
                patched_ = true;
                state_ = "exit_rollback_failed";
                return false;
            }
            if (!engine_patch::restore(measure_site_)) {
                patched_ = true;
                state_ = "measure_rollback_failed";
                return false;
            }
            ...
            patched_ = true;
            measure_stub_ = measure_stub;
            exit_stub_ = exit_stub;
            reason = "ok";
        }
    }
    state_ = reason;
...
bool initialize() {
    ...
    if (length == 0 && !group) {
        state_ = "disabled";
        if (x3m::config::get(L"X3M_LOD_SWITCH_LOG", setting, 4))
            log("cull_census_lod_switch state=census_off cap=0 table=0");
        SetLastError(error);
        return false;
    }
    bool applied = false;
    ...
    if (!requested)
        ...
        applied = install_at(measure_site_va, exit_site_va);
    ...
    unsigned cap = 0;
    if (cap_length && !applied)
        lod_switch = "census_off";
    else if (cap_length && !parse_lod_switch_cap(cap_text, cap_length < 8 ? unsigned(cap_length) : 8u, &cap))
        lod_switch = "invalid";
    else if (cap && !set_lod_switch_log(cap))
        lod_switch = "alloc_failed";
    else if (cap)
        lod_switch = "on";
    else if (!cap_length && group && applied)
        lod_switch = set_lod_switch_log(lod_switch_debug_cap) ? "on" : "alloc_failed";
    log("cull_census requested=%u patched=%u reason=%s measure_site=0x%08lx exit_site=0x%08lx write_measure=%s write_exit=%s stub_measure=0x%08lx stub_exit=0x%08lx ring=%u",
        requested ? 1u : 0u, patched_ ? 1u : 0u, state_, static_cast<unsigned long>(measure_site_va),
        static_cast<unsigned long>(exit_site_va),
        measure_site_.patched_in ? (measure_site_.atomic_write ? "atomic" : "plain") : "none",
        exit_site_.patched_in ? (exit_site_.atomic_write ? "atomic" : "plain") : "none",
        static_cast<unsigned long>(measure_stub_), static_cast<unsigned long>(exit_stub_), ring_size);
    log("cull_census_lod_switch state=%s cap=%u table=%u", lod_switch, lod_switch_cap_,
        lod_switch_cap_ ? track_size : 0u);
    SetLastError(error);
    return applied;
}
bool shutdown() {
    ...
    clear_tracks();
    patched_ = false;
    measure_stub_ = exit_stub_ = 0; // the stubs stay in the arena (a thread may still be inside them)
...
std::uintptr_t measure_stub_address() {
    return patched_ ? measure_stub_ : 0;
}
std::uintptr_t exit_stub_address() {
    return patched_ ? exit_stub_ : 0;
}
void begin_frame(bool capture) {
    ...
    unmeasured_.store(0, std::memory_order_relaxed);
...
bool set_lod_switch_log(unsigned cap) {
    ...
    if (cap && !track_) {
        track_ = static_cast<TrackSlot*>(
            VirtualAlloc(nullptr, sizeof(TrackSlot) * track_size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE));
        if (!track_) {
            lod_switch_cap_ = 0;
            return false;
        }
...
Stats stats() {
    ...
    s.unmeasured = unmeasured_.load(std::memory_order_relaxed);
    s.exited = exited_.load(std::memory_order_relaxed);
    ...
    if (captured) {
        ...
        for (std::uint32_t i = 0; i < entries && ring_; ++i) {
            ...
            log("cull_census device=%llu frame=%llu view=%08lx node=%08lx model=%08lx s=%ld measure=%ld d=%ld radius=%ld thr_1dc=%ld thr_1d8=%ld limit=%ld flags_in=%08lx flags_out=%08lx lod=%ld verdict=%s%s%s%s%s",
                device, frame, (unsigned long)e.view, (unsigned long)e.node, (unsigned long)e.model, (long)e.s,
                (long)e.measure, (long)e.d, (long)e.radius, (long)e.thr_1dc, (long)e.thr_1d8, (long)e.limit,
                (unsigned long)e.flags_in, (unsigned long)e.flags_out, (long)e.lod, verdict_name(verdict),
                verdict == Verdict::culled_small ? " scope=all" : "", ladder, body_of(e.model),
                flag31_suffix(e.flag31));
        }
    }
