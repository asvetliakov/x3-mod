...
inline const char* name(Walk w) {
    switch (w) {
    case Walk::Found: return "found";
    case Walk::ReadFailure: return "read_failure";
    case Walk::NoManager: return "no_manager";
    case Walk::Malformed: return "malformed";
    case Walk::Head: return "head";
    case Walk::Bound: return "bound";
    case Walk::Dead: return "dead";
    case Walk::NoScene: return "no_scene";
    case Walk::SameId: return "same_id";
    case Walk::Loading: return "loading";
    case Walk::BadCount: return "bad_count";
    case Walk::BadIndex: return "bad_index";
...
template <class Read> Result walk(Read& read, std::uint32_t last_ready_id) {
    ...
    Result out;
    auto counted = [&](std::uintptr_t at, void* to, std::size_t n) {
        ++out.reads;
        return read(at, to, n);
    };
    ...
    if (!manager) {
        out.status = Walk::NoManager;
        return out;
    }
    if (manager & 3) {
        out.status = Walk::Malformed;
        return out;
    }
    ...
    if (marker != 0xcafe) {
        out.status = Walk::Dead;
        return out;
    }
    if (!sb::field(counted, node, 0x130, block)) return out; // scene, galaxy, dust, index, stars, neb, flags, size
    if (!block[0]) {
        out.status = Walk::NoScene;
        return out;
    } // caught between the index write and the scene
    if (!sb::field(counted, node, 8, out.id)) return out;
    if (last_ready_id && out.id == last_ready_id) {
        out.status = Walk::SameId;
        return out;
    }
    ...
    s.class48 = 1;
    s.sector_id = out.id;
    s.index = std::int32_t(block[3]);
    if (!sb::field(counted, 0x607040, 0, s.count) || !sb::field(counted, 0x606fc0, 0, s.table)) return out;
    if (!s.table || !s.count) {
        out.status = Walk::Loading;
        return out;
    }
    if (s.count < 0 || s.count > 4096) {
        out.status = Walk::BadCount;
        return out;
    }
    if (s.table & 3) {
        out.status = Walk::Malformed;
    ...
    if (s.index < 0 || s.index >= s.count) {
        out.status = Walk::BadIndex;
        return out;
    }
    if (!sb::read_record(counted, s)) {
        out.status = s.status == sb::Status::ReadFailure ? Walk::ReadFailure : Walk::BadRecord;
        return out;
    }
...
inline bool other_sector(std::uint32_t id, std::uint32_t last_id) {
    return id && last_id && id != last_id;
}
...
inline const char* name(Plan p) {
    return p == Plan::AlreadyStarted  ? "already_started"
           : p == Plan::CurrentSector ? "current_sector"
           : p == Plan::Recentre      ? "recentred"
                                      : "started";
}
inline Plan plan(const Record& r, std::uint64_t key, unsigned recipe, bool resident, std::uint32_t id,
...
inline const char* name(Decision d) {
    return d == Decision::Confirmed ? "confirmed" : d == Decision::Discarded ? "discarded" : "none";
}
