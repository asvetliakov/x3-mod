...
// 004c34f7  0f 85 c9 00 00 00        JNE  0x004c35c6               <- site; rel32 at 004c34f9 -> 00 00 00 00 (JNE 0x004c34fd)
// 004c34fd  83 7c 24 74 00           CMP  dword [ESP+0x74],0       ; occlusion id; writes every flag the cmp above set
// 004c3502  89 54 24 18              MOV  [ESP+0x18],EDX           ; placeholder kept as the fallback
...
inline const char* mode_name(Mode m) { return m == Mode::all ? "all" : "record0"; }
...
inline bool parse_mode(const Char* text, Mode* out) {
    ...
    if (equals("record0")) { *out = Mode::record0; return true; }
    if (equals("all")) { *out = Mode::all; return true; }
    return false;
}
...
template <class Ops>
inline const char* install(Ops& ops, std::uintptr_t window, bool window_open, bool* live, bool* atomic, std::uint32_t* protection) {
    ...
    *live = true; // ownership published before the mutation, so a failed rollback stays registered
    ...
    const bool verified = flushed && ops.read(at, readback, write_length) && !std::memcmp(readback, patched_write, write_length);
...
template <class Ops>
inline const char* restore(Ops& ops, std::uintptr_t at, std::uint32_t protection, unsigned char found[write_length], bool* found_read) {
...
static_assert(window_va + site_offset == site_va && site_va + 2 == write_va && window_va + write_offset == write_va, "offsets");
...
static_assert(site_va + site_length == lod0_va && lod0_va + expected_write[0] == placeholder_va, "jne rel32 +0xc9 reaches the placeholder bind");
static_assert(patched_write[0] == 0 && patched_write[1] == 0 && patched_write[2] == 0 && patched_write[3] == 0, "rel32 0: the next instruction");
