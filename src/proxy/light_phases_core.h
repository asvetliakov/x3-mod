...
struct Errors {
    std::uint64_t nested = 0, overflow = 0, mismatch = 0, unmatched = 0, clock_failures = 0, clock_reversal = 0,
                  reentry = 0, mode_refused = 0;
};
struct Sample {
    bool valid = false;
...
struct Accumulator {
    ...
    void abandon() noexcept {
        errors.unmatched += depth;
        depth = 0;
        opened = 0;
    }
    void refuse_mode() noexcept {
        ++errors.mode_refused;
        poisoned = true;
        abandon();
    }
    ...
    void stamp(unsigned site, std::uint32_t token, std::uint32_t return_address, std::uint64_t now) noexcept {
        ...
        if (now && opened && now < opened) {
            ++errors.clock_reversal;
            poisoned = true;
        }
        if (!poisoned && opened) {
            pending.ticks[caller] += now - opened;
            ++pending.calls[caller];
        }
        opened = 0;
...
public:
    ...
    bool close(Summary& out) noexcept {
        ...
        out.valid_frames = valid_count_;
        out.invalid_frames = count_ - valid_count_;
        ...
        for (unsigned b = 0; b < buckets; ++b) {
            for (unsigned i = 0; i < valid_count_; ++i) {
                out.entries[b] += values[i].entries[b];
                out.calls[b] += values[i].calls[b];
                out.ticks[b] += values[i].ticks[b];
            }
