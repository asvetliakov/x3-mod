...
inline constexpr const char* const interval_names[interval_count] = {"prepare", "setup",           "particles",
                                                                     "other",   "between_prepare", "outside_setup"};
...
struct Accumulator {
    ...
    std::uint64_t prepare_skipped = 0; // no pass_end since the previous material (the frame's first, or a skipped pass
                                       // loop)
    ...
    void close_setup(std::uint64_t begin_clock, bool& begin_armed, std::uint64_t begin_submission) noexcept {
        if (!p_clock) return;
        if (!begin_armed && begin_clock >= p_clock)
            partition(1, 5, begin_clock - p_clock, p_submission, begin_submission);
        else
            ++setup_skipped;
    }
    void partition(unsigned inside, unsigned outside, std::uint64_t elapsed, std::uint64_t from,
                   std::uint64_t to) noexcept {
        if (to < from || to - from > elapsed) {
            ++scope_errors;
            return;
        }
        ticks[inside] += to - from;
        ticks[outside] += elapsed - (to - from);
    }
    void material(std::uint64_t now, std::uint64_t end_clock, std::uint64_t begin_clock, bool& begin_armed,
                  std::uint64_t submission, std::uint64_t end_submission, std::uint64_t begin_submission,
                  bool in_view) noexcept {
        ++materials;
        if (!in_view) ++outside_materials;
        if (!now) {
            ++clock_failures;
            broken = true;
            begin_armed = false;
        ...
        if (broken) {
            ...
            if (end_clock && end_clock > p_clock) {
                if (now >= end_clock)
                    partition(0, 4, now - end_clock, end_submission, submission);
                else
                    ++clock_errors;
            } else
                ++prepare_skipped;
    ...
    void discard(bool& begin_armed) noexcept {
        ...
        materials = particle_views = 0;
        p_clock = p_submission = 0;
        submit_end_seen = 0;
        broken = false;
        begin_armed = false;
    }
    ...
    void take(std::uint64_t frame, std::uint64_t frequency, std::uint64_t views_us, std::uint64_t view_setup_us,
              std::uint64_t view_submit_us, std::uint32_t views, std::uint32_t passes, std::uint64_t begin_clock,
              bool& begin_armed, Sample& out, std::uint64_t begin_submission) noexcept {
        if (!broken) close_setup(begin_clock, begin_armed, begin_submission);
        out = Sample{};
        out.frame = frame;
        out.materials = materials;
        out.particle_views = particle_views;
        out.passes = passes;
        out.views = views;
        out.views_us = views_us;
        out.view_setup_us = view_setup_us;
        out.view_submit_us = view_submit_us;
        for (unsigned i = 0; i < stamped_intervals; ++i)
        ...
        if (views_us >= attributed)
            out.interval_us[3] = views_us - attributed;
        else
            ++other_underflow;
        if (passes) {
...
public:
    ...
    void add(const Sample& s) noexcept {
        if (count_ < window_frames) {
            materials_[count_] = s.materials;
            particle_views_[count_] = s.particle_views;
            passes_[count_] = s.passes;
            views_[count_] = s.views;
            for (unsigned i = 0; i < interval_count; ++i) interval_[i][count_] = s.interval_us[i];
            per_pass_[0][count_] = s.prepare_per_pass_ns;
            per_pass_[1][count_] = s.setup_per_pass_ns;
    ...
    bool close(Summary& out) noexcept {
        ...
        out.frames = count_;
        out.materials_p50 = percentile(materials_, 50);
        out.particle_views_p50 = percentile(particle_views_, 50);
        ...
        out.prepare_per_pass_p50 = percentile(per_pass_[0], 50);
        out.setup_per_pass_p50 = percentile(per_pass_[1], 50);
...
private:
    ...
    std::uint64_t materials_[window_frames]{}, particle_views_[window_frames]{}, passes_[window_frames]{},
        views_[window_frames]{};
