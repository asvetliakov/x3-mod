...
bool client_on_screen(HWND window, RECT* out) {
    ...
    if (!MapWindowPoints(window, nullptr, reinterpret_cast<POINT*>(&r), 2) && GetLastError() != ERROR_SUCCESS)
        return false;
    *out = r;
    return true;
}
...
void initialize() {
    ...
    if (enabled_)
        log("cursor_reassert_mode requested=1 window_frames=%u arm=launch_once,WM_ACTIVATE_active,WM_ACTIVATEAPP_on gates=thread,foreground,visible,cursor_hidden,pointer_in_client_or_clip "
            "sequence=set_arrow,show_true,show_false,set_previous disable_on_mismatch=1",
            core::window_frames);
}
bool enabled() {
    return enabled_ && machine_.state != core::State::disabled;
}
void observe(UINT message, WPARAM wparam) noexcept {
    if (!enabled_) return;
    if (core::observe(machine_, message, static_cast<std::uint32_t>(wparam))) {
        arm_pending_ = true;
        armed_tick_ = GetTickCount();
    }
}
void arm_launch() noexcept {
    if (!enabled_) return;
    if (core::arm_launch(machine_)) {
        arm_pending_ = true;
        armed_tick_ = GetTickCount();
    }
}
...
core::Decision present_with(HWND window, unsigned long long frame, const core::Gates& gates) {
    ...
    if (arm_pending_) {
        arm_pending_ = false;
        log("cursor_reassert_arm frame=%llu hwnd=%p armed_by=%s message=%s arms=%u tick_ms=%lu", frame,
            static_cast<void*>(window), core::arm_source(machine_.armed_by), core::arm_message(machine_.armed_by),
            machine_.arms, static_cast<unsigned long>(armed_tick_));
    }
    const std::uint32_t armed_by = machine_.armed_by;
    const auto decision = core::present(machine_, gates);
    if (decision.step == core::Step::refuse)
        log("cursor_reassert frame=%llu hwnd=%p action=refused reason=%s window_frames=%u refusals=%u armed_by=%s",
            frame, static_cast<void*>(window), decision.reason, core::window_frames, machine_.refusals,
            core::arm_source(armed_by));
        ...
        const bool balanced = before_ok && after_ok &&
                              core::balanced(sequence, before.flags, after.flags, before.hCursor, after.hCursor);
        ...
        log("cursor_reassert frame=%llu hwnd=%p action=fired armed_by=%s before_flags=%lu before_cursor=%p previous=%p arrow=%p up=%d down=%d after_flags=%lu after_cursor=%p "
            "clip_ok=%d clip=%ld,%ld,%ld,%ld pointer_in_client=%u clip_is_client=%u balanced=%u disabled=%u fires=%u message=%s",
            frame, static_cast<void*>(window), core::arm_source(armed_by), static_cast<unsigned long>(before.flags),
            static_cast<void*>(before.hCursor), sequence.previous, sequence.arrow, sequence.up, sequence.down,
            static_cast<unsigned long>(after.flags), static_cast<void*>(after.hCursor), int(clip_ok), clip.left,
            clip.top, clip.right, clip.bottom, unsigned(gates.pointer_in_client), unsigned(gates.clip_is_client),
            unsigned(balanced), unsigned(machine_.state == core::State::disabled), machine_.fires,
            core::arm_message(armed_by));
    }
