...
// 0047d023  8b 87 2c 01 00 00             MOV  EAX,[EDI+0x12c]                ; flags dead: 0x0047d029 TEST writes them first
//
...
constexpr unsigned char expected_window[window_length] = {
    0xf7,0x87,0x2c,0x01,0x00,0x00,0x00,0x00,0x00,0x80, 0x74,0x05, 0xc6,0x44,0x24,0x18,0x01};
...
inline const char* mode_name(Mode m) { return m == Mode::distance ? "distance" : "size"; }
...
inline bool parse_mode(const Char* text, Mode* out) {
    ...
    if (equals("size")) { *out = Mode::size; return true; }
    if (equals("distance")) { *out = Mode::distance; return true; }
    return false;
}
...
inline const char* install(Ops& ops, std::uintptr_t window, bool window_open, bool* live, bool* atomic, std::uint32_t* protection) {
    ...
    *live = true; // ownership published before the mutation, so a failed rollback stays registered
    ...
    const bool verified = flushed && ops.read(site, readback, site_length) && !std::memcmp(readback, patched_site, site_length);
...
// registered.
template <class Ops>
inline const char* restore(Ops& ops, std::uintptr_t site, std::uint32_t protection, unsigned char found[site_length], bool* found_read) {
    *found_read = ops.read(site, found, site_length);
    ...
    unsigned char back[site_length]{};
    const bool verified = flushed && ops.read(site, back, site_length) && !std::memcmp(back, expected_site, site_length);
    const bool protected_again = ops.protect(site, site_length, protection, &unused);
...
static_assert(site_va + site_length + expected_site[1] == target_va && patched_site[1] == expected_site[1], "same rel8 target");
