...
void sanitize(std::string& text) {
    for (char& c : text)
        if (static_cast<unsigned char>(c) < 0x21u || static_cast<unsigned char>(c) > 0x7eu) c = '_';
...
bool hash_file(const std::wstring& path, std::string& hex, unsigned long long& bytes) {
    ...
    if (ok) {
        ...
        hex.assign(64, '0');
        for (int i = 0; i < 32; ++i) {
            hex[std::size_t(i) * 2] = table[digest[i] >> 4];
            hex[std::size_t(i) * 2 + 1] = table[digest[i] & 0xf];
        }
    }
...
std::string manifest_sha256(const std::wstring& directory) {
    ...
    for (std::size_t i = 0; i < 64; ++i)
        if (!hex_digit(text[at + i])) return "none";
...
void log_module(HMODULE module, const char* name) {
    ...
    try {
        if (module) {
            ...
            if (length && length < path.size()) {
                path.resize(length);
                path_utf8 = session_log::redact_path(utf8(path)); // profile prefix redacted before sanitize (always
                                                                  // tier)
                sanitize(path_utf8);
                std::string full;
                hex = hash_file(path, full, bytes) && full.size() == 64 ? full.substr(0, 16)
                                                                        : std::string("unavailable");
            } else
                path_utf8 = "unavailable";
        }
    ...
    const unsigned long long microseconds = frequency.QuadPart > 0 && end.QuadPart > begin.QuadPart
                                                ? static_cast<unsigned long long>((end.QuadPart - begin.QuadPart) *
    ...
    log("loaded_module name=%s path=%s size=%llu sha256=%s hash_us=%llu image_size=%lu stamp=%08lx exports=%lu wine_builtin=%d",
        name, path_utf8.c_str(), bytes, hex.c_str(), microseconds, image.size, image.stamp, image.exports,
        image.wine_builtin);
}
...
bool is_option(const wchar_t* entry) {
    return _wcsnicmp(entry, L"X3M_", 4) == 0;
}
...
std::vector<std::string> collect(bool (*select)(const wchar_t*), std::size_t limit, bool redact = false) {
    ...
    for (const wchar_t* entry = block; *entry; entry += std::wcslen(entry) + 1) {
        ...
        if (redact)
            value = _stricmp(name.c_str(), "WINEUSERNAME") == 0 ? std::string() : session_log::redact_value(value);
        sanitize(name);
        if (!value.empty()) sanitize(value);
        if (value.size() > limit) {
            value.resize(limit);
            value += "\xe2\x80\xa6";
        } // U+2026, kept out of sanitize
...
void log_identity(HMODULE self) {
    ...
    std::string hex = "unavailable", path_utf8 = "unknown", manifest = "none", option_line,
                environment_line = " count=0", exe_hex = "unavailable";
    ...
    try {
        ...
        if (length && length < path.size()) {
            ...
            manifest = manifest_sha256(separator == std::wstring::npos ? std::wstring(L".")
    ...
    const unsigned long long exe_microseconds = frequency.QuadPart > 0 && exe_end.QuadPart > exe_begin.QuadPart
                                                    ? static_cast<unsigned long long>(
                                                          (exe_end.QuadPart - exe_begin.QuadPart) * 1000000ll /
                                                          frequency.QuadPart)
                                                    : 0ull;
    ...
    const unsigned long max_app = static_cast<unsigned long>(
    ...
    log("proxy_identity sha256=%s bytes=%llu path=%s manifest_sha256=%s source_commit=%s attach_us=%llu exe_sha256=%s exe_bytes=%llu exe_hash_us=%llu exe_laa=%d exe_max_app=%08lx",
        hex.c_str(), bytes, path_utf8.c_str(), manifest.c_str(), X3M_SOURCE_COMMIT, microseconds, exe_hex.c_str(),
        exe_bytes, exe_microseconds, laa ? 1 : 0, max_app);
