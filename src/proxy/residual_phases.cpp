...
void emit_window() {
    ...
    log("residual_phases qpc=%llu frame=%llu frames=%u materials_p50=%llu particle_views_p50=%llu passes_p50=%llu views_p50=%llu prepare_p50_us=%llu prepare_p95_us=%llu setup_p50_us=%llu setup_p95_us=%llu prepare_per_pass_p50_ns=%llu setup_per_pass_p50_ns=%llu particles_p50_us=%llu particles_p95_us=%llu other_p50_us=%llu other_p95_us=%llu self_p50_us=%llu dispatch_cost_ns=%llu prepare_skipped=%llu setup_skipped=%llu view_skipped=%llu other_underflow=%llu clock_errors=%llu clock_failures=%llu unmatched=%llu dropped=%llu early=%u foreign=%u",
        emitted,s.frame,s.frames,s.materials_p50,s.particle_views_p50,s.passes_p50,s.views_p50,
        s.interval_p50[0],s.interval_p95[0],s.interval_p50[1],s.interval_p95[1],s.prepare_per_pass_p50,s.setup_per_pass_p50,
        s.interval_p50[2],s.interval_p95[2],s.interval_p50[3],s.interval_p95[3],s.self_p50,detail::dispatch_cost_ns,
...
x3m_residual_phase_enter(unsigned index) {
...
bool initialize() {
    ...
    initialized=true;
    const bool wanted=log_tier::draw_trace_flag(L"X3M_RESIDUAL_PHASES"); // X3M_RESIDUAL_PHASES=1 or X3M_DRAW_TRACE=1 (log_tiers.h)
    if(!wanted)return false;
    const char* status="telemetry_off";
    if(telemetry::enabled()){
        frequency=telemetry::frequency();
        pass_link=pass_phases::shared_accumulator();frame_link=frame_phases::shared_tracker();
        if(!frequency)status="clock_unavailable";
        else if(!frame_phases::active.load(std::memory_order_acquire))status="frame_phases_off"; // the frame boundary, the views phase and submit_end come from the frame group
        else if(!pass_phases::active.load(std::memory_order_acquire))status="pass_phases_off";   // the pass count and the pass_end/pass_begin clocks come from the pass group
    ...
    for(unsigned i=0;i<sites::Count;++i)log("residual_phase_site index=%u address=%08lx length=%u patched=%u status=%s",i,
...
namespace detail {
void frame_impl(std::uint64_t frame,bool sampled,std::uint64_t views_us,std::uint64_t view_setup_us,std::uint64_t view_submit_us,std::uint32_t views) noexcept {
    ErrorGuard error;
