...
constexpr unsigned node_reserve = 8, draw_reserve = 64; // headroom kept free by scene-end eviction, so the draw site
...
constexpr double first_seen_range = .8; // of the outermost half-extent: a node first sighted nearer the eye than this
                                        // did not enter through the box edge
...
constexpr bool excluded_class(std::uint32_t flags12c, std::uint32_t flags130) noexcept {
    return (flags12c & excluded_flags12c) || (flags130 & excluded_flags130);
}
...
constexpr unsigned idle_flush_frames = 300; // presented frames without a scene end (menus, loading) before the store is
                                            // flushed
constexpr const char* flush_name(Flush f) noexcept {
    switch (f) {
    case Flush::Epoch: return "epoch";
    case Flush::Reset: return "reset";
    case Flush::Device: return "device";
    case Flush::Teardown: return "teardown";
    case Flush::Sun: return "sun";
    case Flush::Observer: return "observer";
    case Flush::Idle: return "idle";
    default: return "none";
    }
}
enum class Expiry : std::uint8_t {
    Retired,
    BoxExit,
    Age,
    Evicted,
...
enum class Revalidation : std::uint8_t {
    Known = 0,
    Dead = 1,
    ContextLost = 2
}; // ContextLost: the node's recorded registry or camera is gone; the node cannot be confirmed
...
inline double abs_d(double v) noexcept {
    return v < 0. ? -v : v;
}
inline unsigned age_bucket(std::uint64_t frames) noexcept {
    return frames < 60 ? 0u : frames < 600 ? 1u : frames < 3600 ? 2u : frames < 14400 ? 3u : 4u;
}
...
template <unsigned Capacity> struct IndexMap {
    ...
    template <class KeyOf> void erase(std::uint64_t key, std::uint16_t index, KeyOf key_of) noexcept {
        ...
        for (; n < slots && table[i] != index; i = (i + 1) & (slots - 1), ++n)
...
struct DrawKey {
    ...
    std::uint32_t stream_offset = 0, stride = 0, topology = 0, primitives = 0, first = 0, min_vertex = 0,
                  vertex_count = 0;
    std::int32_t base_vertex = 0;
    std::uint32_t indexed = 0;
    bool operator==(const DrawKey& o) const noexcept {
        return vb == o.vb && ib == o.ib && declaration == o.declaration && stream_offset == o.stream_offset &&
               stride == o.stride && topology == o.topology && primitives == o.primitives && first == o.first &&
               min_vertex == o.min_vertex && vertex_count == o.vertex_count && base_vertex == o.base_vertex &&
               indexed == o.indexed;
    }
};
...
struct BufferStamp {
    ...
    bool same_object(const BufferStamp& o) const noexcept {
        return identity == o.identity && allocation == o.allocation && generation == o.generation;
    }
};
...
inline BufferState buffer_verdict(const BufferStamp& b, const BufferView& v, bool hold) noexcept {
    ...
    if (v.allocation != b.allocation || v.generation != b.generation)
        return hold ? BufferState::Changed : BufferState::Gone;
...
struct Node {
    ...
    std::uintptr_t node = 0, registry = 0, camera = 0; // the scope of the last sighting: the full revalidation's
                                                       // arguments
    ...
    bool used = false, is_static = false, dirty = false, fresh = false, changed = false, lod_changed = false,
         model_changed = false;
...
struct Resource {
    std::uintptr_t identity = 0;
    std::uint32_t uses = 0;
    bool used = false, owed = false;
    std::uint16_t single_node = none;
};
...
struct Sighting {
    ...
    const float* hi = nullptr; // the range's own extent of this revision, when known
};
// Resources the owner must AddRef now (at the draw, while the application's binding is live).
struct Acquired {
    std::uintptr_t identity[3]{};
    unsigned count = 0;
};
...
struct FrameStats {
    ...
    float drift_p99 = 0, drift_max = 0;
    std::uint32_t sun_relatch = 0, cam_jump = 0, transit_survivors = 0;
    std::uint32_t far_alternate_due_to_retained = 0; // the far cascade skipped by the budget this frame only because of
                                                     // retained issues (live issues alone fit)
    std::uint32_t revalidate_context_lost = 0; // nodes a full revalidation could not confirm because their recorded
                                               // registry or camera is gone
    std::uint32_t release_queue_full = 0; // acquisitions refused while owed references still hold the resource table
                                          // (diagnostic; the queue itself cannot drop a reference)
    std::uint32_t reclassified_after_unseen = 0; // a retained node resubmitted beyond eps of its stored rows: the
                                                 // moved-while-unseen residual, observed
    std::uint32_t admitted_checked = 0;          // retained records whose buffers were checked before issue this frame
    std::uint32_t buffer_views = 0; // distinct buffer identities looked up in the registry this frame (records sharing
                                    // a buffer inherit its view)
    std::uint32_t refused = 0, moving_dropped = 0, abandoned = 0, deferred = 0; // beyond the contract's list: capacity
                                                                                // refusals, moving nodes dropped
                                                                                // unseen, sightings of a frame without
                                                                                // a scene end, draws ignored while a
                                                                                // flush is pending
    std::uint32_t nodes_live = 0, nodes_unseen = 0, records = 0, records_unseen = 0, statics = 0, moving = 0; // levels
    std::uint64_t age_max = 0;
    std::uint32_t reclassified_cascade[renderer::shadow_cascade_max]{}; // nodes static at cascade c's eps that moved
                                                                        // beyond it this sighting (reclassified= is the
                                                                        // base tier)
    std::uint32_t gate_sightings = 0; // sightings the owner fed for draws the static gate refused from every cascade
                                      // (no record, no lease)
    std::uint32_t footprint_refused[renderer::shadow_cascade_max]{}; // retained records cascade c's minimum-footprint
                                                                     // gate dropped this frame (the live counterpart is
                                                                     // FrameCounts::footprint_refused)
};
...
struct Totals {
    ...
    std::uint64_t retired = 0, box_exit = 0, age = 0, evicted = 0, buffer_changed = 0, buffer_gone = 0,
                  buffer_orphaned = 0, reclassified = 0;
    std::uint64_t lod_replaced = 0, model_replaced = 0, moving_dropped = 0, revalidated = 0, journal_overflow = 0,
                  refused = 0, promoted = 0;
...
struct FrameInput {
    ...
    double eps_cascade[renderer::shadow_cascade_max] = {eps_default, eps_default, eps_default, eps_default,
                                                        eps_default}; // per cascade
                                                                      // (renderer::shadow_cascade_class_eps); never
                                                                      // below eps
...
inline void sun_rows(const double world[12], const renderer::ShadowReplayBasis& basis, double base[3][4]) noexcept {
    for (unsigned a = 0; a < 3; ++a)
        for (unsigned k = 0; k < 4; ++k)
            base[a][k] = basis.axes[a][0] * world[k] + basis.axes[a][1] * world[4 + k] +
                         basis.axes[a][2] * world[8 + k];
}
...
inline double drift2(const double a[12], const double b[12], const float lo[3], const float hi[3]) noexcept {
    ...
    for (unsigned corner = 0; corner < 8; ++corner) {
        const double p[3] = {double((corner & 1) ? hi[0] : lo[0]), double((corner & 2) ? hi[1] : lo[1]),
                             double((corner & 4) ? hi[2] : lo[2])};
        double d2 = 0;
        for (unsigned i = 0; i < 3; ++i) {
            const double d = (a[i * 4] - b[i * 4]) * p[0] + (a[i * 4 + 1] - b[i * 4 + 1]) * p[1] +
                             (a[i * 4 + 2] - b[i * 4 + 2]) * p[2] + (a[i * 4 + 3] - b[i * 4 + 3]);
            d2 += d * d;
        }
...
inline void world_bounds(const double world[12], const float lo[3], const float hi[3], double centre[3],
                         float half[3]) noexcept {
    for (unsigned i = 0; i < 3; ++i) {
        double c = world[i * 4 + 3], h = 0;
        for (unsigned j = 0; j < 3; ++j) {
            const double mid = .5 * (double(lo[j]) + double(hi[j])), ext = .5 * (double(hi[j]) - double(lo[j]));
            c += world[i * 4 + j] * mid;
            h += abs_d(world[i * 4 + j]) * ext;
        }
        centre[i] = c;
        half[i] = float(h);
    }
...
struct Store {
    ...
    std::uint16_t dirty[node_capacity];
    unsigned dirty_count = 0;
    unsigned pending_count = 0; // resource slots owed one Release by the owner (pop_owed)
    std::uint16_t admitted[draw_capacity];
    unsigned admitted_count = 0; // this frame's unseen records with a cascade (draw indices)
    float scratch_key[draw_capacity];
    std::uint16_t scratch_index[draw_capacity];
    float drift_samples[node_capacity];
    unsigned drift_count = 0;
    ...
    double eye[3]{};
    std::uint64_t transit_frame = 0;
    bool transit_open = false;
    ...
    void reset_storage() noexcept {
        ...
        node_free_count = node_capacity;
        draw_free_count = draw_capacity;
        resource_free_count = resource_capacity;
        dirty_count = pending_count = admitted_count = drift_count = 0;
        nodes_used = draws_used = resources_used = 0;
        epochs_set = false;
        flush_pending = Flush::None;
        transit_open = false;
    }
    std::uint16_t find_node(std::uint64_t serial) const noexcept {
        return node_map.find(serial, [this](std::uint16_t i) { return nodes[i].serial; });
    }
    ...
    std::uintptr_t known_declaration(std::uint64_t serial, const DrawKey& key, const BufferStamp& vb,
                                     const BufferStamp& ib) const noexcept {
        const std::uint16_t index = find_node(serial);
        if (index == none) return 0;
        for (std::uint16_t i = nodes[index].head; i != none; i = draws[i].next) {
            const Draw& d = draws[i];
            if (d.key == key && d.vb.same_object(vb) && d.ib.same_object(ib)) return d.declaration;
        }
        return 0;
    }
    ...
    Seen seen(const Sighting& s, std::uint64_t now, Acquired& acquired) noexcept {
        acquired.count = 0;
        if (excluded_class(s.flags12c, s.flags130)) {
            ++frame.excluded_class;
            return Seen::Excluded;
        }
        if (flush_pending != Flush::None) {
            ++frame.deferred;
            return Seen::Deferred;
        }
        if (!epochs_set || !nodes_used) {
            load_epoch = s.load_epoch;
            registry_epoch = s.registry_epoch;
            observer_epoch = s.observer_epoch;
            epochs_set = true;
        } else if (load_epoch != s.load_epoch || registry_epoch != s.registry_epoch ||
                   observer_epoch != s.observer_epoch) {
            // The whole store goes at this frame's scene end, before the replay; nothing is released at a draw.
            flush_pending = Flush::Epoch;
            ++frame.deferred;
            return Seen::Deferred;
        }
        ...
        if (index == none) {
            if (!node_free_count) {
                count_refused();
                return Seen::Refused;
            } // the scene end evicts into the reserve; this node is recorded from its next sighting
            index = node_free[--node_free_count];
            ++nodes_used;
            Node& n = nodes[index];
            n = Node{};
            n.used = true;
            ...
            n.model = s.model;
            n.lod = s.lod;
            n.flags12c = s.flags12c;
            n.registry = s.registry;
            n.camera = s.camera;
            n.camera_handle = s.camera_handle;
            n.first_seen = now;
            n.last_seen = ~std::uint64_t(0);
            node_map.insert(s.serial, index);
            ++frame.new_nodes;
        }
        Node& n = nodes[index];
        if (n.last_seen != now) { // the first draw of this sighting
            n.unseen_before = n.fresh ? 0 : now - n.last_seen - 1;
            n.last_seen = now;
            n.cursor = n.head;
            n.partial = false;
            if (!n.fresh) {
                n.model_changed = n.model != s.model;
                n.lod_changed = n.lod != s.lod;
            }
            n.model = s.model;
            n.lod = s.lod;
            n.node = s.node;
            ...
            if (!n.dirty) {
                n.dirty = true;
                if (dirty_count < node_capacity) dirty[dirty_count++] = index;
            }
        }
        const auto matches = [&](const Draw& d) noexcept {
            return d.stamp != now && d.key == s.key && d.vb.same_object(s.vb) && d.ib.same_object(s.ib) &&
                   (!hold || d.declaration == s.declaration);
        };
        std::uint16_t found = none;
        if (n.cursor != none && matches(draws[n.cursor]))
            found = n.cursor;
        else
        ...
        if (found != none) {
            ...
            if (rewritten) {
                ...
                ++totals.buffer_changed;
                d.vb.revision = s.vb.revision;
                d.ib.revision = s.ib.revision;
                d.streak = 0;
                std::memset(d.streak_cascade, 0, sizeof d.streak_cascade);
                if (!s.lo) d.extent_known = false;
            }
        ...
        const std::uint16_t slot = draw_free[--draw_free_count];
        ++draws_used;
        Draw& d = draws[slot];
        d = Draw{};
        d.used = true;
        ...
        if (hold) {
            ...
            for (unsigned i = 0; ok && i < 3; ++i)
                if (wanted[i]) ok = acquire(wanted[i], d.resource[i], acquired, index);
            if (!ok) { // table full or an identity missing: nothing was AddRef'd yet, so the fresh entries simply leave
                for (unsigned i = 0; i < 3; ++i)
                    if (d.resource[i] != none) unacquire(d.resource[i], acquired);
                acquired.count = 0;
                d = Draw{};
                draw_free[draw_free_count++] = slot;
                --draws_used;
                n.partial = true;
                count_refused();
                return Seen::Refused;
        ...
        if (n.tail == none)
            ...
            draws[n.tail].next = slot;
        n.tail = slot;
        ++n.draws;
        if (!n.fresh) n.changed = true;
        return Seen::New;
    }
    void set_eye(const renderer::CameraState& camera) noexcept {
        eye_valid = camera.valid;
        if (camera.valid) camera_position(camera, eye);
    }
    ...
    void flush(Flush reason) noexcept {
        for (unsigned i = 0; i < node_capacity; ++i)
            if (nodes[i].used) remove_node(std::uint16_t(i), Expiry::Flushed, 0);
        dirty_count = admitted_count = 0;
        epochs_set = false;
        flush_pending = Flush::None;
        transit_open = false;
        frame.flush = reason;
        ++totals.flushes[unsigned(reason)];
    }
    ...
    template <class Verdict> void revalidate(Verdict verdict, std::uint64_t now) noexcept {
        for (unsigned i = 0; i < node_capacity; ++i) {
            ...
            if (v == Revalidation::Dead)
                remove_node(std::uint16_t(i), Expiry::Retired, now);
            else if (v == Revalidation::ContextLost)
                remove_node(std::uint16_t(i), Expiry::ContextLost, now);
        }
    }
    ...
    void drop_resource(std::uint16_t slot, std::uint64_t now) noexcept {
        ...
        if (owner != none) {
            if (nodes[owner].used) remove_node(owner, Expiry::BufferOrphaned, now);
            return;
        }
    ...
    void abandon_sightings() noexcept {
        for (unsigned i = 0; i < dirty_count; ++i)
            if (nodes[dirty[i]].used && nodes[dirty[i]].dirty) remove_node(dirty[i], Expiry::Abandoned, 0);
        dirty_count = 0;
        admitted_count = 0;
    }
    ...
    template <class View> void end_scene(const FrameInput& in, View view) noexcept {
        ...
        for (unsigned guard = 0;
             guard < node_reserve && (node_free_count < node_reserve || draw_free_count < draw_reserve); ++guard)
            if (!evict_farthest(in.frame)) break;
        finalize_seen(in);
        const auto stamp_state = [&](const BufferStamp& b) noexcept {
            return buffer_verdict(b, buffer_view(b.identity, view), hold);
        };
        ...
        apply_caps(in);
        if (jumped) {
            transit_frame = in.frame;
            transit_open = true;
            for (auto& n : nodes) n.pre_transit = n.used;
        } else if (transit_open && in.frame >= transit_frame + transit_window) {
            for (auto& n : nodes) {
                if (n.used && n.pre_transit) ++frame.transit_survivors;
                n.pre_transit = false;
            }
            transit_open = false;
        }
...
private:
    ...
    template <class View> const BufferView& buffer_view(std::uintptr_t identity, View& view) noexcept {
        ...
        if (s.identity != identity || s.walk != walk_serial) {
            s.identity = identity;
            s.walk = walk_serial;
    ...
    static void set_extent(Draw& d, const float lo[3], const float hi[3]) noexcept {
        ...
        std::memcpy(d.lo, lo, sizeof d.lo);
        std::memcpy(d.hi, hi, sizeof d.hi);
        d.extent_known = true;
        d.bounds_dirty = true;
    }
    bool acquire(std::uintptr_t identity, std::uint16_t& slot, Acquired& acquired, std::uint16_t node) noexcept {
        const std::uint16_t known = resource_map.find(
            identity, [this](std::uint16_t i) { return std::uint64_t(resources[i].identity); });
        if (known != none) {
            ...
            if (r.owed && !r.uses) {
                ...
                --pending_count;
                r.single_node = node;
            } // the reference still held is reused: no Release owed, no AddRef needed
            else if (r.single_node != node)
                r.single_node = none;
            ++r.uses;
            slot = known;
            return true;
        }
        if (!resource_free_count) {
            if (pending_count) ++frame.release_queue_full;
            return false;
        } // owed slots hold the table until the owner releases them
        slot = resource_free[--resource_free_count];
        ++resources_used;
    ...
    void free_resource(std::uint16_t slot, bool owed) noexcept {
        ...
        r = Resource{};
        resource_free[resource_free_count++] = slot;
        --resources_used;
    }
...
private:
    ...
    void free_draw(std::uint16_t slot) noexcept {
        Draw& d = draws[slot];
        for (unsigned i = 0; i < 3; ++i)
            if (d.resource[i] != none && !--resources[d.resource[i]].uses) free_resource(d.resource[i], true);
        d = Draw{};
        draw_free[draw_free_count++] = slot;
        --draws_used;
    }
    void remove_node(std::uint16_t index, Expiry why, std::uint64_t now) noexcept {
        ...
        case Expiry::Retired:
            ++frame.retired;
            ++totals.retired;
            if (unseen_static) ++totals.expired_retired[bucket];
            break;
        case Expiry::BoxExit:
            ++frame.box_exit;
            ++totals.box_exit;
            if (unseen_static) ++totals.expired_box[bucket];
            break;
        case Expiry::Age:
            ++frame.age;
            ++totals.age;
        ...
        case Expiry::Evicted:
            ...
            ++totals.evicted;
            break;
        case Expiry::BufferChanged:
            ++frame.buffer_changed;
            ++totals.buffer_changed;
            if (unseen_static) ++totals.expired_gone[bucket];
            break;
        case Expiry::BufferGone:
            ++frame.buffer_gone;
            ++totals.buffer_gone;
            if (unseen_static) ++totals.expired_gone[bucket];
            break;
        case Expiry::BufferOrphaned:
            ++frame.buffer_orphaned;
            ++totals.buffer_orphaned;
            if (unseen_static) ++totals.expired_gone[bucket];
            break;
        case Expiry::Moving:
            ++frame.moving_dropped;
            ++totals.moving_dropped;
            break;
        case Expiry::Abandoned: ++frame.abandoned; break;
        case Expiry::ContextLost:
            ++frame.revalidate_context_lost;
            ++totals.revalidate_context_lost;
            break;
        default: break;
        }
        for (std::uint16_t i = n.head; i != none;) {
    ...
    bool note_camera(const FrameInput& in) noexcept {
        ...
        double now_eye[3];
        camera_position(in.camera, now_eye);
        bool jumped = false;
        if (eye_known_ && in.set.count) {
            double d2 = 0;
            for (unsigned k = 0; k < 3; ++k) {
                const double d = now_eye[k] - last_eye_[k];
                d2 += d * d;
            }
            const double limit = 2. * double(in.set.cascades[in.set.count - 1].half_extent);
            jumped = d2 > limit * limit;
        }
        std::memcpy(last_eye_, now_eye, sizeof last_eye_);
        eye_known_ = true;
        std::memcpy(eye, now_eye, sizeof eye);
        eye_valid = true;
    ...
    void finalize_seen(const FrameInput& in) noexcept {
        ...
        for (unsigned q = 0; q < dirty_count; ++q) {
            ...
            if (!n.draws) {
                remove_node(index, Expiry::Empty, 0);
                continue;
            }
            const bool set_changed = !n.fresh && (n.changed || n.lod_changed || n.model_changed);
            if (set_changed) ++frame.superseded;
            if (!n.fresh && n.lod_changed) {
                ++frame.lod_replaced;
                ++totals.lod_replaced;
            }
            if (!n.fresh && n.model_changed) {
                ++frame.model_replaced;
                ++totals.model_replaced;
            }
            ...
            const unsigned tiers = in.set.count < renderer::shadow_cascade_max ? in.set.count
                                                                               : renderer::shadow_cascade_max;
            ...
            for (std::uint16_t i = n.head; i != none; i = draws[i].next) {
                ...
                if (verify) {
                    double w[12];
                    if (!world_rows(in.camera, d.rows, w)) {
                        d.streak = 0;
                        std::memset(d.streak_cascade, 0, sizeof d.streak_cascade);
                        if (!d.world_valid) {
                            all_static = false;
                        ...
                        const double d2 = drift2(w, d.world, d.extent_known ? d.lo : zero,
                                                 d.extent_known ? d.hi : zero);
                        if (d2 > worst) worst = d2;
                        if (d2 > eps2) {
                            moved = true;
                            std::memcpy(d.world, w, sizeof w);
                            d.streak = 0;
                            d.bounds_dirty = true;
                        } else if (d.streak < static_sightings)
                            ++d.streak;
                        for (unsigned c = 0; c < tiers; ++c) {
                            const double e = in.eps_cascade[c];
                            if (d2 > e * e) {
                                moved_c |= std::uint8_t(1u << c);
                                d.streak_cascade[c] = 0;
                            } else if (d.streak_cascade[c] < static_sightings)
                                ++d.streak_cascade[c];
                        }
                    } else {
                        std::memcpy(d.world, w, sizeof w);
                ...
                if (d.bounds_dirty && d.complete()) {
                    world_bounds(d.world, d.lo, d.hi, d.centre, d.half);
                    d.bounds_dirty = false;
                    n.bounds_valid = false;
                }
                all_static &= d.complete() && d.streak >= static_sightings;
                for (unsigned c = 0; c < tiers; ++c)
                    if (!(d.complete() && d.streak_cascade[c] >= static_sightings))
                        static_c &= std::uint8_t(~(1u << c));
            }
            ...
            if (was_static && moved) {
                ...
                ++totals.reclassified;
                if (n.unseen_before) {
                    ++frame.reclassified_after_unseen;
                    ++totals.reclassified_after_unseen;
                }
            }
            n.is_static = all_static && !moved;
            for (unsigned c = 0; c < tiers; ++c)
                if ((n.static_mask >> c & 1u) && (moved_c >> c & 1u)) ++frame.reclassified_cascade[c];
            n.static_mask = std::uint8_t(static_c & ~moved_c);
            n.moved_mask = std::uint8_t((n.moved_mask | moved_c) & ~n.static_mask); // verified moved at a tier until
                                                                                    // promoted there again
            if (!was_static && n.is_static) {
                ++frame.promoted;
                ++totals.promoted;
            }
            if (resight) {
    ...
    void node_bounds(Node& n) noexcept {
        ...
        for (std::uint16_t i = n.head; i != none; i = draws[i].next) {
            ...
            any = true;
            for (unsigned k = 0; k < 3; ++k) {
                const double a = d.centre[k] - double(d.half[k]), b = d.centre[k] + double(d.half[k]);
                if (a < lo[k]) lo[k] = a;
                if (b > hi[k]) hi[k] = b;
            }
        }
        ...
        if (any)
            for (unsigned k = 0; k < 3; ++k) {
                n.centre[k] = .5 * (lo[k] + hi[k]);
                n.half[k] = float(.5 * (hi[k] - lo[k]));
            }
    }
    // Sun-space interval of a world AABB along axis `a` of the shared basis.
    static void sun_interval(const renderer::ShadowReplayBasis& basis, unsigned a, const double centre[3],
                             const float half[3], double& mid, double& radius) noexcept {
        mid = radius = 0;
        for (unsigned j = 0; j < 3; ++j) {
            mid += basis.axes[a][j] * centre[j];
            radius += abs_d(basis.axes[a][j]) * double(half[j]);
        }
    }
    ...
    static double sun_side(const renderer::ShadowReplayBasis& basis, unsigned a, const double world[12],
...
public:
    // The world AABB wholly inside the view frustum (its bounding sphere against the four side planes and the eye
    // plane).
    static bool inside_frustum(const renderer::CameraState& c, const double centre[3], const float half[3]) noexcept {
        double v[3], w[3];
        const double radius = renderer::sqrt_sd(double(half[0]) * half[0] + double(half[1]) * half[1] +
                                                double(half[2]) * half[2]);
        for (unsigned j = 0; j < 3; ++j) {
            w[j] = 0;
            for (unsigned i = 0; i < 3; ++i) w[j] += centre[i] * double(c.r[i * 3 + j]);
            v[j] = w[j] + double(c.t[j]);
        }
        if (v[2] - radius <= 0.) return false;
        const double m00 = double(c.m00), m11 = double(c.m11);
        const double side_x = (v[2] - abs_d(v[0]) * m00) / renderer::sqrt_sd(1. + m00 * m00),
                     side_y = (v[2] - abs_d(v[1]) * m11) / renderer::sqrt_sd(1. + m11 * m11);
...
private:
    template <class Check> void walk_unseen(const FrameInput& in, Check check) noexcept {
        ...
        bool shared_axes[renderer::shadow_cascade_max]; // cascade c has cascade 0's axes (always, under the latched
                                                        // sun)
        for (unsigned c = 0; c < renderer::shadow_cascade_max; ++c)
            shared_axes[c] = c >= cascades || !c || renderer::shadow_replay_axes_equal(in.bases[c], in.bases[0]);
        for (unsigned c = 0; c < cascades; ++c)
            for (unsigned a = 0; a < 3; ++a) {
                double m = 0;
                for (unsigned j = 0; j < 3; ++j) m += in.bases[c].axes[a][j] * in.bases[c].center_d[j];
                centre_sun[c][a] = m;
            }
        for (unsigned index = 0; index < node_capacity; ++index) {
            ...
            if (!n.is_static || !n.bounds_valid) {
                remove_node(std::uint16_t(index), Expiry::Moving, in.frame);
                continue;
            }
            const std::uint64_t age = in.frame - n.last_seen;
            if (age > in.age_cap) {
                remove_node(std::uint16_t(index), Expiry::Age, in.frame);
                continue;
            }
            if (cascades) { // box exit: outside 2 x the outermost cascade box about its snapped centre
                ...
                const double e = 2. * double(outer.half_extent), towards = 2. * double(outer.depth_toward_light),
                             behind = 2. * double(outer.depth_behind);
                bool outside = false;
                for (unsigned a = 0; a < 3 && !outside; ++a) {
                    double mid, radius;
                    sun_interval(in.bases[cascades - 1], a, n.centre, n.half, mid, radius);
                    const double s = mid - centre_sun[cascades - 1][a];
                    outside = a < 2 ? (s - radius > e || s + radius < -e)
                                    : (s - radius > behind || s + radius < -towards);
                }
                if (outside) {
                    remove_node(std::uint16_t(index), Expiry::BoxExit, in.frame);
                    continue;
                }
            }
            if ((index + in.frame) % check_period == 0) {
                BufferState worst = BufferState::Quiet;
                for (std::uint16_t i = n.head; i != none && worst != BufferState::Gone; i = draws[i].next) {
                    const BufferState s = check(static_cast<const Draw&>(draws[i]));
                    if (unsigned(s) > unsigned(worst)) worst = s;
                }
                if (worst != BufferState::Quiet) {
                    remove_node(std::uint16_t(index),
                                worst == BufferState::Gone ? Expiry::BufferGone : Expiry::BufferChanged, in.frame);
                    continue;
                }
            }
            if (cascades) {
                ...
                if (issue_state != BufferState::Quiet) {
                    remove_node(std::uint16_t(index),
                                issue_state == BufferState::Gone ? Expiry::BufferGone : Expiry::BufferChanged,
                                in.frame);
                    continue;
                }
            }
            ...
            ++frame.statics;
            frame.records += n.draws;
            frame.records_unseen += n.draws;
            if (age > frame.age_max) frame.age_max = age;
            if (in.camera.valid) {
                if (inside_frustum(in.camera, n.centre, n.half))
                    ++frame.unseen_in_frustum;
                else
                    ++frame.unseen_outside;
            }
            ...
            for (std::uint16_t i = n.head; i != none; i = draws[i].next) {
                ...
                int computed = -1; // the cascade whose axes mid/radius (and, with the gate on, sides) hold (0 for every
                                   // cascade sharing cascade 0's)
                for (unsigned c = 0; c < cascades; ++c) {
                    if (!renderer::shadow_cascade_active(in.set, c))
                        continue; // dropped by the ladder: no record carries it
                    const int want = !c || !shared_axes[c] ? int(c) : 0;
                    if (want != computed) {
                        for (unsigned a = 0; a < 3; ++a)
                            sun_interval(in.bases[want], a, d.centre, d.half, mid[a], radius[a]);
                        // The gate's measure, once per axes group and only while the option is on.
                        if (in.footprint.count)
                            for (unsigned a = 0; a < 2; ++a)
                                sides[a] = sun_side(in.bases[want], a, d.world, d.lo, d.hi);
                        computed = want;
                    }
                    const auto& box = in.set.cascades[c];
                    const double e = double(box.half_extent), x = mid[0] - centre_sun[c][0],
                                 y = mid[1] - centre_sun[c][1], z = mid[2] - centre_sun[c][2];
                    // As the draw-time mask: the light side is open (a caster nearer the light is pancaked).
                    if (x + radius[0] >= -e && x - radius[0] <= e && y + radius[1] >= -e && y - radius[1] <= e &&
                        z - radius[2] <= double(box.depth_behind)) {
                        ...
                        if (renderer::shadow_cascade_footprint_drops(in.footprint, c, side)) {
                            ++frame.footprint_refused[c];
                            continue;
                        }
                        d.cascades |= std::uint8_t(1u << c);
                        ++frame.would[c];
                    }
                }
    ...
    void apply_caps(const FrameInput& in) noexcept {
        ...
        for (unsigned c = 0; c < cascades; ++c) {
            ...
            for (unsigned q = 0; q < admitted_count; ++q) {
                ...
                double d2 = 0;
                for (unsigned k = 0; k < 3; ++k) {
                    const double v = d.centre[k] - eye[k];
                    d2 += v * v;
                }
                scratch_key[admitted[q]] = float(d2);
                scratch_index[count++] = admitted[q];
            }
            const unsigned keep = in.room[c];
            std::nth_element(scratch_index, scratch_index + keep, scratch_index + count,
                             [this](std::uint16_t a, std::uint16_t b) {
                                 return scratch_key[a] < scratch_key[b] || (scratch_key[a] == scratch_key[b] && a < b);
                             });
        ...
        unsigned kept = 0;
        for (unsigned q = 0; q < admitted_count; ++q)
            if (draws[admitted[q]].cascades) admitted[kept++] = admitted[q];
        admitted_count = kept;
    }
    double last_eye_[3]{};
    bool eye_known_ = false;
};
} // namespace x3m::shadow_retention
