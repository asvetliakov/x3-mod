...
struct Pending {
    std::uint32_t thread = 0, ebx = 0, ebp = 0;
...
void pose(std::uint32_t cockpit, std::uint32_t ship, std::uint32_t camera, Pose& p) {
    ...
    if (field(0x00606f38, 0, defaults)) {
        ...
        if (field(defaults, 0, screen) && field(screen, 4, dimensions) && dimensions[0] > 0 && dimensions[1] > 0) {
            p.screen[0] = std::uint32_t(dimensions[0]);
            p.screen[1] = std::uint32_t(dimensions[1]);
            p.valid |= 1024;
        }
    }
}
...
void admission(Event& e) {
    ...
    if ((e.valid & 4) && field(e.cockpit, 0x10, e.view_object) && field(e.cockpit, 0x1d8, e.aim_gun) &&
        field(e.cockpit, 0x150, e.mode) && field(e.cockpit, 0x58, e.camera))
        e.valid |= 8;
    ...
    if (!(e.args[3] & 2))
        e.gate = 1;
    else if (!(e.args[3] & 0x20))
        e.gate = 2;
    else if (!(e.valid & 1))
        e.gate = 8;
    else if (!e.cursor_active)
        e.gate = 3;
    else if (!(e.valid & 4))
        e.gate = 4;
    else if (!(e.valid & 8))
        e.gate = 8;
    else if (e.view_object != e.args[0])
        e.gate = 5;
    else if (!(e.valid & 16))
        e.gate = 8;
    else if (e.aim_gun < 0 || e.aim_gun >= e.gun_count)
        e.gate = 6;
    else if (e.mapped_gun != std::int32_t(e.args[2]))
        e.gate = 7;
             ...
             e.context.ship != e.args[0] || !e.context.camera || e.context.camera != e.camera || !age_valid)
        e.gate = 9;
}
void observe(unsigned phase, std::uint32_t* regs) {
    ...
    if (phase == 3) {
        ...
        CursorUpdate w{};
        w.sequence = ++writer_sequence;
        w.qpc = stamp;
        w.pose_sequence = context.sequence;
        w.mode = context.mode;
        w.thread = GetCurrentThreadId();
        w.active = std::int32_t(regs[6]);
        w.x = std::int32_t(regs[5]);
        w.y = std::int32_t(regs[7]);
        ++counts.writer_updates;
        if (w.active) ++counts.writer_on;
        if (last_cursor_write.sequence && w.active != last_cursor_write.active) ++counts.writer_changes;
        if (last_cursor_write.sequence && (w.x != last_cursor_write.x || w.y != last_cursor_write.y))
            ++counts.writer_moves;
        const unsigned bucket = !context.ship ? 0 : context.mode == 1 ? 1 : 2;
        if (!writer_first[bucket].sequence) writer_first[bucket] = w;
        writer_last[bucket] = w;
        last_cursor_write = w;
        const auto end = now();
        const auto ticks = end > stamp ? end - stamp : 0;
        ++counts.handler_calls;
        counts.ticks += ticks;
    ...
    if (phase == 0) {
        ...
        if (p) {
            p->ebx = ebx;
            p->ebp = ebp;
            p->serial = ++event_serial;
            p->slot = -1;
        } else {
            ++event_serial;
            ++counts.thread_overflow;
        }
        Event e{};
        e.serial = event_serial;
        e.thread = thread;
        e.ebx = ebx;
        e.ebp = ebp;
        e.entry_qpc = stamp;
        e.context = context;
        e.last_cursor_write = last_cursor_write;
        if (!field(ebx, 8, e.args)) {
        ...
        if (p && p->ebx == ebx && p->ebp == ebp && p->slot < 0) {
        ...
        } else if (!p || p->ebx != ebx || p->ebp != ebp || unsigned(p->slot) >= used ||
                   events[p->slot].serial != p->serial) {
            ++counts.orphans;
        } else {
            ...
            if (phase == 1) {
                e.admitted = true;
                e.ray_qpc = stamp;
                e.ray_length = std::int32_t(regs[7]);
...
extern "C" __attribute__((force_align_arg_pointer)) void __cdecl x3m_chase_aim_enter(std::uint32_t* regs,
                                                                                     unsigned phase) {
...
void* emit(unsigned phase, void*** next_out) {
    ...
    e.byte(0x50);
    e.byte(0xe8);
    e.rel32(reinterpret_cast<const void*>(&x3m_chase_aim_enter));
    e.byte(0x83);
    e.byte(0xc4);
    e.byte(8);
    for (unsigned i = 0; i < 8; ++i) {
...
void log_pose(std::uint64_t frame, std::uint64_t serial, const char* which, const Pose& p) {
    log("chase_aim_pose frame=%llu serial=%llu which=%s valid=0x%lx ship_id=0x%lx camera_pos=%ld,%ld,%ld ship_pos=%ld,%ld,%ld fov=%lu viewport=%lu,%lu,%lu,%lu plane=%lu,%lu default_plane=%lu,%lu screen=%lu,%lu",
        ...
        static_cast<long>(p.ship_pos[0]), static_cast<long>(p.ship_pos[1]), static_cast<long>(p.ship_pos[2]),
        static_cast<unsigned long>(p.fov), static_cast<unsigned long>(p.viewport[0]),
        static_cast<unsigned long>(p.viewport[1]), static_cast<unsigned long>(p.viewport[2]),
        static_cast<unsigned long>(p.viewport[3]), static_cast<unsigned long>(p.plane[0]),
        static_cast<unsigned long>(p.plane[1]), static_cast<unsigned long>(p.default_plane[0]),
        static_cast<unsigned long>(p.default_plane[1]), static_cast<unsigned long>(p.screen[0]),
        static_cast<unsigned long>(p.screen[1]));
    const std::int32_t* matrices[] = {p.camera_basis, p.relative, p.ship_basis};
    const char* names[] = {"camera", "relative", "ship"};
    for (unsigned m = 0; m < 3; ++m) {
        const auto* a = matrices[m];
        log("chase_aim_basis frame=%llu serial=%llu which=%s matrix=%s rows=%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld", frame,
            serial, which, names[m], static_cast<long>(a[0]), static_cast<long>(a[1]), static_cast<long>(a[2]),
            static_cast<long>(a[4]), static_cast<long>(a[5]), static_cast<long>(a[6]), static_cast<long>(a[8]),
            static_cast<long>(a[9]), static_cast<long>(a[10]));
    }
}
}
bool initialize() {
    ...
    initialized = true;
    if (!telemetry::enabled() || !chase_camera::installed() || !chase_camera::wanted()) {
        SetLastError(error);
        return false;
    }
    ...
    if (!QueryPerformanceFrequency(&frequency) || frequency.QuadPart <= 0) {
        okay = false;
        install_status = "no_qpc_frequency";
    }
    ...
    for (unsigned i = 0; i < site_count && okay; ++i) {
        ...
        if (okay) {
            ...
            if (!stub || !next) {
                okay = false;
                failure = "stub_failed";
            } else if (!engine_patch::store_pointer(next, *sites[i].entry)) {
                okay = false;
                failure = "chain_pointer_failed";
            } else if (!engine_patch::push_front(sites[i], stub)) {
                okay = false;
    ...
    if (!okay) {
        bool restored = true;
        for (auto& site : sites)
            if (site.patched_in) restored = engine_patch::restore(site) && restored;
    ...
    log("chase_aim_trace installed=%u status=%s sites=%u samples_per_report=%u scope=read_only_player_fire lifetime=process",
        unsigned(okay), install_status,
        unsigned(sites[0].patched_in) + unsigned(sites[1].patched_in) + unsigned(sites[2].patched_in) +
            unsigned(sites[3].patched_in),
        capacity);
    SetLastError(error);
    return okay;
}
...
void camera_context(std::uintptr_t cockpit, std::uintptr_t ship, std::uintptr_t camera, std::uint32_t mode,
                    bool applied, std::uint64_t handler_frame) {
    if (!enabled.load(std::memory_order_acquire)) return;
    const DWORD error = GetLastError();
    AcquireSRWLockExclusive(&lock);
    ++context.sequence;
    context.qpc = now();
    context.cockpit = std::uint32_t(cockpit);
    context.ship = std::uint32_t(ship);
    context.camera = std::uint32_t(camera);
    context.mode = mode;
    context.applied = applied;
    context.handler_frame = handler_frame;
...
void report(std::uint64_t frame) {
    if (!enabled.load(std::memory_order_acquire)) return;
    Counts c;
    Event sample[capacity];
    CursorUpdate first[3], last[3];
    unsigned n;
    AcquireSRWLockExclusive(&lock);
    c = counts;
    counts = {};
    n = used;
    std::memcpy(first, writer_first, sizeof first);
    std::memcpy(last, writer_last, sizeof last);
    for (unsigned i = 0; i < 3; ++i) {
        writer_first[i] = {};
        writer_last[i] = {};
    }
    std::memcpy(sample, events, n * sizeof(Event));
    used = 0;
    ...
    log("chase_aim_window frame=%llu entries=%llu rays=%llu finals=%llu samples=%u dropped=%llu orphans=%llu read_failures=%llu sampled_cone_clamped=%llu thread_overflow=%llu omitted_followups=%llu gates=%llu,%llu,%llu,%llu,%llu,%llu,%llu,%llu,%llu,%llu timing_scope=player_fire_and_cursor_writer_after_first_qpc excludes=stub_cpu_boundary_first_qpc_and_fire_player_filter handler_calls=%llu handler_ticks=%llu max_ticks=%llu frequency=%llu",
        frame, c.entries, c.rays, c.finals, n, c.dropped, c.orphans, c.read_failures, c.cone_clamped, c.thread_overflow,
        c.omitted_followups, c.gates[0], c.gates[1], c.gates[2], c.gates[3], c.gates[4], c.gates[5], c.gates[6],
        c.gates[7], c.gates[8], c.gates[9], c.handler_calls, c.ticks, c.max_ticks, std::uint64_t(frequency.QuadPart));
    log("chase_cursor_window frame=%llu updates=%llu active=%llu inactive=%llu active_changes=%llu moves=%llu", frame,
        c.writer_updates, c.writer_on, c.writer_updates - c.writer_on, c.writer_changes, c.writer_moves);
    for (unsigned b = 0; b < 3; ++b)
        for (unsigned k = 0; k < 2; ++k) {
            const auto& w = k ? last[b] : first[b];
            if (!w.sequence) continue;
            log("chase_cursor_update frame=%llu bucket=%u which=%s seq=%llu qpc=%llu pose_seq=%llu mode=%lu thread=%lu cursor=%ld,%ld,%ld",
                frame, b, k ? "last" : "first", w.sequence, w.qpc, w.pose_sequence, static_cast<unsigned long>(w.mode),
                static_cast<unsigned long>(w.thread), static_cast<long>(w.active), static_cast<long>(w.x),
                static_cast<long>(w.y));
        }
    for (unsigned i = 0; i < n; ++i) {
        const auto& e = sample[i];
        log("chase_aim_writer frame=%llu serial=%llu writer_seq=%llu writer_qpc=%llu writer_pose_seq=%llu writer_mode=%lu writer_thread=%lu cursor=%ld,%ld,%ld",
            frame, e.serial, e.last_cursor_write.sequence, e.last_cursor_write.qpc, e.last_cursor_write.pose_sequence,
            static_cast<unsigned long>(e.last_cursor_write.mode),
            static_cast<unsigned long>(e.last_cursor_write.thread), static_cast<long>(e.last_cursor_write.active),
            static_cast<long>(e.last_cursor_write.x), static_cast<long>(e.last_cursor_write.y));
        log("chase_aim_event frame=%llu serial=%llu thread=%lu ebx=0x%lx ebp=0x%lx ship=0x%lx target=0x%lx gun=%lu fire_flags=0x%lx mask=0x%lx valid=0x%lx gate=%lu cursor=%ld,%ld,%ld cockpit=0x%lx view_object=0x%lx mode=%lu aim_gun=%ld gun_count=%ld mapped_gun=%ld admitted=%u finals=%lu cursor_marker=%ld pose_seq=%llu pose_handler_frame=%llu pose_qpc=%llu entry_qpc=%llu ray_qpc=%llu final_qpc=%llu pose_mode=%lu pose_applied=%u pose_age_ticks=%llu pose_age_limit_ms=2000 camera=0x%lx picked_target=0x%lx",
            frame, e.serial, static_cast<unsigned long>(e.thread), static_cast<unsigned long>(e.ebx),
            static_cast<unsigned long>(e.ebp), static_cast<unsigned long>(e.args[0]),
            static_cast<unsigned long>(e.args[1]), static_cast<unsigned long>(e.args[2]),
            static_cast<unsigned long>(e.args[3]), static_cast<unsigned long>(e.args[4]),
            static_cast<unsigned long>(e.valid), static_cast<unsigned long>(e.gate), static_cast<long>(e.cursor_active),
            static_cast<long>(e.cursor_x), static_cast<long>(e.cursor_y), static_cast<unsigned long>(e.cockpit),
            static_cast<unsigned long>(e.view_object), static_cast<unsigned long>(e.mode), static_cast<long>(e.aim_gun),
            static_cast<long>(e.gun_count), static_cast<long>(e.mapped_gun), unsigned(e.admitted),
            static_cast<unsigned long>(e.finals), static_cast<long>(e.cursor_marker), e.context.sequence,
            e.context.handler_frame, e.context.qpc, e.entry_qpc, e.ray_qpc, e.final_qpc,
            static_cast<unsigned long>(e.context.mode), unsigned(e.context.applied), e.pose_age_ticks,
            static_cast<unsigned long>(e.camera), static_cast<unsigned long>(e.picked_target));
        log("chase_aim_ray frame=%llu serial=%llu depth=%ld length=%ld ray=%ld,%ld,%ld cone=%ld,%ld,%ld clamped=%u origin=%ld,%ld,%ld first_dir=%ld,%ld,%ld final_dir=%ld,%ld,%ld muzzle=%ld,%ld,%ld endpoint=%ld,%ld,%ld",
            frame, e.serial, static_cast<long>(e.depth), static_cast<long>(e.ray_length), static_cast<long>(e.ray[0]),
            static_cast<long>(e.ray[1]), static_cast<long>(e.ray[2]), static_cast<long>(e.cone[0]),
            static_cast<long>(e.cone[1]), static_cast<long>(e.cone[2]), unsigned(e.cone_clamped),
            static_cast<long>(e.group_origin[0]), static_cast<long>(e.group_origin[1]),
            static_cast<long>(e.group_origin[2]), static_cast<long>(e.first_direction[0]),
            static_cast<long>(e.first_direction[1]), static_cast<long>(e.first_direction[2]),
            static_cast<long>(e.direction[0]), static_cast<long>(e.direction[1]), static_cast<long>(e.direction[2]),
            static_cast<long>(e.muzzle[0]), static_cast<long>(e.muzzle[1]), static_cast<long>(e.muzzle[2]),
            static_cast<long>(e.endpoint[0]), static_cast<long>(e.endpoint[1]), static_cast<long>(e.endpoint[2]));
        log_pose(frame, e.serial, "camera_event", e.context.pose);
        log_pose(frame, e.serial, "fire_event", e.fire_pose);
    }
}
}
