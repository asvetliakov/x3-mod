...
public:
    ...
    bool frame(std::uint64_t now, std::uint64_t draws) noexcept {
        ...
        for (const Bucket& bucket : buckets_) { frames += bucket.frames; sum_draws += bucket.draws; ticks += bucket.ticks; }
        head_ = (head_ + 1) % bucket_count; buckets_[head_] = Bucket{}; bucket_start_ = now;
        if (!frames || !ticks) return false;
        const double seconds = double(ticks) / double(frequency_);
        format(line_, sizeof line_, double(frames) / seconds, seconds * 1000.0 / double(frames), (sum_draws + frames / 2) / frames);
        return true;
    }
