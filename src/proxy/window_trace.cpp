...
Entry& push(Kind kind) {
    ...
    e = Entry{};
    e.slot = Slot::pending; e.kind = kind; e.tick = GetTickCount(); e.frame = current_frame_;
    ++ring_next_;
    return e;
}
...
void record_return(const CWPRETSTRUCT& m) {
    ...
    if (core::transition_message(message)) {
        ...
        e.seq = ++message_seq_; e.depth = depth_; e.message = message; e.hwnd = m.hwnd; e.wparam = m.wParam; e.lparam = m.lParam; e.result = m.lResult;
        if (m.hwnd == target_) transition_pending_ = true;
    } else if (message == core::wm_setcursor) {
        ++frame_setcursor_;
        const std::uint32_t hit = static_cast<std::uint32_t>(m.lParam) & 0xffffu, trigger = (static_cast<std::uint32_t>(m.lParam) >> 16) & 0xffffu;
        ...
        if (core::setcursor_row(setcursor_, GetTickCount(), static_cast<std::uint32_t>(m.lResult), hit, trigger)) {
            ...
            e.seq = ++message_seq_; e.depth = depth_; e.message = message; e.hwnd = m.hwnd; e.wparam = m.wParam; e.lparam = m.lParam; e.result = m.lResult;
            e.a = hit; e.b = trigger; e.c = suppressed; setcursor_.suppressed = 0;
        }
    }
}
void write(const Entry& e) {
    ...
    case Kind::message:
        log("window_msg seq=%u depth=%u frame=%llu tick_ms=%lu hwnd=%p msg=%04x name=%s wparam=%08lx lparam=%08lx result=%08lx",
            e.seq, e.depth, e.frame, static_cast<unsigned long>(e.tick), static_cast<void*>(e.hwnd), e.message, core::message_name(e.message),
            static_cast<unsigned long>(e.wparam), static_cast<unsigned long>(e.lparam), static_cast<unsigned long>(e.result));
        break;
    case Kind::setcursor:
        log("window_msg seq=%u depth=%u frame=%llu tick_ms=%lu hwnd=%p msg=0020 name=WM_SETCURSOR wparam=%08lx hit=%u trigger=%04x result=%08lx suppressed=%u",
            e.seq, e.depth, e.frame, static_cast<unsigned long>(e.tick), static_cast<void*>(e.hwnd), static_cast<unsigned long>(e.wparam), e.a, e.b,
            static_cast<unsigned long>(e.result), e.c);
        break;
    case Kind::frame:
    ...
    case Kind::cursor_set:
        log("cursor_call seq=%u frame=%llu tick_ms=%lu thread=%u op=set handle=%08x previous=%08x count=%u",
            e.seq, e.frame, static_cast<unsigned long>(e.tick), e.d, e.a, e.b, e.e);
        break;
    case Kind::cursor_pos:
        log("cursor_call seq=%u frame=%llu tick_ms=%lu thread=%u op=pos x=%d y=%d result=%u count=%u",
            e.seq, e.frame, static_cast<unsigned long>(e.tick), e.d, static_cast<int>(e.a), static_cast<int>(e.b), e.c, e.e);
        break;
    }
}
void flush(const char* reason, unsigned long long frame) {
    ...
    for (std::uint32_t i = first; i != ring_next_; ++i) {
        ...
        e.slot = Slot::written;
        if (now - e.tick > core::ring_ms) { ++expired; continue; }
        write(e); ++written;
    }
    log("window_trace_flush frame=%llu reason=%s written=%u expired=%u overwritten=%u", frame, reason, written, expired, overwritten_);
    overwritten_ = 0;
}
struct Snapshot {
...
void snapshot(HWND window, unsigned long long device, unsigned long long frame) {
    ...
    s.foreground = GetForegroundWindow(); s.focus = GetFocus(); s.active = GetActiveWindow(); s.capture = GetCapture();
    s.visible = IsWindowVisible(window); s.iconic = IsIconic(window);
    s.style = GetWindowLongW(window, GWL_STYLE); s.exstyle = GetWindowLongW(window, GWL_EXSTYLE);
    CURSORINFO info{}; info.cbSize = sizeof info;
    s.cursor_ok = GetCursorInfo(&info);
    if (s.cursor_ok) { s.cursor_flags = info.flags; s.cursor = info.hCursor; s.point = info.ptScreenPos; }
    s.clip_ok = GetClipCursor(&s.clip);
    GetWindowRect(window, &s.window); GetClientRect(window, &s.client);
    ...
    if (s.monitor_ok) { s.monitor = mi.rcMonitor; s.work = mi.rcWork; }
    ...
    log("cursor_snapshot device=%llu frame=%llu burst=%u foreground=%p focus=%p active=%p capture=%p visible=%d iconic=%d style=%08lx exstyle=%08lx "
        ...
        device, frame, burst_index_ - 1, static_cast<void*>(s.foreground), static_cast<void*>(s.focus), static_cast<void*>(s.active), static_cast<void*>(s.capture),
        s.visible, s.iconic, static_cast<unsigned long>(s.style), static_cast<unsigned long>(s.exstyle), s.cursor_ok, static_cast<unsigned long>(s.cursor_flags),
        static_cast<void*>(s.cursor), s.point.x, s.point.y, s.clip_ok, s.clip.left, s.clip.top, s.clip.right, s.clip.bottom,
        s.window.left, s.window.top, s.window.right, s.window.bottom, s.client_origin.x, s.client_origin.y, s.client.right, s.client.bottom,
        s.monitor_ok, s.monitor.left, s.monitor.top, s.monitor.right, s.monitor.bottom, s.work.left, s.work.top, s.work.right, s.work.bottom);
}
void drain_cursor_events(core::CursorCounts& counts) {
    ...
    for (unsigned i = 0; i < n; ++i) {
        ...
        e.tick = c.tick; e.seq = c.seq; e.a = c.a; e.b = c.b; e.c = c.result; e.d = c.thread; e.e = c.op == 0 ? counts.set : counts.pos;
    }
    cursor_seen_ = newest;
}
...
void attach(HWND window, unsigned long long device) {
    ...
    if (observed_.installed) {
        ...
        log("window_trace_hooks device=%llu window=%p installed=0 reason=already_hooked hooked_device=%llu hook_thread=%lu thread=%lu", device,
            static_cast<void*>(window), hook_device_, static_cast<unsigned long>(hook_thread_), static_cast<unsigned long>(thread));
        SetLastError(saved_error);
        return;
    }
    if (!window_thread || window_thread != thread) {
        log("window_trace_hooks device=%llu window=%p window_thread=%lu render_thread=%lu installed=0 reason=%s trace=%u reassert=%u", device, static_cast<void*>(window),
            static_cast<unsigned long>(window_thread), static_cast<unsigned long>(thread), window_thread ? "foreign_thread" : "no_window", unsigned(enabled_), unsigned(reassert));
    ...
    hook_thread_ = thread; target_ = window;
    DWORD error = 0;
    call_hook_ = SetWindowsHookExW(WH_CALLWNDPROC, x3m_window_hook_call, nullptr, thread);
    if (!call_hook_) error = GetLastError();
    if (enabled_ && call_hook_) { ret_hook_ = SetWindowsHookExW(WH_CALLWNDPROCRET, x3m_window_hook_ret, nullptr, thread); if (!ret_hook_) error = GetLastError(); }
    if (enabled_ && ret_hook_) { get_hook_ = SetWindowsHookExW(WH_GETMESSAGE, x3m_window_hook_get, nullptr, thread); if (!get_hook_) error = GetLastError(); }
    ...
    if (!complete) { // all or none
        ...
        enabled_ = false; cursor_source_ = nullptr;
    }
...
void detach(unsigned long long device) {
    ...
    if (GetCurrentThreadId() != hook_thread_) {
        ...
        hook_device_ = 0; orphaned_ = true;
        log("window_trace_hooks device=%llu removed=0 reason=foreign_release release_thread=%lu hook_thread=%lu action=left_for_shutdown", device,
            static_cast<unsigned long>(GetCurrentThreadId()), static_cast<unsigned long>(hook_thread_));
        SetLastError(saved_error);
        return;
    }
    const HHOOK get = take(get_hook_), ret = take(ret_hook_), call = take(call_hook_);
...
void present(HWND window, unsigned long long device, unsigned long long frame, unsigned long long markers) {
    ...
    if (GetCurrentThreadId() != hook_thread_) {
        if (!thread_mismatch_logged_) {
            ...
            log("window_trace_thread_mismatch device=%llu frame=%llu present_thread=%lu hook_thread=%lu action=skip", device, frame,
                static_cast<unsigned long>(GetCurrentThreadId()), static_cast<unsigned long>(hook_thread_));
        }
        SetLastError(saved_error);
        return;
...
extern "C" LRESULT CALLBACK x3m_window_hook_ret(int code, WPARAM wparam, LPARAM lparam) {
    ...
    if (code == HC_ACTION && lparam) {
        ...
        ++observed_.rets; observed_.last_message = m.message; observed_.last_result = m.lResult;
        if (depth_) --depth_;
...
extern "C" LRESULT CALLBACK x3m_window_hook_get(int code, WPARAM wparam, LPARAM lparam) {
    ...
    if (code == HC_ACTION && wparam == PM_REMOVE && lparam) {
        ...
        ++observed_.gets;
        if (m.message == x3m::window_trace::core::wm_mousemove) ++frame_mousemove_;
        else if (m.message == x3m::window_trace::core::wm_ncmousemove) ++frame_ncmousemove_;
    }
    cpu.before_original();
    const LRESULT result = CallNextHookEx(nullptr, code, wparam, lparam);
    cpu.after_original();
