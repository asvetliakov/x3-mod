...
inline bool same(const Snapshot& a, const Snapshot& b) noexcept {
    return a.renderer && a.record && a.manager && a.device && a.device == a.manager_device &&
           b.device == b.manager_device && a.renderer == b.renderer && a.record == b.record && a.device == b.device &&
           a.manager == b.manager;
}
...
template <class Reader> Result collect(std::uintptr_t base, Snapshot& out, Reader&& reader) noexcept {
    out = {};
    ...
    Snapshot value{};
    if (!reader(static_cast<std::uint32_t>(base) + renderer_rva, &value.renderer, sizeof(value.renderer)))
    ...
    value.manager = fields[1];
    if (!value.record || value.record > last - 3u || !value.manager || value.manager > last - 7u)
