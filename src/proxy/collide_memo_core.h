...
constexpr std::uintptr_t caller_va = 0x0047f1b0, query_va = 0x004e2780, descent_va = 0x004e2530, leaf_va = 0x004e2190,
                         triangle_va = 0x004e2a50;
constexpr std::uintptr_t sat_va = 0x004e3280, matrix_helpers_va = 0x004e1ff0, vector_helpers_va = 0x004dfd80,
                         ftol_va = 0x0052b5d0;
constexpr unsigned call_length = 5, memo_pre_length = 28, memo_post_length = 19;
constexpr unsigned caller_length = 0x179, target_length = 0x60, query_length = 0x1e9, descent_length = 0x247,
                   leaf_length = 0x39e, triangle_length = 0x830;
constexpr unsigned sat_length = 0x62e, matrix_helpers_length = 0x197, vector_helpers_length = 0x163, ftol_length = 0xab;
constexpr unsigned char memo_pre_window[memo_pre_length] = {0x8b, 0x84, 0x24, 0x94, 0x00, 0x00, 0x00, 0x51, 0x8d, 0x4c,
                                                            0x24, 0x34, 0xd9, 0x1c, 0x24, 0x51, 0x8b, 0x8c, 0x24, 0x98,
                                                            0x00, 0x00, 0x00, 0x8d, 0x54, 0x24, 0x68, 0x52};
constexpr unsigned char memo_post_window[memo_post_length] = {
    0x33, 0xc0, 0x83, 0xc4, 0x28, 0x39, 0x05, 0x4c, 0x85, 0x60, 0x00, 0x5e, 0x0f, 0x95, 0xc0, 0x83, 0xc4, 0x64, 0xc3};
...
constexpr std::uint64_t caller_fnv1a = 0xfdd929ef070d4324ull, target_fnv1a = 0x9d655aae0a820ae8ull,
                        query_fnv1a = 0x5a4d7c6efe584a18ull;
constexpr std::uint64_t descent_fnv1a = 0xcef4870863cdd1deull, leaf_fnv1a = 0xa9766de75c6ecfcaull,
                        triangle_fnv1a = 0x90c2eb0126ac4f2aull;
// The SAT 0x004e3280, the matrix helpers 0x004e1ff0/0x004e20d0/0x004e2130, the vector helpers
// 0x004dfd80/0x004dfe60/0x004dfeb0 and ftol 0x0052b5d0 (which reads the process-constant SSE2 flag [0x006619ec]:
// cvttsd2si when set, otherwise an x87 path under the control word). The SAT module patches the SAT's call site inside
// the descent (a hole above), not these.
constexpr std::uint64_t sat_fnv1a = 0xad8a2cb66c0bf30cull, matrix_helpers_fnv1a = 0xd21cd0c0e39e9854ull,
                        vector_helpers_fnv1a = 0xc5107d96ea42adcdull, ftol_fnv1a = 0xc648662b5a549dd9ull;
...
constexpr std::uintptr_t visits_va = 0x00608544, triangles_va = 0x00608548, contacts_va = 0x0060854c,
                         root_block_va = 0x00596928;
...
struct Key {
    std::uint32_t words[key_words];
};
struct Outputs {
    std::uint32_t visits, triangles, tolerance_integer, root_block[root_block_words];
};
struct Entry {
    Key key;
    Outputs outputs;
...
struct Counters {
    std::uint32_t hits, misses, stored, contacts, ineligible, evictions, skipped_visits, skipped_triangles, verified,
        verify_mismatches, foreign_thread, reentered, clears, stuck_busy;
...
inline std::uint32_t hash(const Key& key) {
    ...
    for (unsigned i = 0; i < 33; ++i)
        if (i != minimum_value_word)
            h = (h ^ key.words[i]) * 0x01000193u; // transforms, mode and the model pointers; the stamps only confirm
    return h ^ (h >> 15);
}
// An entry is live when it was stored or hit in `frame` or the frame before (unsigned, so a wrapped counter still
// works).
inline bool live(const Entry& e, std::uint32_t frame) {
    return e.valid && frame - e.frame <= 1u;
}
...
inline bool same_words(const Key& x, const Key& y, unsigned begin, unsigned end) {
    return !std::memcmp(x.words + begin, y.words + begin, 4 * (end - begin));
}
...
inline bool answers(const Entry& e, const Key& key) {
    if (!same_words(e.key, key, 0, minimum_value_word) || !same_words(e.key, key, model_words_begin, key_words))
        return false;
...
public:
    ...
    // Stores a no-contact result; the way taken is a dead one, else the one touched longest ago. True when a live entry
    // was evicted.
    bool store(const Key& key, const Outputs& outputs, std::uint32_t frame) {
        Entry* set = entries_ + (hash(key) % sets) * ways;
        Entry* victim = nullptr;
        for (unsigned w = 0; w < ways && victim == nullptr; ++w)
            if (!live(set[w], frame)) victim = &set[w];
        bool evicted = false;
        if (victim == nullptr) {
            victim = set;
            for (unsigned w = 1; w < ways; ++w)
                if (frame - set[w].frame > frame - victim->frame) victim = &set[w];
            evicted = true;
        }
        victim->key = key;
        victim->outputs = outputs;
        victim->frame = frame;
        victim->valid = true;
    ...
    // Diagnostic only (a miss has already been decided): which group of words keeps the nearest stored query from
    // answering.
    MissClass classify(const Key& key) const {
        const Entry* e = candidate(by_a_[hash_words(key, 0, 12) % (ways * sets)], key, 0, 12);
        if (e == nullptr)
            return candidate(by_b_[hash_words(key, 13, 25) % (ways * sets)], key, 13, 25) != nullptr ? miss_xform_a
                                                                                                     : miss_none_found;
    ...
    void clear() {
        ...
        std::memset(by_a_, 0, sizeof by_a_);
        std::memset(by_b_, 0, sizeof by_b_);
    }
...
private:
    const Entry* candidate(std::uint16_t index, const Key& key, unsigned begin, unsigned end) const {
        ...
        return e.valid && same_words(e.key, key, model_words_begin, model_words_begin + 2) &&
                       same_words(e.key, key, begin, end)
                   ? &e
                   : nullptr;
    }
    Entry entries_[ways * sets];
    std::uint16_t by_a_[ways * sets], by_b_[ways * sets]; // last entry stored per (models, transform of a) and per
                                                          // (models, transform of b)
};
}
