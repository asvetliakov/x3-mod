...
DrawInput DrawInputReader::read(IDirect3DDevice9* device, const DrawArguments& args,
    ...
    try {
        ...
        key.topology = args.topology;
        key.first = args.first;
        key.primitives = args.primitives;
        if (key.indexed) {
            key.base_vertex = args.base_vertex;
            key.min_vertex = args.minimum_vertex;
            key.vertex_count = args.vertex_count;
        }
        if (args.method != DrawMethod::Primitive && args.method != DrawMethod::Indexed) reject(UserMemory);
        if (!args.primitives || (args.topology != D3DPT_TRIANGLELIST && args.topology != D3DPT_TRIANGLESTRIP))
            reject(DrawRange);
        if (scope &&
        ...
        if (profile) {
            key.position_program = profile->hash;
            matrix_register = profile->matrix_register;
        } else
            reject(PositionProgram);
        ...
        if (profile) {
            if (FAILED(device->GetVertexShaderConstantF(matrix_register, observation.submitted_wvp.data(), 4)))
                reject(SubmittedRows | QueryFailure);
            for (const auto value : observation.submitted_wvp)
                if ((bits(value) & 0x7fffffffu) > bits(1e15f)) reject(SubmittedRows);
        } else
            reject(SubmittedRows);
        ...
        if (SUCCEEDED(device->GetVertexDeclaration(&declaration.p)) && declaration.p &&
            ...
            for (UINT i = 0; i < count - 1; ++i) {
                ...
                if (element.Usage == D3DDECLUSAGE_POSITION && element.UsageIndex == 0) {
                    ++positions;
                    key.position_offset = element.Offset;
                    key.position_type = element.Type;
        ...
        Ref<IDirect3DVertexBuffer9> vb;
        Ref<IDirect3DIndexBuffer9> ib;
        UINT frequency = 0;
        ...
        if (FAILED(stream_hr) || !vb.p || FAILED(vb.p->GetDesc(&vb_desc)) || !(key.vertex_buffer = resource_id(vb.p)))
            reject(BufferDescription);
        ...
        if (!key.stride || key.position_offset > key.stride || position_bytes > key.stride - key.position_offset)
            reject(PositionLayout);
        ...
        const std::uint64_t consumed = args.topology == D3DPT_TRIANGLELIST ? std::uint64_t(args.primitives) * 3
                                                                           : std::uint64_t(args.primitives) + 2;
        if (key.indexed) {
            const std::int64_t first = std::int64_t(args.base_vertex) + args.minimum_vertex;
            first_vertex = first;
            if (first < 0 || !args.vertex_count)
                reject(DrawRange);
            else
                last = std::uint64_t(first) + args.vertex_count - 1;
            D3DINDEXBUFFER_DESC ib_desc{};
            if (FAILED(device->GetIndices(&ib.p)) || !ib.p || FAILED(ib.p->GetDesc(&ib_desc)) ||
                !(key.index_buffer = resource_id(ib.p)))
                reject(BufferDescription);
        ...
        const ownership::IndexRangeRequest index_request = key.indexed
                                                               ? ownership::IndexRangeRequest{key.index_revision,
        ...
        if (key.indexed && !(result.blockers & (BufferDescription | BufferRevision | DrawRange | UserMemory))) {
            ...
            result.index_range_verified = hr == S_OK && view.status == S_OK && view.requested && view.generation &&
                                          view.revision == key.index_revision && view.known &&
                                          view.minimum <= view.maximum && view.minimum >= args.minimum_vertex &&
                                          std::uint64_t(view.maximum) <
                                              std::uint64_t(args.minimum_vertex) + args.vertex_count &&
                                          first_vertex >= 0;
        }
        ...
        if (!key.stride || start > vb_desc.Size || position_bytes > vb_desc.Size - start ||
            last > (vb_desc.Size - start - position_bytes) / key.stride)
            reject(DrawRange);
        ...
        const ownership::FinitePositionRequest position_request{key.vertex_revision,
                                                                key.stream_offset,
                                                                key.stride,
                                                                key.position_offset,
                                                                first_vertex,
                                                                key.indexed ? args.vertex_count : consumed,
        ...
        if (!(result.blockers & geometry_blockers) && result.index_range_verified) {
            ...
            result.vertex_finite_verified = hr == S_OK && view.status == S_OK && view.requested &&
                                            ...
                                            view.revision == key.vertex_revision &&
                                            (!key.indexed || view.generation == result.indices.generation);
        } else {
            result.finite_positions.reason = (result.blockers & PositionLayout)
                                                 ? ownership::FiniteEvidenceReason::InvalidLayout
                                             : (result.blockers & (DrawRange | UserMemory))
                                                 ? ownership::FiniteEvidenceReason::InvalidRange
                                             : (result.blockers & BufferRevision)
                                                 ? ownership::FiniteEvidenceReason::TrackingUnavailable
                                             : (result.blockers & BufferDescription)
                                                 ? ownership::FiniteEvidenceReason::MissingAllocation
                                                 : ownership::FiniteEvidenceReason::IndexUnknown;
        }

        D3DCAPS9 caps{};
        if (FAILED(device->GetDeviceCaps(&caps)) || caps.NumSimultaneousRTs < 1 || caps.NumSimultaneousRTs > 4 ||
            caps.MaxStreams < 1 || caps.MaxStreams > 16)
            reject(TargetLayout | QueryFailure);
        else {
            for (UINT i = 1; i < caps.MaxStreams; ++i) {
                UINT f = 0;
                if (FAILED(device->GetStreamSourceFreq(i, &f)) || f != 1) reject(PositionLayout);
            }
            for (UINT i = 1; i < caps.NumSimultaneousRTs; ++i) {
                Ref<IDirect3DSurface9> extra;
                const auto hr = device->GetRenderTarget(i, &extra.p);
                if (extra.p || (FAILED(hr) && hr != D3DERR_NOTFOUND)) reject(TargetLayout);
            }
        }
        Ref<IDirect3DSurface9> color, depth;
        ...
        if (!color_desc.Width || !color_desc.Height || color_desc.MultiSampleType != D3DMULTISAMPLE_NONE ||
            ...
            depth_desc.Width != color_desc.Width || depth_desc.Height != color_desc.Height || viewport.X ||
            viewport.Y || viewport.Width != color_desc.Width || viewport.Height != color_desc.Height ||
            (bits(viewport.MinZ) & 0x7fffffffu) != 0 || bits(viewport.MaxZ) != bits(1.0f))
            reject(TargetLayout);
        ...
        for (const auto state : states) {
            DWORD value = 0;
            if (FAILED(device->GetRenderState(state.name, &value)) || value != state.value) reject(RasterState);
        }
        ...
        if (FAILED(device->GetRenderState(D3DRS_ZFUNC, &zfunc)) ||
            ...
            FAILED(device->GetRenderState(D3DRS_SLOPESCALEDEPTHBIAS, &slope)))
            reject(RasterState | QueryFailure);
        if ((zfunc != D3DCMP_LESS && zfunc != D3DCMP_LESSEQUAL) || cull < D3DCULL_NONE || cull > D3DCULL_CCW ||
            (write & 7) != 7 || (bias & 0x7fffffffu) || (slope & 0x7fffffffu))
            reject(RasterState);
        result.cull = static_cast<D3DCULL>(cull);
        if (!(result.blockers & (PositionProgram | PositionLayout | SubmittedRows)))
            observation.proofs |= renderer::PositionReviewed;
        if (!(result.blockers & (BufferDescription | BufferRevision | DrawRange | UserMemory)))
            observation.proofs |= renderer::GeometryUnchanged;
        if (!(result.blockers & (PositionProgram | PositionLayout | BufferDescription | DrawRange | SubmittedRows |
                                 PixelCoverage | RasterState | TargetLayout | UserMemory)))
            observation.proofs |= renderer::CoverageSupported;
        ...
        constexpr auto local_proofs = renderer::GeometryUnchanged | renderer::PositionReviewed |
                                      renderer::CoverageSupported;
        if (geometry_frame.value && !result.blockers && (observation.proofs & local_proofs) == local_proofs &&
            result.replay_source.qualified() && result.vertex_finite_verified && result.index_range_verified) {
            const ownership::GeometryLeaseRequest request{result.finite_positions.generation, position_request,
                                                          key.indexed, index_request};
            result.lease_status = ownership::acquire_geometry_lease(geometry_frame, vb.p, key.indexed ? ib.p : nullptr,
                                                                    request, &result.geometry_lease);
        }
    } catch (...) {
        ...
        result.vertex_finite_verified = false;
        result.index_range_verified = false;
    }
    return result;
}
void DrawInputReader::complete(DrawInput& input, HRESULT result) noexcept {
    if (SUCCEEDED(result))
        input.observation.proofs |= renderer::SubmissionSucceeded;
    else {
        input.blockers |= SubmissionFailure;
        input.observation.proofs &= ~renderer::SubmissionSucceeded;
    }
}
} // namespace x3m
