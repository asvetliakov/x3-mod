...
inline std::int64_t width(const Rect& r) {
    return std::int64_t(r.right) - r.left;
}
inline std::int64_t height(const Rect& r) {
    return std::int64_t(r.bottom) - r.top;
}
...
constexpr std::uint32_t ws_ex_dlgmodalframe = 0x00000001u, ws_ex_windowedge = 0x00000100u,
                        ws_ex_clientedge = 0x00000200u, ws_ex_staticedge = 0x00020000u;
constexpr std::uint32_t decorated_style = ws_caption | ws_thickframe;
constexpr std::uint32_t decorated_exstyle = ws_ex_dlgmodalframe | ws_ex_windowedge | ws_ex_clientedge |
                                            ws_ex_staticedge;
...
inline Decision decide(const Input& in) {
    ...
    if (std::int64_t(in.backbuffer_width) != width(in.monitor) ||
        std::int64_t(in.backbuffer_height) != height(in.monitor))
...
inline const char* action_name(Action a) {
    return a == Action::move ? "moved" : a == Action::noop ? "noop" : "refused";
}
}
