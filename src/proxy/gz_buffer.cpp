...
struct State {
    ...
    unsigned capacity = 0, fill = 0, pos = 0;
    LONG base = 0;
    bool error = false, at_end = false;
    unsigned index = 0;
    uint32_t calls = 0, small_calls = 0, real_reads = 0, direct_reads = 0, getcs = 0, tells = 0, seeks = 0,
             seeks_served = 0, seeks_real = 0; // per file: 32-bit keeps the hot path to single adds
    uint64_t served_bytes = 0, real_bytes = 0;
};
struct Slot {
    std::atomic<void*> file{nullptr};
    State state;
};
...
State* lookup(const void* file) {
    ...
    for (unsigned i = 0; i < slot_count; ++i)
        if (slots[i].file.load(std::memory_order_acquire) == file) {
            hint.store(i, std::memory_order_relaxed);
...
void probe(State& s, void* file) {
    ...
    if (t < 0)
        s.error = true;
    else if (t != s.base + LONG(s.fill))
        s.base = t - LONG(s.fill);
}
void resync(State& s, void* file, LONG result) {
    ...
    s.base = t;
    s.error = false;
    s.at_end = false; // e.g. a forward seek that ran into the end: moved, not in error
}
int buffered_read(State& s, void* file, unsigned char* out, unsigned len) {
    ...
    for (;;) {
        ...
        if (avail) {
            ...
            if (n <= 16) {
                const unsigned char* src = s.buf + s.pos;
                for (unsigned i = 0; i < n; ++i) out[done + i] = src[i];
            } else
                std::memcpy(out + done, s.buf + s.pos, n);
            s.pos += n;
            done += n;
            if (done == len) {
                s.served_bytes += done;
        ...
        if (want >= s.capacity) { // large remainder: straight into the caller's buffer
            r = real.read(file, out + done, want);
            ++s.direct_reads;
            if (r > 0) {
                s.base += LONG(s.fill) + r;
                s.fill = s.pos = 0;
                done += unsigned(r);
                s.real_bytes += uint64_t(r);
            }
        } else {
            r = real.read(file, s.buf, s.capacity);
            ++s.real_reads;
            s.base += LONG(s.fill);
            s.pos = 0;
            s.fill = r > 0 ? unsigned(r) : 0;
            if (r > 0) s.real_bytes += uint64_t(r);
        }
        if (r < 0)
            s.error = true;
        else {
            if (r == 0) s.at_end = true;
            probe(s, file);
        }
    }
}
LONG buffered_seek(State& s, void* file, LONG offset, int whence) {
    ...
    if (whence == SEEK_SET || whence == SEEK_CUR) {
        ...
        if (target >= s.base && target <= s.base + LONG(s.fill)) {
            s.pos = unsigned(target - s.base);
            ++s.seeks_served;
            return target;
        }
        if (target < s.base && s.error && real.rewind) {
            real.rewind(file);
            s.error = false;
            s.at_end = false;
        }
...
void accumulate(const State& s) {
    totals.calls.fetch_add(s.calls, std::memory_order_relaxed);
    totals.small_calls.fetch_add(s.small_calls, std::memory_order_relaxed);
    totals.served_bytes.fetch_add(s.served_bytes, std::memory_order_relaxed);
    totals.real_reads.fetch_add(s.real_reads, std::memory_order_relaxed);
    totals.real_bytes.fetch_add(s.real_bytes, std::memory_order_relaxed);
    totals.direct_reads.fetch_add(s.direct_reads, std::memory_order_relaxed);
    totals.getcs.fetch_add(s.getcs, std::memory_order_relaxed);
    totals.tells.fetch_add(s.tells, std::memory_order_relaxed);
    totals.seeks.fetch_add(s.seeks, std::memory_order_relaxed);
    totals.seeks_served.fetch_add(s.seeks_served, std::memory_order_relaxed);
    totals.seeks_real.fetch_add(s.seeks_real, std::memory_order_relaxed);
    totals.errors.fetch_add(s.error, std::memory_order_relaxed);
}
}

bool requested() {
    wchar_t setting[8]{};
    return x3m::config::get(L"X3M_GZ_BUFFER", setting, 8) == 1 && setting[0] == L'1';
}
unsigned requested_capacity() {
    wchar_t setting[16]{};
    const DWORD n = x3m::config::get(L"X3M_GZ_BUFFER_KB", setting, 16);
    ...
    unsigned long kb = 0;
    bool digits = n > 0 && n < 16;
    for (DWORD i = 0; digits && i < n; ++i) {
        if (setting[i] < L'0' || setting[i] > L'9')
            digits = false;
        else if (kb <= maximum_capacity_kb)
            kb = kb * 10 + unsigned(setting[i] - L'0');
    }
    if (!digits) kb = 0;
...
bool initialize(const Originals& originals, unsigned capacity) {
    if (!originals.open || !originals.read || !originals.seek || !originals.tell || !originals.getc || !originals.close)
        return false;
    ...
    if (free_table) {
        ...
        is_enabled.store(true, std::memory_order_release);
    }
    ReleaseSRWLockExclusive(&table_lock);
    return free_table;
}
bool enabled() {
    return is_enabled.load(std::memory_order_acquire);
}
bool buffered(const void* file) {
    return lookup(file) != nullptr;
}
...
void* open(const char* path, const char* mode) {
    ...
    Slot* slot = nullptr;
    unsigned char* stale_buf = nullptr;
    if (buf) {
        AcquireSRWLockExclusive(&table_lock);
        ...
        for (auto& candidate : slots)
            if (candidate.file.load(std::memory_order_relaxed) == file) {
                slot = &candidate;
                stale_buf = candidate.state.buf;
                break;
            }
        if (!slot)
            for (auto& candidate : slots)
                if (!candidate.file.load(std::memory_order_relaxed)) {
                    slot = &candidate;
                    break;
                }
        if (slot) {
            slot->state = State{};
            slot->state.buf = buf;
            slot->state.capacity = capacity_bytes;
    ...
    std::free(stale_buf);
    if (slot)
        totals.buffered_opens.fetch_add(1, std::memory_order_relaxed);
    else {
        std::free(buf);
        totals.passthrough_opens.fetch_add(1, std::memory_order_relaxed);
    }
    return file;
}
...
int close(void* file) {
    ...
    accumulate(copy);
    totals.closes.fetch_add(1, std::memory_order_relaxed);
    ...
    log("gz_buffer_file slot=%u capacity=%u calls=%llu small_calls=%llu served_bytes=%llu real_reads=%llu real_bytes=%llu direct_reads=%llu getcs=%llu tells=%llu seeks=%llu seeks_served=%llu seeks_real=%llu error=%u end=%u close=%d",
        copy.index, copy.capacity, (unsigned long long)copy.calls, (unsigned long long)copy.small_calls,
        (unsigned long long)copy.served_bytes, (unsigned long long)copy.real_reads, (unsigned long long)copy.real_bytes,
        (unsigned long long)copy.direct_reads, (unsigned long long)copy.getcs, (unsigned long long)copy.tells,
        (unsigned long long)copy.seeks, (unsigned long long)copy.seeks_served, (unsigned long long)copy.seeks_real,
        unsigned(copy.error), unsigned(copy.at_end), result);
    return result;
}
Statistics statistics() {
    Statistics s;
    s.opens = totals.opens.load();
    s.buffered_opens = totals.buffered_opens.load();
    s.passthrough_opens = totals.passthrough_opens.load();
    s.closes = totals.closes.load();
    s.calls = totals.calls.load();
    s.small_calls = totals.small_calls.load();
    s.served_bytes = totals.served_bytes.load();
    s.real_reads = totals.real_reads.load();
    s.real_bytes = totals.real_bytes.load();
    s.direct_reads = totals.direct_reads.load();
    s.getcs = totals.getcs.load();
    s.tells = totals.tells.load();
    s.seeks = totals.seeks.load();
    s.seeks_served = totals.seeks_served.load();
    s.seeks_real = totals.seeks_real.load();
    s.errors = totals.errors.load();
    return s;
}
}
