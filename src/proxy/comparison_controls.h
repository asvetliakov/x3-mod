...
// The emitter keys (F5/F6) only flip which prebuilt pixel-shader variant
...
private:
    void latch(const ComparisonKeys& keys) noexcept {
        ...
        screen_additive_down_ = keys.screen_additive; source_gain_down_ = keys.source_gain;
    ...
    bool screen_additive_down_ = false, source_gain_down_ = false;
};
} // namespace x3m
