...
constexpr float axis_screen_min = 1e-3f;   // |d q / d s| below this x the perpendicular rate: no screen axis (end-on at the vanishing point)
constexpr float screen_elongation = 1.5f;  // without a world axis: 2D extents ratio (sqrt of the covariance eigenvalue ratio) below it -> the W x W disc
constexpr float min_move_px = 0.25f;       // a scale whose largest displacement stays below this is dropped (never a write that moves nothing)
...
inline void plan_instance(const Frame& f, const float* positions, std::uint32_t n, float w_px, float l_px, Plan* out) noexcept {
    *out = Plan{};
    ...
    float cxx = 0.f, cxy = 0.f, cyy = 0.f;
    {
        ...
        for (std::uint32_t t = 0; t + 2u < n; t += 3u) {
            const float x0 = q[t][0] - cx, y0 = q[t][1] - cy, x1 = q[t + 1][0] - cx, y1 = q[t + 1][1] - cy, x2 = q[t + 2][0] - cx, y2 = q[t + 2][1] - cy;
            ...
            sa += at; sm[0] += at * (1.f / 3.f) * gx; sm[1] += at * (1.f / 3.f) * gy;
        ...
        if (sa > 0.f && finite_f(sa)) {
            ...
            cxx = sxx * inv - mx * mx; cxy = sxy * inv - mx * my; cyy = syy * inv - my * my;
        } else {
            for (std::uint32_t i = 0; i < n; ++i) {
    ...
    for (std::uint32_t t = 0; t + 2u < n; t += 3u) {
        ...
        m1[0] += third * sum[0]; m1[1] += third * sum[1]; m1[2] += third * sum[2];
    ...
    const bool screen_elongated = lambda > 0.f && lambda >= screen_elongation * screen_elongation * (lambda_minor > 0.f ? lambda_minor : 0.f);
    ...
    if (area > 0.f && finite_f(area)) {
        ...
        mu[0] = m1[0] * inv_a; mu[1] = m1[1] * inv_a; mu[2] = m1[2] * inv_a;
        c3[0] = m2[0] * inv_a - mu[0] * mu[0]; c3[1] = m2[1] * inv_a - mu[0] * mu[1]; c3[2] = m2[2] * inv_a - mu[0] * mu[2];
        c3[3] = m2[3] * inv_a - mu[1] * mu[1]; c3[4] = m2[4] * inv_a - mu[1] * mu[2]; c3[5] = m2[5] * inv_a - mu[2] * mu[2];
    }
    const float trace3 = c3[0] + c3[3] + c3[5];
    bool world_elongated = false;
    if (trace3 > 0.f && finite_f(trace3) && major_axis3(c3, axis, &lambda1) && lambda1 >= axis_elongation * (trace3 - lambda1)) {
    ...
    if (!out->world_axis && (world_elongated || !screen_elongated)) {
        out->disc = true; e1[0] = 1.f; e1[1] = 0.f; l_px = w_px;
    }
...
struct DrawStats {
    ...
    std::uint32_t lengthened = 0, widened = 0, world_axis = 0, disc = 0; // expanded along e_L / e_W; planned on the world axis; W x W disc
};
...
inline bool plan_draw(const Frame& f, const float* positions, const std::uint32_t* extras, std::uint32_t count,
                      float w_px, float l_px, Plan* plans, unsigned plan_capacity, DrawStats* stats, Histogram* hist = nullptr) noexcept {
    *stats = DrawStats{};
    ...
    for (std::uint32_t i = 0; i < instances; ++i) {
        ...
        switch (plan.verdict) {
        case Verdict::Expanded: ++stats->expanded; stats->lengthened += plan.s1 > 1.f; stats->widened += plan.s2 > 1.f; break;
        ...
        if (plan.verdict == Verdict::Expanded || plan.verdict == Verdict::Untouched) {
            stats->world_axis += plan.world_axis; stats->disc += plan.disc;
            if (hist) { ++hist->instances; ++hist->half_length[hist_bucket(plan.a)]; ++hist->width[hist_bucket(2.f * plan.b)]; }
        }
    }
    return true;
}
...
inline bool histogram_draw(const Frame& f, const float* positions, const std::uint32_t* extras, std::uint32_t count, Histogram* hist) noexcept {
    ...
    for (std::uint32_t i = 0; i < count / p; ++i) {
        ...
        for (std::uint32_t v = 0; v < p && ok; ++v) {
            ...
            if (!ok) break;
            for (unsigned k = 0; k < 2; ++k) { if (q[k] < lo[k]) lo[k] = q[k]; if (q[k] > hi[k]) hi[k] = q[k]; }
        }
        if (!ok) continue;
        const float sx = hi[0] - lo[0], sy = hi[1] - lo[1];
        const float major = sx > sy ? sx : sy, minor = sx > sy ? sy : sx;
        ++hist->instances; ++hist->half_length[hist_bucket(0.5f * major)]; ++hist->width[hist_bucket(minor)];
    }
    return true;
}
