...
struct Entry { std::uint32_t key=0; std::uintptr_t value=0; std::uint64_t serial=0; unsigned state=0; };
struct Scope {
    void* previous_seh;
    void* handler;
    unsigned kind;
...
struct Lock {
    Lock(){while(model_lock.test_and_set(std::memory_order_acquire)) {}}
...
struct BaselineEntry { std::uintptr_t address; std::uint32_t words[3]; };
...
bool bind_registry(std::uintptr_t value){
    ...
    if(registry!=value){registry=value;registry_dead=false;clear_entries();return increment(registry_epoch)&&increment(revision);}
    return true;
}
...
Lookup lookup(std::uintptr_t map,std::uint32_t key,std::uintptr_t& value){
    ...
    for(unsigned n=0;address && n<512;++n){
        ...
        if(item[1]==key){value=item[2];return Lookup::Found;}
        address=item[0];
    }
    return address?Lookup::Unavailable:Lookup::Missing;
}
bool initial_snapshot(){
    baseline_complete=false;baseline_count=0;clear_entries();
    std::uintptr_t map=0;std::uint32_t header[4]{},again[4]{};
    ...
    if(!bucket_count || bucket_count>baseline_buckets.size() || (bucket_count&(bucket_count-1)) || expected_count>capacity)return false;
    if(!header[0]){
        baseline_complete=expected_count==0 && read_memory(map,again,sizeof again) && !std::memcmp(header,again,sizeof header);
        return baseline_complete;
    }
    if(!read_memory(header[0],baseline_buckets.data(),bucket_count*4))return false;
    baseline_pointers.fill(0);unsigned count=0;
    auto reject=[](){clear_entries();baseline_count=0;return false;};
    for(unsigned bucket=0;bucket<bucket_count;++bucket){
        ...
        while(address){
            ...
            auto& raw=baseline_records[count];raw.address=address;
            if(!read_memory(address,raw.words,sizeof raw.words))return reject();
            const auto key=raw.words[1],value=raw.words[2];std::uint32_t handle=0;
            if(!key || !value || value>0xffffffd7u || (key&(bucket_count-1))!=bucket ||
               !read_memory(value+0x28,&handle,4) || handle!=key || find(key,false))return reject();
            unsigned position=((value>>2)*2654435761u)%capacity;
            while(baseline_pointers[position] && baseline_pointers[position]!=value)position=(position+1)%capacity;
            ...
            *entry={key,value,next_serial,1};++count;address=raw.words[0];
        }
    }
    if(count!=expected_count || !read_memory(map,again,sizeof again) || std::memcmp(header,again,sizeof header))return reject();
    ...
    for(unsigned i=0;i<count;++i){
        ...
        if(!read_memory(baseline_records[i].address,words,sizeof words) ||
           std::memcmp(words,baseline_records[i].words,sizeof words) ||
           !read_memory(words[2]+0x28,&handle,4) || handle!=words[1])return reject();
    }
    ...
    baseline_count=count;baseline_complete=true;return true;
}
    ...
    for(const auto& p:patches){
        ...
        if(!p.owned || !read_memory(reinterpret_cast<std::uintptr_t>(p.site),bytes,p.size) ||
           std::memcmp(bytes,p.ours.data(),p.size)){
            clear_entries();increment(revision);fail(Reason::ObserverFailure,"hook_ownership_lost");return false;
        }
    }
    return true;
}
void finish(Scope* scope,bool normal){
    ...
    Lock lock;scope->entered=false;
    if(in_flight)--in_flight;
    else {fail(Reason::ObserverFailure,"scope_underflow");return;}
    if(!normal){clear_entries();increment(load_epoch);increment(revision);return;}
    if(!observation || !verify_ownership() || scope->kind!=Insert || scope->registry_epoch!=registry_epoch ||
       scope->load_epoch!=load_epoch || registry_dead || registry!=scope->map || !scope->key || !scope->value)return;
    std::uintptr_t current=0;
    if(lookup(scope->map,scope->key,current)!=Lookup::Found || current!=scope->value)return;
    auto* entry=find(scope->key,true);
    if(!entry){clear_entries();increment(revision);fail(Reason::CapacityExhausted,"capacity_exhausted");return;}
...
__attribute__((force_align_arg_pointer)) void __cdecl x3m_lifetime_enter(Scope* scope,const Registers* registers){
    ...
    scope->entered=false;scope->kind=registers->kind;scope->args[0]=scope->args[1]=0;
    if(scope->kind==Insert){scope->args[0]=registers->args[0];scope->args[1]=registers->args[1];}
    else if(scope->kind==Destroy || scope->kind==Load)scope->args[0]=registers->args[0];
    ...
    if(observation){
        ...
        if(observation){
            std::uintptr_t current=0;
            if(scope->kind==Load){clear_entries();increment(load_epoch);increment(revision);}
            ...
            if(observation && (scope->kind==Load || (registry && scope->map==registry))){
                if(in_flight==std::numeric_limits<unsigned>::max())fail(Reason::CounterExhausted,"scope_exhausted");
                else {++in_flight;scope->entered=true;scope->registry_epoch=registry_epoch;scope->load_epoch=load_epoch;}
            }
        }
    }
    SetLastError(error);
...
__attribute__((force_align_arg_pointer)) void __cdecl x3m_lifetime_leave(Scope* scope){
    const DWORD error=GetLastError();finish(scope,true);SetLastError(error);
}
__attribute__((force_align_arg_pointer)) EXCEPTION_DISPOSITION __cdecl x3m_lifetime_unwind(
    ...
    const DWORD error=GetLastError();
    if(record->ExceptionFlags&(EXCEPTION_UNWINDING|EXCEPTION_EXIT_UNWIND))finish(static_cast<Scope*>(frame),false);
    SetLastError(error);
...
__attribute__((naked)) void x3m_lifetime_insert(){__asm__ __volatile__("pushl $0\n\tjmp _x3m_lifetime_dispatch");}
__attribute__((naked)) void x3m_lifetime_remove(){__asm__ __volatile__("pushl $1\n\tjmp _x3m_lifetime_dispatch");}
__attribute__((naked)) void x3m_lifetime_destroy(){__asm__ __volatile__("pushl $2\n\tjmp _x3m_lifetime_dispatch");}
__attribute__((naked)) void x3m_lifetime_load(){__asm__ __volatile__("pushl $3\n\tjmp _x3m_lifetime_dispatch");}
}

namespace x3m::object_lifetime {
namespace {
bool any_owned(){for(const auto& p:patches)if(p.owned)return true;return false;}
void release_trampolines(){
    if(any_owned() || (retain_retired_dispatch && dispatch_published))return;
    for(auto& p:patches){if(p.trampoline)VirtualFree(p.trampoline,0,MEM_RELEASE);p={};}
    for(auto& target:x3m_lifetime_originals)target=0;
}
bool restore_all(unsigned fail_stage=0,unsigned fail_site=0){
    observation=false;bool ok=true;
    for(unsigned i=4;i--;) {
        auto& p=patches[i];if(!p.owned)continue;
        ...
        if(!read_memory(reinterpret_cast<std::uintptr_t>(p.site),current,p.size) ||
           (std::memcmp(current,p.before.data(),p.size) && std::memcmp(current,p.ours.data(),p.size))){
            diagnostic="foreign_patch";ok=false;continue;
        }
        DWORD old=0,unused=0;
        if(fault==1 || !VirtualProtect(p.site,p.size,PAGE_EXECUTE_READWRITE,&old)){diagnostic="restore_protect_failed";ok=false;continue;}
        ...
        if(flushed && protected_again)p.owned=false;
        else {diagnostic="restore_incomplete";ok=false;}
    }
    if(ok){
        if(retain_retired_dispatch && dispatch_published)retired_dispatch=true;
        release_trampolines();diagnostic=retired_dispatch?"retired":"disabled";
    }
    return ok;
}
struct Sites {void* insert;void* remove;void* destroy;void* load;void* target;std::uintptr_t engine;};
bool install(const Sites& sites,unsigned requested_capacity,unsigned fault,unsigned fault_site,bool snapshot=true,bool retain=true){
    if(retired_dispatch){diagnostic="retired_no_reinstall";return false;}
    if(any_owned() || !requested_capacity || requested_capacity>MaxEntries){diagnostic="invalid_install";return false;}
    release_trampolines();
    retain_retired_dispatch=retain;dispatch_published=false;
    const void* addresses[]={sites.insert,sites.remove,sites.destroy,sites.load};
    const void* dispatchers[]={reinterpret_cast<void*>(&x3m_lifetime_insert),reinterpret_cast<void*>(&x3m_lifetime_remove),
                             reinterpret_cast<void*>(&x3m_lifetime_destroy),reinterpret_cast<void*>(&x3m_lifetime_load)};
    const unsigned sizes[]={5,6,5,5};
    const unsigned char expected[3][6]={{0x55,0x8b,0x6c,0x24,0x08,0},{0x8b,0x4f,0x04,0x83,0xe9,0x01},{0x53,0x8b,0x5c,0x24,0x08,0}};
    for(unsigned i=0;i<4;++i){
        auto& p=patches[i];p.site=static_cast<unsigned char*>(const_cast<void*>(addresses[i]));p.size=sizes[i];
        if(!read_memory(reinterpret_cast<std::uintptr_t>(p.site),p.before.data(),p.size)){diagnostic="unreadable_site";release_trampolines();return false;}
        if(i<3 && std::memcmp(p.before.data(),expected[i],p.size)){diagnostic="site_mismatch";release_trampolines();return false;}
        if(i==3){
            std::uint32_t displacement=0;std::memcpy(&displacement,p.before.data()+1,4);
            if(p.before[0]!=0xe8 || reinterpret_cast<std::uintptr_t>(p.site)+5+displacement!=reinterpret_cast<std::uintptr_t>(sites.target)){
                diagnostic="load_target_mismatch";release_trampolines();return false;
            }
            ...
            if(!p.trampoline){diagnostic="trampoline_allocation_failed";release_trampolines();return false;}
            auto* code=static_cast<unsigned char*>(p.trampoline);std::memcpy(code,p.before.data(),p.size);code[p.size]=0xe9;
            const std::uint32_t displacement=reinterpret_cast<std::uintptr_t>(p.site)+p.size-(reinterpret_cast<std::uintptr_t>(code)+p.size+5);
            std::memcpy(code+p.size+1,&displacement,4);DWORD unused=0;
            if(!VirtualProtect(code,16,PAGE_EXECUTE_READ,&unused) || !FlushInstructionCache(GetCurrentProcess(),code,16)){
                diagnostic="trampoline_protection_failed";release_trampolines();return false;
            }
            x3m_lifetime_originals[i]=reinterpret_cast<std::uintptr_t>(code);
        }
        p.ours.fill(0x90);p.ours[0]=i==3?0xe8:0xe9;
        const std::uint32_t displacement=reinterpret_cast<std::uintptr_t>(dispatchers[i])-(reinterpret_cast<std::uintptr_t>(p.site)+5);
        std::memcpy(p.ours.data()+1,&displacement,4);
    }
    for(unsigned i=0;i<4;++i){
        auto& p=patches[i];const unsigned failure=i==fault_site?fault:0;
        if(failure==1 || !VirtualProtect(p.site,p.size,PAGE_EXECUTE_READWRITE,&p.protection)){
            restore_all();diagnostic="patch_protect_failed";return false;
        }
        p.owned=true;dispatch_published=true;std::memcpy(p.site,p.ours.data(),p.size);
        ...
        if(!flushed || !protected_again){
            ...
            diagnostic=restored?"patch_rolled_back":"rollback_incomplete";return false;
        }
    }
    {
        Lock lock;capacity=requested_capacity;engine_slot=sites.engine;registry=0;registry_dead=false;in_flight=0;clear_entries();
        disabled_reason=Reason::Disabled;
        if(!increment(observer_epoch) || !increment(load_epoch) || !increment(registry_epoch) || !increment(revision)){
            restore_all();return false;
        }
        baseline_complete=false;baseline_count=0;
        if(snapshot)initial_snapshot();
        if(disabled_reason==Reason::CounterExhausted){restore_all();return false;}
        observation=true;diagnostic=baseline_complete?"active":"active_without_baseline";
    }
    return true;
}
bool digest_bytes(const void* bytes,DWORD size,const char* expected){
    HCRYPTPROV provider=0;HCRYPTHASH hash=0;unsigned char digest[32]{};DWORD count=32;
    bool ok=CryptAcquireContextW(&provider,nullptr,nullptr,PROV_RSA_AES,CRYPT_VERIFYCONTEXT) &&
        CryptCreateHash(provider,CALG_SHA_256,0,0,&hash) && CryptHashData(hash,static_cast<const BYTE*>(bytes),size,0) &&
        CryptGetHashParam(hash,HP_HASHVAL,digest,&count,0) && count==32;
    if(ok)for(unsigned i=0;i<32;++i){
...
bool code_fingerprints(std::uintptr_t base){
    ...
    for(const auto& region:regions)
        if(!read_memory(base+region.rva,data,region.size) || !digest_bytes(data,region.size,region.hash))return false;
    return true;
}
}
bool initialize(){
    ...
    else diagnostic="executable_mismatch";
    SetLastError(error);return result;
}
bool active(){return observation;}
bool recovery_required(){return any_owned()&&!observation;}
const char* status(){return diagnostic.load();}
Stats stats(){Lock lock;return {baseline_complete,baseline_count};}
bool current(std::uintptr_t map,std::uintptr_t node,std::uint32_t node_handle,
             std::uintptr_t camera,std::uint32_t camera_handle,Snapshot* out){
    if(!out)return false;
    const DWORD error=GetLastError();*out={};
    {
        Lock lock;
        out->observer_epoch=observer_epoch;out->load_epoch=load_epoch;out->registry_epoch=registry_epoch;out->mutation_revision=revision;
        auto evaluate=[&]()->Reason {
            ...
            if(map!=registry){bind_registry(map);return Reason::RegistryChanged;}
            if(registry_dead)return Reason::RegistryUnavailable;
            const auto* n=find(node_handle,false);const auto* c=find(camera_handle,false);
            ...
            if(lookup(map,node_handle,nvalue)!=Lookup::Found || lookup(map,camera_handle,cvalue)!=Lookup::Found){
                retire(node_handle);retire(camera_handle);increment(revision);return Reason::LookupUnavailable;
            }
            if(nvalue!=node || cvalue!=camera){retire(node_handle);retire(camera_handle);increment(revision);return Reason::PointerMismatch;}
            out->node_serial=n->serial;out->camera_serial=c->serial;return Reason::Known;
        };
        out->reason=evaluate();out->known=out->reason==Reason::Known;
        out->load_epoch=load_epoch;out->registry_epoch=registry_epoch;out->mutation_revision=revision;
    }
...
bool shutdown(){
    ...
    const bool result=restore_all();SetLastError(error);return result;
}
#ifdef X3M_OBJECT_LIFETIME_FIXTURE
bool fixture_install(const FixtureSites& s,unsigned count,unsigned failure,unsigned site,bool snapshot,bool retain){
    const DWORD error=GetLastError();
    const bool result=install({s.insert_entry,s.remove_nonempty,s.destroy_entry,s.load_call,s.load_target,s.engine_slot},count,failure,site,snapshot,retain);
    SetLastError(error);return result;
}
