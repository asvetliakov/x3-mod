...
struct Accumulator {
    ...
    void begin(std::uint64_t t, Kind k = Miss) {
        ...
        pending = true;
        reasons = t ? 0u : unsigned(Clock);
        start = t;
        descent_ticks = descent_count = 0;
        depth = 0;
        kind = k;
        if (k == Miss)
            ++frame.misses;
    ...
    void invalidate(Reason why = Reentry) {
        if (pending) reasons |= why;
    }
    ...
    void leave(std::uint64_t t) {
        ...
        if (--depth) return;
        if (!t || t < descent_start) {
            invalidate(Clock);
            return;
        }
        descent_ticks += t - descent_start;
        ++descent_count;
    }
    void record_invalid() {
        ++frame.invalid;
        frame.clock += bool(reasons & Clock);
        frame.reentry += bool(reasons & Reentry);
        frame.foreign += bool(reasons & Foreign);
        frame.unwind += bool(reasons & Unwind);
        frame.reset += bool(reasons & Reset);
        frame.cpu_mode += bool(reasons & CpuMode);
    }
    void end(std::uint64_t t, std::uint32_t visits = 0, std::uint32_t triangles = 0, std::uint32_t contacts = 0) {
        ...
        if (reasons)
            ...
            const auto query = t - start;
            frame.query += query;
            frame.descent += descent_ticks;
            frame.non_descent += query - descent_ticks;
            ++frame.queries;
            frame.descents += descent_count;
        }
        pending = false;
        depth = 0;
    }
    void abandon(Reason why = Unwind) {
        if (pending) {
            invalidate(why);
            record_invalid();
        }
    ...
    Sample take() {
        abandon();
        const auto out = frame;
        frame = {};
        return out;
    }
};
struct Window {
    Sample samples[window_frames]{};
    unsigned size = 0;
    bool add(const Sample& s) {
        if (size == window_frames) return false;
        samples[size++] = s;
        return true;
    }
    unsigned valid_frames() const {
        unsigned count = 0;
        for (unsigned i = 0; i < size; ++i) count += samples[i].invalid == 0;
        return count;
    }
    std::uint64_t quantile(std::uint64_t Sample::* field, unsigned percentile) const {
        std::uint64_t values[window_frames];
        unsigned count = 0;
        for (unsigned i = 0; i < size; ++i)
            if (!samples[i].invalid) values[count++] = samples[i].*field;
        if (!count) return 0;
    ...
    Sample sum() const {
        ...
        for (unsigned i = 0; i < size; ++i) {
            ...
            if (!x.invalid) {
                s.query += x.query;
                s.descent += x.descent;
                s.non_descent += x.non_descent;
            }
            s.queries += x.queries;
            s.descents += x.descents;
            s.invalid += x.invalid;
            s.misses += x.misses;
            s.verifies += x.verifies;
            s.ineligible += x.ineligible;
            s.visits += x.visits;
            s.triangles += x.triangles;
            s.contacts += x.contacts;
            s.clock += x.clock;
            s.reentry += x.reentry;
            s.foreign += x.foreign;
            s.unwind += x.unwind;
            s.reset += x.reset;
            s.cpu_mode += x.cpu_mode;
        }
        return s;
    }
};
