...
| 3 | FP16 lighting and HDR emissive | The [first material slice](architecture/scene-linear-materials.md) has design/disassembly, combined shaders and detached/live D3D9 qualification. Candidate installation and game acceptance are pending; only covered opaque material evaluation becomes linear, not all scene blending or HDR output. |
...
| 6 | New material shaders | First Argon slice implements ten archive pairs, including uncaptured variants. Offline proof, 167 GPU cases and 1,220 actual live-route checks pass; independent review is complete; installation and gameplay acceptance remain. Whole-scene linear lighting still needs a policy for every color writer. |
