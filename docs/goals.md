...
| 8 | Better directional/self shadows | In progress | The sun-share lane is integrated default-off; run81 (2026-09-16) still had zero availability because the mip-bias default left the exact cutout arm off and the two cutout pairs vetoed, fixed lane-only on main (`e62722a`, installed in the run29 candidate); run88 (2026-09-17) then reported the lane available on all 14,924 frames under linear materials but still no cutout draws; the one-cascade depth replay runs at 42 µs with no consumer, and the legacy-shading sun application contract is ratified. No shadows are applied. | [directional-shadows.md](verification/directional-shadows.md) |
