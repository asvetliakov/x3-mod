...
| 4 | HDR bloom | Implementation not started; next visual feature after the current integration and user tests. Reviewed disassembly establishes the [replacement contract](reverse-engineering/bloom-compositor-skip.md); later-material inherited-state coverage remains open. Build on FP16; initial bloom inherits the current scene-content limitation. |
