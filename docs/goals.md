...
| 3 | FP16 lighting and HDR emissive | In progress | The opt-in linear material path stays installed but the user plays original hulls (2026-09-15); emitters above 1.0 use `--screen-emission-additive` (bolts, accepted run 26) and one `--emission-source-gain` over the effects program (engines, gate, beams, flares; screen-blended materials drawn additively under the gain, merged 2026-09-17 after the effect-shader census showed the engines were refused); position lights and signs on hull programs remain a follow-up; brackets rejected. | [linear-emission-cost.md](architecture/linear-emission-cost.md) |
| 4 | HDR bloom | Installed | Live integration with authored-glow correction at gain 0.375 / scatter 0.65 is installed and accepted for halo strength in run 9; the bolt halo (run 28/29) was the unbounded bloom source saturating the tonemapper, and `--bloom-source-clamp` (2026-09-17, absent = unchanged) is queued for the user bracket in run 30. | [hdr-bloom-boundary.md](architecture/hdr-bloom-boundary.md) |
...
| 8 | Better directional/self shadows | In progress | The sun-share lane is integrated default-off; run81 (2026-09-16) still had zero availability because the mip-bias default left the exact cutout arm off and the two cutout pairs vetoed, fixed lane-only on main (`e62722a`, installed in the run29 candidate); the one-cascade depth replay runs at 42 µs with no consumer, and the legacy-shading sun application contract is ratified. No shadows are applied. | [directional-shadows.md](verification/directional-shadows.md) |
...
| 14 | Additional improvements | In progress | Crypto cache, reader and adjacency fast paths are accepted (runs 17–19), speech and the selection pause are accepted (runs 16/18); run 29 measures a busy view at 14.7 ms p50 (~240 draws) vs 8.1 ms facing away with Present never the wait, the sampling profiler attributes nothing under FEX, and `--frame-timing` proxy/game buckets (2026-09-17) are queued to split the cost. | [loading-observations.md](reverse-engineering/loading-observations.md) |
