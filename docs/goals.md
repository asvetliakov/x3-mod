...
| 8 | Better directional/self shadows | In progress: sun-share extraction is host-qualified for 108 pixel originals and preserves existing output. Live RT2 publication, GPU precision, portable replay feasibility and cascades remain; no rendered shadows are implemented. The inherited material sanitizer constant-read-port violation must be fixed before native shader qualification. |
