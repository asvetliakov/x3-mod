...
- **Installed:** run40 candidate `d4d824a4…` from main `d415264f` (ladder follow-ups included)
  ([build](../verification/results/run40-candidate-build.json),
  [install](../verification/results/run40-candidate-install.json)); rollback
  (first run40 build `9968bf84…`) in `/tmp/x3-candidate-eBwRHq/rollback`, run39
  `cc966fb2…` in `/tmp/x3-candidate-wRyHZV/rollback`. Run 39 flew
