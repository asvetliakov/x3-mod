...
Previous handoff: [archive/handoff-2026-09-17.md](archive/handoff-2026-09-17.md).

## Where things stand (updated 2026-09-18 after the shadow rebuild)

- **Installed:** run40 candidate `c47f039c…` from main `e8ae3357` (run116 flicker fix, telemetry options)
  ([build](../verification/results/run40-candidate-build.json),
  [install](../verification/results/run40-candidate-install.json)); rollback
  (run116 build `d4d824a4…`) in `/tmp/x3-candidate-iK0cir/rollback`; older builds
  in `/tmp/x3-candidate-eBwRHq` and `/tmp/x3-candidate-wRyHZV`. Run 40 A was flown
  once as run116 on `d4d824a4` (band gone, 30 km reach, distant flicker → fixed);
  re-fly A, then A2/B/C. Run 39 flew
  session A only (run115): the half-pixel band fix, five cascades to 30 km,
  caster pool control and the adaptive ladder are in this build; run 40
  carries A2/B/C ([user-runs.md](verification/user-runs.md) §40).
- **Run 39 is queued** ([user-runs.md](verification/user-runs.md)): A set R
  cascades + census, A2 the 50k far cascade, B retained casters live, C hull
  emitters at gain 4. Hotkeys: Ctrl+Shift+F12 shadows at rest, Ctrl+Shift+F4
  hull emitters, F6 effect gains, F8 capture.
- **Shadow rebuild on main** (all reviewed Opus + Fable): run 38 root cause
  (sun read from PS c4 regardless of program) fixed by per-program register
  resolution and a latched sun; positional sun from the engine light array
  (hook-free poll, cross-checked per draw; sun ≈1.57e7 units, positions are
  integers × 0.01); N ≤ 4 cascades with anchored snapping, half-texel fix,
  pancaking, extent cache; caster retention stages 1–2 on the retirement
  journal; at-rest toggle. Design notes: shadow-cascade-extents.md (set R
  ratified, 50k segment in run 39), shadow-caster-retention.md.
- **In flight (worktrees, interrupted by a session limit, resume them):**
  far-cascade caster policy (static-only cascades, importance drop order,
  per-cascade records; `agent-a46f2892f9578bec0`), own-ship-adaptive C0 with
  ratio guard (`agent-aa59f5a76493f240f`), toggle follow-ups on a new branch
  (`agent-a7f963eb00e39f8c4`). They ride the candidate after run 39.
- **Closed:** technique trampoline (0.007 ms/frame); object-lifetime FX-state
  failures (FEX environment, control added).
- **Open:** distant flicker every ~30 s (run 38 A) — recheck in run 39; K for
  the adaptive C0 needs a big-ship flight; native Windows unverified.

## Previous content (2026-09-17 evening)
