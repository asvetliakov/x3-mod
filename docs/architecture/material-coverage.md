...
the 30 exact pairs in the installed material route. It uses the complete archive
...
The current linear-material implementation admits exactly **30 pairs / 24
programs**: ten Argon SM3 `DEFAULT`, ten Argon SM3 `BUMPMAP`, and ten shared
Khaak/Teladi/Teladi_nodiff/Xenon SM3 `DEFAULT` pairs. The last ten identities
occur under all four family names and count once in the global total. The five
highest-quality PS2 `effects`/`engine` `DEFAULT` emission pairs have qualified
producer/composition work, but live integration remains pending; they are not
...
| `effects` | A | 12 / 12 | 0 / 16 / 6 / 0 | D 10, I 10, IB 2 | Emissive/transparent. The shared base pair plus two effects-only pairs are 3 of the deduplicated five-pair high-quality D work; all 22 family pairs still need a live composition/ownership result. |
| `engine` | A | 10 / 7 | 0 / 6 / 6 / 0 | D 5, I 5, IB 2 | Emissive/transparent. The shared base pair plus two engine-only pairs are the other 3 incidences in that five-pair work; all 12 family pairs still need live integration. |
...
| `khaak` | A | 45 / 56 | 20 / 60 / 24 / 0 | D 52, B 52 | **10 converted**: SM3 D, shared with the next three listed shared families. The other 94 family pairs remain. |
...
| `split` | A | 45 / 56 | 20 / 60 / 24 / 0 | D 52, B 52 | Opaque/material candidate; all 104 remain. The SM3 D ten-pair algebra is the first bounded expansion group. |
| `standard_lighting` | A | 45 / 81 | 30 / 89 / 24 / 1 | D 52, B 52, BL 52 | General opaque/material candidate; 143 complete pairs plus the anomalous incomplete pass remain. Technique memberships overlap and must stay exact-pair keyed. |
...
| `terran` | A | 45 / 56 | 20 / 60 / 24 / 0 | D 52, B 52 | Opaque/material candidate; all 104 remain. |
| `xenon` | A | 45 / 56 | 20 / 60 / 24 / 0 | D 52, B 52 | Same complete set and coverage as `khaak`; **10 shared SM3 D pairs converted**, 94 remain. |
...
2. Complete the ordered linear-composition and temporal-coverage boundary for
   the five qualified high-quality `effects`/`engine` pairs, then extend it to
