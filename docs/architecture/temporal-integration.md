...
and from the linked verification documents. Steps 1 and 2 of the order of
work below are implemented (sections at the end); step 3 is not.
...
allocated only when the surface path is used. `run` still creates a
`D3DSBT_ALL` state block every frame; caching it is a later performance item.
