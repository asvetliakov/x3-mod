...
| `0x004c34ea` | `0x004c0150` per-draw state: `t_OcclusionTexture` (group record `+0x74`) gets the material's texture only when `node+0x14c == 0`, else the `NONE_OCCL_DECAL` placeholder ([texture-lookup.md](texture-lookup.md) §12); `--lod-occlusion all` sets the rel32 of the `jne` at `0x004c34f7` to 0 so every record binds the material's map (§12.4) |
