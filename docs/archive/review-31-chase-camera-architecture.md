...
This is the prototype review and its branch-fix status. Later integration
[review 35](review-35-chase-integration.md) adds handler timing for A10 and
per-device/vanilla-transition cut fixes; gameplay measurements and A8 menu
behavior remain pending. The original findings below retain their historical
scope.
...
`loader.cpp`, `capture.cpp`, `motion_output.cpp`, [loading-probes.md](../verification/loading-probes.md).
...
Status column added 2026-09-13 after the fixes commit (design:
[chase-camera.md](../architecture/chase-camera.md)); the tunables were renamed
`X3M_CHASE_*` / `--chase-*` at the same time (31b O7), and the first applied
frame's static inferences are logged once (31b O2/O3).
...
- **Bytes/identity/window**: `object_trace::executable_verified` first,
  ...
  twelve loading-probe sites ([loading-probes.md](../verification/loading-probes.md), "Byte
  verification").
...
Command: `python3 tools/manage.py launch --direct --camera chase` (defaults:
`rot_tau` 0.15, clamp 8°, `pos_tau` 0.20, `pos_lag_clamp` 0.10, tightness 0,
scene fix off); a second run adds the route/TAA flags for items 5 and 8.

1. Install line: `status=active atomic_write=0`, no `bytes_mismatch`, the
   tunables echoed with `combat=off scene_fix=0
   predicate=view_object_is_ref_object lifetime=process`; the default
   `--direct` run has no `chase_camera` lines (`launch --dry-run --direct`
   sets only `X3M_CAMERA=vanilla`).
2. Back view (F2 cycle), 300-frame lines: `verdict=0`, `applied` growing,
   ...
   `refused_inactive` growing only if other cockpits exist and
   `cockpits_seen` stable (1 expected in flight, 2+ with a monitor/target
   view); `write_refused=0`.
3. `lag_deg` > 0 while turning and ≤ 8°, back under 0.1° within ~1 s after
   the turn stops; `dt_ms` ≈ the frame time and unchanged with SETA on;
   `basis_dev_deg` ≈ 0 in free flight.
...
5. Gate jump: `snap_reason` 16 then 4 with `coalesced` +1 and `snaps` +2,
   exactly one `camera_cut=1` on the TAA frame lines, no ghosting
   afterwards (A4).
6. Internal view `verdict=1`, front/side `verdict=2` with the vanilla pose;
   docking `verdict=3` (or 7 if the docking sequence uses connect mode 3 /
   the `+0x1a0` flag: report `connect=` and `flags_1a0=`) and a snap on
   undock; a cutscene passes through.
...
9. Change the resolution in the options and return to the back view: the
   camera must still be chase and the log shows `chase_camera_last_device
   kept=1` if the device was recreated (A3).
10. Frame time unchanged within noise; the handler's telemetry span (if
    added) in the microsecond range (A10, open).
11. The one-shot `chase_camera first_applied` line (O2/O3): `half_vfov_tan`
    ≈ 0.75 at 4:3 or 0.5625 at 16:9 with `fov298=0x4000` (zoom off),
    `ref == view_obj`, `boom_local` with `z < 0`, `|x|` small and `y` within
    the thresholds; report the values.
12. Select a target (and an enemy target): `tracking=` shows 1 (or 4) with
    `locked=1` and `locked_frames` growing; with nothing selected `tracking=0
    locked=0` (A9 field semantics, still with tightness 0). Only then try
    `--chase-combat-tightness 0.5`.
13. Front→back view transition (F2 cycle) while turning: no repeated pops at
    the end of the animation (A6); report `verdict` flips if any.
