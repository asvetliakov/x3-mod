...
Root integration note: merge into main preserves the newer normalizer, reader
initialization ordering and review-30 fixes. Independent staged-diff review
found no production merge defect; it identified missing `crypt_cache.h/.cpp`
dependencies in the loading and mesh-hook runner manifests, now added. Root
also corrected a stale comment to describe active-last initialization. The
merged loading/capture sources pass a production-flag syntax check, the six
crypto host tests pass and the merged CLI dry-run selects the requested X3
configuration without launching. Five conflicting historical loading artifacts
were resolved to main's newer review-30 evidence, not relabelled post-merge
results. Full integrated runtime qualification and installation remain pending
the adjacency/reader checkpoint; the branch's accepted results remain scoped to
its recorded source maps.
