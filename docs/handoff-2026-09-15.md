...
- Optional, after the fill verdict, if the user still wants the headlight to reach farther: a
  `--headlight-range <scale>` option. Bounded `disassemble` first: find the per-node point-light
  admission (the writer of the per-node light count `g_nNumLightPoint`/i0 and the range compare
  against the node field near `+0x158`, see `station-material-distance.md` "Run 22" open issues),
  then a byte-verified, reversible patch in the style of `lod-selection.md` option 1, default-off.
  It removes the lit→black step at ≈1,300 units but not the 1/(1+0.01d) falloff; the fill term is the
  floor. Not started.
