...
- Shadows: the user prefers going straight to cascaded shadow maps (skip screen-space sun shadows as
  a shipped feature; keep it only as fallback if geometry replay proves infeasible). Not yet started;
...
2. **Chase design note** `docs/architecture/chase-view-restore-and-hud-anchor.md` (Fable `design`
   agent): two user requests — (a) restore the external chase view automatically after a sector
   change (gate/jumpdrive), default-off option; needs the engine's view-mode writer (cockpit `+0x150`,
   `chase-camera.md`) and whether writing it from the hook equals the view key; (b) the central HUD
   sits at screen centre but the camera pitches 13° down, so the ship's forward ray projects above
   centre (`tan 13° / tan(vfov/2)` of half height at infinity); decide the anchor and its fixture. If
   the note exists, ratify or reject and brief `implement` (Fable for the engine write); if not, redo
   the design brief from this paragraph.
