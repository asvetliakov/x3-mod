...
Bottle **X3**, **CrossOver Preview.app**. Run86 DLL SHA-256:
`27881669462aa3463a215e4dd42aed0e7eed5e0b620111863b3530cd4323f317`
(57,329,900 bytes), built once from clean reviewed main `9375a1e7` in a detached worktree (`/tmp/x3-run86-candidate/src`).
Retained DLL: `/tmp/x3-run86-candidate/build/d3d9.dll`. Installed 2026-09-25 12:37
([qualification](../verification/results/run86-candidate-qualification.json), [install](../verification/results/run86-candidate-install.json)).
Rollback: Run85 `c430294a…` at `/tmp/x3-run85-candidate/build/d3d9.dll` (unflown), then Run84 `189a34f0…` (Run 84 A flown).

Changes against Run85 (`c430294a…`), all opt-in or tooling, no default behaviour change: **`--effects-stage`** phase 1 of the
effects modernisation ([design](architecture/effects-modernisation-opus.md): HDR bolt capsules and shield-hit shells / decals drawn
in the TAA resolve bracket over the game's own effects, texture keys hashed at upload, `--effects-shields`, `--effects-census`,
`--effects-bolt-views`; 9375a1e7), **`--chase-view-restore-dock`** (the rear chase view kept across docking at a station; the
docked screen shows the parked ship from behind; 40cd3023), the **fog family loader** accepting an empty table (28cbd43e) and the
launcher's **`fog families:` / `lod overlay:` report lines** with the `fog-families` action and the LOD overlay for selected mod
packages (`lod_overlay.py --mod`; b36925d0). Game data: install-fleet4 overlay unchanged; new `drive_c/X3/x3m/effect_keys.json`
(copied by install) and `fog-families.bin` (64 B empty table: 14 compiled families cover 14, 2 refused) + `fog-families.json`.
Run85's far clip 7x7 + far ramp 60/68 (the plants' sparkles) are included and still unflown.

Qualification at `9375a1e7`: build 0 warnings (43 s); x87 119 roots / 713 reachable / zero violations; imports 231 -> 231 and 17
exports unchanged; temporal pass 744 / 278, lattice 598 / 90 (no numerical row differs from Run85); motion output 235 cases (new
seam-ownership-effects-stage; 0 differing stable fields vs Run85); effects stage GPU 121/121 + keys 8/8 (own pass 0.009 ms;
advisory targets missed: full-screen shell 0.54/0.78 ms, 60 bolts + 2 shells 0.155 ms at 5120x1440); fog family file 68/68;
