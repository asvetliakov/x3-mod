...
Bottle **X3**, **CrossOver Preview.app**. Run82 DLL SHA-256:
`cd8ef8e4674d17d359b4a21bbb0b3d2a245e859db78819097eb1698ab2ba6279`
(56,777,721 bytes), built once from clean reviewed main `d1a4e360` in a detached worktree (`/tmp/x3-run82-candidate/src`).
Retained DLL: `/tmp/x3-run82-candidate/build/d3d9.dll`. Installed 2026-09-25 00:26
([qualification](../verification/results/run82-candidate-qualification.json), [install](../verification/results/run82-candidate-install.json)).
Rollback: Run81 `b4e945ed…` at `/tmp/x3-run81-candidate/build/d3d9.dll` (Run 81 A flown), then Run80 `593112dc…`.

Changes against Run81 (`b4e945ed…`), all from Run 81 A: **FOV remap** (`--fov N` in game units, default 90 = "90 horizontal on
16:9"; the constructor immediate and a stub at the INS_SetFocus store remap the game's number through an 81-entry table for
N 50..130, so the in-game menu 70..100 works in the same units; `--fov game` = vanilla; 250cc48d), **`--sun-flare-fix on`**
(saturating stub at 0x0047e391 for the lens-flare collector's 32-bit overflow that hid the sun near the view centre at wide
F on 32:9; 72439e2c), **`--chase-fov-compensate on`** (chase boom x 0.75/half_vfov_tan, the ship keeps its screen size;
7187771f), the small-parts cull reading the scene view's projection (5a81df27), **`--taa-box-resolution half` as the
launcher default** (S4 accepted in run310; c2aa8d4e), **fade RT2 owner admits alpha-tested station cutouts** (d1a4e360;
the stabiliser-0 retry is Run 82 A launch 2), and the opt-in **`--taa-thin-region-source both|screen|vote`** (4855dc5f).
Game data unchanged: fleet overlay install-fleet3 (620 bodies,
[record](../verification/results/lod-overlay-batch/install-fleet3/install.json)).

Qualification at `d1a4e360`: host suite 262 modules / 2,739 tests / 0 failing; x87 116 roots / 684 reachable / zero
violations; imports and exports unchanged against Run81; temporal pass 744 / 278 (report identical), lattice 610 / 107;
motion output 234 cases (337,330 checks); fov patch 194/194; sun flare fix 79/79; lod occlusion 108/108; cull fixture 153;
chase fire 307; GPU sync 32/32; identity verifiers 27/27 per variant (7 corrupt cases over 6 verifiers); site verifiers fov
29/29, sun 19/19, lod_occlusion 20/20, cull 21/21 + 21/21, Terran 16/16; fog pass / route bridge / fog family / bloom /
cull census reused from Run81 (no such source changed); five dry runs exit 0 (the stand adds exactly X3M_FOV=90,
X3M_SUN_FLARE_FIX=on, X3M_CHASE_FOV_COMPENSATE=1, X3M_TAA_BOX_RESOLUTION=half + _DEFAULT=1; vanilla sends none). Not a
native Windows execution. The install record verifies installed bytes and unchanged X3AP.exe `fdbf3418…` and
