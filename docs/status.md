...
Bottle **X3**, **CrossOver Preview.app**. Run83 DLL SHA-256:
`f0259ceb6ef3eac7a407c49762dda9a182898d27ae4e32eb242ec922c1a52b8e`
(56,846,411 bytes), built once from clean reviewed main `80394271` in a detached worktree (`/tmp/x3-run83-candidate/src`).
Retained DLL: `/tmp/x3-run83-candidate/build/d3d9.dll`. Installed 2026-09-25 04:25
([qualification](../verification/results/run83-candidate-qualification.json), [install](../verification/results/run83-candidate-install.json)).
Rollback: Run82 `cd8ef8e4…` at `/tmp/x3-run82-candidate/build/d3d9.dll` (Run 82 A flown), then Run81 `b4e945ed…`.

Changes against Run82 (`cd8ef8e4…`): **FOV savegame load remap** (third claim at 0x0041c8c1, exact-match rule: a pre-patch save's
vanilla value loads remapped, a modded save never drifts; `fov_confirm ... after=save_load_complete`; 3fb71497), **TAA mask fold**
(no mask draw; the A' resolve computes the gates, far weight and vote flag and writes the depth history as a third target; the
sentinel class is retired and `--taa-sentinel-stabiliser` is refused; the screen search is compiled but off by default =
`--taa-thin-region-source vote`, `both` turns it on; state-block stream fix; fixture TAA at rest 5.79 -> 3.93 ms against the 0.7
baseline; 80394271), **`--window-monitor-rect` on by default** (the game's popup window is moved from the work-area origin under
the macOS menu bar to the full monitor at create/reset) with opt-in **`--window-trace`** and **`--cursor-reassert`** (6f122cfa),
and the **`--original-fill` launcher default** (0.01; launcher only). Game data: **install-fleet4** overlay (620 bodies; the Terran
solar-plant coarse record baked through the louvre recipe, 3 draws per node; accepted in run321,
[record](../verification/results/lod-overlay-batch/install-fleet4/install.json)).

Qualification at `80394271`: host suite 267 modules / 2,771 tests / 0 failing (second run; the first had one lock collision in
`test_sun_share_lane`, passed alone); x87 119 roots / 690 reachable / zero violations; imports 217 -> 231 (15 user32 window,
cursor and hook functions), exports unchanged; temporal pass 744 / 278, lattice 561 / 90; motion output 234 summary cases / 250
case lines (337,330 checks); fov patch 268/268; gpu sync 32/32; window mode 19/19; cursor re-assert 36/36; cull small parts 153;
verify_fov_site 44/44; identity verifiers 27/27 per variant (8 corrupt cases FAIL); sun flare, lod occlusion, chase fire, fog
pass / route bridge / family, bloom and cull census reused from Run82 (no dependent source changed); five dry runs (the stand adds
X3M_TAA_THIN_REGION_SOURCE=vote/_DEFAULT=1, X3M_WINDOW_MONITOR_RECT=1/_DEFAULT=1, X3M_ORIGINAL_FILL + _DEFAULT=1 and no
stabiliser variable; vanilla none; `--taa-sentinel-stabiliser` exits 2). Not a native Windows execution. The install record
