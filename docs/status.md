...
Bottle **X3**, **CrossOver Preview.app**. Run87 DLL SHA-256:
`94c4ef4202abb6a4ffaa459ee540003c8f8636d62e0a3166e82a305efef90081`
(56,856,749 bytes), built once from clean reviewed main `22593133` in a detached worktree (`/tmp/x3-run87-candidate/src`).
Retained DLL: `/tmp/x3-run87-candidate/build/d3d9.dll`. Installed 2026-09-25 18:06
([qualification](../verification/results/run87-candidate-qualification.json), [install](../verification/results/run87-candidate-install.json)).
Rollback: Run86 `27881669…` at `/tmp/x3-run86-candidate/build/d3d9.dll` (Run 86 A flown; carries the two dropped opt-ins), then
Run85 `c430294a…` (unflown, same as Run87 minus the fog loader).

Content: Run85's **far clip 7x7 + far ramp 60/68** (the fog-band plants' sparkles, accepted run333) plus the **fog family
loader** accepting an empty table (28cbd43e). The effects stage (9375a1e7) and the chase view across docking (40cd3023) were flown
in Run 86 A and **dropped by the user** (removed 479cd036, 22593133). Launcher/tooling: `fog families:` and `lod overlay:` report
lines, the `fog-families` action, LOD overlay for selected mod packages (`lod_overlay.py --mod`). Game data: install-fleet4 overlay
unchanged; `drive_c/X3/x3m/fog-families.bin` (64 B empty table: 14 compiled families cover 14, 2 refused) + `fog-families.json`.

Qualification at `22593133`: build 0 warnings (46 s); x87 119 roots / 690 reachable / zero violations; imports 231 -> 231 and 17
exports unchanged against Run85; host suite 270 modules / 2,810 tests / 0 failing; fog family file 68/68 (empty_table loaded);
temporal pass 744 / 278, lattice 598 / 90 (rows identical to Run85); motion output 234 cases (0 differing stable fields vs Run85
and Run86); fog route bridge PASS (42,063 bridge checks); the other closures identical to Run85 and nested from its record
(the ownership fixtures nest the Run86 entry: never run under Wine on the Run85-equal ownership code in any record, an open
evidence gap); four dry runs (A stand = Run85's environment exactly, B `--taa-far-clip 3x3`, C `--taa-debug`, D vanilla) plus
the two dropped options refused as unknown. Not a native Windows execution. The install record verifies installed bytes and
unchanged X3AP.exe `fdbf3418…` and cxbottle.conf `cc5d6c00…`.
