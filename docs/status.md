...
Updated 2026-09-26 09:10: main 44c60abb carries three uninstalled DLL changes since Run90 (developer logging options trimmed to five incl. `--draw-trace`; hotkeys removed except F8, `--capture-start`/`-delay` gone; the `x3m.ini` config file with the schema-generated template and `tools/release/package.py`), no candidate built by user request (more follow-ups coming); the next candidate (Run91) needs a player-mode flight (`x3run --config`). Installed: Run90 DLL `6f6be732…` from f9cceb17 installed 04:00 (logging tiers: `--debug` / `--perf`, `x3m.log` in the game directory; obsolete options and code removed; `--taa-k` / `--taa-sentinel` removed; plus Run89's single shadow map removal and cascade 3 at 4096, never flown). Run 90 A queued (supersedes Run 89 A). In flight for the next candidate: the developer-option trim, inventory reorganisation, section-5 variable removal. Earlier: Run88 6fe194bd accepted in Run 88 A. Run56 accepted for media stability: the user
