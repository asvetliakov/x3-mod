...
Bottle **X3**, **CrossOver Preview.app**. Run80 DLL SHA-256:
`593112dc619f33bc9c90b115862d20649b1e7ff20916be510d205f4613504710`
(56,609,477 bytes), built once from clean reviewed main `9cf5decf` in a detached worktree (`/tmp/x3-run80-candidate/src`).
Retained DLL: `/tmp/x3-run80-candidate/build/d3d9.dll`. Installed 2026-09-24 19:49
([qualification](../verification/results/run80-candidate-qualification.json), [install](../verification/results/run80-candidate-install.json)).
Rollback: Run79 `d3683ced…` at `/tmp/x3-run79-candidate/build/d3d9.dll` (Run 79 A accepted A' and the Terran patch), then
Run78 `d4ba9f05…`.

Changes against Run79 (`d3683ced…`): **A' only**: the dilated camera chain is removed and `--taa-region-hold` is refused
(bb3a691f; 19 TAA programs, refusal = region off with one row); opt-in, all default off and unflown: **`--lod-occlusion
record0|all`** (EXE patch so lower LOD records bind their occlusion map, ac381be7), **`--taa-thin-vote on|off`** (B,
4de081ae), **`--fade-rt2-owner on|off`** (ed038a7d), **`--lod-switch-log [N]`** (e8af9e46); bolt shape-refusal telemetry
(aa6d2e18). Terran station LOD patch and the A' region hold unchanged and accepted. Programs above 512 slots by design.
Game data: fleet overlay `addon/05` (493 bodies, 1,992,076,927 B) + `addon/06` (127), 620 bodies, rebaked with the new
resolver and alpha rule ([record](../verification/results/lod-overlay-batch/install-fleet3/install.json), replaces
install-fleet2's 611; the Terran red plates are their own atlas tiles); rollback to fleet2 needs a rebake from 33aba284.

Qualification at `9cf5decf`: host suite 255 modules / 2,667 tests (after the host-only fixes 1c37598a; the DLL is
unaffected); x87 116 roots / 683 reachable / zero violations; imports: two new UCRT imports (`_aligned_malloc/_free`,
the aligned thin-vote cache), exports unchanged; temporal pass 744 / 278 (report identical), lattice 554 / 91; motion
output 221 cases; fog pass 52/52 gates; route bridge 110 names PASS (42,135 checks); fog family 67 cases; GPU sync 32/32;
identity verifiers 25/25 per variant with the new lod_occlusion site (41 anchors, corrupt-site cases FAIL with identity
true); site verifiers lod_occlusion 20/20, Terran 16/16; cull census 119 (lod-switch checks); cull fixture 132; bloom 47/47;
six dry runs exit 0, `--taa-region-hold` refused. Not a native Windows execution. The install record verifies installed
