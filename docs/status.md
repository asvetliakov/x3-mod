...
Bottle **X3**, **CrossOver Preview.app**. Run81 DLL SHA-256:
`b4e945ed9e1f1c9273e9e028475551cf7c34ff61792b72f3d4a90185643ae3b4`
(56,719,270 bytes), built once from clean reviewed main `9e1645be` in a detached worktree (`/tmp/x3-run81-candidate/src`).
Retained DLL: `/tmp/x3-run81-candidate/build/d3d9.dll`. Installed 2026-09-24 22:15
([qualification](../verification/results/run81-candidate-qualification.json), [install](../verification/results/run81-candidate-install.json)).
Rollback: Run80 `593112dc…` at `/tmp/x3-run80-candidate/build/d3d9.dll` (Run 80 A accepted its opt-ins), then Run79
`d3683ced…`.

Changes against Run80 (`593112dc…`): **new defaults** (user decisions after Run 80 A): `--taa-thin-vote on`,
`--fade-rt2-owner on`, `--lod-occlusion all`, `--fov 58.7155` (Hor+ from a vertical FOV, F 0x3470 written over the cockpit
registry constructor's 0x4000; `--fov game` = vanilla; a5fddc6a), and under `--taa` the age programs `--taa-far-stabiliser
0.985` and `--taa-thin-region 0.97` (9e1645be; explicit off spellings; `default=` markers on the configured rows). **Opt-in,
unflown**: `--taa-box-resolution half` (S4, a2409fd5), `--shadow-alpha-casters on` (alpha-tested sun-shadow casters,
17868e9d). Thin-vote telemetry (c59fc6ac). Game data unchanged: fleet overlay install-fleet3 (620 bodies,
[record](../verification/results/lod-overlay-batch/install-fleet3/install.json)); rollback to fleet2 needs a rebake from
33aba284.

Qualification at `9e1645be`: host suite 259 modules / 2,703 tests / 0 failing; x87 116 roots / 684 reachable / zero
violations; imports: two new UCRT imports (`floor`, `frexp`), exports unchanged; temporal pass 744 / 278 (report identical),
lattice 594 / 92; motion output 226 cases (334,886 checks); fov patch 134/134; lod occlusion patch 108/108; cull fixture
142; GPU sync 32/32; identity verifiers 26/26 per variant (41 anchors, 5 corrupt-site refusals incl. the FOV site); site
verifiers fov 18/18, lod_occlusion 20/20, Terran 16/16, cull 21/21 + 21/21; fog pass / route bridge / fog family / bloom /
cull census reused from Run80 (no fog or bloom source changed); four dry runs exit 0 (stand sends all five defaults and
`X3M_FOV=58.7155`; all-off spellings send off; vanilla sends none). Not a native Windows execution. The install record
