...
Bottle **X3**, **CrossOver Preview.app**. Run85 DLL SHA-256:
`c430294a1bd136ee266899dbff2e1623a96f9d64a62cfd664a239fcb1cffdecf`
(56,857,261 bytes), built once from clean reviewed main `53e7aba5` in a detached worktree (`/tmp/x3-run85-candidate/src`).
Retained DLL: `/tmp/x3-run85-candidate/build/d3d9.dll`. Installed 2026-09-25 09:25
([qualification](../verification/results/run85-candidate-qualification.json), [install](../verification/results/run85-candidate-install.json)).
Rollback: Run84 `189a34f0…` at `/tmp/x3-run84-candidate/build/d3d9.dll` (Run 84 A flown), then Run83 `f0259ceb…`.

Changes against Run84 (`189a34f0…`): **`--taa-far-clip 7x7|3x3`** (default 7x7: far pixels outside the thin region clip their
history against the in-place 7x7 box instead of the 3x3, gated on the far weight times the camera-relative openness, so a sub-pixel
far line missed by the current jitter phase is no longer erased) and the **far stabiliser ramp 60/68** (full 0.985 weight from
87,040 view units = 17.4 km at 5120 width; was 102-166k), the pixel tier of the ratified thin classification
([design](architecture/taa-thin-classification.md); 22776b6f). The far-gate default stays camera (a screen-default attempt failed
the fixture: 19.2 codes vs 4.9 on a 0.4 px line under fractional pans). `thin_vote_frame` gains `max_unvoted_fraction`. Game data
unchanged: install-fleet4 overlay; the fog family file is not written for vanilla (16 families, 14 compiled cover them; the launch
line says "missing" until the tool writes an empty-packet file, follow-up in flight).

Qualification at `53e7aba5`: build 0 warnings (44 s); x87 119 roots / 690 reachable / zero violations; imports 231 -> 231 and 17
exports unchanged; host suite 269 modules / 2,795 tests (3 modules failed only on a `manage.py --help` format defect, fixed on main
89f4b014 and rerun OK); temporal pass 744 / 278, lattice 598 / 90 with 22 FAR_JITTER_LINE rows (7x7 on the camera gate 0 sparkles
on every row, 3x3 on the 0.4 px line 72 at rest; hold program 1,017 slots); motion output 234 summary cases / 250 case lines
(337,330 checks; 7 seam-thin-vote cases differ only in the new max_unvoted_fraction field); gpu sync, cursor re-assert, lod
occlusion, sun occlusion and the rest reused from Run84 (no dependent source changed); four dry runs (stand: X3M_TAA_FAR_CLIP=7x7
/_DEFAULT=1, X3M_TAA_FAR_GATE=camera/_DEFAULT=1, X3M_TAA_FAR_STABILISER=0.985,0,60,68,0.03,0.25; `--taa-far-clip 3x3` 3x3/0;
`--taa-debug` adds X3M_TAA_DEBUG=1; vanilla none). Open: no motion-output case exercises the 7x7 path (temporal fixture only); not
flown. Not a native Windows execution. The install record verifies installed bytes and unchanged X3AP.exe `fdbf3418…` and
