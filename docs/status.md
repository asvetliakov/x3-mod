...
Bottle **X3**, **CrossOver Preview.app**. Run84 DLL SHA-256:
`189a34f08b0b2bacf53286e0b2d284a4303ac87713e0b0e90af68805ef23e75d`
(56,854,000 bytes), built once from clean reviewed main `46cd4f9d` in a detached worktree (`/tmp/x3-run84-candidate/src`).
Retained DLL: `/tmp/x3-run84-candidate/build/d3d9.dll`. Installed 2026-09-25 07:15
([qualification](../verification/results/run84-candidate-qualification.json), [install](../verification/results/run84-candidate-install.json)).
Rollback: Run83 `f0259ceb…` at `/tmp/x3-run83-candidate/build/d3d9.dll` (Run 83 A/B flown), then Run82 `cd8ef8e4…`.

Changes against Run83 (`f0259ceb…`): **`--taa-far-gate camera|screen`** (default camera: on the camera-gate resolve the far
stabiliser's 0.985 weight follows the camera-relative openness instead of screen speed, removing the one-frame sparkles on far
plant edges under a pan at the cost of softer far static content during fractional pans; `screen` = the Run83 gate; the Run 84 A
A/B decides the default; 46cd4f9d), **`--sun-occlusion` on by default** with the core dimming (`--no-sun-occlusion` restores the
vanilla probe; 50a5f98e), **`--cursor-reassert` arming once at launch** and `--window-trace` recording from the first Present
(b45b3735), and the **`--original-fill 0.01` launcher default** (07cdeb45). Game data unchanged: install-fleet4 overlay.

Qualification at `46cd4f9d`: host suite 268 modules / 2,784 tests / 0 failing; x87 119 roots / 690 reachable / zero violations;
imports and exports unchanged against Run83; temporal pass 744 / 278 (report identical), lattice 581 / 90 (FAR_CAMERA_PAN in both
gates); motion output 234 summary cases / 250 case lines (337,330 checks, 0 differing vs Run83); gpu sync 32/32; cursor re-assert
42/42; lod occlusion 108/108 (runner marker change); sun occlusion hook 74/74 + GPU 128/128; fov patch, window mode, sun flare,
cull, chase, fog, bloom, census, site verifiers and the identity matrix reused from Run83 (no dependent source changed); four dry
runs (the stand + trace/re-assert sends X3M_TAA_FAR_GATE=camera/_DEFAULT=1, X3M_SUN_OCCLUSION=1/_CORE_F=1/_DEFAULT=1,
X3M_ORIGINAL_FILL=0.01/_DEFAULT=1, source vote/1, no stabiliser variable; `--taa-far-gate screen` screen/0; `--no-sun-occlusion`
none; vanilla none). Not a native Windows execution. The install record verifies installed bytes and unchanged X3AP.exe
