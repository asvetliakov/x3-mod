...
| Host tests | `PYTHONPATH=verification/probe python3 -m unittest` over `test_collide_memo`, `test_collide_sat_sse2`, `test_collide_box_cull`, `test_collide_narrow_census`, `test_cull_small_parts`, `test_env_experiment_launch`, `test_d3dx_override_launch` | 67 tests, OK (the two launcher-experiment modules were deleted 2026-09-22) |
...
**Removed 2026-09-25** (user decision): `--profile-raw` (the raw witness and periodic callback) and `--audio-sites` (the 14 audio game-phase sites; 33 remain); `--profile` and `--profile-interval-us` stay; `docs/verification/launcher-options-inventory.md`, "4. Removed".
...
**Removed 2026-09-26** (user decision, developer options trimmed to four): the launcher options `--game-phases`, `--game-phase-threshold-ms`, `--pass-phases`, `--residual-phases`, `--light-phases`, `--loop-phases`, `--submit-phases`, `--telemetry-draw` and `--frame-timing-state-stamps` (exit 2). `X3M_GAME_PHASES` (tape threshold at its 20 ms default), `X3M_PASS_PHASES`, `X3M_RESIDUAL_PHASES`, `X3M_LIGHT_PHASES`, `X3M_LOOP_PHASES` and `X3M_TELEMETRY_DRAW` are expanded in the DLL from `--draw-trace` (`X3M_DRAW_TRACE`, with the frame phases; requires `--perf` or `--debug`); `X3M_SUBMIT_PHASES` stays a fixture-only read (it claims the lens call the sun-occlusion default patches); `X3M_GAME_PHASE_THRESHOLD_MS` and `X3M_FRAME_TIMING_STATE_STAMPS` stay fixture reads. The flight recipes above that name these options now read `--perf --draw-trace`.
