...
| 69 A/B/C | Merged-LOD pilot overlay (addon/05.cat, installed DLL): two-group collapse (A), glow collapse (B), compact placement at 80 / 150 px (C) | 3 | A completed: run257, look fine, engine glows lost (exhaust light maps), 297 vs 447 draws ([note](../architecture/engine-frame-time.md#run-257-merged-lod-pilot-in-flight-2026-09-23)). B completed: run258, glows back (user). C queued 2026-09-23. |
...
Run 69 C is the only queued run. Run 69 B returned **run258** (engine glows back with the glow collapse; user asks for 80 / 150). Run 69 A returned **run257** (the pilot draws as designed, 297 vs 447 draws; engine glows lost because the two-group collapse dropped the exhaust materials' light maps; fixed by the glow collapse, reinstalled).


**Run 69 C (queued 2026-09-23): the installed Run68 DLL `39c8c70d…` (no new DLL); the overlay is now the compact build at the larger switch sizes: `addon/05.cat` `cf6fd61e…` / `05.dat` `bd311b54…` ([install record](../verification/results/lod-overlay-pilot/install.json)). Start the game fresh.**

Session C, switch sizes ships 80 px / stations 150 px, compact placement (the coarse record is LOD 1 now, so the bump-mapped technique applies to it, and the original LOD 1–3 records are gone). Judge at the fighter-save view and at the Argon Prime stand of Run 68 B: (1) the Titan / Colossus / Mammoth below 80 px and the outpost below 150 px with one hull texture plus the exhaust glows: acceptable, or is the texture substitution visible now (details of other materials painted with the dominant hull texture)? (2) the switch pop when crossing 80 px; (3) anything else missing at distance. One delayed F8 at each view. Same command as Run 69 A/B:
