...
| 69 A/B | Merged-LOD pilot overlay (addon/05.cat, installed DLL): two-group collapse (A), glow collapse (B) | 2 | A completed: run257, look fine, engine glows lost (exhaust light maps), 297 vs 447 draws ([note](../architecture/engine-frame-time.md#run-257-merged-lod-pilot-in-flight-2026-09-23)); B queued 2026-09-23. |
...
Session B, the same two captures as Run 69 A: the fighter-save busy view (Titan at ~41 px, below the 50 px switch) and the closer one (Titan at ~56 px, above it). The coarse records now keep the exhaust materials as their own groups, so each ship is 5 draws (the outpost 3) and the engine glows should be back at distance. Judge: (1) engine glows present on the Titan / Colossus / Mammoth while they are below 50 px; (2) anything else missing at distance (window lights on the hull are expected to be gone, they sit on near-black light maps); (3) any pop at the 50 px switch. If the glows are back and nothing else is wrong, say whether the switch sizes should go up (ships 80, stations 150 would cover nearly every heavy body at the Argon Prime stand). Same command as Run 69 A:
