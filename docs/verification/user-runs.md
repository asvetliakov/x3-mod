...
| 69 A | Merged-LOD pilot overlay at the stand (addon/05.cat, installed DLL) | 1 | Queued 2026-09-23. |
...
Run 69 is the only queued run. Run 68 returned **run254** (A: SETA smear gone by the numbers, 7 % / 6 % against 27 % / 34 %; strict + band + exit reset become defaults; the SETA station blur is resample softening, a motion-weight option is being built), **run255** (B: 466 draws at the stand, the pilot placement corrected to the pad rule, T_pad 50/100) and **run256** (C: the toggle worked without a notice, no visible or median-cost difference, the pass stays off).


**Run 69 (queued 2026-09-23): the installed Run68 DLL `39c8c70d…` (no new DLL); the merged-LOD pilot overlay `addon/05.cat` + `05.dat` is installed (SHA-256 `c5737a0a…` / `66f4f7a6…`, [install record](../verification/results/lod-overlay-pilot/install.json)). The game must be started fresh so it mounts the new catalogue.**

Session A, the pilot overlay at the run248/run255 stand. The four bodies argon_TL, argon_M2, argon_M1 (switch below 50 px) and military_outpost_middleb (below 100 px) now carry one coarse record with two material groups (opaque + alpha cut-outs), so at the stand each of those nodes should draw 2 draws instead of 31–34 and the census should report LOD index 4 (ships) / 3 (outpost). Judge the look: (1) at the stand (ships at 30–45 px), do the TL / M2 / M1 and the outpost look acceptable with one texture over the whole hull (the lattice cut-outs are kept; details of other materials get the dominant texture)? (2) fly slowly toward one of them and back through ~50 px: is the switch between the fine and the coarse record a visible pop, and where? (3) do any attached parts (turrets, antennas, docking arms) vanish while the coarse record is drawn (the engine hides flagged children below the switch size)? One delayed F8 at the stand and one while close enough that the same ship is above the switch size. Vanilla LOD, no lod-scale.
...
To remove the pilot: delete `addon/05.cat`, `05.dat` and `05.x3m-lod.json`; the originals are untouched (34 archive files hash-checked at install).
