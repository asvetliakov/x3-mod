...
Run 69 D is the only queued run. Run 69 C returned **run259** / **run260** (80 / 150 px acceptable; the coarse model is sun-lit but 23 % weaker and loses 72 % of its light-map self-illumination, from the collapse; fixed by the area-ranked rule and the synthesized material). Run 69 B returned **run258** (engine glows back with the glow collapse; user asks for 80 / 150). Run 69 A returned **run257** (the pilot draws as designed, 297 vs 447 draws; engine glows lost because the two-group collapse dropped the exhaust materials' light maps; fixed by the glow collapse, reinstalled).


**Run 69 D (queued 2026-09-23): the installed Run68 DLL `39c8c70d…` (no new DLL); the overlay is now the area-ranked build: `addon/05.cat` `87bf16cd…` / `05.dat` `c2f0abd6…` ([install record](../verification/results/lod-overlay-pilot/install.json)). Start the game fresh.**

Session D, same switch sizes (80 / 150 px), compact placement, collapse `glow-area 70`: besides the exhausts, the largest light-mapped hull materials up to 70 % of the lit area keep their own material (TL 9 draws, M2 8, M1 9, outpost 10 instead of 5 / 5 / 5 / 3), and the outpost's collapsed group uses a synthesized material with the area-weighted diffuse and specular strengths of the materials it absorbs. Fly the same paired captures: the military outpost just below 150 px (coarse) and just above (fine) at an unchanged view, and one ship pair across 80 px if you can. Judge: (1) is the "no sun lighting" impression gone on the outpost and on the ships; (2) what still changes at the switch (expected: a smaller texture difference on the ships, since about 33 % of their hull area still gets the dominant plating texture); (3) any pop. Same command as Run 69 C:
