...
| 74 A | Re-baked overlay: aspect thresholds + area-weighted texel rule, 22 bodies, busy sector, Run73 DLL | 1 | Queued 2026-09-23 18:30 ([record](../verification/results/lod-overlay-batch/install-run74/install.json)). |
...
Run 73 B and C and Run 74 A are queued. Run 73 A returned run272 (triaged; the overlay bodies draw 2-4, the rest is refused bodies, pipes and signs).
...
**Run 74 A (queued 2026-09-23 18:30): Run73 DLL unchanged. Game data: the flown-sector overlay was re-baked with the aspect-aware thresholds and the area-weighted texel rule and installed in `addon/05` (22 bodies, the two tech laser factories and the Argon gate recovered; 651 -> 67 drawn groups below the switch; 127 MB; [record](../verification/results/lod-overlay-batch/install-run74/install.json)).** Fly the Run 73 A command in the same busy sector. Watch: (1) the tech laser factories and the gate near their switch (roughly 1.5 km for the factories, look for a stretched or smeared strip on a hull plate, which would be a span-clamped face); (2) flat stations (equipment dock, spacedock) now switch at two thirds and half the old distance, so look for a visible pop when approaching them; (3) capital ships switch closer too (M2 at 6 km instead of 8.4). Two F8 bursts: one with the busy-sector stations at a distance, one at a tech factory just outside its switch. Report FPS against Run 73 A.
