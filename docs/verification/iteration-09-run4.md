...
synthetic evidence in [hdr-scene-path verification](hdr-scene-path.md)). The
...
frame, `dirty_at_present=0`)", [hdr-scene-path.md](hdr-scene-path.md)), now
...
* It is a **self-consistency check of one write-back**, not the synthetic
  ...
  [hdr-scene-path.md](hdr-scene-path.md) §"Case 1" come from double rounding
