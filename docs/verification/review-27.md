# Review 27: loading trampolines, light hooks, resource reader (paused 2: two suite failures to triage)
...
**TODO (paused 2, orchestrator's account switch):** the two failures are in
units this branch does not touch (`object_lifetime.cpp`, the mesh adjacency
cache/`mesh_adjacency_fast`), both last changed on main by `b10d129` "WIP
checkpoint … adjacency parity rewrite (unverified)", which this branch merged
in `20f7ffd`. Before merging: run the same two suites on plain main at
`28a003d`/`b10d129` to confirm they fail there too (then they are main's WIP,
not this review's), or bisect against the merge. Failing object-lifetime
checks: names not exposed in the summary JSON — read verification/results/object-lifetime.txt with grep.
Mesh adjacency runner failure detail: see verification/results/mesh-adjacency-cache-summary.json.
Also TODO: the fixture result files regenerated by this chain are committed
with the checkpoint; the four `verification/results/bottle-X3/loading-*` and
`mesh-adjacency-cache-off-fixture.txt` files were reverted to main's copies
(their X3 reruns were not part of this chain).
