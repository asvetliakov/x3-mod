...
Obsolete-option removal (2026-09-25, bottle X3, `wine_lock.py`), after the review fixes: `run_motion_output.py` PASS, 229 cases, 346,327 checks, every case kept from Run89 with its Run89 count; deleted with their options: `seam-taa-taps16`, `seam-taa-thin-taps16-refused`, `seam-taa-region-hold-ignored` (164 each), `seam-thin-vote-far-on-source-both` / `-screen` (61 each) = 614 checks; added `seam-taa-no-filter-refused` (103): the seam's filter query refuses at attach (`X3M_FIXTURE_TAA_FILTER_FAULT`), TAA is off with one `motion_output_taa initialize=8876086a references=0 reason=fp16_filter effect=taa_off` row, the fixture's coverage and pixel-ABI oracles see no jitter on any draw, and the presented frames equal `seam-on`'s byte for byte (measured; `verification/results/launcher-defaults/compare_motion_counts_2026-09-25.py` over the committed per-case records `motion-cases-run89.json` / `motion-cases-2026-09-25.json`). `run_sun_share_live.py` over the same fixture and seam: passed, 25 cases.

**In-game keys removed 2026-09-26** (user decision; `docs/architecture/comparison-hotkeys.md`, "Removed 2026-09-26"): `seam-ownership-shadow-replay-cascades-toggle` (283 checks) and `seam-ownership-shadow-replay-toggle-single` (250) deleted with the Ctrl+Shift+F12 seam. Rerun on bottle X3 under `wine_lock.py` after the review fixes (F8 flag cached at `initialize_log`): 46 cases (`seam-exit-path`, the 27 shadowreplay cases with `seam-log-tiers`, 7 lightmapwiden, 4 lightmapfade, 6 sunapply and the capture case `production-burst-lazy`), 32,056 checks, every case exit 0 at its committed count (`verification/results/logging-tiers/motion-cases-hotkeys-2026-09-26.json`); the suite is 229 cases / 345,859 checks (`compare_motion_counts.py` PASS: the committed 2026-09-26 record minus the two deleted cases, the rerun cases from the new file).
