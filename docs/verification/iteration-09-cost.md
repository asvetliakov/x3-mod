...
[iteration-08.md §6](iteration-08.md) closed with *"the route's own cost is
...
**gate is 69 %** (11.6–13.1 µs per draw); [route-cost-run1.md](route-cost-run1.md)
...
([telemetry.md](telemetry.md#route-and-boundary-cost)). Only the mutually
...
[iteration-08.md §6](iteration-08.md) — `--window-mode-split-us`. Medians over
...
[route-cost-run1.md §3](route-cost-run1.md) counts 4,382–7,831 stamps per frame
...
[route-cost-run1.md §3](route-cost-run1.md), which symbolizes the same spans
against the in-process profiler and is the stronger evidence wherever the two
overlap.

| # | lever | now | expected saving | evidence |
| ---: | --- | ---: | ---: | --- |
| 1 | **Replace the observers' self-`ReadProcessMemory` with `VirtualQuery`-validated direct reads** (`object_trace`, `object_lifetime`, `scene_hook`) | `route_gate` **3,410 µs/frame = 11.6 µs/draw = 69 % of the route** | **−3.2 ms/frame** (flight B) / −5.5 ms (flight A) | high — [route-cost-run1.md](route-cost-run1.md) puts 92–98 % of the gate span in ~21 `NtReadVirtualMemory` syscalls per routed draw. This report independently shows the gate costs 11.6 µs/draw in the sector against **0.26 µs/draw in the menu**, where the same gate rejects at gate 2 before the observer reads — a 45× ratio that is consistent with the reads being the span |
...
| 4 | **`X3M_MOTION_RT_MODE=lazy`** | `route_set_rt` **611 µs/frame** (0.773 µs × 4 per routed draw), *nested inside* `route_draw` | **−244 µs/frame** (40 %), bracketed −0.2…−0.6 ms | low — the fixture's burst case gives 20 → 12 binds ([motion-output.md](motion-output.md#lazy-rt-binding-equivalence-x3m_motion_rt_mode)); with 70 % of draws routing and rejections interleaved, the gameplay bind structure is unmeasured. **This is the smallest of the three real levers**, not the first one to reach for |
