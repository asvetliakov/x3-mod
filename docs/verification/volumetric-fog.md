...
**Removed 2026-09-25** (user decision): `--fog-shadow-pass`, `--fog-far-bins` (only the 40-bin programs remain), the `--volumetric-fog-look` refusal stub with its DLL ignore line, and the `X3M_VOLUMETRIC_FOG_ANISOTROPY` read; `docs/verification/launcher-options-inventory.md`, "4. Removed".
...
**Removed 2026-09-26** (user decisions): `--volumetric-fog-everywhere` with `X3M_VOLUMETRIC_FOG_EVERYWHERE`, the forced bluewell profile of `fog_sector_frame` (its `forced` field and the `forced_profile=` field of `volumetric_fog_sector`) and the forced target of `FogSectorLatch::update`; the `X3M_FOG_LOOK_<NAME>` overrides (24 scalars, `AMBIENT_SUN` / `AMBIENT_AWAY`) and the mote tunables `X3M_FOG_MOTES_RADIUS/NEAR/GAIN/SOFT/DRIFT/SEED` (values baked as the struct defaults; `X3M_FOG_MOTES_MAX_PX` stays; `volumetric_fog_look_mode` reads `look=single constants=baked`). Fixture (bottle X3): `fog_density_shader_run.py` PASS 28 / 28 gates, pass fixture 130 checks, 42 non-provenance digests of the committed `summary.json` / `s2.json` / `q4.json` identical, the look hash sets (`pass_off_bit_identical`, `s2_pass_off_bit_identical`) unchanged (measured; `verification/results/logging-tiers/fog_hashes_compare.py`; the scale-2 reference regenerated without the retired grid cases, the scale-4 one `/tmp/x3-run77-fog-ref-scale4`). Route bridge (build, `--baseline` from an archive of 6f16dbf6 at `/tmp/x3-fog-bridge-base`, `build-exit`, `run --cases /tmp/x3-fog-family-gpu-inputs-final/cases.txt`, `check`): PASS twice, 31,745 and 31,451 bridge checks (the per-frame checks follow the readiness timing: 31,907 on 2026-09-25), 95 check names in both runs (110 before the 2026-09-25 removal of the 15 shadow A/B names), 4 exit checks, no FAIL row (measured). The bridge's owner mock keeps `fog_everywhere_` because the 6f16dbf6 baseline fragment reads it.

**Ctrl+Alt+F9 / F10 / F11 removed 2026-09-26** (user decision; `docs/architecture/comparison-hotkeys.md`, "Removed 2026-09-26"): the fog runs at its launch strength with its launch motes; `volumetric_fog_toggle` / `_step` / `_dust_motes_toggle` stay as seams of the fog route fixtures (the motes export went), the `key=ctrl_alt_f11` / `keys=ctrl_alt_f9,ctrl_alt_f10` fields are gone and `volumetric_fog_begin_frame` runs every frame at the frame boundary; the route bridge was not rerun (its motes check drops the `key=` substring only).
