...
function(x3m_add_fog_field_assets target)
  ...
  # The recipe is the resource inventory; configuration does not bake fields.
  set_property(DIRECTORY APPEND PROPERTY CMAKE_CONFIGURE_DEPENDS "${_recipe}" "${_tool}")
  execute_process(COMMAND "${Python3_EXECUTABLE}" "${_tool}" --list-profiles
    RESULT_VARIABLE _status OUTPUT_VARIABLE _families OUTPUT_STRIP_TRAILING_WHITESPACE)
  if(NOT _status EQUAL 0 OR NOT _families)
    message(FATAL_ERROR "Cannot read spatial-fog profile inventory")
  endif()
  set(_packets)
  foreach(_family IN LISTS _families)
    list(APPEND _packets "${_dir}/${_family}.fogbin")
  endforeach()
  set(_meta "${_dir}/fog_field_assets_metadata_inc.h")
  set(_resource "${_dir}/fog_field_assets_resource_inc.h")
  set(_rows "${_dir}/fog_field_assets_entries.rc")
  set(_outputs ${_packets} "${_meta}" "${_resource}" "${_rows}" "${_dir}/manifest.json")
  add_custom_command(OUTPUT ${_outputs}
    COMMAND "${Python3_EXECUTABLE}" "${_tool}" --output-dir "${_dir}"
    DEPENDS "${_tool}" "${_recipe}" VERBATIM COMMENT "Baking spatial-fog family fields")
  add_custom_target(x3m_fog_field_assets DEPENDS ${_outputs})
  file(MAKE_DIRECTORY "${_dir}")
  configure_file("${CMAKE_CURRENT_SOURCE_DIR}/cmake/fog_field_assets.rc.in" "${_dir}/fog_field_assets.rc" @ONLY)
  set_source_files_properties("${_dir}/fog_field_assets.rc" "${_meta}" "${_resource}" "${_rows}" PROPERTIES GENERATED TRUE)
  set_source_files_properties("${_dir}/fog_field_assets.rc" PROPERTIES OBJECT_DEPENDS "${_packets};${_resource};${_rows}")
