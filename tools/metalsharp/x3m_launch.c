...
static int read_kv(const char *path, const char *prefix, int set_env, char *dir, char *exe, char *args) {
    ...
    char line[32768];
    int count = 0;
    if (!f) { logline("cannot open %s", path); return -1; }
    while (fgets(line, sizeof line, f)) {
        ...
        *eq = 0;
        if (set_env) {
            if (!SetEnvironmentVariableA(line, eq + 1)) logline("SetEnvironmentVariable %s failed %lu", line, GetLastError());
            else count++;
        } else {
            if (!strcmp(line, "dir")) strncpy(dir, eq + 1, 32767);
            else if (!strcmp(line, "exe")) strncpy(exe, eq + 1, 1023);
            else if (!strcmp(line, "args")) strncpy(args, eq + 1, 4095);
            count++;
        }
    }
    fclose(f);
...
static void log_inherited(void) {
    static const char *prefixes[] = {"WINE", "DXMT", "METALSHARP", "ROSETTA", "MVK_", "VK_", "DYLD_", "MS_", "CX_", "X3M_LAUNCH", NULL};
    char *env = GetEnvironmentStringsA(), *p = env;
    if (!env) return;
    for (; *p; p += strlen(p) + 1) {
        int i;
        for (i = 0; prefixes[i]; i++)
            if (!strncmp(p, prefixes[i], strlen(prefixes[i]))) { logline("inherited %s", p); break; }
    }
...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline_unused, int show) {
    ...
    STARTUPINFOA si; PROCESS_INFORMATION pi; DWORD code = 1; SYSTEMTIME now;
    (void)inst; (void)prev; (void)cmdline_unused; (void)show;
    GetModuleFileNameA(NULL, self, sizeof self);
    strcpy(base, self); *strrchr(base, '\\') = 0;
    snprintf(path, sizeof path, "%s\\x3m-launch.log", base);
    logf = fopen(path, "w");
    GetLocalTime(&now);
    logline("x3m-launch %04u-%02u-%02u %02u:%02u:%02u pid %lu self %s", now.wYear, now.wMonth, now.wDay, now.wHour, now.wMinute, now.wSecond, GetCurrentProcessId(), self);
    log_inherited();
    snprintf(path, sizeof path, "%s\\x3m-launch.cfg", base);
    if (read_kv(path, "cfg", 0, dir, exe, args) < 0 || !dir[0] || !exe[0]) { logline("cfg incomplete: dir='%s' exe='%s'", dir, exe); return 2; }
    ...
    } else strcpy(overrides, "d3d9=n,b");
    ...
    if (!CreateProcessA(NULL, cmdline, NULL, NULL, FALSE, 0, NULL, dir, &si, &pi)) { logline("CreateProcess failed %lu", GetLastError()); return 4; }
