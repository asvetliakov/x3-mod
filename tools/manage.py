...
def main():
    ...
    parser.add_argument('--sun-occlusion-core-f', action='store_true', help='Flight comparison (X3M_SUN_OCCLUSION_CORE_F=1): the per-pixel clipped core bodies of the sun are also multiplied by the visibility fraction f (default: clipped only; ghosts and streaks carry f); requires --sun-occlusion')
    ...
    if (args.sun_occlusion_radius is not None or args.sun_occlusion_curve is not None or args.sun_occlusion_core_f) and not args.sun_occlusion:
    ...
    if args.action in ('install', 'uninstall', 'rollback', 'recover'):
        ...
        for name, value in (('X3M_SUN_OCCLUSION', '1' if args.sun_occlusion else None), ('X3M_SUN_OCCLUSION_LOG', '1' if args.sun_occlusion_log else None),
                            ...
                            ('X3M_SUN_OCCLUSION_CORE_F', '1' if args.sun_occlusion_core_f else None)):
            if value is None:
                env.pop(name, None)
            else:
                env[name] = value
