...
def main():
    ...
    parser.add_argument('--collide-sat-sse2', action='store_true', help='Replace the engine\'s x87 OBB-OBB separating-axis test of the collision BVH descent with an SSE2 reimplementation (X3M_COLLIDE_SAT_SSE2=1; default absent = nothing patched; independent of --collide-box-cull and --collide-narrow-census): the sole call of 0x004e3280, at 0x004e25a3, is redirected. The box test only prunes the descent and the replacement separates only where the engine\'s compare separates with a 2^-20 relative margin to spare, and on an unordered (NaN) compare exactly as the engine does, so the same contacts are found. No counters of its own: fly it with --collide-narrow-census and compare narrow_us / node_pairs / tri_tests with the option on and off. Exact executable and bytes only, otherwise fails closed to vanilla (docs/reverse-engineering/sector-collide.md, section 12.8)')
    parser.add_argument('--collide-descent-sse2', action='store_true', help='Replace the engine\'s x87 OBB-tree descent of the collision narrow phase (0x004e2530: head checks, per-child transform composition, recursion) with an SSE2 reimplementation (X3M_COLLIDE_DESCENT_SSE2=1; default absent = nothing patched; independent of --collide-sat-sse2, --collide-box-cull and --collide-narrow-census): the sole external call, at 0x004e2956, is redirected. Same node pairs in the same order, the same early-out checks at every entry, the SAT of --collide-sat-sse2, and the engine\'s own leaf triangle test, so the same contacts in the same order; never caps the descent. The census keeps counting node pairs and triangle tests. Exact executable and bytes only, otherwise fails closed to vanilla (docs/reverse-engineering/sector-collide.md, section 13)')
    ...
    if args.action == 'install':
        ...
        # Collide SAT SSE2: same rule (independent of the box cull and the narrow census).
        if args.collide_sat_sse2:
            env['X3M_COLLIDE_SAT_SSE2'] = '1'
        else:
            env.pop('X3M_COLLIDE_SAT_SSE2', None)
        # Collide descent SSE2: same rule (independent of the SAT replacement, the box cull and the narrow census).
        if args.collide_descent_sse2:
            env['X3M_COLLIDE_DESCENT_SSE2'] = '1'
        else:
            env.pop('X3M_COLLIDE_DESCENT_SSE2', None)
