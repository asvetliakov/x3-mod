...
def main():
    ...
    parser.add_argument('--taa-sentinel-stabiliser', default=None, metavar='S[,E]', help='Sentinel stabiliser of the TAA resolve against the flicker of distant stations the engine draws blended without depth, under a camera pan (X3M_TAA_SENTINEL_STABILISER; requires --taa-thin-region with W > 0 and the camera gate; default absent = off, bit-identical; suggested 0.7): unrouted pixels on the empty-depth sentinel (sky, far blended stations) get thin-region strength S (0..1) through the camera gate, always with the history clipped to the 7x7 min/max box of the current colour, closed within 8 px of anything routed that moves against the camera path. E (default 1, 0 = none) is the emitter bound in scene luma: next to anything brighter (lasers, trails, suns) the box shrinks to the inner 3x3, so their ghost stays within one pixel. Slight ghosting of unrouted content over sky (at most 3 px, decaying) is the accepted trade (docs/architecture/temporal-integration.md)')
    ...
    parser.add_argument('--volumetric-fog-look', type=int, choices=(0, 1, 2, 3), default=None, help='Starting look preset of the stored fog range: 0 current law (default), 1 shaped (density remap, thicker cores, two-lobe phase, coloured ambient, tinted extinction), 2 adds Beer-powder and sun-ward self-shadow, 3 adds the per-frame sample offset. Ctrl+Alt+F11 cycles in flight; --fps-overlay shows L0..L3. Looks 1-3 use their own two-lobe phase: --volumetric-fog-anisotropy has no effect under them. Tuning: X3M_FOG_LOOK_<NAME> environment variables, read once (X3M_VOLUMETRIC_FOG_LOOK; requires --volumetric-fog-range stored).')
    ...
    if args.action in ('install', 'uninstall', 'rollback', 'recover'):
        ...
        env['X3M_VOLUMETRIC_FOG_LOOK'] = str(args.volumetric_fog_look or 0)
