...
def main():
    ...
    if args.action in ('install', 'uninstall', 'rollback', 'recover'):
        ...
        # LOD occlusion: always explicit on a modded launch (record0, the engine's bytes, unless
        # --lod-occlusion all), so a stale shell value cannot patch the gate; dropped under --vanilla (refused above).
        if args.vanilla:
