...
# "Defaults promoted"): every functional option of the Run 84 A stand command is a launcher default on a
...
def apply_promoted_defaults(args):
    ...
    # Capture: no automatic capture on a default launch; F8 captures 8 frames on demand after 300 frames (the stand's values).
    fill('capture_start', 999999)
    fill('capture_frames', 8)
    fill('capture_delay', 300)
    fill('direct', True)
    fill('camera', 'chase')
    ...
    for dest, off in (('camera', 'vanilla'), ('resource_read', 'native'), ('mesh_adjacency', 'native'),
                      ('capture_start', 120), ('capture_frames', 1), ('capture_delay', 0)):
        if getattr(args, dest) is None:
...
def main():
    ...
    parser.add_argument('--capture-start', type=int, default=None, help='[launcher default since 2026-09-25: 999999 on every modded launch = no automatic capture, F8 captures on demand; --capture-start 120 restores the automatic capture at frame 120; 120 under --vanilla] Present count of the automatic first capture (X3M_CAPTURE_START)')
    parser.add_argument('--capture-frames', type=int, choices=range(0, 65), metavar='0..64', default=None,
                        help='[launcher default since 2026-09-25: 8 on every modded launch, 1 under --vanilla] Consecutive capture frames (X3M_CAPTURE_FRAMES). Above 8 is meant for the raw --taa-debug dumps (32 frames separate the 8-frame jitter ripple from slower crawl): about 40 MB per frame at 1280x768 on the HDR route (hdr + taa rgba16f 7.9 MB each, motion rgba32f 15.7 MB, depth 3.9 MB or 15.7 MB on the sun lane, present bgra8 3.9 MB), so 1.3-1.7 GB for 32 frames')
    parser.add_argument('--capture-delay', type=int, choices=range(0, 36001), metavar='0..36000', default=None,
                        help='[launcher default since 2026-09-25: 300 on every modded launch, 0 under --vanilla; --capture-delay 0 = start at once] Frames between the F8 press and the start of the capture burst (X3M_CAPTURE_DELAY; 0 = start at once). '
    ...
    parser.add_argument('--debug', action='store_true', help='Debug logging tier (X3M_DEBUG=1; inert under --vanilla, where no proxy loads): the rendering-state rows every frame (motion_output_frame, hdr_frame, camera_state, the shadow/sun state rows), the F8-frame censuses and bounds (cull census, LOD switch rows, object bounds), the traces (window, music, media cue, sun, sector background, loading probes, collide narrow census and query phases), the retention census and the 300-frame health windows; telemetry counters on; the ten frame-phase stamps are its only engine patches (the heavy attribution is --draw-trace). About 8.0 KB of log per frame (about 1.7 GB per hour at 60 fps; 9.4 KB, 2.0 GB per hour with --perf; F8 capture frames add their burst). Independent of --perf; the stand is --debug --perf (docs/architecture/logging-tiers.md)')
    parser.add_argument('--perf', action='store_true', help='Performance logging tier (X3M_PERF=1; inert under --vanilla, where no proxy loads): frame_end every frame, the per-frame fog and shadow cost rows, the 300-frame frame-time and engine-phase windows with their slow-frame witnesses, the 1 Hz telemetry summaries and loading metrics, the family rows every 60 frames and the FPS overlay (Ctrl+Alt+F7 hides it); the cheap tier, carried by the stand command (the per-draw route cost and the engine-stamp families are --draw-trace). About 1.7 KB per frame (about 360 MB per hour at 60 fps; gzip about 20x). What a stutter report needs (docs/architecture/logging-tiers.md)')
    # Removed 2026-09-26 (logging tiers): --telemetry and --frame-timing stay registered as hidden refusal stubs that name
    # their replacement (registered first because argparse abbreviated them to --telemetry-draw and
    # --frame-timing-state-stamps, both removed the same day); both are refused below.
