...
def apply_promoted_defaults(args):
    ...
    fill('shadow_cascade_sizes', '2048,4096,4096,2048,2048' if five else '2048', cascades)
...
def main():
    ...
    parser.add_argument('--shadow-replay-depth', action='store_true', default=None, help='[launcher default since 2026-09-25 on every modded launch with --motion-output --ownership; --no-shadow-replay-depth = off; not sent under --vanilla] One-cascade depth replay of the slice-0 caster candidates into a private sun-space map at every scene end (X3M_SHADOW_REPLAY_DEPTH=1; implies --shadow-replay-candidates and requires its prerequisites --motion-output --ownership only; DLL default off): one shadow_replay_depth line per frame, nothing samples the map, no shadows are applied (docs/architecture/shadow-replay-gates.md, "Implemented: cascade-0 depth replay fixture")')
    ...
    parser.add_argument('--shadow-cascades', default=None, metavar='E0,E1,...|default', help='[launcher default since 2026-09-25: 250,1500,7500,37500,150000 on every modded launch with --shadow-replay-depth; --no-shadow-cascades = off; not sent under --vanilla] Sun-shadow cascades (X3M_SHADOW_CASCADES; DLL default off: the single --shadow-replay-extent map; requires --shadow-replay-depth): 1..5 ascending half-extents in world units, each 50..150000, of camera-centred texel-snapped maps replayed in one transaction; "default" means 250,1500,7500,25000 (a 30 km reach: 250,1500,7500,37500,150000). Every cascade reaches 2 x the largest extent towards the light, the apply quad selects the first cascade containing a pixel with a 10 %% blend band and fades the last one to lit; the far cascade replays on even frames only while the frame exceeds --shadow-cascade-budget (docs/architecture/shadow-cascades.md)')
    parser.add_argument('--no-shadow-cascades', dest='shadow_cascades', action='store_const', const=PROMOTED_OFF, help='Turn the --shadow-cascades launcher default off')
    parser.add_argument('--shadow-cascade-sizes', default=None, metavar='N[,N...]', help='[launcher default since 2026-09-25: 2048,4096,4096,2048,2048 whenever five --shadow-cascades are on (user decision 2026-09-25: the 4096 maps go to cascades 1 and 2, 15 cm and 73 cm per texel; 0, 3 and 4 stay 2048; a map costs 4*N^2 bytes, 176 MiB in all, +96 MiB net against 2048 x5), 2048 for every cascade with another count; not sent under --vanilla] Map side per cascade, 64..4096, one value for all or one per cascade, DLL default 4096 (X3M_SHADOW_CASCADE_SIZES; requires --shadow-cascades); memory is 4 N^2 bytes per map plus one depth attachment of the largest size')
    ...
    # every modded --motion-output launch unless --no-sun-occlusion is given. Not with --submit-phases (both claim the lens
    # traversal call; the default is then not sent, never an error) and never under --vanilla. X3M_SUN_OCCLUSION_DEFAULT
    # tells the DLL's sun_occlusion_config row where the value came from.
    args.sun_occlusion_default = False
    if not args.sun_occlusion and not args.no_sun_occlusion and args.motion_output and not args.submit_phases and not args.vanilla:
    ...
    if (args.sun_occlusion_radius is not None or args.sun_occlusion_curve is not None or args.sun_occlusion_core_f is not None) and not args.sun_occlusion:
        parser.error('--sun-occlusion-radius/--sun-occlusion-curve/--sun-occlusion-core-f require --sun-occlusion (given, or the default on a modded --motion-output launch without --submit-phases or --no-sun-occlusion).')
