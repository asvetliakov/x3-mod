...
def main():
    ...
    parser.add_argument('--chase-view-restore-dock', action='store_true', help='Also keep the rear chase view across docking at a station: the same one-use ticket accepts the measured docking destructor chain (StopAllMonitors in RunPlayerTrade, warp 0) and substitutes 258 for the SelectMode(1) the dock performs at the same seam, so the new cockpit is built in the rear view; while docked the screen shows the engine\'s rear view of the parked ship instead of the cockpit (unflown). A save made while docked stores 258 (script cell and native mode), so loading it shows the rear view even with this option off or under --vanilla. No site of its own; logs path=dock (X3M_CHASE_VIEW_RESTORE_DOCK=1, always explicit; default off; requires --chase-view-restore; refused under --vanilla; docs/reverse-engineering/chase-view-docking.md)')
    ...
    if args.chase_view_restore_dock and not args.chase_view_restore:
        parser.error('--chase-view-restore-dock requires --chase-view-restore.')
    if args.vanilla and args.chase_view_restore_dock:
        parser.error('--chase-view-restore-dock cannot be combined with --vanilla: a vanilla launch loads the builtin d3d9, so the proxy restore ticket never runs')
    ...
    if args.action in ('install', 'uninstall', 'rollback', 'recover'):
        ...
        # Docking path of the same ticket: always explicit so a stale shell value cannot enable it (default off).
        env['X3M_CHASE_VIEW_RESTORE_DOCK'] = '1' if args.chase_view_restore_dock else '0'
