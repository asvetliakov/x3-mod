...
def main():
    ...
    parser.add_argument('--taa-sentinel-stabiliser', default=None, metavar='S[,E]', help='Sentinel stabiliser of the TAA resolve against the flicker of distant stations the engine draws blended without depth, under a camera pan (X3M_TAA_SENTINEL_STABILISER; requires --taa-thin-region with W > 0 and the camera gate; default off (0) on every modded --taa launch since 2026-09-25, marked X3M_TAA_SENTINEL_STABILISER_DEFAULT=1 (accepted 2026-09-25 after Run 82 A launch 2, run313/run314: the fade RT2 owner now covers the alpha-tested station cutouts, docs/architecture/fade-rt2-ownership.md section 5 conditions 1, 2 and 4 met); an explicit value sends marker 0, and an explicit 0.7 restores the previous look (the Run 61/62 default, run216/run221) for an A/B; "off" is the same as 0; nothing is sent without --taa or under --vanilla, and the DLL fallback when the variable is unset stays 0.7 under the camera gate; S > 0 needs the camera gate: unrouted pixels on the empty-depth sentinel (sky, far blended stations) get thin-region strength S (0..1) through the camera gate, always with the history clipped to the 7x7 min/max box of the current colour, closed where the region hold\'s camera closure is (a routed pixel moving against the camera path, or its nearest-depth 3x3 neighbour, held one jitter cycle). E (default 1, 0 = none) is the emitter bound in scene luma: next to anything brighter (lasers, trails, suns) the box shrinks to the inner 3x3, so their ghost stays within one pixel. Slight ghosting of unrouted content over sky (at most 3 px, decaying) is the accepted trade (docs/architecture/temporal-integration.md)')
    ...
    parser.add_argument('--taa-box-resolution', choices=('full', 'half'), default=None, help='Resolution of the box of the TAA thin region\'s camera gate (X3M_TAA_BOX_RESOLUTION; requires --taa; refused under --vanilla; default half on every modded --taa launch since Run 82 (accepted in Run 81 A launch 2, docs/verification/temporal-resolve.md), marked X3M_TAA_BOX_RESOLUTION_DEFAULT=1; full is the explicit opt-out (marker 0); nothing is sent without --taa or under --vanilla; the DLL default when the variable is unset stays full): half draws the separable min/max box (the sentinel stabiliser\'s rows and columns) at half resolution in both axes, each 2x2 block taking the 8x8 window that contains its four pixels\' 7x7 windows, so the clip it feeds is never tighter than full; the emitter bound (lasers, trails, suns over sky) takes the block\'s inner 4x4 where all four pixels would take their 3x3 (docs/architecture/taa-high-resolution.md S4). Meaningful only with the camera gate (the default with --taa-thin-region); an odd frame size, a refused program or refused targets keep full (one motion_output_taa_box_resolution row)')
    ...
    parser.add_argument('--taa-thin-region-source', choices=('both', 'screen', 'vote'), default=None, help='What feeds the TAA thin-region flag (X3M_TAA_THIN_REGION_SOURCE; requires --taa and --taa-thin-region with W > 0 (the default 0.97 counts); vote also requires --taa-thin-vote on (the default counts); refused under --vanilla; forwarded only when given; DLL default both): both = the screen-space fragmented-depth search (a 7-tap line crossing geometry and its background twice) united with the draw-time thin vote, today\'s mask; screen = the search alone (the vote keeps running and logging, its RT2 .a is not read); vote = the vote alone (the search is skipped in the tests draw; no mask draw is removed). For the Run 82 A/B (docs/architecture/taa-thin-geometry-alternatives.md section 3.2); the DLL logs taa_thin_region_source requested= configured= reason= and configures both when a prerequisite is missing With vote the emissive vote (--taa-thin-region-emissive, default 1 on the HDR route) still runs; pass --taa-thin-region-emissive 0 for a vote-only arm.')
    ...
    # Evaluated before the derived defaults below (thin-region gate and emissive vote, sentinel stabiliser, the options
    ...
    # Default off (accepted 2026-09-25 after Run 82 A launch 2, run313/run314; docs/architecture/fade-rt2-ownership.md
    # section 5): off on every modded --taa launch unless given, now that the fade RT2 owner covers the alpha-tested station
    # cutouts. The DLL's own fallback when the variable is unset stays the Run61/Run62 0.7 under the camera gate, so the
    # launcher sends 0 explicitly with X3M_TAA_SENTINEL_STABILISER_DEFAULT=1 (the configured row's sentinel_stabiliser_default=);
    # an explicit value (0.7 restores the previous look for an A/B) sends marker 0. Without --taa, or under --vanilla, nothing.
    args.taa_sentinel_stabiliser_default = False
    if args.taa_sentinel_stabiliser is None and args.taa and not args.vanilla:
        args.taa_sentinel_stabiliser = '0'
        args.taa_sentinel_stabiliser_default = True
    ...
    if args.action in ('install', 'uninstall', 'rollback', 'recover'):
        ...
        for name, value in (('X3M_TAA_HISTORY_WEIGHT', args.taa_history_weight), ('X3M_TAA_FAR_STABILISER', args.taa_far_stabiliser), ('X3M_TAA_THIN_REGION', args.taa_thin_region), ('X3M_TAA_THIN_REGION_GATE', args.taa_thin_region_gate), ('X3M_TAA_THIN_REGION_EMISSIVE', args.taa_thin_region_emissive), ('X3M_TAA_SENTINEL_STABILISER', args.taa_sentinel_stabiliser),
                            ...
                            ('X3M_TAA_HISTORY_TAPS', args.taa_history_taps), ('X3M_TAA_BOX_RESOLUTION', args.taa_box_resolution), ('X3M_TAA_THIN_VOTE', args.taa_thin_vote), ('X3M_TAA_THIN_REGION_SOURCE', args.taa_thin_region_source), ('X3M_FADE_RT2_OWNER', args.fade_rt2_owner),
                            ('X3M_TAA_BOX_RESOLUTION_DEFAULT', None if args.taa_box_resolution is None else '1' if args.taa_box_resolution_default else '0'),
                            ('X3M_TAA_SENTINEL_STABILISER_DEFAULT', None if args.taa_sentinel_stabiliser is None else '1' if args.taa_sentinel_stabiliser_default else '0'),
            ...
            if value is not None:
                ...
                env.pop(name, None)
        # Retired 2026-09-23 (cleanup batch 6) and X3M_TAA_REGION_HOLD (2026-09-24, A' only): never inherited, never set (the
        # DLL no longer reads them; it logs one ignored line for a stale X3M_TAA_REGION_HOLD).
        for name in ('X3M_TAA_CURRENT_FILTER', 'X3M_TAA_LINE_FILTER', 'X3M_TAA_THIN_CLIP', 'X3M_TAA_ADAPTIVE_WEIGHT', 'X3M_TAA_REGION_HOLD'):
            env.pop(name, None)
