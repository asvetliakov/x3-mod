...
FOG_MARCH_SCALE_DEFAULT = '4'  # Run 77 C/C2 (2026-09-24): quarter-resolution fog march; 2 is the opt-out, and the shadow pass clamps 4 to 2
...
# Variables of removed options (docs/verification/launcher-options-inventory.md, section 4 "Removed", and the
...
REMOVED_VARIABLES = (
    ...
    # Removed 2026-09-26 with their DLL reads: the in-game adjacency dump and the forced fog profile.
    'X3M_MESH_ADJACENCY_DUMP', 'X3M_VOLUMETRIC_FOG_EVERYWHERE',
    # Removed 2026-09-26, values baked into the DLL (inventory section 5): the chase dt clamp, the fog look's 24 scalars and
    # two ambient hues, and the mote tunables except MAX_PX.
    'X3M_CHASE_MAX_DT',
    *(f'X3M_FOG_LOOK_{name}' for name in (
        'COVERAGE', 'EXPONENT', 'SIGMA_SCALE', 'FORWARD_G', 'FORWARD_WEIGHT', 'BACK_G', 'ALBEDO_WHITE', 'AMBIENT_GAIN', 'EXTINCTION_TINT',
        'SCATTER_LIFT', 'LIFT_FLOOR', 'SHADOW_FLOOR', 'SKY_CAP', 'TAPER_START', 'SELF_SHADOW', 'POWDER', 'TAP_DISTANCE', 'TAP_LENGTH',
        'COVERAGE_VARIATION', 'WARP_CYCLES_NEAR', 'WARP_NEAR', 'WARP_CYCLES_FAR', 'WARP_FAR', 'SHADOW_JITTER', 'AMBIENT_SUN', 'AMBIENT_AWAY')),
    'X3M_FOG_MOTES_RADIUS', 'X3M_FOG_MOTES_NEAR', 'X3M_FOG_MOTES_GAIN', 'X3M_FOG_MOTES_SOFT', 'X3M_FOG_MOTES_DRIFT', 'X3M_FOG_MOTES_SEED',
    # Removed 2026-09-26 with --capture-delay (F8 under --debug starts its burst at once; the DLL no longer reads it).
    'X3M_CAPTURE_DELAY',
)
...
TIERED_VARIABLES = (
    ...
    # --draw-trace since 2026-09-26 (the group variable, then its members, whose options were removed): the per-draw route
    # cost and the engine-stamp families
    'X3M_DRAW_TRACE', 'X3M_TELEMETRY_DRAW', 'X3M_GAME_PHASES', 'X3M_GAME_PHASE_THRESHOLD_MS', 'X3M_PASS_PHASES', 'X3M_RESIDUAL_PHASES', 'X3M_LIGHT_PHASES',
    'X3M_LOOP_PHASES',
    # fixture-only since 2026-09-26 (options removed; the runners and host fixtures set them): the explicit frame_end cadence,
    # the submit stamps (they claim the lens call the sun-occlusion default patches), the state-call stamps, the memo verify
    'X3M_FRAME_END_STRIDE', 'X3M_SUBMIT_PHASES', 'X3M_FRAME_TIMING_STATE_STAMPS', 'X3M_COLLIDE_MEMO_VERIFY',
    # explicit developer options (sent only when given)
    'X3M_GPU_SYNC_TIMING', 'X3M_TAA_DEBUG', 'X3M_PROFILE', 'X3M_PROFILE_INTERVAL_US',
)
...
# Stand-command promotion (user decision 2026-09-25, docs/verification/launcher-options-inventory.md,
# section 1 "Defaults"): every functional option of the Run 84 A stand command is a launcher default on a
# modded launch, plus --music-keep (accepted run273) and --shadow-alpha-casters on (flown clean run309);
# the telemetry/debug options of that command stay opt-in. Each default applies only when the option is not
# given and its own prerequisites hold after the earlier defaults, so turning a prerequisite off (--no-taa,
# --no-shadow-replay-depth, --camera vanilla, ...) switches its dependants off instead of refusing; an
# explicit dependant is still refused as before. --vanilla, and every other action, fill in the old off values.
# PROMOTED_OFF is what the --no-<name> opt-out of a valued option stores: it becomes None (not sent).
...
def apply_promoted_defaults(args):
    ...
    opted_out = set()
    for dest in ('bloom_source_clamp', 'screen_emission_additive', 'screen_emission_additive_alpha', 'shadow_cascades',
                 'shadow_cascade_adaptive_c0', 'volumetric_fog'):
        if getattr(args, dest) == PROMOTED_OFF:
            setattr(args, dest, None)
            opted_out.add(dest)
    modded = args.action == 'launch' and not args.vanilla
    filled = set()

    def fill(dest, value, when=True):
        if getattr(args, dest) is None and dest not in opted_out and modded and when:
            setattr(args, dest, value)
            filled.add(dest)

    # Capture: F8 (under --debug only) captures a burst of 8 frames at once.
    fill('capture_frames', 8)
    fill('direct', True)
    fill('camera', 'chase')
    fill('chase_view_restore', True, args.camera == 'chase')
    fill('ownership', True)
    fill('object_trace', True, bool(args.ownership) and args.object_lifetime is not False)  # the trio moves together
    fill('object_lifetime', True, bool(args.object_trace and args.ownership))
    fill('motion_output', True)
    fill('taa', True, bool(args.motion_output and args.object_trace and args.object_lifetime))
    fill('hdr', True, bool(args.motion_output))
    fill('hdr_tonemap', True, bool(args.hdr))
    fill('hdr_bloom', True, bool(args.hdr_tonemap and args.scene_hook != 'off'))
    fill('bloom_source_clamp', 1.0, bool(args.hdr_bloom))
    fill('crypt_cache', True)
    fill('gz_buffer', True)
    ...
    fill('shadow_alpha_casters', 'on', bool(args.shadow_replay_depth))
    fill('shadow_cascades', '250,1500,7500,37500,150000', bool(args.shadow_replay_depth))
    cascades = args.shadow_cascades is not None
    five = cascades and args.shadow_cascades != 'default' and len(args.shadow_cascades.split(',')) == 5
    fill('shadow_cascade_drop_order', 'importance', cascades)
    fill('shadow_cascade_records', '1024,1024,2048,4096,4096', five)
    # User decision 2026-09-25: 4096 maps for cascades 1 and 2 (1,500 and 7,500 units), 2048 for 0, 3 and 4.
    fill('shadow_cascade_sizes', '2048,4096,4096,4096,2048' if five else '2048', cascades)
    fill('shadow_cascade_adaptive_c0', 1.5, cascades)
    fill('shadow_caster_retention', True, cascades)
    fill('volumetric_fog', 0.02, bool(args.motion_output and args.taa and args.hdr and args.shadow_replay_depth and cascades))
    fill('volumetric_fog_cards', 'replace', args.volumetric_fog is not None)
    fill('volumetric_fog_range', 'stored', args.volumetric_fog is not None and args.volumetric_fog > 0.0)
    fill('music_keep', True)
    for dest in ('direct', 'chase_view_restore', 'ownership', 'object_trace', 'object_lifetime', 'motion_output', 'taa', 'hdr',
                 'hdr_tonemap', 'hdr_bloom', 'crypt_cache', 'gz_buffer', 'dat_handles', 'sun_shadow_lane', 'shadow_replay_depth',
                 'sun_shadow_apply', 'shadow_caster_retention', 'music_keep'):
        if getattr(args, dest) is None:
            setattr(args, dest, False)
    for dest, off in (('camera', 'vanilla'), ('resource_read', 'native'), ('mesh_adjacency', 'native'),
                      ('capture_frames', 1)):
        if getattr(args, dest) is None:
            setattr(args, dest, off)
    return filled
...
# --fov (docs/reverse-engineering/field-of-view.md sections 7.3 and 6.1): the game's own FOV number N, read as
# N degrees horizontal on 16:9 (the X4 convention), bounded like the in-game FOV menu (SG_MIN_FOV/SG_MAX_FOV
# 70..100). The DLL puts F'(N) with tan(F'/2) = 0.75*tan(F/2), F = (N << 16) / 360, into the registry
# constructor and remaps every INS_SetFocus value the same way, so the menu keeps working in the same model.
# The default 90 gives F' = 0x3470 (vertical 58.72 deg), the value the earlier vertical default produced.
FOV_MIN_DEG, FOV_MAX_DEG = 70.0, 100.0
FOV_DEFAULT_SETTING = '90'


def fov_setting(text):
    """X3M_FOV for a --fov argument: 'game', or N with at most four decimals, trailing zeros dropped (the DLL's
    parser takes no exponent); None when refused."""
    if text == 'game':
        return 'game'
    try:
        value = float(text)
    ...
    if not (math.isfinite(value) and FOV_MIN_DEG <= value <= FOV_MAX_DEG):
        return None
    setting = f'{value:.4f}'.rstrip('0').rstrip('.')
...
ORIGINAL_FILL_DEFAULT = 0.01  # --original-fill on modded --hdr launches (user decision 2026-09-25: 0.02, then 0.01 the same day; original-shading-critique.md 1a)
...
def hull_emitters_requested(args):
    ...
    above 1: the guide lights belong to the effects group and
...
def main():
    ...
    parser.add_argument('--debug', action='store_true', help='Debug logging tier (X3M_DEBUG=1; inert under --vanilla, where no proxy loads): the rendering-state rows every frame (motion_output_frame, hdr_frame, camera_state, the shadow/sun state rows), the F8-frame censuses and bounds (cull census, LOD switch rows, object bounds), the traces (window, music, media cue, sun, sector background, loading probes, collide narrow census and query phases), the retention census and the 300-frame health windows; telemetry counters on; the ten frame-phase stamps are its only engine patches (the heavy attribution is --draw-trace). About 8.0 KB of log per frame (about 1.7 GB per hour at 60 fps; 9.4 KB, 2.0 GB per hour with --perf; F8 capture frames add their burst; F8, the capture key, is read only under this tier). Independent of --perf; the stand is --debug --perf (docs/architecture/logging-tiers.md)')
    parser.add_argument('--perf', action='store_true', help='Performance logging tier (X3M_PERF=1; inert under --vanilla, where no proxy loads): frame_end every frame, the per-frame fog and shadow cost rows, the 300-frame frame-time and engine-phase windows with their slow-frame witnesses, the 1 Hz telemetry summaries and loading metrics, the family rows every 60 frames and the FPS overlay (shown for the whole session); the cheap tier, carried by the stand command (the per-draw route cost and the engine-stamp families are --draw-trace). About 1.7 KB per frame (about 360 MB per hour at 60 fps; gzip about 20x). What a stutter report needs (docs/architecture/logging-tiers.md)')
    parser.add_argument('--draw-trace', action='store_true', help='Heavy per-draw and engine-stamp attribution (X3M_DRAW_TRACE=1, expanded in the DLL; default off; requires --perf or --debug, which give the telemetry counters and the frame boundary it attributes against; not sent under --vanilla): the per-draw route cost fields on motion_output_frame (X3M_TELEMETRY_DRAW: gate_us, route_draw_us, set_rt_us, lazy_flush_us, jitter_us; two QueryPerformanceCounter reads per routed draw, about 1 ms per frame by docs/verification/route-cost-run1.md) and the byte-verified engine-stamp families X3M_GAME_PHASES (segment tape at 20 ms), X3M_PASS_PHASES, X3M_RESIDUAL_PHASES, X3M_LIGHT_PHASES and X3M_LOOP_PHASES (trampolines installed at start-up, each failing closed per site). Not the view_submit stamps (fixture-only: they claim the lens call the sun-occlusion default patches). For one stutter-attribution flight, not for the stand (docs/architecture/logging-tiers.md)')
    ...
    parser.add_argument('--mesh-adjacency', choices=['native', 'fast'], default=None, help='[launcher default since 2026-09-25: fast on every modded launch; --mesh-adjacency native = off; fast arms without the logging groups; not sent under --vanilla; verify was removed from the launcher on 2026-09-26, the fixture sets X3M_MESH_ADJACENCY=verify] ID3DXMesh::GenerateAdjacency service (X3M_MESH_ADJACENCY): native forwards; fast answers from the exact-equality computation and falls through to D3DX on any qualification failure (docs/verification/mesh-adjacency-fast.md)')
    ...
    parser.add_argument('--resource-read', choices=['native', 'fast'], default=None, help='[launcher default since 2026-09-25: fast on every modded launch; --resource-read native = off; not sent under --vanilla; verify was removed from the launcher on 2026-09-26] Archive reader 0x004e8880 service (X3M_RESOURCE_READ; exact executable only): native leaves the game\'s reader alone; fast returns our decode (one fread, word XOR, one inflate, no memset) and falls back to the original on any deviation (docs/verification/resource-reader.md)')
    ...
    parser.add_argument('--hdr-tonemap', action='store_true', default=None, help='[launcher default since 2026-09-25 on every modded launch with --hdr; --no-hdr-tonemap = off; not sent under --vanilla] AgX write-back of the FP16 scene (X3M_HDR_TONEMAP=agx; requires --hdr), Auto capped at +1.3 EV by default; --hdr-exposure fixed disables frame metering. Default off: identity write-back.')
    ...
    parser.add_argument('--no-shadow-replay-depth', dest='shadow_replay_depth', action='store_false', help='Turn the --shadow-replay-depth launcher default off')
    parser.add_argument('--shadow-alpha-casters', choices=('on', 'off'), default=None, help='[launcher default since 2026-09-25: on on every modded launch with --shadow-replay-depth (flown clean run309); --shadow-alpha-casters off = off; not sent under --vanilla] Alpha-tested routed draws cast sun shadows and keep their alpha test in the caster pass (X3M_SHADOW_ALPHA_CASTERS; DLL default off; on requires --shadow-replay-depth): the draw\'s stage-0 texture (2D, managed pool) is sampled at its TEXCOORD0 and the caster pixel is discarded below its own ALPHAREF (GREATEREQUAL / GREATER; ALWAYS casts opaque; the other comparisons and a draw without TEXCOORD0 stay excluded); one shadow_alpha_casters line per frame. Alpha from the diffuse texture only (docs/architecture/shadow-replay-gates.md, "Alpha-tested casters")')
    parser.add_argument('--shadow-cascades', default=None, metavar='E0,E1,...|default', help='[launcher default since 2026-09-25: 250,1500,7500,37500,150000 on every modded launch with --shadow-replay-depth; --no-shadow-cascades = off; not sent under --vanilla] Sun-shadow cascades (X3M_SHADOW_CASCADES; DLL default off: no map, no replayed sun shadows; requires --shadow-replay-depth): 1..5 ascending half-extents in world units, each 50..150000, of camera-centred texel-snapped maps replayed in one transaction; "default" means 250,1500,7500,25000 (a 30 km reach: 250,1500,7500,37500,150000). Every cascade reaches 2 x the largest extent towards the light, the apply quad selects the first cascade containing a pixel with a 10 %% blend band and fades the last one to lit; the far cascade replays on even frames only while the frame exceeds --shadow-cascade-budget (docs/architecture/shadow-cascades.md)')
    parser.add_argument('--no-shadow-cascades', dest='shadow_cascades', action='store_const', const=PROMOTED_OFF, help='Turn the --shadow-cascades launcher default off: X3M_SHADOW_CASCADES=0 disables the replayed sun shadows (no map; --sun-shadow-apply then logs one sun_shadow_apply_mode row with cascades=0 and applies nothing)')
    parser.add_argument('--shadow-cascade-sizes', default=None, metavar='N[,N...]', help='[launcher default since 2026-09-25: 2048,4096,4096,4096,2048 whenever five --shadow-cascades are on (user decisions 2026-09-25: the 4096 maps go to cascades 1, 2 and 3, 15 cm, 73 cm and 3.7 m per texel; 0 and 4 stay 2048; a map costs 4*N^2 bytes, 224 MiB in all, +144 MiB net against 2048 x5), 2048 for every cascade with another count; not sent under --vanilla] Map side per cascade, 64..4096, one value for all or one per cascade, DLL default 4096 (X3M_SHADOW_CASCADE_SIZES; requires --shadow-cascades); memory is 4 N^2 bytes per map plus one depth attachment of the largest size')
    ...
    parser.add_argument('--shadow-cascade-records', default=None, metavar='N[,N...]', help='[launcher default since 2026-09-25: 1024,1024,2048,4096,4096 whenever five --shadow-cascades are on; not sent under --vanilla] Record capacity per cascade, 1..4096, one value for all or one per cascade, DLL default 1024 (X3M_SHADOW_CASCADE_RECORDS; requires --shadow-cascades): the caster list is sized to the largest at device creation (state memory, about 400 bytes per record), so a far cascade can carry more than the 1,024 the default list holds (docs/architecture/shadow-cascade-extents.md, "Caster pool control")')
    ...
    parser.add_argument('--hdr-exposure', choices=['fixed', 'auto'], default=None, help='Exposure policy (X3M_HDR_EXPOSURE; requires --hdr-tonemap when explicit): Auto capped at +1.3 EV by default, or fixed EV0')
    ...
    parser.add_argument('--no-collide-memo', dest='collide_memo', action='store_false', default=None, help='Turn the no-contact collision memo off (it is on by default on a modded launch)')
    ...
    parser.add_argument('--sun-occlusion-radius', type=float, default=None, metavar='U', help='Half-width of the sun disc the visibility pass samples, as a fraction of the back-buffer width (X3M_SUN_OCCLUSION_RADIUS; 0.005 <= U <= 0.25, default 0.04 = 51 px at 1280; the engine\'s record size saturates for the sun, so the radius is not derived; calibrate from the lens_*.bgra8 readback of a clear-sky burst: radius_px in the sun_visibility line should match the glow disc; requires --sun-occlusion, given or the default)')
    parser.add_argument('--sun-occlusion-curve', type=float, default=None, metavar='EXP', help='Exponent on the used visibility fraction (X3M_SUN_OCCLUSION_CURVE; 0.25 <= EXP <= 4, default 1; requires --sun-occlusion, given or the default)')
    parser.add_argument('--sun-occlusion-core-f', choices=('on', 'off'), default=None, help='Whether the per-pixel clipped core bodies of the sun are also multiplied by the visibility fraction f (X3M_SUN_OCCLUSION_CORE_F, default on whenever the sun occlusion is on, given or the default, accepted in flight in run235): on = the clipped disc dims with the covered fraction like the ghosts and streaks; off = clip only, the Run64 behaviour. Requires --sun-occlusion, given or the default (without it nothing is patched)')
    ...
    # per-row options (all plain unknown arguments; the parser takes no abbreviations).
    if args.draw_trace and not (args.debug or args.perf):  # the DLL turns X3M_TELEMETRY on for either group
        parser.error('--draw-trace requires --perf or --debug: they give the telemetry counters and the frame boundary its per-draw '
                     'and engine-stamp attribution is measured against.')
    ...
    # Evaluated before the derived defaults below (thin-region gate and emissive vote, the thin-region source, the options
    # that need an age program), which therefore see the defaulted values.
    for attribute in ('taa_far_stabiliser', 'taa_thin_region'):
        if getattr(args, attribute) is not None and getattr(args, attribute).strip().lower() == 'off':
            setattr(args, attribute, '0')
    if args.taa and not args.vanilla:
        base_weight = args.taa_history_weight if args.taa_history_weight is not None else 0.9
        for attribute, value in (('taa_far_stabiliser', TAA_FAR_STABILISER_DEFAULT), ('taa_thin_region', TAA_THIN_REGION_DEFAULT)):
            if getattr(args, attribute) is None and base_weight <= float(value):
    ...
    if args.taa_far_stabiliser is not None:
        ...
        far += [0.0, 60.0, 68.0, 0.03, 0.25][len(far) - 1:]  # F0,F1 60,68 since 2026-09-25 (run327/run332 plant sparkles; 80,130 before)
        base_weight = args.taa_history_weight if args.taa_history_weight is not None else 0.9
    ...
    if args.taa_box_resolution is not None and not args.taa:
        parser.error('--taa-box-resolution requires --taa.')
    # Run 82 default (S4 half box accepted after Run 81 A launch 2, docs/verification/temporal-resolve.md): half on every
    # modded --taa launch unless given; full is the explicit opt-out. X3M_TAA_BOX_RESOLUTION_DEFAULT tells the DLL's
    # creation row where the value came from. Without --taa, or under --vanilla, nothing is sent.
    args.taa_box_resolution_default = False
    if args.taa and not args.vanilla and args.taa_box_resolution is None:
        args.taa_box_resolution = 'half'
        args.taa_box_resolution_default = True
    if args.taa_far_gate is not None and args.vanilla:
        parser.error('--taa-far-gate cannot be combined with --vanilla: a vanilla launch loads the builtin d3d9.')
    if args.taa_far_gate is not None and not args.taa:
        parser.error('--taa-far-gate requires --taa.')
    # Far weight on the camera gate (2026-09-25, run327 sparkles; taa-mask-fold.md section 4.2 addendum): camera on every modded
    # --taa launch unless given; screen is the previous gate. X3M_TAA_FAR_GATE_DEFAULT tells the DLL's creation row where the
    # value came from. Without --taa, or under --vanilla, nothing is sent.
    args.taa_far_gate_default = False
    if args.taa and not args.vanilla and args.taa_far_gate is None:
        args.taa_far_gate = 'camera'
        args.taa_far_gate_default = True
    if args.taa_far_clip is not None and args.vanilla:
        parser.error('--taa-far-clip cannot be combined with --vanilla: a vanilla launch loads the builtin d3d9.')
    if args.taa_far_clip is not None and not args.taa:
        parser.error('--taa-far-clip requires --taa.')
    # Far clip (2026-09-25, run327/run332 rest sparkles; taa-mask-fold.md section 4.2 addendum "far clip"): 7x7 on every modded
    # --taa launch unless given; 3x3 is the previous clip. X3M_TAA_FAR_CLIP_DEFAULT tells the DLL's creation row where the value
    # came from. Without --taa, or under --vanilla, nothing is sent.
    args.taa_far_clip_default = False
    ...
    if args.fade_rt2_owner is not None and not args.taa:
        parser.error('--fade-rt2-owner requires --taa.')
    if args.fade_rt2_owner == 'on' and not (args.motion_output and args.hdr):
        parser.error('--fade-rt2-owner on requires --motion-output --hdr.')
    if args.taa_thin_vote is not None and args.vanilla:
        parser.error('--taa-thin-vote cannot be combined with --vanilla: a vanilla launch loads the builtin d3d9.')
    if args.taa_thin_vote is not None and not args.taa:
        parser.error('--taa-thin-vote requires --taa.')
    if args.taa_thin_vote == 'on' and not (args.motion_output and args.ownership and args.sun_shadow_lane):
        parser.error('--taa-thin-vote on requires --motion-output --ownership --sun-shadow-lane.')
    # Run 81 defaults (Run 80 A launches 3-4, docs/verification/temporal-resolve.md): both on whenever a modded launch has
    # their prerequisites. A default never refuses a launch: without them it is not sent, with one launcher line per option
    # naming the missing prerequisites; an explicit value keeps the refusals above. The *_DEFAULT companions tell the DLL's
    # configured rows where the value came from.
    args.taa_thin_vote_default = args.fade_rt2_owner_default = False
    args.defaults_not_sent = []  # printed with the launch lines below
    if args.action == 'launch' and not args.vanilla:
        for option, attribute, needs in (('--taa-thin-vote', 'taa_thin_vote', ('taa', 'motion_output', 'ownership', 'hdr', 'sun_shadow_lane')),
                                         ('--fade-rt2-owner', 'fade_rt2_owner', ('taa', 'motion_output', 'hdr'))):
            if getattr(args, attribute) is not None:
                continue
            missing = ['--' + need.replace('_', '-') for need in needs if not getattr(args, need)]
            if missing:
                args.defaults_not_sent.append(f'default on not sent: {option} (missing {" ".join(missing)})')
            else:
    ...
    if args.volumetric_fog is None and (args.volumetric_fog_cards is not None or args.volumetric_fog_range is not None):
        parser.error('--volumetric-fog-cards and --volumetric-fog-range require --volumetric-fog.')
    ...
    if args.fov is not None:
        ...
        if args.fov_setting is None:
            parser.error(f'--fov out of range: {args.fov!r} (expected game or the game\'s FOV number N in [{FOV_MIN_DEG:g}, {FOV_MAX_DEG:g}], '
                         'the horizontal degrees on 16:9 and the in-game FOV menu\'s own range)')
    ...
    if args.sun_occlusion and args.no_sun_occlusion:
        parser.error('--sun-occlusion and --no-sun-occlusion are mutually exclusive.')
    # Run 83 default (user decision 2026-09-25 after Run 83; Runs 64/65 accepted, docs/verification/sun-occlusion.md): on for
    # every modded --motion-output launch unless --no-sun-occlusion is given, never under --vanilla. (The --submit-phases
    # conflict went with that option on 2026-09-26; X3M_SUBMIT_PHASES is a fixture-only read and the DLL still refuses the pair.) X3M_SUN_OCCLUSION_DEFAULT
    # tells the DLL's sun_occlusion_config row where the value came from.
    args.sun_occlusion_default = False
    if not args.sun_occlusion and not args.no_sun_occlusion and args.motion_output and not args.vanilla:
        args.sun_occlusion = True
        args.sun_occlusion_default = True
    if (args.sun_occlusion_radius is not None or args.sun_occlusion_curve is not None or args.sun_occlusion_core_f is not None) and not args.sun_occlusion:
        parser.error('--sun-occlusion-radius/--sun-occlusion-curve/--sun-occlusion-core-f require --sun-occlusion (given, or the default on a modded --motion-output launch without --no-sun-occlusion).')
    ...
    if args.action in ('install', 'uninstall', 'rollback', 'recover'):
        ...
        if args.perf:
            env['X3M_PERF'] = '1'
        if args.draw_trace and not args.vanilla:  # the heavy attribution (never under --vanilla, where no proxy loads)
            env['X3M_DRAW_TRACE'] = '1'
        env['X3M_CAPTURE_FRAMES'] = str(args.capture_frames)  # the F8 burst (F8 is read only under --debug)
        # No automatic capture: --capture-start went on 2026-09-26, but every modded launch keeps sending 999999 (never),
        # so a DLL whose own default is 120 (Run90 and earlier) captures only on F8; the fixtures set their own start.
        if args.vanilla:
            env.pop('X3M_CAPTURE_START', None)
        else:
            env['X3M_CAPTURE_START'] = CAPTURE_START_NEVER
        ...
        # The single-map variables are gone (2026-09-25): an inherited shell value is dropped, never forwarded.
        for removed in ('X3M_SHADOW_REPLAY_SIZE', 'X3M_SHADOW_REPLAY_EXTENT', 'X3M_SHADOW_REPLAY_DEPTH_HALF', 'X3M_SHADOW_REPLAY_CAP'):
        ...
        # 0 is the opt-out. With the motes off no inherited X3M_FOG_MOTES_MAX_PX survives (the other mote tunables are baked since 2026-09-26).
        ...
        # The guide lights follow the effects gain:
        ...
        if args.vanilla:
            env.pop('X3M_TERRAN_STATION_LOD', None)
        else:
            env['X3M_TERRAN_STATION_LOD'] = args.terran_station_lod or 'size'
        # LOD occlusion: always explicit on a modded launch (all by default since Run 81; off/record0 = the
        # engine's bytes), so a stale shell value cannot decide the gate; X3M_LOD_OCCLUSION_DEFAULT tells the DLL's
        # row whether the value is the default. Both dropped under --vanilla (an explicit value is refused above).
        if args.vanilla:
            env.pop('X3M_LOD_OCCLUSION', None)
            env.pop('X3M_LOD_OCCLUSION_DEFAULT', None)
        else:
        ...
        if args.vanilla:
            env.pop('X3M_SUN_FLARE_FIX', None)
        else:
            env['X3M_SUN_FLARE_FIX'] = args.sun_flare_fix or 'on'
        # Field of view: always explicit on a modded launch (N = 90 unless --fov), so a stale
        ...
        # Collide memo: same rule (its verify switch X3M_COLLIDE_MEMO_VERIFY is fixture-only since 2026-09-26, dropped above).
        if collide_default(args.collide_memo, args):
        ...
        for name, value in (('X3M_SUN_OCCLUSION', '1' if args.sun_occlusion else None), ('X3M_SUN_OCCLUSION_LOG', '1' if args.sun_occlusion_log else None),
                            ...
                            ('X3M_SUN_OCCLUSION_DEFAULT', ('1' if args.sun_occlusion_default else '0') if args.sun_occlusion else None)):
            if value is None:
                env.pop(name, None)
            else:
                env[name] = value
