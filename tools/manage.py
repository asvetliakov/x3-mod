...
def main():
    ...
    parser.add_argument('--collide-memo-advance', action='store_true', help='Conservative advancement for the collision memo (X3M_COLLIDE_MEMO_ADVANCE=1; default off; needs the memo and the SSE2 separating-axis test, both launcher defaults): a query that reached no triangle leaves the smallest gap of its box tests, a lower bound of the distance between the two meshes; while an object has provably moved less than half of it relative to the other, the query is answered no-contact without running. With --collide-memo-verify every such answer runs the engine too (advance_mismatches must stay 0) (docs/reverse-engineering/sector-collide.md, section 14.7)')
    ...
    if args.collide_memo_advance and not (collide_default(args.collide_memo, args) or args.collide_memo_verify):
        parser.error('--collide-memo-advance requires the collision memo (--collide-memo)')
    if args.collide_memo_advance and not collide_default(args.collide_sat_sse2, args):
        parser.error('--collide-memo-advance requires --collide-sat-sse2: the SSE2 separating-axis test measures the gaps')
    ...
    if args.action == 'install':
        ...
        if args.collide_memo_advance:
            env['X3M_COLLIDE_MEMO_ADVANCE'] = '1'
        else:
            env.pop('X3M_COLLIDE_MEMO_ADVANCE', None)
