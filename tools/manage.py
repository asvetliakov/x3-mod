...
def main():
    ...
    parser.add_argument('--sun-occlusion', action='store_true', help='[default off until flown] Partial sun occlusion, step 1 (X3M_SUN_OCCLUSION=1; absent = nothing patched, no cost; requires --motion-output for the route\'s RT2; refused with --submit-phases, whose sort_return_b stamp claims 0x00472490): the sun\'s lens chain fades with the visible fraction of the disc instead of popping off when the engine\'s binary CPU probe fails at about half cover. The flare probe\'s call 0x00471630 is answered "visible" for the main view\'s single sun while a GPU fraction exists (the probe\'s three early tests are replicated in vanilla order; every other view, a second sun, any unhealthy frame = the original probe), a 32-tap 1x1 FP16 visibility pass runs once per frame against the scene depth, and the lens-scene draws inside the bracket at 0x00472491 are multiplied by the smoothed fraction in a wrapped pixel shader. A lens draw that cannot carry the fraction returns the decision to the engine from the next frame on. Exact executable and bytes only, otherwise fails closed to vanilla (docs/architecture/sun-partial-occlusion.md)')
    parser.add_argument('--sun-occlusion-log', action='store_true', help='Diagnostic for the sun-occlusion flight (X3M_SUN_OCCLUSION_LOG=1; works with or without --sun-occlusion, installing the same two redirects observe-only when the feature is off): one sun_probe line per probe call (record fields +0x10/+0x20/+0x24/+0x30/+0x34/+0x38, the vanilla result, the answer given; the original probe always runs), one sun_visibility line per frame (footprint, radius, smoothing weight and a synchronous 1x1 readback of the fraction) and one sun_lens_draw fingerprint per draw inside the lens bracket (shader hashes, stage textures, blend/z/alpha-test state, RT0, primitive counts). For one flight: costs the vanilla probe plus a 1x1 readback stall per frame')
    parser.add_argument('--sun-occlusion-radius', type=float, default=None, metavar='SCALE', help='Scale on the disc radius derived from the lens record (X3M_SUN_OCCLUSION_RADIUS; 0.1 <= SCALE <= 8, default 1; requires --sun-occlusion)')
    ...
    if (args.sun_occlusion_radius is not None or args.sun_occlusion_curve is not None) and not args.sun_occlusion:
        parser.error('--sun-occlusion-radius/--sun-occlusion-curve require --sun-occlusion.')
    if args.sun_occlusion_radius is not None and not (math.isfinite(args.sun_occlusion_radius) and 0.1 <= args.sun_occlusion_radius <= 8.0):
        parser.error(f'--sun-occlusion-radius out of range: {args.sun_occlusion_radius} (expected [0.1, 8])')
