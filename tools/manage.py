...
def main():
    ...
    parser.add_argument('--emission-source-gain', type=float, default=None, metavar='G', help='Source-only encoded gain of the engine-glow family of the twenty additive engine/effects emission pairs (the engine_0000/0001 archive pairs and the base DEFAULT pair), finite 1..8, default 1 = off (X3M_EMISSION_SOURCE_GAIN; requires --hdr; excludes --linear-emissions, whose bracket carries its own --emission-gain; needs neither --linear-materials nor --taa): the pixel program of each pair multiplies its native colour output by G before the game\'s own ADD/ONE/ONE blend into the FP16 scene, so bloom and exposure pick the brighter emitters up; alpha, blend state and draw order stay native, and a pair drawn through any other blend stays native. Gain 1 creates no variant and is byte-identical to a build without the option (docs/architecture/linear-emission-cost.md, "Implemented"). Ctrl+Shift+F6 switches the engine family between G and native during play (no shader is recreated; one emission_source_gain_toggle line per press). The effect family (impact, muzzle and explosion sprites) has its own --effect-source-gain')
    parser.add_argument('--effect-source-gain', type=float, default=None, metavar='G', help='The same source-only encoded gain for the effect family of the twenty pairs (the effects_0000/0001 archive pairs and the base INSTANCE pairs: weapon impact flashes, muzzle glows, explosion sprites), finite 1..8, default 1 = off = original bytes (X3M_EFFECT_SOURCE_GAIN; same gate as --emission-source-gain: requires --hdr, excludes --linear-emissions). Independent of --emission-source-gain, so the engine glow can be gained while impacts stay native and vice versa (docs/architecture/linear-emission-cost.md, "Family split"). Ctrl+Shift+F4 switches the effect family between G and native during play (F7 is the telemetry phase marker and F8 the capture key; no shader is recreated)')
    ...
    if args.effect_source_gain is not None and not args.hdr:
        parser.error('--effect-source-gain requires --hdr.')
    if args.effect_source_gain is not None and args.linear_emissions:
        parser.error('--effect-source-gain excludes --linear-emissions (use --emission-gain inside the linear route).')
    if args.effect_source_gain is not None and not (math.isfinite(args.effect_source_gain) and 1.0 <= args.effect_source_gain <= 8.0):
        parser.error('--effect-source-gain must be finite and within [1, 8].')
    ...
    if args.action == 'install':
        ...
        env['X3M_EFFECT_SOURCE_GAIN'] = repr(args.effect_source_gain if args.effect_source_gain is not None else 1.0)
