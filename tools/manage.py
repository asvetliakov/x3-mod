...
def main():
    ...
    parser.add_argument('--light-map-far-fade', default=None, metavar='P0,P1[,G]', help='Fade the hull light-map gain with distance: a routed hull draw keeps --hull-lightmap-gain while its pixel footprint (world units per pixel at the object origin, the --taa-far-stabiliser measure) is below P0 and falls linearly to G (default 1 = the game\'s own brightness, within [0, gain]) at P1, so sub-pixel glowing windows of distant objects stop shimmering under TAA. 0 < P0 < P1 <= 1e6; suggested 60,120. The option latches the camera projection itself (no --taa needed). Default off. Requires an active light-map gain (--hdr, not --linear-materials, gain above 1).')
    ...
    if args.taa_thin_region is not None:
        ...
        if thin[0] > 0:
            ...
            if gate_given and args.taa_far_stabiliser is not None and len(args.taa_far_stabiliser.split(',')) == 6 and [float(v) for v in args.taa_far_stabiliser.split(',')[4:]] != thin[2:]:
    ...
    if args.action == 'install':
        ...
        for name, value in (('X3M_TAA_CURRENT_FILTER', args.taa_current_filter), ('X3M_TAA_HISTORY_WEIGHT', args.taa_history_weight), ('X3M_TAA_LINE_FILTER', args.taa_line_filter), ('X3M_TAA_FAR_STABILISER', args.taa_far_stabiliser), ('X3M_TAA_THIN_REGION', args.taa_thin_region),
