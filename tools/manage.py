...
def main():
    ...
    parser.add_argument('--taa-history-taps', choices=('5', '16'), default=None, help='History reconstruction of the TAA resolve (X3M_TAA_HISTORY_TAPS; requires --taa; DLL default 5): 5 = Catmull-Rom through five hardware-bilinear taps (docs/architecture/taa-high-resolution.md S3), 16 = the 16-tap point form of the earlier builds, for an in-flight A/B. A device that cannot filter FP16 / R32F textures draws 16 whatever is asked (logged as motion_output_taa_history_taps)')
    parser.add_argument('--taa-region-hold', choices=('on', 'off'), default=None, help='Region hold (A\') of the thin-region camera gate (X3M_TAA_REGION_HOLD; requires --taa; DLL default on; inert without the camera gate): on = the mask chain is its tests draw alone and the resolve composes the thin region itself, the region held one jitter cycle after a pixel was last flagged and the camera gate\'s closure held as long (the screen gate is not held, so the region reopens the frame a pan stops), both carried along the reprojection in the age target (docs/architecture/taa-plan-lifted-slot-cap.md step 1); off = the two dilation draws of the earlier builds (11x11 region, 17x17 speed gate), for an in-flight A/B. A device without FP16 / R32F filtering, or --taa-history-taps 16, keeps the dilations whatever is asked (logged as motion_output_taa_history_taps region_hold=0)')
    ...
    if args.taa_region_hold is not None and not args.taa:
        parser.error('--taa-region-hold requires --taa.')
    ...
    if args.action in ('install', 'uninstall', 'rollback', 'recover'):
        ...
        for name, value in (('X3M_TAA_HISTORY_WEIGHT', args.taa_history_weight), ('X3M_TAA_FAR_STABILISER', args.taa_far_stabiliser), ('X3M_TAA_THIN_REGION', args.taa_thin_region), ('X3M_TAA_THIN_REGION_GATE', args.taa_thin_region_gate), ('X3M_TAA_THIN_REGION_EMISSIVE', args.taa_thin_region_emissive), ('X3M_TAA_SENTINEL_STABILISER', args.taa_sentinel_stabiliser),
                            ...
                            ('X3M_TAA_HISTORY_TAPS', args.taa_history_taps), ('X3M_TAA_REGION_HOLD', args.taa_region_hold), ('X3M_TAA_THIN_VOTE', args.taa_thin_vote), ('X3M_FADE_RT2_OWNER', args.fade_rt2_owner),
