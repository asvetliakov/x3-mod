#!/usr/bin/env python3
"""Bake and packetize the two qualified spatial-fog atlases."""
...
def resource_header():
    return ("// Generated by tools/build/bake_fog_fields.py; do not edit.\n#pragma once\n"
            f"#define X3M_FOG_BLUEWELL_RESOURCE_ID {recipe.PROFILES['bluewell']['resource_id']}\n"
            f"#define X3M_FOG_FOGGREENOUTLANDS_RESOURCE_ID {recipe.PROFILES['foggreenoutlands']['resource_id']}\n")
...
def main() -> int:
    ...
    enums = {'bluewell': 'Bluewell', 'foggreenoutlands': 'Foggreenoutlands'}
    for name, profile in recipe.PROFILES.items():
        ...
        if row['decoded_sha256'] != profile['atlas_sha256']:
            raise SystemExit(f'{name} qualified atlas mismatch: {row["decoded_sha256"]}')
        (args.output_dir/f'{name}.fogbin').write_bytes(packet)
        rows.append(dict(name=name, enum=enums[name], profile_id=profile['id'],
                         resource_id=profile['resource_id'], base_sigma=profile['base_sigma'],
                         resource_sha256=sha256(packet), **row))
    metadata = metadata_header(rows); resources = resource_header()
