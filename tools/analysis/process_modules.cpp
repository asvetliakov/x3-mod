...
int main(int argc, char** argv) {
    if (argc < 2) {
        fprintf(stderr, "usage: process_modules.exe WINDOWS_PID [HEX_ADDRESS ...]\n");
        return 2;
    }
    const DWORD pid = strtoul(argv[1], nullptr, 10);
    HANDLE snapshot = CreateToolhelp32Snapshot(TH32CS_SNAPMODULE | TH32CS_SNAPMODULE32, pid);
    if (snapshot == INVALID_HANDLE_VALUE) {
        printf("module_error pid=%lu error=%lu\n", pid, GetLastError());
        return 1;
    }
    ...
    if (!process) {
        printf("process_error pid=%lu error=%lu\n", pid, GetLastError());
        return 1;
    }
    ...
    for (int i = 2; i < argc; ++i) {
        ...
        if (VirtualQueryEx(process, address, &memory, sizeof memory))
            printf("memory address=%p base=%p allocation=%p bytes=%lu state=%08lx protect=%08lx type=%08lx\n", address,
                   memory.BaseAddress, memory.AllocationBase, static_cast<DWORD>(memory.RegionSize), memory.State,
                   memory.Protect, memory.Type);
        else
            printf("memory_error address=%p error=%lu\n", address, GetLastError());
    }
    CloseHandle(process);
    return 0;
}
