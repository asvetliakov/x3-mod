...
int main(int argc, char** argv) {
    ...
    if (!disassemble) { std::fprintf(stderr, "GetProcAddress failed: %lu\n",GetLastError()); FreeLibrary(library); return 1; }
    WIN32_FIND_DATAA file{};
    HANDLE files = FindFirstFileA((std::string(argv[2])+"\\*.bin").c_str(), &file);
    if (files == INVALID_HANDLE_VALUE) { std::fprintf(stderr, "FindFirstFile failed for %s: %lu\n",argv[2],GetLastError()); FreeLibrary(library); return 1; }
    ...
    do {
        ...
        if (!input) { ++failures; continue; }
        std::fseek(input,0,SEEK_END);
        long length = std::ftell(input);
        std::rewind(input);
        if (length < 8 || length > 1024*1024 || length % 4) { std::fclose(input); ++failures; continue; }
        std::vector<DWORD> code(static_cast<size_t>(length)/4);
        const bool read_ok = std::fread(code.data(),1,static_cast<size_t>(length),input) == static_cast<size_t>(length);
        std::fclose(input);
        ...
        if (!read_ok || code.back() != 0x0000ffff) { ++failures; continue; }
        ...
        if (saved) ++processed; else ++failures;
