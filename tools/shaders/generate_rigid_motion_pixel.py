...
SHADERS = {
    ...
    # Thin vote (X3M_TAA_THIN_VOTE; docs/architecture/taa-thin-geometry-alternatives.md 3.2): .a = the relocated c218.x.
    'current_depth_thin': dict(source=ROOT / 'src/temporal/current_depth_thin_ps.hlsl',
                               header=ROOT / 'src/renderer/current_depth_thin_pixel_program_inc.h',
                               provenance=ROOT / 'verification/results/current-depth-thin-pixel-program.json'),
    # Fade owner (X3M_FADE_RT2_OWNER; docs/architecture/fade-rt2-ownership.md): .a = max(w * c218.z + c218.x, c218.y).
    'current_depth_owner': dict(source=ROOT / 'src/temporal/current_depth_owner_ps.hlsl',
                                header=ROOT / 'src/renderer/current_depth_owner_pixel_program_inc.h',
                                provenance=ROOT / 'verification/results/current-depth-owner-pixel-program.json'),
