...
way for the baseline (run294-far-bins-scale4 used frames 900..4200). usage: dt_windows.py"""
import re, glob, collections, statistics as st
for run in ('291', '294', '295', '296', '297', '298'):
    ...
    print(f"run{run} dither={opt.get('X3M_HDR_DITHER')} taa_debug={opt.get('X3M_TAA_DEBUG')} gpu_sync={opt.get('X3M_GPU_SYNC_TIMING')} scale={opt.get('X3M_FOG_MARCH_SCALE')} "
