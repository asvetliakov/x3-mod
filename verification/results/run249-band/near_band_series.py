"""d1-2 variant of mid_band_series.py (sed-derived). Temporal signature of dark-sky pixels by distance class (darksky.py definition), frames f:
...
for f in map(int,sys.argv[2:]):
    ...
    for name,m in(('d1-2',dkall&(d>=1)&(d<=2)),):
        s=sky&(d>=1)&(d<=2)
