"""Run 268 (Run 70 C2): copy of run265-atlas/pair_diff.py with three additions: object_bounds rows with inside=0 are ignored;
with ANCHOR=1 (6th arg "anchor") the fine box is not its own bounds union but the coarse box mapped through the node origin's
projected position and view-depth ratio (for a pair where one frame is clipped at the screen edge); and a display-unit block
(AgX, decode gamma2.2, ev from hdr_frame, agx as tools/analysis/taa_resolve_replay.agx) on the same aligned intersection I.
...
usage: pair_diff268.py CAPDIR LOG NODE COARSE_FRAME FINE_FRAME [anchor|-] [PNG_OUT]"""
import sys, re
import numpy as np
from PIL import Image
cap, log, node, fc, ff = sys.argv[1:6]; ANCHOR = len(sys.argv) > 6 and sys.argv[6] == 'anchor'; png = sys.argv[7] if len(sys.argv) > 7 else None
sys.path.insert(0, __import__('os').path.join(__import__('os').path.dirname(__file__), '../../../tools/analysis'))
import struct, agx_reference as ar
EV = {}; VIEW = {}; WPOS = {}; _cur = None; _rows = {}
EMIS = 0.06
kv = re.compile(r'(\w+)=(\S+)'); P = {fc: None, ff: None}; B = {fc: [], ff: []}
with open(log, errors='replace') as fh:
    for line in fh:
        if line.startswith('hdr_frame '):
            d = dict(kv.findall(line))
            if d.get('frame') in P: EV[d['frame']] = float(d['ev'])
            continue
        if line.startswith('object_context '):
            d = dict(kv.findall(line)); _cur = (d['frame'], {}) if d.get('frame') in P and d.get('node') == node and d['frame'] not in VIEW else None
            continue
        if _cur and line.startswith('object_matrix '):
            d = dict(kv.findall(line)); _cur[1].setdefault(d['role'], {})[int(d['row'])] = [struct.unpack('<f', bytes.fromhex(b)[::-1])[0] for b in d['bits'].split(',')]
            if len(_cur[1].get('view', {})) == 4 and len(_cur[1].get('world', {})) == 4:
                VIEW[_cur[0]] = np.array([_cur[1]['view'][i] for i in range(4)]); WPOS[_cur[0]] = np.array(_cur[1]['world'][3]); _cur = None
            continue
        if line.startswith('sun_shadow_apply_params ') or (line.startswith('object_bounds ') and f'node={node}' in line and ' inside=0' not in line):
...
def load(f):
    ...
    disp = np.clip(v @ np.array(ar.M_OUT).T, 0, 1); dl = 0.2126 * disp[..., 0] + 0.7152 * disp[..., 1] + 0.0722 * disp[..., 2]
    q = WPOS[f] @ VIEW[f]; org = ((q[0] / q[2] * m00 + 1) / 2 * W, (1 - q[1] / q[2] * m11) / 2 * H, q[2])
    return dict(lum=lum, s=s, mask=mask, nl=nl, nh=nh, same=same, w=w, dl=dl, org=org, box=(max(y0, 0), y1, max(x0, 0), x1))
C, F = load(fc), load(ff)
...
if ANCHOR:
    sc = C['org'][2] / F['org'][2]; ocx, ocy = C['org'][:2]; ofx, ofy = F['org'][:2]
    fx0, fx1 = int(round(ofx + (cx0 - ocx) * sc)), int(round(ofx + (cx1 - ocx) * sc)); fy0, fy1 = int(round(ofy + (cy0 - ocy) * sc)), int(round(ofy + (cy1 - ocy) * sc))
    print(f"anchor: origin coarse ({ocx:.1f},{ocy:.1f}) z={C['org'][2]:.0f}, fine ({ofx:.1f},{ofy:.1f}) z={F['org'][2]:.0f}; fine box mapped to ({fx0},{fy0})-({fx1},{fy1})")
...
cd_, fd_ = C['dl'][cy0:cy1, cx0:cx1], np.roll(np.roll(rs(F['dl'], t), dy, 0), dx, 1); dd_ = cd_ - fd_
nb = I & ~band
print(f"display (AgX ev {EV[fc]:.3f}/{EV[ff]:.3f}, luma 0..1): mean C/F={cd_[I].mean():.4f}/{fd_[I].mean():.4f} mean d={dd_[I].mean():+.4f} mean|d|={np.abs(dd_[I]).mean():.4f} "
      f"(x255={np.abs(dd_[I]).mean() * 255:.1f}); off-band mean|d|={np.abs(dd_[nb]).mean():.4f}; band mean|d|={np.abs(dd_[band]).mean():.4f}; "
      f"low-pass(3px) mean|d|={np.abs(gauss(np.where(I, dd_, 0.), 3.) / np.maximum(gauss(I.astype(float), 3.), 1e-6))[I].mean():.4f}; p90|d|={np.percentile(np.abs(dd_[I]), 90):.4f}")
for name, k in (('emissive', emis), ('highlight', high), ('edge band', band), ('plating', plat)):
    if k.any(): print(f"  display {name:10s} mean C/F={cd_[k].mean():.4f}/{fd_[k].mean():.4f} mean d={dd_[k].mean():+.4f} mean|d|={np.abs(dd_[k]).mean():.4f}")
for f, X in ((fc, C), (ff, F)):
    m = X['mask']; ns = (X['lum'] * (1 - X['s']))[m]; su = (X['lum'] * X['s'])[m]
    print(f"nonsun frame {f}: mask px={int(m.sum())} share>0.06={(ns > 0.06).mean():.3f} share>0.2={(ns > 0.2).mean():.3f} mean={ns.mean():.4f} p50/p90/p99={np.percentile(ns, 50):.4f}/{np.percentile(ns, 90):.4f}/{np.percentile(ns, 99):.4f} "
          f"sum/1000={ns.sum() / 1000:.2f} sum*z^2 rel={ns.sum() * X['org'][2] ** 2 / 1e12:.2f} | sun part mean={su.mean():.4f} p99={np.percentile(su, 99):.4f}")
if png:
