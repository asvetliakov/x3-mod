...
Run 79 A: flag31 (the race-18 root-node bit, patched by terran_station_lod) added per body.
...
CEN = re.compile(r'^cull_census device=\d+ frame=(\d+) view=(\w+) node=(\w+) model=(\w+) s=(-?\d+) .*? lod=(-?\d+) verdict=(\w+)(?: scope=\w+)?(?: lods=(\S+) thr=(\S+)(?: body=(\S+))?)?(?: flag31=(\d))?')
...
for line in open(log, errors='replace'):
    if line.startswith('cull_census device'):
        ...
        if b in mk: rows[f].append((b, int(m.group(5)), int(m.group(6)), m.group(8), m.group(9), m.group(7), m.group(2), m.group(11)))
...
for bf in bursts:
    ...
    per = collections.defaultdict(lambda: {'s': [], 'lod': collections.Counter(), 'lad': set(), 'view': collections.Counter(), 'f31': collections.Counter()})
    for f in bf:
        for b, s, lod, lods, thr, verdict, view, f31 in rows[f]:
            e = per[b]; e['f31'][f31] += 1; e['s'].append(s); e['lod'][lod] += 1; e['lad'].add((lods, thr)); e['view'][view] += 1
            tp = mk[b][2][-1]
    ...
    for b, e in sorted(per.items()):
        ...
        for f in bf:
            for (db, lod), c in draws[f].items():
                if db == b: dr[lod] += c
        print(f'slot{slot} {b} rows={len(e["s"])} views={dict(e["view"])} flag31={dict(e["f31"])} lod={dict(e["lod"])} ladder={sorted(e["lad"])} marker_thr={thr} '
