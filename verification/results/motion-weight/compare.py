#!/usr/bin/env python3
"""Motion history weight (docs/architecture/taa-motion-history-weight.md): the ledger's evidence rows from the fixture outputs.
...
Prints: the MOTION_WEIGHT rows, every RESOLVE_BUDGET row (before / after), the LOOP_TIMING_SUMMARY and
...
VOLATILE = ('RESOLVE_BUDGET', 'LOOP_TIMING', 'LINE_TIMING', 'RESULT PASS', 'LATTICE_BASE', 'FLICKER_BASE', 'LINE_BASE', 'FAR_BASE', 'CHECK ', 'SETA_EXIT', 'MOTION_WEIGHT')
...
def main():
    ...
    for name in ('temporal-pass.txt', 'temporal-lattice.txt'):
        ...
        for line in rows(new, 'MOTION_WEIGHT '):
            print(line)
