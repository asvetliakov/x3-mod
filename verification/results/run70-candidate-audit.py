"""Producer for the run70-candidate-qualification.json audit and comparison fields.

  python3 verification/results/run70-candidate-audit.py pe BASE.dll CANDIDATE.dll
      import DLLs / imported functions added or removed, export names, x3m_ exports
      (parses `i686-w64-mingw32-objdump -p`).
  python3 verification/results/run70-candidate-audit.py diff OLD.json NEW.json
      leaf-by-leaf comparison of two fixture summaries (changed / added / removed paths).
  python3 verification/results/run70-candidate-audit.py dryrun OLD.txt NEW.txt [NEW=OLD,...]
  ...
  python3 verification/results/run70-candidate-audit.py bridge OLD/bridge.log NEW/bridge.log
      per-name CHECK counts and statuses of two fog_route_bridge_run.py bridge logs (names only in one,
      names whose count differs, shadow_ab / motes_ab name counts, MOTES_AB row fields that differ).
"""
import json
import re
import subprocess
...
def bridge(path):
    ...
    counts, status = {}, {}
    for name, state in re.findall(r'^CHECK (\S+) (\w+)\s*$', text, re.M):
        counts[name] = counts.get(name, 0) + 1
        status[state] = status.get(state, 0) + 1
    return counts, status, re.findall(r'^MOTES_AB (.*)$', text, re.M)


def main():
    if sys.argv[1] == 'bridge':
        (a, sa, ma), (b, sb, mb) = bridge(sys.argv[2]), bridge(sys.argv[3])
        rows = []
        for x, y in zip(ma, mb):
            fx, fy = dict(re.findall(r'(\w+)=(\S+)', x)), dict(re.findall(r'(\w+)=(\S+)', y))
            rows.append({k: [fx.get(k), fy.get(k)] for k in sorted(set(fx) | set(fy)) if fx.get(k) != fy.get(k)})
        print(json.dumps(dict(
            checks=[sum(a.values()), sum(b.values())], status=[sa, sb], names=[len(a), len(b)],
            only_old=sorted(set(a) - set(b)), only_new=sorted(set(b) - set(a)),
            count_changed={n: [a[n], b[n]] for n in sorted(set(a) & set(b)) if a[n] != b[n]},
            shadow_ab_names=[sum(n.startswith('shadow_ab') for n in x) for x in (a, b)],
            motes_ab_names=[sum(n.startswith('motes_ab') for n in x) for x in (a, b)],
            motes_ab_rows=[len(ma), len(mb)], motes_ab_row_field_changes=rows), indent=1))
        return
