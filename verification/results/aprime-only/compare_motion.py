#!/usr/bin/env python3
"""(From verification/results/fade-rt2-owner.) A' only, dilated camera chain removed (2026-09-24): the full motion-output run
against the committed summary (5a4bbd52, written at 1a12984e). Every committed case but the three removed ones is still present
and every recorded leaf value is identical except run-bound ones: binary / trace hashes and directories, and wall-clock
measurements (leaf names ending in _us / _ms / _ns / us / ns or containing 'bench', 'ticks', 'elapsed', 'seconds'). Prints the
removed and added cases, the cases whose non-clock leaves differ (with the leaf names) and a total line.
...
# The off-path cases of the removed option (the hold-off twins and the too-long parse of X3M_TAA_REGION_HOLD).
REMOVED = {'seam-taa-region-hold-off', 'seam-taa-region-hold-too-long', 'seam-taa-thin-hold-off'}
