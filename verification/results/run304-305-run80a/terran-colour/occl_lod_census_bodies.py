"""Per-body variant (run304-305-run80a): keys are body|s5. s5 (t_OcclusionTexture of the XT pixel shader 5f82ecacd39529cd) binding against the drawn node's LOD index, over a
...
for log in sys.argv[1:]:
    ...
    for d in draws:
        ...
        b = body.get(d['model'], '?')
        groups[(d["lod"] != 0, over.get(b, "-"))][f"{b.rsplit(chr(47),1)[-1]}|{d['s5']}:{d['wh']}:L{d['lvl']}"] += 1
    ...
    for (nz, slot), c in sorted(groups.items()):
        print(f"  lod{'>0' if nz else '=0'} slot={slot} draws={sum(c.values())} distinct_s5={len(c)} top:",
              ' '.join(f'{k}x{v}' for k, v in c.most_common(40)))
