#!/usr/bin/env python3
"""Run323 adds depth_1_<frame> channel 0 at the sun box. For each hdr_1_<frame>.rgba16f capture (5120x1440 RGBA16F) of a run dir: sun NDC from camera_state + shadow_replay_sun,
...
for fr in frames:
    ...
    if 0 <= px < W and 0 <= py < H:
        ...
        dp = f'{D}/depth_1_{fr}.rgba32f'
        if os.path.exists(dp):
            dd = np.fromfile(dp, dtype=np.float32).reshape(H, W, 4)[y0:y1, x0:x1, 0]; box += f' depth centre {float(dd[20,20]) if dd.shape==(41,41) else float("nan"):.6f} box min {float(np.nanmin(dd)):.6f} max {float(np.nanmax(dd)):.6f}'
