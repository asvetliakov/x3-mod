...
Angle variant: bins by the sun angle off the view axis. usage: sun_group_angle.py LOG"""
...
for fr in sorted(set(cam) & set(sun)):
    ...
    rb = min(int(math.degrees(math.atan2(math.hypot(v[0], v[1]), v[2]))/2.5), 16)
    has = grp.get(fr, 0) > 0
    res[(per, rb)][0 if has else 1] += 1
    if not has: absent_xy[per].append((round(x, 3), round(y, 3), round(math.hypot(x, y), 3), fr))
print('(period, off-axis angle bin lower edge, deg): present / absent  (bin 16 = >= 40 deg)')
for k in sorted(res): print(k[0], f'{k[1]*2.5:.1f}', res[k][0], '/', res[k][1])
