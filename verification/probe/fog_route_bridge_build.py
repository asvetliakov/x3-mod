...
def main():
    p=argparse.ArgumentParser();p.add_argument('--production-root',type=Path,required=True);p.add_argument('--spatial-root',type=Path,required=True);p.add_argument('--asset-data',type=Path,required=True);p.add_argument('--output',type=Path,required=True);a=p.parse_args()
    ...
    while pending:
        ...
        for name in re.findall(r'^\s*#include\s+"([^"\n]+)"',source.read_text(),re.M):
            candidates=[source.parent/name,out/name,prod/'src/proxy'/name,prod/'src/renderer'/name,data/name]
    ...
    (out/'build.json').write_text(json.dumps({'executable':str(exe),'sha256':sha(exe),'command':command,'inputs':before},indent=2)+'\n');print(exe)
