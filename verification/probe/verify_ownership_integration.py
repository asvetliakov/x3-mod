...
def verify():
    ...
    assert len(build['cases'])==18 and all(c['exit']==0 for c in build['cases'].values()),'Incomplete integration run'
    ...
    for mode in MODES:
        for name, executable, _ in selected_fixtures(mode):
            ...
            if mode in ('on','copy_depth','scene_depth','finite_on'):assert len(wrapped)==(16 if name=='lifetime' else 1),(mode,name,wrapped)
            ...
            geometry = verify_geometry(trace, mode in ('finite_on','finite_without_ownership'), mode == 'finite_on')
            if name in ('smoke', 'capture'):
            ...
            report['cases'][f'{mode}-{name}']={'devices':expected,'destroyed':len(destroyed),'reused_device_addresses':reused, **geometry}
    ...
    for finite_mode, baseline_mode, name in (('finite_on','on','capture'), ('finite_without_ownership','off','capture')):
        assert (results/f'ownership-integration-{finite_mode}-{name}.txt').read_bytes() == (results/f'ownership-integration-{baseline_mode}-{name}.txt').read_bytes(), 'Finite wiring changes original capture API results'
    report['limits']=['Synthetic DLL integration, not gameplay validation.','Direct3D9Ex remains native and uninstrumented.','Finite enabled cases verify wiring and unsupported-input refusal; positive finite uploads have separate original-data fixtures.','Original-preserving depth-copy allocation smoke only; numerical/content behavior has a separate fixture.']
