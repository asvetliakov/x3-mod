...
DEFINE_GUID(CLSID_CWMSPDecMediaObject_local, 0x874131cb, 0x4ecc, 0x443b, 0x89, 0x48, 0x74, 0x6b, 0x89, 0x59, 0x5d,
            0x20);
DEFINE_GUID(CLSID_CWMADecMediaObject_local, 0x2eeb4adf, 0x4578, 0x4d10, 0xbc, 0xa7, 0xbb, 0x95, 0x5f, 0x56, 0x32, 0x0a);
DEFINE_GUID(DMOCATEGORY_AUDIO_DECODER_local, 0x57f2db8b, 0xe6bb, 0x4513, 0x9d, 0x43, 0xdc, 0xd2, 0xa6, 0x59, 0x31,
            0x25);
struct IDMOWrapperFilterLocal : public IUnknown {
    virtual HRESULT STDMETHODCALLTYPE Init(REFCLSID clsid, REFCLSID category) = 0;
};
...
struct Trace {
    ...
    template <class F> HRESULT step(int stream, const char* name, int attempt, bool loud, F f) {
        ...
        if (loud) {
            std::printf("REPLICA_BEGIN seq=%d stream=%d name=%s attempt=%d\n", seq, stream, name, attempt);
            std::fflush(stdout);
        }
        LARGE_INTEGER a{}, b{};
        QueryPerformanceCounter(&a);
        const DWORD now = GetTickCount();
        step_name.store(name);
        step_stream.store(stream);
        step_began.store(now ? now : 1); // never ahead of the watchdog's clock; 0 means idle
        HRESULT hr = f();
        step_began.store(0);
        step_name.store(nullptr);
        QueryPerformanceCounter(&b);
        if (loud) {
            std::printf("REPLICA_STAGE seq=%d stream=%d name=%s attempt=%d hr=%08lx wall_ms=%.3f\n", seq++, stream,
                        name, attempt, static_cast<unsigned long>(hr),
                        1000. * (b.QuadPart - a.QuadPart) / frequency.QuadPart);
            std::fflush(stdout);
        }
        return hr;
    }
...
enum Mode {
    ...
    MODE_GAME_DS_STEREO,
    MODE_GAME_DMO,
    MODE_GAME_DMO_FALLBACK,
    MODE_GAME_DMO_SKIP,
    MODE_GAME_DMO_HOOK
};
static bool game_dmo(Mode m) {
    return m == MODE_GAME_DMO || m == MODE_GAME_DMO_FALLBACK || m == MODE_GAME_DMO_SKIP || m == MODE_GAME_DMO_HOOK;
}
static bool game_ds(Mode m) {
    return m == MODE_GAME_DS || m == MODE_GAME_DS_STEREO || game_dmo(m);
}
...
struct Stream {
    ...
    bool created{};
    HRESULT early_update_hr = E_PENDING;
    int state{1};
    bool pending{};
    Mode mode{MODE_GAME};
    FILE* dump{};
    int cycle{};
    std::uint64_t dumped{}; // --dump-pcm sink, the play loop's cycle number and the running byte offset
    Stream(Trace& x, int i, Mode m)
        : t(x)
        , index(i)
        , mode(m) {}
    bool need(const char* name, HRESULT hr) {
        if (FAILED(hr)) {
            if (!std::strcmp(fatal, "none")) {
                fatal = name;
                fatal_hr = hr;
                std::printf("REPLICA_FAILURE stream=%d name=%s hr=%08lx\n", index, name,
...
#define NEED(name, expr)                                                                                               \
    do {                                                                                                               \
        if (!need(name, t.step(index, name, 1, true, [&] { return (expr); }))) return false;                           \
    } while (0)
    bool create(const wchar_t* path, IDirectSound8* sound) {
        const bool made = build(path, sound);
        if (!made && game_ds(mode))
            cleanup(); // 004d0160: MOV EAX,EBX; CALL 004d1d40 on the constructing thread, HRESULT discarded
        return made;
    }
    bool build(const wchar_t* path, IDirectSound8* sound) {
        ...
        for (int i = 1; i <= 2; ++i) {
            added = t.step(index, "add_audio", i, true,
        ...
        WAVEFORMATEX wanted{WAVE_FORMAT_PCM,
                            channels,
                            PRIMARY_RATE,
                            PRIMARY_RATE * channels * 2u,
        ...
        if (game_dmo(mode)) {
            ...
            HRESULT made = t.twice(index, "dmo_wrapper_create", [&] {
                return CoCreateInstance(CLSID_DMOWrapperFilter_local, nullptr,
            ...
            if (SUCCEEDED(made) && mode == MODE_GAME_DMO_HOOK) {
                ...
                std::vector<std::uint32_t> object(0x40, 0);
                object[0x9c / 4] = reinterpret_cast<std::uint32_t>(wrapper);
                const std::uint32_t esp_before = [] {
                    std::uint32_t v;
                    asm volatile("movl %%esp,%0" : "=r"(v));
                    return v;
                }();
                const HRESULT inited = t.step(index, "dmo_wrapper_init_hooked", 1, true,
                                              [&] { return replica_media_init(object.data()); });
                const std::uint32_t esp_after = [] {
                    std::uint32_t v;
                    asm volatile("movl %%esp,%0" : "=r"(v));
                    return v;
                }();
                ...
                std::printf(
                    "REPLICA_SITE stream=%d hr=%08lx esi=%08lx edi=%08lx ebx_ok=%d esp_ok=%d object=%08lx wrapper=%08lx\n",
                    index, static_cast<unsigned long>(inited), static_cast<unsigned long>(replica_site_witness[0]),
                    static_cast<unsigned long>(replica_site_witness[1]),
                ...
                HRESULT inited = E_FAIL;
                if (SUCCEEDED(t.step(index, "dmo_wrapper_qi", 1, true, [&] {
                        return wrapper->QueryInterface(IID_IDMOWrapperFilter_local, reinterpret_cast<void**>(&init));
                    }))) {
                    for (int i = 1; i <= 2; ++i) {
                        inited = t.step(index, "dmo_wrapper_init", i, true, [&] {
                            return init->Init(CLSID_CWMSPDecMediaObject_local, DMOCATEGORY_AUDIO_DECODER_local);
                        });
                        // Hook action at the 004cfd46 return: on REGDB_E_CLASSNOTREG retry with the registered WMA
                        // decoder DMO; the game then sees the retry's HRESULT (Windows behaviour: one successful
                        // attempt).
                        if (inited == REGDB_E_CLASSNOTREG && mode == MODE_GAME_DMO_FALLBACK)
                            inited = t.step(index, "dmo_wrapper_init_fallback", i, true, [&] {
                                return init->Init(CLSID_CWMADecMediaObject_local, DMOCATEGORY_AUDIO_DECODER_local);
                            });
                        if (SUCCEEDED(inited)) break;
                    }
                    init->Release();
                }
                if (FAILED(inited) && mode == MODE_GAME_DMO_SKIP)
                    t.step(index, "dmo_wrapper_skip", 1, true, [&] {
                        wrapper->Release();
                        wrapper = nullptr;
                        return S_OK;
                    });
        ...
        if (mode == MODE_EXPLICIT) {
            ...
            opened = t.step(index, "add_source", 1, true,
                            [&] { return graph->AddSourceFilter(path, L"X File Source", &source); });
            if (SUCCEEDED(opened)) {
                IPin* pin{};
                opened = t.step(index, "find_output", 1, true, [&] { return source->FindPin(L"Output", &pin); });
                if (SUCCEEDED(opened))
                    for (int i = 1; i <= 2; ++i) {
                        opened = t.step(index, "render", i, true, [&] { return graph->Render(pin); });
        ...
        t.release(index, "release_audio_pre", audio);
        t.release(index, "release_media_pre", media);
        ...
        std::printf("REPLICA_FORMAT stream=%d tag=%u rate=%lu channels=%u bits=%u align=%u avg=%lu\n", index,
                    format.wFormatTag, static_cast<unsigned long>(format.nSamplesPerSec), format.nChannels,
                    format.wBitsPerSample, format.nBlockAlign, static_cast<unsigned long>(format.nAvgBytesPerSec));
        if (dump) {
            std::printf("REPLICA_PCM_FORMAT stream=%d tag=%u ch=%u rate=%lu bits=%u\n", index, format.wFormatTag,
                        format.nChannels, static_cast<unsigned long>(format.nSamplesPerSec), format.wBitsPerSample);
            std::fflush(stdout);
        }
        if (format.wFormatTag != WAVE_FORMAT_PCM || !format.nBlockAlign || !format.nAvgBytesPerSec ||
            format.nAvgBytesPerSec > 1000000)
            return need("pcm_domain", E_INVALIDARG);
        pcm.resize(format.nAvgBytesPerSec * 2); // +0x60: two seconds per application sample
        NEED("activate_audio_data", CoCreateInstance(CLSID_AMAudioData, nullptr, CLSCTX_INPROC_SERVER, IID_IAudioData,
        ...
        if (sound) {
            ...
            t.step(index, "create_dsound_buffer", 1, true,
                   [&] { return sound->CreateSoundBuffer(&desc, &buffer, nullptr); });
        }
        ...
        if (!game_ds(mode))
            t.step(index, "get_duration", 1, true, [&] { return position->get_Duration(&duration); }); // not in
                                                                                                       // 004cf460
        LONG seekable{};
        t.step(index, "can_seek_forward", 1, true, [&] { return position->CanSeekForward(&seekable); }); // 004d03c4
        NEED("stream_run", multi->SetState(STREAMSTATE_RUN));                                            // 004d03f5
        if (mode == MODE_EARLY_UPDATE) {
            early_update_hr = t.step(index, "early_update", 1, true,
...
#undef NEED
    ...
    const char* poll(HRESULT& hr, DWORD& bytes) {
        ...
        if (buffer) {
            DWORD play{}, write{};
            hr = t.step(index, "ds_position", 1, false, [&] { return buffer->GetCurrentPosition(&play, &write); });
            if (FAILED(hr)) return "ds_error";
        }
        if (state == 2) {
        ...
        if (state == 4) {
            ...
            const HRESULT times = t.step(index, "sample_times", 1, false,
                                         [&] { return sample->GetSampleTimes(&start, &end, &now); });
            if (dump && ptr && actual) {
                ...
                std::printf("REPLICA_PCM cycle=%d bytes=%lu t_start=%lld t_end=%lld src=%s offset=%llu written=%lu\n",
                            cycle, static_cast<unsigned long>(actual), static_cast<long long>(t0),
                            static_cast<long long>(t1), exposed ? "sample" : "offset",
        ...
        for (int i = 1; i <= 2; ++i) {
            hr = t.step(index, "sample_update", i, false,
                        [&] { return sample->Update(SSUPDATE_ASYNC, nullptr, nullptr, 0); });
            if (SUCCEEDED(hr)) break;
        }
        if (hr == S_OK) {
        ...
        state = 2;
        return FAILED(hr) ? "update_error" : "queued";
    }
    void cleanup() {
        if (game_ds(mode)) {
            cleanup_game();
        ...
        if (pending) {
            ...
            HRESULT hr = t.step(index, "sample_cancel", 1, true, [&] {
                ...
                } while (DWORD(GetTickCount() - begin) < 2000);
                return HRESULT_FROM_WIN32(WAIT_TIMEOUT);
            });
            if (hr == MS_S_PENDING || hr == HRESULT_FROM_WIN32(WAIT_TIMEOUT)) {
                std::printf("REPLICA_ABORT stage=sample_cancel_unresolved stream=%d hr=%08lx\n", index,
                            static_cast<unsigned long>(hr));
                std::fflush(stdout);
                ExitProcess(126);
            }
        ...
        t.release(index, "release_position", position);
        t.release(index, "release_control", control);
        t.release(index, "release_buffer", buffer);
        t.release(index, "release_sample", sample);
        t.release(index, "release_data", data);
        t.release(index, "release_audio", audio);
        t.release(index, "release_media", media);
    ...
    // 004d1d40 destructor: Stop(+0x44), IMediaControl::Stop(+0x74), SetState(STOP), Release +0x70/+0x74, helper
    // 004d1c20 (section 12: straight-line RemoveFilter(graph, slot); Release(slot) for +0xa4/+0xa0/+0x9c/+0xac/+0xa8,
    // HRESULTs discarded, no retry), Release graph +0x78 (Wine's filter_graph_Release loop runs inside it), then
    // 004d1a40 repeats Stop/Stop/SetState(STOP) on what is still held, frees the PCM buffer and releases
    // +0x44/+0x6c/+0x58/+0x54; 004d1b70 releases +0x04. No sample cancel: the game relies on the STOP transitions
    // alone.
    void cleanup_game() {
        for (int round = 1; round <= 2; ++round) {
            ...
            if (round == 1) {
                ...
                t.release(index, "release_media", media);
                if (graph && wrapper)
                    t.step(index, "remove_dmo_wrapper", 1, true, [&] { return graph->RemoveFilter(wrapper); });
                t.release(index, "release_dmo_wrapper", wrapper);
                if (graph && source)
                    t.step(index, "remove_source", 1, true, [&] { return graph->RemoveFilter(source); });
                t.release(index, "release_source", source);
                t.release(index, "release_graph", graph);
            }
        }
...
static LRESULT CALLBACK replica_wndproc(HWND h, UINT m, WPARAM w, LPARAM l) {
...
int wmain(int argc, wchar_t** argv) {
    ...
    if (argc < 6) return 2;
    bool ok{};
    const Mode mode = parse_mode(argv[1], ok);
    if (!ok) return 2;
    const int dwell_ms = _wtoi(argv[2]);
    if (dwell_ms < 0 || dwell_ms > 20000) return 2;
    const wchar_t* dump_path = nullptr;
    int dump_seconds = 60;
    for (int i = 6; i < argc; ++i) {
        if (!wcscmp(argv[i], L"--dump-pcm") && i + 1 < argc)
            dump_path = argv[++i];
        else if (!wcscmp(argv[i], L"--dump-seconds") && i + 1 < argc) {
            dump_seconds = _wtoi(argv[++i]);
            if (dump_seconds < 1 || dump_seconds > 600) return 2;
        } else
            return 2;
    }
    FILE* dump = nullptr;
    if (dump_path) {
        ...
        if (!dump) {
            std::printf("REPLICA_ABORT stage=dump_open\n");
            return 5;
        }
    }
    ...
    char mode_text[24];
    std::snprintf(mode_text, sizeof mode_text, "%ls", argv[1]);
    std::printf("REPLICA_HEADER schema=1 mode=%s streams=%d dwell_ms=%d watchdog_ms=%lu thread=%lu audible=0\n",
                mode_text, streams, dwell_ms, static_cast<unsigned long>(WATCHDOG_MS),
                static_cast<unsigned long>(owner_thread));
    if (dump) {
        std::printf("REPLICA_PCM_HEADER dump=1 dump_seconds=%d\n", dump_seconds);
        std::fflush(stdout);
    }
    Trace t;
    if (mode == MODE_GAME_DMO_HOOK) {
        ...
        unsigned char now[8]{};
        std::memcpy(now, replica_init_site, 8);
        std::printf("REPLICA_HOOK installed=%d site=%08lx patched=%d stub=%08lx tail=%08lx\n", installed,
                    static_cast<unsigned long>(site), now[0] == 0xe9,
                    reinterpret_cast<unsigned long>(x3m::voice_dmo_fallback::fixture_stub()),
                    reinterpret_cast<unsigned long>(x3m::voice_dmo_fallback::fixture_tail()));
        if (installed) {
            ...
            std::printf("REPLICA_STUB_BYTES kind=stub base=%08lx hex=", reinterpret_cast<unsigned long>(stub));
            for (unsigned i = 0; i < 144; ++i) std::printf("%02x", stub[i]);
            std::putchar('\n');
            std::printf("REPLICA_STUB_BYTES kind=tail base=%08lx hex=", reinterpret_cast<unsigned long>(tail));
            for (unsigned i = 0; i < 32; ++i) std::printf("%02x", tail[i]);
            std::putchar('\n');
        }
        std::fflush(stdout);
        if (!installed) {
            std::printf("REPLICA_ABORT stage=hook_install\n");
            return 4;
        }
    }
    HRESULT init = t.step(0, "co_initialize", 1, true, [] { return CoInitialize(nullptr); });
    if (FAILED(init)) {
        std::printf("REPLICA_ABORT stage=co_initialize hr=%08lx\n", static_cast<unsigned long>(init));
        return 0;
    }
    HWND window{};
    if (game_ds(mode)) {
        // 004dadd1/004dae0d: RegisterClassA with a real WndProc, CreateWindowExA(0, cls, cls, WS_VISIBLE|0xca0000, 0,
        // 0, 640, 480, ...).
        WNDCLASSA wc{};
        wc.lpfnWndProc = replica_wndproc;
        wc.hInstance = GetModuleHandleA(nullptr);
        wc.lpszClassName = "X3VoiceStartupReplicaGame";
        wc.hCursor = LoadCursorA(nullptr, MAKEINTRESOURCEA(32512));
        RegisterClassA(&wc);
        window = CreateWindowExA(0, wc.lpszClassName, wc.lpszClassName, 0x10000000 | 0xca0000, 0, 0, 640, 480, nullptr,
                                 nullptr, wc.hInstance, nullptr);
    } else {
        WNDCLASSW wc{};
        wc.lpfnWndProc = DefWindowProcW;
        wc.hInstance = GetModuleHandleW(nullptr);
        wc.lpszClassName = L"X3VoiceStartupReplica";
        RegisterClassW(&wc);
        window = CreateWindowW(wc.lpszClassName, L"X3 startup replica", WS_OVERLAPPEDWINDOW, 0, 0, 64, 64, nullptr,
                               nullptr, wc.hInstance, nullptr);
    }
    IDirectSound8* sound{};
    IDirectSoundBuffer* primary{};
    IDirectSound3DListener* listener{};
    LONG primary_volume = 0;
    bool volume_read = false;
    HRESULT ds = t.step(0, "directsound_create", 1, true,
                        [&] { return DirectSoundCreate8(&DSDEVID_DefaultPlayback, &sound, nullptr); });
    if (sound && game_ds(mode)) {
        DSCAPS caps{};
        caps.dwSize = sizeof(caps);
        static_assert(sizeof(DSCAPS) == 0x60, "004de180 DSCAPS size");
        t.step(0, "directsound_caps", 1, true, [&] { return sound->GetCaps(&caps); });
    }
    HRESULT coop = E_POINTER;
    if (sound && window)
        coop = t.step(0, "directsound_cooperative", 1, true,
                      [&] { return sound->SetCooperativeLevel(window, DSSCL_PRIORITY); });
    if (FAILED(coop) && sound) {
        sound->Release();
        sound = nullptr;
    }
    std::printf("REPLICA_STARTUP ds_hr=%08lx coop_hr=%08lx ds_present=%d window=%d\n", static_cast<unsigned long>(ds),
                static_cast<unsigned long>(coop), sound != nullptr, window != nullptr);
    if (sound && game_ds(mode)) {
        // 004de201..004de390: primary buffer with the 0xd1 -> 0x11 -> 0x1 fallback, 3D listener QI (tolerated),
        // Play(LOOPING), then SetFormat(0x608af8), GetVolume and the listener setup with CommitDeferredSettings.
        static const DWORD chain[3] = {DSBCAPS_PRIMARYBUFFER | DSBCAPS_CTRL3D | DSBCAPS_CTRLPAN | DSBCAPS_CTRLVOLUME,
                                       DSBCAPS_PRIMARYBUFFER | DSBCAPS_CTRL3D, DSBCAPS_PRIMARYBUFFER};
        static_assert(DSBCAPS_PRIMARYBUFFER == 0x1 && DSBCAPS_CTRL3D == 0x10 && DSBCAPS_CTRLPAN == 0x40 &&
                          DSBCAPS_CTRLVOLUME == 0x80,
                      "SDK DirectSound flags required");
        DWORD flags = 0;
        HRESULT create_hr = E_FAIL, play_hr = E_FAIL, format_hr = E_FAIL, commit_hr = E_FAIL;
        for (int i = 0; i < 3 && !primary; ++i) {
            flags = chain[i];
            create_hr = t.step(0, "primary_create", i + 1, true, [&] {
                DSBUFFERDESC d{};
                d.dwSize = sizeof(d);
                static_assert(sizeof(DSBUFFERDESC) == 0x24, "004de201 DSBUFFERDESC size");
                d.dwFlags = flags;
                return sound->CreateSoundBuffer(&d, &primary, nullptr);
            });
        }
        if (primary) {
            if (FAILED(t.step(0, "listener_qi", 1, true, [&] {
                    return primary->QueryInterface(IID_IDirectSound3DListener, reinterpret_cast<void**>(&listener));
                })))
                listener = nullptr;
            play_hr = t.step(0, "primary_play", 1, true, [&] { return primary->Play(0, 0, DSBPLAY_LOOPING); });
            if (play_hr == S_OK) {
                WAVEFORMATEX wfx{
                    WAVE_FORMAT_PCM, 2, PRIMARY_RATE, PRIMARY_RATE * 4, 4, 16, 0}; // 0x608af8 from 004b8740
                format_hr = t.step(0, "primary_set_format", 1, true, [&] { return primary->SetFormat(&wfx); });
                volume_read = SUCCEEDED(
                    t.step(0, "primary_get_volume", 1, true, [&] { return primary->GetVolume(&primary_volume); }));
                if (listener) {
                    t.step(0, "listener_doppler", 1, true,
                           [&] { return listener->SetDopplerFactor(1.0f, DS3D_IMMEDIATE); });
                    t.step(0, "listener_distance", 1, true,
                           [&] { return listener->SetDistanceFactor(0.001f, DS3D_IMMEDIATE); }); // _0x565624
                    t.step(0, "listener_rolloff", 1, true,
                           [&] { return listener->SetRolloffFactor(1.5f, DS3D_IMMEDIATE); }); // _0x565620
                    t.step(0, "listener_position", 1, true,
                           [&] { return listener->SetPosition(0.f, 0.f, 0.f, DS3D_IMMEDIATE); });
                    commit_hr = t.step(0, "listener_commit", 1, true,
                                       [&] { return listener->CommitDeferredSettings(); });
                }
            }
        }
        std::printf(
            "REPLICA_PRIMARY flags=%lx create_hr=%08lx play_hr=%08lx format_hr=%08lx volume=%ld listener=%d commit_hr=%08lx\n",
            static_cast<unsigned long>(flags), static_cast<unsigned long>(create_hr),
            static_cast<unsigned long>(play_hr), static_cast<unsigned long>(format_hr),
            static_cast<long>(primary_volume), listener != nullptr, static_cast<unsigned long>(commit_hr));
        std::fflush(stdout);
    }
    ...
    auto dwell = [&](const char* where) {
        ...
        const DWORD begin = GetTickCount();
        unsigned pumps = 0;
        while (DWORD(GetTickCount() - begin) < DWORD(dwell_ms) && !hung.load()) {
            t.step(0, "pump", 1, false, [&] {
                pump();
                return S_OK;
            });
            ++pumps;
        ...
        std::printf("REPLICA_DWELL after=%s pumps=%u elapsed_ms=%lu\n", where, pumps,
                    static_cast<unsigned long>(GetTickCount() - begin));
        std::fflush(stdout);
    };
    ...
    for (int n = 1; n <= streams; ++n) {
        ...
        Stream* s = new Stream(t, index, mode);
        if (index == 1) s->dump = dump;
        all.push_back(s);
        t.step(index, "pump", 1, true, [&] {
            pump();
            return S_OK;
        });
        const bool made = s->create(argv[2 + index], sound);
        std::printf(
            "REPLICA_STREAM stream=%d created=%d role=%s fatal=%s hr=%08lx duration=%.3f early_update_hr=%08lx\n",
            index, made, index == 1 ? "played" : "restored", s->fatal, static_cast<unsigned long>(s->fatal_hr),
            s->duration, static_cast<unsigned long>(s->early_update_hr));
        std::fflush(stdout);
        dwell(index == 1 ? "played_construction" : "restored_construction");
    }
    Stream* played = nullptr;
    for (Stream* s : all)
        if (s->index == 1) played = s;
    if (played && played->created) {
        ...
        if (SUCCEEDED(run)) {
            const DWORD begin = GetTickCount();
            int cycles = 0, completed = 0, queued = 0, pending_polls = 0, errors = 0, eos = 0, stuck = 0;
            ...
            while (DWORD(GetTickCount() - begin) < budget_ms && (dump || completed < 5) && !eos && !stuck &&
                   errors < 4 && !hung.load()) {
                t.step(1, "pump", 1, false, [&] {
                    pump();
                    return S_OK;
                });
                played->cycle = cycles + 1;
                DWORD got{};
                const char* outcome = played->poll(hr, got);
                ++cycles;
                if (!std::strcmp(outcome, "consumed")) {
                    ++completed;
                    bytes += got;
                } else if (!std::strcmp(outcome, "queued"))
                    ++queued;
                else if (!std::strcmp(outcome, "pending"))
                    ++pending_polls;
                else if (!std::strcmp(outcome, "eos"))
                    ++eos;
                else if (!std::strcmp(outcome, "eos_status_stuck"))
                    ++stuck;
                else if (std::strstr(outcome, "error"))
                    ++errors;
                if (std::strcmp(outcome, last)) {
                    std::printf("REPLICA_POLL cycle=%d state=%d outcome=%s hr=%08lx bytes=%lu elapsed_ms=%lu\n", cycles,
                                played->state, outcome, static_cast<unsigned long>(hr), static_cast<unsigned long>(got),
                                static_cast<unsigned long>(GetTickCount() - begin));
                    last = outcome;
                }
            ...
            std::printf(
                "REPLICA_PLAY stream=1 cycles=%d completed=%d queued=%d pending_polls=%d eos=%d stuck=%d errors=%d bytes=%llu elapsed_ms=%lu\n",
                cycles, completed, queued, pending_polls, eos, stuck, errors, static_cast<unsigned long long>(bytes),
                static_cast<unsigned long>(GetTickCount() - begin));
        } else
            std::printf(
                "REPLICA_PLAY stream=1 cycles=0 completed=0 queued=0 pending_polls=0 eos=0 stuck=0 errors=1 bytes=0 elapsed_ms=0\n");
        std::fflush(stdout);
    }
    ...
    if (primary && volume_read)
        t.step(0, "primary_set_volume", 1, true, [&] { return primary->SetVolume(primary_volume); });
    t.release(0, "release_listener", listener);
    t.release(0, "release_primary", primary);
    t.release(0, "release_directsound", sound);
    if (window) DestroyWindow(window);
    t.step(0, "co_uninitialize", 1, true, [] {
        CoUninitialize();
        return S_OK;
    });
    ...
    if (dump) {
        std::fflush(dump);
        std::printf("REPLICA_PCM_TOTAL bytes=%llu\n", static_cast<unsigned long long>(dumped_total));
        std::fclose(dump);
    }
