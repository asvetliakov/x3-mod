...
NativePS original_set_ps = nullptr;
bool fail_next_pixel_shader = false;
unsigned failed_pixel_setters = 0;
HRESULT WINAPI checked_set_ps(IDirect3DDevice9* d, IDirect3DPixelShader9* shader) {
    if (fail_next_pixel_shader) {
        fail_next_pixel_shader = false;
        ++failed_pixel_setters;
        return E_FAIL;
    }
...
HRESULT WINAPI checked_draw(IDirect3DDevice9* d, D3DPRIMITIVETYPE type, UINT count, const void* vertices, UINT stride) {
    ...
    DWORD adaptive = TRUE;
    if (d->GetNPatchMode() != 0.f || FAILED(d->GetRenderState(D3DRS_ENABLEADAPTIVETESSELLATION, &adaptive)) || adaptive)
        return D3DERR_INVALIDCALL;
    ++npatch_draw_checks;
    return original_draw(d, type, count, vertices, stride);
}
...
void bytes(const std::string& path, const void* p, size_t n) {
    ...
    require(bool(out.write((const char*)p, n)), "write bytes");
}
template <class T> T read(std::ifstream& f) {
    T v{};
    require(bool(f.read((char*)&v, sizeof(v))), "truncated input");
    return v;
}
...
std::vector<Case> cases(const std::string& directory) {
    ...
    char magic[8];
    require(bool(f.read(magic, 8)) && !std::memcmp(magic, "X3BP0004", 8), "input magic");
    unsigned n = read<unsigned>(f);
    require(n == 47, "input count");
    std::vector<Case> out;
    for (unsigned i = 0; i < n; ++i) {
        Case c{};
        c.w = read<unsigned>(f);
        c.h = read<unsigned>(f);
        c.mode = read<unsigned>(f);
        c.levels = read<unsigned>(f);
        c.strength = read<float>(f);
        c.sharp = read<float>(f);
        c.threshold = read<float>(f);
        c.exposure = read<float>(f);
        c.authored_glow_gain = read<float>(f);
        c.highlight_gain = read<float>(f);
        c.scatter = read<float>(f);
        c.source_clamp = read<float>(f);
        c.dither = read<float>(f);
        ...
        params.strength = c.strength;
        params.threshold = c.threshold;
        params.levels = c.levels;
        params.scatter = c.scatter;
        params.authored_glow_gain = c.authored_glow_gain;
        params.highlight_gain = c.highlight_gain;
        ...
        require(bool(f.read((char*)c.pixels.data(), c.pixels.size() * 2)), "input pixels");
        out.push_back(std::move(c));
    }
    require(f.peek() == std::char_traits<char>::eof(), "trailing input");
    return out;
}
struct Texture {
    ...
    Texture(IDirect3DDevice9* d, unsigned w, unsigned h, D3DFORMAT format, bool rt, bool gpu_source = false) {
        check(d->CreateTexture(w, h, 1, rt ? D3DUSAGE_RENDERTARGET : 0, format,
                               (rt || gpu_source) ? D3DPOOL_DEFAULT : D3DPOOL_MANAGED, &texture.p, nullptr),
              "CreateTexture");
        check(texture->GetSurfaceLevel(0, &surface.p), "GetSurfaceLevel");
    }
    void upload(const Case& c) {
        D3DSURFACE_DESC desc{};
        check(surface->GetDesc(&desc), "Upload desc");
        ...
        if (desc.Pool == D3DPOOL_DEFAULT) {
            ...
            check(device->CreateTexture(c.w, c.h, 1, 0, desc.Format, D3DPOOL_SYSTEMMEM, &staging.p, nullptr),
                  "Upload staging");
            target = staging.p;
        }
        D3DLOCKED_RECT l{};
        check(target->LockRect(0, &l, nullptr, 0), "Lock input");
        for (unsigned y = 0; y < c.h; ++y)
            std::memcpy((char*)l.pBits + y * l.Pitch, c.pixels.data() + y * c.w * 4, c.w * 8);
...
std::vector<DWORD> image(IDirect3DDevice9* d, IDirect3DSurface9* s) {
    D3DSURFACE_DESC desc{};
    check(s->GetDesc(&desc), "Image desc");
    Com<IDirect3DSurface9> staging;
    check(d->CreateOffscreenPlainSurface(desc.Width, desc.Height, desc.Format, D3DPOOL_SYSTEMMEM, &staging.p, nullptr),
          "Readback surface");
    check(d->GetRenderTargetData(s, staging.p), "GetRenderTargetData");
    D3DLOCKED_RECT l{};
    check(staging->LockRect(&l, nullptr, D3DLOCK_READONLY), "Readback lock");
    std::vector<DWORD> out(desc.Width * desc.Height);
    for (unsigned y = 0; y < desc.Height; ++y)
        std::memcpy(out.data() + y * desc.Width, (char*)l.pBits + y * l.Pitch, desc.Width * 4);
    check(staging->UnlockRect(), "Readback unlock");
    return out;
}
const char* names[] = {"quad_vs",
                       "bloom_extract_gamma_ps",
                       "bloom_extract_srgb_ps",
                       "bloom_extract_none_ps",
                       "bloom_extract_even_gamma_ps",
                       "bloom_extract_even_srgb_ps",
                       "bloom_extract_even_none_ps",
                       "bloom_down_ps",
                       "bloom_up_ps",
                       "bloom_agx_ps",
                       "taa_sharpen_ps",
                       "hdr_writeback_ps"};
struct Programs {
    Com<ID3DXBuffer> code[12];
    BloomPrograms bundle{};
    Programs(const char* compiler_path, const std::string& dir) {
        HMODULE m = LoadLibraryA(compiler_path);
        require(m, "Load compiler");
        FARPROC symbol = GetProcAddress(m, "D3DXCompileShader");
        decltype(&D3DXCompileShader) compile = nullptr;
        static_assert(sizeof(symbol) == sizeof(compile));
        std::memcpy(&compile, &symbol, sizeof(symbol));
        require(compile, "Compile symbol");
        ...
        for (unsigned i = 0; i < 12; ++i) {
            std::ifstream f(dir + "/" + names[i] + ".hlsl");
            std::string source{std::istreambuf_iterator<char>(f), {}};
            require(!source.empty(), "Shader source");
...
const D3DSAMPLERSTATETYPE samplers[] = {D3DSAMP_ADDRESSU,      D3DSAMP_ADDRESSV,      D3DSAMP_ADDRESSW,
                                        D3DSAMP_BORDERCOLOR,   D3DSAMP_MAGFILTER,     D3DSAMP_MINFILTER,
                                        D3DSAMP_MIPFILTER,     D3DSAMP_MIPMAPLODBIAS, D3DSAMP_MAXMIPLEVEL,
                                        D3DSAMP_MAXANISOTROPY, D3DSAMP_SRGBTEXTURE};
struct State {
    Com<IDirect3DSurface9> rt[4], depth;
    Com<IDirect3DBaseTexture9> tex[3];
    ...
    State(IDirect3DDevice9* d, unsigned mrt) {
        auto append = [&](const void* p, size_t n) {
            ...
            values.resize(offset + n / 4);
            std::memcpy(values.data() + offset, p, n);
        };
        for (unsigned i = 0; i < mrt; ++i) {
            HRESULT h = d->GetRenderTarget(i, &rt[i].p);
            require(SUCCEEDED(h) || h == D3DERR_NOTFOUND, "Get RT");
        }
        HRESULT dh = d->GetDepthStencilSurface(&depth.p);
        require(SUCCEEDED(dh) || dh == D3DERR_NOTFOUND, "Get depth");
        D3DVIEWPORT9 vp{};
        RECT sc{};
        check(d->GetViewport(&vp), "Get VP");
        check(d->GetScissorRect(&sc), "Get scissor");
        append(&vp, sizeof(vp));
        append(&sc, sizeof(sc));
        for (auto s : states) {
            DWORD v = 0;
            check(d->GetRenderState(s, &v), "Get RS");
            values.push_back(v);
        }
        for (unsigned i = 0; i < 3; ++i) {
            check(d->GetTexture(i, &tex[i].p), "Get texture");
            for (auto s : samplers) {
                DWORD v = 0;
                check(d->GetSamplerState(i, s, &v), "Get sampler");
                values.push_back(v);
            }
        }
        float c[32 * 4]{};
        check(d->GetPixelShaderConstantF(0, c, 32), "Get constants");
        append(c, sizeof(c));
        check(d->GetVertexShader(&vs.p), "Get VS");
        check(d->GetPixelShader(&ps.p), "Get PS");
        check(d->GetVertexDeclaration(&decl.p), "Get declaration");
        UINT off = 0, stride = 0, freq = 0;
        DWORD fvf = 0;
        check(d->GetStreamSource(0, &stream.p, &off, &stride), "Get stream");
        check(d->GetStreamSourceFreq(0, &freq), "Get freq");
        check(d->GetFVF(&fvf), "Get FVF");
        values.insert(values.end(), {off, stride, freq, fvf, DWORD(d->GetSoftwareVertexProcessing())});
        const float npatch = d->GetNPatchMode();
        append(&npatch, sizeof(npatch));
    }
    bool same(const State& b) const {
        for (unsigned i = 0; i < 4; ++i)
            if (rt[i].p != b.rt[i].p) return false;
        for (unsigned i = 0; i < 3; ++i)
            if (tex[i].p != b.tex[i].p) return false;
        return values == b.values && depth.p == b.depth.p && vs.p == b.vs.p && ps.p == b.ps.p && decl.p == b.decl.p &&
               stream.p == b.stream.p;
    }
};
struct HostBindings {
...
void seed(IDirect3DDevice9* d, Texture& main, Texture& sentinel, IDirect3DTexture9* secondary, IDirect3DSurface9* mrt,
    ...
    check(d->SetRenderTarget(0, main.surface.p), "Seed main");
    check(d->SetRenderTarget(1, mrt), "Seed MRT");
    check(d->SetDepthStencilSurface(nullptr), "Seed null depth");
    D3DVIEWPORT9 vp{
        outgoing ? 1u : 0u, outgoing ? 1u : 0u, outgoing ? c.w - 2 : c.w, outgoing ? c.h - 2 : c.h, .125f, .875f};
    if (!outgoing) {
        vp.MinZ = 0;
        vp.MaxZ = 1;
    }
    check(d->SetViewport(&vp), "Seed VP");
    RECT sc{1, 1, LONG(c.w - 1), LONG(c.h - 1)};
    check(d->SetScissorRect(&sc), "Seed scissor");
    const DWORD hostile[] = {TRUE,
                             TRUE,
                             TRUE,
                             TRUE,
                             TRUE,
    ...
    const DWORD sv[] = {D3DTADDRESS_WRAP,
                        D3DTADDRESS_MIRROR,
                        D3DTADDRESS_CLAMP,
                        0x10203040,
                        D3DTEXF_LINEAR,
    ...
    for (unsigned stage = 0; stage < 3; ++stage) {
        ...
        for (unsigned i = 0; i < sizeof(sv) / 4; ++i)
            check(d->SetSamplerState(stage, samplers[i], sv[i]), "Seed sampler");
    }
    float constants[32 * 4];
    for (unsigned i = 0; i < 128; ++i) constants[i] = float(i + 1) * .03125f + (outgoing ? 2.f : 0.f);
    check(d->SetPixelShaderConstantF(0, constants, 32), "Seed constants");
    check(d->SetVertexShader(host.vs.p), "Seed VS");
    check(d->SetPixelShader(host.ps.p), "Seed PS");
    if (outgoing)
        check(d->SetVertexDeclaration(host.declaration.p), "Seed declaration");
    else
        check(d->SetFVF(D3DFVF_XYZ | D3DFVF_TEX1), "Seed FVF");
    check(d->SetStreamSource(0, vb, 4, 20), "Seed stream");
    check(d->SetStreamSourceFreq(0, 1), "Seed frequency");
    check(d->SetSoftwareVertexProcessing(TRUE), "Seed SWVP");
    check(d->SetNPatchMode(hostile_npatch), "Seed NPatch");
}
void assert_state(IDirect3DDevice9* d, const State& before, unsigned mrt, const char* label) {
    State after(d, mrt);
    require(before.same(after), label);
}
...
void fill_report(const std::string& label, DWORD requested, const std::vector<DWORD>& pixels) {
    unsigned mismatches = 0, first = 0xffffffffu;
    DWORD observed = requested;
    for (unsigned i = 0; i < pixels.size(); ++i)
        if (pixels[i] != requested) {
            if (!mismatches) {
                first = i;
                observed = pixels[i];
            }
    ...
    std::printf("FILL label=%s requested=%08lx mismatches=%u first=%u observed=%08lx\n", label.c_str(),
                (unsigned long)requested, mismatches, first, (unsigned long)observed);
}
...
void neutral_fill_control(IDirect3DDevice9* d, const std::string& dir) {
    ...
    auto pixels = image(d, target.surface.p);
    bytes(dir + "/neutral_fill.bgra8", pixels.data(), pixels.size() * 4);
...
unsigned run_case(IDirect3DDevice9* d, const D3DCAPS9& caps, void* const* native, const BloomPrograms& programs,
                  const Case& c, unsigned index, const std::string& dir, bool faults, BloomPass& pass,
                  BloomCandidate* reset_token = nullptr, bool post_reset = false) {
    if (!pass.enabled()) check(pass.attach(d, native, caps, programs), "Attach");
    require(pass.enabled(), pass.caps().reason);
    const unsigned base_refs = pass.references();
    HostBindings host(d, programs);
    ...
    BloomPrepare input{};
    input.scene = scene.texture.p;
    input.decode = static_cast<x3::temporal::AgxDecode>(c.mode);
    input.sharpen = c.sharp;
    input.filter.levels = c.levels;
    input.filter.threshold = c.threshold;
    input.filter.scatter = c.scatter;
    input.filter.strength = c.strength;
    input.filter.authored_glow_gain = c.authored_glow_gain;
    input.filter.highlight_gain = c.highlight_gain;
    if (c.source_clamp > 0.f) input.filter.source_clamp = c.source_clamp;
    require(x3::temporal::prepare(input.agx, c.exposure, 0, input.decode, x3::temporal::AgxLook::none),
            "AgX constants");
    x3::temporal::set_dither(input.agx, c.dither > 0.f);
    auto& boundary = input.boundary;
    boundary.main = main.surface.p;
    check(main.surface->GetDesc(&boundary.main_desc), "Main desc");
    boundary.frame = 7;
    boundary.reset = post_reset ? 3 : 2;
    ...
    for (unsigned test = 0; test < runs; ++test) {
        if (!pass.enabled()) {
            pass.shutdown();
            check(pass.attach(d, native, caps, programs), "Reattach");
        }
        ...
        BloomPreparation prep;
        {
            ...
            if (test == 12) {
                pass.before_reset();
                pass.set_fault(BloomFault::Allocate);
            }
        ...
        if (test >= 10 && test <= 13) {
            ...
            check(d->EndScene(), "End failed prepare");
            ++checks;
            std::printf("CONTROL test=%u pass=1\n", test);
            continue;
        }
        ...
        if (test == 14) {
            pass.set_fault(BloomFault::Setup);
            pass.set_fault(BloomFault::Recovery);
        }
        const unsigned draws_before = npatch_draw_checks, setters_before = failed_pixel_setters;
        if (test == 15) {
            fail_next_pixel_shader = true;
            pass.set_fault(BloomFault::Recovery);
        }
        ...
        if (test == 9) {
            seed(d, main, sentinel, scene.texture.p, mrt.surface.p, vb.p, host, c, false);
            auto other = pass.prepare(input);
            require(other.ready && !pass.valid(saved_token), "Reprepare revoke");
            seed(d, main, sentinel, scene.texture.p, mrt.surface.p, vb.p, host, c, true);
        }
        const std::string prefix = (post_reset ? "reset_" : "") + std::string("c") + std::to_string(index) + "_t" +
                                   std::to_string(test);
        ...
        BloomCommit result;
        std::vector<DWORD> pixels, mrt_pixels;
        {
            State outgoing(d, caps.NumSimultaneousRTs);
            result = pass.commit(saved_token, boundary);
            pixels = image(d, main.surface.p);
            mrt_pixels = image(d, mrt.surface.p);
            bytes(dir + "/" + prefix + "_post.bgra8", pixels.data(), pixels.size() * 4);
            bytes(dir + "/" + prefix + "_mrt_post.bgra8", mrt_pixels.data(), mrt_pixels.size() * 4);
            if (test != 5)
                assert_state(d, outgoing, caps.NumSimultaneousRTs, "Outgoing state restore");
            else {
                State actual(d, caps.NumSimultaneousRTs);
                require(!outgoing.same(actual), "Partial restoration fault did not alter state");
            }
        }
        ...
        if (test == 15)
            require(failed_pixel_setters == setters_before + 1 && !fail_next_pixel_shader &&
                        npatch_draw_checks == draws_before,
                    "Draw setter failure issued a native draw");
        if (test == 14 || test == 15)
            require(!result.write_attempted && result.original_preserved && result.state_preserved &&
                        result.recovery == S_FALSE && pass.enabled(),
                    "Setup-only failure attempted rollback");
        if (test == 0)
            require(result.committed && result.state_preserved && result.write_attempted, "Success flags");
        else
            require(!result.committed, "Fault published committed");
        if (test == 2 || test == 3)
            require(result.original_preserved && result.state_preserved && result.write_attempted, "Rollback flags");
        if (test == 3) require(result.requalification_required && !pass.enabled(), "Restore failure must disable");
        if (test == 4)
            require(!result.original_preserved && result.state_preserved && !pass.enabled(), "Recovery failure flags");
        if (test == 5)
            require(result.original_preserved && !result.state_preserved && !pass.enabled(), "Recovery state flags");
        if (test == 1 || test >= 6)
            require(!result.write_attempted && result.original_preserved, "Rejected boundary wrote");
        check(d->EndScene(), "EndScene");
        for (unsigned i = 0; i < pixels.size(); ++i)
            require((pixels[i] >> 24) == (baseline[i] >> 24), "Original alpha changed");
        if (test == 0 || test == 4)
            require(pixels != baseline, "Candidate did not write real RGB");
        else
            exact_image(pixels, baseline, "Original backup not recovered exactly");
        exact_image(mrt_pixels, mrt_baseline, "Secondary MRT was written");
        if (test == 0)
            bytes(dir + (post_reset ? "/reset_case.bgra8" : "/case_" + std::to_string(index) + ".bgra8"), pixels.data(),
                  pixels.size() * 4);
        ++checks;
        if (faults) std::printf("CONTROL test=%u pass=1\n", test);
        boundary.frame = 7;
        boundary.reset = 2;
    }
    if (reset_token) {
        ...
        auto pending = pass.prepare(input);
        require(pending.ready && pass.valid(pending.candidate), "Pre-Reset candidate");
        *reset_token = pending.candidate;
        check(d->EndScene(), "End pre-Reset candidate");
    }
    pass.before_reset();
    if (reset_token) require(!pass.valid(*reset_token), "Reset failed to revoke live candidate");
    require(pass.resource_bytes() == 0 && pass.references() == base_refs && pass.transient_views() == 0,
            "Reset resource cleanup");
    if (!reset_token) {
        pass.shutdown();
        require(pass.references() == 0 && pass.transient_views() == 0 && !pass.enabled(), "Shutdown reference cleanup");
    }
    check(d->SetTexture(0, nullptr), "Unbind0");
    check(d->SetTexture(1, nullptr), "Unbind1");
    check(d->SetTexture(2, nullptr), "Unbind2");
    check(d->SetRenderTarget(1, nullptr), "Unbind MRT");
    check(d->SetStreamSource(0, nullptr, 0, 0), "Unbind stream");
    std::printf("%s index=%u width=%u height=%u checks=%u pass=1\n", post_reset ? "RESET_CASE" : "CASE", index, c.w,
                c.h, checks);
    return checks;
}
}
...
int X3M_BLOOM_PASS_FIXTURE_ENTRY(int argc, char** argv) {
    try {
        require(argc == 3, "usage: fixture compiler.dll directory");
        const std::string dir = argv[2];
        auto input = cases(dir);
        Programs programs(argv[1], dir);
        ...
        for (const char* name : {"d3d9.dll", "wined3d.dll", "d3dx9_37.dll"}) {
            ...
            if (!loaded) continue;
            char path[32768]{};
            DWORD length = GetModuleFileNameA(loaded, path, sizeof(path));
            require(length && length < sizeof(path), "Module path");
            std::printf("MODULE name=%s path=%s\n", name, path);
        }
        HWND window = CreateWindowA("STATIC", "BloomPass fixture", WS_OVERLAPPEDWINDOW, 0, 0, 128, 128, nullptr,
        ...
        Com<IDirect3DDevice9> device;
        check(api->CreateDevice(0, D3DDEVTYPE_HAL, window, D3DCREATE_MIXED_VERTEXPROCESSING, &pp, &device.p),
              "Create mixed device");
        neutral_fill_control(device.p, dir);
        D3DCAPS9 caps{};
        check(device->GetDeviceCaps(&caps), "Caps");
        require(caps.NumSimultaneousRTs >= 2 && caps.NumSimultaneousRTs <= 4, "MRT control requires 2..4");
        void** actual = *reinterpret_cast<void***>(device.p);
        void* native[119];
        std::memcpy(native, actual, sizeof(native));
        original_draw = reinterpret_cast<NativeDraw>(native[83]);
        native[83] = reinterpret_cast<void*>(&checked_draw);
        original_set_ps = reinterpret_cast<NativePS>(native[107]);
        native[107] = reinterpret_cast<void*>(&checked_set_ps);
        HRESULT npatch = device->SetNPatchMode(2.5f);
        if (SUCCEEDED(npatch) && device->GetNPatchMode() == 2.5f)
            hostile_npatch = 2.5f;
        else
            check(device->SetNPatchMode(0.f), "Disable unsupported NPatch");
        ...
        if (SUCCEEDED(adaptive))
            check(device->GetRenderState(D3DRS_ENABLEADAPTIVETESSELLATION, &observed_adaptive), "Read adaptive probe");
        if (SUCCEEDED(adaptive) && observed_adaptive == TRUE)
            hostile_adaptive = TRUE;
        else
            check(device->SetRenderState(D3DRS_ENABLEADAPTIVETESSELLATION, FALSE),
                  "Disable unsupported adaptive tessellation");
        std::printf("ADAPTIVE accepted=%u hr=%08lx\n", unsigned(hostile_adaptive), (unsigned long)adaptive);
        unsigned checks = 0;
        BloomPass pass;
        BloomCandidate reset_token{};
        for (unsigned i = 0; i < input.size(); ++i)
            checks += run_case(device.p, caps, native, programs.bundle, input[i], i, dir, i == 0, pass,
                               i + 1 == input.size() ? &reset_token : nullptr);
        ...
        check(device->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &back.p), "Backbuffer");
        check(device->SetRenderTarget(0, back.p), "Restore backbuffer");
        back.p->Release();
        back.p = nullptr;
        ...
        BloomBoundary rejected{};
        rejected.admitted = true;
        rejected.frame = 7;
        rejected.reset = 3;
        rejected.thread = GetCurrentThreadId();
        auto old = pass.commit(reset_token, rejected);
        require(!old.committed && !old.write_attempted, "Pre-Reset token wrote after Reset");
        ...
        require(kResetCase < input.size() && input[kResetCase].w == 9 && input[kResetCase].h == 7 &&
                    input[kResetCase].mode == 2,
                "Post-Reset case identity");
        checks += run_case(device.p, caps, native, programs.bundle, input[kResetCase], kResetCase, dir, false, pass,
                           nullptr, true);
