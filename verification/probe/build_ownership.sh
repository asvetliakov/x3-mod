...
if [ "${1:-}" != "--baseline-only" ]; then
  i686-w64-mingw32-g++ -std=c++17 -O2 -msse2 -mfpmath=sse -mstackrealign -mincoming-stack-boundary=2 -Wall -Wextra -Werror -static -static-libgcc -static-libstdc++ -DX3M_OWNERSHIP_WRAPPED ownership_fixture.cpp ../../src/ownership/d3d9_ownership.cpp ../../src/ownership/execution_state.cpp ../../src/ownership/finite_buffer_evidence.cpp ../../src/ownership/portable_managed_upload.cpp -o build/ownership_wrapped.exe -ldxguid -luser32 -ladvapi32
fi
