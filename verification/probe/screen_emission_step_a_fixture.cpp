...
Image surface_image(Fixture &f, IDirect3DSurface9 *s) {
  ...
  for (unsigned y = 0; y < f.height; ++y)
    std::memcpy(im.data() + y * f.width * 4,
                static_cast<unsigned char *>(lock.pBits) + y * lock.Pitch, f.width * 8);
...
RECT coverage_rect(const Image &mask, unsigned w, unsigned h, LONG margin) {
  ...
  return RECT{std::max(0L, l - margin), std::max(0L, t - margin), std::min(LONG(w), r + margin), std::min(LONG(h), b + margin)};
}
// Channel differences of `got` against `ref_in` inside the rectangle and
// `ref_out` outside it (all four lanes).
struct Split { unsigned inside = 0, outside = 0; };
...
struct Sources {
  ...
  void load_pair(Fixture &f, const char *programs, unsigned p) {
    ...
    for (unsigned g = 0; g < 4; ++g) {
      ...
      config.gain = gains[g];
      Words code;
      need(re::linear_emission_sm1_pixel_variant(pw.data(), pw.size(), config, code) == re::LinearEmissionResult::Applied,
           "packed transformer");
...
double sanitize(double v) { return std::isnan(v) || v <= 0 ? 0. : std::min(v, 65504.); }
double decode(double v) { v = sanitize(v); return v > 0 ? std::pow(std::max(v, 1e-10), 2.2) : 0.; }
double encode(double v) { v = sanitize(v); return v > 0 ? std::pow(std::max(v, 1e-22), 1 / 2.2) : 0.; }
...
unsigned fade_oracle_diff(const Image &got, const Image &before, const RECT &quad, unsigned w, unsigned h, double &max_fraction) {
  unsigned d = 0;
  for (unsigned p = 0; p < w * h; ++p) {
    const bool in = inside(quad, p % w, p / w);
    for (unsigned k = 0; k < 4; ++k) {
      const unsigned short a = before[p * 4 + k], g = got[p * 4 + k];
      if (!in || k == 3) { if (g != a) ++d; continue; }
...
struct Pass {
  ...
  ~Pass() { pass.before_reset(); pass.detach(); }
};
LinearEmissionBoundary packed_boundary(IDirect3DSurface9 *scene, IDirect3DPixelShader9 *ps, std::uint64_t frame, const RECT *rect) {
  LinearEmissionBoundary b{scene, ps, frame, true, nullptr};
  b.policy = packed_policy;
  if (rect) { b.region = *rect; b.region_known = true; }
  return b;
}
struct Functional {
  Fixture &f;
  ...
  Functional(Fixture &fx, const char *p) : f(fx), programs(p), slots(device_slots(fx.d.p)) { targets(); }
  void targets() {
    f.target(work);
    f.target(pristine);
  ...
  void reset_work(IDirect3DSurface9 *from = nullptr) {
    api(f.d->StretchRect(from ? from : f.a.surface.p, nullptr, work.surface.p, nullptr, D3DTEXF_NONE), "work reset");
  }
  ...
  Bracket packed_bracket(LinearEmissionPass &pass, IDirect3DSurface9 *scene, unsigned which, unsigned schedule, Sources &s,
                         const RECT *rect, bool new_frame = true) {
...
void corpus(Functional &x, Pass &P, const char *rect_arg) {
  ...
  if (rect_arg) need(std::sscanf(rect_arg, "%ld,%ld,%ld,%ld", &straddle.left, &straddle.top, &straddle.right, &straddle.bottom) == 4,
                     "--rect l,t,r,b");
  for (unsigned p = 0; p < 9; ++p) {
    ...
    for (unsigned which = 0; which < CASES; ++which) {
      ...
      for (unsigned schedule = 0; schedule < 3; ++schedule) {
        ++rows;
        if (FAILED(s.created[gain_index(which)])) { ++unsupported; continue; }
        auto r = run_case(f, which, s.layout, schedule, s.vs.p, s.original.p, s.packed[gain_index(which)].p, s.measure.p);
        ...
        need(b.prep.ready && SUCCEEDED(b.source) && b.done.image == LinearEmissionImage::Linear && !b.done.candidate_bound &&
                 b.done.recovery == S_FALSE && SUCCEEDED(b.done.restore) && SUCCEEDED(b.done.composition),
        ...
        need(used.left == rect.left && used.top == rect.top && used.right == rect.right && used.bottom == rect.bottom, "bracket rectangle");
        ...
        exact_inside += !d.inside; exact_outside += !d.outside; exact_mask += !md; conservative += !outside;
        const bool ok = !d.inside && !d.outside && !md && !outside;
        exact += ok; per_schedule[schedule] += ok;
        std::printf("STEP_A_ROW pair=%u case=%u name=%s schedule=%u known=%u rect=%ld,%ld,%ld,%ld inside_diff=%u outside_diff=%u "
                    "mask_diff=%u red_outside=%u prototype_failed=%u surviving=%u overlap=%u\n",
                    p, which, cases[which], schedule, unsigned(known), used.left, used.top, used.right, used.bottom, d.inside,
                    d.outside, md, outside, unsigned(r.metrics.failed()), r.metrics.surviving, r.metrics.overlap);
        if (!ok && !witness_saved) {
          witness_saved = true;
          const std::string prefix = "step_a_p" + std::to_string(p) + "_c" + std::to_string(which) + "_s" + std::to_string(schedule);
        ...
        if (p == 0 && which == 0 && schedule == 0) {
          ...
          std::printf("STEP_A_STRADDLE rect=%ld,%ld,%ld,%ld inside_diff=%u outside_diff=%u red_outside=%u witness_fired=%u\n",
                      sb.done.region.left, sb.done.region.top, sb.done.region.right, sb.done.region.bottom, sd.inside, sd.outside,
                      fired, unsigned(fired > 0));
        }
      }
    }
  }
  std::printf("STEP_A_CORPUS rows=%u unsupported=%u exact=%u exact_inside=%u exact_outside=%u exact_mask=%u conservative=%u "
...
void sequences(Functional &x, Pass &P, Sources &s, IDirect3DPixelShader9 *fade_ps) {
  ...
  for (const bool fade_first : {true, false}) {
    ...
    if (fade_first) {
      ...
      packed_inside = d.inside; packed_outside = d.outside;
      ++x.frame;
    } else {
    ...
    std::printf("STEP_A_SEQUENCE order=%s fade_rect=%ld,%ld,%ld,%ld packed_rect=%ld,%ld,%ld,%ld fade_diff=%u fade_max_fraction=%.6g "
                "packed_inside_diff=%u packed_outside_diff=%u\n",
                fade_first ? "fade_packed" : "packed_fade", fade_rect.left, fade_rect.top, fade_rect.right, fade_rect.bottom,
                packed_rect.left, packed_rect.top, packed_rect.right, packed_rect.bottom, fade_diff, max_fraction, packed_inside,
                packed_outside);
  }
}
...
void alias(Functional &x, Pass &P, Sources &s, IDirect3DPixelShader9 *emission_ps) {
  ...
  x.reset_work();
  const RECT rect = coverage_rect(run_case(f, 0, s.layout, 0, s.vs.p, s.original.p, s.packed[0].p, s.measure.p).mask, f.width, f.height, 1);
  ...
  for (unsigned i = 0; i < 2; ++i) {
    ...
    need(e.prep.ready && SUCCEEDED(e.source) && e.done.image == LinearEmissionImage::Linear && e.done.candidate_bound,
  ...
  for (std::size_t i = 0; i < c[0].size(); ++i) { c_diff += c[0][i] != c[1][i]; m_diff += m[0][i] != m[1][i]; }
  f.detach();
...
void ladder(Functional &x, Pass &P, Sources &s) {
  ...
  constexpr LinearEmissionPassFault faults[] = {
      LinearEmissionPassFault::Copy, LinearEmissionPassFault::RegionScissor, LinearEmissionPassFault::PlaneInit,
      LinearEmissionPassFault::SourceBind, LinearEmissionPassFault::None, LinearEmissionPassFault::Composite,
      LinearEmissionPassFault::CompositeScissor, LinearEmissionPassFault::Restore, LinearEmissionPassFault::RegionRecovery,
  ...
  for (unsigned stage = 1; stage <= 10; ++stage) {
    ...
    if (stage <= 4) {
      need(!prep.ready && prep.operation == E_FAIL && prep.state_preserved && SUCCEEDED(prep.saved), "pre-source refusal");
      ...
      if (stage == 10) {
        need(FAILED(source) && done.composition == S_FALSE && done.restore == E_FAIL && done.recovery == S_OK, "restore+source");
        first = source;
        Com<IDirect3DSurface9> rt0;
      ...
      if (stage == 5 || stage == 9) {
        ...
        need(SUCCEEDED(source) && done.composition == E_FAIL && SUCCEEDED(done.restore) && done.recovery == S_OK, "composite recovery");
        first = done.composition;
      } else if (stage == 8) {
        need(SUCCEEDED(source) && SUCCEEDED(done.composition) && done.restore == E_FAIL && done.recovery == S_FALSE, "restore failure");
        first = done.restore;
    ...
    ++x.frame;
    std::printf("STEP_A_FAILURE stage=%u label=%s native=1 prepared=%u first=%08lx recovery=%08lx coverage=%u blocked=%u "
...
void capability(Functional &x) {
  ...
  // before any getter.
  {
    ...
    api(three.attach(f.d.p, x.slots.data(), caps, display.Format, D3DFMT_D24S8, all_policies), "three-target attach");
    need(three.caps().supported_policies == 7 && three.caps().available_policies == 7 && !three.caps().supports(packed_policy),
    ...
    need(!refusal.ready && refusal.operation == D3DERR_NOTAVAILABLE && refusal.saved == S_FALSE && refusal.state_preserved &&
             refusal.restore == S_FALSE && three.coverage_valid(),
  ...
  for (unsigned which = 0; which < 2; ++which) {
    auto caps = f.caps;
    if (which == 0) caps.DestBlendCaps &= ~DWORD(D3DPBLENDCAPS_INVSRCALPHA);
    else caps.PrimitiveMiscCaps &= ~DWORD(D3DPMISCCAPS_INDEPENDENTWRITEMASKS);
    LinearEmissionPass pass;
    api(pass.attach(f.d.p, x.slots.data(), caps, display.Format, D3DFMT_D24S8, all_policies), "capability attach");
    need(!pass.caps().supports(packed_policy) && pass.caps().available_policies == (which ? 1u : 7u), "capability gate");
    ++refused;
    pass.detach();
  }
  f.detach();
...
void reset_case(Functional &x, Pass &P, Sources &s) {
  ...
  need(P.pass.references() == 7 && !P.pass.reference_accounting_busy() && !P.pass.coverage_valid() && !P.pass.owning_candidate(),
...
void functional(const char *programs, const char *rect_arg) {
  ...
  std::printf("STEP_A_ATTACH supported=%u available=%u allocations=%u references=%u\n", P.pass.caps().supported_policies,
              P.pass.caps().available_policies, P.pass.allocations(), P.pass.references());
  corpus(x, P, rect_arg);
  Sources s;
  ...
  api(f.d->CreatePixelShader(reinterpret_cast<const DWORD *>(emission_words.data()), &emission_ps.p), "emission producer");
  sequences(x, P, s, fade_ps.p);
  alias(x, P, s, emission_ps.p);
  ladder(x, P, s);
  capability(x);
  reset_case(x, P, s);
  std::printf("STEP_A_COMPLETE policy=8 pairs=9 cases=20 schedules=3 owned_targets=5 target_bytes=%llu live_publication=0\n",
              5ull * W * H * 8);
}
...
void rect_vertices(Fixture &f, const RECT &rect) {
  ...
  for (unsigned i = 0; i < 2; ++i) {
    const float x0 = float(rect.left) + (i ? .25f : .05f) * float(rect.right - rect.left),
                ...
                y0 = float(rect.top) + .05f * float(rect.bottom - rect.top), y1 = float(rect.top) + .95f * float(rect.bottom - rect.top);
    const float l = -1 + x0 * 2 / f.width, r = -1 + x1 * 2 / f.width, t = 1 - y0 * 2 / f.height, b = 1 - y1 * 2 / f.height;
    const float z = i ? .3f : .2f;
    const DWORD color = ((i ? 192u : 128u) << 24) | 0x3070d0u;
    Vertex quad[] = {{l, t, z, i ? .75f : .25f, .5f, color}, {r, t, z, i ? .75f : .25f, .5f, color},
                     {l, b, z, i ? .75f : .25f, .5f, color}, {r, b, z, i ? .75f : .25f, .5f, color}};
...
void timing(const char *programs) {
  for (const auto &size : {std::make_pair(1280u, 768u), std::make_pair(1920u, 1080u)}) {
    ...
    for (const auto &area : {std::make_pair(56L, 42L), std::make_pair(150L, 161L)}) {
      const RECT rect{LONG(w / 2) - area.first / 2, LONG(h / 2) - area.second / 2, LONG(w / 2) - area.first / 2 + area.first,
                      LONG(h / 2) - area.second / 2 + area.second};
        ...
        for (const unsigned dips : {1u, 4u, 16u})
          for (unsigned iteration = 0; iteration < 10; ++iteration) {
            ...
            for (unsigned k = 0; k < dips; ++k) {
              if (!packed) { api(issue(f, 0), "timing native source"); continue; }
              const auto boundary = packed_boundary(x.work.surface.p, s.packed[0].p, x.frame, &rect);
              need(P.pass.prepare(boundary).ready, "timing bracket prepared");
              need(P.pass.finish(issue(f, 0)).image == LinearEmissionImage::Linear, "timing bracket completed");
            }
            ...
            ++x.frame;
            if (iteration >= 2)
              std::printf("STEP_A_TIMING width=%u height=%u policy=%s rect=%ld,%ld,%ld,%ld area=%ld dips=%u iteration=%u completed_ms=%.6f\n",
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    for (int i = 2; i < argc; ++i) {
      if (!std::strcmp(argv[i], "timing")) bench = true;
      else if (!std::strncmp(argv[i], "--rect=", 7)) rect = argv[i] + 7;
      else need(false, "unknown argument");
    }
    if (bench) timing(argv[1]);
    else functional(argv[1], rect);
