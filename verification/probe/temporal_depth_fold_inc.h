...
// G32R32F depth) a far-program run writes the next depth history as COLOR1 of the mask chain's first draw instead of the
// copy draw. Every run starts from the fixture's hostile state and must restore it exactly (Snapshot, COLORWRITEENABLE1
// set to 5 beforehand). Per configuration (the camera program with and without the sentinel stabiliser, the screen-gate
// thin region, the far stabiliser alone; three frames of a pan) the colour history, the depth history, the age target and
// the final mask must be byte-identical between:
//   lane fold / G32R32F fold / lane with the folding programs refused at creation (the copy draw) and an R32F twin holding
//   the same .r (the StretchRect path), all with the lane term off;
//   on the camera programs, lane fold against lane copy with the lane term on (c9.w = 1: the fold reads .b from s1's
//   centre texel, the copy path from s5).
// Negative control: a lane whose one texel .r is one ulp up must differ from the R32F twin in the depth history.
// Faults (lane input, lane term on): the RT1 bind refused, then the fold draw failed: the run fails like any draw
// failure (E_FAIL, nothing published, history dropped), RT1 is unbound right after the draw attempt, the caller's state
// (RT1, COLORWRITEENABLE1) comes back, and the next run folds again from an empty history. Reset: the device is Reset
// between two runs of a lane pass (before_reset / after_reset) and the fold resumes byte-identical to its R32F twin.
#include <memory>
...
struct CreateRefusal {
    ...
    static HRESULT WINAPI hook(IDirect3DDevice9* d,const DWORD* words,IDirect3DPixelShader9** out){
        ...
        if(words==reinterpret_cast<const DWORD*>(r::temporal_line_mask_depth_program())||words==reinterpret_cast<const DWORD*>(r::temporal_line_mask_camera_depth_program())){++refused;if(out)*out=nullptr;return D3DERR_OUTOFVIDEOMEMORY;}
...
// Records SetRenderTarget(1, ...) while alive; with `fail`, refuses the first bind of a surface other than `caller`.
struct TargetProbe {
    using Set=HRESULT(WINAPI*)(IDirect3DDevice9*,DWORD,IDirect3DSurface9*);
    static inline Set original=nullptr;static inline bool fail=false,refused=false;static inline IDirect3DSurface9* caller=nullptr;
    ...
    static HRESULT WINAPI hook(IDirect3DDevice9* d,DWORD index,IDirect3DSurface9* surface){
        if(index==1){calls.push_back(!surface?0:surface==caller?2:1);if(surface&&surface!=caller&&fail&&!refused){refused=true;return E_FAIL;}}
        return original(d,index,surface);}
    TargetProbe(IDirect3DDevice9* d,IDirect3DSurface9* callerSurface,bool refuse):previous(*reinterpret_cast<void***>(d)),device(d){
        ...
        fail=refuse;refused=false;caller=callerSurface;calls.clear();*reinterpret_cast<void***>(d)=table;}
    ~TargetProbe(){*reinterpret_cast<void***>(device)=previous;}
    // A pass-owned bind of RT1 followed by an unbind before the caller's surface comes back.
...
struct Scene {
    ...
    explicit Scene(IDirect3DDevice9* device):d(device){
        ...
        // Motion: every third row routed (alpha 1, 1.5 px/frame against the camera), the sentinel unrouted (alpha -1: the
        // sentinel stabiliser's class), the rest on the camera path (alpha 0).
...
struct Case{const char* name;bool camera,sentinel,thin;};
FrameInputs inputs(const Scene& s,const Case& c){
    ...
    if(c.sentinel)in.sentinel_strength=.7f;
    return in;}
void setup(IDirect3DDevice9* d,TemporalPass& pass,const DWORD* resolver,const Case& c,bool refuse){
    ...
    if(c.sentinel)check("fold configure sentinel",pass.configure_sentinel());}
// One run from the hostile state, restored exactly; the four published targets as bytes.
...
Result run(IDirect3DDevice9* d,Fixture& f,TemporalPass& pass,FrameInputs in,const char* label){
    ...
    if(SUCCEEDED(r.hr)){require(o.color&&o.depth&&o.age&&o.stabiliser_mask,"fold: a far-program run publishes the colour, depth, age and mask targets");
        r.out={bytes(d,o.color),bytes(d,o.depth),bytes(d,o.age),bytes(d,o.stabiliser_mask)};}
    return r;}
...
void depth_fold_cases(IDirect3DDevice9* d,Compiler compiler,const DWORD* resolver,D3DPRESENT_PARAMETERS& pp){
    ...
    const Case cases[]={{"camera_sentinel",true,true,true},{"camera",true,false,true},{"thin_screen",false,false,true},{"far_only",false,false,false}};
    {Fixture f(d,compiler);Scene s(d);
        for(const auto& c:cases){
            // 0 R32F twin, 1 lane fold, 2 lane copy (fold refused), 3 G32R32F fold; camera: 4 lane fold and 5 lane copy with the
            // lane term on; camera_sentinel: 6 the perturbed lane (negative control).
            struct Variant{const char* name;IDirect3DTexture9* depth;bool refuse,laneTerm;int against;bool folds;};
            std::vector<Variant> v={{"r32f",s.depth32.p,false,false,-1,false},{"lane_fold",s.lane.p,false,false,0,true},{"lane_copy",s.lane.p,true,false,0,false},{"g32r32f_fold",s.lane2.p,false,false,0,true}};
            if(c.camera){v.push_back({"lane_term_fold",s.lane.p,false,true,5,true});v.push_back({"lane_term_copy",s.lane.p,true,true,-1,false});}
            if(c.sentinel)v.push_back({"negative_control",s.laneBad.p,false,false,0,true});
            ...
            for(size_t i=0;i<v.size();++i){
                ...
                ok=ok&&folded[i]==(v[i].folds?3u:0u)&&history[i]==2&&std::strcmp(reason[i],v[i].folds?"lane_mrt":v[i].refuse?"program":"r32f_depth")==0;
        ...
        // Faults on the camera program with the sentinel stabiliser, lane input with the lane term on, history warm.
        const Case& c=cases[0];
        for(unsigned fault=0;fault<2;++fault){
            TemporalPass pass;setup(d,pass,resolver,c,false);FrameInputs in=inputs(s,c);in.current_depth=s.lane.p;in.camera_lane_parallax[0]=1e-4f;in.camera_lane_parallax[3]=1;
            check("fold fault warm",run(d,f,pass,in,"fold fault warm run restores the hostile state").hr);
            Result failed;bool unbound=false,reached=false;
            {std::unique_ptr<Fault> draw;if(fault==1)draw=std::make_unique<Fault>(d,1); // the first draw of the run is the folded tests draw
                TargetProbe probe(d,f.rt[1].p,fault==0);failed=run(d,f,pass,in,fault==0?"fold RT1 refusal restores RT1 and COLORWRITEENABLE1":"fold draw failure restores RT1 and COLORWRITEENABLE1");
                unbound=TargetProbe::unbound_after_bind();reached=fault==0?TargetProbe::refused:Fault::draws>=1;}
            const auto diagnostics=pass.diagnostics();
            const Result recovered=run(d,f,pass,in,"fold recovery restores the hostile state");
            std::printf("DEPTH_FOLD_FAULT kind=%s reached=%u hr=%08lx operation=%08lx history_valid=%u folded=%u rt1_unbound_after_bind=%u recovered_hr=%08lx recovered_folded=%u recovered_history=%u\n",
                fault==0?"rt1_bind":"fold_draw",unsigned(reached),(unsigned long)failed.hr,(unsigned long)diagnostics.operation,unsigned(diagnostics.history_valid),unsigned(failed.folded),unsigned(unbound),
            ...
            ++numeric_checks;require(reached&&failed.hr==E_FAIL&&diagnostics.operation==E_FAIL&&!diagnostics.history_valid&&!failed.folded&&unbound&&recovered.hr==S_OK&&recovered.folded&&!recovered.history,
                fault==0?"fold: a refused RT1 bind fails the run, unbinds RT1, restores the caller and folds again next run":"fold: a failed fold draw fails the run, unbinds RT1, restores the caller and folds again next run");}
    }
