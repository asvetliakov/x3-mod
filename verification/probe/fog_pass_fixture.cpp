...
std::uint16_t double_to_half(double v) { // round to nearest, v in the normal fp16 range or 0
    if (v <= 0) return 0;
    int e;
    const double m = std::frexp(v, &e); // v = m 2^e, m in [.5, 1)
    int exponent = e - 1 + 15;
    long mantissa = std::lround((m * 2 - 1) * 1024);
    if (mantissa == 1024) {
        mantissa = 0;
        ++exponent;
    }
...
fog_reference::Params reference_params(unsigned w, unsigned h) {
    ...
    p.m11 = 1.7320508;
    p.m00 = p.m11 * double(h) / double(w);
    p.m20 = 1. / w;
    p.m21 = -1. / h; // the latch with the quad pixel-centre term
    p.tau_max = .05;
    p.radius = kSceneRadius;
    p.g = .3;
    p.margin = 1;
    p.decode = 2.2;
    for (unsigned i = 0; i < 3; ++i) {
        p.sun[i] = kSun[i];
        p.radiance[i] = 3.14159265358979 * (i == 0 ? 1. : i == 1 ? .9 : .7);
    }
    return p;
}
...
double analytic_visible(const double q[3]) {
    if (q[2] < kPlateZ) {
        const double t = (kPlateZ - q[2]) / kSun[2], x = q[0] + kSun[0] * t, y = q[1] + kSun[1] * t;
        if (std::fabs(x) <= kPlateHalfX && y >= kPlateY0 && y <= kPlateY1) return 0;
    }
    if (q[2] < kWallZ) {
        const double t = (kWallZ - q[2]) / kSun[2], y = q[1] + kSun[1] * t;
        if (y < 0) return 0;
    }
    return 1;
}
...
Scene make_scene(unsigned w, unsigned h, const fog_reference::Params& p) {
    ...
    for (unsigned y = 0; y < h; ++y)
        for (unsigned x = 0; x < w; ++x) {
            double ray[3], length;
            fog_reference::ray_of(x, y, w, h, p, ray, &length);
            double z = -1;
            const double px = ray[0] * kPlateZ, py = ray[1] * kPlateZ;
            if (std::fabs(px) <= kPlateHalfX && py >= kPlateY0 && py <= kPlateY1)
                z = kPlateZ;
            else if (ray[1] * kWallZ < 0)
                z = kWallZ;
            float* texel = &s.depth[(std::size_t(y) * w + x) * 4];
            texel[0] = z > 0 ? float(p.m22 + p.m32 / z) : -1.f;
            texel[1] = .5f;
            texel[2] = z > 0 ? float(z) : 0.f;
            texel[3] = 0.f;
        }
    return s;
}
std::vector<fog_reference::Cascade> make_cascades() {
    ...
    for (unsigned c = 0; c < 3; ++c) {
        ...
        k.size = kMapSize;
        k.valid = true;
        k.map.assign(std::size_t(kMapSize) * kMapSize, 1.f);
        const double e = kExtents[c];
        for (unsigned i = 0; i < 3; ++i) {
            k.rows[i] = right[i] / e;
            k.rows[4 + i] = up[i] / e;
            k.rows[8 + i] = forward[i] / (2 * kDepthHalf);
        }
        k.rows[11] = .5;
        k.bias = 3. * (2 * e / kMapSize) / (2 * kDepthHalf);
        for (unsigned j = 0; j < kMapSize; ++j)
            for (unsigned i = 0; i < kMapSize; ++i) {
                const double sx = 2. * i / kMapSize - 1, sy = 1 - 2. * j / kMapSize;
                const double o[3] = {right[0] * sx * e + up[0] * sy * e, right[1] * sx * e + up[1] * sy * e,
                                     right[2] * sx * e + up[2] * sy * e};
                double nearest = 1e30;
                {
                    const double t = (kPlateZ - o[2]) / forward[2], x = o[0] + forward[0] * t,
                                 y = o[1] + forward[1] * t;
                    if (std::fabs(x) <= kPlateHalfX && y >= kPlateY0 && y <= kPlateY1) nearest = std::min(nearest, t);
                }
                {
                    const double t = (kWallZ - o[2]) / forward[2], y = o[1] + forward[1] * t;
                    if (y < 0) nearest = std::min(nearest, t);
                }
                if (nearest < 1e29)
                    k.map[std::size_t(j) * kMapSize + i] = float(std::clamp(nearest / (2 * kDepthHalf) + .5, 0., 1.));
            }
    }
    return out;
}
// ---- hooked vtable: fault injection and per-draw timing ----
using CreateTextureFn = HRESULT(WINAPI*)(IDirect3DDevice9*, UINT, UINT, UINT, DWORD, D3DFORMAT, D3DPOOL,
...
struct Faults {
    HRESULT texture_error = E_OUTOFMEMORY;
    unsigned texture_fail_at = 0, texture_calls = 0, shader_fail_at = 0, shader_calls = 0, draw_fail_at = 0,
             draw_calls = 0, stretch_fail_at = 0, stretch_calls = 0;
    HRESULT draw_error = E_FAIL;
} faults;
void* hooked[119];
void* original[119];
HRESULT WINAPI hook_create_texture(IDirect3DDevice9* d, UINT w, UINT h, UINT l, DWORD u, D3DFORMAT f, D3DPOOL p,
                                   IDirect3DTexture9** t, HANDLE* s) {
...
HRESULT WINAPI hook_stretch(IDirect3DDevice9* d, IDirect3DSurface9* a, const RECT* ar, IDirect3DSurface9* b,
                            const RECT* br, D3DTEXTUREFILTERTYPE f) {
...
HRESULT complete_fence(IDirect3DQuery9* q) {
    ...
    for (;;) {
        hr = q->GetData(nullptr, 0, D3DGETDATA_FLUSH);
        if (hr != S_FALSE) return hr;
        if (GetTickCount() - begin > 5000) return E_FAIL;
        Sleep(0);
    }
}
struct DrawTimer {
...
struct Snapshot {
    ...
    explicit Snapshot(IDirect3DDevice9* d) {
        ...
        float c[17 * 4]{};
        check("snapshot constants", d->GetPixelShaderConstantF(0, c, 17));
        add(c);
    }
...
struct Frame {
    ...
    Com<IDirect3DSurface9> depth_surface, rt1_surface, target_surface, auto_depth, read_lit, read_target, read_depth,
        read_rt1, read_sky, read_level;
    Frame(IDirect3DDevice9* d, unsigned width, unsigned height, D3DFORMAT depth_format = D3DFMT_A32B32G32R32F)
        : w(width)
        , h(height)
        , hw((width + 1) / 2)
        , hh((height + 1) / 2) {
        check("depth texture",
              d->CreateTexture(w, h, 1, D3DUSAGE_RENDERTARGET, depth_format, D3DPOOL_DEFAULT, &depth.p, nullptr));
        check("depth surface", depth->GetSurfaceLevel(0, &depth_surface.p));
        ...
        check("target texture", d->CreateTexture(w, h, 1, D3DUSAGE_RENDERTARGET, D3DFMT_A16B16G16R16F, D3DPOOL_DEFAULT,
                                                 &target.p, nullptr));
        check("target surface", target->GetSurfaceLevel(0, &target_surface.p));
        check("target staging",
              d->CreateTexture(w, h, 1, 0, D3DFMT_A16B16G16R16F, D3DPOOL_SYSTEMMEM, &target_sys.p, nullptr));
        for (auto& v : vertex)
            check("vertex texture", d->CreateTexture(4, 4, 1, 0, D3DFMT_A32B32G32R32F, D3DPOOL_DEFAULT, &v.p, nullptr));
        check("auto depth", d->GetDepthStencilSurface(&auto_depth.p));
        check("lit readback",
              d->CreateOffscreenPlainSurface(hw, hh, D3DFMT_A8R8G8B8, D3DPOOL_SYSTEMMEM, &read_lit.p, nullptr));
        check("target readback",
              d->CreateOffscreenPlainSurface(w, h, D3DFMT_A16B16G16R16F, D3DPOOL_SYSTEMMEM, &read_target.p, nullptr));
        check("depth readback",
              d->CreateOffscreenPlainSurface(w, h, depth_format, D3DPOOL_SYSTEMMEM, &read_depth.p, nullptr));
        check("rt1 readback",
              d->CreateOffscreenPlainSurface(w, h, D3DFMT_A8R8G8B8, D3DPOOL_SYSTEMMEM, &read_rt1.p, nullptr));
        check("sky readback",
              d->CreateOffscreenPlainSurface(1, 1, D3DFMT_A16B16G16R16F, D3DPOOL_SYSTEMMEM, &read_sky.p, nullptr));
        check("level readback",
              d->CreateOffscreenPlainSurface(8, 8, D3DFMT_A16B16G16R16F, D3DPOOL_SYSTEMMEM, &read_level.p, nullptr));
    }
    void upload_depth(IDirect3DDevice9* d, const Scene& s, unsigned channels) {
        D3DLOCKED_RECT lr{};
        check("lock depth", depth_sys->LockRect(0, &lr, nullptr, 0));
        for (unsigned y = 0; y < h; ++y)
            for (unsigned x = 0; x < w; ++x)
                std::memcpy(static_cast<char*>(lr.pBits) + y * lr.Pitch + x * channels * 4,
                            &s.depth[(std::size_t(y) * w + x) * 4], channels * 4);
    ...
    void upload_maps(IDirect3DDevice9* d, const std::vector<fog_reference::Cascade>& cascades) {
        for (unsigned c = 0; c < 3; ++c) {
            ...
            maps_sys[c].reset();
            check("map texture",
                  d->CreateTexture(n, n, 1, D3DUSAGE_RENDERTARGET, D3DFMT_R32F, D3DPOOL_DEFAULT, &maps[c].p, nullptr));
            check("map staging", d->CreateTexture(n, n, 1, 0, D3DFMT_R32F, D3DPOOL_SYSTEMMEM, &maps_sys[c].p, nullptr));
            D3DLOCKED_RECT lr{};
            check("lock map", maps_sys[c]->LockRect(0, &lr, nullptr, 0));
            for (unsigned y = 0; y < n; ++y)
                std::memcpy(static_cast<char*>(lr.pBits) + y * lr.Pitch, &cascades[c].map[std::size_t(y) * n], n * 4);
            check("unlock map", maps_sys[c]->UnlockRect(0));
            check("update map", d->UpdateTexture(maps_sys[c].p, maps[c].p));
        }
    }
    // Engine-space scene: the constant sky colour on sentinel pixels, a pattern below and above one on geometry, alpha
    // .375.
    std::vector<double> fill_target(IDirect3DDevice9* d, const Scene& s, const double sky[3]) {
        ...
        D3DLOCKED_RECT lr{};
        check("lock target", target_sys->LockRect(0, &lr, nullptr, 0));
        for (unsigned y = 0; y < h; ++y) {
            auto* row = reinterpret_cast<std::uint16_t*>(static_cast<char*>(lr.pBits) + y * lr.Pitch);
            for (unsigned x = 0; x < w; ++x) {
                const bool is_sky = s.depth[(std::size_t(y) * w + x) * 4] < 0;
                const double rgba[4] = {is_sky ? sky[0] : .02 + .9 * double(x) / w,
                                        is_sky ? sky[1] : 1.6 * double(y) / h + .01, is_sky ? sky[2] : .5, .375};
                for (unsigned c = 0; c < 4; ++c) {
                    row[x * 4 + c] = double_to_half(rgba[c]);
                    values[(std::size_t(y) * w + x) * 4 + c] = half_to_double(row[x * 4 + c]);
                }
            }
        }
    ...
    template <class T>
    std::vector<T> read(IDirect3DDevice9* d, IDirect3DSurface9* source, IDirect3DSurface9* sink, unsigned width,
                        unsigned height, unsigned channels) {
        check("readback", d->GetRenderTargetData(source, sink));
        D3DLOCKED_RECT lr{};
        check("lock readback", sink->LockRect(&lr, nullptr, D3DLOCK_READONLY));
        std::vector<T> out(std::size_t(width) * height * channels);
        for (unsigned y = 0; y < height; ++y)
            std::memcpy(&out[std::size_t(y) * width * channels], static_cast<char*>(lr.pBits) + y * lr.Pitch,
    ...
    void hostile(IDirect3DDevice9* d) {
        check("bind rt0", d->SetRenderTarget(0, target_surface.p));
        check("bind rt1", d->SetRenderTarget(1, rt1_surface.p));
        check("bind rt2", d->SetRenderTarget(2, depth_surface.p));
        check("bind depth", d->SetDepthStencilSurface(auto_depth.p));
        for (UINT i = 0; i < 4; ++i)
            check("bind vertex texture", d->SetTexture(D3DVERTEXTEXTURESAMPLER0 + i, vertex[i].p));
        for (auto [s, v] :
             {std::pair{D3DRS_ALPHABLENDENABLE, DWORD(TRUE)}, std::pair{D3DRS_SRCBLEND, DWORD(D3DBLEND_ONE)},
              std::pair{D3DRS_DESTBLEND, DWORD(D3DBLEND_INVSRCCOLOR)},
              std::pair{D3DRS_BLENDOP, DWORD(D3DBLENDOP_REVSUBTRACT)}, std::pair{D3DRS_SCISSORTESTENABLE, DWORD(TRUE)},
              std::pair{D3DRS_ZENABLE, DWORD(TRUE)}, std::pair{D3DRS_COLORWRITEENABLE, DWORD(1)},
              std::pair{D3DRS_CULLMODE, DWORD(D3DCULL_CW)}, std::pair{D3DRS_FILLMODE, DWORD(D3DFILL_WIREFRAME)},
              std::pair{D3DRS_SRGBWRITEENABLE, DWORD(TRUE)}, std::pair{D3DRS_FOGENABLE, DWORD(TRUE)},
              std::pair{D3DRS_ALPHATESTENABLE, DWORD(TRUE)}, std::pair{D3DRS_STENCILENABLE, DWORD(TRUE)},
              std::pair{D3DRS_MULTISAMPLEMASK, DWORD(0)}, std::pair{D3DRS_CLIPPING, DWORD(TRUE)}})
        ...
        for (UINT i = 0; i < 4; ++i) {
            ...
            check("hostile sampler", d->SetSamplerState(i, D3DSAMP_ADDRESSU, D3DTADDRESS_WRAP));
        ...
        float garbage[17 * 4];
        for (unsigned i = 0; i < 17 * 4; ++i) garbage[i] = float(i) * .37f - 9.f;
    ...
    void unbind(IDirect3DDevice9* d, IDirect3DSurface9* backbuffer) {
        check("unbind rt1", d->SetRenderTarget(1, nullptr));
        check("unbind rt2", d->SetRenderTarget(2, nullptr));
        check("unbind rt0", d->SetRenderTarget(0, backbuffer));
        for (UINT i = 0; i < 4; ++i) {
            check("unbind texture", d->SetTexture(i, nullptr));
            check("unbind vertex texture", d->SetTexture(D3DVERTEXTEXTURESAMPLER0 + i, nullptr));
        }
    }
};
...
void card_mask_cases(IDirect3DDevice9* d, Frame& f) {
    ...
    check("card save", d->CreateStateBlock(D3DSBT_ALL, &saved.p));
    check("card rt0", d->SetRenderTarget(0, f.target_surface.p));
    check("card rt1", d->SetRenderTarget(1, nullptr));
    check("card rt2", d->SetRenderTarget(2, nullptr));
    ...
    const Vertex vertices[] = {{-.5f, -.5f, 0, 1, 0xff808080, 0},
                               {float(f.w) - .5f, -.5f, 0, 1, 0xff808080, 0},
                               {-.5f, float(f.h) - .5f, 0, 1, 0xff808080, 0},
                               {float(f.w) - .5f, float(f.h) - .5f, 0, 1, 0xff808080, 0}};
    const WORD indices[] = {0, 1, 2, 2, 1, 3};
    Com<IDirect3DVertexBuffer9> vb;
    Com<IDirect3DIndexBuffer9> ib;
    check("card vb",
          d->CreateVertexBuffer(sizeof vertices, 0, D3DFVF_XYZRHW | D3DFVF_DIFFUSE, D3DPOOL_MANAGED, &vb.p, nullptr));
    check("card ib", d->CreateIndexBuffer(sizeof indices, 0, D3DFMT_INDEX16, D3DPOOL_MANAGED, &ib.p, nullptr));
    void* memory = nullptr;
    check("card vb lock", vb->Lock(0, 0, &memory, 0));
    std::memcpy(memory, vertices, sizeof vertices);
    check("card vb unlock", vb->Unlock());
    check("card ib lock", ib->Lock(0, 0, &memory, 0));
    std::memcpy(memory, indices, sizeof indices);
    check("card ib unlock", ib->Unlock());
    check("card stream", d->SetStreamSource(0, vb.p, 0, sizeof(Vertex)));
    check("card indices", d->SetIndices(ib.p));
    check("card vs", d->SetVertexShader(nullptr));
    check("card ps", d->SetPixelShader(nullptr));
    check("card fvf", d->SetFVF(D3DFVF_XYZRHW | D3DFVF_DIFFUSE));
    ...
    for (auto [state, value] :
         {std::pair{D3DRS_ZENABLE, DWORD(0)}, std::pair{D3DRS_ZWRITEENABLE, DWORD(0)},
          std::pair{D3DRS_STENCILENABLE, DWORD(0)}, std::pair{D3DRS_ALPHATESTENABLE, DWORD(0)},
          std::pair{D3DRS_FOGENABLE, DWORD(0)}, std::pair{D3DRS_SCISSORTESTENABLE, DWORD(0)},
          std::pair{D3DRS_LIGHTING, DWORD(0)}, std::pair{D3DRS_SRGBWRITEENABLE, DWORD(0)},
    ...
    D3DVIEWPORT9 vp{0, 0, f.w, f.h, 0, 1};
    check("card viewport", d->SetViewport(&vp));
    unsigned calls = 0, draws = 0;
    auto set = [&](DWORD mask) {
        ++calls;
        const HRESULT hr = d->SetRenderState(D3DRS_COLORWRITEENABLE, mask);
        SetLastError(0x1234);
        return hr;
    };
    Snapshot pre(d);
    ...
    ++draws;
    const HRESULT native = d->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 0, 4, 0, 2);
    SetLastError(0x4321);
    ...
    bool alpha = true;
    for (std::size_t i = 3; i < visible.size(); i += 4) alpha &= visible[i] == before[i];
    require("card_mask_native_hresult_rgb_alpha",
            native == vanilla && SUCCEEDED(vanilla) && visible != before && alpha);
    ...
    DWORD restored = 0;
    check("card mask read", d->GetRenderState(D3DRS_COLORWRITEENABLE, &restored));
    ...
    std::printf("CARD_INVALID_TOPOLOGY native=%08lx masked=%08lx mask=%lu restore=%08lx\n",
                (unsigned long)failed_native, (unsigned long)failed_masked, (unsigned long)restored,
                (unsigned long)mask.restore);
    require("card_mask_invalid_topology_parity",
            failed_native == failed_masked && restored == 7 && mask.restore == S_OK);
    ...
    std::printf(
        "CARD_FAILED_DRAW cause=no_indices native=%08lx masked=%08lx operation=%08lx mask=%lu restore=%08lx setter_calls=%u state_equal=%u\n",
        (unsigned long)missing_native, (unsigned long)missing_masked, (unsigned long)mask.operation,
        (unsigned long)restored, (unsigned long)mask.restore, calls - calls_before_failure,
        unsigned(failed_pre == failed_post));
    require("card_mask_failed_draw_restore", missing_native == D3DERR_INVALIDCALL && missing_native == missing_masked &&
                                                 mask.operation == S_OK && restored == 7 && mask.restore == S_OK &&
                                                 calls - calls_before_failure == 2 && failed_pre == failed_post);
    check("card restore indices", d->SetIndices(ib.p));
    require("card_mask_auxiliary_unchanged",
            rt1 == f.read<std::uint8_t>(d, f.rt1_surface.p, f.read_rt1.p, f.w, f.h, 4) &&
...
FogFrame frame_inputs(Frame& f, const fog_reference::Params& p, const std::vector<fog_reference::Cascade>& cascades) {
    FogFrame in;
    in.depth_share = f.depth.p;
    in.target = f.target_surface.p;
    in.width = f.w;
    in.height = f.h;
    in.count = 3;
    for (unsigned c = 0; c < 3; ++c) {
        in.cascades[c].map = f.maps[c].p;
        in.cascades[c].valid = cascades[c].valid;
        in.cascades[c].bias = float(cascades[c].bias);
    ...
    q.m22 = float(p.m22);
    q.m32 = float(p.m32);
    q.tau_max = float(p.tau_max);
    q.radius = float(p.radius);
    q.anisotropy = float(p.g);
    q.margin = float(p.margin);
    q.decode_exponent = float(p.decode);
    for (unsigned i = 0; i < 3; ++i) {
        q.sun_view[i] = float(p.sun[i]);
        q.sun_radiance[i] = float(p.radiance[i]);
    }
    q.jitter_index = p.jitter_index;
    q.update_sky = true;
    q.sky_blend = .25f;
...
std::vector<double> lit_values(Frame& f, IDirect3DDevice9* d, IDirect3DTexture9* lit) {
    Com<IDirect3DSurface9> s;
    check("lit surface", lit->GetSurfaceLevel(0, &s.p));
...
std::array<double, 4> sky_value(Frame& f, IDirect3DDevice9* d, FogPass& pass) {
    Com<IDirect3DSurface9> s;
    check("sky surface", pass.fixture_sky()->GetSurfaceLevel(0, &s.p));
...
struct Compare {
    double mean_abs = 0, max = 0;
    unsigned within = 0, pixels = 0;
};
Compare compare(const std::vector<double>& a, const std::vector<double>& b, double tolerance) {
    Compare c;
    c.pixels = unsigned(a.size());
    for (std::size_t i = 0; i < a.size(); ++i) {
        const double e = std::fabs(a[i] - b[i]);
        c.mean_abs += e;
        c.max = std::max(c.max, e);
        c.within += e <= tolerance;
    }
    c.mean_abs /= double(a.size());
    return c;
}
double ms(LONGLONG ticks, LONGLONG f) {
    return double(ticks) * 1000. / double(f);
}
...
void timing(IDirect3DDevice9* d, FogPass& pass, unsigned w, unsigned h, LONGLONG freq,
            const std::vector<fog_reference::Cascade>& cascades) {
    auto p = reference_params(w, h);
    p.radius = 10000;
    p.tau_max = .02; // the production medium
    Frame f(d, w, h);
    const Scene s = make_scene(w, h, p);
    f.upload_depth(d, s, 4);
    f.upload_maps(d, cascades);
    f.fill_target(d, s, kSkyEngine);
    ...
    const bool stamps = SUCCEEDED(d->CreateQuery(D3DQUERYTYPE_TIMESTAMPDISJOINT, &disjoint.p)) &&
                        SUCCEEDED(d->CreateQuery(D3DQUERYTYPE_TIMESTAMPFREQ, &frequency.p)) &&
                        SUCCEEDED(d->CreateQuery(D3DQUERYTYPE_TIMESTAMP, &begin.p)) &&
                        SUCCEEDED(d->CreateQuery(D3DQUERYTYPE_TIMESTAMP, &end.p));
    f.hostile(d);
    check("timing begin", d->BeginScene());
    FogFrame in = frame_inputs(f, p, cascades);
    in.caller_scene_open = true;
    FogResult out;
    check("timing warm", pass.execute(in, &out));
    for (const bool sky : {true, false}) {
        in.params.update_sky = sky;
        std::vector<double> on, off, submit, gpu;
        unsigned calls = 0;
        for (unsigned i = 0; i < 36; ++i) { // 3 warmup pairs, 15 measured pairs (on, off alternating)
            ...
            if (stamp) {
                check("disjoint begin", disjoint->Issue(D3DISSUE_BEGIN));
                check("stamp begin", begin->Issue(D3DISSUE_END));
            }
            QueryPerformanceCounter(&a);
            if (run) {
                check("timed execute", pass.execute(in, &out));
                calls = out.device_calls;
            }
            QueryPerformanceCounter(&b);
            if (stamp) {
                check("stamp end", end->Issue(D3DISSUE_END));
                check("stamp frequency", frequency->Issue(D3DISSUE_END));
                check("disjoint end", disjoint->Issue(D3DISSUE_END));
            }
            check("tail fence", complete_fence(fence.p));
            QueryPerformanceCounter(&c);
            if (stamp && i >= 6) {
                BOOL dj = TRUE;
                UINT64 fq = 0, t0 = 0, t1 = 0;
                auto wait = [&](IDirect3DQuery9* q, void* data, DWORD size) {
                    const DWORD start = GetTickCount();
                    HRESULT hr;
                    while ((hr = q->GetData(data, size, D3DGETDATA_FLUSH)) == S_FALSE && GetTickCount() - start < 5000)
                        Sleep(0);
                    return hr;
                };
                if (wait(disjoint.p, &dj, sizeof dj) == S_OK && wait(frequency.p, &fq, sizeof fq) == S_OK &&
                    wait(begin.p, &t0, sizeof t0) == S_OK && wait(end.p, &t1, sizeof t1) == S_OK && !dj && fq &&
                    t1 >= t0)
        ...
        std::printf(
            "TIMING width=%u height=%u sky=%u fenced_on_ms=%.4f fenced_off_ms=%.4f submit_ms=%.4f chain_ms=%.4f gpu_ms=%.4f gpu_timestamp_ms=%.4f timestamp_samples=%zu device_calls=%u samples=%zu\n",
            w, h, unsigned(sky), on_m, off_m, submit_m, on_m - off_m, std::max(0., on_m - off_m - submit_m),
            median(gpu), gpu.size(), calls, on.size());
    }
    ...
    for (unsigned i = 0; i < 8; ++i) {
        draw_timer = {};
        draw_timer.fence = fence.p;
        check("quads execute", pass.execute(in, &out));
        const DrawTimer sample = draw_timer;
        draw_timer = {};
        check("quads fences", sample.failure);
        require("per_quad_draw_count", sample.index == 4);
        if (i >= 2)
            for (unsigned q = 0; q < 4; ++q) per_quad[q].push_back(ms(sample.ticks[q], freq));
    }
    ...
    for (unsigned q = 0; q < 4; ++q) {
        const double m = median(per_quad[q]);
        total += m;
        std::printf(" %s_ms=%.4f", names[q], m);
    }
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        cls.lpszClassName = "X3FogPassFixture";
        RegisterClassA(&cls);
        HWND window = CreateWindowA(cls.lpszClassName, "X3 fog pass fixture", WS_OVERLAPPEDWINDOW, 90, 90, 128, 128,
        ...
        check("backbuffer", d->GetRenderTarget(0, &backbuffer.p));
        D3DDISPLAYMODE mode{};
        ...
        hooked[23] = reinterpret_cast<void*>(&hook_create_texture);
        hooked[106] = reinterpret_cast<void*>(&hook_create_ps);
        hooked[83] = reinterpret_cast<void*>(&hook_draw);
        hooked[34] = reinterpret_cast<void*>(&hook_stretch);
        std::printf("CAPS ps=%08lx vs=%08lx ps30_slots=%u rts=%u stretch_from_textures=%d\n",
                    ...
                    unsigned(caps.MaxPixelShader30InstructionSlots), unsigned(caps.NumSimultaneousRTs),
                    (caps.DevCaps2 & D3DDEVCAPS2_CAN_STRETCHRECT_FROM_TEXTURES) != 0);
        {
            FogPass twin;
            D3DCAPS9 c = caps;
            c.PixelShaderVersion = D3DPS_VERSION(2, 0);
            require("twin_ps_2_0", FAILED(twin.attach(d, hooked, c, mode.Format)) &&
                                       std::string(twin.caps().reason) == "ps_3_0" && twin.references() == 0);
            c = caps;
            c.MaxPixelShader30InstructionSlots = 64;
            require("twin_ps_slots", FAILED(twin.attach(d, hooked, c, mode.Format)) &&
                                         std::string(twin.caps().reason) == "ps_slots" && twin.references() == 0);
            c = caps;
            c.MaxPixelShader30InstructionSlots = 512; // the ps_3_0 minimum every device has
            {
                const bool attached = SUCCEEDED(twin.attach(d, hooked, c, mode.Format));
                require("twin_ps_slots_512",
                        attached && twin.caps().enabled && twin.caps().largest_program_slots <= 512);
                twin.detach();
            }
            c = caps;
            c.DestBlendCaps &= ~DWORD(D3DPBLENDCAPS_INVSRCALPHA);
            require("twin_blend_factors", FAILED(twin.attach(d, hooked, c, mode.Format)) &&
                                              std::string(twin.caps().reason) == "blend_factors" &&
                                              twin.references() == 0);
            c = caps;
            c.DevCaps2 &= ~DWORD(D3DDEVCAPS2_CAN_STRETCHRECT_FROM_TEXTURES);
            require("twin_stretch_rect", FAILED(twin.attach(d, hooked, c, mode.Format)) &&
                                             std::string(twin.caps().reason) == "stretch_rect" &&
                                             twin.references() == 0);
            faults = {};
            faults.shader_fail_at = 3;
            require("fault_shader_create", FAILED(twin.attach(d, hooked, caps, mode.Format)) &&
                                               std::string(twin.caps().reason) == "programs" &&
                                               twin.references() == 0 && !twin.caps().enabled);
            faults = {};
            // A transient attach failure is not sticky: the same object attaches on the retry (the caller retries after
            // Reset).
            require("attach_retry_after_failure", SUCCEEDED(twin.attach(d, hooked, caps, mode.Format)) &&
                                                      twin.caps().enabled && twin.references() == 6);
            twin.detach();
        }
        FogPass pass;
        check("attach", pass.attach(d, hooked, caps, mode.Format));
        std::printf("ATTACH enabled=%d largest_program_slots=%u references=%u\n", pass.caps().enabled,
                    pass.caps().largest_program_slots, pass.references());
        require("attach_enabled",
                pass.caps().enabled && pass.references() == 6 && pass.caps().largest_program_slots <= 512);
        // Allocation failure on the third target: every partial target is released.
        faults = {};
        faults.texture_fail_at = 3;
        require("fault_target_create",
                pass.prepare(W, H) == E_OUTOFMEMORY && pass.references() == 6 && pass.allocations() == 0);
        faults = {};
        auto p = reference_params(W, H);
        ...
        if (cards_only) {
            ...
            f->hostile(d);
            FogResult out{};
            check("card warmup", pass.execute(frame_inputs(*f, p, cascades), &out));
            require("card_resources_warm", out.applied && SUCCEEDED(out.restore) && pass.resources_ready(W, H) &&
                                               !pass.resources_ready(W + 1, H));
            pass.before_reset();
            require("card_resources_reset", !pass.resources_ready(W, H));
            f->unbind(d, backbuffer.p);
            delete f;
            f = nullptr;
            backbuffer.reset();
            check("card Reset", d->Reset(&pp));
            ...
            teardown_mark("device_release_begin");
            device.reset();
            d = nullptr;
            teardown_mark("device_release_end");
            teardown_mark("api_release_begin");
            api.reset();
            teardown_mark("api_release_end");
            ...
            const DWORD window_error = GetLastError();
        ...
        // Refusals (the off path): nothing touched, the target byte-identical.
        {
            ...
            FogResult out;
            FogFrame in = frame_inputs(*f, p, cascades);
            in.params.tau_max = 0.f;
            const bool zero = pass.execute(in, &out) == E_INVALIDARG && out.failed == FogStage::Validate &&
                              !out.applied;
            in = frame_inputs(*f, p, cascades);
            in.caller_queries_idle = false;
            const bool queries = pass.execute(in, &out) == E_INVALIDARG;
            in = frame_inputs(*f, p, cascades);
            in.params.anisotropy = .95f;
            const bool g = pass.execute(in, &out) == E_INVALIDARG;
            in = frame_inputs(*f, p, cascades);
            in.params.sun_view[2] = 2.f;
            const bool sun = pass.execute(in, &out) == E_INVALIDARG;
            in = frame_inputs(*f, p, cascades);
            in.target = f->rt1_surface.p;
            const bool eight = pass.execute(in, &out) == E_INVALIDARG;
            Snapshot post(d);
            require("refuse_zero_strength", zero);
            require("refuse_unknown_queries", queries);
            require("refuse_bad_anisotropy", g);
            require("refuse_bad_sun", sun);
            require("refuse_8bit_target", eight);
            require("refusal_state_untouched", pre == post);
            require("refusal_target_bit_identical",
                    before == f->read<std::uint16_t>(d, f->target_surface.p, f->read_target.p, W, H, 4));
        ...
        std::vector<double> clean_lit;
        std::vector<std::uint16_t> clean_after;
        for (const unsigned phase : {0u, 3u}) {
            ...
            Snapshot pre(d);
            FogResult out;
            const HRESULT hr = pass.execute(frame_inputs(*f, p, cascades), &out);
            Snapshot post(d);
            require((label + "_execute").c_str(), SUCCEEDED(hr) && out.applied && out.sky_updated &&
                                                      out.cascades_bound == 3 && out.lit && out.half_width == f->hw &&
                                                      out.failed == FogStage::None);
            ...
            const auto twin = fog_reference::march(
                scene.depth, W, H, p, [&](const double q[3]) { return fog_reference::map_visibility(cascades, p, q); });
            ...
            double shaft = 0, sky_lit = 0;
            unsigned shaft_n = 0, sky_n = 0, sky_bad = 0;
            for (unsigned j = 0; j < f->hh; ++j)
                for (unsigned i = 0; i < f->hw; ++i) {
                    double ray[3], length;
                    fog_reference::ray_of(2 * i, 2 * j, W, H, p, ray, &length);
                    const bool is_sky = scene.depth[(std::size_t(2 * j) * W + 2 * i) * 4] < 0;
                    const double v = lit[std::size_t(j) * f->hw + i];
                    if (!is_sky && std::fabs(ray[0]) < .1 && ray[1] < -.1 && ray[1] > -.4) {
                        shaft += v;
                        ++shaft_n;
                    } // rays under the plate: through its shadow volume
                    if (is_sky && std::fabs(ray[0]) > .6) {
                        sky_lit += v;
                        ++sky_n;
                        sky_bad += v != 1.;
                    } // beside the plate, above the wall: no occluder toward the sun on any sample
                }
            std::printf("ORACLE scene=phase%u label=shaft mean=%.4f pixels=%u %s\n", phase, shaft / shaft_n, shaft_n,
                        shaft_n && shaft / shaft_n < .9 ? "PASS" : "FAIL");
            ...
            std::printf("ORACLE scene=phase%u label=sky_clear mean=%.4f pixels=%u not_one=%u %s\n", phase,
                        sky_lit / sky_n, sky_n, sky_bad, sky_bad == 0 ? "PASS" : "FAIL");
            ...
            double coverage = 0;
            const auto mean = fog_reference::sky_mean(scene.depth, engine, W, H, p.decode, &coverage);
            double sky_error = 0;
            for (unsigned c = 0; c < 3; ++c) sky_error = std::max(sky_error, std::fabs(sky[c] - mean[c]) / mean[c]);
            std::printf("SKY scene=phase%u coverage=%.4f gpu=%.6f,%.6f,%.6f cpu=%.6f,%.6f,%.6f max_rel=%.5f\n", phase,
                        coverage, sky[0], sky[1], sky[2], mean[0], mean[1], mean[2], sky_error);
            ...
            const double gain_bound = 4. * *std::max_element(p.radiance, p.radiance + 3) * fog_phase(p.g, 1.) *
                                      (1 - std::exp(-p.tau_max));
            unsigned over = 0, alpha_changed = 0, bound_bad = 0, darker = 0;
            double max_rel = 0, sky_cap_error = 0;
            for (unsigned y = 0; y < H; ++y)
                for (unsigned x = 0; x < W; ++x) {
                    const std::size_t i = std::size_t(y) * W + x;
                    const auto ref = fog_reference::composite(scene.depth, lit, W, H, x, y, &engine[i * 4], sky_rgb, p);
                    for (unsigned c = 0; c < 3; ++c) {
                        const double expected = std::pow(ref.linear_out[c], 1 / p.decode),
                                     actual = half_to_double(after[i * 4 + c]);
                        const double rel = std::fabs(actual - expected) / std::max(expected, 1e-3);
                        max_rel = std::max(max_rel, rel);
                        over += rel > 2e-3;
                        const double linear_in = std::pow(engine[i * 4 + c], p.decode),
                                     linear_out = std::pow(actual, p.decode);
                        ...
                        // Sky cap in closed form (no reference call): a sentinel pixel beside the shaft carries the
                        // whole medium, F = 1.
                        if (scene.depth[i * 4] < 0 && ref.lit == 1.) {
                            const double cap = 1 - std::exp(-p.tau_max),
                                         closed = linear_in * (1 - cap) +
                                                  fog_reference::hue_of(sky_rgb)[c] * p.radiance[c] * ref.phase * cap;
            ...
            std::printf(
                "APPLY scene=phase%u channels=%zu over=%u max_rel=%.6f alpha_changed=%u bound_bad=%u darker=%u gain_bound=%.6f sky_cap_error=%.3g\n",
                phase, std::size_t(W) * H * 3, over, max_rel, alpha_changed, bound_bad, darker, gain_bound,
                sky_cap_error);
            ...
            require((label + "_rt2_untouched").c_str(),
                    depth_before == f->read<float>(d, f->depth_surface.p, f->read_depth.p, W, H, 4));
            require((label + "_rt1_untouched").c_str(),
                    rt1_before == f->read<std::uint8_t>(d, f->rt1_surface.p, f->read_rt1.p, W, H, 4));
        }
        p.jitter_index = 0;
        { // Sky history blend: a second update with another sky colour moves the history by the blend weight.
            ...
            FogResult out;
            FogFrame in = frame_inputs(*f, p, cascades);
            in.params.sky_blend = .25f;
            check("blend execute", pass.execute(in, &out));
            const auto after = sky_value(*f, d, pass);
            const auto mean = fog_reference::sky_mean(scene.depth, engine, W, H, p.decode, nullptr);
            double error = 0;
            for (unsigned c = 0; c < 3; ++c) {
                const double expected = .75 * before[c] + .25 * mean[c];
                error = std::max(error, std::fabs(after[c] - expected) / expected);
            }
            std::printf("SKY scene=blend max_rel=%.5f\n", error);
            require("sky_history_blend", error <= .01);
            in.params.update_sky = false;
            const auto held = sky_value(*f, d, pass);
            ...
            Scene covered = scene;
            for (std::size_t i = 0; i < covered.depth.size(); i += 4)
                if (covered.depth[i] < 0) {
                    covered.depth[i] = float(p.m22 + p.m32 / kWallZ);
                    covered.depth[i + 2] = float(kWallZ);
                }
            f->upload_depth(d, covered, 4);
            in.params.update_sky = true;
        ...
        { // Cascade 0 absent: its samples fall through to cascade 1 (coarser texels); the shaft stays.
            std::vector<fog_reference::Cascade> partial = cascades;
            partial[0].valid = false;
            f->fill_target(d, scene, kSkyEngine);
            FogResult out;
            check("partial execute", pass.execute(frame_inputs(*f, p, partial), &out));
            const auto lit = lit_values(*f, d, out.lit);
            const auto twin = fog_reference::march(
                scene.depth, W, H, p, [&](const double q[3]) { return fog_reference::map_visibility(partial, p, q); });
            const Compare c = compare(lit, twin, 1. / 16 + 1e-3);
            std::printf(
                "REFERENCE scene=cascade0_absent kind=map_twin pixels=%u mean_abs=%.6f max=%.4f within_one_step=%u\n",
                c.pixels, c.mean_abs, c.max, c.within);
            require("cascade_fallthrough",
                    out.cascades_bound == 2 && c.mean_abs <= .002 && double(c.within) / c.pixels >= .998);
            ...
            require("no_cascade_all_lit", out.cascades_bound == 0 && out.applied &&
                                              std::all_of(flat.begin(), flat.end(), [](double v) { return v == 1.; }));
        }
        { // R32F RT2 (no view-depth lane): z from the device depth; the same field within float32 depth precision.
            Frame narrow(d, W, H, D3DFMT_R32F);
            narrow.upload_depth(d, scene, 1);
            narrow.upload_maps(d, cascades);
            narrow.fill_target(d, scene, kSkyEngine);
            FogResult out;
            check("r32f execute", pass.execute(frame_inputs(narrow, p, cascades), &out));
            const Compare c = compare(lit_values(narrow, d, out.lit), clean_lit, 1. / 16 + 1e-3);
            std::printf(
                "REFERENCE scene=r32f kind=against_view_depth_lane pixels=%u mean_abs=%.6f max=%.4f within_one_step=%u\n",
                c.pixels, c.mean_abs, c.max, c.within);
            require("r32f_depth_fallback", c.mean_abs <= .002 && double(c.within) / c.pixels >= .998);
            narrow.unbind(d, backbuffer.p);
        }
        { // A lost device during target creation: D3DERR_DEVICELOST at the Targets stage, nothing retained, the state
          // and
            // the scene untouched; the same pass allocates and runs on the retry (the caller neither counts nor
            // disables).
            FogPass fresh;
            check("fresh attach", fresh.attach(d, hooked, caps, mode.Format));
            f->fill_target(d, scene, kSkyEngine);
            const auto before = f->read<std::uint16_t>(d, f->target_surface.p, f->read_target.p, W, H, 4);
            f->hostile(d);
            Snapshot pre(d);
            FogResult out;
            faults = {};
            faults.texture_fail_at = 2;
            faults.texture_error = D3DERR_DEVICELOST;
            const HRESULT hr = fresh.execute(frame_inputs(*f, p, cascades), &out);
            faults = {};
            Snapshot post(d);
            require("targets_device_lost", hr == D3DERR_DEVICELOST && out.failed == FogStage::Targets && !out.applied &&
                                               fresh.references() == 6 && fresh.allocations() == 0);
            require("targets_device_lost_untouched",
                    pre == post && before == f->read<std::uint16_t>(d, f->target_surface.p, f->read_target.p, W, H, 4));
            require("targets_retry_runs", SUCCEEDED(fresh.execute(frame_inputs(*f, p, cascades), &out)) &&
                                              out.applied && fresh.allocations() == 1 &&
                                              lit_values(*f, d, out.lit) == clean_lit);
            fresh.detach();
        }
        // ---- fault ladder ----
        {
            ...
            HRESULT hr = pass.execute(frame_inputs(*f, p, cascades), &out);
            faults = {};
            require("fault_march_failed", hr == E_FAIL && out.failed == FogStage::March && !out.applied &&
                                              out.lit == nullptr && SUCCEEDED(out.restore));
            faults.stretch_fail_at = 1;
            hr = pass.execute(frame_inputs(*f, p, cascades), &out);
            faults = {};
            require("fault_copy_failed", hr == E_FAIL && out.failed == FogStage::Copy && !out.applied);
            faults.draw_fail_at = 2;
            hr = pass.execute(frame_inputs(*f, p, cascades), &out);
            faults = {};
            require("fault_sky_failed", hr == E_FAIL && out.failed == FogStage::SkyLevel && !out.applied);
            Snapshot post(d);
            require("fault_restored_target_untouched",
                    pre == post && before == f->read<std::uint16_t>(d, f->target_surface.p, f->read_target.p, W, H, 4));
            ...
            faults = {};
            faults.draw_fail_at = 2;
            faults.draw_error = D3DERR_DEVICELOST; // the recovery above drew: the counter restarts
            check("lost scene", d->BeginScene());
            FogFrame lost_frame = frame_inputs(*f, p, cascades);
            lost_frame.caller_scene_open = true;
            lost_frame.params.update_sky = false;
            hr = pass.execute(lost_frame, &out);
            faults = {};
            check("lost scene end", d->EndScene());
            require("fault_lost_mid_chain",
                    hr == D3DERR_DEVICELOST && out.failed == FogStage::Composite && out.restore == D3DERR_DEVICELOST);
        ...
        pass.before_reset();
        {
            ...
            require("refuse_while_reset_pending",
                    pass.execute(frame_inputs(*f, p, cascades), &out) == D3DERR_DEVICENOTRESET &&
                        pass.prepare(W, H) == D3DERR_DEVICENOTRESET);
        }
        f->unbind(d, backbuffer.p);
        delete f;
        f = nullptr;
        ...
        {
            Frame g(d, W, H);
            g.upload_depth(d, scene, 4);
            g.upload_maps(d, cascades);
            g.fill_target(d, scene, kSkyEngine);
            g.hostile(d);
            Snapshot pre(d);
            FogResult out;
            const HRESULT hr = pass.execute(frame_inputs(g, p, cascades), &out);
            Snapshot post(d);
            const bool same_lit = SUCCEEDED(hr) && lit_values(g, d, out.lit) == clean_lit;
            const bool same_target = g.read<std::uint16_t>(d, g.target_surface.p, g.read_target.p, W, H, 4) ==
                                     clean_after;
            require("reset_bit_identical", same_lit && same_target && pre == post && pass.references() == 15 &&
                                               pass.allocations() == 2 && out.sky_updated);
        ...
        check("timing unbind", d->SetRenderTarget(1, nullptr));
        check("timing unbind", d->SetRenderTarget(2, nullptr));
        check("timing unbind", d->SetRenderTarget(0, backbuffer.p));
