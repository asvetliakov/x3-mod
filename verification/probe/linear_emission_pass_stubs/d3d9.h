...
constexpr HRESULT S_OK = 0, S_FALSE = 1, E_FAIL = -1, E_NOINTERFACE = -2, E_INVALIDARG = -3, E_OUTOFMEMORY = -4,
                  D3DERR_NOTAVAILABLE = -5, D3DERR_INVALIDCALL = -6, D3DERR_NOTFOUND = -7;
...
enum D3DFORMAT { D3DFMT_UNKNOWN, D3DFMT_A16B16G16R16F, D3DFMT_A8R8G8B8, D3DFMT_D24S8 };
...
constexpr DWORD D3DCULL_NONE = 1, D3DFILL_SOLID = 3, D3DBLEND_ONE = 2, D3DBLEND_ZERO = 1, D3DBLEND_INVSRCCOLOR = 4,
                D3DBLEND_SRCALPHA = 5, D3DBLEND_INVSRCALPHA = 6, D3DBLENDOP_ADD = 1, D3DTEXF_NONE = 0,
                D3DPBLENDCAPS_ONE = 2, D3DPBLENDCAPS_INVSRCALPHA = 32, D3DTEXF_POINT = 1, D3DTADDRESS_CLAMP = 3,
                D3DZB_TRUE = 1, D3DCLEAR_TARGET = 1, D3DUSAGE_RENDERTARGET = 1,
                D3DUSAGE_QUERY_POSTPIXELSHADER_BLENDING = 2, D3DPMISCCAPS_COLORWRITEENABLE = 1,
                D3DPMISCCAPS_SEPARATEALPHABLEND = 2, D3DPMISCCAPS_INDEPENDENTWRITEMASKS = 4,
                D3DPMISCCAPS_MRTPOSTPIXELSHADERBLENDING = 8, D3DFVF_XYZRHW = 4, D3DFVF_TEX1 = 256,
                D3DPRASTERCAPS_SCISSORTEST = 0x01000000;
constexpr DWORD D3DPS_VERSION(unsigned a, unsigned b) {
    return a * 256 + b;
}
constexpr DWORD D3DVS_VERSION(unsigned a, unsigned b) {
    return a * 256 + b;
}
constexpr unsigned char D3DDECLTYPE_FLOAT4 = 3, D3DDECLTYPE_FLOAT2 = 1, D3DDECLMETHOD_DEFAULT = 0,
...
struct D3DCAPS9 {
    DWORD NumSimultaneousRTs = 3, PixelShaderVersion = D3DPS_VERSION(3, 0), VertexShaderVersion = D3DVS_VERSION(3, 0),
          PrimitiveMiscCaps = D3DPMISCCAPS_MRTPOSTPIXELSHADERBLENDING, MaxTextureWidth = 4096, MaxTextureHeight = 4096,
          RasterCaps = 0, SrcBlendCaps = 0, DestBlendCaps = 0;
};
...
inline HRESULT IDirect3DTexture9::GetSurfaceLevel(UINT, IDirect3DSurface9** out) {
    *out = nullptr;
    if (!level_null && level && (SUCCEEDED(level_hr) || level_output_on_failure)) {
        level->AddRef();
        *out = level;
    }
    return level_hr;
