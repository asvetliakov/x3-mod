...
inline void ray_of(unsigned x, unsigned y, unsigned w, unsigned h, const Params& p, double ray[3], double* length) {
    ...
    ray[0] = ((u * 2 - 1) - p.m20) / p.m00;
    ray[1] = ((1 - v * 2) - p.m21) / p.m11;
    ray[2] = 1;
...
inline double map_visibility(const std::vector<Cascade>& cascades, const Params& p, const double q[3]) {
    for (const Cascade& c : cascades) {
        ...
        for (unsigned r = 0; r < 3; ++r)
            s[r] = c.rows[r * 4] * q[0] + c.rows[r * 4 + 1] * q[1] + c.rows[r * 4 + 2] * q[2] + c.rows[r * 4 + 3];
        if (!(std::max(std::fabs(s[0]), std::fabs(s[1])) <= p.margin && s[2] >= 0 && s[2] <= 1)) continue;
        const double n = c.size;
        const int i = int(std::clamp(std::floor((s[0] * .5 + .5 + .5 / n) * n), 0., n - 1)),
                  j = int(std::clamp(std::floor((-s[1] * .5 + .5 + .5 / n) * n), 0., n - 1));
...
inline double ign(unsigned i, unsigned j, unsigned jitter_index) {
    ...
    float inner = 0.06711056f * cx + 0.00583715f * cy;
    inner -= std::floor(inner);
    float outer = 52.9829189f * inner;
    outer -= std::floor(outer);
    return double(outer);
}
...
inline std::vector<double> march(const std::vector<float>& depth, unsigned w, unsigned h, const Params& p,
...
inline double phase(double g, double cosine) {
    return (1 - g * g) / (4 * 3.14159265358979 * std::pow(std::max(1 + g * g - 2 * g * cosine, 1e-6), 1.5));
}
...
struct Pixel {
    double transmittance, lit, phase;
    std::array<double, 3> linear_out;
};
...
inline Pixel composite(const std::vector<float>& depth, const std::vector<double>& lit_half, unsigned w, unsigned h,
                       unsigned x, unsigned y, const double engine[3], const double sky[3], const Params& p) {
    const unsigned hw = (w + 1) / 2, hh = (h + 1) / 2;
    double ray[3], length;
    ray_of(x, y, w, h, p, ray, &length);
    ...
    for (int b = 0; b < 2; ++b)
        for (int a = 0; a < 2; ++a) {
            const unsigned tx = unsigned(std::clamp(bx + a, 0., double(hw - 1))),
                           ty = unsigned(std::clamp(by + b, 0., double(hh - 1)));
            const double bilinear = (a ? fx - bx : 1 - (fx - bx)) * (b ? fy - by : 1 - (fy - by));
            const double zh = view_depth(
                &depth[(std::size_t(std::min(2 * ty, h - 1)) * w + std::min(2 * tx, w - 1)) * 4], p);
            const double weight = bilinear * (std::exp(-std::fabs(zh - zf) / (.05 * std::min(zf, zh))) + 1e-4);
            sum += weight * lit_half[std::size_t(ty) * hw + tx];
            weights += weight;
        }
    out.lit = sum / weights;
    ...
    for (unsigned c = 0; c < 3; ++c) {
        ...
        out.linear_out[c] = linear * out.transmittance +
                            hue[c] * p.radiance[c] * out.phase * out.lit * (1 - out.transmittance);
    }
    return out;
}
...
inline std::array<double, 3> sky_mean(const std::vector<float>& depth, const std::vector<double>& engine_rgba,
                                      unsigned w, unsigned h, double decode, double* coverage) {
    double sum[3]{}, count = 0;
    for (unsigned n = 0; n < 96; ++n)
        for (unsigned m = 0; m < 96; ++m) {
            const unsigned x = std::min(unsigned((m + .5) / 96. * w), w - 1),
                           y = std::min(unsigned((n + .5) / 96. * h), h - 1);
            const std::size_t i = std::size_t(y) * w + x;
            if (!(depth[i * 4] < 0.f)) continue;
            for (unsigned c = 0; c < 3; ++c) {
                const double e = std::clamp(engine_rgba[i * 4 + c], 0., 65504.);
                sum[c] += std::min(e > 0 ? std::pow(e, decode) : 0., 4.);
            }
            ++count;
        }
