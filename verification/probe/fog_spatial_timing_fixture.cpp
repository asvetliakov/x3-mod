...
int main(int argc, char** argv) {
    ...
    try {
        ...
        cls.lpszClassName = "X3FogSpatialTiming";
        RegisterClassA(&cls);
        Window window;
        window.handle = CreateWindowA(cls.lpszClassName, "Detached production fog timing", WS_OVERLAPPEDWINDOW, 0, 0,
        ...
        return paired ? fog_spatial_timing::qualify_pairs(device.p, mode.Format, argv[1], argv[2])
                      : fog_spatial_timing::qualify(device.p, mode.Format, argv[1], argv[2]);
