...
static void link(Node& parent, Node* const* children, unsigned count) {
    ...
    for (unsigned i = 0; i < count; ++i) { put(children[i]->bytes, 0, i + 1 < count ? addr(children[i + 1]) : addr(sentinel)); put(children[i]->bytes, core::parent_offset, addr(&parent)); }
}
...
static Result run(Node& root, View& view, std::uint32_t flag = 0) {
    ...
    fixture_call(&f);
    Result r{}; r.edi = f.out[0]; r.esi = f.out[1]; r.ebp = f.out[2]; r.ebx = f.out[4]; r.edx = f.out[5]; r.ecx = f.out[6]; r.eax = f.out[7]; r.flags = f.out[8] & 0x8d5; // CF PF AF ZF SF OF
...
struct Expected { std::int32_t s, measure, d, limit; };
static Expected expected(const Node& n, const View& v, const Node* parent) {
    Expected e{};
    const std::int32_t d0 = std::int32_t(get(n.bytes, d_offset)), scale = std::int32_t(get(v.bytes, view_scale)), w = std::int32_t(get(v.bytes, view_w)), r = std::int32_t(get(n.bytes, core::radius_offset));
...
static bool row_matches(const Row* r, const Node& n, const Node& reference, const Expected& e, const char* verdict) {
    ...
    return r->s == e.s && r->measure == e.measure && r->d == e.d && r->limit == e.limit && r->radius == std::int32_t(get(n.bytes, core::radius_offset))
        && r->thr_1d8 == std::int32_t(get(n.bytes, core::threshold_1d8_offset)) && r->thr_1dc == std::int32_t(get(n.bytes, core::threshold_1dc_offset))
        && r->model == get(n.bytes, core::model_offset) && r->lod == std::int32_t(get(reference.bytes, core::lod_offset)) && r->flags_out == get(reference.bytes, core::flags12c_offset)
        && ((r->flags_out & 2) != 0) == kept && !std::strcmp(r->verdict, verdict) && (std::strcmp(verdict, "kept") == 0) == kept;
}

static const unsigned char* site_a() { return synthetic_measure_window + core::measure_site_offset; }
static const unsigned char* site_b() { return synthetic_exit_window + core::exit_site_offset; }
static bool sites_original() { return !std::memcmp(site_a(), core::measure_site, core::site_length) && !std::memcmp(site_b(), core::exit_site, core::site_length); }
static bool windows_original() { return !std::memcmp(synthetic_measure_window, core::measure_window, core::measure_window_length) && !std::memcmp(synthetic_exit_window, core::exit_window, core::exit_window_length); }
...
static double bench_us(Node& root, View& view, bool arm, unsigned loops = 20000) {
    ...
    auto once = [&] { reset_tree(); if (arm) census::begin_frame(true); run(root, view); };
...
int main() {
    ...
    if (!windows_original()) { std::printf("CULL CENSUS CPU checks=%u failures=%u\n", checks, failures); return 1; }

    // ---- the tree: R with children A..J, A with two grandchildren ----
    View view; view_set(view, 1280, 0, 0x4000, 2);
    ...
    node_set(C, &R, 3000, 100000, 0x9002, 0, 40, 0x5003, 3, 100, 50);              // measure 38 < 1dc 40 (flag), s 19 -> lod 2 == last with 0x8000 -> culled_other
    node_set(E, &R, 3000, 100000, 0x1002, 0, 40, 0x5004, 3, 100, 50);              // as C without the fade flag -> kept, lod 2
    node_set(F, &R, 3000, 100000, 0x1002, 0, 0, 0x5005, 4, 100, 50, 25);           // s 19 < 25 -> lod 3 -> kept, +0x130 |= 0x100000
    ...
    check(!(get(reference[iA].bytes, 0x12c) & 2) && !(get(reference[iB].bytes, 0x12c) & 2) && !(get(reference[iC].bytes, 0x12c) & 2) && !(get(reference[igA2].bytes, 0x12c) & 2), "native: A, B, C, gA2 culled");
    check((get(reference[iE].bytes, 0x12c) & 2) && get(reference[iE].bytes, 0x14c) == 2 && (get(reference[iF].bytes, 0x12c) & 2) && get(reference[iF].bytes, 0x14c) == 3 && (get(reference[iF].bytes, 0x130) & 0x100000), "native: E LOD 2, F LOD 3 with the far flag");
    check((get(reference[iC].bytes, 0x130) & 0x100000) && get(reference[iC].bytes, 0x14c) == 2, "native: C detail flag and LOD 2 before the fade");
    check(!(get(reference[iG].bytes, 0x12c) & 2) && !(get(reference[iH].bytes, 0x12c) & 2) && (get(reference[iI].bytes, 0x12c) & 2) && (get(reference[iJ].bytes, 0x12c) & 2) && (get(reference[igA1].bytes, 0x12c) & 2), "native: G, H early exits, I, J and gA1 kept");

    // ---- option off / engine site absent / changed bytes ----
    SetEnvironmentVariableW(L"X3M_CULL_CENSUS", nullptr);
    check(!census::initialize() && !std::strcmp(census::state(), "disabled") && sites_original(), "unset variable: disabled, sites untouched");
    ...
    check(!census::initialize() && !std::strcmp(census::state(), "bytes_mismatch") && census::measure_stub_address() == 0, "engine sites absent in this process: bytes_mismatch, nothing patched");
    synthetic_measure_window[3] ^= 1;
    check(!census::install_at(addr(site_a()), addr(site_b())) && !std::strcmp(census::state(), "bytes_mismatch") && sites_original(), "changed measure-window byte: bytes_mismatch");
    synthetic_measure_window[3] ^= 1;
    synthetic_exit_window[1] ^= 1;
    check(!census::install_at(addr(site_a()), addr(site_b())) && !std::strcmp(census::state(), "bytes_mismatch") && sites_original(), "changed exit-window byte: bytes_mismatch, measure site untouched");
    synthetic_exit_window[1] ^= 1;
    check(!census::install_at(0, addr(site_b())) && !std::strcmp(census::state(), "invalid_site"), "null site refused");

    // ---- install ----
    check(census::install_at(addr(site_a()), addr(site_b())) && !std::strcmp(census::state(), "ok"), "install_at synthetic sites");
    if (!census::measure_stub_address()) { std::printf("FAIL install state=%s\nCULL CENSUS CPU checks=%u failures=%u\n", census::state(), checks, failures + 1); return 1; }
    check(site_a()[0] == 0xe9 && site_a()[5] == core::measure_site[5] && site_b()[0] == 0xe9 && site_b()[5] == core::exit_site[5], "both sites are jmp dispatcher with the sixth byte untouched");
    check(!std::memcmp(synthetic_measure_window, core::measure_window, core::measure_site_offset) && !std::memcmp(synthetic_exit_window, core::exit_window, core::exit_site_offset), "window bytes before the sites untouched");
    {
        const std::uint32_t at = std::uint32_t(census::measure_stub_address()), slot = (at + core::measure_stub_length + 3) & ~3u;
        unsigned char want[core::measure_stub_length]; core::encode_measure_stub(at, addr(const_cast<unsigned char*>(&x3m_cull_census_enabled)), addr(reinterpret_cast<const void*>(&x3m_cull_census_measure)), slot, want);
        check(!std::memcmp(reinterpret_cast<const void*>(at), want, core::measure_stub_length), "measure stub bytes as encoded");
        const std::uint32_t bt = std::uint32_t(census::exit_stub_address()), bslot = (bt + core::exit_stub_length + 3) & ~3u;
        unsigned char bwant[core::exit_stub_length]; core::encode_exit_stub(bt, addr(const_cast<unsigned char*>(&x3m_cull_census_enabled)), addr(reinterpret_cast<const void*>(&x3m_cull_census_exit)), bslot, bwant);
        check(!std::memcmp(reinterpret_cast<const void*>(bt), bwant, core::exit_stub_length), "exit stub bytes as encoded");
        check(*reinterpret_cast<void**>(slot) != nullptr && *reinterpret_cast<void**>(bslot) != nullptr, "continuation slots point at the tails");
    }
    check(!census::install_at(addr(site_a()), addr(site_b())) && !std::strcmp(census::state(), "already_installed"), "second install refused");
    ...
    check(patched.preserved && patched.x87_empty && same_outputs(patched, native), "patched disarmed: registers, ESP, x87 and EAX/ECX/EDX/EFLAGS as native");
    ...
    check(patched.preserved && patched.x87_empty && same_outputs(patched, native), "patched armed: registers, ESP, x87 and EAX/ECX/EDX/EFLAGS as native");
    identical = true; for (unsigned i = 0; i < all_count; ++i) identical = identical && !std::memcmp(all[i]->bytes, reference[i].bytes, sizeof(Node));
    check(identical, "patched armed: every node as native");
    census::Stats s = census::stats();
    check(s.entries == 10 && s.exited == 10 && s.unmeasured == 2 && s.overflow == 0, "armed: 10 measured and exited, 2 early exits unmeasured, no overflow");
    ...
    {
        unsigned long device = 0, frame = 0, entries = 0, overflow = 0, unmeasured = 0, exited = 0; unsigned ring = 0;
        const int fields = frame_lines.empty() ? 0 : std::sscanf(frame_lines[0].c_str(), "cull_census_frame device=%lu frame=%lu entries=%lu overflow=%lu unmeasured=%lu exited=%lu ring=%u", &device, &frame, &entries, &overflow, &unmeasured, &exited, &ring);
        check(fields == 7 && device == 7 && frame == 41 && entries == 10 && overflow == 0 && unmeasured == 2 && exited == 10 && ring == core::ring_size, "frame row fields");
        ...
        const unsigned long order[10] = {addr(&R), addr(&A), addr(&gA1), addr(&gA2), addr(&B), addr(&C), addr(&E), addr(&F), addr(&I), addr(&J)};
        bool ordered = rows.size() == 10; for (unsigned i = 0; ordered && i < 10; ++i) ordered = rows[i].node == order[i];
        check(ordered, "rows in traversal order (parent before children, siblings in list order)");
        check(rows.size() == 10 && rows[0].view == addr(&view) && rows[0].device == 7 && rows[0].frame == 41, "row carries the view, device and frame");
        check(row_matches(row_of(rows, R), R, reference[iR], expected(R, view, nullptr), "kept"), "R row: s 128, measure 256, kept, LOD 0");
        check(row_matches(row_of(rows, A), A, reference[iA], expected(A, view, &R), "culled_size"), "A row: measure 1 below limit 4, culled_size");
        check(row_matches(row_of(rows, B), B, reference[iB], expected(B, view, &R), "culled_min"), "B row: measure 0, culled_min");
        check(row_matches(row_of(rows, C), C, reference[iC], expected(C, view, &R), "culled_other"), "C row: last-LOD fade, culled_other, LOD 2");
        ...
        check(row_matches(row_of(rows, I), I, reference[iI], expected(I, view, &R), "kept"), "I row: D below 640 saturates s and measure");
        check(row_matches(row_of(rows, J), J, reference[iJ], expected(J, view, &R), "kept"), "J row: measure 10, kept, LOD 0");
        check(row_matches(row_of(rows, gA1), gA1, reference[igA1], expected(gA1, view, &A), "kept"), "gA1 row: parent threshold 4 is the limit, kept");
        check(row_matches(row_of(rows, gA2), gA2, reference[igA2], expected(gA2, view, &A), "culled_size"), "gA2 row: own threshold 8 is the limit, culled_size");
        const Row* ri = row_of(rows, I); check(ri && ri->s == 0x7000000 && ri->measure == 0x7000000, "I row saturated values");
    ...
    {
        View env; view_set(env, 1280, 0x1000000, 0x4000, 2);
        reset_tree();
        const Result nat = run(R, env);
        static Node env_reference[all_count]; for (unsigned i = 0; i < all_count; ++i) env_reference[i] = *all[i];
        reset_tree(); census::begin_frame(true);
        const Result pat = run(R, env);
        bool same = pat.preserved && pat.x87_empty && same_outputs(pat, nat); for (unsigned i = 0; i < all_count; ++i) same = same && !std::memcmp(all[i]->bytes, env_reference[i].bytes, sizeof(Node));
    ...
    {
        View wide; view_set(wide, 1280, 0, 0x4000, 4);
        reset_tree(); run(R, wide);
        static Node far_reference[all_count]; for (unsigned i = 0; i < all_count; ++i) far_reference[i] = *all[i];
        reset_tree(); census::begin_frame(true); run(R, wide);
        bool same = true; for (unsigned i = 0; i < all_count; ++i) same = same && !std::memcmp(all[i]->bytes, far_reference[i].bytes, sizeof(Node));
    ...
    // ---- overflow: 8,200 measured nodes keep 8,192 rows and count the rest ----
    {
        ...
        auto* pool = static_cast<Node*>(VirtualAlloc(nullptr, sizeof(Node) * (many + 1), MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE));
        check(pool != nullptr, "overflow pool");
        Node& root = pool[0];
        node_set(root, nullptr, 20000, 100000, 0x1002, 0, 0, 0x6000);
        std::vector<Node*> kids; kids.reserve(many);
        for (unsigned i = 1; i <= many; ++i) { node_set(pool[i], &root, 1000, 100000, 0x1002, 0, 0, 0x6000 + i); kids.push_back(&pool[i]); }
        ...
        s = census::stats();
        check(s.entries == core::ring_size && s.overflow == many + 1 - core::ring_size && s.unmeasured == 0 && s.exited == core::ring_size, "overflow: ring full, 9 overflow, every kept row exited");
        bool all_kept = true; for (unsigned i = 0; i <= many; ++i) all_kept = all_kept && (get(pool[i].bytes, 0x12c) & 2);
        check(all_kept, "overflow: the pass itself unaffected (every node kept)");
        census::present(7, 44, true);
        unsigned long device = 0, frame = 0, entries = 0, overflow = 0, unmeasured = 0, exited = 0; unsigned ring = 0;
        const int fields = frame_lines.empty() ? 0 : std::sscanf(frame_lines[0].c_str(), "cull_census_frame device=%lu frame=%lu entries=%lu overflow=%lu unmeasured=%lu exited=%lu ring=%u", &device, &frame, &entries, &overflow, &unmeasured, &exited, &ring);
        check(fields == 7 && entries == core::ring_size && overflow == 9 && entry_lines.size() == core::ring_size, "overflow: frame row overflow=9, 8192 entry rows");
        Row last{}; check(!entry_lines.empty() && parse_row(entry_lines.back(), last) && last.node == addr(&pool[core::ring_size - 1]) && !std::strcmp(last.verdict, "kept"), "overflow: the last row is the 8,192nd node, exited");
        frame_lines.clear(); entry_lines.clear();
    ...
    std::printf("CULL CENSUS BENCH native_pass_us=%.4f patched_disarmed_us=%.4f patched_armed_us=%.4f nodes_per_pass=12 measured_per_pass=10 harness=fixture_call_included game_fps=unmeasured\n",
                native_us, disarmed_us, armed_us);
    // ---- re-install, restore, then the closed window refuses ----
    check(census::install_at(addr(site_a()), addr(site_b())) && census::shutdown() && sites_original(), "re-install and restore");
    x3m::engine_patch::close_install_window("fixture");
    check(!census::install_at(addr(site_a()), addr(site_b())) && !std::strcmp(census::state(), "late_claim") && sites_original(), "closed install window: late_claim, sites untouched");
