...
int decide_command(char** a) {
    ProbeInputs in;
    in.ready = number(a[0]) != 0;
    in.record_owner = std::uintptr_t(number(a[1]));
    in.view = std::uintptr_t(number(a[2]));
    in.main_view = std::uintptr_t(number(a[3]));
    in.owner_flags = std::uint32_t(std::strtoul(a[4], nullptr, 0));
    in.video_flags = std::uint32_t(std::strtoul(a[5], nullptr, 0));
    in.view_flags = std::uint32_t(std::strtoul(a[6], nullptr, 0));
    in.x = std::int32_t(number(a[7]));
    in.y = std::int32_t(number(a[8]));
    in.top = std::int32_t(number(a[9]));
    in.bottom = std::int32_t(number(a[10]));
    in.left = std::int32_t(number(a[11]));
    in.right = std::int32_t(number(a[12]));
    std::printf("decision=%d eligible=%d\n", int(decide(in)),
                int(eligible(in.record_owner, in.view, in.main_view, in.owner_flags)));
    return 0;
}
...
int ready_command(int count, char** a) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (c == 'b')
            ...
            answers += state.probe(std::uintptr_t(number(a[i] + 1)), true) ? '1' : '0';
        else if (c == 'd')
            answers += state.probe(std::uintptr_t(number(a[i] + 1)), false) ? '1' : '0';
    }
...
int footprint_command(char** a) {
    Latch l;
    l.valid = true;
    l.x = std::int32_t(number(a[0]));
    l.y = std::int32_t(number(a[1]));
    l.size = std::int32_t(number(a[2]));
    l.accumulator = std::int32_t(number(a[3]));
    l.fov = std::uint32_t(number(a[4]));
    l.scale_x = std::int32_t(number(a[5]));
    l.top = l.left = 0;
    l.bottom = l.right = 0x10000;
    const Footprint f = footprint(l, unsigned(number(a[6])), unsigned(number(a[7])), float(std::atof(a[8])));
    std::printf("valid=%d known=%d saturated=%d u=%.7f v=%.7f ru=%.7f rv=%.7f\n", int(f.valid), int(f.radius_known),
                int(f.saturated), f.u, f.v, f.radius_u, f.radius_v);
    return 0;
}
// blend <enable> <src> <dst> <op> <srgb> <alphatest> <ref> <func> <fog>
int blend_command(char** a) {
    BlendState s;
    std::uint32_t* fields[] = {&s.enable,     &s.src,       &s.dst,        &s.op, &s.srgb_write,
                               &s.alpha_test, &s.alpha_ref, &s.alpha_func, &s.fog};
...
int view_command(const char* name) {
    ...
    const std::uint32_t registry = 0x1000, header = 0x2000, buckets = 0x3000, link_a = 0x4000, link_b = 0x4100,
                        cockpit = 0x5000, camera = 0x6000, handle = 0x2a;
    memory[cockpit_registry_va] = registry;
    memory[registry] = header;
    memory[registry + 0x10] = handle;
    memory[header] = buckets;
    memory[header + 4] = 8;
    memory[buckets + 4 * (7 & handle)] = link_a;
    memory[link_a] = link_b;
    memory[link_a + 4] = 0x99;
    memory[link_a + 8] = 0x7777;
    memory[link_b] = 0;
    memory[link_b + 4] = handle;
    memory[link_b + 8] = cockpit;
    memory[cockpit + 0x58] = camera;
    memory[camera + view_flags] = view_flag_sector_camera | 1;
    const std::string which = name;
    if (which == "no_marker")
        memory[camera + view_flags] = 1;
    else if (which == "no_handle")
        memory[registry + 0x10] = 0;
    else if (which == "bad_count")
        memory[header + 4] = 6;
    else if (which == "missing")
        memory[link_b + 4] = 0x55;
    else if (which == "cycle")
        memory[link_b] = link_a, memory[link_b + 4] = 0x55;
    else if (which == "misaligned")
        memory[cockpit + 0x58] = camera + 2;
    else if (which == "unreadable")
        memory.erase(camera + view_flags);
...
int variant_command(int count, char** a) {
    ...
    const auto result = lens_visibility_pixel_variant(words.data(), words.size(), LensVisibilityScale(number(a[0])),
                                                      out, &layout);
    std::printf("result=%s sampler=%u constant=%u output=%u fetch=%u words=", lens_visibility_result_name(result),
                layout.sampler, layout.constant, layout.output_temporary, layout.fetch_temporary);
...
int clip_variant_command(int count, char** a) {
    ...
    LensVisibilityLayout layout{};
    const auto result = lens_visibility_pixel_clip_variant(
        words.data(), words.size(), LensVisibilityScale(number(a[0])), unsigned(number(a[1])), float(std::atof(a[2])),
        float(std::atof(a[3])), number(a[4]) != 0, out, &layout);
    std::printf("result=%s sampler=%u depth_sampler=%u constant=%u output=%u fetch=%u words=",
                lens_visibility_result_name(result), layout.sampler, layout.depth_sampler, layout.constant,
                layout.output_temporary, layout.fetch_temporary);
    for (std::size_t i = 0; i < out.size(); ++i) std::printf("%s%08x", i ? "," : "", out[i]);
    std::printf("\n");
    return 0;
}
...
int vertex_variant_command(int count, char** a) {
    ...
    unsigned matrix = 999;
    bool origin = false;
    const auto result = lens_visibility_vertex_variant(words.data(), words.size(), unsigned(number(a[0])), out, &matrix,
                                                       &origin);
...
int texcoord_command(int count, char** a) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        (second ? ps : vs).push_back(std::uint32_t(std::strtoul(a[i], nullptr, 16)));
    }
...
int body_command(char** a) {
    ...
    const BodyCentre c = classify_body(rows, number(a[0]) != 0, float(std::atof(a[1])), float(std::atof(a[2])),
                                       float(std::atof(a[3])));
...
int main(int argc, char** argv) {
    ...
    if (command == "saturated" && argc == 4) {
        std::printf("saturated=%d\n",
                    int(size_saturated(std::int32_t(number(argv[2])), std::int32_t(number(argv[3])))));
        return 0;
    }
    ...
    if (command == "alpha" && argc == 3) {
        std::printf("alpha=%.7f\n", smoothing_alpha(std::atof(argv[2])));
        return 0;
    }
    // jitter <active> <jx_px> <jy_px> <width> <height>: the visibility pass's RT2 read offset in uv.
    if (command == "jitter" && argc == 7) {
        const JitterUv j = jitter_uv(number(argv[2]) != 0, float(std::atof(argv[3])), float(std::atof(argv[4])),
                                     unsigned(number(argv[5])), unsigned(number(argv[6])));
        std::printf("u=%.8f v=%.8f\n", j.u, j.v);
        return 0;
    }
    if (command == "sites") {
        std::printf(
            "probe_site=0x%lx probe_target=0x%lx lens_site=0x%lx lens_target=0x%lx gates=0x%llx gates_length=%u\n",
            (unsigned long)probe_site_va, (unsigned long)probe_target_va, (unsigned long)lens_site_va,
            (unsigned long)lens_target_va, (unsigned long long)probe_gates_fnv1a, probe_gates_length);
        return 0;
    }
    return 2;
}
