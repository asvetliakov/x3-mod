...
static std::string last_probe_line; static unsigned probe_lines = 0;
namespace x3m {
void log(const char* format, ...) { // self-preserving, as the production log()
    std::va_list a; va_start(a, format);
    call_preserved([&] { char text[768]; std::vsnprintf(text, sizeof text, format, a);
        if (!std::strncmp(text, "sun_probe ", 10)) { last_probe_line = text; ++probe_lines; } else std::printf("%s\n", text); });
    va_end(a);
}
namespace object_trace { bool executable_verified() { return true; } }
}
extern "C" {
std::uint32_t fx_out[8], fx_esp_before, fx_lens_out[8], fx_lens_esp_before, fx_pad, fx_df, fx_flags, fx_lens_flags;
unsigned fx_probe_calls, fx_lens_calls; int fx_probe_answer; const void* fx_probe_args[2]; const void* fx_probe_return; const void* fx_lens_arg;
bool fx_bracket_inside; unsigned fx_order, fx_begin_at, fx_original_at, fx_end_at; std::uintptr_t fx_lens_stack;
...
int __cdecl fx_probe_original(const void* record, const void* view) {
    ++fx_probe_calls; fx_probe_args[0] = record; fx_probe_args[1] = view; fx_probe_return = __builtin_return_address(0);
    return fx_probe_answer;
}
int __cdecl fx_other_target(const void*, const void*) { return 7; }
int __cdecl fx_lens_original(const void* view) {
    ++fx_lens_calls; fx_lens_arg = view; fx_bracket_inside = so::bracket_open(); fx_original_at = ++fx_order;
...
void listener_begin() {
    ++begins; fx_begin_at = ++fx_order; listener_df += direction_flag();
    if (dirty_listener) { volatile double x = 3.0; asm volatile("fldl %0\n\tfsqrt\n\tfstp %%st(0)" :: "m"(x) : "memory"); unsigned m = 0x7f80; asm volatile("ldmxcsr %0" :: "m"(m) : "memory"); SetLastError(5); }
}
void listener_end() { ++ends; fx_end_at = ++fx_order; listener_df += direction_flag(); if (dirty_listener) SetLastError(6); }
...
bool probe(const std::uint32_t* record, const std::uint32_t* view, int expected_eax, unsigned expected_original_calls) {
    ...
    unsigned mxcsr = 0x7f80 | 0x6000, mxcsr_after = 0; double a = 1.25, b = 2.5, ra = 0, rb = 0;
    ...
    unsigned restore = 0x1f80; asm volatile("ldmxcsr %0" :: "m"(restore) : "memory");
    bool ok = int(fx_out[0]) == expected_eax && fx_probe_calls - before == expected_original_calls;
    ok = ok && fx_out[1] == 0x11111111 && fx_out[2] == 0x22222222 && fx_out[3] == std::uint32_t(reinterpret_cast<std::uintptr_t>(record)) && fx_out[4] == 0x44444444;
    ok = ok && fx_out[5] == fx_esp_before && fx_out[6] == std::uint32_t(reinterpret_cast<std::uintptr_t>(record)) && fx_out[7] == std::uint32_t(reinterpret_cast<std::uintptr_t>(view));
    ok = ok && error == 0x1234 && mxcsr_after == mxcsr && ra == a && rb == b && ((fx_flags & 0x400u) != 0) == (fx_df != 0);
...
bool lens(const std::uint32_t* view, unsigned expected_begins, bool expected_inside) {
    ...
    unsigned mxcsr = 0x7f80 | 0x2000, mxcsr_after = 0; double a = 7.5, ra = 0;
    ...
    unsigned restore = 0x1f80; asm volatile("ldmxcsr %0" :: "m"(restore) : "memory");
    bool ok = fx_lens_calls - calls == 1 && fx_lens_arg == view && begins - begins_before == expected_begins && ends - ends_before == expected_begins && fx_bracket_inside == expected_inside;
    ok = ok && fx_lens_out[1] == 0x11111111 && fx_lens_out[2] == 0x22222222 && fx_lens_out[3] == std::uint32_t(reinterpret_cast<std::uintptr_t>(view)) && fx_lens_out[4] == 0x44444444;
    ok = ok && fx_lens_out[5] == fx_lens_esp_before && fx_lens_out[6] == std::uint32_t(reinterpret_cast<std::uintptr_t>(view)) && !so::bracket_open();
    ok = ok && error == 0x4321 && mxcsr_after == mxcsr && ra == a;
    if (expected_begins) ok = ok && fx_begin_at < fx_original_at && fx_original_at < fx_end_at;
    return ok;
}
DWORD WINAPI foreign_thread(void*) { fx_probe_answer = 1; fx_probe_caller(record_block, view_block); return 0; }
so::Addresses addresses() {
    so::Addresses a; a.probe_site = reinterpret_cast<std::uintptr_t>(fx_probe_site); a.probe_target = reinterpret_cast<std::uintptr_t>(&fx_probe_original);
    a.lens_site = reinterpret_cast<std::uintptr_t>(fx_lens_site); a.lens_target = reinterpret_cast<std::uintptr_t>(&fx_lens_original);
    a.config_global = reinterpret_cast<std::uintptr_t>(&config_pointer); a.main_view = &main_view_value;
    return a;
}
...
int main() {
    ...
    { so::Addresses bad = addresses(); bad.probe_target = reinterpret_cast<std::uintptr_t>(&fx_other_target);
      check("rollback_refused", !so::install_at(bad, true, false) && !std::strcmp(so::state(), "target_mismatch") && !so::installed());
      check("rollback_bytes", !std::memcmp(probe_before, fx_probe_site, 5) && !std::memcmp(lens_before, fx_lens_site, 5)); }
    check("install", so::install_at(addresses(), true, false) && so::installed() && so::override_enabled() && !so::logging());
    { std::int32_t rel = 0; std::memcpy(&rel, fx_probe_site + 1, 4);
      check("probe_redirect", fx_probe_site[0] == 0xe8 && fx_probe_site + 5 + rel == reinterpret_cast<unsigned char*>(&x3m_sun_probe_thunk));
      std::memcpy(&rel, fx_lens_site + 1, 4);
      check("lens_redirect", fx_lens_site[0] == 0xe8 && fx_lens_site + 5 + rel == reinterpret_cast<unsigned char*>(&x3m_sun_lens_thunk)); }
    ...
    fx_probe_answer = 1; frame(record_block, 1, 1, "frame1_original_hidden");
    check("original_return_address", fx_probe_return == fx_probe_site + 5);
    check("latch", so::frame_inputs().single && so::frame_inputs().latch.record == reinterpret_cast<std::uintptr_t>(record_block) && so::frame_inputs().latch.x == 0x2000 &&
                   so::frame_inputs().latch.y == -0x1000 && so::frame_inputs().latch.size == 655 && so::frame_inputs().latch.accumulator == 200 && so::frame_inputs().latch.fov == 0x2aaa &&
                   so::frame_inputs().latch.scale_x == 0x10000 && !so::frame_inputs().answered);
    ...
    dirty_listener = true; check("lens_bracket", lens(view_block, 1, true)); dirty_listener = false;
    // Gates, vanilla order.
    config_block[core::config_video_flags / 4] = 0; frame(record_block, 1, 0, "gate1_flares_off_hidden"); config_block[core::config_video_flags / 4] = core::video_flag_flares;
    ...
    so::device_reset(); frame(record_block, 1, 1, "reset_frame_original"); frame(record_block, 0, 0, "after_reset_ready_again");
    so::device_reset(); frame(record_block, 1, 1, "reset_drops_readiness");
    // A frame without any probe leaves no previous frame behind.
    frame(record_block, 0, 0, "ready_before_gap"); so::present(); so::present(); frame(record_block, 1, 1, "gap_original");
    // Main view unknown.
    frame(record_block, 0, 0, "ready_before_unknown_main"); main_view_value = 0; frame(record_block, 1, 1, "unknown_main_original"); main_view_value = reinterpret_cast<std::uintptr_t>(view_block);
    // The 4-byte stack contract and a hostile direction flag: every alignment of ESP at the site, DF clear and set,
    // on the override path, the original's path and the bracket. The handlers run with DF clear; the site gets its flags back.
    fx_probe_answer = 1; frame(record_block, 1, 1, "warm_before_abi_matrix");
    { bool ok = true; unsigned handlers_df = 0;
      for (unsigned pad = 0; pad < 16; pad += 4) for (unsigned df = 0; df < 2; ++df) {
          fx_pad = pad; fx_df = df; fx_probe_answer = 1;
          so::present(); ok = probe(record_block, view_block, 0, 0) && ok; so::report_pass(true);
          ok = probe(foreign_record, view_block, 1, 1) && ok;
          listener_df = 0; ok = lens(view_block, 1, true) && ok; handlers_df += listener_df;
      }
      fx_pad = 0; fx_df = 0;
      check("stack_alignments_and_direction_flag", ok && handlers_df == 0); }
    // present() closes a bracket an unwind left open.
    x3m_sun_lens_begin(); check("bracket_left_open", so::bracket_open()); so::present(); check("present_closes_bracket", !so::bracket_open());
    // Another thread: straight to the original, counted.
    { const unsigned before = fx_probe_calls; HANDLE t = CreateThread(nullptr, 0, &foreign_thread, nullptr, 0, nullptr); WaitForSingleObject(t, INFINITE); CloseHandle(t);
    ...
    fx_probe_answer = 1; frame(record_block, 1, 1, "warm_before_block"); frame(record_block, 0, 0, "ready_before_block");
    so::block("fixture"); frame(record_block, 1, 1, "blocked_original"); frame(record_block, 1, 1, "blocked_stays");
    so::device_reset(); frame(record_block, 1, 1, "blocked_reset_frame"); frame(record_block, 1, 1, "blocked_survives_reset");
    check("blocked_counted", so::counters().blocked == 1);
    const so::Counters totals = so::counters();
    std::printf("COUNTERS probes=%u own=%u visible=%u hidden=%u original=%u brackets=%u multi=%u blocked=%u\n", totals.probes, totals.own, totals.answered_visible, totals.answered_hidden,
                totals.original, totals.brackets, totals.multi_record_frames, totals.blocked);
    check("counters", totals.answered_hidden == 2 && totals.multi_record_frames == 1 && totals.blocked == 1 && totals.probes == totals.answered_visible + totals.answered_hidden + totals.original);

    // Restore, then the log mode: the original runs exactly once per probe; an overridden answer stays the override's.
    check("shutdown", so::shutdown() && !so::installed() && !std::memcmp(probe_before, fx_probe_site, 5) && !std::memcmp(lens_before, fx_lens_site, 5));
    ...
    frame(record_block, 1, 1, "observe_frame1"); frame(record_block, 1, 1, "observe_frame2_still_original");
    ...
    { so::Addresses production = addresses(); production.main_view = nullptr;
    ...
    check("late_claim", !so::install_at(addresses(), true, false) && !std::strcmp(so::state(), "late_claim") && !std::memcmp(probe_before, fx_probe_site, 5) && !std::memcmp(lens_before, fx_lens_site, 5));
