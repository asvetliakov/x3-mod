...
int main() {
    ...
    for (std::uint32_t flags = 0; flags < 512; ++flags) {
        ...
        for (std::uint32_t id :
             {0u, 1u, 3u, 101u, 144u, 244u, 800u, 810u, 811u, 812u, 2004u, 8404u, 8509u, 10001u, 0xffffffffu})
    ...
    check(selector_scope(selector, 0x5a, game) && !selector_scope(selector, 0x11, game) &&
          !selector_scope(speech, 0x5a, game));
    ...
    check(cache.refuses(812, now + 1, retry) && cache.refuses(812, now + retry - 1, retry) &&
          cache.find(812)->refusals == 2);
    ...
    check(cache.used == cache_entries && cache.evictions == 1 && !cache.find(1000) && cache.find(1001) &&
          cache.find(2000));
    ...
    p.id = 4;
    pending.push(p);
    check(pending.pop(0x1000, out) && out.id == 3 && pending.depth == 0 && pending.stale == 1); // the inner entry was
                                                                                                // unwound past its
                                                                                                // return
    ...
    p.id = 5;
    p.ret = 0xa1;
    check(pending.push(p) && pending.last_return == 0xa1);
    p.esp = 0x1000;
    p.id = 6;
    p.ret = 0xa2;
    check(pending.push(p) && pending.depth == 1 && pending.stale == 1 && pending.items[0].id == 6 &&
          pending.last_return == 0xa2);
    p.esp = 0x0ff0;
    p.id = 7;
    check(pending.push(p) && pending.depth == 2); // a nested call keeps the outer entry
    p.esp = 0x1010;
    p.id = 8;
    check(pending.push(p) && pending.depth == 1 && pending.stale == 3 && pending.items[0].id == 8);
    for (unsigned i = 0; i < 100; ++i) {
        p.esp = 0x1010;
        check(pending.push(p) && pending.depth == 1);
    } // repeated unwinds at one frame never exhaust the slots
    check(pending.stale == 103);
    pending = {};
    for (unsigned i = 0; i < pending_depth; ++i) {
        p.esp = 0x3000 - 0x10 * i;
        check(pending.push(p));
    }
    p.esp = 0x2000;
    ...
    for (unsigned f = 1; f <= 10; ++f) {
        ...
        for (unsigned a = 0; a < (f <= 5 ? 1u : 3u); ++a)
            window.entry(entry(812, selector, f == 1 ? observed : refused, 0, f));
    }
    ...
    check(window.close(s));
    check(s.frame == 10 && s.frames == 10 && s.attempts == 20 + 2 + 9 && s.failures == 1 && s.refused == 19 &&
          s.successes == 10 && s.unobserved == 1);
    check(s.attempts_frame_p50 == 3 && s.attempts_frame_max == 3);
    check(s.id_count == id_slots && s.ids[0].id == 812 && s.ids[0].count == 20 && s.ids[1].id == 245 &&
          s.ids[2].id == 300 && s.id_overflow == 4);
    check(!window.close(s));
    for (unsigned f = 0; f < window_frames; ++f) window.frame(f, 0);
    check(window.full() && window.close(s) && s.frames == window_frames && s.attempts == 0 &&
          s.attempts_frame_p50 == 0 && s.id_count == 0);
    // Video blit witness: the consumer's return range and the line cadence.
    Addresses video_game = game;
    video_game.blit_begin = 0x004d0c40;
    video_game.blit_end = 0x004d14e0;
    check(video_blit_caller(0x004d0d27, video_game) && video_blit_caller(0x004d14ba, video_game) &&
          video_blit_caller(0x004d0c40, video_game) && video_blit_caller(0x004d14df, video_game));
    check(!video_blit_caller(0x004d14e0, video_game) && !video_blit_caller(0x004d0c3f, video_game) &&
          !video_blit_caller(0x00401000, video_game));
    check(!video_blit_caller(0x004d0d27, game) && !video_blit_caller(0, game)); // an empty range admits nothing
    VideoBlit video;
    check(video.lock_enter() && video.lock_result(false) && !video.lock_result(false) && video.locks == 1 &&
          video.locks_total == 1 && video.depth == 0);
    unsigned lines = 0;
    for (unsigned i = 1; i < 3 * video_blit_line_interval; ++i) {
        const bool line = video.lock_enter();
        lines += line;
        check(video.lock_result(i == 7) == line);
    }
    check(lines == 2 && video.locks == 3 * video_blit_line_interval && video.failures == 1);
    check(video.unlock_enter() && video.unlock_result(true) && !video.unlock_enter() && !video.unlock_result(false) &&
          video.unlocks == 2 && video.failures == 2);
    ...
    check(!video.lock_enter() && video.reentries == 1 && video.depth == 2 &&
          video.locks == 3 * video_blit_line_interval + 1);
    ...
    for (unsigned i = 0; i < video_blit_line_interval - 1; ++i) {
        check(!video.lock_enter());
        check(!video.lock_result(false));
    } // blits 182..240
    check(video.lock_enter() && video.depth == 1 && video.locks_total == 4 * video_blit_line_interval + 1); // blit 241
                                                                                                            // is on the
                                                                                                            // interval
    check(!video.unlock_enter() && video.reentries == 2 && !video.unlock_result(false) && video.depth == 1); // a nested
                                                                                                             // unlock
                                                                                                             // does not
                                                                                                             // steal
                                                                                                             // the
    ...
    video.close();
    check(video.locks == 0 && video.unlocks == 0 && video.failures == 0 && video.reentries == 0 &&
          video.locks_total == 4 * video_blit_line_interval + 1 && video.unlock_written);
    refuse_allocation = false;
    std::printf(
        "media_cue_host checks=%u failures=0 cache_bytes=%zu pending_bytes=%zu ring_bytes=%zu window_bytes=%zu cache_entries=%u pending_depth=%u lines_per_second=%u\n",
        checks, sizeof cache, sizeof pending, sizeof ring, sizeof window, cache_entries, pending_depth,
        lines_per_second);
    return 0;
}
