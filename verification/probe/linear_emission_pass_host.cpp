...
struct Device : IDirect3DDevice9 {
  ...
  bool fault(unsigned slot) {
    return int(slot) == fault_slot && ++fault_calls == fault_at;
  }
  ...
  bool supported(D3DRENDERSTATETYPE s) {
    if (s == D3DRS_COLORWRITEENABLE)
      return caps.PrimitiveMiscCaps & D3DPMISCCAPS_COLORWRITEENABLE;
    if (s == D3DRS_SEPARATEALPHABLENDENABLE)
  ...
  static Device &d(IDirect3DDevice9 *p) {
    ...
    if (device.device_calls_forbidden)
      ++device.forbidden_device_accesses;
    return device;
  }
  template <class F> void slot(unsigned n, F f) {
    slots[n] = reinterpret_cast<void *>(+f);
  }
  Device() {
    slot(6, [](IDirect3DDevice9 *p, IDirect3D9 **o) -> HRESULT {
      return d(p).output(6, &d(p).factory, o);
    });
    slot(9,
         [](IDirect3DDevice9 *p, D3DDEVICE_CREATION_PARAMETERS *o) -> HRESULT {
           (void)d(p);
           *o = {};
           return S_OK;
         });
    slot(18,
         [](IDirect3DDevice9 *p, UINT, UINT, D3DBACKBUFFER_TYPE,
            IDirect3DSurface9 **o) -> HRESULT {
           return d(p).output(18, d(p).back, o);
         });
    slot(23,
         [](IDirect3DDevice9 *p, UINT w, UINT h, UINT, DWORD, D3DFORMAT,
            D3DPOOL, IDirect3DTexture9 **o, HANDLE *) -> HRESULT {
           auto &a = d(p);
           ...
           if (a.alias_pool && a.first_pool)
             s->identity = a.first_pool;
    ...
    slot(37, [](IDirect3DDevice9 *p, DWORD i, IDirect3DSurface9 *v) -> HRESULT {
      ...
      if (i == 0 && v == a.back &&
          (a.rt[1] || a.rt[2] ||
           (v->desc.Format == D3DFMT_A8R8G8B8 && a.depth))) {
        ++a.bad_back_binds;
        return E_FAIL;
      }
      bind(a.rt[i], v);
    ...
    slot(38,
         [](IDirect3DDevice9 *p, DWORD i, IDirect3DSurface9 **o) -> HRESULT {
           auto &a = d(p);
           if (i >= a.caps.NumSimultaneousRTs) {
             ++a.bad_rt_calls;
             *o = nullptr;
    ...
    slot(43,
         [](IDirect3DDevice9 *p, DWORD, const D3DRECT *, DWORD, D3DCOLOR, float,
            DWORD) -> HRESULT {
           ++d(p).clears;
           return S_OK;
         });
    ...
    slot(58,
         [](IDirect3DDevice9 *p, D3DRENDERSTATETYPE s, DWORD *v) -> HRESULT {
           auto &a = d(p);
           if (s == D3DRS_COLORWRITEENABLE3)
             ++a.rs3_calls;
           if (!a.supported(s)) {
    ...
    slot(86,
         [](IDirect3DDevice9 *p, const D3DVERTEXELEMENT9 *,
            IDirect3DVertexDeclaration9 **o) -> HRESULT {
           return d(p).output(86, d(p).make<IDirect3DVertexDeclaration9>(), o);
         });
    slot(87,
         [](IDirect3DDevice9 *p, IDirect3DVertexDeclaration9 *v) -> HRESULT {
           bind(d(p).decl, v);
           return S_OK;
         });
    slot(88,
         [](IDirect3DDevice9 *p, IDirect3DVertexDeclaration9 **o) -> HRESULT {
    ...
    slot(91,
         [](IDirect3DDevice9 *p, const DWORD *,
            IDirect3DVertexShader9 **o) -> HRESULT {
    ...
    slot(93, [](IDirect3DDevice9 *p, IDirect3DVertexShader9 **o) -> HRESULT {
      return d(p).output(93, d(p).vs, o);
    });
    slot(100,
         [](IDirect3DDevice9 *p, UINT, IDirect3DVertexBuffer9 *v, UINT o,
            UINT s) -> HRESULT {
           auto &a = d(p);
           bind(a.stream, v);
           a.offset = o;
    ...
    slot(108, [](IDirect3DDevice9 *p, IDirect3DPixelShader9 **o) -> HRESULT {
      return d(p).output(108, d(p).ps, o);
    });
    // No production path may submit the application's source draw.
    slot(82,
         [](IDirect3DDevice9 *p, D3DPRIMITIVETYPE, int, UINT, UINT, UINT,
            UINT) -> HRESULT {
           ++d(p).source_draws;
           return S_OK;
         });
  ...
  void unbind() {
    for (auto *&t : textures)
      bind(t, static_cast<IDirect3DBaseTexture9 *>(nullptr));
    for (auto *&r : rt)
      bind(r, static_cast<IDirect3DSurface9 *>(nullptr));
  ...
  void no_leaks() {
    ...
    for (auto &v : objects)
      check(v->refs == 0, "COM output leaked");
  ...
  for (unsigned slot : {6u, 106u, 91u, 86u})
    for (unsigned nth = 1; nth <= (slot == 106 ? 2u : 1u); ++nth)
      for (bool null : {false, true}) {
        ...
        check(!p.caps().enabled && p.references() == 0,
              "partial programs retained");
        p.detach();
        d.no_leaks();
      }
  for (bool level : {false, true})
    for (unsigned nth = 1; nth <= 4; ++nth)
      for (bool null : {false, true}) {
        ...
        check(FAILED(p.ensure_targets(17, 11)),
              "allocation accepted malformed output");
        check(p.allocations() == 0 && p.references() == 4 &&
                  !p.coverage_target(),
        ...
        check(p.ensure_targets(17, 11) == S_OK && p.allocations() == 4,
              "same size allocated again");
        p.detach();
        d.no_leaks();
      }
}
...
void identity_faults() {
  ...
  for (unsigned mode = 0; mode < 3; ++mode)
    for (bool source_side : {false, true}) {
      ...
      check(!prep.ready && prep.state_preserved,
            "source unknown identity admitted");
...
void allocation_rollback() {
  ...
  check(p.coverage_target() == old && p.allocations() == 4 &&
            p.references() == 8,
        "failed resize destroyed committed pool");
  d.fault_slot = -1;
  check(p.ensure_targets(18, 12) == S_OK, "resize retry");
  check(p.coverage_target() != old && p.allocations() == 8,
        "resize allocation commit count");
  p.detach();
  d.no_leaks();
}
void reset_outputs() {
...
void draw_contract(bool optional_caps, bool fail_source, bool alias_scene,
                   bool null_identity) {
  ++scenarios;
  Device d;
  if (optional_caps)
    d.caps.PrimitiveMiscCaps |= D3DPMISCCAPS_COLORWRITEENABLE |
  ...
  check(prepared.ready && p.reference_accounting_busy(),
        "prepare or saved ownership");
  ...
  check(done.image == (fail_source ? LinearEmissionImage::Incomplete
                                   : LinearEmissionImage::Linear),
        "completion image");
  check(d.draws == draws + (fail_source ? 0 : 1),
        "failed source composed or replayed");
  ...
  check(!p.reference_accounting_busy() && !p.owning_candidate(),
        "ack retained bracket");
  check(p.fixture_completion().image == done.image,
        "ack changed completion validity");
  check(p.coverage_valid() == !fail_source, "coverage/source outcome");
  if (fail_source) {
    b.scene = a;
    check(!p.prepare(b).ready, "failed source unblocked by ack");
    check(p.recover_native().image == LinearEmissionImage::None,
          "ack left recovery pending");
  }
  check(d.bad_state_calls == 0 && d.bad_rt_calls == 0 && d.rs3_calls == 0,
        "unsupported state or RT accessed");
  a->Release();
  p.detach();
  d.no_leaks();
}
void clean_refusals() {
  for (auto fault :
       {LinearEmissionPassFault::Save, LinearEmissionPassFault::Copy,
        LinearEmissionPassFault::EmissionClear,
    ...
    check(d.rt[0] == a && d.rs[D3DRS_ALPHABLENDENABLE] == prior,
          "refusal changed application target/state");
    check(p.coverage_target() == mask && p.coverage_valid() &&
              !p.reference_accounting_busy(),
          "clean refusal invalidated prior coverage or retained getters");
    check(p.prepare({a, aug, 1, true}).ready,
          "clean refusal blocked subsequent enhancement");
    p.before_reset();
    p.detach();
    d.no_leaks();
  }
  ...
  check(!prep.ready && prep.state_preserved && p.coverage_valid(),
        "failed save policy");
  check(t->refs == before && !p.reference_accounting_busy(),
        "failed save output leaked");
  d.fault_slot = -1;
  p.detach();
  d.no_leaks();
}
void reset_brackets() {
  for (bool pending : {false, true}) {
    ...
    if (pending)
      check(p.finish(S_OK).candidate_bound, "reset pending candidate");
    auto *bound = d.rt[0];
    check(p.reference_accounting_busy(),
          "reset bracket expected retained state");
    p.before_reset();
    check(!d.bad_back_binds && d.rt[0] == d.back && !d.rt[1] && !d.rt[2] &&
              !d.depth,
          "reset must detach MRTs and depth before replacing RT0");
    check(b->refs == 0 && e->refs == 0 && m->refs == 0 && bound->refs == 0,
          "reset retained owned target binding");
    check(p.references() == 4 && !p.coverage_valid() &&
              !p.reference_accounting_busy() && !p.owning_candidate(),
          "reset retained bracket state");
    check(p.ensure_targets(17, 11) == S_OK && p.allocations() == 8,
          "reset cannot recreate pool");
...
void detach_after_failed_reset() {
  for (unsigned phase = 0; phase < 3; ++phase) {
    ...
    if (phase) {
      ...
      check(p.prepare({a, d.make<IDirect3DPixelShader9>(), 1, true}).ready,
            "failed Reset prepare");
      if (phase == 2)
        check(p.finish(S_OK).candidate_bound, "failed Reset pending");
    }
    p.before_reset();
    ...
    p.before_reset();
    p.detach();
    check(d.forbidden_device_accesses == 0,
          "post-failed-Reset cleanup touched device");
...
void recovery() {
  for (unsigned mode = 0; mode < 4; ++mode) {
    ...
    unsigned draws = d.draws;
    if (mode == 0)
      check(done.image == LinearEmissionImage::Native && done.candidate_bound,
            "composite fallback");
    if (mode == 1 || mode == 2)
      check(done.image == LinearEmissionImage::Incomplete &&
                !p.owning_candidate() && !p.coverage_valid(),
            "restore failure exposed candidate");
    if (mode == 2)
      p.inject(LinearEmissionPassFault::RecoveryRestore);
    ...
    check(recovered.image == ((mode == 2 || mode == 3)
                                  ? LinearEmissionImage::Incomplete
                                  : LinearEmissionImage::Native),
          "recovery image");
    check(p.recover_native().image == LinearEmissionImage::None,
          "second recovery allowed");
    if (mode == 2) {
      check(!p.owning_candidate() && !p.coverage_valid(),
            "failed recovery exposed candidate");
    } else {
      auto **slot = p.owning_candidate();
      check(slot && *slot == d.rt[0], "recovery owning slot");
      std::swap(a, *slot);
      check(p.acknowledge_exchange(true) == S_OK, "recovery ack");
      check(p.coverage_valid() == (mode == 0),
            "recovery acknowledgement validity");
    }
    a->Release();
    p.detach();
    d.no_leaks();
...
int main() {
  ...
  for (bool caps : {false, true})
    for (bool failed : {false, true})
      draw_contract(caps, failed, false, false);
  draw_contract(false, false, true, false);
  draw_contract(false, false, true, true);
  std::cout << "linear_emission_pass_host scenarios=" << scenarios
