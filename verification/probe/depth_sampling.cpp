...
static void renderAndRead(IDirect3DDevice9* device, Compiler compiler,
    ...
    check("CreateTexture INTZ", device->CreateTexture(size, size, 1, D3DUSAGE_DEPTHSTENCIL,
          D3DFORMAT(MAKEFOURCC('I','N','T','Z')), D3DPOOL_DEFAULT, &depthTexture.p, nullptr));
    ...
    check("DrawPrimitiveUP three depth triangles", device->DrawPrimitiveUP(D3DPT_TRIANGLELIST, 3, triangles, sizeof(Vertex)));
    ...
    for (const auto& point : {Point{8,8,.25f,"near_with_far_rejected"},
        ...
        std::printf("SAMPLE generation=%u output=%s name=%s xy=%u,%u expected=%.6f "
                    ...
                    point.name, point.x, point.y, point.expected, actual[0], actual[1],
...
int main(int argc, char** argv) {
    ...
    RegisterClassA(&cls);
    HWND window = CreateWindowA(cls.lpszClassName, "X3 disposable depth probe",
    ...
    try {
        ...
        for (D3DFORMAT color : {D3DFMT_A8R8G8B8, D3DFMT_A16B16G16R16F}) {
            ...
            check("CheckDepthStencilMatch INTZ", api->CheckDepthStencilMatch(0, D3DDEVTYPE_HAL,
                  mode.Format, color, D3DFORMAT(MAKEFOURCC('I','N','T','Z'))));
        }
