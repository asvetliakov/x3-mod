...
constexpr std::uint32_t H = 0xc000, W_16_9 = (0xc000u * 1920) / 1080, W_21_9 = (0xc000u * 2560) / 1080,
                        W_32_9 = (0xc000u * 5120) / 1440, W_48_9 = (0xc000u * 48) / 9;
...
struct Vector {
    ...
    std::int32_t x, y, z, r;
    int vanilla, fixed;
}; // vanilla/fixed: 1 on, 2 off, 0 = model only
struct Result {
    int pad;
...
Result execute(const Vector& v) {
    std::memcpy(node + 0xa0, &v.r, 4);
    std::memcpy(node + 0xf0, &v.x, 4);
    std::memcpy(node + 0xf4, &v.y, 4);
    std::memcpy(node + 0xf8, &v.z, 4);
    ...
    r.eax = sun_exit[0];
    r.ecx = sun_exit[1];
    r.regs_ok = sun_exit[3] == reinterpret_cast<std::uintptr_t>(node) && sun_exit[4] == 0x51515151u &&
                sun_exit[5] == 0xd1d1d1d1u && sun_exit[6] == 0xb0b0b0b0u && sun_exit[7] == sun_entry_esp &&
                sun_exit[8] == H && sun_exit[9] == v.w && GetLastError() == 0x5f1a;
    return r;
}
std::int32_t half(std::int32_t v) {
    return v / 2;
...
struct Model {
    int pad;
    std::uint32_t eax, ecx;
    bool ecx_known, overflow;
    std::uint32_t y_bound;
};
Model model(const Vector& v, bool fixed) {
    Model m{2, 0, 0, false, false, 0};
    if (!(v.z > 100) || !(2 * v.r < v.z)) return m;
    const std::int32_t by = fixmul(std::int32_t(H), fixmul(std::int32_t(v.tan), half(v.z), false),
                                   false); // the
                                           // vertical
                                           // bound,
                                           // independent
    ...
    std::int32_t ecx = half(v.x);
    if (ecx < 0) ecx = -ecx;
    m.ecx = std::uint32_t(ecx);
    m.ecx_known = true;
    m.eax = std::uint32_t(bound);
    if (ecx >= bound) return m;
    ecx = half(v.y);
    if (ecx < 0) ecx = -ecx;
    m.ecx = std::uint32_t(ecx);
    m.eax = std::uint32_t(by);
    m.pad = ecx >= by ? 2 : 1;
    return m;
}
bool matches(const Result& r, const Model& m) {
    return r.regs_ok && r.pad == m.pad && (!m.ecx_known || (r.ecx == m.ecx && r.eax == m.eax));
}
...
// A deterministic sweep: wide and narrow planes, F from 0x3470 to 0x5000, z across the int32 range (half of them past
// 1e9), x inside, near and outside the bound, y inside or outside.
std::vector<Vector> sweep(unsigned n) {
    ...
    for (unsigned i = 0; i < n; ++i) {
        ...
        v.z = (next() & 1) ? std::int32_t(1000000000u + next() % 1147483647u)
                           : std::int32_t(3000u + next() % 1000000000u);
        const unsigned kind = next() % 4;
        if (kind == 0)
            v.x = std::int32_t(next());
        else
            v.x = scaled((kind == 1 ? 0.5 : kind == 2 ? 0.999 : 1.01) * ((next() & 1) ? 1 : -1), v.z, v.w, v.tan);
...
// The chain the claim built at `site`: jmp dispatcher; dispatcher = jmp [entry]; entry = stub; stub = the 20 bytes +
// slot; *slot = the tail; tail = the six displaced bytes + jmp 0x0047e397.
bool chain_ok(std::uintptr_t site, std::uintptr_t stub, char* detail, unsigned size) {
    ...
    if (!read_mem(site, jump, 6) || jump[0] != 0xe9 || jump[5] != 0xc8) {
    ...
    if (!read_mem(stub, stub_bytes, sites::stub_length) ||
        std::memcmp(stub_bytes, sites::stub_code, sites::stub_code_length)) {
        std::snprintf(detail, size, "stub bytes");
        return false;
    }
    std::memcpy(&slot, stub_bytes + sites::stub_code_length, 4);
    if ((slot & 3) || !read_mem(slot, &tail_at, 4) || !read_mem(tail_at, tail, 11) ||
        std::memcmp(tail, sites::expected_site, 6) || tail[6] != 0xe9) {
        std::snprintf(detail, size, "slot %08lx tail %08lx", static_cast<unsigned long>(slot),
                      static_cast<unsigned long>(tail_at));
        return false;
    }
    std::memcpy(&rel, tail + 7, 4);
    if (tail_at + 11 + rel != sites::jge_va) {
        std::snprintf(detail, size, "tail jumps to %08lx", static_cast<unsigned long>(tail_at + 11 + rel));
        return false;
    }
    return true;
}
void set_mode(const wchar_t* value) {
    SetEnvironmentVariableW(L"X3M_SUN_FLARE_FIX", value);
}
std::string last_log() {
...
std::string install_row(const char* status, const char* reason, const char* mode, const char* setting,
    ...
    std::snprintf(text, sizeof text,
                  "sun_flare_fix site=%08lx status=%s reason=%s mode=%s setting=%s write=%s stub=%08lx",
...
struct Pass {
    unsigned table_ok = 0, sweep_ok = 0, overflow = 0;
    std::vector<Result> sweep_results;
};
Pass run_all(const char* step, bool fixed, const std::vector<Vector>& vectors) {
    ...
    for (const Vector& v : table()) {
        ...
        std::printf("VECTOR step=%s case=%s pad=%d want=%d eax=%08lx ecx=%08lx regs=%d ok=%d\n", step, v.name, r.pad,
                    want, static_cast<unsigned long>(r.eax), static_cast<unsigned long>(r.ecx), r.regs_ok ? 1 : 0,
                    ok ? 1 : 0);
    }
...
int main() {
    ...
    const Refusal refusals[] = {
        {nullptr, true, "default_unset_off", install_row("off", "off", "off", "-", "none")},
        ...
        {L"on", false, "refuse_executable_mismatch",
         install_row("refused", "executable_mismatch", "on", "on", "none")}};
    ...
    check(!r && fix::state() == std::string("bytes_mismatch") && engine_patch::arena_used() == arena &&
              last_log() == install_row("refused", "bytes_mismatch", "on", "on", "none"),
          "refuse_changed_window_untouched", last_log().c_str());
    check(poke(engine + window_offset + 12, &original_byte, 1) && page_is(engine, reference, nullptr),
    ...
    check(applied && fix::patched() && stub && fix::state() == std::string("ok") &&
              ...
              last_log() == install_row("patched", "ok", "on", "on", "atomic", stub),
          "install_ok_atomic_row", last_log().c_str());
    unsigned char patched[6]{};
    check(read_mem(site, patched, 6) && patched[0] == 0xe9 && patched[5] == 0xc8 && page_is(engine, reference, patched),
          "install_jump_sixth_byte_kept_rest_of_page_unchanged");
    ...
    std::snprintf(detail, sizeof detail, "same=%u of %u non_overflowing past_x=%u of %u overflowing", same,
                  unsigned(vectors.size()) - vanilla.overflow, past_x, vanilla.overflow);
    std::printf(
        "COMPARE non_overflowing=%u identical=%u overflowing=%u changed=%u past_x_patched=%u past_x_vanilla=%u\n",
        unsigned(vectors.size()) - vanilla.overflow, same, vanilla.overflow, differ, past_x, vanilla_past_x);
    ...
    arm();
    check(!fix::install_at(sites::window_va) && fix::patched() && fix::state() == std::string("already_installed") &&
              g.reads == 0,
          "second_install_refused_already_installed");
    check(initialize_checked("initialize_again") && fix::patched() && page_is(engine, reference, patched),
          "initialize_again_no_second_claim");
    arm();
    bool clean = false;
    ...
    std::snprintf(want, sizeof want,
                  "sun_flare_fix_restore site=0047e391 status=restored found=%02x%02x%02x%02x%02x%02x registered=0",
                  patched[0], patched[1], patched[2], patched[3], patched[4], patched[5]);
    check(clean && !fix::patched() && fix::stub_address() == 0 && fix::state() == std::string("restored") &&
              one_row(rows, want),
    ...
    check(!r && !fix::patched() && fix::state() == std::string("chain_failed") &&
              last_log() == install_row("refused", "chain_failed", "on", "on", "atomic") && g.restores == 1 &&
    ...
    check(!r && !fix::patched() && fix::state() == std::string("readback_mismatch") &&
              last_log() == install_row("refused", "readback_mismatch", "on", "on", "atomic") && g.restores == 1 &&
    ...
    check(!r && fix::patched() && fix::state() == std::string("rollback_failed") && fix::stub_address() == 0 &&
              last_log() == install_row("patched_unverified", "rollback_failed", "on", "on", "atomic") &&
              read_mem(site, live, 6) && live[0] == 0xe9 && live[5] == 0xc8,
    ...
    f.restore_fail = 1;
    arm(f);
    std::snprintf(want, sizeof want, "status=restore_failed found=%02x%02x%02x%02x%02x%02x registered=1", live[0],
    ...
    check(!clean && fix::patched() && g.restores == 0 &&
              one_row(rows, "status=restore_not_owned found=e910203040c8 registered=1") &&
              page_is(engine, reference, foreign),
    ...
    check(clean && !fix::patched() && g.restores == 0 &&
              one_row(rows, "status=restored found=0facd0103bc8 registered=0"),
    ...
    if (writer && roview) {
        ...
        if (!raised) {
            arm();
            check(!fix::install_at(reinterpret_cast<std::uintptr_t>(roview) + window_offset) && !fix::patched() &&
    ...
    check(!r && last_log() == install_row("refused", "late_claim", "on", "on", "none") && g.reads == 0 &&
              engine_patch::arena_used() == arena && page_is(engine, reference, nullptr),
