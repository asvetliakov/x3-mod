...
int main(int argc,char** argv) {
    try {
        ...
        std::cout<<"{\"rows\":[";
        bool first=true; unsigned applied=0, supported=0; long long create_ns=0; unsigned max_slots=0;
        for (const auto& name:names) {
            const auto original=read(corpus/(name+".bin"));
            const auto saved=original;
            Words probe{91,92}; bool entered=true;
            const auto begin=std::chrono::steady_clock::now();
            const auto status=linear_material_original_fill_pixel_variant(original.data(),original.size(),0.05f,probe,true,entered);
            ...
            for (bool depth:{false,true}) {
                ...
                for (unsigned f=0;f<3;++f) {
                    Words result{0x12345678}; bool fill_applied=true;
                    const auto outcome=linear_material_original_fill_pixel_variant(original.data(),original.size(),fills[f],result,depth,fill_applied);
                    require(outcome==LinearMaterialResult::Applied,"reviewed original admission for every K");
                    require(original==saved,"immutable input");
                    if (f==0) require(!fill_applied && result==motion,"K=0 is the plain motion variant byte for byte");
                    else {
                        fill_applied_any=fill_applied_any||fill_applied;
                        if (!fill_applied) require(result==motion,"a refused site keeps the plain motion variant");
                        else {
                            const auto [instructions,slots]=weighted_slots(result);
                            variant_instructions[depth]=instructions; variant_slots[depth]=slots;
                            require(slots<=512,"minimum SM3 static slot budget");
                            max_slots=std::max(max_slots,slots);
                        }
                    }
                    Words alias=original; bool alias_applied=false;
                    require(linear_material_original_fill_pixel_variant(alias.data(),alias.size(),fills[f],alias,depth,alias_applied)==LinearMaterialResult::Applied &&
                            alias==result && alias_applied==fill_applied,"input/output alias");
                    write(out/(name+"-ofill-"+std::to_string(f)+"-"+std::to_string(depth)+".bin"),result);
                }
            }
            for (float invalid:{-0.001f,0.5001f,std::numeric_limits<float>::infinity(),-std::numeric_limits<float>::infinity(),std::numeric_limits<float>::quiet_NaN()}) {
                Words guard{91,92}; bool guard_applied=true;
                require(linear_material_original_fill_pixel_variant(original.data(),original.size(),invalid,guard,true,guard_applied)==LinearMaterialResult::InvalidConfig &&
                        guard==Words({91,92}) && !guard_applied,"invalid fill rollback");
                Words alias=original;
                require(linear_material_original_fill_pixel_variant(alias.data(),alias.size(),invalid,alias,true,guard_applied)==LinearMaterialResult::InvalidConfig &&
                        alias==saved,"invalid fill alias rollback");
            }
            for (std::size_t offset:{std::size_t(0),std::size_t(2),original.size()/2,original.size()-1}) {
                auto broken=original; broken[offset]^=1; const auto before=broken; bool broken_applied=true;
                require(linear_material_original_fill_pixel_variant(broken.data(),broken.size(),0.05f,broken,true,broken_applied)!=LinearMaterialResult::Applied &&
                        broken==before && !broken_applied,"corrupted original alias rollback");
            }
            {
                Words guard{91,92}; bool guard_applied=true;
                require(linear_material_original_fill_pixel_variant(nullptr,original.size(),0.05f,guard,true,guard_applied)==LinearMaterialResult::InvalidInput && guard==Words({91,92}) && !guard_applied,"null rollback");
                require(linear_material_original_fill_pixel_variant(original.data(),1,0.05f,guard,true,guard_applied)==LinearMaterialResult::InvalidInput && guard==Words({91,92}),"short rollback");
                require(linear_material_original_fill_pixel_variant(original.data(),original.size()-1,0.05f,guard,true,guard_applied)!=LinearMaterialResult::Applied && guard==Words({91,92}),"truncation rollback");
            }
            if (fill_applied_any) ++applied;
            std::cout<<(first?"":",")<<"{\"name\":\""<<name<<"\",\"words\":"<<original.size()<<",\"status\":0,\"fill_applied\":"<<(fill_applied_any?1:0)
                     <<",\"motion_slots\":["<<motion_slots[0]<<','<<motion_slots[1]<<"],\"variant_slots\":["<<variant_slots[0]<<','<<variant_slots[1]<<']'
                     <<",\"motion_instructions\":["<<motion_instructions[0]<<','<<motion_instructions[1]<<"],\"variant_instructions\":["<<variant_instructions[0]<<','<<variant_instructions[1]<<"]}";
            first=false;
        }
        Words authored{0xffff0300u,0xffffu}, result{91,92}; bool authored_applied=true;
        require(linear_material_original_fill_pixel_variant(authored.data(),authored.size(),0.05f,result,true,authored_applied)==LinearMaterialResult::UnsupportedShader &&
                result==Words({91,92}) && !authored_applied,"unreviewed valid framing");
        std::cout<<"],\"programs\":"<<names.size()<<",\"supported\":"<<supported<<",\"applied\":"<<applied<<",\"max_variant_slots\":"<<max_slots
