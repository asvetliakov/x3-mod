...
void basic(){
    ...
    {
        ApplicationAdmission root(monitor);check(root.admitted(),"outer admitted");
        check(monitor.snapshot().active_roots==1,"one root");
        {
            ApplicationAdmission nested(monitor);check(nested.admitted(),"nested admitted");
        ...
        {
            ReplayAdmission replay(root);check(replay.admitted(),"sole root promoted");
        ...
        ReplayAdmission stale(root);check(stale.result()==AdmissionResult::InactiveBoundary,"retired boundary refuses");
        // Models a final child's parent dispatch after the child ticket ends.
        ApplicationAdmission parent(monitor);ReplayAdmission parent_replay(parent);
    ...
    balanced(monitor);std::puts("CASE basic PASS");
}
void waiting(){
    for(unsigned operation=0;operation<6;++operation){
        AdmissionMonitor monitor;std::atomic<bool> native_dispatch{false};
        {
            ApplicationAdmission root(monitor);ReplayAdmission replay(root);
            ...
            replay.finish();
            {
                ...
                check(!retry.admitted()||native_dispatch.load(std::memory_order_acquire),"re-promotion cannot overtake a waiting application");
            }
            worker.join();
...
void existing_roots(){
    AdmissionMonitor monitor;std::atomic<bool> release{false};
    {
        ApplicationAdmission root(monitor);
        std::vector<std::thread> workers;
        for(unsigned n=0;n<8;++n)workers.emplace_back([&]{
            ApplicationAdmission call(monitor);check(call.admitted(),"parallel ordinary root");
        ...
        release.store(true,std::memory_order_release);for(auto& worker:workers)worker.join();
        ReplayAdmission allowed(root);check(allowed.admitted(),"promotion after outstanding work publishes");
    }
    balanced(monitor);std::puts("CASE existing_roots PASS");
}
void vetoes(){
    AdmissionMonitor monitor;
    {
        ...
        ReplayAdmission denied(root);check(denied.result()==AdmissionResult::Vetoed,"callback veto prevents promotion");
        ApplicationAdmission nested(monitor);check(nested.admitted(),"veto does not reject application calls");
    }
    {
        ApplicationAdmission after_reset(monitor);ReplayAdmission denied(after_reset);
        check(denied.result()==AdmissionResult::Vetoed,"veto survives all prior scope retirement");
    }
    const auto state=monitor.snapshot();
    check(state.first_veto==AdmissionVeto::PrivateUnknown&&state.vetoes==3,"first reason immutable and all vetoes retained");
    balanced(monitor);
    {
        AdmissionMonitor first,second;ApplicationAdmission root(first),wrong(second);
...
void invariants(){
    AdmissionMonitor monitor;
    {
        ...
        check(callback.result()==AdmissionResult::SameThreadReentry&&!callback.admitted(),"same-thread callback diagnosed without blocking or admission");
        check(monitor.snapshot().active_roots==1&&monitor.snapshot().replay_active,"refusal does not retire active replay");
        check(!root.finish(),"root cannot finish before replay");
        replay.finish();ReplayAdmission next(root);
    ...
    {
        ApplicationAdmission root(foreign);ReplayAdmission replay(root);
        std::thread worker([&]{
            check(!root.finish()&&!replay.finish(),"foreign thread cannot retire scopes");
            ReplayAdmission wrong(root);check(!wrong.admitted(),"foreign boundary promotion rejected");
        });worker.join();
        check(foreign.snapshot().replay_active,"foreign refusal leaves owner token intact");
    }
    balanced(foreign);std::puts("CASE invariants PASS");
}
void racing(){
    unsigned promoted=0;
    for(unsigned round=0;round<128;++round){
        AdmissionMonitor monitor;std::atomic<unsigned> before{0},after{0};
        {
            ApplicationAdmission root(monitor);
            std::thread worker([&]{
                ApplicationAdmission update(monitor);check(update.admitted(),"racing ordinary entry admitted");
                before.store(1,std::memory_order_relaxed);std::this_thread::yield();
            ...
            });
            {
                ...
                if(replay.admitted()){
                    ++promoted;
                    check(before.load(std::memory_order_relaxed)==after.load(std::memory_order_relaxed),"replay cannot observe half publication");
            ...
            worker.join();check(before==1&&after==1,"racing publication eventually completes");
        }
        balanced(monitor);
    }
...
void nested_stress(){
    AdmissionMonitor monitor;std::vector<std::thread> workers;
...
void veto_race(){
    for(unsigned round=0;round<32;++round){
        AdmissionMonitor monitor;
        {
            ApplicationAdmission root(monitor);std::atomic<bool> go{false};
            ...
            check(first.admitted()||first.result()==AdmissionResult::Vetoed,"veto/promotion has one serialized decision");
            announcer.join();first.finish();
            check(monitor.snapshot().first_veto==AdmissionVeto::UnobservedRoute,"concurrent veto is retained");
            ReplayAdmission second(root);check(second.result()==AdmissionResult::Vetoed,"no later promotion after concurrent veto");
        }
        balanced(monitor);
    }
...
int main(){
    basic();waiting();existing_roots();vetoes();invariants();racing();nested_stress();veto_race();
