...
int main() {
    ...
    const auto displacement = static_cast<uint32_t>(reinterpret_cast<uintptr_t>(&original) -
                                                    reinterpret_cast<uintptr_t>(site + 5));
    std::memcpy(initial + 1, &displacement, 4);
    std::memcpy(site, initial, sizeof initial);
    DWORD old = 0;
    if (!VirtualProtect(site, 4096, PAGE_EXECUTE_READ, &old) || !FlushInstructionCache(GetCurrentProcess(), site, 6))
        return 2;
    const auto invoke = reinterpret_cast<void (*)()>(site);
    invoke();
    check(originals == 1, "unpatched_original");
    ...
    invoke();
    check(originals == 2 && signals == 1, "tail_original_once");
    ...
    check(!x3m::scene_hook::fixture_install_compositor(site, reinterpret_cast<void*>(1), &callbacks),
    ...
    invalid.cleanup = nullptr;
    check(!x3m::scene_hook::fixture_install_compositor(site, reinterpret_cast<void*>(&original), &invalid),
          "missing_cleanup_refused");
    check(x3m::scene_hook::fixture_install_compositor(site, reinterpret_cast<void*>(&original), &callbacks),
          "bridge_install");
    ...
    invoke();
    check(originals == 5 && pres == 2, "restored_original_once");
