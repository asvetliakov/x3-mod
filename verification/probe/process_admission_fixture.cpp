...
bool same(const State& a, const State& b) {
    return a.error == b.error && a.mxcsr == b.mxcsr && !std::memcmp(a.fp, b.fp, sizeof(a.fp));
}
struct Worker {
    HANDLE start = nullptr;
    AdmissionMonitor* selected = nullptr;
...
DWORD WINAPI enter(void* raw) {
    ...
    const State after;
    worker.cpu_preserved = same(before, after);
    original.restore();
...
int main(int argc, char** argv) {
    ...
    if (enabled)
        setting = L"1";
    else if (!std::strcmp(argv[1], "malformed"))
        setting = L"01";
    else if (!std::strcmp(argv[1], "long"))
        setting = L"11111111";
    ...
    check(start != nullptr, "start barrier");
    if (!start) return 1;
    Worker workers[8]{};
    HANDLE threads[8]{};
    unsigned created = 0;
    for (unsigned n = 0; n < 8; ++n) {
        workers[n].start = start;
        threads[n] = CreateThread(nullptr, 0, enter, &workers[n], 0, nullptr);
        check(threads[n] != nullptr, "worker creation");
        if (!threads[n]) break;
        ++created;
    }
    check(SetEvent(start) != 0, "release first-access race");
    if (created != 8) {
        if (created) WaitForMultipleObjects(created, threads, TRUE, 15000);
        return 1;
    }
    const DWORD joined = WaitForMultipleObjects(8, threads, TRUE, 15000);
    check(joined == WAIT_OBJECT_0, "workers finish");
    if (joined != WAIT_OBJECT_0) return 1;
    for (unsigned n = 0; n < 8; ++n) {
    ...
    seed();
    const State before;
    auto* selected = process_admission_monitor();
    const State after;
    original.restore();
