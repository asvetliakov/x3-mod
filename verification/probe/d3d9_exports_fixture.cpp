...
const char* const names[] = {"D3DPERF_BeginEvent",
                             "D3DPERF_EndEvent",
                             "D3DPERF_GetStatus",
...
int main(int argc, char** argv) {
    ...
    value[0] = 0;
    GetEnvironmentVariableA("USERPROFILE", value, sizeof value);
    std::printf("ENV USERPROFILE=%s\n", value[0] ? value : "-");
    HMODULE proxy = LoadLibraryA("d3d9.dll");
    if (!proxy) {
        std::printf("RESULT FAIL LoadLibrary error=%lu\n", GetLastError());
        return 1;
    }
    char path[MAX_PATH]{};
    ...
    auto on12ex = reinterpret_cast<HRESULT(WINAPI*)(UINT, void*, UINT, IDirect3D9Ex**)>(
        GetProcAddress(proxy, "Direct3DCreate9On12Ex"));
    ...
    if (SUCCEEDED(ex_result) && ex) {
        ex->Release();
        check(true, "a 9On12Ex factory was returned and released");
    } else
        check(on12ex != nullptr && ex_result == D3DERR_NOTAVAILABLE && ex == nullptr,
              "Direct3DCreate9On12Ex without a backend export answers D3DERR_NOTAVAILABLE and null");
    FreeLibrary(proxy);
