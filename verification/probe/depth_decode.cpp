...
struct Module {
    ...
    explicit Module(const char* path)
        ...
        GetModuleFileNameA(p, resolved, MAX_PATH);
        std::printf("MODULE %s\n", resolved);
    }
    ~Module() { FreeLibrary(p); }
};
template <class T> T symbol(HMODULE p, const char* name) {
    FARPROC address = GetProcAddress(p, name);
    T result = nullptr;
...
void check(const char* name, HRESULT hr) {
    if (FAILED(hr)) {
        std::printf("API %s result=%08lx FAILED\n", name, hr);
...
void compile(Compiler fn, const std::string& source, const char* profile, ID3DXBuffer** result) {
    ...
    HRESULT hr = fn(source.c_str(), UINT(source.size()), nullptr, nullptr, "main", profile,
...
std::vector<float> values() {
    ...
    while (result.size() < 32) {
        ...
        seed ^= seed >> 17;
        seed ^= seed << 5;
        result.push_back(float(seed & 0xffffff) / 16777215.f);
    }
    return result;
}
...
void drawQuad(IDirect3DDevice9* device, UINT width, UINT height) {
    const Vertex quad[] = {vertex(0, 0, 0, 0, 0, width, height), vertex(float(width), 0, 0, 1, 0, width, height),
                           vertex(0, float(height), 0, 0, 1, width, height),
                           vertex(float(width), float(height), 0, 1, 1, width, height)};
...
void copyDepth(IDirect3DDevice9* d, IDirect3DTexture9* destination, IDirect3DSurface9* source) {
    Com<IDirect3DBaseTexture9> previous, after;
    Com<IDirect3DSurface9> afterDepth;
    ...
    std::printf("COPY_STATE inside_scene=1 original_depth=%u texture=%u pointsize=%u %s\n", afterDepth.p == source,
                previous.p == after.p, oldPoint == afterPoint, pass ? "PASS" : "FAIL");
    if (!pass) throw std::runtime_error("copy state mismatch");
}

void runCase(IDirect3DDevice9* d, Compiler compiler, const std::string& hlsl, UINT width, UINT height,
             unsigned generation) {
    ...
    Com<IDirect3DPixelShader9> solid, decode;
    Com<IDirect3DVertexDeclaration9> declaration;
    Com<ID3DXBuffer> vc, pc, dc;
    compile(
        compiler,
        "void main(float4 p:POSITION0,float2 uv:TEXCOORD0,out float4 op:POSITION0,out float2 ou:TEXCOORD0){op=p;ou=uv;}",
        "vs_3_0", &vc.p);
    ...
    check("Create original D24X8",
          d->CreateDepthStencilSurface(width, height, D3DFMT_D24X8, D3DMULTISAMPLE_NONE, 0, FALSE, &source.p, nullptr));
    check("Create D24X8 snapshot", d->CreateTexture(width, height, 1, D3DUSAGE_DEPTHSTENCIL, D3DFMT_D24X8,
                                                    D3DPOOL_DEFAULT, &snapshot.p, nullptr));
    check("Get snapshot surface", snapshot->GetSurfaceLevel(0, &snapshotSurface.p));
    check("Create scene color",
          d->CreateRenderTarget(width, height, D3DFMT_A8R8G8B8, D3DMULTISAMPLE_NONE, 0, FALSE, &color.p, nullptr));
    check("Create decoded R32F",
          d->CreateRenderTarget(width, height, D3DFMT_R32F, D3DMULTISAMPLE_NONE, 0, FALSE, &decoded.p, nullptr));
    check("Create readback R32F",
          d->CreateOffscreenPlainSurface(width, height, D3DFMT_R32F, D3DPOOL_SYSTEMMEM, &readback.p, nullptr));
    ...
    check("SetVP", d->SetViewport(&viewport));
    check("SetDecl", d->SetVertexDeclaration(declaration.p));
    check("SetVS", d->SetVertexShader(vs.p));
    check("SetPS", d->SetPixelShader(solid.p));
    for (auto state : {D3DRS_ALPHABLENDENABLE, D3DRS_ALPHATESTENABLE, D3DRS_SRGBWRITEENABLE, D3DRS_FOGENABLE})
        check("DisableRS", d->SetRenderState(state, FALSE));
    check("CullNone", d->SetRenderState(D3DRS_CULLMODE, D3DCULL_NONE));
    check("Zenable", d->SetRenderState(D3DRS_ZENABLE, TRUE));
    check("Zwrite", d->SetRenderState(D3DRS_ZWRITEENABLE, TRUE));
    check("Zalways", d->SetRenderState(D3DRS_ZFUNC, D3DCMP_ALWAYS));
    check("ColorWrite", d->SetRenderState(D3DRS_COLORWRITEENABLE, 15));
    check("SentinelTexture", d->SetTexture(0, sentinel.p));
    check("SentinelPoint", d->SetRenderState(D3DRS_POINTSIZE, 0x40500000));
    check("Clear source", d->Clear(0, nullptr, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, 0, 1, 0));
    std::vector<Vertex> geometry;
    const auto depths = values();
    for (unsigned i = 0; i < depths.size(); ++i) {
        float x = float((i % 8) * width / 8), y = float((i / 8) * height / 4), r = x + width / 8.f,
              b = y + height / 4.f, z = depths[i];
        geometry.insert(geometry.end(), {vertex(x, y, z, 0, 0, width, height), vertex(r, y, z, 0, 0, width, height),
                                         vertex(x, b, z, 0, 0, width, height), vertex(r, y, z, 0, 0, width, height),
                                         vertex(r, b, z, 0, 0, width, height), vertex(x, b, z, 0, 0, width, height)});
    }
    check("Begin original scene", d->BeginScene());
    check("Draw original depth tiles",
          d->DrawPrimitiveUP(D3DPT_TRIANGLELIST, UINT(geometry.size() / 3), geometry.data(), sizeof(Vertex)));
    ...
    for (unsigned phase = 0; phase < 2; ++phase) {
        ...
        check("Capture all state", d->CreateStateBlock(D3DSBT_ALL, &state.p));
        check("Detach source DS for decode", d->SetDepthStencilSurface(nullptr));
        check("Bind decodeRT", d->SetRenderTarget(0, decoded.p));
        check("Set decodeVP", d->SetViewport(&viewport));
        check("Disable decodeZ", d->SetRenderState(D3DRS_ZENABLE, FALSE));
        check("Disable decodeZwrite", d->SetRenderState(D3DRS_ZWRITEENABLE, FALSE));
        check("Bind snapshot", d->SetTexture(0, snapshot.p));
        check("DecodePS", d->SetPixelShader(decode.p));
        check("Point min", d->SetSamplerState(0, D3DSAMP_MINFILTER, D3DTEXF_POINT));
        check("Point mag", d->SetSamplerState(0, D3DSAMP_MAGFILTER, D3DTEXF_POINT));
        check("No mip", d->SetSamplerState(0, D3DSAMP_MIPFILTER, D3DTEXF_NONE));
        check("ClampU", d->SetSamplerState(0, D3DSAMP_ADDRESSU, D3DTADDRESS_CLAMP));
        check("ClampV", d->SetSamplerState(0, D3DSAMP_ADDRESSV, D3DTADDRESS_CLAMP));
        check("No sRGB", d->SetSamplerState(0, D3DSAMP_SRGBTEXTURE, FALSE));
        check("Begin decode", d->BeginScene());
        drawQuad(d, width, height);
        check("End decode", d->EndScene());
        check("Readback validation only", d->GetRenderTargetData(decoded.p, readback.p));
        D3DLOCKED_RECT lock{};
        check("Lock validation", readback->LockRect(&lock, nullptr, D3DLOCK_READONLY));
        for (unsigned i = 0; i < depths.size(); ++i) {
            UINT x = (i % 8) * width / 8 + width / 16, y = (i / 8) * height / 4 + height / 8;
            float actual = 0;
            std::memcpy(&actual, static_cast<unsigned char*>(lock.pBits) + y * lock.Pitch + x * 4, 4);
            double err = std::fabs(double(actual) - depths[i]), lsbs = err * 16777215.0;
            bool pass = std::isfinite(actual) && lsbs <= 2.0;
            ...
            const bool endpoints = (depths[i] == 0 ? actual == 0 : actual > 0) &&
                                   (depths[i] == 1 ? actual == 1 : actual < 1);
            pass &= endpoints;
            all &= pass;
            std::printf(
                "SAMPLE generation=%u width=%u height=%u after_source_clear=%u index=%u expected=%.9f actual=%.9f error_d24_lsb=%.6f endpoints=%u %s\n",
        ...
        if (phase) {
            ...
            HRESULT hs = d->CreateQuery(D3DQUERYTYPE_TIMESTAMP, &start.p),
                    he = d->CreateQuery(D3DQUERYTYPE_TIMESTAMP, &end.p),
                    hf = d->CreateQuery(D3DQUERYTYPE_TIMESTAMPFREQ, &freq.p),
                    hd = d->CreateQuery(D3DQUERYTYPE_TIMESTAMPDISJOINT, &disjoint.p);
            ...
            check("Begin warmup", d->BeginScene());
            for (unsigned i = 0; i < 8; ++i) drawQuad(d, width, height);
            check("End warmup", d->EndScene());
            check("Warmup event", event->Issue(D3DISSUE_END));
            BOOL done = FALSE;
            waitQuery(event.p, &done, sizeof(done));
            if (!done) throw std::runtime_error("warmup event did not report completion");
            LARGE_INTEGER frequency, t0, t1;
            QueryPerformanceFrequency(&frequency);
            QueryPerformanceCounter(&t0);
            if (gpu)
                gpu = SUCCEEDED(freq->Issue(D3DISSUE_END)) && SUCCEEDED(disjoint->Issue(D3DISSUE_BEGIN)) &&
                      SUCCEEDED(start->Issue(D3DISSUE_END));
            constexpr unsigned iterations = 32;
            check("Begin timed", d->BeginScene());
            for (unsigned i = 0; i < iterations; ++i) drawQuad(d, width, height);
            check("End timed", d->EndScene());
            if (gpu) gpu = SUCCEEDED(end->Issue(D3DISSUE_END)) && SUCCEEDED(disjoint->Issue(D3DISSUE_END));
            check("Timed event", event->Issue(D3DISSUE_END));
            done = FALSE;
            waitQuery(event.p, &done, sizeof(done));
            ...
            UINT64 a = 0, b = 0, hz = 0;
            if (gpu) {
                waitQuery(start.p, &a, sizeof(a));
                waitQuery(end.p, &b, sizeof(b));
                waitQuery(freq.p, &hz, sizeof(hz));
                waitQuery(disjoint.p, &discontinuous, sizeof(discontinuous));
            ...
            for (unsigned i = 0; i < 16; ++i) {
                LARGE_INTEGER aTime, bTime;
                QueryPerformanceCounter(&aTime);
                check("Begin serialized decode", d->BeginScene());
                drawQuad(d, width, height);
                check("End serialized decode", d->EndScene());
                check("Serialized event", event->Issue(D3DISSUE_END));
                done = FALSE;
                waitQuery(event.p, &done, sizeof(done));
                QueryPerformanceCounter(&bTime);
                if (!done) throw std::runtime_error("serialized event did not complete");
            ...
            for (double value : serialTimes) serialTotal += value;
            std::printf("TIMING generation=%u width=%u height=%u iterations=%u gpu_ms_per_decode=%.6f "
                        ...
                        generation, width, height, iterations, gpuMs,
                        1000.0 * double(t1.QuadPart - t0.QuadPart) / frequency.QuadPart / iterations, serialTotal / 16,
                        (serialTimes[7] + serialTimes[8]) * .5, serialTimes.front(), serialTimes.back(),
                        unsigned(discontinuous));
        }
        check("Unbind decoder texture", d->SetTexture(0, nullptr));
        check("Restore scene RT", d->SetRenderTarget(0, color.p));
        check("Restore scene DS", d->SetDepthStencilSurface(source.p));
        check("Apply saved state", state->Apply());
        check("Restore viewport", d->SetViewport(&viewport));
        Com<IDirect3DSurface9> restored, restoredColor;
        Com<IDirect3DBaseTexture9> restoredTexture;
        Com<IDirect3DPixelShader9> restoredPS;
        DWORD restoredPoint = 0, restoredZ = 0, restoredZwrite = 0;
        check("Get restored DS", d->GetDepthStencilSurface(&restored.p));
        check("Get restored texture", d->GetTexture(0, &restoredTexture.p));
        check("Get restored POINTSIZE", d->GetRenderState(D3DRS_POINTSIZE, &restoredPoint));
        check("Get restored RT", d->GetRenderTarget(0, &restoredColor.p));
        check("Get restored PS", d->GetPixelShader(&restoredPS.p));
        check("Get restored Z", d->GetRenderState(D3DRS_ZENABLE, &restoredZ));
        check("Get restored Zwrite", d->GetRenderState(D3DRS_ZWRITEENABLE, &restoredZwrite));
        bool restoredOK = restored.p == source.p && restoredTexture.p == sentinel.p && restoredPoint == 0x40500000 &&
                          restoredColor.p == color.p && restoredPS.p == solid.p && restoredZ == TRUE &&
                          restoredZwrite == TRUE;
        std::printf("RESTORE generation=%u width=%u height=%u phase=%u %s\n", generation, width, height, phase,
                    restoredOK ? "PASS" : "FAIL");
        all &= restoredOK;
    }
    check("Unbind texture", d->SetTexture(0, nullptr));
    check("Unbind depth", d->SetDepthStencilSurface(nullptr));
    check("Restore backbuffer", d->SetRenderTarget(0, backbuffer.p));
    check("Unbind VS", d->SetVertexShader(nullptr));
    check("Unbind PS", d->SetPixelShader(nullptr));
    check("Unbind declaration", d->SetVertexDeclaration(nullptr));
...
int main(int argc, char** argv) {
    ...
    cls.lpszClassName = "X3DepthDecodeProbe";
    RegisterClassA(&cls);
    HWND window = CreateWindowA(cls.lpszClassName, "X3 original depth decoder", WS_OVERLAPPEDWINDOW, 100, 100, 128, 128,
    ...
    try {
        ...
        if (!file) throw std::runtime_error("open decoder HLSL");
        std::string hlsl((std::istreambuf_iterator<char>(file)), {});
        Module d3dx(argv[1]), runtime("d3d9.dll");
        auto compiler = symbol<Compiler>(d3dx.p, "D3DXCompileShader");
        auto create = symbol<IDirect3D9*(WINAPI*)(UINT)>(runtime.p, "Direct3DCreate9");
        Com<IDirect3D9> api;
        ...
        for (unsigned generation = 0; generation < 2; ++generation) {
            runCase(device.p, compiler, hlsl, 64, 64, generation);
            runCase(device.p, compiler, hlsl, 1280, 768, generation);
            if (!generation) {
                check("Reset after released resources", device->Reset(&pp));
                std::puts("RESET PASS");
            }
        }
        std::puts("RESULT PASS: 256 precision samples, 4 inside-scene copies, 8 state restorations, 4 timings, Reset");
        result = 0;
