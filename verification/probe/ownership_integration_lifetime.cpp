...
static void expect(const char* name, bool valid) {
...
int main() {
    ...
    auto address = library ? GetProcAddress(library, "Direct3DCreate9") : nullptr;
    ...
    cls.lpszClassName = "X3OwnershipIntegrationLifetime";
    RegisterClassA(&cls);
    HWND window = CreateWindowA(cls.lpszClassName, "X3 ownership DLL lifetime fixture", WS_OVERLAPPEDWINDOW, 80, 80,
    ...
    for (unsigned round = 0; round < 16; ++round) {
        ...
        if (!check("create device", factory->CreateDevice(0, D3DDEVTYPE_HAL, window,
                                                          D3DCREATE_HARDWARE_VERTEXPROCESSING, &pp, &device))) {
            factory->Release();
            break;
        }
        IDirect3DTexture9* child = nullptr;
        if (!check("create final child",
                   device->CreateTexture(8, 8, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &child, nullptr))) {
            device->Release();
            factory->Release();
            break;
        }
        auto expected_factory = factory;
        auto expected_device = device;
        factory->Release();
        factory = nullptr;
        device->Release();
        device = nullptr;
        if (check("recover device through child", child->GetDevice(&device))) {
            ...
            if (check("recover factory through device", device->GetDirect3D(&factory))) {
                expect("factory identity", factory == expected_factory);
                factory->Release();
                factory = nullptr;
            }
            device->Release();
    ...
    std::printf("INTEGRATION LIFETIME RESULT failures=%u\n", failures);
    return failures ? 1 : 0;
}
