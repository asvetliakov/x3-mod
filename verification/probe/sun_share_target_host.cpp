...
bool SUCCEEDED(HRESULT h) {
    return h >= 0;
}
bool FAILED(HRESULT h) {
    return h < 0;
}
static int fail_format = 0;
static bool fail_level = false;
static unsigned live = 0, creates = 0;
struct IDirect3DSurface9 {
    D3DFORMAT format;
    explicit IDirect3DSurface9(D3DFORMAT f)
        : format(f) {
        ++live;
    }
    ~IDirect3DSurface9() { --live; }
};
struct IDirect3DTexture9 {
    D3DFORMAT format;
    HRESULT GetSurfaceLevel(unsigned, IDirect3DSurface9** out) {
        if (fail_level) return E_FAIL;
        *out = new IDirect3DSurface9(format);
        return S_OK;
    }
};
...
HRESULT create(D, UINT, UINT, UINT, DWORD, D3DFORMAT format, DWORD, IDirect3DTexture9** out, void*) {
    ++creates;
    if (int(format) == fail_format) return E_FAIL;
    *out = new IDirect3DTexture9{format};
    return S_OK;
}
struct MotionOutput {
    D device_ = nullptr;
    bool sun_lane_requested_ = true, sun_lane_qualified_ = true, sun_lane_active_ = false, sun_lane_failed_ = false,
         target_failed_ = false, depth_enabled_ = true;
    D3DFORMAT lane_depth_format() const noexcept {
        return sun_lane_active_ ? D3DFMT_A32B32G32R32F : D3DFMT_R32F;
    } // motion_output.h: the lane's RT2 is A32B32G32R32F
    struct {
        unsigned format = 77;
    } main_depth_;
    ...
    bool ensure_target(UINT, UINT) noexcept;
    void release_target() noexcept;
    bool latch(UINT w, UINT h) {
        const bool ok = ensure_target(w, h);
        if (ok)
            history_.begin_frame({generation_, w, h});
        else
            history_.invalidate();
        return ok;
    }
...
void check(bool value) {
    ++checks;
    if (!value) throw std::runtime_error("sun target host check " + std::to_string(checks));
}
RigidDrawKey key(unsigned node) {
    RigidDrawKey k{};
    k.object_lifetime = node;
    k.camera_lifetime = k.draw_domain = k.camera = k.vertex_buffer = k.declaration = k.position_program = k.stride =
        k.primitives = k.pass = 1;
    k.node = node;
    return k;
}
...
void seed(MotionOutput& output) {
    SubmittedMatrix previous{};
    check(output.latch(64, 64));
    check(!output.history_.lookup_and_record(key(1), matrix(), previous));
    check(!output.history_.lookup_and_record(key(2), matrix(), previous));
    check(output.history_.commit(true));
}
int main() {
    for (unsigned change = 0; change < 7; ++change) {
        MotionOutput output;
        seed(output);
        output.sun_lane_failed_ = true;
        ...
        if (change == 4) {
            output.sun_lane_active_ = false;
            width = 32;
        } // Ordinary resize invalidates.
        ...
        const bool ok = output.latch(width, height);
        check(ok == (change < 5));
        SubmittedMatrix previous{};
        check(output.history_.lookup_and_record(key(1), matrix(), previous) == (change == 0));
        if (change == 0) {
            check(previous == matrix());
            check(output.history_.lookup_and_record(key(2), matrix(), previous));
            check(!output.history_.lookup_and_record(key(1), matrix(), previous)); // Original fixture's duplicate
                                                                                   // fault.
