...
float saturate(float value){return std::max(0.f,std::min(1.f,value));}
constexpr UINT width=64;
struct Vertex{float p[4];float value[4];};
...
unsigned render(IDirect3DDevice9* device,Compiler compiler,unsigned generation,
                unsigned profile,bool color,bool psSaturate,bool gradient){
    ...
    std::string ps="float4 main(float4 v:"+std::string(semantic)+
        "):COLOR0{return "+(psSaturate?"saturate(v)":"v")+";}";
    compile(compiler,vs,vsProfile,&vc.p);compile(compiler,ps,psProfile,&pc.p);
    Com<IDirect3DVertexShader9> vshader; Com<IDirect3DPixelShader9> pshader;
    ...
    check("CreateRenderTarget FP16",device->CreateRenderTarget(width,width,D3DFMT_A16B16G16R16F,D3DMULTISAMPLE_NONE,0,FALSE,&target.p,nullptr));
    check("CreateReadback FP16",device->CreateOffscreenPlainSurface(width,width,D3DFMT_A16B16G16R16F,D3DPOOL_SYSTEMMEM,&readback.p,nullptr));
    ...
    D3DVIEWPORT9 viewport={0,0,width,width,0,1};check("SetViewport",device->SetViewport(&viewport));
    check("SetVertexDeclaration",device->SetVertexDeclaration(declaration.p));
    check("SetVertexShader",device->SetVertexShader(vshader.p));check("SetPixelShader",device->SetPixelShader(pshader.p));
    ...
    Vertex triangle[]={vertex(0,0,.25f),vertex(width,0,gradient?4.f:.25f),vertex(0,width,gradient?16.f:.25f)};
    ...
    D3DLOCKED_RECT lock{};check("LockRect",readback->LockRect(&lock,nullptr,D3DLOCK_READONLY));
    ...
    for(const auto& point:points){
        ...
        float expected[3][4]={{red,4,16,1},{gradient?.25f+.75f*(bx+by):.25f,1,1,1},{saturate(red),1,1,1}};
        float actual[4]{};
        const auto* row=static_cast<unsigned char*>(lock.pBits)+point[1]*lock.Pitch;
        for(unsigned channel=0;channel<4;++channel){
            unsigned short half;std::memcpy(&half,row+point[0]*8+channel*2,2);actual[channel]=halfFloat(half);
        }
        unsigned matching=0;
        ...
        std::printf("SAMPLE generation=%u profile=%u semantic=%s ps_saturate=%u gradient=%u xy=%u,%u "
            ...
            generation,profile,semantic,psSaturate,gradient,point[0],point[1],red,
            actual[0],actual[1],actual[2],actual[3],matching,(matching&(color?7:1))?"PASS":"FAIL");
    }
    check("UnlockRect",readback->UnlockRect());
    check("RestoreBackbuffer",device->SetRenderTarget(0,backbuffer.p));
    check("UnbindVertexShader",device->SetVertexShader(nullptr));check("UnbindPixelShader",device->SetPixelShader(nullptr));
...
int main(int argc,char** argv){
    ...
    WNDCLASSA cls{};cls.lpfnWndProc=DefWindowProcA;cls.hInstance=GetModuleHandleA(nullptr);cls.lpszClassName="X3VertexColorProbe";
    RegisterClassA(&cls);
    HWND window=CreateWindowA(cls.lpszClassName,"X3 original interpolation probe",WS_OVERLAPPEDWINDOW,100,100,128,128,nullptr,nullptr,cls.hInstance,nullptr);
    int result=1;
    try{
        ...
        Com<IDirect3D9> api;api.p=create(D3D_SDK_VERSION);if(!api.p)throw std::runtime_error("Direct3DCreate9");
        D3DADAPTER_IDENTIFIER9 adapter{};check("GetAdapterIdentifier",api->GetAdapterIdentifier(0,0,&adapter));
        ...
        pp.BackBufferWidth=width;pp.BackBufferHeight=width;pp.BackBufferFormat=D3DFMT_A8R8G8B8;pp.PresentationInterval=D3DPRESENT_INTERVAL_IMMEDIATE;
        Com<IDirect3DDevice9> device;
        check("CreateDevice",api->CreateDevice(0,D3DDEVTYPE_HAL,window,D3DCREATE_HARDWARE_VERTEXPROCESSING|D3DCREATE_PUREDEVICE,&pp,&device.p));
        for(unsigned generation=0;generation<2;++generation){
            for(unsigned profile:{2u,3u})for(bool color:{false,true})for(bool clamp:{false,true})for(bool gradient:{false,true})
        ...
        char path[MAX_PATH]{};HMODULE wine=GetModuleHandleA("wined3d.dll");if(wine)GetModuleFileNameA(wine,path,MAX_PATH);
        std::printf("BACKEND wined3d=%s\n",wine?path:"not loaded");
        std::puts("RESULT PASS: 96 numeric samples, 32 cases, two generations with Reset");result=0;
