...
struct Snapshot {
    ...
    Snapshot(IDirect3DDevice9* d,UINT w,UINT h):width(w),height(h) {
        ok("snapshot target",d->CreateRenderTarget(w,h,D3DFMT_R32F,D3DMULTISAMPLE_NONE,0,FALSE,&color.p,nullptr));
        ok("snapshot readback",d->CreateOffscreenPlainSurface(w,h,D3DFMT_R32F,D3DPOOL_SYSTEMMEM,&readback.p,nullptr));
    }
    ...
    void verify(IDirect3DDevice9* d,const char* epoch,bool cleared) {
        ...
        for(const auto& p:points) {
            float actual; std::memcpy(&actual,static_cast<const char*>(lr.pBits)+p.y*lr.Pitch+p.x*4,4);
...
void log(const char* format,...) {
    ...
    trace.emplace_back(buffer);std::printf("TRACE %s\n",buffer);
}
}
...
struct PostClearQueryFault {
    ...
    explicit PostClearQueryFault(IDirect3DDevice9* value):native(value),original(*reinterpret_cast<void***>(value)) {
        expect("post-Clear fault is not nested",active==nullptr);
        std::memcpy(table.data(),original,sizeof(table));table[40]=reinterpret_cast<void*>(&getDepth);
        active=this;*reinterpret_cast<void***>(native)=table.data();
    }
    ~PostClearQueryFault(){*reinterpret_cast<void***>(native)=original;active=nullptr;}
    static HRESULT WINAPI getDepth(IDirect3DDevice9* device,IDirect3DSurface9** out){
        ...
        ++fault.calls;if(out)*out=nullptr;return E_FAIL;
    }
};
...
struct Scene {
    ...
    Scene(IDirect3DDevice9* device,Compiler compiler):d(device) {
        ok("scene main",d->GetRenderTarget(0,&main.p));ok("scene depth",d->GetDepthStencilSurface(&depth.p));
        ok("copy color texture",d->CreateTexture(64,64,1,D3DUSAGE_RENDERTARGET,D3DFMT_A8R8G8B8,D3DPOOL_DEFAULT,&copied.p,nullptr));
        ok("A texture",d->CreateTexture(32,32,1,D3DUSAGE_RENDERTARGET,D3DFMT_A8R8G8B8,D3DPOOL_DEFAULT,&a.p,nullptr));
        ok("B texture",d->CreateTexture(32,32,1,D3DUSAGE_RENDERTARGET,D3DFMT_A8R8G8B8,D3DPOOL_DEFAULT,&b.p,nullptr));
        ok("copy surface",copied->GetSurfaceLevel(0,&copySurface.p));ok("A surface",a->GetSurfaceLevel(0,&aSurface.p));ok("B surface",b->GetSurfaceLevel(0,&bSurface.p));
        ...
        for(unsigned i=0;i<4;++i){
            ...
            ok("bloom PS",d->CreatePixelShader(static_cast<DWORD*>(pc->GetBufferPointer()),&bloom[i].p));pc.reset();
        }
        const D3DVERTEXELEMENT9 elements[]={{0,0,D3DDECLTYPE_FLOAT4,D3DDECLMETHOD_DEFAULT,D3DDECLUSAGE_POSITION,0},
            {0,16,D3DDECLTYPE_FLOAT2,D3DDECLMETHOD_DEFAULT,D3DDECLUSAGE_TEXCOORD,0},D3DDECL_END()};
        ok("scene declaration",d->CreateVertexDeclaration(elements,&decl.p));
        ok("scene VS bind",d->SetVertexShader(vs.p));ok("scene declaration bind",d->SetVertexDeclaration(decl.p));
        for(auto pair:{std::pair<D3DRENDERSTATETYPE,DWORD>{D3DRS_CULLMODE,D3DCULL_NONE},{D3DRS_ALPHABLENDENABLE,FALSE},
            {D3DRS_ALPHATESTENABLE,FALSE},{D3DRS_FOGENABLE,FALSE},{D3DRS_SRGBWRITEENABLE,FALSE},{D3DRS_ZFUNC,D3DCMP_LESS},
            {D3DRS_STENCILENABLE,FALSE},{D3DRS_SCISSORTESTENABLE,FALSE},{D3DRS_COLORWRITEENABLE,15}})
    ...
    ~Scene(){d->SetTexture(0,nullptr);d->SetVertexShader(nullptr);d->SetPixelShader(nullptr);d->SetVertexDeclaration(nullptr);}
    void viewport(UINT size){D3DVIEWPORT9 vp={0,0,size,size,0,1};ok("scene viewport",d->SetViewport(&vp));}
    void clear(x3m::SceneCapture& capture,DWORD flags){
        ...
        capture.after_clear(d,hr);ok("scene clear",hr);
    }
    struct V{float x,y,z,w,u,v;};
    ...
    void draw(x3m::SceneCapture& capture,IDirect3DPixelShader9* ps,bool writes,bool geometry=false){
        ok("draw PS",d->SetPixelShader(ps));ok("draw depth enable",d->SetRenderState(D3DRS_ZENABLE,writes));ok("draw depth write",d->SetRenderState(D3DRS_ZWRITEENABLE,writes));
        ...
        const auto kind=geometry?D3DPT_TRIANGLELIST:D3DPT_TRIANGLESTRIP;const UINT primitives=geometry?3:2;
        ok("draw BeginScene",d->BeginScene());capture.before_draw(d,kind,primitives);
    ...
    void prefix(x3m::SceneCapture& capture,bool failCopy,unsigned fillMode=0,bool badInitial=false){
        ...
        for(unsigned i=0;i<4;++i){
            ...
            hr=d->SetRenderTarget(0,surfaces[i]);capture.after_set_rt(d,0,hr);ok("bloom target",hr);
            viewport(i==3?64:32);ok("bloom input",d->SetTexture(0,textures[i]));draw(capture,bloom[i].p,false);
        }
...
static void exercise(IDirect3D9* api,HWND window,Compiler compiler,DWORD flags,Case test,std::uint64_t frame){
    ...
    pp.BackBufferWidth=64;pp.BackBufferHeight=64;pp.BackBufferFormat=D3DFMT_A8R8G8B8;pp.EnableAutoDepthStencil=TRUE;
    pp.AutoDepthStencilFormat=D3DFMT_D24X8;pp.PresentationInterval=D3DPRESENT_INTERVAL_IMMEDIATE;
    Com<IDirect3DDevice9> d;ok("create case device",api->CreateDevice(0,D3DDEVTYPE_HAL,window,flags,&pp,&d.p));
    {
        ...
        if(test==Case::GenerationMismatch){
            own::CopyDepthView old{},now{};ok("prior generation",own::get_copy_depth_view(d.p,&old));
        ...
        if(test==Case::FailedFinalClear){
            ...
            clearResult=D3DERR_INVALIDCALL;std::puts("INJECT final Clear callback D3DERR_INVALIDCALL");
        }else clearResult=d->Clear(0,nullptr,D3DCLEAR_ZBUFFER,0,1,0);
        if(test==Case::GenerationMismatch){
            ...
            own::CopyDepthView before{},after{};ok("pre-fault complete copy view",own::get_copy_depth_view(d.p,&before));
            expect("query fault starts from otherwise confirmable copy",before.available&&before.copy_valid&&before.source_bound&&SUCCEEDED(before.status)&&before.source_epoch==before.copy_epoch+1);
            auto* native=own::borrowed_native_device(d.p);expect("post-Clear native seam",native!=nullptr);
            void** original=*reinterpret_cast<void***>(native);
            {
                PostClearQueryFault fault(native);capture.after_clear(d.p,clearResult);
        ...
        if(test==Case::Positive||test==Case::ScratchFills){
            ...
            saved.capture(native,view.texture,compiler);saved.verify(native,"adapter-preserved-before-clear",false);
        }
        if(test==Case::FailedDrawAfter){
            ok("negative draw BeginScene",d->BeginScene());capture.before_draw(d.p,D3DPT_TRIANGLELIST,1);
            // Indexed drawing without an index buffer is a real invalid call.
            const HRESULT hr=d->DrawIndexedPrimitive(D3DPT_TRIANGLELIST,0,0,3,0,1);capture.after_draw(hr);
            expect("actual postselection indexed draw failed",FAILED(hr));ok("negative draw EndScene",d->EndScene());
        }
        ...
        }else{ok("actual Present",present);capture.end_frame(test==Case::FailedPresentAfter?D3DERR_DEVICELOST:present);}
        if(test==Case::FailedPresentAfter)std::puts("INJECT Present callback DEVICELOST after successful native Present");
        unsigned ends=0,confirmed=0,copies=0,validCopies=0,boundaries=0,boundaryConfirmed=0;
        for(const auto& line:trace){
            if(line.find("scene_depth_frame phase=end")!=std::string::npos){++ends;if(line.find("confirmed=1")!=std::string::npos)++confirmed;}
        ...
        if(test==Case::RejectedDepthCopy)expect("candidate copy failure not confirmed",copies==1&&validCopies==0);
        if(test==Case::GenerationMismatch)expect("retired successful copy never confirmed",copies==1&&validCopies==1&&boundaries==1);
...
int main(int argc,char** argv){
    std::setvbuf(stdout,nullptr,_IONBF,0);HWND window=nullptr;int result=1;
    try{
        if(argc!=3)throw std::runtime_error("expected D3DX and production decoder paths");
        std::ifstream shader(argv[2]);std::ostringstream source;source<<shader.rdbuf();decode_source=source.str();expect("production decoder source",!decode_source.empty());
        WNDCLASSA wc{};wc.lpfnWndProc=DefWindowProcA;wc.hInstance=GetModuleHandleA(nullptr);wc.lpszClassName="X3SceneCaptureFixture";
        RegisterClassA(&wc);window=CreateWindowA(wc.lpszClassName,"X3 original scene adapter fixture",WS_OVERLAPPEDWINDOW,0,0,128,128,nullptr,nullptr,wc.hInstance,nullptr);expect("hidden window",window!=nullptr);
        ...
        auto* native=create(D3D_SDK_VERSION);expect("native factory",native!=nullptr);Com<IDirect3D9> factory;
