...
int random_mode(unsigned seed, unsigned cases, unsigned points) {
    ...
    for (unsigned c = 0; c < cases; ++c) {
        ...
        v.x = kind == 3 ? unsigned(unit(rng) * 64) : 0;
        v.y = kind == 3 ? unsigned(unit(rng) * 64) : 0;
        v.width = 1 + unsigned(unit(rng) * 2047);
        v.height = 1 + unsigned(unit(rng) * 2047);
        Box box{};
        ...
        if (kind == 6) {
            ...
            const float m[16] = {float(s), 0, 0,   float(tx),     0, float(s), 0, float(ty),
                                 0,        0, .5f, float(.5 * d), 0, 0,        1, float(d)};
        ...
        if (reason == Reason::Bound) {
            for (unsigned n = 0; n < points; ++n) {
                ...
                for (unsigned k = 0; k < 4; ++k)
                    clip[k] = double(rows[4 * k]) * p[0] + double(rows[4 * k + 1]) * p[1] +
                              double(rows[4 * k + 2]) * p[2] + double(rows[4 * k + 3]);
                if (!(clip[3] > 0)) {
                    ++wneg;
                    continue;
                }
                const double sx = v.x + (clip[0] / clip[3] + 1) * v.width * .5,
                             sy = v.y + (1 - clip[1] / clip[3]) * v.height * .5;
                bool clip_flag = false;
                if (covered(rect, v, sx, sy, &clip_flag)) {
                    if (clip_flag)
                        ++clipped;
                    else
                        ++inside;
        ...
        std::printf(
            "CASE index=%u kind=%u jitter=%u reason=%u rect=%d,%d,%d,%d viewport=%u,%u,%u,%u inside=%u outside=%u clipped=%u w_nonpositive=%u\n",
            c, kind, jitter, unsigned(reason), rect.left, rect.top, rect.right, rect.bottom, v.x, v.y, v.width,
            v.height, inside, outside, clipped, wneg);
    }
...
int case_mode(int argc, char** argv) {
    if (argc < 2 + 16 + 3 + 3 + 4 + 3) {
        std::fprintf(stderr, "case arguments\n");
        return 2;
    }
    float rows[16];
    Box box{};
    ...
    auto number = [&](const char* text) {
        return std::string(text) == "nan"   ? std::numeric_limits<double>::quiet_NaN()
               : std::string(text) == "inf" ? std::numeric_limits<double>::infinity()
                                            : std::strtod(text, nullptr);
    };
    for (auto& r : rows) r = float(number(argv[i++]));
    for (auto& c : box.centre) c = number(argv[i++]);
    for (auto& h : box.half) h = number(argv[i++]);
    v.x = unsigned(std::strtoul(argv[i++], nullptr, 10));
    v.y = unsigned(std::strtoul(argv[i++], nullptr, 10));
    v.width = unsigned(std::strtoul(argv[i++], nullptr, 10));
    v.height = unsigned(std::strtoul(argv[i++], nullptr, 10));
    const bool rows_known = std::atoi(argv[i++]) != 0, bound_known = std::atoi(argv[i++]) != 0,
               fill = std::atoi(argv[i++]) != 0;
    ...
    std::printf("REGION bound=%u reason=%u pad=%u rect=%d,%d,%d,%d f=%.9f\n", region.bound, unsigned(region.reason),
                region.pad, region.rect.left, region.rect.top, region.rect.right, region.rect.bottom,
                area_fraction(region.rect, v));
    return 0;
}
...
bool read_span(std::uintptr_t address, void* out, std::size_t size) noexcept {
    ...
    for (const auto& s : spans)
        if (s.bytes && address >= s.base && address + size <= s.base + s.size) {
            std::memcpy(out, s.bytes + (address - s.base), size);
            return true;
        }
    return false;
}
std::uintptr_t scope_descriptor = descriptor;
std::uint32_t scope_depth = 1;
bool scope(std::uintptr_t* d, std::uint32_t* depth) noexcept {
    *d = scope_descriptor;
    *depth = scope_depth;
...
void put32(unsigned char* at, std::uint32_t v) {
    std::memcpy(at, &v, 4);
}
void reset_memory(unsigned records_count = 2, std::uintptr_t back = descriptor, std::uintptr_t match_vb = vb,
                  std::uintptr_t match_ib = ib) {
    std::memset(descriptor_bytes, 0, sizeof descriptor_bytes);
    std::memset(part_bytes, 0, sizeof part_bytes);
    std::memset(record_bytes, 0, sizeof record_bytes);
    put32(descriptor_bytes + 0, part);
    put32(descriptor_bytes + 4, 1u << 16);
    put32(descriptor_bytes + 8, records_count);
    put32(descriptor_bytes + 0xc, records);
    const std::int32_t aabb[7] = {4 * 100, 4 * -200, 4 * 50, 0,
                                  4 * 300, 4 * 150,  4 * 75}; // centre, pad, half (4 x int16 units)
    std::memcpy(part_bytes + 0x40, aabb, sizeof aabb);
    put32(part_bytes + 0x64, std::uint32_t(back));
    put32(record_bytes + 0 * 0x1a8 + 0xc, other_vb);
    put32(record_bytes + 0 * 0x1a8 + 0x10, other_ib);
    put32(record_bytes + 1 * 0x1a8 + 0xc, std::uint32_t(match_vb));
    put32(record_bytes + 1 * 0x1a8 + 0x10, std::uint32_t(match_ib));
    ...
    scope_depth = 1;
    contents[0] = {vb, 7, true};
    contents[1] = {ib, 3, true};
}
void print(const char* label, const Result& r, const BoundTable& t) {
    std::printf(
        "TABLE %s status=%u name=%s hit=%u poisoned_now=%u evicted=%u depth=%u part=%08x aabb=%d,%d,%d,%d,%d,%d vb_rev=%llu ib_rev=%llu centre=%.6f,%.6f,%.6f half=%.6f,%.6f,%.6f used=%u poisoned=%u evictions=%u reads=%u\n",
        label, unsigned(r.status), status_name(r.status), r.hit, r.poisoned_now, r.evicted, r.depth, unsigned(r.part),
        r.aabb[0], r.aabb[1], r.aabb[2], r.aabb[3], r.aabb[4], r.aabb[5], (unsigned long long)r.vb_revision,
        (unsigned long long)r.ib_revision, r.box.centre[0], r.box.centre[1], r.box.centre[2], r.box.half[0],
        r.box.half[1], r.box.half[2], t.used(), t.poisoned(), t.evictions(), reads);
    reads = 0;
}
int run() {
    ...
    print("wrapper_mismatch", t.resolve(Query{109, 209, other_vb, other_ib}, env), t); // same ids, other identities:
                                                                                       // poison, never a hit
    reset_memory();
    ...
    scope_descriptor = descriptor;
    spans[0] = {descriptor, sizeof descriptor_bytes, descriptor_bytes};
    put32(part_bytes + 0x64, std::uint32_t(descriptor));
    ...
    std::uint64_t ids[BoundTable::probe_window + 1];
    unsigned found = 0;
    const auto slot = [](std::uint64_t id) {
        return std::uint32_t((id * 0x9e3779b97f4a7c15ull) >> 32) & (BoundTable::capacity - 1);
    };
    const std::uint32_t target = slot(1000);
    for (std::uint64_t id = 1000; found < BoundTable::probe_window + 1 && id < 10000000; ++id)
        if (slot(id) == target) ids[found++] = id;
    BoundTable e;
    e.reserve();
    for (unsigned i = 0; i < BoundTable::probe_window; ++i) {
        const Result r = e.resolve(Query{ids[i], 202, vb, ib}, env);
        if (r.status != Status::Bound || r.evicted) return 4;
    }
    contents[0].revision = 9;
    (void)e.resolve(Query{ids[1], 202, vb, ib}, env);
    contents[0].revision = 7;                                                                     // poison ids[1]
    print("peek_window_full", e.peek(Query{ids[BoundTable::probe_window], 202, vb, ib}, env), e); // a miss on a full
                                                                                                  // window derives
                                                                                                  // without evicting
    print("window_full", e.resolve(Query{ids[BoundTable::probe_window], 202, vb, ib}, env), e);   // evicts ids[0], the
                                                                                                  // oldest unpoisoned
    ...
    reset_memory();
    e.clear();
    print("cleared", e.resolve(query, env), e);
    return 0;
}
} // namespace table
...
std::vector<float> buffer(std::size_t vertices) {
    return std::vector<float>(vertices * stride / sizeof(float));
}
void put(std::vector<float>& b, std::size_t i, float x, float y, float z) {
    b[i * 6] = x;
    b[i * 6 + 1] = y;
    b[i * 6 + 2] = z;
}
...
Box extent(const float* positions, std::uint32_t count) {
    ...
    for (unsigned a = 0; a < 3; ++a) {
        box.centre[a] = (lo[a] + hi[a]) / 2;
        box.half[a] = (hi[a] - lo[a]) / 2;
    }
    return box;
}
struct Look {
...
Look look(const Table& t, std::uintptr_t key, std::uint32_t count) {
    Look l;
    l.status = t.lookup(key, count, &l.positions, &l.revision, &l.scanned);
    return l;
}
void print(const char* label, const Look& l, std::uint32_t count, const Table& t, unsigned extra = 0) {
    ...
    std::printf(
        "PREFIX %s status=%u name=%s box=%.6f,%.6f,%.6f,%.6f,%.6f,%.6f scanned=%u revision=%llu used=%u evictions=%llu publications=%llu sentinel_bytes=%llu window_end=%llu extra=%u\n",
        label, unsigned(l.status), lookup_name(l.status), box.centre[0], box.centre[1], box.centre[2], box.half[0],
        box.half[1], box.half[2], l.scanned, (unsigned long long)l.revision, t.used(),
        (unsigned long long)t.evictions(), (unsigned long long)t.publications(), (unsigned long long)t.sentinel_bytes(),
        (unsigned long long)t.window_end_scans(), extra);
}
// Fake production binding for resolve_locked_prefix: one published record.
Table* env_table = nullptr;
bool env_prefix(std::uintptr_t wrapper, std::uint32_t vertex_count, const float** positions, std::uint32_t* scanned,
...
int run(unsigned seed, unsigned cases) {
    ...
    print("unmarked_lock_ignored",
          (t.begin_lock(1, true, b.data(), b.size() * sizeof(float), 7), t.finish_lock(1, 7), look(t, 1, 96)), 96, t,
          unsigned(b[0] == 0.f));
    t.mark(1);
    print("marked_unknown", look(t, 1, 96), 96, t);
    t.begin_lock(1, true, b.data(), b.size() * sizeof(float), 7);
    std::uint32_t words[3];
    std::memcpy(words, b.data() + 299 * 6, sizeof words);
    print("math_pending", look(t, 1, 96), 96, t,
          unsigned(words[0] == sentinel_word && words[2] == sentinel_word)); // whole window sentinelled
    write_math();
    ...
    t.begin_lock(1, false, nullptr, 0, 7);
    t.finish_lock(1, 7);
    ...
    t.mark(3);
    t.begin_lock(3, true, b.data(), b.size() * sizeof(float), 7);
    t.begin_lock(3, true, b.data(), b.size() * sizeof(float), 7);
    t.finish_lock(3, 7);
    ...
    auto prefix = [&] {
        for (unsigned i = 0; i < 100; ++i) {
            const float v = i % 2 ? 1.f : -1.f;
            put(g, i, v, v, v);
        }
    };
    auto tail = [&](float value) {
        for (unsigned i = 100; i < 300; ++i) put(g, i, value, value, value);
    };
    const auto publish = [&](std::uintptr_t key, auto&& writer) {
        t.mark(key);
        t.begin_lock(key, true, g.data(), g.size() * sizeof(float), 7);
        writer();
        return t.finish_lock(key, 7);
    };
    ...
    n = publish(2, [&] {
        prefix();
        tail(std::numeric_limits<float>::quiet_NaN());
    });
    ...
    n = publish(2, [&] {
        prefix();
        tail(std::numeric_limits<float>::infinity());
    });
    ...
    n = publish(2, [&] {
        prefix();
        put(g, 5, std::numeric_limits<float>::quiet_NaN(), 0, 0);
    });
    ...
    n = publish(2, [&] {
        prefix();
        std::uint32_t s[3] = {sentinel_word, sentinel_word, sentinel_word};
        std::memcpy(g.data() + 50 * 6, s, sizeof s);
    });
    ...
    {
        const std::uint64_t s0 = t.sentinel_bytes();
        n = publish(2, [&] { prefix(); });
        const std::uint64_t s1 = t.sentinel_bytes();
        print("sentinel_extent_after_50", look(t, 2, 100), 100, t, unsigned(s1 - s0)); // previous scan: 50 vertices ->
                                                                                       // 1200 bytes
        const std::uint64_t s2 = t.sentinel_bytes();
        publish(2, [&] { prefix(); });
    ...
    print("length_partial",
          Look{Lookup(scan(g.data(), 10 * stride + 7, positions.data(), &partial)), nullptr, partial.bad_from,
               partial.vertices},
          0, t, unsigned(partial.window_end));
    Scan capped{};
    auto big = buffer(max_vertices + 10);
    print("length_capped",
          Look{Lookup(scan(big.data(), big.size() * sizeof(float), positions.data(), &capped)), nullptr,
               capped.bad_from, capped.vertices},
          0, t, unsigned(capped.window_end));
    // Eviction: capacity + 1 distinct keys; the oldest (key 100) goes; the
    // storage pool stays at capacity blocks.
    Table e;
    for (unsigned k = 0; k <= Table::capacity; ++k) {
        e.mark(100 + k);
        e.begin_lock(100 + k, true, b.data(), b.size() * sizeof(float), 7);
        write_math();
        e.finish_lock(100 + k, 7);
    }
    ...
    Table r;
    r.mark(0xd000);
    r.begin_lock(0xd000, true, b.data(), b.size() * sizeof(float), 7);
    write_math();
    r.finish_lock(0xd000, 7);
    env_table = &r;
    ...
    auto res = [&](const char* label, const x3m::fade_region::Result& out) {
        ...
        std::printf(
            "RESOLVE %s status=%u name=%s source=%u bound=%u refusal=%u scanned=%u revision=%llu positions=%u box=%.6f,%.6f,%.6f,%.6f,%.6f,%.6f\n",
            label, unsigned(out.status), x3m::fade_region::status_name(out.status), unsigned(out.source),
            out.status == x3m::fade_region::Status::Bound, out.prefix_refusal, out.scanned,
            (unsigned long long)out.vb_revision, out.positions != nullptr, box.centre[0], box.centre[1], box.centre[2],
            box.half[0], box.half[1], box.half[2]);
    };
    ...
    res("no_binding",
        x3m::fade_region::resolve_locked_prefix(x3m::fade_region::Query{5, 0, 0xd000, 0}, 200, no_prefix));
    ...
    for (unsigned c = 0; c < cases; ++c) {
        ...
        for (unsigned a = 0; a < 3; ++a) {
            centre[a] = unit(rng) * 2000 - 1000;
            half[a] = unit(rng) * 100;
        }
        const float tail = float(unit(rng) * 4000 - 2000);
        Table s;
        s.mark(9);
        s.begin_lock(9, true, v.data(), v.size() * sizeof(float), 1);
        ...
        for (unsigned i = 0; i < count; ++i) {
            float p[3];
            for (unsigned a = 0; a < 3; ++a) {
                p[a] = float(centre[a] + (unit(rng) * 2 - 1) * half[a]);
        ...
        for (unsigned i = 0; i < count && ok; ++i)
            for (unsigned a = 0; a < 3; ++a)
                ok = ok && std::memcmp(&l.positions[i * 3 + a], &v[i * 6 + a], sizeof(float)) == 0;
        if (ok) {
            const Box box = extent(l.positions, count);
            for (unsigned a = 0; a < 3; ++a)
                ok = ok && box.centre[a] - box.half[a] == double(lo[a]) && box.centre[a] + box.half[a] == double(hi[a]);
        }
        if (ok && look(s, 9, count + 1).status != (recycled ? Lookup::Bound : Lookup::Beyond))
            ok = false; // one past the prefix
        if (!ok)
            ++failures;
        else if (recycled)
            ++garbage;
    ...
    auto time_scan = [&](std::uint32_t written, bool sentinel_tail) {
        ...
        s.begin_lock(9, true, v.data(), v.size() * sizeof(float), 1);
        if (!sentinel_tail)
            for (unsigned i = 0; i < max_vertices; ++i) put(v, i, 1, 2, 3);
        ...
        for (unsigned i = 0; i < iterations; ++i) {
            v[i % 6] = float(i);
            scan(v.data(), v.size() * sizeof(float), positions.data(), &timing);
            sink = sink + positions[3];
        }
        const double ns = double(std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::steady_clock::now() -
                                                                                      begin)
                                     .count()) /
                          iterations;
        s.finish_lock(9, 1);
    ...
    std::printf(
        "PREFIX_RANDOM cases=%u failures=%u exact=%u garbage=%u scan_ns_1056=%.0f vertices_1056=%u scan_ns_window=%.0f vertices_window=%u\n",
...
int near_mode(unsigned seed, unsigned cases, unsigned points) {
    ...
    for (unsigned c = 0; c < cases; ++c) {
        ...
        Box box{};
        for (unsigned a = 0; a < 3; ++a) {
            box.centre[a] = unit(rng) * 8 - 4;
            box.half[a] = unit(rng) * 3;
        }
        ...
        const double tx = unit(rng) * 4 - 2, ty = unit(rng) * 4 - 2;
        const float rows[16] = {
            float(s), 0, 0,        float(tx - s * world),           0, float(s), 0, float(ty - s * world),
        ...
        for (unsigned corner = 0; corner < 8; ++corner) {
            double p[3];
            for (unsigned a = 0; a < 3; ++a)
                p[a] = box.centre[a] + ((corner >> a) & 1u ? 1 : -1) * (box.half[a] + half_float_expansion);
        ...
        if (behind_corners == 0 && (reason != plain_reason || std::memcmp(&rect, &plain, sizeof rect) != 0))
            fail = true;
        ...
        if (reason == Reason::Bound) {
            for (unsigned n = 0; n < points; ++n) {
                ...
                for (unsigned kk = 0; kk < 4; ++kk)
                    clip[kk] = double(rows[4 * kk]) * p[0] + double(rows[4 * kk + 1]) * p[1] +
                               double(rows[4 * kk + 2]) * p[2] + double(rows[4 * kk + 3]);
                if (clip[2] < 0) {
                    ++invisible;
                    continue;
                } // behind the near plane: D3D never rasterises it
                if (!(clip[3] > 0)) {
                    fail = true;
                    continue;
                }
                ...
                for (unsigned kk = 0; kk < 4; ++kk) {
                    if (kk == 2) continue;
                    const double sum = std::fabs(double(rows[4 * kk]) * p[0]) +
                                       std::fabs(double(rows[4 * kk + 1]) * p[1]) +
                ...
                for (int sx_sign = -1; sx_sign <= 1; sx_sign += 2)
                    for (int sy_sign = -1; sy_sign <= 1; sy_sign += 2)
                        for (int sw_sign = -1; sw_sign <= 1; sw_sign += 2) {
                            ...
                            if (!covered(rect, v, qx, qy, &q_clip)) {
                                ++outside_fp32;
                                if (pad < pad_limit) fail = true;
                            }
                        }
            }
        }
        ...
        if (fail) ++failures;
        std::printf(
            "NEAR index=%u reason=%u plain_reason=%u behind=%u clipped=%u pad=%u rect=%d,%d,%d,%d viewport=%u,%u inside=%u outside=%u outside_fp32=%u capped=%u clipped_points=%u invisible=%u fail=%u\n",
            c, unsigned(reason), unsigned(plain_reason), behind_corners, cut.clipped, pad, rect.left, rect.top,
            rect.right, rect.bottom, v.width, v.height, inside, outside, outside_fp32, unsigned(pad >= pad_limit),
            clipped, invisible, unsigned(fail));
    }
    std::printf("RESULT cases=%u failures=%u\n", cases, failures);
    return failures ? 1 : 0;
}
...
int near_case_mode(int argc, char** argv) {
    if (argc < 2 + 16 + 3 + 3 + 4) {
        std::fprintf(stderr, "near-case arguments\n");
        return 2;
    }
    float rows[16];
    Box box{};
    ...
    std::printf("REGION bound=%u reason=%u clipped=%u pad=%u rect=%d,%d,%d,%d f=%.9f\n", region.bound,
                unsigned(region.reason), region.clipped, region.pad, region.rect.left, region.rect.top,
                region.rect.right, region.rect.bottom, area_fraction(region.rect, v));
    return 0;
}
...
std::vector<Point> visible_polygon(const float rows[16], const float* tri, const Viewport& v, unsigned* behind) {
    ...
    for (unsigned i = 0; i < poly.size(); ++i) {
        const Clip& a = poly[i];
        const Clip& b = poly[(i + 1) % poly.size()];
...
struct Footprint {
    std::uint64_t covered = 0, outside = 0;
    std::int32_t min_x = 0, min_y = 0, max_x = -1, max_y = -1;
};
...
void rasterise(const std::vector<Point>& poly, const Viewport& v, const Rect& rect, std::mt19937_64& rng,
               Footprint* fp) {
    ...
    const std::int32_t x0 = std::int32_t(std::floor(std::max(lo_x, vx0))),
                       x1 = std::int32_t(std::ceil(std::min(hi_x, vx1)));
    const std::int32_t y0 = std::int32_t(std::floor(std::max(lo_y, vy0))),
                       y1 = std::int32_t(std::ceil(std::min(hi_y, vy1)));
    ...
    if (pixels <= 40000) {
        for (std::int32_t y = y0; y < y1; ++y)
            for (std::int32_t x = x0; x < x1; ++x) test(x, y);
        return;
    }
    ...
    for (const Point& p : poly)
        for (int ox = -2; ox <= 2; ++ox)
            for (int oy = -2; oy <= 2; ++oy)
                test(std::int32_t(std::floor(p.x)) + ox, std::int32_t(std::floor(p.y)) + oy);
}
int run(unsigned seed, unsigned cases, unsigned triangles_max) {
    ...
    for (unsigned c = 0; c < cases; ++c) {
        Viewport v{0, 0, 16 + unsigned(unit(rng) * 2032), 16 + unsigned(unit(rng) * 1064)};
        if (c % 5 == 4) {
            v.x = unsigned(unit(rng) * 40);
            v.y = unsigned(unit(rng) * 40);
        }
        // x' = s x + tx, y' = s y + ty, z' = k (z + d - zn), w = z + d: near plane at w = zn.
        const double s = 0.25 + unit(rng) * 8, d = unit(rng) * 10 - 5, zn = 0.01 + unit(rng) * 3, k = 0.05 + unit(rng);
        ...
        if (kind == 2) {
            centre[2] = -d - zn - 5 - unit(rng) * 5;
            half[2] = std::min(half[2], 4.0);
        }
        if (kind == 3) {
            centre[2] = -d + zn;
            half[2] = 0.02 + unit(rng) * 0.2;
        }
        for (unsigned t = 0; t < triangles; ++t) {
            double base[3];
            const double along = unit(rng);
            for (unsigned a = 0; a < 3; ++a)
                base[a] = kind == 1 ? centre[a] - half[a] + 2 * half[a] * along
                                    : centre[a] + (unit(rng) * 2 - 1) * half[a];
            const double size = kind == 1 ? 0.05 + unit(rng) * 0.2 : 0.02 + unit(rng) * 1.5;
            for (unsigned k2 = 0; k2 < 3; ++k2)
                for (unsigned a = 0; a < 3; ++a)
                    positions[t * 9 + k2 * 3 + a] = float(base[a] + world * (a < 3) + (unit(rng) * 2 - 1) * size);
        }
        Rect rect{};
        PrefixHull hull{};
        ...
        if (reason == Reason::Bound) {
            ...
            if (box_reason != Reason::Bound)
                fail = true;
            else {
                aabb_px = area(box_rect);
                const bool fallback = area(rect) == 1 && rect.left == std::int32_t(v.x) &&
                                      rect.top == std::int32_t(v.y) && fp.covered == 0;
                if (!fallback && (rect.left < box_rect.left || rect.top < box_rect.top || rect.right > box_rect.right ||
        ...
        capped += at_cap;
        std::printf(
            "HULL index=%u kind=%u triangles=%u reason=%u behind=%u clipped=%u pad=%u capped=%u rect=%d,%d,%d,%d hull_px=%llu aabb_px=%llu viewport=%u,%u covered=%llu outside=%llu footprint=%d,%d,%d,%d fail=%u\n",
            c, kind, triangles, unsigned(reason), behind, hull.behind, hull.pad, unsigned(at_cap), rect.left, rect.top,
            rect.right, rect.bottom, (unsigned long long)(reason == Reason::Bound ? area(rect) : 0),
            (unsigned long long)aabb_px, v.width, v.height, (unsigned long long)fp.covered,
            (unsigned long long)fp.outside, fp.min_x, fp.min_y, fp.max_x + 1, fp.max_y + 1, unsigned(fail));
    }
...
int case_mode(int argc, char** argv) {
    if (argc < 2 + 16 + 4 + 1) {
        std::fprintf(stderr, "hull-case arguments\n");
        return 2;
    }
    float rows[16];
    Viewport v{};
    ...
    if (argc < i + int(count) * 3) {
        std::fprintf(stderr, "hull-case positions\n");
        return 2;
    }
    ...
    Rect box_rect{};
    NearClip cut{true, 0};
    const Reason box_reason = count && count % 3 == 0 ? project_box(rows, hull.aabb, v, &box_rect, &cut)
                                                      : Reason::BoundUnknown;
    std::printf(
        "HULL_CASE bound=%u reason=%u clipped=%u pad=%u rect=%d,%d,%d,%d f=%.9f hull_px=%llu aabb_reason=%u aabb_rect=%d,%d,%d,%d aabb_px=%llu covered=%llu outside=%llu footprint=%d,%d,%d,%d\n",
        region.bound, unsigned(region.reason), region.clipped, region.pad, region.rect.left, region.rect.top,
        region.rect.right, region.rect.bottom, area_fraction(region.rect, v),
        (unsigned long long)(region.bound ? area(region.rect) : 0), unsigned(box_reason), box_rect.left, box_rect.top,
        box_rect.right, box_rect.bottom, (unsigned long long)(box_reason == Reason::Bound ? area(box_rect) : 0),
        (unsigned long long)fp.covered, (unsigned long long)fp.outside, fp.min_x, fp.min_y, fp.max_x + 1, fp.max_y + 1);
    return 0;
}
} // namespace hull_mode
...
int table(int, char**) {
    ...
    for (const auto& row : x3m::fade_route::vertex_programs)
        std::printf("ROW vs=%016llx alpha=%u fog=%u\n", (unsigned long long)row.hash, row.registers.alpha,
                    row.registers.fog);
    return 0;
}
...
int fraction(int argc, char** argv) {
    ...
    const float m00 = std::strtof(argv[23], nullptr), m11 = std::strtof(argv[24], nullptr),
                m20 = std::strtof(argv[25], nullptr), m21 = std::strtof(argv[26], nullptr);
    ...
    std::printf("FRACTION ok=%u distance=%.9g fraction=%.9g permille=%u admit=%u\n", ok, double(distance), double(f),
                permille, ok && x3m::fade_route::admit(permille, threshold));
    return 0;
}
...
int hysteresis(int argc, char** argv) {
    ...
    for (int i = 3; i < argc; ++i) {
        unsigned long long key = 0, frame = 0;
        unsigned permille = 0;
        ...
        std::printf("ADMIT step=%d key=%llu frame=%llu permille=%u admit=%u held=%u entries=%u evicted=%u\n", i - 3,
                    key, frame, permille, admitted, held, table.count, unsigned(table.evicted));
    }
    return 0;
}
} // namespace fade_route_mode
...
int main(int argc, char** argv) {
    ...
    if (argc >= 3 && std::strcmp(argv[1], "--fade-route-hysteresis") == 0)
        return fade_route_mode::hysteresis(argc, argv);
    if (argc >= 5 && std::strcmp(argv[1], "--hull") == 0)
        return hull_mode::run(unsigned(std::strtoul(argv[2], nullptr, 10)),
                              unsigned(std::strtoul(argv[3], nullptr, 10)),
                              unsigned(std::strtoul(argv[4], nullptr, 10)));
    if (argc >= 2 && std::strcmp(argv[1], "--hull-case") == 0) return hull_mode::case_mode(argc, argv);
    if (argc >= 5 && std::strcmp(argv[1], "--near") == 0)
        return near_mode(unsigned(std::strtoul(argv[2], nullptr, 10)), unsigned(std::strtoul(argv[3], nullptr, 10)),
                         unsigned(std::strtoul(argv[4], nullptr, 10)));
    ...
    if (argc >= 4 && std::strcmp(argv[1], "--prefix") == 0)
        return prefix_mode::run(unsigned(std::strtoul(argv[2], nullptr, 10)),
                                unsigned(std::strtoul(argv[3], nullptr, 10)));
    if (argc >= 5 && std::strcmp(argv[1], "--random") == 0)
        return random_mode(unsigned(std::strtoul(argv[2], nullptr, 10)), unsigned(std::strtoul(argv[3], nullptr, 10)),
                           unsigned(std::strtoul(argv[4], nullptr, 10)));
    if (argc >= 2 && std::strcmp(argv[1], "--case") == 0) return case_mode(argc, argv);
    std::fprintf(
        stderr,
        "usage: --random seed cases points | --case ... | --near seed cases points | --near-case ... | --hull seed cases triangles | --hull-case ... | --table | --prefix seed cases\n");
    return 2;
}
