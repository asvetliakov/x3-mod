#!/usr/bin/env python3
"""Run the Terran-station LOD patch fixture in the X3 bottle and write a compact record.

Invoke only as:
  X3M_FIXTURE_BOTTLE=X3 python3 verification/probe/wine_lock.py python3 verification/probe/run_terran_lod_patch.py

Builds the fixture first (build_terran_lod_patch.py, host only, into the untracked
build/verification/terran-lod-patch/), runs it once, keeps its stdout beside the
executable and writes verification/results/bottle-X3/terran-lod-patch.json:
bottle, toolchain and build audit, every CHECK with its outcome, the executed
branch per memory and step, the protection seen around the write, the
FlushInstructionCache experiment, and the install/restore log rows parsed with
verify_terran_lod_site's parsers. Never launches the game; the game EXE is not read.
"""
import hashlib
import json
import os
...
import bottle
import build_terran_lod_patch as build
import verify_terran_lod_site as verifier
ROOT = Path(__file__).resolve().parents[2]
NAME = 'terran-lod-patch.json'


PRODUCTION_SOURCES = ('src/proxy/terran_station_lod.cpp', 'src/proxy/terran_station_lod.h', 'src/proxy/terran_lod_sites.h',
    ...
    for line in stdout.splitlines():
        ...
        if tag == 'CHECK':
            ...
            report['branches'].append({'memory': f['memory'], 'step': f['step'],
                                       'flag31': None if f['flag31'] == '-' else int(f['flag31']), 'clear': None if f['clear'] == '-' else int(f['clear'])})
        elif tag == 'PROTECT':
...
def main():
    ...
    (build.BUILD / 'terran-lod-patch.stdout').write_text(run.stdout)
    (build.BUILD / 'terran-lod-patch.stderr').write_text(run.stderr)
    ...
    record = {'fixture': 'verification/probe/terran_lod_patch_fixture.cpp', 'runner': 'verification/probe/run_terran_lod_patch.py',
