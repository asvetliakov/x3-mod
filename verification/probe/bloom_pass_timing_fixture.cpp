...
struct DiskPrograms {
    ...
    explicit DiskPrograms(const std::string& directory) {
        BloomBytecode* slots[] = {&bundle.vertex,&bundle.extract[0],&bundle.extract[1],
            &bundle.extract[2],&bundle.extract[3],&bundle.extract[4],&bundle.extract[5],
            &bundle.down,&bundle.up,&bundle.candidate,&bundle.sharpen,&bundle.copy};
...
HRESULT WINAPI timed_draw(IDirect3DDevice9* device,D3DPRIMITIVETYPE type,UINT count,
                          const void* vertices,UINT stride) {
    if(!draw_timer.armed||draw_timer.captured)
        return draw_timer.native(device,type,count,vertices,stride);
...
Case timing_case(unsigned width,unsigned height) {
    Case c{};c.w=width;c.h=height;c.mode=0;c.strength=1;c.sharp=0;
    c.threshold=1;c.exposure=1;c.authored_glow_gain=.1f;c.highlight_gain=.05f;
...
struct TimingImage {
    ...
    TimingImage(IDirect3DDevice9* device,unsigned width,unsigned height)
        ...
        boundary.depth=nullptr;boundary.frame=1;boundary.reset=1;
        boundary.thread=GetCurrentThreadId();boundary.admitted=true;
    }
...
BloomPrepare timing_input(TimingImage& image,bool authored) {
    BloomPrepare input{};input.scene=image.scene.texture.p;input.boundary=image.boundary;
    input.decode=x3::temporal::AgxDecode::gamma22;input.filter.levels=5;
    input.filter.threshold=1;input.filter.knee=.5f;input.filter.scatter=.7f;
    input.filter.strength=authored?1.f:.05f;
    input.filter.authored_glow_gain=authored?.1f:0.f;
    input.filter.highlight_gain=.05f;
    require(x3::temporal::prepare(input.agx,1,0,input.decode,x3::temporal::AgxLook::none),
            "Timing AgX constants");
    return input;
}

HRESULT attach_report(BloomPass& pass,IDirect3DDevice9* device,void* const* native,
                      const D3DCAPS9& caps,const BloomPrograms& programs,
    ...
    std::printf("CREATE_PS metric=%s bundle=%s accepted=%u attach_hr=%08lx programs_hr=%08lx\n",
        metric,label,SUCCEEDED(hr)&&pass.enabled()?1u:0u,(unsigned long)hr,
        (unsigned long)pass.caps().programs);
    return hr;
}
...
void validate_result(const BloomPreparation& prepared,const BloomCommit& committed) {
    ...
    require(committed.committed&&committed.state_preserved&&committed.write_attempted,
            "Timing commit");
}

LONGLONG measure_full(IDirect3DDevice9* device,BloomPass& pass,BloomPrepare& input,
                      TimingImage& image,IDirect3DQuery9* fence) {
    image.bind(device);++image.boundary.frame;input.boundary=image.boundary;
    check(device->BeginScene(),"Timing BeginScene");
    check(complete_fence(fence),"Timing leading fence");
    LARGE_INTEGER begin{},end{};require(QueryPerformanceCounter(&begin)!=FALSE,"Timing QPC begin");
...
LONGLONG measure_extract(IDirect3DDevice9* device,BloomPass& pass,BloomPrepare& input,
                         TimingImage& image,IDirect3DQuery9* fence) {
    image.bind(device);++image.boundary.frame;input.boundary=image.boundary;
    draw_timer.fence=fence;draw_timer.armed=true;draw_timer.captured=false;
    draw_timer.failure=S_OK;draw_timer.ticks=0;
    ...
    validate_result(prepared,committed);
    require(draw_timer.captured&&!draw_timer.armed&&SUCCEEDED(draw_timer.failure)
            &&draw_timer.ticks>0,"Extraction draw timing");
...
void paired_batch(const char* metric,unsigned width,unsigned height,LONGLONG frequency,
                  BloomPass& old_pass,BloomPass& new_pass,BloomPrepare& old_input,
                  BloomPrepare& new_input,TimingImage& image,IDirect3DDevice9* device,
                  IDirect3DQuery9* fence,Measure measure) {
    for(unsigned pair=0;pair<kWarmupPairs;++pair) {
        if(pair%2==0) {measure(device,old_pass,old_input,image,fence);measure(device,new_pass,new_input,image,fence);}
        else {measure(device,new_pass,new_input,image,fence);measure(device,old_pass,old_input,image,fence);}
    }
    ...
    for(unsigned pair=0;pair<kMeasuredPairs;++pair) {
        ...
        old_values.push_back(old_ticks);new_values.push_back(new_ticks);
        std::printf("SAMPLE metric=%s width=%u height=%u pair=%u order=%s old_ticks=%lld old_ms=%.6f new_ticks=%lld new_ms=%.6f\n",
            metric,width,height,pair,old_first?"old-new":"new-old",
            (long long)old_ticks,milliseconds(old_ticks,frequency),
            (long long)new_ticks,milliseconds(new_ticks,frequency));
    }
    const double old_median=median_ms(old_values,frequency),new_median=median_ms(new_values,frequency);
    std::printf("SUMMARY metric=%s width=%u height=%u warmup_pairs=%u measured_pairs=%u old_median_ms=%.6f new_median_ms=%.6f ratio=%.6f\n",
        metric,width,height,kWarmupPairs,kMeasuredPairs,old_median,new_median,
        old_median>0?new_median/old_median:0.);
}

void dimension(IDirect3DDevice9* device,const D3DCAPS9& caps,void* const* actual,
               const DiskPrograms& old_programs,const DiskPrograms& new_programs,
               unsigned width,unsigned height,LONGLONG frequency) {
    TimingImage image(device,width,height);
    Com<IDirect3DQuery9> fence;check(device->CreateQuery(D3DQUERYTYPE_EVENT,&fence.p),"Create EVENT query");
    auto old_input=timing_input(image,false),new_input=timing_input(image,true);
    {
        ...
        paired_batch("full",width,height,frequency,old_pass,new_pass,old_input,new_input,
                     image,device,fence.p,measure_full);
        old_pass.shutdown();new_pass.shutdown();
    }
    {
        void* hooked[119];std::memcpy(hooked,actual,sizeof(hooked));
        draw_timer.native=reinterpret_cast<NativeDraw>(actual[83]);
        hooked[83]=reinterpret_cast<void*>(&timed_draw);
        BloomPass old_pass,new_pass;
        check(attach_report(old_pass,device,hooked,caps,old_programs.bundle,"extract","old"),"Attach old extract");
        check(attach_report(new_pass,device,hooked,caps,new_programs.bundle,"extract","new"),"Attach new extract");
        paired_batch("extract_first_draw",width,height,frequency,old_pass,new_pass,
                     old_input,new_input,image,device,fence.p,measure_extract);
        old_pass.shutdown();new_pass.shutdown();
    }
...
int main(int argc,char** argv) {
    try {
        ...
        LARGE_INTEGER frequency{};require(QueryPerformanceFrequency(&frequency)!=FALSE
            &&frequency.QuadPart>0,"QPC frequency");
        HMODULE module=LoadLibraryA("d3d9.dll");require(module,"Load D3D9");
        ...
        require(create,"D3D symbol");Com<IDirect3D9> api;api.p=create(D3D_SDK_VERSION);
        require(api.p,"Create D3D9");
        HWND window=CreateWindowA("STATIC","Bloom timing fixture",WS_OVERLAPPEDWINDOW,
            0,0,128,128,nullptr,nullptr,GetModuleHandleA(nullptr),nullptr);require(window,"Window");
        ...
        check(api->CreateDevice(0,D3DDEVTYPE_HAL,window,D3DCREATE_HARDWARE_VERTEXPROCESSING,
                                &pp,&device.p),"Create timing device");
        D3DCAPS9 caps{};check(device->GetDeviceCaps(&caps),"Timing caps");
        ...
        void** vtable=*reinterpret_cast<void***>(device.p);void* actual[119];
