...
constexpr HRESULT S_OK = 0, S_FALSE = 1, E_FAIL = -1, E_ABORT = -2, E_NOINTERFACE = -3, D3DERR_DEVICELOST = -4,
                  D3DERR_DEVICENOTRESET = -5, D3DERR_NOTAVAILABLE = -6, E_INVALIDARG = -7;
...
struct IDirect3DTexture9 : IUnknown {
    ...
    HRESULT GetLevelDesc(UINT, D3DSURFACE_DESC* out) {
        *out = desc;
        return desc_hr;
    }
    HRESULT GetDevice(IDirect3DDevice9** out) {
        *out = nullptr;
        if (device && (SUCCEEDED(device_hr) || output_on_failure)) {
            device->AddRef();
            *out = device;
        }
        return device_hr;
...
struct IDirect3DSurface9 : IUnknown {
    explicit IDirect3DSurface9(IDirect3DTexture9* source = nullptr)
        : texture(source) {}
    ...
    HRESULT GetContainer(int, void** out) {
        *out = nullptr;
        if (texture && (SUCCEEDED(container_hr) || output_on_failure)) {
            texture->AddRef();
            *out = texture;
        }
        return container_hr;
...
inline HRESULT IDirect3DTexture9::GetSurfaceLevel(UINT, IDirect3DSurface9** out) {
    *out = nullptr;
    if (level && (SUCCEEDED(level_hr) || output_on_failure)) {
        level->AddRef();
        *out = level;
    }
    return level_hr;
