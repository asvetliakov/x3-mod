...
struct ReadableSession {
    ...
    ReadableSession(Create create, HWND window, UINT budget = 0, UINT limit = 4096) {
        ...
        check(native != nullptr, "readable factory");
        Options options;
        options.track_buffer_writes = true;
        options.track_buffer_lock_attempts = true;
        options.track_execution_state = true;
        options.prepare_readable_managed_uploads = true;
    ...
    FiniteUploadStatistics statistics() {
        FiniteUploadStatistics result;
        ok(get_finite_upload_statistics(device.p, &result), "readable statistics");
        return result;
    }
    void empty(unsigned sidecars) {
        ...
        check(!s.payload_bytes && !s.peak_payload_bytes && !s.global_payload_bytes && !s.scans && !s.classified_bytes &&
                  !s.publications && !s.uploads,
              "zero atlas allocation and typed payload work");
    }
    void close() {
        empty(0);
        check(device.p->Release() == 0, "readable device no retained references");
        device.p = nullptr;
        check(factory.p->Release() == 0, "readable factory no retained references");
        factory.p = nullptr;
    }
};
void metadata(Create create, HWND window) {
    ...
    description(vb.p);
    D3DINDEXBUFFER_DESC app{}, native{};
    ...
    app.Usage = native.Usage;
    check(!std::memcmp(&app, &native, sizeof app), "IB remaining descriptor unchanged");
    s.empty(2);
    ...
    const std::uint16_t i[6] = {2, 1, 0, 0, 1, 2};
    void* map = nullptr;
    ok(vb->Lock(0, 0, &map, 0x800), "original full VB Lock");
    std::memcpy(map, v, sizeof v);
    ok(vb->Unlock(), "original VB Unlock");
    ok(ib->Lock(0, 0, &map, 0x800), "original full IB Lock");
    std::memcpy(map, i, sizeof i);
    ok(ib->Unlock(), "original IB Unlock");
    s.empty(2);
    ...
    check(!std::memcmp(map, v, sizeof v), "exact VB payload preserved");
    ok(vb->Unlock(), "fixture verify VB Unlock");
    ok(ib->Lock(0, 0, &map, D3DLOCK_READONLY), "fixture verify IB Lock");
    check(!std::memcmp(map, i, sizeof i), "exact IB payload preserved");
    ok(ib->Unlock(), "fixture verify IB Unlock");
    s.empty(2);
    const auto old = s.statistics().generation;
    const HRESULT reset = s.device->Reset(&s.pp);
    std::printf("RESET hr=%08lx\n", static_cast<unsigned long>(reset));
    ok(reset, "readable metadata Reset");
    check(s.statistics().generation > old, "Reset advances metadata generation");
    description(vb.p);
    s.empty(2);
    vb.reset();
    ib.reset();
...
struct NoReadMap {
    ...
    static HRESULT WINAPI lock(IUnknown*, UINT offset, UINT size, void** out, DWORD flags) {
        ++locks;
        if (offset || size || flags != 0x800 || !out) return D3DERR_INVALIDCALL;
        *out = forbidden;
        SetLastError(0x13572468);
        return S_OK;
    }
    static HRESULT WINAPI unlock(IUnknown*) {
        ++unlocks;
    ...
    void** old;
    void* table[14];
    explicit NoReadMap(IUnknown* object)
        : resource(object)
        , old(*reinterpret_cast<void***>(object)) {
        forbidden = VirtualAlloc(nullptr, 4096, MEM_COMMIT | MEM_RESERVE, PAGE_NOACCESS);
        check(forbidden != nullptr, "inaccessible mapping allocation");
        locks = unlocks = 0;
        std::memcpy(table, old, sizeof table);
        auto l = &lock;
        auto u = &unlock;
        std::memcpy(&table[11], &l, sizeof l);
        std::memcpy(&table[12], &u, sizeof u);
        *reinterpret_cast<void***>(resource) = table;
    }
    ~NoReadMap() {
        *reinterpret_cast<void***>(resource) = old;
...
void no_payload_read(Create create, HWND window) {
    ...
    ReadableSession s(create, window, 32u * 1024u * 1024u);
    Com<IDirect3DVertexBuffer9> vb;
    ok(s.device->CreateVertexBuffer(48, 8, D3DFVF_XYZ, D3DPOOL_MANAGED, &vb.p, nullptr), "no-read VB");
    {
        NoReadMap spy(borrowed_native_buffer_for_lock_contract(vb.p));
        void* p = nullptr;
        const HRESULT locked = vb->Lock(0, 0, &p, 0x800);
        const DWORD lock_error = GetLastError();
        const HRESULT unlocked = vb->Unlock();
        const DWORD unlock_error = GetLastError();
        check(locked == S_OK && unlocked == S_OK && p == NoReadMap::forbidden,
...
void fallback(Create create, HWND window) {
    for (bool attach : {false, true})
        for (bool clear : {false, true}) {
            ...
            Com<IDirect3DVertexBuffer9> vb;
            {
                ...
                check(CreationFault::retry_state && same_state(after, selected),
                      "fallback full incoming and selected outgoing CPU state preserved");
            }
            D3DVERTEXBUFFER_DESC a{}, b{};
            ok(vb->GetDesc(&a), "fallback app descriptor");
            ok(borrowed_native_buffer_for_lock_contract(vb.p)->GetDesc(&b), "fallback actual descriptor");
            check(a.Usage == 8 && b.Usage == 8, "fallback remains actual WRITEONLY");
            s.empty(0);
            vb.reset();
            s.close();
        }
    ReadableSession s(create, window, 0, 0);
    Com<IDirect3DVertexBuffer9> vb;
    ...
    ok(borrowed_native_buffer_for_lock_contract(vb.p)->GetDesc(&d), "metadata budget actual desc");
    check(d.Usage == 8, "metadata admission failure rolls readable conversion back");
    s.empty(0);
    vb.reset();
    s.close();
}
void incompatible(Create create) {
    for (unsigned variant = 0; variant < 3; ++variant) {
        auto* native = create(D3D_SDK_VERSION);
        check(native != nullptr, "invalid-option factory");
        Options o;
        o.prepare_readable_managed_uploads = true;
        o.track_buffer_writes = variant != 0;
        o.capture_finite_positions = variant == 1;
        o.locked_prefix_bounds = variant == 2;
        IDirect3D9* output = nullptr;
        check(wrap_factory(native, &output, o) == E_INVALIDARG && !output,
...
int main() {
    try {
        ...
        HWND window = CreateWindowExA(0, "STATIC", "lattice readable fixture", WS_OVERLAPPEDWINDOW, 0, 0, 64, 64,
                                      nullptr, nullptr, GetModuleHandleA(nullptr), nullptr);
        check(window != nullptr, "window");
        incompatible(create);
        metadata(create, window);
        no_payload_read(create, window);
        fallback(create, window);
