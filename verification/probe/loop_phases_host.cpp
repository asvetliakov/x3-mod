...
static void stamp(unsigned index) {
    accumulator.stamp(index, clock_ticks);
}
...
static void sector_b(std::uint64_t passb) {
    stamp(4);
    at(passb);
    stamp(5);
}
...
int main() {
    ...
    check(accumulator.sectors == 2 && accumulator.containers == 4 && accumulator.dispatches == 16 &&
          accumulator.open == none && accumulator.last == 0);
    check(accumulator.ticks[0] == 11 && accumulator.ticks[1] == 22 && accumulator.ticks[2] == 33 &&
          accumulator.ticks[3] == 44);
    ...
    check(s.frame == 7 && s.sectors == 2 && s.containers == 4 && s.dispatches == 16 && s.dt_us == 9000 &&
          s.input_us == 4321);
    check(s.interval_us[0] == 11 && s.interval_us[1] == 22 && s.interval_us[2] == 33 && s.interval_us[3] == 44 &&
          s.sum_us == 110);
    check(s.max_interval_us == 40 && s.max_owner == 3);
    check(s.self_us == 16 * dispatch_cost_ns / 1000);
    check(accumulator.sectors == 0 && accumulator.containers == 0 && accumulator.dispatches == 0 &&
          accumulator.ticks[3] == 0 && accumulator.max_ticks == 0 && accumulator.max_owner == none);
    // Frequency scaling: 10 MHz ticks (100 ns) convert to microseconds; one pathological sector.
    sector_a(100, 5000, 300);
    sector_a(100, 200, 300);
    sector_b(400);
    sector_b(400);
    accumulator.take(8, 10000000, 0, 0, s);
    check(s.interval_us[0] == 20 && s.interval_us[1] == 520 && s.interval_us[2] == 60 && s.interval_us[3] == 80 &&
          s.sum_us == 680);
    ...
    sector_b(5);
    skipped_b();
    accumulator.take(9, 1000000, 0, 0, s);
    check(s.sectors == 1 && s.containers == 2 && s.dispatches == 3 && s.sum_us == 5 && s.interval_us[0] == 0 &&
          s.max_owner == 3);
    ...
    at(9);
    stamp(3);
    check(accumulator.orphans == 1 && accumulator.ticks[0] == 0 && accumulator.ticks[1] == 7 &&
          accumulator.ticks[2] == 9 && accumulator.open == none);
    ...
    stamp(4);
    accumulator.stamp(5, 0);
    accumulator.stamp(4, 0);
    at(3);
    stamp(5);
    ...
    for (unsigned i = 0; i < window_frames; ++i) {
        Sample f{};
        f.frame = 1000 + i;
        f.sum_us = i % 3 == 0 ? slow_threshold_us + 1 + i : 9;
        f.interval_us[1] = f.sum_us;
        f.max_owner = 1;
        reduction.add(f);
    }
    check(reduction.close(summary) && summary.frames == 300 && summary.slow == 100 && summary.slow_count == slow_limit);
    check(summary.slow_frames[0].frame == 1000 && summary.slow_frames[0].sum_us == slow_threshold_us + 1 &&
          summary.slow_frames[63].frame == 1000 + 63 * 3);
    check(summary.slow_frames[63].interval_us[1] == slow_threshold_us + 1 + 63 * 3 &&
          summary.slow_frames[63].max_owner == 1);
    // Exactly the threshold is not slow; one over is.
    {
        Sample f{};
        f.frame = 1;
        f.sum_us = slow_threshold_us;
        reduction.add(f);
        f.frame = 2;
        f.sum_us = slow_threshold_us + 1;
        reduction.add(f);
    }
    ...
    std::printf(
        "loop_phases_host checks=%u failures=0 accumulator_bytes=%zu window_bytes=%zu summary_bytes=%zu dispatch_cost_ns=%llu slow_limit=%u\n",
        checks, sizeof(Accumulator), sizeof(Window), sizeof(Summary), static_cast<unsigned long long>(dispatch_cost_ns),
        slow_limit);
    return 0;
}
