...
struct Camera {
    ...
    Camera() {
        const double ax = -28 * 3.14159265358979323846 / 180, ay = 50 * 3.14159265358979323846 / 180,
                     az = 35 * 3.14159265358979323846 / 180;
        const double cx = std::cos(ax), sx = std::sin(ax), cy = std::cos(ay), sy = std::sin(ay), cz = std::cos(az),
                     sz = std::sin(az);
        const double Rx[3][3] = {{1, 0, 0}, {0, cx, -sx}, {0, sx, cx}},
                     Ry[3][3] = {{cy, 0, sy}, {0, 1, 0}, {-sy, 0, cy}},
                     Rz[3][3] = {{cz, -sz, 0}, {sz, cz, 0}, {0, 0, 1}};
        double RxRy[3][3]{};
        for (int i = 0; i < 3; ++i)
            for (int j = 0; j < 3; ++j)
                for (int k = 0; k < 3; ++k) RxRy[i][j] += Rx[i][k] * Ry[k][j];
        for (int i = 0; i < 3; ++i)
            for (int j = 0; j < 3; ++j)
                for (int k = 0; k < 3; ++k) R[i][j] += Rz[i][k] * RxRy[k][j];
    }
    // p_world = R^T p_view + W
    void world(const double view[3], float out[3]) const {
        for (int a = 0; a < 3; ++a) {
            double s = W[a];
            for (int k = 0; k < 3; ++k) s += R[k][a] * view[k];
            out[a] = float(s);
        }
    }
    void rows(unsigned width, unsigned height, float out[16]) const {
        ...
        double t[3]{};
        for (int i = 0; i < 3; ++i)
            for (int k = 0; k < 3; ++k) t[i] -= R[i][k] * W[k];
        const double M[16] = {R[0][0] * fx, R[0][1] * fx, R[0][2] * fx, t[0] * fx,   R[1][0] * fy, R[1][1] * fy,
                              R[1][2] * fy, t[1] * fy,    R[2][0] * Q,  R[2][1] * Q, R[2][2] * Q,  t[2] * Q - Q * zn,
...
void bolt_centre(Batch batch, double u, Lcg& rng, double out[3]) {
    ...
    case Batch::Fan176: { // four guns, bolts fanning out ahead, view depth 148..1842
        ...
        out[0] = g[0] + z * (0.010 + 0.015 * (rng.next() - .5));
        out[1] = g[1] + z * (0.006 + 0.012 * (rng.next() - .5));
        out[2] = z;
        return;
    }
...
struct Fixture {
    ...
    unsigned frame = 0;
    void create_buffers() {
        api(d->CreateVertexBuffer(bytes, D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, 0, D3DPOOL_DEFAULT, &bullets, nullptr),
            "bullet buffer");
        api(d->CreateVertexBuffer(64, 0, 0, D3DPOOL_MANAGED, &instance, nullptr), "instance buffer");
        void* data = nullptr;
        api(instance->Lock(0, 0, &data, 0), "instance lock");
        std::memset(data, 0, 64);
        api(instance->Unlock(), "instance unlock");
        api(d->CreateOffscreenPlainSurface(W, H, D3DFMT_A8R8G8B8, D3DPOOL_SYSTEMMEM, &readback, nullptr),
            "readback surface");
    }
    ...
    void write(unsigned quads, float tail, bool nested = false, int nan_vertex = -1) {
        ...
        for (unsigned q = 0; q < quads; ++q) {
            ...
            const float corners[6][2] = {{c[0] - s, c[1] - s}, {c[0] + s, c[1] - s}, {c[0] - s, c[1] + s},
    ...
    void write_near(const float (*vertices)[3]) {
        ...
        for (unsigned q = 0; q < 16; ++q)
            for (unsigned k = 0; k < 6; ++k) put_vertex(data, q * 6 + k, vertices[k], k);
    ...
    void write_batch(Batch batch, unsigned bolts, unsigned seed, bool zero_tail) {
        ...
        for (unsigned i = 0; i < bolts; ++i) {
            ...
            double c[3];
            bolt_centre(batch, u, rng, c);
            const double hw = 1.5, L = 40;
            const double corners[6][3] = {{c[0] - hw, c[1], c[2]},     {c[0] + hw, c[1], c[2]},
                                          {c[0] - hw, c[1], c[2] + L}, {c[0] + hw, c[1], c[2]},
                                          {c[0] + hw, c[1], c[2] + L}, {c[0] - hw, c[1], c[2] + L}};
            for (unsigned k = 0; k < 6; ++k) {
                float p[3];
                camera.world(corners[k], p);
                put_vertex(data, i * 6 + k, p, k);
            }
        }
    ...
    void draw(const char* label, unsigned quads, bool instanced = false, const float* rows_override = nullptr,
        ...
        long l = 0, t = 0, r = 0, b = 0;
        {
            ...
            for (unsigned y = 0; y < vh; ++y) {
                const auto* row = reinterpret_cast<const DWORD*>(static_cast<const unsigned char*>(locked.pBits) +
                                                                 y * locked.Pitch);
        ...
        std::printf(
            "FRAME %u label=%s quads=%u vertices=%u instanced=%u draw=%08lx viewport=%u,%u covered=%u footprint=%ld,%ld,%ld,%ld\n",
            frame, label, quads, quads * 6, instanced, hr, vw, vh, covered, l, t, covered ? r + 1 : 0,
            covered ? b + 1 : 0);
    }
};
} // namespace
...
int main(int argc, char** argv) {
    ...
    if (argc != 3) {
        std::puts("usage: fixture <vs.bin> <ps.bin>");
        return 2;
    }
    WNDCLASSA cls{};
    cls.lpfnWndProc = DefWindowProcA;
    cls.hInstance = GetModuleHandleA(nullptr);
    cls.lpszClassName = "X3LockedPrefix";
    RegisterClassA(&cls);
    HWND window = CreateWindowA(cls.lpszClassName, "Locked-prefix bound fixture", WS_OVERLAPPEDWINDOW, 0, 0, W, H,
                                nullptr, nullptr, cls.hInstance, nullptr);
    HMODULE runtime = LoadLibraryA("d3d9.dll");
    if (!window || !runtime) {
        std::puts("FAIL window or d3d9");
        return 2;
    }
    try {
        ...
        Fixture f;
        api(factory->CreateDevice(0, D3DDEVTYPE_HAL, window, D3DCREATE_HARDWARE_VERTEXPROCESSING, &pp, &f.d),
            "CreateDevice");
        ...
        f.write(17, 0.f);
        f.draw("first_draw_unknown", 17); // marks; refused (unknown)
        ...
        float rows[16];
        f.camera.rows(W, H, rows);
        auto batches = [&](const char* suffix) {
            std::string fan = std::string("fan_176") + suffix, straddle = std::string("straddle_72") + suffix,
                        big = std::string("big_605_origin_tail") + suffix, small = std::string("small_27") + suffix;
            f.write_batch(Batch::Fan176, 176, 1001, false);
            f.draw(fan.c_str(), 176, false, rows, W, H);
            f.write_batch(Batch::Straddle72, 72, 1002, false);
            f.draw(straddle.c_str(), 72, false, rows, W, H);
            f.write_batch(Batch::Big605, 605, 1003, true);
            f.draw(big.c_str(), 605, false, rows, W, H);
            f.write_batch(Batch::Small27, 27, 1004, false);
            f.draw(small.c_str(), 27, false, rows, W, H);
        };
        batches("");
        f.write(17, 0.f);
        f.draw("bound", 17);
        ...
        const float straddle[6][3] = {{0, 0, -1},   {0, .75f, 3}, {1.5f, .75f, 3},
                                      {0, .75f, 3}, {0, .75f, 3}, {0, .75f, 3}};
        const float exact[6][3] = {{0, -.25f, 1},   {.5f, -.25f, 1}, {0, .75f, 3},
                                   {.5f, -.25f, 1}, {1.5f, .75f, 3}, {0, .75f, 3}};
        const float behind[6][3] = {{0, -.25f, -1},   {.5f, -.25f, -1}, {0, .75f, -3},
                                    {.5f, -.25f, -1}, {1.5f, .75f, -3}, {0, .75f, -3}};
        const float beam[6][3] = {{0, -.25f, 1},   {.5f, -.25f, 1},  {0, 250, 1000},
                                  {.5f, -.25f, 1}, {500, 250, 1000}, {0, 250, 1000}};
        f.write_near(straddle);
        f.draw("near_straddle", 16, false, Fixture::near_rows);
        f.write_near(exact);
        f.draw("near_exact", 16, false, Fixture::near_rows);
        f.write_near(behind);
        f.draw("near_behind", 16, false, Fixture::near_rows);
        f.write_near(beam);
        f.draw("near_beam", 16, false, Fixture::near_rows);
        f.write(16, nan);
        f.draw("nan_tail_96", 16); // 96 written, NaN from 96: the exact prefix is clean
        f.write(17, nan);
        f.draw("nan_tail_102", 17); // 102 written over a NaN window: still clean (step B refused this)
        f.write(17, 0.f, false, 3);
        f.draw("prefix_nan_refused", 17); // NaN inside the drawn prefix: refused
        f.write(17, 0.f, true);
        f.draw("nested_lock_invalid", 17);
        f.write(17, 0.f);
        f.draw("instanced_refused", 17, true);
        f.write(17, 0.f);
        f.draw("bound_after_instanced", 17);
        f.write(1024, 0.f);
        f.draw("full_buffer", 1024);
        ...
        f.create_buffers();
        f.write(17, 0.f);
        f.draw("after_reset_unknown", 17);
        f.write(17, 0.f);
        f.draw("after_reset_bound", 17);
        release(f.bullets);
        release(f.instance);
        release(f.readback);
        release(f.declaration);
        release(vs);
        release(ps);
