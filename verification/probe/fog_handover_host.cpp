...
bool require(const char* name, bool value) {
    ++checks_run;
    failures += !value;
...
bool ramp_length(long long resident, long long ready) {
    return resident > 0 && ready - resident >= long(kReadinessRampFrames) - 1 &&
           ready - resident <= long(kReadinessRampFrames);
}
const double kCamera[3] = {1.0e6 + 1234.5, 2.0e5 - 77.25, -3.0e5 + 4096.125};
const CacheIdentity kIdentity{0x5ec7, 1, kNoOffset};

struct Latch {
    unsigned rects[kLevelCount]{};
    std::size_t bytes[kLevelCount]{};
    bool whole[kLevelCount]{}, far_view = false;
};
struct Gpu {
    std::vector<std::uint8_t> staging[kLevelCount], atlas[kLevelCount];
    Gpu() {
        for (int l = 0; l < kLevelCount; ++l) {
            staging[l].assign(kAtlasBytes, 0xcd);
            atlas[l].assign(kAtlasBytes, 0xcd);
        }
    }
    ...
    Latch frame(DensityCache& cache, std::size_t budget) {
        ...
        for (int l = 0; l < kLevelCount; ++l)
            if (cache.level_dirty(l)) views[l] = {staging[l].data(), kAtlasPitch};
        ...
        for (unsigned i = 0; i < n; ++i) {
            ...
            for (int y = 0; y < r.height; ++y)
                std::memcpy(atlas[r.level].data() + std::size_t(r.y + y) * kAtlasPitch + std::size_t(r.x) * kTexelBytes,
                            staging[r.level].data() + std::size_t(r.y + y) * kAtlasPitch +
                                std::size_t(r.x) * kTexelBytes,
                            std::size_t(r.width) * kTexelBytes);
            ++out.rects[r.level];
            out.bytes[r.level] += r.bytes();
            out.whole[r.level] = out.whole[r.level] ||
                                 (r.x == 0 && r.y == 0 && r.width == kAtlasWidth && r.height == kAtlasHeight);
        }
...
Frame frame(DensityCache& cache, Gpu& gpu, const double camera[3], std::uint64_t f, unsigned slabs = 1,
            std::size_t budget = kDefaultUploadBudget) {
    Frame out;
...
std::vector<std::uint8_t> static_atlas(int level, const NodeKey& origin, const WorldOffset& offset = kNoOffset) {
    ...
    for (int g = 0; g < kGroupCount; ++g)
        generate_tile(kLevelDelta[level], origin, offset, g,
...
bool settle(DensityCache& cache, Gpu& gpu, const double camera[3], std::uint64_t& f, std::size_t budget,
            std::size_t* max_latch_bytes = nullptr) {
...
void cold_step_then_warm_ramp() {
    ...
    std::printf(
        "COLD_STEP resident_frame=%lld ready_frame=%lld arm_frame=%llu latches=%u upload_bytes=%llu drawable_us=%lld fill_us=%lld fill_busy_us=%lld fill_cpu_us=%lld\n",
        resident, ready, (unsigned long long)report.arm_frame, report.latches, (unsigned long long)report.upload_bytes,
        (long long)report.drawable_us, (long long)report.fill_us, (long long)report.fill_busy_us,
        (long long)report.fill_cpu_us);
    ...
    require("cold_step_report_due_once_in_that_frame",
            report.due && report.step && !report.cold_fill && report.arm_frame == 1 &&
                report.drawable_frame == std::uint64_t(resident) && report.ready_frame == std::uint64_t(ready) &&
                report.drawable_us >= 0 && report.ready_us >= report.drawable_us);
    require("cold_step_report_fill_measured", report.fill_us >= 0 && report.fill_us <= report.drawable_us &&
                                                  report.fill_busy_us >= 0 && report.latches >= 1);
    require("cold_step_report_taken_once", !cache.take_handover().due);
    require("cold_step_settles_to_the_static_field",
            settle(cache, gpu, kCamera, f, kDefaultUploadBudget) && equal_static(gpu, cache, "cold_step"));

    // Warm: the same identity, the camera jumps 300 far nodes (outside the window): residency is lost and refilled,
    // ramped.
    const double jumped[3] = {kCamera[0] + 300 * kFarDelta, kCamera[1], kCamera[2]};
    long long warm_resident = -1, warm_ready = -1;
    float previous = 0.f, largest_step = 0.f;
    bool dropped = false, report_due = false;
    ...
    std::printf("WARM_RAMP resident_frame=%lld ready_frame=%lld frames=%lld largest_step=%.6f\n", warm_resident,
                warm_ready, warm_ready - warm_resident, double(largest_step));
    require("warm_jump_drops_readiness_at_once", dropped);
    require("warm_refill_keeps_the_ramp",
            ramp_length(warm_resident, warm_ready) && largest_step <= 1.f / kReadinessRampFrames + 1e-6f);
    require("warm_refill_reports_nothing", !report_due);

    // Cold again through invalidate (the proxy's load gap): the step returns.
    cache.invalidate();
    long long again_resident = -1, again_ready = -1;
    HandoverReport again;
    ...
    require("invalidate_is_a_cold_start_and_steps", again_resident > 0 && again_ready == again_resident && again.due &&
                                                        again.arm_frame > std::uint64_t(warm_ready));
}
...
void cold_ramp_legacy() {
    ...
    unsigned far_latches = 0;
    for (int i = 0; i < 2000 && ready < 0; ++i) {
        const Frame fr = frame(cache, gpu, kCamera, ++f);
        if (resident < 0) {
            far_latches += fr.latch.rects[1] != 0;
            max_bytes = std::max(max_bytes, fr.latch.bytes[0] + fr.latch.bytes[1]);
        }
        if (fr.state.resident[1] && resident < 0) {
            resident = (long long)f;
            first = fr.state.ready[1];
        }
        if (fr.state.ready[1] >= 1.f) ready = (long long)f;
        if (fr.report.due) report = fr.report;
    }
    std::printf(
        "COLD_RAMP resident_frame=%lld ready_frame=%lld frames=%lld far_latches=%u max_latch_bytes=%zu report_latches=%u\n",
    ...
    require("legacy_report_after_the_ramp", report.due && !report.step && !report.cold_fill && !report.whole_atlas &&
                                                report.drawable_frame == std::uint64_t(resident) &&
                                                report.ready_frame == std::uint64_t(ready));
    require("legacy_far_goes_up_in_several_budgeted_latches",
            far_latches > 1 && max_bytes <= kDefaultUploadBudget && report.latches > 1);
}
...
void cold_fill() {
    ...
    const std::uint64_t far_first = intersect(window_box(window_origin(kFarDelta, kCamera[0], kCamera[1], kCamera[2])),
                                              grow(need_box(1, kCamera), kFirstFillSlack[1]))
                                        .nodes();
    ...
    std::printf("COLD_FILL far_first_nodes=%llu generated=%llu held=%u\n", (unsigned long long)far_first,
                (unsigned long long)after_far, unsigned(held));
    require("cold_fill_generates_only_the_far_need_box_first",
            after_far == far_first && fr.latch.rects[0] + fr.latch.rects[1] == 0);
    ...
    std::printf("COLD_FILL_LATCH far_rects=%u far_bytes=%zu whole=%u fine_rects=%u budget=%zu\n", fr.latch.rects[1],
                fr.latch.bytes[1], unsigned(fr.latch.whole[1]), fr.latch.rects[0], kDefaultUploadBudget);
    require("cold_fill_whole_atlas_in_one_latch_past_the_budget", fr.latch.rects[1] == 1 && fr.latch.whole[1] &&
                                                                      fr.latch.bytes[1] == kAtlasBytes &&
                                                                      kAtlasBytes > kDefaultUploadBudget);
    require("cold_fill_fine_waits_for_the_next_latch", fr.latch.rects[0] == 0);
    require("cold_fill_whole_atlas_equals_the_cpu_cache",
            std::memcmp(gpu.atlas[1].data(), cache.cache_bytes(1), kAtlasBytes) == 0);
    ...
    fr = frame(cache, gpu, kCamera, ++f);
    std::printf(
        "COLD_FILL_REPORT ready=%.3f frames=%llu latches=%u upload_bytes=%llu whole=%u fill_us=%lld fill_busy_us=%lld fill_cpu_us=%lld drawable_us=%lld busy_us=%lld\n",
        double(fr.state.ready[1]), (unsigned long long)(fr.report.ready_frame - fr.report.arm_frame), fr.report.latches,
        (unsigned long long)fr.report.upload_bytes, unsigned(fr.report.whole_atlas), (long long)fr.report.fill_us,
        (long long)fr.report.fill_busy_us, (long long)fr.report.fill_cpu_us, (long long)fr.report.drawable_us,
        (long long)fr.report.busy_us);
    require("cold_fill_resident_and_stepped_in_frame_3", fr.state.resident[1] && fr.state.ready[1] == 1.f &&
                                                             fr.report.due && fr.report.ready_frame == 3 &&
                                                             fr.report.drawable_frame == 3);
    require("cold_fill_report_one_latch_whole_atlas",
            fr.report.cold_fill && fr.report.whole_atlas && fr.report.latches == 1 &&
                fr.report.upload_bytes == kAtlasBytes && fr.report.fill_us >= 0 && fr.report.fill_busy_us >= 0);
    // The rest goes up under the budget: no second latch past it, and the field ends identical to a from-scratch fill.
    std::size_t max_bytes = 0;
    require("cold_fill_settles_to_the_static_field",
            settle(cache, gpu, kCamera, f, kDefaultUploadBudget, &max_bytes) && equal_static(gpu, cache, "cold_fill"));
    ...
    bool whole = false, over = false;
    long long resident = -1, ready = -1;
    for (int i = 0; i < 4000 && ready < 0; ++i) {
        fr = frame(cache, gpu, jumped, ++f);
        whole = whole || fr.latch.whole[1];
        over = over || fr.latch.bytes[0] + fr.latch.bytes[1] > kDefaultUploadBudget;
        if (fr.state.resident[1] && resident < 0) resident = (long long)f;
        if (fr.state.ready[1] >= 1.f) ready = (long long)f;
    }
    ...
    cache.set_handover(true, false);
    fr = frame(cache, gpu, jumped, ++f);
    require("cold_fill_switched_off_releases_the_hold",
            held_again && !fr.latch.whole[1] && fr.latch.bytes[0] + fr.latch.bytes[1] <= kDefaultUploadBudget &&
                cache.worker_step());
    require("cold_fill_switched_off_settles",
            settle(cache, gpu, jumped, f, kDefaultUploadBudget) && equal_static(gpu, cache, "cold_fill_off"));
    ...
    require("cold_fill_after_gpu_reset_one_whole_latch_then_resident",
            fr.latch.whole[1] && fr.latch.rects[1] == 1 && next.state.resident[1] && next.state.ready[1] == 1.f);
}
...
void parent_walk() {
    ...
    };
    {
        ...
        require("walk_depth0_direct_match_without_walk",
                s.status == sb::Status::Ready && s.anchor_check == sb::Check::Match &&
                    s.anchor_walk == sb::AnchorWalk::None && s.anchor_depth == 0);
    }
    for (unsigned depth = 1; depth <= 3; ++depth) {
        Memory m = chain(depth);
        const auto s = sb::sample(m, sb::anchor_walk_limit);
        Memory legacy = chain(depth);
        const auto off = sb::sample(legacy);
        char name[64];
        std::snprintf(name, sizeof name, "walk_depth%u_found_ready_same_identity", depth);
        require(name, s.status == sb::Status::Ready && s.anchor_check == sb::Check::Mismatch &&
                          !sb::anchor_refused(s) && s.anchor_walk == sb::AnchorWalk::Found && s.anchor_depth == depth &&
                          s.anchor_last == 0x7000 && s.sector == 0x7000 && s.index == off.index &&
                          s.record == off.record && std::string(s.family) == off.family);
    ...
    }
    {
        Memory m = chain(4);
        const auto s = sb::sample(m, sb::anchor_walk_limit);
        require("walk_depth4_exceeds_the_bound_and_refuses", s.status == sb::Status::AnchorMismatch &&
                                                                 s.anchor_walk == sb::AnchorWalk::Bound &&
                                                                 s.anchor_depth == 3);
    }
    {
        Memory m = chain(1, 0xc000);
        m.word(0xc048, 1);
        const auto s = sb::sample(m, sb::anchor_walk_limit);
        require("walk_other_sector_refuses", s.status == sb::Status::AnchorMismatch &&
                                                 s.anchor_walk == sb::AnchorWalk::OtherSector && s.anchor_depth == 1 &&
                                                 s.anchor_last == 0xc000);
    }
    {
        Memory m = chain(1, 0);
        const auto s = sb::sample(m, sb::anchor_walk_limit);
        require("walk_null_parent_refuses",
                s.status == sb::Status::AnchorMismatch && s.anchor_walk == sb::AnchorWalk::ReadFailure);
    }
    {
        Memory m = chain(1, 0x7002);
        const auto s = sb::sample(m, sb::anchor_walk_limit);
        require("walk_misaligned_parent_refuses_without_reading_it",
                s.status == sb::Status::AnchorMismatch && s.anchor_walk == sb::AnchorWalk::ReadFailure &&
                    std::none_of(m.requested.begin(), m.requested.end(),
                                 [](const auto& r) { return r.first == 0x7002 + 0x48; }));
    }
    {
        Memory m = chain(1, 0xd000);
        const auto s = sb::sample(m, sb::anchor_walk_limit); // 0xd000 unmapped: its class word cannot be read
        require("walk_unreadable_object_refuses", s.status == sb::Status::AnchorMismatch &&
                                                      s.anchor_walk == sb::AnchorWalk::ReadFailure &&
                                                      s.anchor_depth == 1);
    }
    {
        Memory m = setup();
        m.word(0x500c, 0x8000);
        m.word(0x8054, 0x9000);
        m.word(0x9048, 2);
        m.word(0x9054, 0x9000); // a parent cycle
        const auto s = sb::sample(m, sb::anchor_walk_limit);
        const auto walked = std::count_if(m.requested.begin(), m.requested.end(),
                                          [](const auto& r) { return r.first >= 0x9000 && r.first < 0x9100; });
        require("walk_cycle_is_bounded", s.anchor_walk == sb::AnchorWalk::Bound && walked == 7);
    }
    {
        Memory m = chain(2);
        const auto s = sb::sample(m, 1); // a smaller limit is honoured
        require("walk_limit_argument_is_honoured",
                s.anchor_walk == sb::AnchorWalk::Bound && s.status == sb::Status::AnchorMismatch);
    }
    {
        Memory walk = chain(0), legacy = chain(0);
        sb::sample(walk, sb::anchor_walk_limit);
        sb::sample(legacy);
        require("walk_direct_match_reads_exactly_the_legacy_reads", walk.requested == legacy.requested);
    }
    sb::AnchorSpan span;
    Memory docked = chain(1), deeper = chain(2), undocked = chain(0);
    const bool first = span.update(sb::sample(docked, 3)), again = span.update(sb::sample(docked, 3)),
               deeper_change = span.update(sb::sample(deeper, 3));
...
void cold_fill_edges() {
    { // A new identity (invalidate) during the hold, before the whole-atlas latch: only the new field goes up, once.
        ...
        unsigned wholes = 0, far_rects_before = 0;
        long long resident = -1, ready = -1;
        ...
        require("hold_invalidate_one_whole_latch_of_the_new_identity",
                held && wholes == 1 && far_rects_before == 0 && ready == resident && ready > 0);
        require("hold_invalidate_settles_to_the_new_field",
                settle(cache, gpu, kCamera, f, kDefaultUploadBudget) &&
                    equal_static(gpu, cache, "hold_invalidate", other.offset));
    }
    { // The whole latch is lost (confirm_uploads(false)): it is taken again, and the report counts both latches.
        ...
        unsigned wholes = lost.latch.whole[1];
        HandoverReport report;
        long long ready = -1;
        for (int i = 0; i < 200 && ready < 0; ++i) {
            const Frame fr = frame(cache, gpu, kCamera, ++f);
        ...
        require("lost_whole_latch_is_taken_again", lost.latch.whole[1] && wholes == 2 && report.due &&
                                                       report.whole_atlas && report.latches >= 2 &&
                                                       report.upload_bytes >= 2 * kAtlasBytes);
    }
    { // The camera moves 20 far nodes before the latch: no whole latch until a box holding its need box is published.
        ...
        unsigned wholes = waiting.latch.whole[1] + after.latch.whole[1];
        long long resident = -1, ready = -1;
        for (int i = 0; i < 200 && ready < 0; ++i) {
        ...
        require("whole_latch_waits_for_the_need_box", !waiting.latch.whole[1] && waiting.latch.rects[1] == 0 &&
                                                          wholes == 1 && ready == resident && ready > 0);
    }
    { // The real worker thread: parked on the condition variable during the hold, woken by the latch; the far staging
        ...
        cache.set_handover(true, true);
        cache.configure(kIdentity);
        unsigned wholes = 0, far_views_before = 0, far_rects_before = 0;
        long long resident = -1, ready = -1;
        bool idle = false;
        ...
        while (seconds() < 120 && !idle) {
            ...
            if (!wholes && !l.whole[1]) {
                far_views_before += l.far_view;
                far_rects_before += l.rects[1];
            }
            wholes += l.whole[1];
        ...
        std::printf(
            "THREADED_COLD_FILL frames=%llu wholes=%u far_views_before_latch=%u resident=%lld ready=%lld idle=%u seconds=%.2f\n",
            (unsigned long long)f, wholes, far_views_before, resident, ready, unsigned(idle), seconds());
        require("threaded_hold_then_one_whole_latch",
                wholes == 1 && far_rects_before == 0 && ready == resident && ready > 0);
        require("threaded_far_staging_not_offered_while_unpublished", far_views_before == 0);
        require("threaded_worker_woken_after_the_latch_settles",
                idle && equal_static(gpu, cache, "threaded_cold_fill"));
        cache.stop();
    }
}
...
void prefill_cache() {
    ...
    require("prefill_fills_the_far_need_box_at_the_origin_and_holds",
            filled &&
                nodes == intersect(window_box(window_origin(kFarDelta, 0, 0, 0)),
                                   grow(need_box(1, origin), kFirstFillSlack[1]))
                             .nodes() &&
                held);
    ...
    std::printf("PREFILL ready_frame=%lld wholes=%u arm_frame=%llu latches=%u\n", ready, wholes,
                (unsigned long long)report.arm_frame, report.latches);
    // The arrival (20 km off) needs one extension slab per axis under the hold (one slab per frame here), then the
    // latch.
    require("prefill_hands_over_after_three_extension_slabs_and_the_latch",
            wholes == 1 && ready == resident && ready > 0 && ready <= 5 && report.due && report.whole_atlas &&
                report.latches == 1);
    require("prefill_settles_to_the_destination_field", settle(cache, gpu, arrival, f, kDefaultUploadBudget) &&
                                                            equal_static(gpu, cache, "prefill", destination.offset));
...
void run273_cases() {
    ...
    { // D invariant: after invalidate the worker parks; nothing is generated around the previous camera.
        ...
        const NodeKey origin_far = cache.worker_origin(1),
                      expect = window_origin(kFarDelta, elsewhere[0], elsewhere[1], elsewhere[2]);
        std::printf("INVALIDATE_PARKS parked=%u generated_after=%llu\n", unsigned(parked),
                    (unsigned long long)(cache.stats().nodes_generated - before));
        require("invalidate_parks_the_worker_until_a_camera_is_posted",
                parked && cache.stats().nodes_generated > before && origin_far.x == expect.x &&
                    origin_far.y == expect.y && origin_far.z == expect.z);
    }
    { // A: a same-key transit is a cold start (the proxy's invalidate on the sector change): one whole latch, stepped.
        ...
        long long resident = -1, ready = -1;
        HandoverReport report;
        float largest = 0.f, previous = 0.f;
        for (int i = 0; i < 400 && ready < 0; ++i) {
            const Frame fr = frame(cache, gpu, arrival, ++f, 4);
            wholes += fr.latch.whole[1];
            if (fr.state.resident[1] && resident < 0) resident = (long long)f;
            largest = std::max(largest, fr.state.ready[1] - previous);
            previous = fr.state.ready[1];
            if (fr.state.ready[1] >= 1.f) ready = (long long)f;
            if (fr.report.due) report = fr.report;
        }
        std::printf("TRANSIT_COLD resident_frame=%lld ready_frame=%lld wholes=%u largest_step=%.3f\n", resident, ready,
                    wholes, double(largest));
        require("same_key_transit_steps_with_one_whole_latch", wholes == 1 && ready > 0 && ready == resident &&
                                                                   largest >= 1.f && report.due && report.step &&
                                                                   report.cold_fill && report.whole_atlas);
        require("same_key_transit_settles_to_the_static_field",
                settle(cache, gpu, arrival, f, kDefaultUploadBudget) && equal_static(gpu, cache, "transit"));
    }
    { // A, identity (Run75 bridge): only another sector id (both known) is a transit; a heap-token change of the same
      // sector
        // (same index and key, same or unread id) re-configures the same identity: no epoch, the resident field and
        // image stay. Cache level only: the invalidate is applied by hand from the rule; the production sample's wiring
        // (no density call, camera kept) is proven by run273_transit in fog_card_motion_cases_inc.h (test_fog_cards).
        ...
        require("transit_needs_another_known_sector_id", fp::other_sector(3221, 2317) && !fp::other_sector(77, 77) &&
                                                             !fp::other_sector(0, 77) && !fp::other_sector(77, 0) &&
                                                             !fp::other_sector(0, 0));
        DensityCache cache;
        Gpu gpu;
        std::uint64_t f = 0;
        ...
        bool epoch = false;
        for (const auto ids : {std::pair<std::uint32_t, std::uint32_t>{77, 77}, {0, 77}, {77, 0}}) // token 0x1000 ->
                                                                                                   // 0x7000, index 2
                                                                                                   // both
            if (fp::other_sector(ids.second, ids.first)) {
                epoch = true;
                cache.invalidate();
            }
        ...
        std::printf("HEAP_TOKEN_CHANGE epoch=%u ready_far=%.3f generated_after=%llu image_same=%u\n", unsigned(epoch),
                    double(fr.state.ready[1]), (unsigned long long)(cache.stats().nodes_generated - before),
                    unsigned(gpu.atlas[1] == image));
        require("heap_token_change_same_sector_keeps_the_field",
                !epoch && fr.state.ready[1] >= 1.f && !fr.latch.whole[1] && cache.stats().nodes_generated == before &&
                    gpu.atlas[1] == image && equal_static(gpu, cache, "heap_token"));
    }
    { // A with R3: the resident key's prefill re-centres at the destination's origin as a cold start; confirmed keeps
      // it.
        DensityCache cache;
        Gpu gpu;
        std::uint64_t f = 0;
        ...
        const bool filled = cache.worker_step();
        const std::uint64_t first = intersect(window_box(window_origin(kFarDelta, 0, 0, 0)),
                                              grow(need_box(1, origin), kFirstFillSlack[1]))
                                        .nodes();
        const bool held = !cache.worker_step();
        ...
        std::printf("RECENTRE_PREFILL first_nodes=%llu ready_frames=%lld wholes=%u\n", (unsigned long long)first,
                    ready - (long long)start, wholes);
        require("same_key_prefill_recentres_at_the_origin_as_a_cold_start", posted && filled && held && kept);
        require("same_key_prefill_hands_over_stepped_after_the_latch",
                wholes == 1 && ready > 0 && ready == resident && ready - (long long)start <= 5);
    }
    { // B: the confirmed prefill is adopted when the arrival camera is the first post; a stale post would discard it.
        const CacheIdentity destination{0x9999, 1, WorldOffset{3 * kFarDelta, 0, -5 * kFarDelta}};
        const double arrival[3] = {153045.0, 168066.0, -16679.0};     // run273 frame 24765, 227 km from the origin
                                                                      // (measured)
        const double stale[3] = {-134686.9 * 3, 5846.421, -190359.5}; // the source sector's camera, unrelated
                                                                      // coordinates
        // Nodes generated between the prefill and the whole latch (far level only: the hold parks the fine level).
        auto run = [&](bool post_stale, std::uint64_t& after, std::uint64_t& first_fills, unsigned& wholes,
                       long long& ready, long long& resident) {
            DensityCache cache;
            Gpu gpu;
            std::uint64_t f = 0;
            cache.start_stepped();
            ...
            for (int i = 0; i < 400 && ready < 0; ++i) {
                ...
                if (fr.latch.whole[1] && !wholes) {
                    after = cache.stats().nodes_generated - prefilled;
                    first_fills = cache.stats().first_fills;
                }
                wholes += fr.latch.whole[1];
                if (fr.state.resident[1] && resident < 0) resident = (long long)f;
                if (fr.state.ready[1] >= 1.f) ready = (long long)f;
        ...
        std::uint64_t adopted_nodes = 0, stale_nodes = 0, adopted_fills = 0, stale_fills = 0;
        unsigned adopted_wholes = 0, stale_wholes = 0;
        long long ar = -1, ares = -1, sr = -1, sres = -1;
        run(false, adopted_nodes, adopted_fills, adopted_wholes, ar, ares);
        run(true, stale_nodes, stale_fills, stale_wholes, sr, sres);
        const std::uint64_t first = intersect(window_box(window_origin(kFarDelta, arrival[0], arrival[1], arrival[2])),
                                              grow(need_box(1, arrival), kFirstFillSlack[1]))
                                        .nodes();
        std::printf(
            "PREFILL_ADOPTED nodes_to_latch=%llu first_fills=%llu stale_post_nodes_to_latch=%llu stale_first_fills=%llu first_fill=%llu wholes=%u ready_frame=%lld\n",
            (unsigned long long)adopted_nodes, (unsigned long long)adopted_fills, (unsigned long long)stale_nodes,
            (unsigned long long)stale_fills, (unsigned long long)first, adopted_wholes, ar);
        require("confirmed_prefill_adopted_extends_less_than_a_first_fill",
                adopted_wholes == 1 && ar > 0 && ar == ares && adopted_nodes < first && adopted_fills == 1);
        require("stale_post_after_a_prefill_costs_a_first_fill_or_more",
                stale_wholes == 1 && sr > 0 && stale_nodes >= first && stale_nodes > adopted_nodes && stale_fills >= 2);
    }
    { // D: the prefill on a cache no stored frame has configured (a new game), then the first prepare's gpu_reset,
        ...
        cache.configure(destination);
        cache.gpu_reset();                          // the first prepare_density: the same identity, new atlases
        ...
        require("new_game_prefill_latches_in_the_first_posted_frame_and_steps_in_the_next",
                one.latch.whole[1] && one.latch.rects[1] == 1 && !one.state.resident[1] && two.state.resident[1] &&
                    two.state.ready[1] == 1.f && two.report.due && two.report.latches == 1);
    }
}
...
struct List {
    ...
    List() {
        m.word(0x60850c, manager);
        m.word(manager + 8, 0);
        m.word(manager + 0xc, 0);
        m.word(manager + 0x10, manager + 8);
    }
    std::uint32_t add(std::uint32_t type, std::uint32_t id = 900, std::uint32_t marker = 0xcafe,
                      std::uint32_t scene = 0x7777000, std::int32_t index = 0) {
        const std::uint32_t node = 0x50000 + 0x400 * std::uint32_t(nodes.size());
        m.block(node, 0x180);
        m.word(node + 8, id);
        m.word(node + 0x48, type);
        m.word(node + 0x9c, marker);
        m.word(node + 0x130, scene);
        m.word(node + 0x13c, std::uint32_t(index));
    ...
    void link() {
        ...
        for (std::size_t i = 0; i < nodes.size(); ++i)
            m.word(nodes[i] + 4, i + 1 < nodes.size() ? nodes[i + 1] : manager + 8);
    }
};
...
void prefill_walk() {
    namespace fp = x3m::fog_prefill;
    unsigned worst = 0;
    auto run = [&](List& l, std::uint32_t last = 0) {
        const auto r = fp::walk(l.m, last);
        worst = std::max(worst, r.reads);
        return r;
    };
    {
        List l;
        l.add(kSector, 555);
        const auto r = run(l);
        std::printf("PREFILL_WALK case=tail status=%s steps=%u reads=%u family=%s\n", fp::name(r.status), r.steps,
                    r.reads, r.sample.family);
        require("walk_accepts_the_sector_at_the_tail",
                r.status == fp::Walk::Found && r.steps == 1 && r.id == 555 && r.sample.index == 0 &&
                    std::string(r.sample.family) == "bluewell" &&
                    r.sample.status == x3m::sector_background::Status::Ready && r.sample.sector_id == 555);
    }
    {
        List l;
        for (int i = 0; i < 7; ++i) l.add(kShip);
        l.add(kSector, 556);
        const auto r = run(l);
        std::printf("PREFILL_WALK case=eighth status=%s steps=%u reads=%u\n", fp::name(r.status), r.steps, r.reads);
        require("walk_accepts_the_eighth_node_within_25_reads",
                r.status == fp::Walk::Found && r.steps == 8 && r.reads <= fp::kReadBudget);
    }
    {
        List l;
        for (int i = 0; i < 8; ++i) l.add(kShip);
        l.add(kSector);
        const auto r = run(l);
    ...
    }
    {
        List l;
        const auto r = run(l);
        require("walk_empty_list_is_the_head", r.status == fp::Walk::Head && r.steps == 0);
    }
    {
        List l;
        l.add(kCutScene);
    ...
    {
        List l;
        l.add(kSector, 558, 0xefac);
        require("walk_refuses_a_freed_object", run(l).status == fp::Walk::Dead);
    }
    {
        List l;
        l.add(kSector, 559, 0xcafe, 0);
        require("walk_refuses_a_sector_without_scene", run(l).status == fp::Walk::NoScene);
    }
    {
        List l;
        l.add(kSector, 560);
        require("walk_refuses_the_last_ready_id",
                run(l, 560).status == fp::Walk::SameId && run(l, 561).status == fp::Walk::Found);
    }
    {
        List l;
        l.add(kSector);
        l.m.word(0x60850c, 0);
        require("walk_no_manager", run(l).status == fp::Walk::NoManager);
    }
    {
        List l;
        l.add(kSector);
        l.m.bytes.erase(0x60850c);
        require("walk_unreadable_manager", run(l).status == fp::Walk::ReadFailure);
    }
    {
        List l;
        l.add(kSector);
        l.m.word(List::manager + 0x10, l.nodes[0] + 2);
        require("walk_misaligned_node", run(l).status == fp::Walk::Malformed);
    }
    {
        List l;
        l.add(kShip);
        l.m.bytes.erase(l.nodes[0] + 4);
        require("walk_unreadable_link", run(l).status == fp::Walk::ReadFailure);
    }
    {
        List l;
        l.add(kSector);
        l.m.word(0x606fc0, 0);
        require("walk_table_loading", run(l).status == fp::Walk::Loading);
    }
    {
        List l;
        l.add(kSector, 900, 0xcafe, 0x7777000, 83);
        require("walk_index_out_of_range", run(l).status == fp::Walk::BadIndex);
    }
    {
        List l;
        l.add(kSector);
        l.m.word(0x10134, 99);
        require("walk_bad_record", run(l).status == fp::Walk::BadRecord);
    }
    ...
    require("gate_polls_only_in_a_stall_and_once_per_250ms",
            !before && !at250 && at251 && !again && next && !after_present && stalled);
    ...
    {
        fp::Record none, pending;
        pending.pending = true;
        pending.key = 42;
        pending.recipe = 1;
        require("plan_hitch_same_sector_keeps_the_resident_field",
                fp::plan(none, 42, 1, true, 3221, 0) == fp::Plan::CurrentSector);
        require("plan_token_change_same_id_keeps_the_resident_field",
                fp::plan(none, 42, 1, true, 3221, 3221) == fp::Plan::CurrentSector);
        require("plan_token_change_id_unread_keeps_the_resident_field",
                fp::plan(none, 42, 1, true, 0, 3221) == fp::Plan::CurrentSector &&
                    fp::plan(none, 42, 1, true, 0, 0) == fp::Plan::CurrentSector);
        require("plan_resident_key_in_another_sector_recentres",
                fp::plan(none, 42, 1, true, 3221, 2317) == fp::Plan::Recentre);
        require("plan_new_key_starts_and_pending_waits",
                fp::plan(none, 43, 1, false, 3221, 2317) == fp::Plan::Start &&
                    fp::plan(none, 43, 1, false, 3221, 3221) == fp::Plan::Start &&
                    fp::plan(pending, 42, 1, true, 3221, 3221) == fp::Plan::AlreadyStarted &&
                    fp::plan(pending, 42, 2, true, 3221, 3221) == fp::Plan::CurrentSector);
    }
    ...
    r.pending = true;
    require("decide_confirms_the_same_key_and_discards_the_rest",
            none == fp::Decision::None && fp::decide(r, true, 42, 1) == fp::Decision::Confirmed &&
                fp::decide(r, true, 43, 1) == fp::Decision::Discarded &&
                fp::decide(r, true, 42, 2) == fp::Decision::Discarded &&
                fp::decide(r, false, 42, 1) == fp::Decision::Discarded);
}
...
void card_arming() {
    ...
    s.begin(false);
    s.arm_on_cold_step();
    require("cold_step_arming_respects_refusal_and_inactive",
            !r.may_replace() && r.warmup && !s.may_replace() && !s.armed);
}
} // namespace
