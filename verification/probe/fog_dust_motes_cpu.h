...
struct Mote {
    std::uint32_t index = 0; bool drawn = false;
    double q[3]{}, view[3]{}, centre[2]{}, axis[2]{1, 0}, length = 0, radius = 0, brightness = 0, rho = 0, margin = 0, colour[3]{};
...
// `margin`, when given, receives rho - coverage - cover: how far the raw density sits above (or below) the remap's zero.
inline double density(const fog_cpu::Setup& s, const double q[3], double* margin = nullptr) {
    ...
    for (int a = 0; a < 3; ++a) { local[a] = double(float(s.camera[a] - 65536. * std::floor(s.camera[a] / 65536. + .5))); anchored[a] = local[a] + q[a]; }
    const double wave1[3] = {fog_cpu::look_wave(anchored[1] * k[9][0]), fog_cpu::look_wave(anchored[2] * k[9][0]), fog_cpu::look_wave(anchored[0] * k[9][0])};
    const double wave2[3] = {fog_cpu::look_wave(anchored[2] * k[9][2]), fog_cpu::look_wave(anchored[0] * k[9][2]), fog_cpu::look_wave(anchored[1] * k[9][2])};
    for (int a = 0; a < 3; ++a) { ray[a] = q[a] + wave1[a] * k[9][1] + wave2[a] * k[9][3]; p[a] = s.camera[a] + ray[a]; warped[a] = (local[a] + ray[a]) / 65536.; }
...
inline std::vector<Mote> motes(const x3m::renderer::FogMoteTuning& t, const Frame& f, const fog_cpu::Setup& s) {
    ...
    double wrapped[3];
    for (int a = 0; a < 3; ++a) { wrapped[a] = std::fmod(f.camera[a], side); if (wrapped[a] < 0) wrapped[a] += side; wrapped[a] = double(float(wrapped[a])); }
    for (std::uint32_t i = 0; i < t.count; ++i) {
        Mote& m = out[i]; m.index = i;
        float seed[3]; x3m::renderer::fog_mote_seed(i, t.seed, seed);
        ...
        const double fade = std::min(std::max((radius - d) / (.25 * radius), 0.), 1.) * std::min(std::max((d - t.near_fade) / t.near_fade, 0.), 1.);
        double vp[3];
        for (int j = 0; j < 3; ++j) {
            m.view[j] = vp[j] = 0;
            for (int a = 0; a < 3; ++a) { m.view[j] += q[a] * f.rotation[a][j]; vp[j] += qp[a] * (f.streak ? f.previous_rotation[a][j] : f.rotation[a][j]); }
        }
        double before[2]; window(f, m.view, m.centre); window(f, vp, before);
        double streak[2] = {0, 0};
        if (f.streak && vp[2] >= 1) for (int a = 0; a < 2; ++a) streak[a] = m.centre[a] - before[a];
        const double span = std::sqrt(streak[0] * streak[0] + streak[1] * streak[1]);
        m.length = std::min(span, double(t.streak));
        if (span > 1e-4) { m.axis[0] = streak[0] / span; m.axis[1] = streak[1] / span; }
        const double size = std::min(std::max(double(t.size) * radius / std::max(d, 1e-3), double(t.size)), double(t.max_px));
        m.radius = .5 * size;
        ...
        const float (*k)[4] = s.look;
        double cosine = 0; for (int a = 0; a < 3; ++a) cosine += q[a] * s.sun[a] / std::max(d, 1e-3);
...
inline void bounds(const Mote& m, int box[4]) {
    ...
    box[0] = int(std::floor(x0)); box[1] = int(std::floor(y0)); box[2] = int(std::ceil(x1)); box[3] = int(std::ceil(y1));
}
