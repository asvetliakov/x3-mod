...
// bracket composes them as before and M covers them. Twelve frames across the
// eight jitter phases with the rotating camera (cut at frame 7); the raw FP16
// scene after the quads, RT1, M and the presented frame are dumped per frame.
void run_fade_route_integration(Fixture& f,const char* original_path) {
    ...
    require(routed_script||masked_script,"X3M_FIXTURE_FADE_SCRIPT=routed|masked");
    ...
    // source itself); Q's diffuse is a 16x16 ramp (r = u, g = v, b .5).
    const float diffuse_alpha=routed_script?1.f:.5f;
    ...
    }
    {
        ...
        for(unsigned y=0;y<16;++y)for(unsigned x=0;x<16;++x){const float value[4]={float(x)/15.f,float(y)/15.f,.5f,diffuse_alpha};std::memcpy(static_cast<char*>(lock.pBits)+y*lock.Pitch+x*16,value,16);}
    ...
    // c37/38, g_AlphaValue c39, emissive c40, g_FogClip c41, point light c0-2,
    // b0 fog on, i0 one point light.
    const auto bind=[&](unsigned which) {
        f.scene_states();
        ...
        vc[36][3]=4;vc[37][2]=.0625f;vc[38][2]=.1875f;
        vc[39][0]=routed_script?1.f:.625f;vc[40][0]=.25f;vc[40][1]=.125f;vc[40][2]=.0625f;
        vc[41][0]=routed_script?1.f:.75f;vc[41][1]=routed_script?0.f:.125f;
    ...
    for(unsigned plan=0;plan<frames;++plan) {
        ...
        for(unsigned which=0;which<2;++which) {
            f.scope(&objects[which]);bind(which);
            ...
            require(routed_delta==unsigned(routed_script)&&refused_delta==unsigned(masked_script)&&prepared==unsigned(masked_script),"fade route arm decision matches the script");
            const bool matched=routed_script&&objects[which].recorded;
            for(unsigned y=0;y<f.H;++y)for(unsigned x=0;x<f.W;++x) {
                ...
                if(masked_script&&std::memcmp(&motion[i],&before_motion[i],16))preserved=0;
                ...
                if(routed_script) {
                    ...
                    const bool own=matched?(std::fabs(motion[i]-u)*f.W<.01&&std::fabs(motion[i+1]-v)*f.H<.01&&std::fabs(motion[i+2]-.3f)<4e-6&&motion[i+3]==1):(motion[i]==0&&motion[i+1]==0&&motion[i+2]==0&&motion[i+3]==-1);
                    own_motion+=own;
                    if(!own)std::printf("FADE_ROUTE_PIXEL_DIFF frame=%llu quad=%u x=%u y=%u matched=%u motion=%.9g,%.9g,%.9g,%.9g\n",f.frame,which,x,y,matched,motion[i],motion[i+1],motion[i+2],motion[i+3]);
                    require_quiet(own,"fade route routed quad writes its own RT1 rows (alpha 1; frame 0 the sentinel)");
                }
            }
            ...
            if(routed_script)objects[which].recorded=true;
        }
        ...
        if(masked_script)require(preserved,"fade route bracketed draws preserve RT1");
        require(mask_set==(masked_script?2u*14u*14u:0u),"fade route M covers exactly the bracketed quads");
        ...
        std::printf("FADE_ROUTE frame=%llu script=%s routed=%u matched=%u fade_routed=%u fade_refused=%u prepared=%u covered=%u own_motion=%u mask_set=%u mask_valid=%u threshold=%u draws=2\n",
                    f.frame,script_setting,routed_script,routed_script&&plan>0,f.emission_status(f.d.p,50),f.emission_status(f.d.p,51),f.emission_status(f.d.p,4),covered,own_motion,mask_set,f.emission_mask_valid,f.emission_status(f.d.p,52));
        f.boundary();
