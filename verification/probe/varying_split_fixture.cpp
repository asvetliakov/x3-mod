...
void dcl(Words &w, unsigned usage, unsigned index, unsigned type, unsigned n,
         unsigned mask = 15, Word flags = 0) {
...
const char *names[] = {
    "native",          "split_centroid", "split_plain",    "reference_centroid",
...
bool color_rgb(unsigned k) { return k == SplitColor || k == ReferenceColor; }
bool context_ok(unsigned k, bool flat, unsigned wrap9) {
  return k == Native || color_rgb(k) || (!flat && wrap9 == 0) ||
         (reference(k) && wrap9 == 0);
}
...
Program program(unsigned kind, bool depth) {
  ...
  if (split(kind)) {
    ...
    dcl(s, color_rgb(kind) ? 10 : 5, color_rgb(kind) ? 1 : 9, 1, 0, 7,
        centered(kind) ? centroid : 0);
  }
  for (unsigned i = 0; i < 7; ++i) {
    ...
    dcl(s, usage, semantic, 1, i + 1, mask,
        reference(kind) && i == 0 && centered(kind) ? centroid : 0);
  }
  dcl(v, 5, 7, 6, 9);
  dcl(v, 5, 8, 6, 10);
  dcl(s, 5, 7, 1, 8);
  ...
  for (unsigned i = 0; i < 7; ++i)
    ins(v, 1,
        {dst(6, i + 2, reference(kind) && i == 0 ? 7 : 15),
         reference(kind) && i == 0 ? src(1, 1) : src(2, i + 2)});
  ...
  ins(s, 1,
      {dst(8, 0, 7, kind == Native ? pp | 0x00100000u : 0),
       src(1, reference(kind) ? 1 : 0)});
  ...
  for (unsigned i = 0; i < 7; ++i)
    ins(s, 2,
        {dst(0, 3, 7), src(0, 3),
         src(1, reference(kind) && i == 0 ? 0 : i + 1)});
...
void reverse_color_dcls(Words &w) {
  ...
  for (unsigned j = 0; j < 3; ++j)
    std::swap(w[first + j], w[second + j]);
}
Program diagnostic_program(unsigned control, bool depth) {
  auto p =
      program(control == 0 || control == 7                     ? Native
              : control == 1 || control == 10 || control == 12 ? ReferenceColor
                                                               : SplitColor,
              depth);
  if (control == 3 || control == 5)
    reverse_color_dcls(p.vs);
  if (control == 4 || control == 5)
    reverse_color_dcls(p.ps);
  if (control == 6 || control == 7)
    for (std::size_t i = 1; i < p.ps.size() && p.ps[i] != 0xffff;
         i += 1 + ((p.ps[i] >> 24) & 15))
      if ((p.ps[i] & 0xffff) == 31 && (p.ps[i + 1] & 0xfffff) == 10)
        p.ps[i + 2] &= ~pp;
  if (control == 8 || control == 9) {
    ...
    for (std::size_t i = 1; i < w.size() && w[i] != 0xffff;
      ...
      if (op == 31 || ((w[i] >> 24) & 15) == 0)
        continue;
      Word &d = w[i + 1];
      const unsigned type = ((d >> 28) & 7) | ((d >> 8) & 24);
      if (type == (control == 8 ? 6u : 8u) &&
          (d & 0x7ff) == (control == 8 ? 1u : 0u))
  ...
  if (control == 10) {
    ...
    for (std::size_t i = 1; i < p.ps.size() && p.ps[i] != 0xffff;
         i += 1 + ((p.ps[i] >> 24) & 15))
      if ((p.ps[i] & 0xffff) == 2 && p.ps[i + 3] == src(1, 0))
        p.ps[i + 3] = src(2, 1);
  }
  ...
  for (std::size_t i = 1; i < w.size() && w[i] != 0xffff;) {
    ...
    if ((w[i] & 0xffff) == 31) {
      ...
      if (type == (vertex ? 6u : 1u))
        out.push_back({w[i + 1] & 15, (w[i + 1] >> 16) & 15, d & 0x7ff,
                       (d >> 16) & 15, d & 0x00700000});
    }
    i += count + 1;
  }
  return out;
...
bool contract(const Program &p) {
  ...
  if (v.size() != 11u + split(p.kind) || s.size() != 10u + split(p.kind))
    return false;
  for (const auto *list : {&v, &s})
    for (std::size_t i = 0; i < list->size(); ++i) {
      ...
      for (std::size_t j = 0; j < i; ++j) {
        const auto &a = (*list)[i];
        const auto &b = (*list)[j];
        if ((a.usage == b.usage && a.index == b.index) ||
            (a.reg == b.reg && (a.mask & b.mask)))
          return false;
      }
    }
  for (const auto &a : s) {
    unsigned matches = 0;
    for (const auto &b : v)
      if (a.usage == b.usage && a.index == b.index && a.mask == b.mask &&
          a.reg + 1 == b.reg)
        ++matches;
    if (matches != 1)
      return false;
  ...
  for (const auto &a : s) {
    if (a.usage == 10 && a.index == 0)
      alpha =
          a.reg == 0 && a.mask == (split(p.kind) ? 8u : 15u) && a.flags == pp;
    if (a.usage == (color_rgb(p.kind) ? 10u : 5u) &&
        a.index == (color_rgb(p.kind) ? 1u : 9u))
      rgb = a.reg == (reference(p.kind) ? 1u : 0u) && a.mask == 7 &&
            a.flags == (centered(p.kind) ? centroid : 0);
  }
  return alpha && rgb;
}
unsigned negative_contracts() {
  ...
  for (unsigned fault = 0; fault < 6; ++fault) {
    ...
    for (std::size_t i = 1; i < p.ps.size(); i += 1 + ((p.ps[i] >> 24) & 15)) {
      ...
      if (fault == 0 && color)
        p.ps[i + 2] |= 7u << 16; // overlapping lanes
      if (fault == 1 && rgb)
        p.ps[i + 1] = 0x8000000a; // duplicate semantic
      if (fault == 2 && rgb)
        p.ps[i + 2] |= pp;
      if (fault == 3 && color)
        p.ps[i + 2] &= ~pp;
      if (fault == 4 && rgb)
        p.ps[i + 2] &= ~centroid;
      if (fault == 5 && rgb)
        p.ps[i + 1] = 0x800a0005; // mismatched semantic
    }
...
void dump_programs(const char *folder, bool diagnostic = false) {
  for (unsigned d = 0; d < 2; ++d)
    for (unsigned k = 0; k < (diagnostic ? 13u : unsigned(KindCount)); ++k) {
      ...
      if (!diagnostic)
        need(contract(p), "positive authored contract");
      const auto stem = std::string(folder) + "/" +
                        (diagnostic ? diagnostic_names[k] : names[k]) + "_" +
                        std::to_string(d);
...
void inputs(unsigned pattern, bool perspective) {
  ...
  const float colors[4][3][3] = {
      ...
      {{1.f / 65536, 1.f / 16384, 1.f / 4096},
       {1.f / 65536 + 1.f / 16777216, 1.f / 16384 + 1.f / 4194304,
        1.f / 4096 + 1.f / 1048576},
       {1.f / 65536 - 1.f / 16777216, 1.f / 16384 - 1.f / 4194304,
        1.f / 4096 - 1.f / 1048576}},
      {{.125f, 128, 32768}, {64, .0625f, 49152}, {.5f, 4096, 65504}}};
  const float alpha[3] = {.5f - 1.f / 4096, .5f + 1.f / 4096,
                          1.f / 16384 + 1.f / 16777216};
  const float fog[3] = {-.125f, .625f, 1.25f};
  for (unsigned i = 0; i < 3; ++i) {
    const float w = perspective ? (.75f + .5f * i) : 1.f;
    vertices[i] = {{(2 * xy[i][0] / W - 1) * w, (1 - 2 * xy[i][1] / H) * w,
                    (.2f + .3f * i) * w, w},
                   {colors[pattern][i][0], colors[pattern][i][1],
                    colors[pattern][i][2], alpha[i]},
                   {fog[i], 0, 0, 0}};
  }
}
std::array<double, 3> weights(unsigned x, unsigned y) {
  ...
  const double a =
      ((py[1] - py[2]) * (x - px[2]) + (px[2] - px[1]) * (y - py[2])) / det;
  const double b =
      ((py[2] - py[0]) * (x - px[2]) + (px[0] - px[2]) * (y - py[2])) / det;
  return {a, b, 1 - a - b};
}
Image read(IDirect3DDevice9 *d, IDirect3DSurface9 *rt,
  ...
  for (unsigned y = 0; y < H; ++y)
    std::memcpy(&a[y * W * 4], static_cast<char *>(l.pBits) + y * l.Pitch,
                W * 16);
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    const auto admitted = [&](unsigned k) {
      return !separate || k == Native || k == ReferenceCentroid ||
             k == ReferenceColor;
    };
    ...
    window = CreateWindowA("STATIC", "varying split", WS_OVERLAPPED, 0, 0, W, H,
    ...
    need(window != nullptr, "window");
    {
      ...
      need(caps.VertexShaderVersion >= D3DVS_VERSION(3, 0) &&
               caps.PixelShaderVersion >= D3DPS_VERSION(3, 0) &&
               caps.NumSimultaneousRTs >= 3,
           "SM3/three MRT capability");
      D3DDISPLAYMODE mode{};
      api(factory->GetAdapterDisplayMode(0, &mode), "mode");
      api(factory->CheckDeviceFormat(0, D3DDEVTYPE_HAL, mode.Format,
                                     D3DUSAGE_RENDERTARGET, D3DRTYPE_SURFACE,
                                     D3DFMT_A32B32G32R32F),
          "FP32 RT format");
      api(factory->CheckDepthStencilMatch(0, D3DDEVTYPE_HAL, mode.Format,
                                          D3DFMT_A32B32G32R32F, D3DFMT_D24X8),
          "D24 match");
      ...
      api(d->CreateDepthStencilSurface(W, H, D3DFMT_D24X8, D3DMULTISAMPLE_NONE,
                                       0, TRUE, &depth.p, nullptr),
          "depth");
      Targets targets;
      for (unsigned i = 0; i < 3; ++i) {
        api(d->CreateRenderTarget(W, H, D3DFMT_A32B32G32R32F,
                                  D3DMULTISAMPLE_NONE, 0, FALSE,
                                  &targets.rt[i].p, nullptr),
            "target");
        api(d->CreateOffscreenPlainSurface(W, H, D3DFMT_A32B32G32R32F,
                                           D3DPOOL_SYSTEMMEM,
                                           &targets.read[i].p, nullptr),
            "staging");
      }
      ...
      api(d->CreateTexture(1, 1, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED,
                           &texture.p, nullptr),
      ...
      *static_cast<Word *>(lock.pBits) = 0x9f4080c0;
      ...
      for (unsigned dep = 0; dep < 2; ++dep)
        for (unsigned kind = 0; kind < count; ++kind) {
          if (!admitted(kind))
            continue;
          const auto p =
              diagnostic ? diagnostic_program(kind, dep) : program(kind, dep);
          api(d->CreateVertexShader(
                  reinterpret_cast<const DWORD *>(p.vs.data()),
                  &bank[dep][kind].vs.p),
              "authored VS create");
          api(d->CreatePixelShader(reinterpret_cast<const DWORD *>(p.ps.data()),
                                   &bank[dep][kind].ps.p),
              "authored PS create");
          creates += 2;
          std::printf(
              "CREATE depth=%u kind=%s vs=1 ps=1 inputs=10 outputs=11\n", dep,
              labels[kind]);
        }
      std::printf("CAPS mrt=%lu vs=%08lx ps=%08lx msaa=0 format=116\n",
                  caps.NumSimultaneousRTs, caps.VertexShaderVersion,
                  caps.PixelShaderVersion);
      std::printf("CONTEXT shading=gouraud+flat wrap9=0 no_msaa=1\n");
      api(d->SetDepthStencilSurface(nullptr), "detach depth");
      for (unsigned i = 0; i < 3; ++i)
        api(d->SetRenderTarget(i, targets.rt[i].p), "bind target");
      ...
      for (auto state :
           {D3DRS_ALPHABLENDENABLE, D3DRS_ALPHATESTENABLE, D3DRS_FOGENABLE,
            D3DRS_DITHERENABLE, D3DRS_SRGBWRITEENABLE, D3DRS_SCISSORTESTENABLE,
            D3DRS_STENCILENABLE, D3DRS_LIGHTING})
      ...
      constants[2][0] = constants[2][1] = constants[2][2] =
          0; // exact PP/full filler reference
      ...
      if (separate) {
        ...
        for (unsigned step = 0; step < 3; ++step) {
          ...
          api(d->SetRenderState(D3DRS_SHADEMODE, requested),
              "classifier set shade");
          DWORD before = 0, after = 0;
          api(d->GetRenderState(D3DRS_SHADEMODE, &before),
              "classifier get shade before");
          need(before == requested,
               "classifier requested state retained before");
        ...
        bool exact_zero = true, exact_gouraud = true,
             gouraud_nonconstant = false;
        ...
        for (unsigned p = 0; p < W * H; ++p) {
          ...
          need(!std::memcmp(&native[0][0][i + 3], &native[2][0][i + 3], 4),
               "classifier stable native alpha");
          if (native[0][1][i + 3] == 0)
            continue;
          Word a, g;
          std::memcpy(&a, &native[0][0][i + 3], 4);
          std::memcpy(&g, &native[1][0][i + 3], 4);
          need(std::isfinite(native[0][0][i + 3]) &&
                   std::isfinite(native[1][0][i + 3]),
        ...
        need(coverage > 1000 && gouraud_nonconstant &&
                 exact_zero != exact_gouraud,
      ...
      for (unsigned flat = 0; flat < (diagnostic ? 1u : 2u); ++flat)
        for (unsigned dep = 0; dep < 2; ++dep)
          for (unsigned pattern = 0; pattern < (diagnostic ? 1u : 4u);
               ++pattern)
            for (unsigned fog = 0; fog < 2; ++fog)
              for (unsigned perspective = 0;
                   perspective < (diagnostic ? 1u : 2u); ++perspective, ++id) {
                ...
                api(d->SetRenderState(D3DRS_SHADEMODE,
                                      flat ? D3DSHADE_FLAT : D3DSHADE_GOURAUD),
                    "case shading");
                DWORD shade_before = flat ? D3DSHADE_FLAT : D3DSHADE_GOURAUD;
                if (separate) {
                  api(d->GetRenderState(D3DRS_SHADEMODE, &shade_before),
                      "case shade before");
                  need(shade_before ==
                           (flat ? D3DSHADE_FLAT : D3DSHADE_GOURAUD),
                ...
                std::vector<unsigned> selected =
                    flat ? std::vector<unsigned>{Native, ReferenceCentroid,
                ...
                if (separate)
                  selected = {Native, ReferenceCentroid, ReferenceColor};
                std::array<std::array<Image, 3>, 13> images;
                for (unsigned k : selected) {
                  need(diagnostic || context_ok(k, flat, 0),
                       "defined interpolation context");
                  auto &shader = bank[dep][k];
                  api(d->SetVertexShader(shader.vs.p), "bind VS");
                  api(d->SetPixelShader(shader.ps.p), "bind PS");
                  api(d->SetRenderTarget(2, targets.rt[2].p),
                      "clear depth-output target");
                  api(d->Clear(0, nullptr, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER,
                               0, 1, 0),
                      "clear");
                  if (!dep)
                    api(d->SetRenderTarget(2, nullptr),
                        "depth-off target unbound");
                  Vertex submitted[3];
                  ...
                  api(d->DrawPrimitiveUP(D3DPT_TRIANGLELIST, 1, submitted,
                                         sizeof(Vertex)),
                      "draw");
                  api(d->EndScene(), "end");
                  ++shader.draws;
                  for (unsigned t = 0; t < 3; ++t) {
                    ...
                    const auto label =
                        std::string(labels[k]) + "_rt" + std::to_string(t);
                    save(label.c_str(), id, images[k][t]);
                  }
                }
                DWORD shade_after = shade_before;
                if (separate) {
                  api(d->GetRenderState(D3DRS_SHADEMODE, &shade_after),
                      "case shade after");
                  need(shade_after == shade_before,
                       "case requested shade after");
                }
                if (diagnostic) {
                  unsigned coverage = 0;
                  for (unsigned p = 0; p < W * H; ++p)
                    coverage += images[0][1][p * 4 + 3] != 0;
                  for (unsigned k : selected) {
                    const unsigned ref = k == 6               ? 7
                                         : k == 11 || k == 12 ? 12
                                                              : 0;
                    unsigned alpha_diff = 0, nonfinite = 0, rgb_diff = 0,
                             temporal_diff = 0;
                    Word first_bits = 0;
                    for (unsigned p = 0; p < W * H; ++p) {
                      const auto i = p * 4;
                      ...
                      alpha_diff += std::memcmp(&images[k][0][i + 3],
                                                &images[ref][0][i + 3], 4) != 0;
                      // Native RGB is intentionally saturated; separate full
                      // RGB is the control.
                      rgb_diff += std::memcmp(&images[k][0][i],
                                              &images[1][0][i], 12) != 0;
                      for (unsigned t = 1; t < 3; ++t)
                        temporal_diff += std::memcmp(&images[k][t][i],
                                                     &images[0][t][i], 16) != 0;
                    }
                    bank[dep][k].coverage += coverage;
                    std::printf(
                        "OBSERVE id=%u depth=%u fog=%u perspective=%u kind=%s "
                        "reference=%s "
                        "coverage=%u alpha_diff=%u alpha_nonfinite=%u "
                        "rgb_diff=%u temporal_diff=%u first_alpha_bits=%08x\n",
                        id, dep, fog, dep, labels[k], labels[ref], coverage,
                        alpha_diff, nonfinite, rgb_diff, temporal_diff,
                        first_bits);
                  }
                  total += coverage;
                  continue;
                ...
                double maximum = 0;
                auto exact = [&](unsigned k, unsigned t, unsigned i,
                                 unsigned count, unsigned ref,
                                 const char *label) {
                  if (std::memcmp(&images[ref][t][i], &images[k][t][i],
                                  count * 4)) {
                    std::printf(
                        "DIFF case=%u kind=%s rt=%u pixel=%u channel=%u "
                        "actual=%.9g reference=%.9g check=%s\n",
                        id, labels[k], t, i / 4, i % 4, images[k][t][i],
                        images[ref][t][i], label);
                ...
                for (unsigned y = 0; y < H; ++y)
                  for (unsigned x = 0; x < W; ++x) {
                    ...
                    for (unsigned k : selected) {
                      ...
                      for (unsigned t = 1; t < 3; ++t)
                        exact(k, t, i, 4, Native, "motion_depth");
                    }
                    if (!separate)
                      exact(SplitColor, 0, i, 3, ReferenceColor,
                            "dedicated_COLOR1_RGB");
                    if (!flat && !separate) {
                      exact(SplitCentroid, 0, i, 3, ReferenceCentroid,
                            "dedicated_TEX9_RGB");
                      exact(SplitPlain, 0, i, 3, ReferencePlain,
                            "dedicated_TEX9_RGB");
                      exact(SplitCentroid, 0, i, 3, SplitPlain,
                            "no_MSAA_centroid");
                    }
                    if (!dep)
                      for (unsigned c = 0; c < 4; ++c)
                        need(images[0][2][i + c] == 0,
                             "depth-off target untouched");
                    if (!covered)
                      continue;
                    ++coverage;
                    const auto b = weights(x, y);
                    if (*std::min_element(b.begin(), b.end()) < .05)
                      continue;
                    ++analytic;
                    double denominator = 0;
                    for (unsigned v = 0; v < 3; ++v)
                      denominator += b[v] / vertices[v].clip[3];
                    for (unsigned c = 0; c < 3; ++c) {
                      double expected = 0;
                      for (unsigned v = 0; v < 3; ++v)
                        expected +=
                            b[v] * vertices[v].rgba[c] / vertices[v].clip[3];
                      expected = effective_flat ? vertices[0].rgba[c]
                      ...
                      const double tolerance =
                          1e-12 + std::ldexp(64., -23) * std::abs(expected);
                      for (unsigned k : selected) {
                        if (k == Native)
                          continue;
                        ...
                        if (!std::isfinite(actual) || error > tolerance) {
                          std::printf(
                              "DIFF case=%u kind=%s pixel=%u channel=%u "
                              "actual=%.12g expected=%.12g tolerance=%.12g\n",
                              id, labels[k], i / 4, c, actual, expected,
                              tolerance);
                ...
                need(coverage > 1000 && analytic > 700,
                     "substantial raster interior");
                for (unsigned k : selected)
                  bank[dep][k].coverage += coverage;
                total += coverage;
                worst = std::max(worst, maximum);
                std::printf(
                    "CASE id=%u flat=%u effective_flat=%u depth=%u pattern=%u "
                    "fog=%u "
        ...
        for (unsigned k = 0; k < count; ++k)
          if (admitted(k))
            std::printf("PROGRAM depth=%u kind=%s draws=%u coverage=%u\n", dep,
                        labels[k], bank[dep][k].draws, bank[dep][k].coverage);
      ...
      for (unsigned i = 1; i < 3; ++i)
        api(d->SetRenderTarget(i, nullptr), "unbind MRT");
      ...
      std::printf("RESULT %s cases=%u creates=%u negatives=%u coverage=%u "
                  "max_fraction=%.9g\n",
                  diagnostic ? "DIAGNOSTIC" : "PASS", id, creates, negatives,
                  total, worst);
    }
