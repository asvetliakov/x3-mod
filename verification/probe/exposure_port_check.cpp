...
struct Scene { const char* name; std::vector<float> mean, max; };
std::vector<Scene> scenes() {
    ...
    { Scene s{"sky", std::vector<float>(256, floor_log), std::vector<float>(256, floor_log)};
      for (unsigned y = 2; y < 6; ++y) for (unsigned x = 2; x < 6; ++x) s.mean[y * 16 + x] = s.max[y * 16 + x] = -3.820943f;
      out.push_back(s); }
    ...
    { Scene s{"sparks", std::vector<float>(256, -5.443855f), std::vector<float>(256, -5.443855f)};
      for (unsigned i : {17u, 70u, 133u, 196u, 238u}) { s.mean[i] = (4.f * 6.f + 12.f * -5.443855f) / 16.f; s.max[i] = 6.f; }
      out.push_back(s); }
    ...
    { Scene s{"emitter", std::vector<float>(256, floor_log), std::vector<float>(256, floor_log)};
      for (unsigned y = 0; y < 16; ++y) for (unsigned x = 0; x < 8; ++x) s.mean[y * 16 + x] = s.max[y * 16 + x] = 0.f;
      for (unsigned y = 4; y < 12; ++y) for (unsigned x = 4; x < 12; ++x) s.mean[y * 16 + x] = s.max[y * 16 + x] = -2.473931f;
      out.push_back(s); }
    return out;
}
std::vector<float> weights16() { std::vector<float> w(256); tile_weights(w.data(), 16, 16, ExposureParams{}.meter_edge_weight); return w; }
...
void print_statistics(const char* tag, const char* name, const MeterStatistics& m, const ExposureTarget& t) {
    std::printf("%s %s %u %u %.9g %.9g %.9g %.9g %.9g %u %.9g %.9g %.9g %.9g\n", tag, name, m.tiles, m.lit, double(m.avg_log_l), double(m.lit_fraction),
                double(m.lit_median_log), double(m.lit_mean_log), double(m.p99_max_log), unsigned(m.neutral), double(t.ev_key), double(t.ev_limit), double(t.ev_target), double(m.lit_weight));
}
} // namespace
...
int main() {
    ...
    std::printf("params %.9g %.9g %.9g %.9g %.9g %.9g %.9g %.9g %.9g %.9g %.9g %.9g %.9g %.9g %.9g %.9g %.9g %u\n", double(p.key), double(p.ev_offset), double(p.ev_min), double(p.ev_max),
                ...
                double(p.meter_bg), double(p.meter_min_lit), double(p.white_target), double(p.key_pull), double(p.ev_deadband), double(p.meter_edge_weight), double(kTonemapWhite), kMeterTileMax);
    // The centre weights of the 16x16 tile image.
    { const auto w = weights16(); std::printf("weights"); for (float v : w) std::printf(" %.9g", double(v)); std::printf("\n"); }
    ...
    // The chain to a tile image: 40x24 level-0 values stop at 10x6 (tile_max 16).
    {
        ...
        for (unsigned i = 0; i < w * h; ++i) { level[i] = float(-8.0 + 0.05 * double(i % 23)); peak[i] = level[i] + float(0.5 * double(i % 3)); }
        unsigned rw = w, rh = h;
    ...
    for (double v : {-5.443855, -0.246413, -2.582891, -13.2877124, 6.0, 0.0}) {
        std::vector<float> mean(256, float(v)); std::vector<TileSample> scratch(256);
        const MeterStatistics m = meter_statistics(mean.data(), mean.data(), nullptr, 256, scratch.data(), p);
        std::printf("target %.9g %.9g %.9g %.9g\n", v, double(ev_key(float(v), p)), double(ev_limit(float(v), p)), double(ev_target(m, p)));
    }
    for (double held : {0.0, 1.0, -2.0})
        for (double fresh : {0.0, 0.2, 0.25, 0.26, 1.1, 1.3, -1.8, -2.5, 2.0})
            std::printf("deadband %.9g %.9g %.9g %.9g\n", held, fresh, double(apply_deadband(float(held), float(fresh), false, p)), double(apply_deadband(float(held), float(fresh), true, p)));
    ...
    {
        auto uniform = [&](float v) { std::vector<float> mean(256, v); std::vector<TileSample> scratch(256); return meter_statistics(mean.data(), mean.data(), nullptr, 256, scratch.data(), p); };
        ExposureState s(p);
        std::printf("simulate");
        ...
        for (float v : levels) { b.step(uniform(v), 0.2f); std::printf(" %.9g %.9g %.9g", double(b.ev_fresh()), double(b.ev_target()), double(b.ev())); }
        std::printf("\n");
        ExposureParams q = p; q.ev_offset = 1.f; q.tau_up = .2f; q.tau_down = .6f;
        ExposureState o(q);
        std::printf("simulate_offset");
        for (unsigned f = 0; f < 10; ++f) { std::printf(" %.9g", double(o.ev())); o.step(uniform(f < 5 ? -5.443855f : -0.246413f), 0.033f); }
        std::printf(" %.9g\n", double(o.ev()));
        ...
        for (const auto& sc : scenes()) {
            ...
            for (unsigned f = 0; f < 10; ++f) { std::printf(" %.9g", double(z.ev())); z.step(m, 0.016f); }
        }
        std::printf(" %.9g\n", double(z.ev()));
    }
