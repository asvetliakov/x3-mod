 ...
 try{
    ...
    seed=fp();seed.x87.control=(seed.x87.control&0xffff0000)|0x007f;seed.x87.status&=0xffff0000;seed.mxcsr=0x1f80;fp(seed);
    {
      ...
      if(enabled){
        ...
        {auto game=seed;game.x87.control=(game.x87.control&0xffff0000)|0x027f;game.mxcsr=0x9fc0;Com<ID3DXMesh>game_reference,game_hook;createMesh(create,device,input,&game_reference.p);createMesh(&D3DXCreateMesh,device,input,&game_hook.p);
            const auto before=lt::fixture_cache_statistics();fp(game);auto expected=generate(game_reference.p,input.epsilon,true);fp(game);auto first=generate(game_hook.p,input.epsilon,false);fp(game);auto second=generate(game_hook.p,input.epsilon,false);const auto after=lt::fixture_cache_statistics();fp(seed);
            parity(expected,first,"game FP state fill parity");parity(expected,second,"game FP state hit parity");
            require(after.bypass_reasons[static_cast<unsigned>(x3m::mesh_adjacency_cache::BypassReason::FloatingPoint)]==before.bypass_reasons[static_cast<unsigned>(x3m::mesh_adjacency_cache::BypassReason::FloatingPoint)]&&after.hits==before.hits+1&&after.misses==before.misses+1,"game FP state is keyed and reused, not bypassed");
            require(after.first_fp_available&&after.first_fp_supported,"first incoming FP state published as supported");
            std::printf("GAME_FP_STATE control=027f mxcsr=9fc0 keyed=1 bypassed=0 hit=1\n");}
      }
