#!/usr/bin/env python3
"""Run the built small-parts cull CPU fixture in the X3 bottle and write a compact result record.
...
EXE = ROOT / 'build/verification/cull-small-parts/cull_small_parts_fixture.exe'
OUT = ROOT / 'verification/results/cull-small-parts-cpu.json'
...
def main():
    ...
    replay = [l for l in run.stdout.splitlines() if l.startswith('REPLAY') or l.startswith('CENSUS')]
    ...
    print(json.dumps({k: record[k] for k in ('checks', 'failures', 'exit_status', 'bench_us', 'failure_lines', 'replay_lines')}))
