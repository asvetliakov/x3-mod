...
struct RealCounts {
    unsigned acquires = 0, acquire_ok = 0, deletes = 0, delete_ok = 0, releases = 0, release_ok = 0, imports = 0,
             import_ok = 0, destroys = 0, destroy_ok = 0;
};
...
static BOOL WINAPI shim_acquire(HCRYPTPROV* out, LPCSTR container, LPCSTR provider, DWORD type, DWORD flags) {
    ...
    if (flags & CRYPT_DELETEKEYSET) {
        ++real_counts.deletes;
        if (r) ++real_counts.delete_ok;
    } else if (r) {
        ++real_counts.acquire_ok;
        ++live_providers;
    }
    SetLastError(e);
    return r;
...
static BOOL WINAPI shim_release(HCRYPTPROV provider, DWORD flags) {
    ++real_counts.releases;
    const BOOL r = CryptReleaseContext(provider, flags);
    const DWORD e = GetLastError();
    if (r) {
        ++real_counts.release_ok;
        --live_providers;
    }
    SetLastError(e);
    return r;
}
static BOOL WINAPI shim_import(HCRYPTPROV provider, const BYTE* data, DWORD length, HCRYPTKEY key, DWORD flags,
                               HCRYPTKEY* out) {
    ++real_counts.imports;
    const BOOL r = CryptImportKey(provider, data, length, key, flags, out);
    const DWORD e = GetLastError();
    if (r) {
        ++real_counts.import_ok;
        ++live_keys;
    }
    SetLastError(e);
    return r;
...
static BOOL WINAPI shim_destroy(HCRYPTKEY key) {
    ++real_counts.destroys;
    const BOOL r = CryptDestroyKey(key);
    const DWORD e = GetLastError();
    if (r) {
        ++real_counts.destroy_ok;
        --live_keys;
    }
    SetLastError(e);
    return r;
...
struct Api {
    cc::AcquireFn acquire;
    cc::ReleaseFn release;
    cc::ImportFn import_key;
    cc::DestroyKeyFn destroy_key;
};
...
static const char* step_names[step_count] = {"delete_before", "new_keyset",  "import_key", "create_hash",
                                             "hash_data",     "verify",      "hash_size",  "hash_value",
...
struct Record {
    ...
    DWORD digest_len = 0;
    BYTE digest[64]{};
    uint64_t ticks = 0, context_ticks = 0;
};
static void record(Record& r, unsigned step, BOOL result) {
    r.steps[step].result = result ? 1 : 0;
    r.steps[step].error = result ? 0 : GetLastError();
}
static void run_check(const Api& api, const BYTE* message, DWORD message_len, const BYTE* signature,
                      DWORD signature_len, const BYTE* blob, DWORD blob_len, Record& r) {
    HCRYPTPROV prov = 0;
    HCRYPTHASH hash = 0;
    HCRYPTKEY key = 0;
    SetLastError(0);
    ...
    const BOOL acquired = api.acquire(&prov, CONTAINER, PROVIDER, PROV_RSA_FULL, CRYPT_NEWKEYSET);
    record(r, 1, acquired);
    const uint64_t t1 = tick();
    r.context_ticks += t1 - t0;
    if (acquired) {
        const BOOL imported = api.import_key(prov, blob, blob_len, 0, 0, &key);
        record(r, 2, imported);
        if (imported) {
            const BOOL created = CryptCreateHash(prov, CALG_MD5, 0, 0, &hash);
            record(r, 3, created);
            if (created) {
                const BOOL hashed = CryptHashData(hash, message, message_len, 0);
                record(r, 4, hashed);
                if (hashed) {
                    const BOOL verified = CryptVerifySignatureA(hash, signature, signature_len, key, nullptr, 0);
                    record(r, 5, verified);
                    r.verified = verified != 0;
                    if (verified) {
                        DWORD size = 0, size_len = sizeof size;
                        const BOOL got_size = CryptGetHashParam(hash, HP_HASHSIZE, reinterpret_cast<BYTE*>(&size),
                                                                &size_len, 0);
                        record(r, 6, got_size);
                        if (got_size && size <= sizeof r.digest) {
                            r.digest_len = size;
                            record(r, 7, CryptGetHashParam(hash, HP_HASHVAL, r.digest, &r.digest_len, 0));
                        }
                    }
                }
            }
...
static bool same(const Record& a, const Record& b, char* detail, size_t detail_len) {
    for (unsigned i = 0; i < step_count; ++i) {
        if (a.steps[i].result != b.steps[i].result) {
            snprintf(detail, detail_len, "step=%s result off=%d on=%d", step_names[i], a.steps[i].result,
                     b.steps[i].result);
            return false;
        }
        if (a.steps[i].result == 0 && a.steps[i].error != b.steps[i].error) {
            snprintf(detail, detail_len, "step=%s error off=0x%08lx on=0x%08lx", step_names[i],
                     (unsigned long)a.steps[i].error, (unsigned long)b.steps[i].error);
            return false;
        }
    }
    if (a.verified != b.verified) {
        snprintf(detail, detail_len, "verified off=%u on=%u", unsigned(a.verified), unsigned(b.verified));
        return false;
    }
    if (a.verified && (a.digest_len != b.digest_len || memcmp(a.digest, b.digest, a.digest_len))) {
        snprintf(detail, detail_len, "digest differs len off=%lu on=%lu", (unsigned long)a.digest_len,
                 (unsigned long)b.digest_len);
        return false;
    }
    return true;
}
struct Timing {
    double total_ms = 0, mean_us = 0, median_us = 0, min_us = 0, max_us = 0, context_mean_us = 0;
};
static Timing timing(const std::vector<Record>& records) {
    Timing t;
    if (records.empty()) return t;
    const double f = frequency();
    std::vector<uint64_t> ticks;
    uint64_t sum = 0, context = 0;
    for (const auto& r : records) {
        ticks.push_back(r.ticks);
        sum += r.ticks;
        context += r.context_ticks;
    }
    std::sort(ticks.begin(), ticks.end());
    t.total_ms = double(sum) * 1e3 / f;
    t.mean_us = double(sum) * 1e6 / f / double(records.size());
    t.median_us = double(ticks[ticks.size() / 2]) * 1e6 / f;
    t.min_us = double(ticks.front()) * 1e6 / f;
    t.max_us = double(ticks.back()) * 1e6 / f;
    t.context_mean_us = double(context) * 1e6 / f / double(records.size());
    return t;
}
static void print_mode(const char* mode, const std::vector<Record>& records, const RealCounts& before,
                       const RealCounts& after) {
    unsigned verified = 0;
    for (const auto& r : records) verified += r.verified;
    const Timing t = timing(records);
    printf(
        "CRYPT_MODE mode=%s iterations=%u verified=%u rejected=%u real_acquires=%u real_acquire_ok=%u real_deletes=%u real_delete_ok=%u real_releases=%u real_imports=%u real_destroys=%u total_ms=%.3f mean_us=%.3f median_us=%.3f min_us=%.3f max_us=%.3f context_mean_us=%.3f\n",
        mode, unsigned(records.size()), verified, unsigned(records.size()) - verified, after.acquires - before.acquires,
        after.acquire_ok - before.acquire_ok, after.deletes - before.deletes, after.delete_ok - before.delete_ok,
        after.releases - before.releases, after.imports - before.imports, after.destroys - before.destroys, t.total_ms,
        t.mean_us, t.median_us, t.min_us, t.max_us, t.context_mean_us);
}
...
static bool sign(HCRYPTPROV signer, const std::vector<BYTE>& message, std::vector<BYTE>& signature) {
    ...
    if (ok) {
        ...
        ok = CryptSignHashA(hash, AT_SIGNATURE, nullptr, 0, signature.data(), &len) != 0;
        signature.resize(len);
    }
...
int main(int argc, char** argv) {
    ...
    blob.resize(blob_len);
    char detail[160];
    snprintf(detail, sizeof detail, "blob_len=%lu", (unsigned long)blob_len);
    ...
    for (unsigned i = 0; i < iterations; ++i) {
        ...
        if (i % 16 == 15 && !signatures[i].empty())
            signatures[i].pop_back(); // truncated
        else if (i % 4 == 3 && !signatures[i].empty())
            signatures[i][signatures[i].size() / 2] ^= 0x5a; // corrupted
        else
            ++expected_verified;
    }
    ...
    // Start from the game's steady state: no container.
    {
        ...
        CryptAcquireContextA(&scratch, CONTAINER, PROVIDER, PROV_RSA_FULL, CRYPT_DELETEKEYSET);
    }
    // Pass 1: cache off.
    std::vector<Record> off(iterations), on(iterations);
    const RealCounts before_off = real_counts;
    for (unsigned i = 0; i < iterations; ++i)
        run_check(direct, messages[i].data(), DWORD(messages[i].size()), signatures[i].data(),
                  DWORD(signatures[i].size()), blob.data(), blob_len, off[i]);
    ...
    cc::Originals originals;
    originals.acquire = shim_acquire;
    originals.release = shim_release;
    originals.import_key = shim_import;
    originals.destroy_key = shim_destroy;
    ...
    for (unsigned i = 0; i < iterations; ++i)
        run_check(cached, messages[i].data(), DWORD(messages[i].size()), signatures[i].data(),
                  DWORD(signatures[i].size()), blob.data(), blob_len, on[i]);
    ...
    for (unsigned i = 0; i < iterations; ++i) {
        verified_off += off[i].verified;
        verified_on += on[i].verified;
        if (!same(off[i], on[i], detail, sizeof detail)) {
            ++mismatches;
            if (mismatches <= 5) printf("FAIL compare iteration=%u %s\n", i, detail);
        }
    }
    ...
    check(iterations < 2 || (off[1].steps[0].result == 0 && on[1].steps[0].result == 0 &&
                             on[1].steps[0].error == off[1].steps[0].error),
          "delete_before_fails_identically");
    ...
    printf(
        "CRYPT_STATS acquires=%llu hits=%llu misses=%llu failed_passthrough=%llu releases_suppressed=%llu imports=%llu import_hits=%llu deletes_emulated=%llu deletes_passthrough=%llu busy_passthrough=%llu evictions=%llu releases=%llu import_passthrough=%llu destroys=%llu destroys_suppressed=%llu providers_cached=%u keys_cached=%u probe_error=0x%08x\n",
        (unsigned long long)s.acquires, (unsigned long long)s.hits, (unsigned long long)s.misses,
        (unsigned long long)s.failed_passthrough, (unsigned long long)s.releases_suppressed,
        (unsigned long long)s.imports, (unsigned long long)s.import_hits, (unsigned long long)s.deletes_emulated,
        (unsigned long long)s.deletes_passthrough, (unsigned long long)s.busy_passthrough,
        (unsigned long long)s.evictions, (unsigned long long)s.releases, (unsigned long long)s.import_passthrough,
        (unsigned long long)s.destroys, (unsigned long long)s.destroys_suppressed, s.providers_cached, s.keys_cached,
    ...
    check(s.failed_passthrough == 1 && s.deletes_passthrough == 1 && s.deletes_emulated == uint64_t(iterations) * 2 - 1,
          "stats_deletes_emulated_after_the_first");
    check(s.releases_suppressed == iterations && s.imports == iterations && s.import_hits == uint64_t(iterations) - 1 &&
              s.destroys_suppressed == iterations,
          "stats_release_import_destroy");
    check(s.providers_cached == 1 && s.keys_cached == 1 && s.evictions == 0, "stats_one_provider_one_key");
    check(after_on.acquires - before_on.acquires == 2 && after_on.acquire_ok - before_on.acquire_ok == 1 &&
              after_on.releases == before_on.releases && after_on.imports - before_on.imports == 1 &&
              after_on.destroys == before_on.destroys,
          "real_calls_on_two_acquires_one_import");
    check(after_off.acquires - before_off.acquires == iterations * 3 &&
              after_off.releases - before_off.releases == iterations,
          "real_calls_off_three_acquires_per_check");
    ...
    // entry; the caller's handle stays valid and its release/destroy are real.
    {
        ...
        check(real_counts.destroys == b.destroys + 1 && real_counts.releases == b.releases + 1,
              "evict_destroy_release_real_calls");
        ...
        Record r;
        run_check(cached, messages[0].data(), DWORD(messages[0].size()), signatures[0].data(),
                  DWORD(signatures[0].size()), blob.data(), blob_len, r);
    ...
    if (exists) {
        CryptReleaseContext(leftover, 0);
        HCRYPTPROV scratch = 0;
        CryptAcquireContextA(&scratch, CONTAINER, PROVIDER, PROV_RSA_FULL, CRYPT_DELETEKEYSET);
    }
    ...
    printf("CRYPT_SHUTDOWN released=%u real_acquires=%u real_releases=%u real_destroys=%u container_absent=%u\n",
           unsigned(released), real_counts.acquires - before_shutdown.acquires,
           real_counts.releases - before_shutdown.releases, real_counts.destroys - before_shutdown.destroys,
           unsigned(!exists));
    check(real_counts.releases - before_shutdown.releases == 1 &&
              real_counts.destroys - before_shutdown.destroys == 1 &&
              real_counts.deletes - before_shutdown.deletes == 1,
          "shutdown_real_calls");
    ...
    CryptReleaseContext(signer, 0);
    {
        HCRYPTPROV scratch = 0;
        CryptAcquireContextA(&scratch, SIGNER, PROVIDER, PROV_RSA_FULL, CRYPT_DELETEKEYSET);
    }
