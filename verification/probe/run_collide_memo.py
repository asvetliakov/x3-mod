...
SCENARIOS = ('static', 'one_step', 'approach', 'modes', 'addresses', 'expiry', 'overflow', 'random', 'verify')
...
def parse(stdout):
    ...
    return {'checks': int(total.group(1)) if total else None, 'failures': int(total.group(2)) if total else None,
            ...
            'approach': _fields(first('APPROACH ')), 'verify': _fields(first('VERIFY ')), 'window_line': first('WINDOW ')[7:] or None,
...
def accepted(record):
    ...
    return bool(record.get('exit_status') == 0 and record.get('failures') == 0 and summary and all(summary[k] == 0 for k in MUST_BE_ZERO) and summary['hits'] > 0
                and summary['contacts'] > 0 and set(record.get('scenarios', {})) == set(SCENARIOS) and record.get('bench'))
...
def main():
    ...
    print(json.dumps({k: record[k] for k in ('checks', 'failures', 'exit_status', 'elapsed_s', 'summary', 'scenarios', 'bench', 'approach', 'verify', 'failure_lines')}))
