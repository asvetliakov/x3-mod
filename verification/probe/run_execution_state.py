...
def run():
    ...
    try:
        ...
        for suffix, extra in [('', []), ('-sanitized', ['-fsanitize=address,undefined', '-fno-omit-frame-pointer']), ('-thread-sanitized', ['-fsanitize=thread', '-fno-omit-frame-pointer'])]:
            ...
            if not match or completed.stderr or int(match[1]) != 61408:
        ...
        if any(item['checks'] != report['runs'][0]['checks'] for item in report['runs']):
