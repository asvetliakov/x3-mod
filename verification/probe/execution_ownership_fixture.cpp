...
struct Barrier {
 IDirect3DDevice9* device=nullptr;IDirect3DQuery9* query=nullptr;
 HANDLE entered=CreateEventA(nullptr,TRUE,FALSE,nullptr),resume=CreateEventA(nullptr,TRUE,FALSE,nullptr),read_done=CreateEventA(nullptr,TRUE,FALSE,nullptr);
 unsigned operation=0;HRESULT result=E_UNEXPECTED,read_result=E_UNEXPECTED;ExecutionView observed;volatile LONG timed_out=0;
 ~Barrier(){CloseHandle(entered);CloseHandle(resume);CloseHandle(read_done);}
 static Barrier* active;
 static HRESULT WINAPI pause_scene(IDirect3DDevice9*){return pause();}
 static HRESULT WINAPI pause(){auto& b=*active;SetEvent(b.entered);if(WaitForSingleObject(b.resume,5000)!=WAIT_OBJECT_0)InterlockedExchange(&b.timed_out,1);return S_OK;}
 static HRESULT WINAPI end_scene(IDirect3DDevice9*){return S_OK;}
 static HRESULT WINAPI end_block(IDirect3DDevice9*,IDirect3DStateBlock9** out){if(out)*out=nullptr;return S_OK;}
 static HRESULT WINAPI issue(IDirect3DQuery9*,DWORD flags){return flags==D3DISSUE_BEGIN?pause():S_OK;}
 static DWORD WINAPI writer(void* p){auto& b=*static_cast<Barrier*>(p);b.result=b.operation==0?b.device->BeginScene():b.operation==1?b.device->BeginStateBlock():b.query->Issue(D3DISSUE_BEGIN);return 0;}
 static DWORD WINAPI reader(void* p){auto& b=*static_cast<Barrier*>(p);b.read_result=get_execution_view(b.device,&b.observed);SetEvent(b.read_done);return 0;}
};
...
HRESULT WINAPI capture_query(IDirect3DDevice9* d,D3DQUERYTYPE type,IDirect3DQuery9** out){auto hr=original_create_query(d,type,out);if(SUCCEEDED(hr)&&out&&*out){captured_query=*out;captured_query->AddRef();}return hr;}
void threads(IDirect3D9* factory,HWND window,unsigned operation){
 Device d(factory,window);Barrier b;b.device=d.app;b.operation=operation;Barrier::active=&b;
 check(b.entered&&b.resume&&b.read_done,"Win32 barrier events");
 if(operation==2){
  auto table=*reinterpret_cast<void***>(d.native);original_create_query=reinterpret_cast<CreateQueryFn>(table[118]);
  {Patch p(d.native,119,118,reinterpret_cast<void*>(&capture_query));require(d.app->CreateQuery(D3DQUERYTYPE_OCCLUSION,&b.query),"thread query");}
  check(captured_query!=nullptr,"retained actual native query for test-only Issue barrier");
 }
 {
  Patch pause(operation==2?static_cast<void*>(captured_query):static_cast<void*>(d.native),operation==2?8:119,
              operation==2?6:operation==0?41:60,operation==2?reinterpret_cast<void*>(&Barrier::issue):reinterpret_cast<void*>(&Barrier::pause_scene));
  Patch finish(d.native,119,operation==1?61:42,operation==1?reinterpret_cast<void*>(&Barrier::end_block):reinterpret_cast<void*>(&Barrier::end_scene));
  HANDLE writer=CreateThread(nullptr,0,&Barrier::writer,&b,0,nullptr);check(writer!=nullptr,"native barrier writer thread");
  bool entered=WaitForSingleObject(b.entered,3000)==WAIT_OBJECT_0;check(entered,"native method entered before snapshot");
  HANDLE reader=CreateThread(nullptr,0,&Barrier::reader,&b,0,nullptr);check(reader!=nullptr,"concurrent observer reader");
  bool read=WaitForSingleObject(b.read_done,2000)==WAIT_OBJECT_0;check(read,"snapshot returns while native call blocked (no observer mutex held)");
  if(read){check(b.read_result==S_OK&&!b.observed.known&&!b.observed.queries_idle&&b.observed.in_flight==1&&b.observed.reason==ExecutionReason::NativeCallInFlight,"in-flight native transition refuses snapshot");
   HRESULT second;if(operation==0)second=d.app->EndScene();else if(operation==1){IDirect3DStateBlock9* output=nullptr;second=d.app->EndStateBlock(&output);if(output)output->Release();}else second=b.query->Issue(D3DISSUE_END);
   check(second==S_OK,"overlapping native result returned unchanged");
   check(!view(d.app).known&&view(d.app).reason==ExecutionReason::OverlappingNativeCalls,"reversed completion permanently tainted before first result");}
  SetEvent(b.resume);
  if(WaitForSingleObject(writer,3000)!=WAIT_OBJECT_0||WaitForSingleObject(reader,3000)!=WAIT_OBJECT_0)ExitProcess(9);
  CloseHandle(writer);CloseHandle(reader);check(b.result==S_OK&&!b.timed_out,"blocked native result completes normally");
 }
 ...
 if(captured_query){captured_query->Release();captured_query=nullptr;}
 d.reset();check(!view(d.app).known,"overlap remains unknown after actual Reset");Barrier::active=nullptr;
 std::printf("CASE thread operation=%u PASS\n",operation);
}
struct CallState {
 unsigned char x87[108];unsigned mxcsr;DWORD error;
 CallState():error(GetLastError()){asm volatile("fnsave %0\n\tfrstor %0\n\tstmxcsr %1":"=m"(x87),"=m"(mxcsr)::"memory");}
 void restore()const{asm volatile("frstor %0\n\tldmxcsr %1"::"m"(x87),"m"(mxcsr):"memory");SetLastError(error);}
 bool equal(const CallState& other)const{return !std::memcmp(x87,other.x87,108)&&mxcsr==other.mxcsr&&error==other.error;}
};
CallState *cpu_seen=nullptr,*cpu_output=nullptr;unsigned cpu_scene_calls=0;
HRESULT WINAPI cpu_scene(IDirect3DDevice9*){*cpu_seen=CallState{};++cpu_scene_calls;cpu_output->restore();return E_FAIL;}
HRESULT WINAPI cpu_reset(IDirect3DDevice9*,D3DPRESENT_PARAMETERS*){*cpu_seen=CallState{};cpu_output->restore();return E_FAIL;}
HRESULT WINAPI cpu_query(IDirect3DDevice9* d,D3DQUERYTYPE type,IDirect3DQuery9** out){*cpu_seen=CallState{};auto hr=original_create_query(d,type,out);cpu_output->restore();return hr;}
void cpu_preservation(IDirect3D9* factory,HWND window,bool disabled=false){
 cpu_scene_calls=0;
 Device d(factory,window);CallState original;
 unsigned short control=0x077f;unsigned mxcsr=0x3fa0;
 asm volatile("fninit\n\tfld1\n\tfldcw %0\n\tldmxcsr %1"::"m"(control),"m"(mxcsr):"memory");SetLastError(0x99224466);CallState incoming;
 control=0x0b7f;mxcsr=0x5f80;asm volatile("fninit\n\tfldz\n\tfldcw %0\n\tldmxcsr %1"::"m"(control),"m"(mxcsr):"memory");SetLastError(0xaabb1177);CallState outgoing;original.restore();
 for(unsigned operation=0;operation<(disabled?1u:3u);++operation){CallState seen;cpu_seen=&seen;cpu_output=&outgoing;
  auto table=*reinterpret_cast<void***>(d.native);original_create_query=reinterpret_cast<CreateQueryFn>(table[118]);
  Patch p(d.native,119,operation==0?41:operation==1?118:16,operation==0?reinterpret_cast<void*>(&cpu_scene):operation==1?reinterpret_cast<void*>(&cpu_query):reinterpret_cast<void*>(&cpu_reset));
  IDirect3DQuery9* q=nullptr;incoming.restore();
  HRESULT hr=operation==0?d.app->BeginScene():operation==1?d.app->CreateQuery(D3DQUERYTYPE_OCCLUSION,&q):d.app->Reset(&d.pp);
  CallState after;original.restore();
  ...
  check(hr==(operation==1?S_OK:E_FAIL),"CPU control native HRESULT unchanged");if(q)q->Release();
 }
 ExecutionView v;incoming.restore();get_execution_view(d.app,&v);CallState getter;original.restore();check(getter.equal(incoming),"snapshot lock preserves caller CPU state");
 incoming.restore();invalidate_execution_state(d.app);CallState invalidate;original.restore();check(invalidate.equal(incoming),"invalidation lock preserves caller CPU state");
...
void timing(IDirect3D9* factory,HWND window){
 Device d(factory,window);constexpr unsigned count=1000;LARGE_INTEGER frequency,start,stop;QueryPerformanceFrequency(&frequency);
 QueryPerformanceCounter(&start);for(unsigned i=0;i<count;++i){require(d.native->BeginScene(),"baseline Begin");require(d.native->EndScene(),"baseline End");}QueryPerformanceCounter(&stop);auto baseline=stop.QuadPart-start.QuadPart;
 QueryPerformanceCounter(&start);for(unsigned i=0;i<count;++i){require(d.app->BeginScene(),"observed Begin");require(d.app->EndScene(),"observed End");}QueryPerformanceCounter(&stop);auto observed=stop.QuadPart-start.QuadPart;
 check(view(d.app).known&&!view(d.app).scene_open&&view(d.app).in_flight==0,"timed completed scope pairs remain known");
 std::printf("TIMING pairs=%u native_ticks=%lld observer_ticks=%lld frequency=%lld\n",count,baseline,observed,frequency.QuadPart);
...
int main(){try{
 ...
 cpu_preservation(factory,window);timing(factory,window);
