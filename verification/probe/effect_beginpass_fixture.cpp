...
struct Counters {
    unsigned long long render_state = 0, sampler_state = 0, texture = 0, texture_stage = 0, vertex_shader = 0,
                       pixel_shader = 0, shader_constant = 0, constant_registers = 0, other = 0;
    unsigned long long total() const {
        return render_state + sampler_state + texture + texture_stage + vertex_shader + pixel_shader + shader_constant +
               other;
    }
};
...
public:
    explicit CountingManager(IDirect3DDevice9* device)
        : device_(device) {}
    Counters counters;
    bool counting = false;
    ...
    HRESULT WINAPI SetTextureStageState(DWORD stage, D3DTEXTURESTAGESTATETYPE type, DWORD value) override {
        counters.texture_stage += counting;
    ...
    HRESULT WINAPI SetSamplerState(DWORD sampler, D3DSAMPLERSTATETYPE type, DWORD value) override {
        counters.sampler_state += counting;
    ...
    HRESULT WINAPI SetPixelShaderConstantF(UINT index, const FLOAT* data, UINT count) override {
        constant(count);
        return device_->SetPixelShaderConstantF(index, data, count);
    }
    HRESULT WINAPI SetPixelShaderConstantI(UINT index, const INT* data, UINT count) override {
        constant(count);
        return device_->SetPixelShaderConstantI(index, data, count);
    }
    HRESULT WINAPI SetPixelShaderConstantB(UINT index, const WINBOOL* data, UINT count) override {
        constant(count);
        return device_->SetPixelShaderConstantB(index, data, count);
    }

private:
    void constant(UINT count) {
        if (counting) {
            ++counters.shader_constant;
            counters.constant_registers += count;
        }
    }
...
void run_regime(ID3DXEffect* effect, CountingManager& manager, const std::vector<Param>& params,
                const Textures& textures, const Regime& regime, D3DXHANDLE technique, unsigned pass_index,
                unsigned iterations, unsigned warmup, unsigned rep) {
    UINT passes = 0;
    ...
    std::printf("REGIME name=%s rep=%u iterations=%u us_median=%.4f us_mean=%.4f us_p05=%.4f us_p95=%.4f "
                ...
                regime.name, rep, iterations, median, sum / n, percentile(microseconds, 0.05),
                percentile(microseconds, 0.95), setter_ticks * 1e6 / frequency_hz / n, double(c.render_state) / n,
                double(c.sampler_state) / n, double(c.texture) / n, double(c.texture_stage) / n,
                double(c.vertex_shader) / n, double(c.pixel_shader) / n, double(c.shader_constant) / n,
                double(c.constant_registers) / n, double(c.other) / n, double(c.total()) / n);
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        check(window != nullptr, "window");
        {
            ...
            Com<IDirect3DTexture9> flat_a, flat_b;
            Com<IDirect3DCubeTexture9> cube_a, cube_b;
            ...
            for (UINT i = 0; i < desc.Parameters; ++i) {
                ...
                if (!handle || effect->GetParameterDesc(handle, &pd) != S_OK) {
                    ++skipped;
                    continue;
                }
                if (pd.Elements) {
                ...
                if (pd.Class == D3DXPC_SCALAR && pd.Type == D3DXPT_FLOAT)
                    params.push_back({handle, Kind::Float});
                else if (pd.Class == D3DXPC_SCALAR && pd.Type == D3DXPT_INT)
                    params.push_back({handle, Kind::Int});
                else if (pd.Class == D3DXPC_SCALAR && pd.Type == D3DXPT_BOOL)
                    params.push_back({handle, Kind::Bool});
                else if (pd.Class == D3DXPC_VECTOR && pd.Type == D3DXPT_FLOAT && pd.Columns <= 4)
                    params.push_back({handle, Kind::Vector});
                else if ((pd.Class == D3DXPC_MATRIX_ROWS || pd.Class == D3DXPC_MATRIX_COLUMNS) &&
                         pd.Type == D3DXPT_FLOAT && pd.Rows == 4 && pd.Columns == 4)
                    params.push_back({handle, Kind::Matrix});
                else if (pd.Type == D3DXPT_TEXTURE || pd.Type == D3DXPT_TEXTURE2D)
                    params.push_back({handle, Kind::Texture2D});
                else if (pd.Type == D3DXPT_TEXTURECUBE)
                    params.push_back({handle, Kind::TextureCube});
                else {
                    taken = false;
                    ++skipped;
                }
            ...
            std::printf(
                "FIXTURE effect=%s bytes=%lu parameters=%lu settable=%lu skipped=%u technique=%s technique_source=%s "
