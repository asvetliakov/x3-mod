...
int main() {
    ...
    material(10, 100); at(5); material(20, 100); at(7); material(30, 100);
    ...
    at(3); material(0, 0, true); at(4); material(40, 100);
    ...
    accumulator.view(2000, 1990); accumulator.view(2005, 1990); accumulator.view(2100, 2050); accumulator.view(2110, 2120);
    check(accumulator.particle_views == 4 && accumulator.ticks[2] == 60 && accumulator.view_skipped == 1 && accumulator.clock_errors == 1);
    ...
    for (unsigned i = 0; i < window_frames; ++i) {
        Sample f{};
        f.frame = i; f.materials = 1000 + i; f.particle_views = 3; f.passes = 900 + i; f.views = 9;
        for (unsigned p = 0; p < interval_count; ++p) f.interval_us[p] = (i + 1) * (p + 1);
        f.prepare_per_pass_ns = 2 * i; f.setup_per_pass_ns = 3 * i; f.self_us = i;
        reduction.add(f);
    }
    check(reduction.full() && reduction.close(summary));
    check(summary.frames == 300 && summary.frame == 299 && summary.materials_p50 == 1150 && summary.passes_p50 == 1050 && summary.particle_views_p50 == 3 && summary.views_p50 == 9);
    ...
    for (unsigned i = 0; i < 5; ++i) { Sample f{}; f.frame = i; f.interval_us[2] = (i * 7) % 5 + 1; reduction.add(f); }
    check(reduction.close(summary) && summary.frames == 5 && summary.interval_p50[2] == 3 && summary.frame == 4);
    for (unsigned i = 0; i < window_frames + 40; ++i) { Sample f{}; f.frame = i; f.interval_us[1] = 9; reduction.add(f); }
    check(reduction.count() == window_frames && reduction.close(summary) && summary.frames == 300 && summary.frame == 339 && summary.interval_p50[1] == 9);
    refuse_allocation = false;
    std::printf("residual_phases_host checks=%u failures=0 accumulator_bytes=%zu window_bytes=%zu dispatch_cost_ns=%llu\n",
