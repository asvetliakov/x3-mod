...
FarRun far_sequence(EdgeScene& s,const DWORD* resolver,const LineConfig& c,unsigned frames,bool validGate=true,bool failMasks=false){
    ...
    for(unsigned n=0;n<frames;++n){const unsigned index=n%latticePhases+1;const double jx=halton(index,2)-.5,jy=halton(index,3)-.5;
        ...
        auto in=flicker_inputs(s,f,jx,jy,false);in.far_weight=c.farW;in.far_filter=c.farA;in.far_d0=farD0;in.far_inv=validGate?farInv:0;in.far_speed_lo=farLo;in.far_speed_hi=farHi;in.thin_region_weight=c.thinW;in.thin_region_relax=c.relax;
...
void far_cases(IDirect3DDevice9* d,Compiler compiler,const DWORD* resolver){
    ...
    const LineConfig base{"far-base",0,0},weight{"far-weight-0.985",0,0,.985f,0},filter{"far-filter-A1",0,0,0,1},both{"far-weight+filter",0,0,.985f,1},
        withThin{"far-weight-0.985+thin-region-0.97",0,0,.985f,0,.97f,1};
    // ---- validation and refusals, hostile state, failed draw, Reset ----
    {s.render(far_objects(0),EdgeBackground{.035f,farBandDepth[0],1},0,0);Output out;const FlickerConfig none{"far-validation",0,0,.1f,.5f,false,.9f};
        ...
        TemporalPass pass;check("far validation initialize",pass.initialize(d,nullptr,resolver));check("far validation configure",pass.configure_far());check("far validation flicker",pass.configure_flicker());
    ...
    for(double drift:{0.,.04,.14,.3}){farDrift=drift;const auto baseRun=far_sequence(s,resolver,base,farFrames);const double baseRipple[3]={far_ripple(baseRun,0),far_ripple(baseRun,1),far_ripple(baseRun,2)};
        for(const LineConfig* c:{&base,&weight,&filter,&both,&withThin}){const auto run=c==&base?baseRun:far_sequence(s,resolver,*c,farFrames);const auto model=line_model(run,*c);double oracle=0,ageOracle=0;unsigned nearDiffers=0,nearPixels=0;
            for(unsigned n=0;n<farFrames;++n)for(UINT y=3;y+3<S;++y)for(UINT x=3;x+3<S;++x){oracle=std::max(oracle,double(std::fabs(px(run.output[n],x,y)-model.color[n][y*S+x])));
                ...
                if(x<9){++nearPixels;nearDiffers+=std::memcmp(&run.output[n][(y*S+x)*4],&baseRun.output[n][(y*S+x)*4],4*sizeof(float))!=0;}}
            ...
            double maskError=0;if(!run.mask.empty())for(unsigned b=0;b<3;++b){const UINT x=(farBandEdge[b]+farBandEdge[b+1])/2;for(UINT y=4;y<28;++y){const float w=far_gate_weight(px(run.depth.back(),x,y));
                maskError=std::max(maskError,double(std::fabs(px(run.mask[0],x,y,1)-(c->farW>0?w:0))));maskError=std::max(maskError,double(std::fabs(px(run.mask[0],x,y,0)-(c->farA>0?w:0))));}}
            ...
            ++numeric_checks;require(nearPixels>0&&nearDiffers==0,"near pixels (farw 0) bit-identical to the ungated plain resolve, all four channels");
    ...
    {const auto baseRun=far_sequence(s,resolver,base,32),gateOff=far_sequence(s,resolver,both,32,false),failed=far_sequence(s,resolver,both,32,true,true);
        ...
        {TemporalPass pass;check("far retry initialize",pass.initialize(d,nullptr,resolver));check("far retry configure",pass.configure_far());const FlickerConfig none{"far-retry",0,0,.1f,.5f,false,.9f};
