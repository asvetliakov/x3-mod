...
static void raise_original(void) {
    ULONG_PTR args[2] = {0x13572468, 0x24681357};
    RaiseException(test_exception_code, 0, 2, args);
}
static void before(const X3mCompositorFrame* frame, void* storage, void* context) {
    Payload* call = (Payload*)storage;
    unsigned i;
    int zero = 1;
    for (i = 0; i < X3M_CB_STORAGE_SIZE; ++i)
        if (((unsigned char*)storage)[i]) zero = 0;
    check(zero && ((uintptr_t)storage % 16) == 0, "opaque storage starts zero and aligned");
    check(context == &context_cookie && frame->flags == 0 &&
              frame->caller_pc == (uintptr_t)&compositor_fixture_caller_pc &&
              frame->caller_stack + 4 == bloom_return_stack_before,
          "binding context and actual caller stack/PC metadata");
    check(x3m_compositor_bridge_active() == 1 && !x3m_compositor_bridge_bind(&binding) &&
              !x3m_compositor_bridge_unbind(),
          "active invocation rejects binding replacement/unbind");
    ++pre_count;
    check(call->ticket == 0, "invocation starts empty");
    call->ticket = 1;
    ++live;
...
static void after(const X3mCompositorFrame* frame, void* storage, void* context) {
    Payload* call = (Payload*)storage;
    check(context == &context_cookie && frame->flags == X3M_CB_ORIGINAL_RETURNED && x3m_compositor_bridge_active() == 1,
          "post has valid original output and active binding");
    ++post_count;
    check(call->ticket == 1 && live == 1, "normal post owns ticket");
...
static void cleanup(const X3mCompositorFrame* frame, void* storage, void* context, int abnormal) {
    ...
    check(context == &context_cookie && x3m_compositor_bridge_active() == 1 && frame->flags == expected_flags,
          "cleanup sees active invocation and exact output validity");
    ++cleanup_count;
...
void bloom_return_original_body(void) {
    ...
    if (mode == 4) {
        volatile unsigned* invalid = (volatile unsigned*)0;
        *invalid = 0xdead;
    }
...
int bloom_return_filter(void* raw) {
    ...
    ++filter_count;
    check(p->ExceptionRecord->ExceptionCode == (mode == 4 ? EXCEPTION_ACCESS_VIOLATION : test_exception_code),
          "same original exception code reaches outer filter");
    if (mode == 4)
        check(p->ExceptionRecord->NumberParameters == 2 && p->ExceptionRecord->ExceptionInformation[0] == 1 &&
                  p->ExceptionRecord->ExceptionInformation[1] == 0,
              "hardware access violation information forwarded");
    else
        check(p->ExceptionRecord->NumberParameters == 2 && p->ExceptionRecord->ExceptionInformation[0] == 0x13572468 &&
                  p->ExceptionRecord->ExceptionInformation[1] == 0x24681357,
              "original exception parameters forwarded");
...
void bloom_return_caught(void) {
    check(x3m_compositor_bridge_active() == 0, "unwind decrements active count before outer handler");
    ++caught;
    check(live == 0 && cleanup_count == 1 && abnormal_count == 1, "ticket released before outer handler");
}
static void reset(unsigned next_mode) {
    mode = next_mode;
    continuation = 0;
    pre_count = post_count = cleanup_count = abnormal_count = live = caught = filter_count = original_count = 0;
}
...
static void same_cpu(const BloomCpu* a, const BloomCpu* b, const char* label) {
    ...
    int same = 1;
    for (i = 0; i < 9; ++i)
        if (i != 3 && a->regs[i] != b->regs[i]) same = 0;
    check(same, label);
    ...
    same = !memcmp(a->x87, b->x87, 2) && !memcmp(a->x87 + 4, b->x87 + 4, 2) && !memcmp(a->x87 + 8, b->x87 + 8, 2);
    {
        ...
        for (i = 0; i < 8; ++i)
            if (((tags >> (((top + i) & 7) * 2)) & 3) != 3 && memcmp(a->x87 + 28 + i * 10, b->x87 + 28 + i * 10, 10))
                same = 0;
    }
...
static void decline_before(const X3mCompositorFrame* frame, void* storage, void* context) {
    ++pre_count;
    check(context == &context_cookie && frame->flags == 0 && ((Payload*)storage)->ticket == 0,
          "declined preparation leaves no ticket");
    bloom_return_clobber();
}
static void decline_after(const X3mCompositorFrame* frame, void* storage, void* context) {
    ++post_count;
    check(context == &context_cookie && frame->flags == X3M_CB_ORIGINAL_RETURNED && original_count == 1 &&
              ((Payload*)storage)->ticket == 0,
          "declined admission still calls original exactly once");
    bloom_return_clobber();
}
static const X3mCompositorBinding declined_binding = {bloom_return_synthetic_original, decline_before, decline_after,
                                                      cleanup, &context_cookie};

int main(void) {
    unsigned skew, kind;
    BloomCpu direct_in, direct_out;
    ...
    for (skew = 0; skew < 16; skew += 4) {
        reset(0);
        bloom_return_test_call(bloom_return_synthetic_original, skew);
        direct_in = bloom_return_seen_input;
        direct_out = bloom_return_observed;
        check(direct_out.error == 0x87654321 && direct_in.error == 0x12345678,
              "synthetic input/output LastError deliberately differ");
        reset(0);
        bloom_return_test_call(bloom_return_bridge, skew);
        ...
        check(pre_count == 1 && post_count == 1 && cleanup_count == 1 && abnormal_count == 0 && live == 0 &&
                  original_count == 1,
              "normal call consumes ticket exactly once");
        ...
        check(bloom_return_observed.regs[3] + 4 == bloom_return_stack_after &&
                  bloom_return_stack_before == bloom_return_stack_after,
              "exact caller ESP restored");
        for (kind = 1; kind <= 4; ++kind) {
            reset(kind);
            expected_live = 1;
            bloom_return_catch(bloom_return_bridge, skew);
            check(caught == 1 && filter_count == 1 && cleanup_count == 1 && abnormal_count == 1 && live == 0,
                  "pre/original/post/hardware abnormal unwind cleaned once");
        ...
        check(continuation == 1 && caught == 0 && post_count == 1 && cleanup_count == 1 && abnormal_count == 0 &&
                  live == 0,
              "continued exception resumes original and cleans normally");
    ...
    for (skew = 0; skew < 16; skew += 4) {
        reset(6);
        bloom_return_test_call(bloom_return_bridge, skew);
        check(pre_count == 1 && post_count == 1 && cleanup_count == 1 && original_count == 1 && live == 0 &&
                  abnormal_count == 0 && x3m_compositor_bridge_active() == 0,
              "declined invocation returns and cleans normally");
    }
    check(x3m_compositor_bridge_unbind(), "quiescent unbind succeeds");
    printf(
        "COMPOSITOR BRIDGE RESULT checks=%u failures=%u normal=8 exceptional=16 continued=4 declined=4 stack_alignments=4\n",
        checks, failures);
    return failures ? 1 : 0;
}
