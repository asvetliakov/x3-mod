...
static void pass(std::uint64_t apply, std::uint64_t draw, std::uint64_t end) {
    accumulator.stamp(0, clock_ticks); at(apply); accumulator.stamp(1, clock_ticks);
    at(draw); accumulator.stamp(2, clock_ticks); at(end); accumulator.stamp(3, clock_ticks);
}
...
int main() {
    ...
    pass(10, 20, 30); at(100); pass(1, 2, 3); at(100); pass(5, 5, 5);
    ...
    check(accumulator.passes == 0 && accumulator.ticks[0] == 0 && accumulator.ticks[1] == 0 && accumulator.ticks[2] == 0 && accumulator.last == 0);
    // Frequency scaling: 10 MHz ticks (100 ns) convert to microseconds.
    pass(100, 200, 300); accumulator.take(8, 10000000, 0, s);
    ...
    at(5); accumulator.stamp(1, clock_ticks); at(7); accumulator.stamp(2, clock_ticks); at(9); accumulator.stamp(3, clock_ticks);
    check(accumulator.orphans == 1 && accumulator.passes == 1 && accumulator.ticks[0] == 0 && accumulator.ticks[1] == 7 && accumulator.ticks[2] == 9);
    // A backward clock is a clock error: the interval is skipped, the chain continues from the new stamp.
    accumulator.stamp(0, clock_ticks); accumulator.stamp(1, clock_ticks - 1); at(4); accumulator.stamp(2, clock_ticks); at(1); accumulator.stamp(3, clock_ticks);
    check(accumulator.clock_errors == 1 && accumulator.passes == 2 && accumulator.ticks[0] == 0 && accumulator.ticks[1] == 7 + 5 && accumulator.ticks[2] == 10);
    // A failed clock (0) degrades to counting: the chain resets, the pass still counts on pass_end.
    accumulator.stamp(0, clock_ticks); accumulator.stamp(1, 0); at(3); accumulator.stamp(2, clock_ticks); at(3); accumulator.stamp(3, 0);
    check(accumulator.clock_failures == 2 && accumulator.orphans == 2 && accumulator.passes == 3 && accumulator.ticks[2] == 10);
    ...
    pass(1, 1, 1); accumulator.take(9, 0, 5, s);
    ...
    for (unsigned i = 0; i < window_frames; ++i) {
        Sample f{};
        f.frame = i; f.passes = 1000 + i;
        for (unsigned p = 0; p < interval_count; ++p) { f.interval_us[p] = (i + 1) * (p + 1); f.sum_us += f.interval_us[p]; }
        f.view_submit_us = 10 * i; f.self_us = f.passes * site_count * dispatch_cost_ns / 1000;
        reduction.add(f);
    }
    check(reduction.full() && reduction.close(summary));
    check(summary.frames == 300 && summary.frame == 299 && summary.passes_p50 == 1150);
    check(summary.interval_p50[0] == 151 && summary.interval_p95[0] == 286 && summary.interval_p50[2] == 453 && summary.interval_p95[2] == 858);
    check(summary.sum_p50 == 151 * 6 && summary.view_submit_p50 == 1500 && summary.self_p50 == 1150 * site_count * dispatch_cost_ns / 1000);
