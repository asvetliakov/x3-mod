...
struct Budget {
    unsigned instructions, slots, max_temp;
};
Budget budget(const Words& result) {
    Budget b{0, 0, 0};
    std::array<unsigned, 16> dimensions{};
    for (std::size_t at = 1; at < result.size() - 1;) {
        ...
        if (op != 0xfffeu && op != 31 && op != 81) {
            ...
            for (unsigned n = 1; n <= count; ++n)
                if (kind(result[at + n]) == temp) b.max_temp = std::max(b.max_temp, index(result[at + n]));
            switch (op) {
            case 1:
            case 2:
            case 4:
...
struct Synthetic {
    Words code;
    Structure s;
    unsigned seed = 0, site = 0, final = 0;
};
Synthetic synthetic(unsigned mutation = 0, bool branch = false) {
    ...
    if (branch) {
        emit(f.code, 42, {});
        emit(f.code, mov, {dst(temp, 0), src(temp, 4)});
        emit(f.code, 43, {});
    }
    ...
    if (mutation == 3) {
        Words with{0xffff0300u};
        emit(with, dcl, {0x90000000u, dst(10, 0, xyzw)});
        with.insert(with.end(), f.code.begin() + 1, f.code.end());
        f.code.swap(with);
        f.seed += 3;
        f.site += 3;
        f.final += 3;
    ...
    require(structure(f.code.data(), f.code.size(), false, f.s, mutation != 6, default_abi, 7, false, false, branch),
            "synthetic structure");
    return f;
}
unsigned synthetic_checks() {
    const auto plan = [](const Synthetic& f, std::array<unsigned, 2> seeds, unsigned fill_site, unsigned fill_sum,
                         std::vector<SunEdit>* out = nullptr) {
        std::vector<SunEdit> edits;
        Word final_destination = 0;
        const bool ok = original_share_resources_free(f.code.data(), f.s) &&
                        original_sun_plan(f.code.data(), f.s, seeds, f.final, fill_site, fill_sum, edits,
                                          final_destination);
        if (out) *out = edits;
    ...
    unsigned refused = 0;
    {
        ...
        require(edits.size() == 4 && edits[0].at == f.seed - 3 && edits[0].words.size() == 8 * 3 + 4 &&
                    edits[1].at == f.site && edits[2].at == f.final && edits[3].at == f.final + 5 &&
                    edits[3].words.size() > 3,
                "plan edit sites");
    ...
    for (unsigned mutation : {1u, 2u, 3u, 5u, 6u}) {
        const auto f = synthetic(mutation);
        refused += !plan(f, {f.seed, 0}, 0, 0);
    }
    {
        const auto f = synthetic(0, true);
        refused += !plan(f, {f.seed, 0}, 0, 0);
    } // final inside a branch is fine, the other arm kills; here the branch arm MOV reads untracked r4: accepted
    require(
        refused == 10,
        "synthetic refusals: untracked fill sum, no/duplicate/misplaced seeds, _sat, sun*sun, sun texcoord, c221 DEF, r20 read");
    ...
    {
        Words body;
        emit(body, mov, {dst(temp, 9), src(temp, 5)});
        emit(body, mov, {dst(color_output, 2), src(temp, 9)});
        require(original_share_range_free(body.data(), 0, body.size(), false, 0, 0), "motion body inside r0-r10");
        Words bad = body;
        emit(bad, mov, {dst(temp, 11), src(temp, 5)});
        require(!original_share_range_free(bad.data(), 0, bad.size(), false, 0, 0), "motion body reaching r11 refused");
        ++refused;
        Words reads = body;
        emit(reads, mov, {dst(temp, 9), lane(constant, 221, 0)});
        require(!original_share_range_free(reads.data(), 0, reads.size(), false, 0, 0),
                "motion body reading c221 refused");
        ++refused;
        Words block;
        original_fill_block(block, 2, 5);
        require(original_share_range_free(block.data(), 0, block.size(), true, 2, 5),
                "authored fill block inside r12/r13/sum, c215/light");
        require(!original_share_range_free(block.data(), 0, block.size(), true, 3, 5),
                "fill block on another sum refused");
        ++refused;
    }
    return refused;
}
int main(int argc, char** argv) {
    try {
        ...
        std::cout << "{\"rows\":[";
        bool first = true;
        unsigned applied = 0, supported = 0, max_slots = 0, max_temp = 0;
        ...
        for (const auto& name : names) {
            ...
            const auto status = linear_material_original_sun_share_pixel_variant(original.data(), original.size(),
                                                                                 0.05f, probe, true, entered);
            ...
            unsigned final_rgb = xt ? xt->final_rgb : p->final_rgb, light = xt ? 6u : p->light0;
            if (xt) {
                for (const auto& u : xt->lights)
                    if (u.value == 6) seeds.push_back(u.operand);
            } else
                for (unsigned k = 0; k < 2; ++k)
                    if (p->color_source[k]) seeds.push_back(p->color_source[k]);
            ...
            for (bool depth : {false, true}) {
                ...
                for (unsigned f = 0; f < 2; ++f) {
                    ...
                    require(linear_material_original_sun_share_pixel_variant(original.data(), original.size(), fills[f],
                                                                             result, depth,
                                                                             share) == LinearMaterialResult::Applied,
                            "share admission for every K");
                    require(original == saved, "immutable input");
                    ...
                    slots[depth][f] = b.slots;
                    instructions[depth][f] = b.instructions;
                    control_slots[depth][f] = cb.slots;
                    temps[depth][f] = b.max_temp;
                    max_slots = std::max(max_slots, b.slots);
                    max_temp = std::max(max_temp, b.max_temp);
                    Words alias = original;
                    bool alias_share = false;
                    require(linear_material_original_sun_share_pixel_variant(alias.data(), alias.size(), fills[f],
                                                                             alias, depth, alias_share) ==
                                    LinearMaterialResult::Applied &&
                                alias == result && alias_share == share,
                            "input/output alias");
                    write(out / (name + "-oshare-" + std::to_string(f) + "-" + std::to_string(depth) + ".bin"), result);
                }
            }
            for (float invalid :
                 {-0.001f, 0.5001f, std::numeric_limits<float>::infinity(), std::numeric_limits<float>::quiet_NaN()}) {
                Words guard{91, 92};
                bool guard_share = true;
                require(linear_material_original_sun_share_pixel_variant(original.data(), original.size(), invalid,
                                                                         guard, true, guard_share) ==
                                LinearMaterialResult::InvalidConfig &&
                            guard == Words({91, 92}) && !guard_share,
                        "invalid fill rollback");
                Words alias = original;
                require(linear_material_original_sun_share_pixel_variant(alias.data(), alias.size(), invalid, alias,
                                                                         true, guard_share) ==
                                LinearMaterialResult::InvalidConfig &&
                            alias == saved,
            ...
            for (std::size_t offset : {std::size_t(0), std::size_t(2), original.size() / 2, original.size() - 1}) {
                ...
                bool broken_share = true;
                require(linear_material_original_sun_share_pixel_variant(broken.data(), broken.size(), 0.05f, broken,
                                                                         true, broken_share) !=
                                LinearMaterialResult::Applied &&
                            broken == before && !broken_share,
                        "corrupted original alias rollback");
            }
            {
                Words guard{91, 92};
                bool guard_share = true;
                require(linear_material_original_sun_share_pixel_variant(nullptr, original.size(), 0.05f, guard, true,
                                                                         guard_share) ==
                                LinearMaterialResult::InvalidInput &&
                            guard == Words({91, 92}) && !guard_share,
                        "null rollback");
                require(linear_material_original_sun_share_pixel_variant(original.data(), 1, 0.05f, guard, true,
                                                                         guard_share) ==
                                LinearMaterialResult::InvalidInput &&
                            guard == Words({91, 92}),
                        "short rollback");
                require(linear_material_original_sun_share_pixel_variant(original.data(), original.size() - 1, 0.05f,
                                                                         guard, true, guard_share) !=
                                LinearMaterialResult::Applied &&
                            guard == Words({91, 92}),
                        "truncation rollback");
            ...
            std::cout << (first ? "" : ",") << "{\"name\":\"" << name << "\",\"words\":" << original.size()
                      << ",\"status\":0,\"share_applied\":" << (share_all ? 1 : 0) << ",\"xt\":" << (xt ? 1 : 0)
                      << ",\"light\":" << light << ",\"final_rgb\":" << final_rgb << ",\"seeds\":[";
            for (std::size_t k = 0; k < seeds.size(); ++k) std::cout << (k ? "," : "") << seeds[k];
            std::cout << "],\"slots\":[[" << slots[0][0] << ',' << slots[0][1] << "],[" << slots[1][0] << ','
                      << slots[1][1] << "]]"
                      << ",\"control_slots\":[[" << control_slots[0][0] << ',' << control_slots[0][1] << "],["
                      << control_slots[1][0] << ',' << control_slots[1][1] << "]]"
                      << ",\"instructions\":[[" << instructions[0][0] << ',' << instructions[0][1] << "],["
                      << instructions[1][0] << ',' << instructions[1][1] << "]]"
                      << ",\"max_temp\":[[" << temps[0][0] << ',' << temps[0][1] << "],[" << temps[1][0] << ','
                      << temps[1][1] << "]]}";
            first = false;
        }
        Words authored{0xffff0300u, 0xffffu}, result{91, 92};
        bool authored_share = true;
        require(linear_material_original_sun_share_pixel_variant(authored.data(), authored.size(), 0.05f, result, true,
                                                                 authored_share) ==
                        LinearMaterialResult::UnsupportedShader &&
                    result == Words({91, 92}) && !authored_share,
                "unreviewed valid framing");
        std::cout << "],\"programs\":" << names.size() << ",\"supported\":" << supported << ",\"applied\":" << applied
                  << ",\"max_slots\":" << max_slots << ",\"max_temp\":" << max_temp
                  << ",\"synthetic_refusals\":" << refusals << ",\"checks\":" << checks
