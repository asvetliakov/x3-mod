...
struct Slots { int total = -1, tex = -1, alu = -1, flow = -1; };
Slots count_slots(const DWORD* code) {
    ...
    if (a) {
        ...
        if (paren && (!eol || paren < eol)) {
            int v = 0; char word[32] = {};
            for (const char* q = paren + 1; q && (!eol || q < eol) && std::sscanf(q, "%d %31[a-z]", &v, word) == 2;) {
                if (!std::strcmp(word, "texture")) s.tex = v;
                else if (!std::strcmp(word, "arithmetic")) s.alu = v;
                else if (!std::strcmp(word, "flow")) s.flow = v;
                q = std::strchr(q, ',');
                if (q) ++q;
            }
        }
...
std::string hlsl_chain(bool pixel, unsigned n, unsigned loop) {
    ...
    if (pixel)
        return s + "sampler2D s : register(s0);\nfloat4 main(float2 uv : TEXCOORD0) : COLOR {\n"
                   "    float4 t = tex2D(s, uv);\n    float4 x = t;\n" + body + "    return x;\n}\n";
    return s + "struct O { float4 p : POSITION; float4 c : TEXCOORD1; };\n"
               "O main(float4 p : POSITION) {\n    float4 t = one;\n    float4 x = t;\n" + body +
               "    O o; o.p = p; o.c = x; return o;\n}\n";
}

bool compile_hlsl(const std::string& src, const char* profile, DWORD flags, std::vector<DWORD>& code, HRESULT& hr, std::string& err) {
    Com<ID3DXBuffer> out, errors;
    hr = compile_fn(src.data(), UINT(src.size()), nullptr, nullptr, "main", profile, flags, out.out(), errors.out(), nullptr);
    err.clear();
    if (errors.p) {
...
void draw_and_read(Ctx& c, bool timed, unsigned executed_hint, HRESULT& draw_hr, float& g, double& first_ms, double& per_draw_ms, unsigned& reps) {
    g = NAN; first_ms = per_draw_ms = -1; reps = 0;
    ...
    double t0 = now_ms();
    draw_hr = quad(c);
    if (FAILED(draw_hr) || !sync(c)) { if (SUCCEEDED(draw_hr)) draw_hr = E_FAIL; return; }
    first_ms = now_ms() - t0;
    ...
    if (SUCCEEDED(c.readback->LockRect(&lr, nullptr, D3DLOCK_READONLY))) {
        const float* px = reinterpret_cast<const float*>(static_cast<const char*>(lr.pBits) + lr.Pitch * (kSize / 2)) + 4 * (kSize / 2);
        g = px[1];
...
void report(Ctx& c, const char* stage, const char* kind, unsigned n, unsigned loop, const std::vector<DWORD>* code,
    ...
    std::printf("CASE stage=%s kind=%s n=%u loop=%u executed_expected=%u compile_hr=%08lx compile_ms=%.0f skipval_hr=%08lx",
    ...
    if (!code) {
        ...
        if (!err.empty()) std::printf("ERROR stage=%s kind=%s n=%u loop=%u text=%s\n", stage, kind, n, loop, first_error(err).c_str());
        std::fflush(stdout);
        return;
    }
    const Slots s = count_slots(code->data());
    std::printf(" words=%u slots=%d slots_tex=%d slots_alu=%d slots_flow=%d", unsigned(code->size()), s.total, s.tex, s.alu, s.flow);
    std::fflush(stdout);
    HRESULT create_hr;
    Com<IDirect3DPixelShader9> ps;
    Com<IDirect3DVertexShader9> vs;
    double t0 = now_ms();
    if (pixel) create_hr = c.d->CreatePixelShader(code->data(), ps.out());
    else create_hr = c.d->CreateVertexShader(code->data(), vs.out());
    std::printf(" create_hr=%08lx create_ms=%.1f", (unsigned long)create_hr, now_ms() - t0);
    if (FAILED(create_hr) || !draw) { std::printf("%s\n", draw ? "" : " draw=skipped"); std::fflush(stdout); return; }
    c.d->SetVertexShader(pixel ? c.vs_pass.p : vs.p);
    c.d->SetPixelShader(pixel ? ps.p : c.ps_color.p);
    HRESULT draw_hr; float g; double first_ms, per_ms; unsigned reps;
    ...
    if (std::isfinite(g)) std::snprintf(value, sizeof value, "g=%.9g executed=%.1f", double(g), double(g - 1.0f) / kStep);
    std::printf(" draw_hr=%08lx %s first_draw_ms=%.1f reps=%u ms_per_draw=%.4f coop=%08lx\n",
                (unsigned long)draw_hr, value, first_ms, reps, per_ms, (unsigned long)c.d->TestCooperativeLevel());
...
void hlsl_case(Ctx& c, bool pixel, unsigned n, unsigned loop, double& last_ms) {
    ...
    HRESULT hr = S_OK, skip_hr = S_OK; std::string err, skip_err;
    ...
    if (!ok) {  // does the refusal come from the validator? (the draw still uses only a D3DX-validated program)
        ...
        compile_hlsl(src, profile, D3DXSHADER_OPTIMIZATION_LEVEL3 | D3DXSHADER_SKIPVALIDATION, unvalidated, skip_hr, skip_err);
        if (SUCCEEDED(skip_hr)) {
            const Slots s = count_slots(unvalidated.data());
            std::printf("SKIPVALIDATION stage=%s n=%u loop=%u hr=%08lx slots=%d\n", profile, n, loop, (unsigned long)skip_hr, s.total);
        } else {
            std::printf("SKIPVALIDATION stage=%s n=%u loop=%u hr=%08lx text=%s\n", profile, n, loop, (unsigned long)skip_hr,
                        first_error(skip_err).c_str());
        }
    }
...
// The same chain as assembly text through D3DXAssembleShader (the D3DX assembler and its validator); created, not drawn.
void asm_case(Ctx& c, bool pixel, unsigned n) {
    ...
    std::string err;
    if (errors.p) err.assign(static_cast<const char*>(errors->GetBufferPointer()),
...
int run(int argc, char** argv) {
    if (argc < 2) { std::printf("usage: %s <d3dx9_37 path> [max hlsl N]\n", argv[0]); return 2; }
    ...
    for (const char* name : {"wined3d.dll", "winevulkan.dll", "d3d11.dll", "dxgi.dll", "winemetal.dll"})
        if (HMODULE m = GetModuleHandleA(name)) { GetModuleFileNameA(m, path, MAX_PATH); std::printf("MODULE name=%s path=%s\n", name, token(path, 260).c_str()); }
    { auto p = GetProcAddress(d3dx, "D3DXCompileShader"); std::memcpy(&compile_fn, &p, sizeof p); }
    { auto p = GetProcAddress(d3dx, "D3DXDisassembleShader"); std::memcpy(&disassemble_fn, &p, sizeof p); }
    { auto p = GetProcAddress(d3dx, "D3DXAssembleShader"); std::memcpy(&assemble_fn, &p, sizeof p); }
    ...
    if (!compile_fn || !disassemble_fn || !assemble_fn || !create) { std::printf("STEP entry_points failed\n"); return 1; }
    Com<IDirect3D9> d3d;
    d3d.p = create(D3D_SDK_VERSION);
    if (!d3d.p) { std::printf("STEP direct3dcreate9 failed\n"); return 1; }
    ...
    WNDCLASSA wc{}; wc.lpfnWndProc = window_proc; wc.hInstance = GetModuleHandleA(nullptr); wc.lpszClassName = "x3m_slot_budget";
    RegisterClassA(&wc);
    HWND window = CreateWindowExA(0, wc.lpszClassName, "x3m slot budget", WS_OVERLAPPEDWINDOW, 0, 0, 256, 256,
                                  nullptr, nullptr, wc.hInstance, nullptr);   // never shown
    D3DPRESENT_PARAMETERS pp{};
    pp.BackBufferWidth = 256; pp.BackBufferHeight = 256; pp.BackBufferFormat = D3DFMT_A8R8G8B8; pp.BackBufferCount = 1;
    ...
    std::printf("CAPS hr=%08lx vs_version=%04lx ps_version=%04lx max_ps30_slots=%lu max_vs30_slots=%lu "
                ...
                (unsigned long)caps_hr, (unsigned long)(caps.VertexShaderVersion & 0xFFFF), (unsigned long)(caps.PixelShaderVersion & 0xFFFF),
                (unsigned long)caps.MaxPixelShader30InstructionSlots, (unsigned long)caps.MaxVertexShader30InstructionSlots,
                (unsigned long)caps.MaxPShaderInstructionsExecuted, (unsigned long)caps.MaxVShaderInstructionsExecuted,
                caps.PS20Caps.NumInstructionSlots, caps.PS20Caps.DynamicFlowControlDepth, caps.PS20Caps.NumTemps,
                caps.PS20Caps.StaticFlowControlDepth, (unsigned long)caps.PS20Caps.Caps,
                caps.VS20Caps.DynamicFlowControlDepth, caps.VS20Caps.NumTemps, caps.VS20Caps.StaticFlowControlDepth,
                (unsigned long)caps.VS20Caps.Caps, (unsigned long)caps.MaxVertexShaderConst);
    std::fflush(stdout);

    Ctx c;
    c.d = device.p;
    bool ok = SUCCEEDED(c.d->CreateTexture(kSize, kSize, 1, D3DUSAGE_RENDERTARGET, D3DFMT_A32B32G32R32F, D3DPOOL_DEFAULT, c.rt.out(), nullptr));
    ok = ok && SUCCEEDED(c.rt->GetSurfaceLevel(0, c.rt_surface.out()));
    ok = ok && SUCCEEDED(c.d->CreateOffscreenPlainSurface(kSize, kSize, D3DFMT_A32B32G32R32F, D3DPOOL_SYSTEMMEM, c.readback.out(), nullptr));
    ...
    if (ok) {
        ...
        if (ok) { for (int y = 0; y < 4; ++y) std::memset(static_cast<char*>(lr.pBits) + y * lr.Pitch, 0xFF, 16); c.white->UnlockRect(0); }
    }
    ...
    std::vector<DWORD> code; HRESULT chr; std::string err;
    ok = ok && compile_hlsl("struct O { float4 p : POSITION; float2 uv : TEXCOORD0; };\n"
    ...
    ok = ok && compile_hlsl("float4 main(float4 c : TEXCOORD1) : COLOR { return c; }\n", "ps_3_0",
    ...
    const unsigned hlsl_n[] = {256, 500, 505, 506, 507, 508, 509, 510, 511, 512, 513, 600, 1024, 2048, 4096, 8192,
                               16384, 32760, 32768, 32769, 65536};
    double last_ms = 0;
    for (unsigned n : hlsl_n) {
        if (n > max_hlsl) break;
        if (last_ms > 90000) { std::printf("HLSLSTOP n=%u reason=previous_compile_ms_%.0f\n", n, last_ms); break; }
