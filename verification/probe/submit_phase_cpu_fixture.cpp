...
static void invoke(void* target,Snapshot& out){
    ...
    check(!std::memcmp(fixture_input_x87,out.x87,108),"incoming x87 image (two live stack values, control word) survives every stamp");
}
...
extern "C" { std::uint32_t body_miss=0,body_skip_block=0,body_hit_record=0; }
static void prepare_engine(){
    std::memset(object,0,sizeof object);
    object[0]=std::uint32_t(address(object));object[0x10/4]=std::uint32_t(address(object));
    for(unsigned i=0;i<4;++i){records[i][0]=i<3?std::uint32_t(address(records[i+1])):0;records[i][3]=0x1000+i;}
...
struct Body { void* body=nullptr;void* spans[site::Count]{};unsigned length=0; };
static Body make_body(){
    ...
    const auto span=[&](unsigned k,void (*callee)()=nullptr){
        ...
        const auto& s=site::kSites[k];b.spans[k]=e.here();
        if(s.rel32_offset){e.bytes(s.expected,s.rel32_offset);e.rel32(target);}
        else e.bytes(s.expected,s.length);
    };
    const auto load=[&](unsigned char opcode){e.byte(opcode);e.dword(std::uint32_t(address(object)));}; // mov reg,&object
    e.byte(0x53);e.byte(0x56);e.byte(0x57);e.byte(0x55);              // push ebx; push esi; push edi; push ebp
    e.byte(0x81);e.byte(0xec);e.dword(0x400);                          // sub esp,0x400
    load(0xbb);load(0xbe);load(0xbf);load(0xbd);load(0xb8);            // ebx, esi, edi, ebp, eax = &object
    span(site::MaterialEnter);e.byte(0x8b);e.byte(0xe5);e.byte(0x5d);  // push ebp; mov ebp,esp; and esp,-16 | mov esp,ebp; pop ebp
    span(site::TechniqueBegin);span(site::TechniqueEnd);
    span(site::BlockBegin);e.byte(0x83);e.byte(0xc4);e.byte(0x04);     // push 1; lea | add esp,4
    span(site::InverseWorldBegin,&fixture_nop_ret);span(site::InverseWorldEnd);
    span(site::InverseViewBegin,&fixture_nop_ret);span(site::InverseViewEnd);
    e.byte(0x83);e.byte(0x3d);e.dword(std::uint32_t(address(&body_skip_block)));e.byte(0x00); // cmp dword [skip],0
    e.byte(0x75);e.byte(static_cast<unsigned char>(site::kSites[site::BlockEnd].length));  // jne over block_end: the geometry guard
    span(site::BlockEnd);
    span(site::EndBegin);span(site::EndEnd);
    e.byte(0x56);e.byte(0x57);                                         // push esi; push edi for material_return's pops
    span(site::MaterialReturn);
    load(0xb8);span(site::WorldEnter);e.byte(0x83);e.byte(0xc4);e.byte(0x08); // sub esp,8; cmp | add esp,8
    span(site::WorldReturnA);span(site::WorldReturnB);
    load(0xb8);span(site::SortEnter);
    span(site::SortReturnA,&fixture_pop_ret);span(site::SortReturnB,&fixture_pop_ret);span(site::SortReturnC,&fixture_pop_ret);
    span(site::WalkBegin);
    e.byte(0x83);e.byte(0x3d);e.dword(std::uint32_t(address(&body_miss)));e.byte(0x00); // cmp dword [miss],0
    e.byte(0x74);e.byte(static_cast<unsigned char>(site::kSites[site::WalkMiss].length+3));   // je over the miss path: the hit
    span(site::WalkMiss,&fixture_nop_ret);e.byte(0x83);e.byte(0xc4);e.byte(0x04);             // push 0x70; call | add esp,4
    e.byte(0x8b);e.byte(0x35);e.dword(std::uint32_t(address(&body_hit_record)));              // mov esi,[hit]: the engine's `mov esi,eax`
    span(site::WalkJoin);
    e.byte(0x81);e.byte(0xc4);e.dword(0x400);e.byte(0x5d);e.byte(0x5f);e.byte(0x5e);e.byte(0x5b);e.byte(0xc3);
...
static bool body_specs(const Body& b,patch::SiteSpec* specs){
    ...
    for(unsigned k=0;k<site::Count;++k){
        specs[k]=site::kSites[k];specs[k].address=address(b.spans[k]);
...
static DWORD WINAPI foreign_thread(LPVOID body){reinterpret_cast<void(*)()>(body)();return 0;}
static void stub_shape_checks(){
    void** next=nullptr;void* stub=submit::fixture_emit(7,&next);
    check(stub&&next,"production context stub emitted");if(!stub||!next)return;
    ...
    check(used<=x3m::lean_stub::context_emitted+3&&used<=x3m::lean_stub::context_reserve,"context stub fits its documented size and reservation");
    check(code[0]==0x9c&&code[1]==0x60&&code[2]==0xfc,"context stub opens with pushfd; pushad; cld");
    // Fixed layout: 3 + sub esp (6) + 8 saves (40) + lea/push (8) + push index (5) + call (5) + add esp (3) + 8 loads (40) + add esp (6).
    check(code[49]==0x8d&&code[56]==0x50&&code[57]==0x68&&code[58]==7&&code[62]==0xe8,"context stub passes the pushad frame pointer and the site index");
    check(code[116]==0x61&&code[117]==0x9d&&code[118]==0xff&&code[119]==0x25,"context stub closes with popad; popfd; jmp [next], no x87 or fxsave save anywhere");
}
static void replay_checks(){
    prepare_engine();
    Body b=make_body();check(b.body!=nullptr,"synthetic submit body emitted");if(!b.body)return;
    patch::SiteSpec specs[site::Count];if(!body_specs(b,specs))return;
    unsigned char original[512];std::memcpy(original,b.body,b.length);
    Snapshot baseline{},baseline_miss{},baseline_skip{},hooked{},after{};
    invoke(b.body,baseline);
    body_miss=1;invoke(b.body,baseline_miss);body_miss=0;
    body_skip_block=1;invoke(b.body,baseline_skip);body_skip_block=0;
    ...
    const auto* gate=submit::fixture_gate();const auto* a=submit::fixture_accumulator();
    // Before the first frame boundary no thread is admitted: early, ignored.
    // The hit path runs every site but walk_miss.
    invoke(b.body,hooked);compare(baseline,hooked);
    check(gate->early.load()==site::Count-1&&gate->foreign.load()==0&&a->stamps==0,"stamps before admission are early and ignored");
    ...
    invoke(b.body,hooked);compare(baseline,hooked);
    bool once=true;for(unsigned i=0;i<detail::interval_count;++i)once=once&&a->calls[i]==1&&a->open_clock[i]==0;
    check(once,"hit path closes each of the nine pairs exactly once and leaves none open");
    check(a->stamps==site::Count-1&&a->idle==3&&a->reopened==0&&a->unmatched==0&&a->clock_errors==0&&a->clock_failures==0,"21 stamps, the surplus sort and world returns idle, no error counter");
    check(a->sort_nodes==3&&a->sort_nodes_max==3,"queue length read from the root global, head and sentinel");
    check(a->walk_lookups==1&&a->walk_misses==0&&a->walk_sampled_iterations==0,"an unsampled lookup reads no engine memory");
    ...
    check(a->material_inner==10&&a->block_inner==4,"dispatches nested in material and block are counted for the net figures");
    // Miss path: walk_miss closes the pair, the join is idle.
    body_miss=1;invoke(b.body,hooked);compare(baseline_miss,hooked);body_miss=0;
    check(a->calls[detail::Walk]==2&&a->walk_misses==1&&a->idle==7&&a->stamps==2*site::Count-1,"miss closes the walk at the malloc path and the join stamp is idle");
    // Geometry-guard skip: block_end never runs, end_begin closes the block.
    body_skip_block=1;invoke(b.body,hooked);compare(baseline_skip,hooked);body_skip_block=0;
    check(a->calls[detail::Block]==3&&a->block_skipped==1&&a->open_clock[detail::Block]==0,"a skipped pass-loop guard closes the block at the End dispatch");
    // Lookups 4..16: the sixteenth is the sampled one, a hit on the third record.
    for(unsigned i=0;i<13;++i){invoke(b.body,hooked);compare(baseline,hooked);}
    check(a->walk_lookups==16&&a->walk_sampled_iterations==3,"the sampled hit counts the records up to the one in the saved ESI, through the saved EDI view");
    // Lookup 32 sampled on the miss path: every linked record.
    for(unsigned i=0;i<15;++i)invoke(b.body,hooked);
    body_miss=1;invoke(b.body,hooked);compare(baseline_miss,hooked);body_miss=0;
    check(a->walk_lookups==32&&a->walk_sampled_iterations==6&&a->walk_misses==2,"the sampled miss counts every linked record");
    LARGE_INTEGER qf{};check(QueryPerformanceFrequency(&qf)&&qf.QuadPart>0,"QPC frequency");
    const std::uint64_t hz=std::uint64_t(qf.QuadPart);
    std::uint64_t ticks[detail::interval_count];std::memcpy(ticks,a->ticks,sizeof ticks);
    ...
    for(unsigned i=0;i<detail::interval_count;++i){converted=converted&&s.interval_us[i]==ticks[i]*1000000ull/hz;counted=counted&&s.calls[i]==32;}
    check(s.frame==2&&converted&&counted,"sample carries the frame, 32 closed pairs per interval and the tick-to-microsecond conversion");
    check(s.stamps==stamps&&s.self_us==std::uint64_t(stamps)*detail::dispatch_cost_ns/1000,"self cost is stamps x the fixture-measured dispatch cost");
    const std::uint64_t inner_us=std::uint64_t(material_inner)*detail::dispatch_cost_ns/1000;
    check(s.material_net_us==(s.interval_us[detail::Material]>inner_us?s.interval_us[detail::Material]-inner_us:0),"material net subtracts the nested dispatches");
    check(s.sort_nodes==32*3&&s.sort_nodes_max==3&&s.walk_misses==2&&s.walk_iterations==6*detail::walk_sample_period,"queue length, misses and the scaled walk iterations reach the sample");
    check(a->stamps==0&&a->calls[0]==0&&a->ticks[detail::Material]==0&&a->walk_lookups==32,"take resets the frame accumulation and keeps the sampling phase");
    ...
    invoke(b.body,after);compare(baseline,after);
    // Byte mismatch: one corrupted opcode refuses the whole group before any claim.
    Body c=make_body();check(c.body!=nullptr,"second synthetic body emitted");if(!c.body)return;
    patch::SiteSpec corrupt[site::Count];if(!body_specs(c,corrupt))return;
    unsigned char untouched[512];std::memcpy(untouched,c.body,c.length);
    corrupt[site::Count-1].expected[0]^=1;
    check(!submit::fixture_install(corrupt,&status,&view_root_word),"install refused on a byte mismatch at the last site");
    ...
    corrupt[site::Count-1].expected[0]^=1;
    patch::SiteSpec partial[site::Count];std::memcpy(partial,corrupt,sizeof partial);
    ...
    submit::fixture_uninstall();
    invoke(c.body,after);compare(baseline,after);
}
...
static void benchmark(){
    ...
    prepare_engine();
    Body b=make_body();check(b.body!=nullptr,"benchmark body emitted");if(!b.body)return;
    ...
    submit::frame(2,true);
    detail::Sample s{};
    check(submit::fixture_last_sample(&s)&&s.stamps==loops*trials*(site::Count-1)&&s.calls[detail::Material]==loops*trials,"hooked benchmark loop counted every stamp");
    ...
    std::printf("SUBMIT PHASE BENCH loops=%u trials=%u stamps_per_loop=%u baseline_ns_per_loop=%.1f hooked_ns_per_loop=%.1f dispatch_ns=%.1f implied_busy_frame_us=%.0f documented_dispatch_ns=%llu arena_used=%u arena_capacity=%u\n",
        loops,trials,unsigned(site::Count-1),baseline_ns,hooked_ns,dispatch_ns,dispatch_ns*10800/1000,static_cast<unsigned long long>(detail::dispatch_cost_ns),patch::arena_used(),patch::arena_capacity());
    const double documented=number(detail::dispatch_cost_ns);
    check(dispatch_ns>0&&documented>=dispatch_ns*0.5&&documented<=dispatch_ns*2.0,"documented submit dispatch cost within 2x of the measured cost");
}
static void late_window_checks(){
    prepare_engine();
    Body b=make_body();check(b.body!=nullptr,"late-window body emitted");if(!b.body)return;
    patch::SiteSpec specs[site::Count];if(!body_specs(b,specs))return;
    unsigned char original[512];std::memcpy(original,b.body,b.length);
...
int main(){
    ...
    std::printf("SUBMIT PHASE CPU sites=%u intervals=%u arena_used=%u arena_capacity=%u checks=%u failures=%u\n",
        unsigned(site::Count),detail::interval_count,patch::arena_used(),patch::arena_capacity(),checks,failures);
    return failures?1:0;
}
