...
namespace x3m::chase_camera { bool installed(){return requested;} bool wanted(){return requested;} }
namespace fire=x3m::chase_fire;
static unsigned checks=0,failures=0;
static void check(bool ok,const char* name){++checks;if(!ok)++failures;std::printf("CHECK %s %s\n",name,ok?"PASS":"FAIL");}
extern "C" {
...
static std::uint32_t address(void* p){return std::uint32_t(reinterpret_cast<std::uintptr_t>(p));}
static void put(std::uintptr_t p,unsigned off,std::uint32_t v){std::memcpy(reinterpret_cast<void*>(p+off),&v,4);}
...
static void branch_code(std::uintptr_t at,unsigned value) {
    ...
    std::memcpy(reinterpret_cast<void*>(at),prefix,2);put(at,2,address(&branch));put(at,6,value);
...
static bool map_fixture() {
    ...
    if(module!=reinterpret_cast<void*>(0x400000) || address(fixture_map)!=0x401000 ||
       reinterpret_cast<std::uintptr_t>(&fixture_call)<0x620000)return false;
    MEMORY_BASIC_INFORMATION info{};
    if(VirtualQuery(reinterpret_cast<void*>(fire::spec.address),&info,sizeof info)!=sizeof info ||
       info.AllocationBase!=module || info.State!=MEM_COMMIT || info.Type!=MEM_IMAGE)return false;
    DWORD old=0;
    ...
    branch_code(0x445a47,1);branch_code(0x445cab,2);
...
static __attribute__((noinline)) void invoke(std::uint32_t* args,Snapshot& out,bool check_error=true) {
    fixture_output=&out;branch=0;SetLastError(0x24681357);
...
static void state_equal(const Snapshot& baseline,const Snapshot& actual,bool override) {
    ...
    if(actual.regs[3]!=baseline.regs[3] && !stack_difference_reported) {
        ...
        std::printf("CHASE FIRE STACK differing_index=3 baseline=0x%08lx actual=0x%08lx expected_from_entry=0x%08lx entry=0x%08lx exit=0x%08lx\n",
            static_cast<unsigned long>(baseline.regs[3]),static_cast<unsigned long>(actual.regs[3]),
            static_cast<unsigned long>(fixture_entry_esp-296),static_cast<unsigned long>(fixture_entry_esp),static_cast<unsigned long>(fixture_exit_esp));
    }
    ...
    if(!x87_difference_reported)for(unsigned i=0;i<108;++i)if(baseline.x87[i]!=actual.x87[i]) {
        x87_difference_reported=true;
        std::uint16_t baseline_tag=0,input_tag=0,post_tag=0;
        std::memcpy(&baseline_tag,baseline.x87+8,2);std::memcpy(&input_tag,fixture_input_x87+8,2);std::memcpy(&post_tag,actual.x87+8,2);
        std::printf("CHASE FIRE X87 cross_call_difference_byte=%u baseline=0x%02x current_input=0x%02x current_post=0x%02x baseline_tag=0x%04x input_tag=0x%04x post_tag=0x%04x\n",
            i,unsigned(baseline.x87[i]),unsigned(fixture_input_x87[i]),unsigned(actual.x87[i]),unsigned(baseline_tag),unsigned(input_tag),unsigned(post_tag));break;
    }
...
static double measure_span(std::uint32_t* args,unsigned iterations,Snapshot& out) {
    fixture_output=&out;
    LARGE_INTEGER f{},begin{},end{};QueryPerformanceFrequency(&f);QueryPerformanceCounter(&begin);
    for(unsigned i=0;i<iterations;++i)fixture_call(0x445a3a,address(args),address(args),100000,1,500);
    QueryPerformanceCounter(&end);
    check(f.QuadPart>0 && begin.QuadPart>0 && end.QuadPart>begin.QuadPart,"whole-span aggregate timing has valid clock");
    check(out.regs[3]==fixture_entry_esp-296 && fixture_exit_esp==fixture_entry_esp,"aggregate final invocation exact stack balance");
...
int main(int argc,char** argv) {
    ...
    check(map_fixture(),"PE-owned actual production branch map");if(failures)return 2;
    ...
    cockpit[0xc/4]=cockpit[0x10/4]=address(ship);cockpit[0x58/4]=address(camera);cockpit[0x150/4]=258;
    ship[8/4]=0x1234;
    camera[0x288/4]=0;camera[0x28c/4]=65535;camera[0x290/4]=0;camera[0x294/4]=65535;
    registry[0]=address(table);registry[4]=99;table[0]=address(bucket);table[1]=1;bucket[0]=address(entry);entry[1]=99;entry[2]=address(cockpit);
    defaults[0]=address(screen);screen[1]=(720u<<16)|1280u;
    put(0x606f38,0,address(defaults));put(0x608504,0,address(registry));
    put(0x607ce8,0,0);put(0x607cec,0,640);put(0x607cf0,0,360);put(0x607c64,0,0x19283746);
    ...
    check((trace_pose.valid&1024) && trace_pose.screen[0]==1280 && trace_pose.screen[1]==720,"diagnostic screen reader follows extra pointer");
    screen[1]=0xffff0001;
    x3m::chase_aim_trace::pose(address(cockpit),address(ship),address(camera),trace_pose);
    check(!(trace_pose.valid&1024),"diagnostic screen reader refuses negative dimensions");screen[1]=(720u<<16)|1280u;
    args[2]=address(ship);args[4]=0;args[5]=0x2a;
    Snapshot zero{},nonzero{},out{};
    invoke(args,zero);check(branch==2,"unpatched inactive CMP takes original JZ");
    put(0x607ce8,0,1);invoke(args,nonzero);check(branch==1,"unpatched active CMP falls through");put(0x607ce8,0,0);
    ...
    if(!expected) {
        ...
        if(!std::strcmp(mode,"off") || !std::strcmp(mode,"late")) {
            check(!std::memcmp(reinterpret_cast<void*>(fire::spec.address),fire::spec.expected,fire::spec.length),"disabled or late native bytes untouched");
            invoke(args,out);check(branch==2,"disabled or late original branch still executes");state_equal(zero,out,false);
        }
    ...
    auto publish=[&]{fire::camera_context(address(cockpit),address(ship),address(camera),258,0,0,true,fire::now());};
    auto refused=[&](const char* name){invoke(args,out);check(branch==2,name);state_equal(zero,out,false);};
    ...
    std::printf("CHASE FIRE SPAN PERF iterations=%u native_us=%.3f hooked_no_witness_us=%.3f eligible_override_us=%.3f early_delta_us=%.3f eligible_delta_us=%.3f scope=full_span_plus_fixture_transport fps_claim=0\n",
        span_iterations,native_us,early_us,eligible_us,early_us-native_us,eligible_us-native_us);
    fire::camera_context(0,0,0,0,0,0,false,0);fire::report(299);
    check(!fire::counts.result[0],"aggregate benchmark counters drained before controls");
    refused("no applied camera witness refuses");
    publish();invoke(args,out);check(branch==1,"applied external cursor fire overrides JZ");state_equal(zero,out,true);
    check(fire::counts.result[0]==1,"effective override counter separate from native input");
    check(*reinterpret_cast<unsigned*>(0x607ce8)==0 && *reinterpret_cast<unsigned*>(0x607cec)==640 && *reinterpret_cast<unsigned*>(0x607cf0)==360 && *reinterpret_cast<unsigned*>(0x607c64)==0x19283746,"native cursor active xy and steering unchanged");
    put(0x607ce8,0,1);invoke(args,out);check(branch==1,"native active path is original fallthrough");state_equal(nonzero,out,false);put(0x607ce8,0,0);
    args[5]=2;refused("boresight flags refuse");args[5]=0x28;refused("nonfire flags refuse");args[5]=0x2a;
    args[4]=1;refused("turret group refuses");args[4]=0;
    args[2]=address(ship)+4;refused("other ship refuses");args[2]=address(ship);
    cockpit[0x150/4]=1;refused("live internal switch refuses without new update");cockpit[0x150/4]=257;refused("other external view refuses");cockpit[0x150/4]=258;
    cockpit[0x1c0/4]=1;refused("scripted connect refuses");cockpit[0x1c0/4]=0;
    cockpit[0x1a0/4]=4;refused("verbatim camera mode refuses");cockpit[0x1a0/4]=0;
    cockpit[0x58/4]+=4;refused("live camera mismatch refuses");cockpit[0x58/4]-=4;
    cockpit[0xc/4]+=4;refused("reference identity mismatch refuses");cockpit[0xc/4]-=4;
    entry[2]+=4;refused("resolved cockpit mismatch refuses");entry[2]-=4;
    ship[2]++;refused("same address new ship id refuses");ship[2]--;
    fire::context.qpc=fire::now()-fire::frequency*3;refused("stale applied pose refuses");publish();
    fire::context.qpc=fire::now()+fire::frequency;refused("future timestamp refuses");publish();
    fire::context.thread++;refused("different thread witness refuses");publish();
    put(0x607cec,0,1280);refused("cursor outside screen refuses");put(0x607cec,0,640);
    camera[0x290/4]=40000;refused("cursor outside normalized viewport refuses");camera[0x290/4]=0;
    screen[1]=0;refused("invalid screen dimensions refuse");screen[1]=(720u<<16)|1280u;
    defaults[0]=0;refused("unreadable screen pointer refuses");defaults[0]=address(screen);
    fire::camera_context(address(cockpit),address(ship),address(camera),1,0,0,false,fire::now());refused("internal update invalidates prior chase witness");publish();
    fire::camera_context(address(cockpit),address(ship),address(camera),258,0,0,false,fire::now());refused("camera write refusal invalidates witness");publish();
    fire::invalidate_camera(address(cockpit)+4);invoke(args,out);check(branch==1,"unrelated monitor retains player witness");
    fire::invalidate_camera(address(cockpit));refused("same unreadable or inactive cockpit invalidates witness");
    check(!fire::read(UINT32_MAX-3,8,&out,4) && !fire::read(3,0,&out,4),"unaligned and overflowing reads refuse");
    ...
    check(fire::counts.timed==100 && fire::counts.ticks>0,"bounded aggregate timing exercised");state_equal(zero,out,true);
    std::printf("CHASE FIRE PERF calls=%llu mean_us=%.3f max_us=%.3f scope=eligible_handler_after_first_qpc\n",fire::counts.timed,
        double(fire::counts.ticks)*1000000.0/double(fire::frequency)/double(fire::counts.timed),double(fire::counts.max_ticks)*1000000.0/double(fire::frequency));
    fire::report(300);check(!fire::counts.result[0] && !fire::counts.timed,"report drains bounded counters");
    fire::shutdown();check(!fire::site.patched_in && !std::memcmp(reinterpret_cast<void*>(fire::spec.address),fire::spec.expected,fire::spec.length),"quiescent restore puts original branch bytes back");
    invoke(args,out);check(branch==2,"restored branch returns to native policy");state_equal(zero,out,false);
    std::printf("CHASE FIRE RESULT mode=%s checks=%u failures=%u\n",mode,checks,failures);return failures?1:0;
}
