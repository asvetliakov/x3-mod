...
def main():
    ...
    try:
        ...
        match = re.search(r'^RESULT PASS checks=(\d+) reads=(\d+) state_checks=(\d+)$', text, re.M)
        checks = sum(line.startswith('CHECK ') and line.endswith(' PASS') for line in text.splitlines())
        if not match or tuple(map(int, match.groups())) != (167, 48, 48) or checks != 167:
            raise RuntimeError('Unexpected or incomplete fixture report')
        meta.update(checks=checks, draw_reads=48, caller_state_snapshots=48, getter_fault_cases=7)
        ...
        meta['limits'] = [
            ...
            'Local input evidence never proves engine lifetimes, finite vertex payload, final color ownership, or stability through later replay.',
