...
double qpc_hz = 1;
double now_ms() { LARGE_INTEGER t{}; QueryPerformanceCounter(&t); return 1000.0 * double(t.QuadPart) / qpc_hz; }

std::string token(std::string s) { for (char& c : s) if (c == ' ') c = '_'; return s.empty() ? "-" : s; }
...
struct FfVertex { float x, y, z, rhw, u0, v0, u1, v1; };
...
struct Stats { double r = 0, g = 0, b = 0; double coverage = 0; HRESULT hr = E_FAIL; };
...
Stats read_rt(Context& c, IDirect3DSurface9* source = nullptr) {
    ...
    if (FAILED(s.hr)) return s;
    double sr = 0, sg = 0, sb = 0; unsigned covered = 0;
    for (int y = 0; y < kSize; ++y) {
        const std::uint32_t* row = reinterpret_cast<const std::uint32_t*>(static_cast<const char*>(lr.pBits) + y * lr.Pitch);
    ...
    s.r = sr / n; s.g = sg / n; s.b = sb / n; s.coverage = covered / n;
    return s;
}
...
void report(const char* name, HRESULT draw_hr, const Stats& s, double er, double eg, double eb, double ecov) {
    std::printf("DRAW name=%s draw_hr=%08lx read_hr=%08lx mean_r=%.1f mean_g=%.1f mean_b=%.1f coverage=%.3f "
                "expect_r=%.0f expect_g=%.0f expect_b=%.0f expect_coverage=%.2f\n", name, (unsigned long)draw_hr,
                (unsigned long)s.hr, s.r, s.g, s.b, s.coverage, er, eg, eb, ecov);
...
HRESULT quad(Context& c) {
    const Vertex v[4] = {{-1, 1, 0.5f, 1, 0, 0}, {1, 1, 0.5f, 1, 1, 0}, {-1, -1, 0.5f, 1, 0, 1}, {1, -1, 0.5f, 1, 1, 1}};
...
bool wait_gpu(IDirect3DDevice9* d, double* ms) {
    ...
    if (FAILED(d->CreateQuery(D3DQUERYTYPE_EVENT, q.out()))) { *ms = -1; return false; }
    q->Issue(D3DISSUE_END);
    for (;;) {
        BOOL done = FALSE;
        const HRESULT hr = q->GetData(&done, sizeof done, D3DGETDATA_FLUSH);
        if (hr == S_OK) { *ms = now_ms() - start; return true; }
        if (FAILED(hr) || now_ms() - start > 5000) { *ms = now_ms() - start; return false; }
        Sleep(0);
    }
}

// --- the game-program sweep: leading def/dcl parse for declared inputs and sampler types ---
struct Declared { std::vector<std::pair<BYTE, BYTE>> inputs; int sampler_type[16] = {}; bool any_sampler = false; };
...
Declared parse_declarations(const std::vector<DWORD>& code, bool vertex) {
    ...
    while (i < code.size()) {
        ...
        if (op == 0x51 || op == 0x30) { i += 6; continue; }   // def, defi
        ...
        if (!vertex && type == 10 && number < 16) { d.sampler_type[number] = int((usage >> 27) & 0xF); d.any_sampler = true; }
        i += 3;
    }
    return d;
}
...
void sweep(Context& c, const char* list_path) {
    ...
    if (!list) { std::printf("SWEEPLIST status=missing path=%s\n", list_path); return; }
    IDirect3DDevice9* d = c.device;
    ...
    while (std::fgets(line, sizeof line, list)) {
        ...
        double wait_ms = -1; bool waited = false;
        ...
        if (!code.empty()) {
            ...
            if (vertex) {
                ...
                if (SUCCEEDED(create_hr)) {
                    ...
                    if (elements.empty()) { elements.push_back({0, 0, D3DDECLTYPE_FLOAT4, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_POSITION, 0}); offset = 16; }
                ...
                if (SUCCEEDED(create_hr)) {
                    ...
                    for (DWORD s = 0; s < 16; ++s) {
                        ...
                        IDirect3DBaseTexture9* t = type == 2 ? static_cast<IDirect3DBaseTexture9*>(c.tex_a.p)
                            ...
                            : type == 4 ? static_cast<IDirect3DBaseTexture9*>(c.volume.p) : nullptr;
                        d->SetTexture(s, t);
                    }
        ...
        if (SUCCEEDED(create_hr)) { ++created; ++kinds[k][0]; } else {
            ++create_failed; ++kinds[k][1];
            if (first_failure == "-") first_failure = name;
        }
        if (SUCCEEDED(create_hr)) { if (SUCCEEDED(draw_hr)) ++drawn; else ++draw_failed; if (!waited) ++wait_failed; }
        std::printf("SWEEP index=%u kind=%s name=%s version=%08lx words=%u create_hr=%08lx draw_hr=%08lx wait_ms=%.2f\n", index,
                    kind, name, (unsigned long)version, unsigned(code.size()), (unsigned long)create_hr, (unsigned long)draw_hr, wait_ms);
        ++index;
        if (d->TestCooperativeLevel() != D3D_OK) { std::printf("SWEEPSTOP reason=device_lost index=%u\n", index); break; }
    }
    std::fclose(list);
    std::printf("SWEEPSUMMARY programs=%u created=%u create_failed=%u vs_created=%u vs_failed=%u ps_created=%u ps_failed=%u "
                "drawn=%u draw_failed=%u wait_failed=%u first_create_failure=%s\n", index, created, create_failed,
                kinds[0][0], kinds[0][1], kinds[1][0], kinds[1][1], drawn, draw_failed, wait_failed, first_failure.c_str());
}

LRESULT CALLBACK window_proc(HWND w, UINT m, WPARAM wp, LPARAM lp) { return DefWindowProcA(w, m, wp, lp); }

int run(int argc, char** argv) {
    if (argc < 4) { std::printf("usage: %s <d3d9 path|builtin> <d3dx9_37 path> <shader list|->\n", argv[0]); return 2; }
    LARGE_INTEGER f{}; QueryPerformanceFrequency(&f); qpc_hz = double(f.QuadPart);
    // Which launcher-passed variables reached the process (dyld prunes DYLD_* it will not honour).
    for (const char* name : {"X3M_ENV_PROBE", "DYLD_LIBRARY_PATH", "DYLD_FALLBACK_LIBRARY_PATH", "DXVK_LOG_LEVEL", "MVK_CONFIG_LOG_LEVEL"}) {
        char value[512] = {};
        const DWORD n = GetEnvironmentVariableA(name, value, sizeof value);
        std::printf("ENV name=%s present=%d value=%s\n", name, n > 0 && n < sizeof value, n > 0 && n < sizeof value ? token(value).c_str() : "-");
    }
    ...
    if (d3dx) {
        ...
        auto a = GetProcAddress(d3dx, "D3DXAssembleShader"); std::memcpy(&assemble_fn, &a, sizeof a);
        auto e = GetProcAddress(d3dx, "D3DXCreateEffect"); std::memcpy(&create_effect_fn, &e, sizeof e);
    }
    Create9 create = nullptr;
    { auto p = GetProcAddress(d3d9, "Direct3DCreate9"); std::memcpy(&create, &p, sizeof p); }
    Com<IDirect3D9> d3d;
    ...
    std::printf("ADAPTER hr=%08lx driver=%s description=%s device_name=%s vendor=%04lx device=%04lx driver_version=%lu.%lu.%lu.%lu\n",
    ...
    std::printf("CAPS hr=%08lx vs=%04lx ps=%04lx max_textures=%lu rts=%lu vs_consts=%lu max_tex=%lux%lu\n", (unsigned long)hr,
                (unsigned long)(caps.VertexShaderVersion & 0xFFFF), (unsigned long)(caps.PixelShaderVersion & 0xFFFF),
                (unsigned long)caps.MaxSimultaneousTextures, (unsigned long)caps.NumSimultaneousRTs,
                (unsigned long)caps.MaxVertexShaderConst, (unsigned long)caps.MaxTextureWidth, (unsigned long)caps.MaxTextureHeight);
    struct { const char* name; DWORD usage; D3DRESOURCETYPE type; D3DFORMAT format; } formats[] = {
    ...
    for (const auto& fmt : formats)
        std::printf("FORMAT name=%s hr=%08lx\n", fmt.name, (unsigned long)d3d->CheckDeviceFormat(
            D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, D3DFMT_X8R8G8B8, fmt.usage, fmt.type, fmt.format));

    WNDCLASSA wc{}; wc.lpfnWndProc = window_proc; wc.hInstance = GetModuleHandleA(nullptr); wc.lpszClassName = "x3m_d3d9_smoke";
    RegisterClassA(&wc);
    ...
    pp.BackBufferWidth = kSize; pp.BackBufferHeight = kSize; pp.BackBufferFormat = D3DFMT_A8R8G8B8; pp.BackBufferCount = 1;
    ...
    bool ok = SUCCEEDED(d->CreateTexture(kSize, kSize, 1, D3DUSAGE_RENDERTARGET, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, c.rt.out(), nullptr));
    ok = ok && SUCCEEDED(c.rt->GetSurfaceLevel(0, c.rt_surface.out()));
    ok = ok && SUCCEEDED(d->CreateOffscreenPlainSurface(kSize, kSize, D3DFMT_A8R8G8B8, D3DPOOL_SYSTEMMEM, c.readback.out(), nullptr));
    ...
    const bool managed = SUCCEEDED(d->CreateTexture(64, 64, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, c.tex_a.out(), nullptr)) &&
                         SUCCEEDED(d->CreateTexture(64, 64, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, c.tex_b.out(), nullptr)) &&
    ...
    for (int face = 0; cube && face < 6; ++face) {
        ...
        for (int y = 0; cube && y < 16; ++y)
            for (int x = 0; x < 16; ++x) reinterpret_cast<std::uint32_t*>(static_cast<char*>(lr.pBits) + y * lr.Pitch)[x] = 0xFF2828C8u;
    ...
    if (SUCCEEDED(d->CreateVolumeTexture(4, 4, 4, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, c.volume.out(), nullptr))) {
        ...
        if (SUCCEEDED(c.volume->LockBox(0, &box, nullptr, 0))) {
            for (int z = 0; z < 4; ++z) for (int y = 0; y < 4; ++y) for (int x = 0; x < 4; ++x)
                reinterpret_cast<std::uint32_t*>(static_cast<char*>(box.pBits) + z * box.SlicePitch + y * box.RowPitch)[x] = 0xFF808080u;
    ...
    struct VsSrc { const char* src; Com<IDirect3DVertexShader9>* out; const char* name; };
    struct PsSrc { const char* src; Com<IDirect3DPixelShader9>* out; const char* name; };
    for (const VsSrc& v : {VsSrc{kVs30, &c.vs30, "vs30"}, VsSrc{kVs11, &c.vs11, "vs11"},
    ...
    for (const PsSrc& p : {PsSrc{kPs30Tex, &c.ps30_tex, "ps30_tex"}, PsSrc{kPs30TexCube, &c.ps30_tex_cube, "ps30_tex_cube"},
    ...
    auto draw_case = [&](const char* name, IDirect3DVertexShader9* vs, IDirect3DPixelShader9* ps, auto setup,
                         double er, double eg, double eb, double ecov) {
        reset_state(c);
        setup();
    ...
    draw_case("alpha_test", c.vs30.p, c.ps30_tex.p, [&] {
    ...
    draw_case("two_samplers_ps11", c.vs11.p, c.ps11_two.p, [&] { d->SetTexture(0, c.tex_a.p); d->SetTexture(1, c.tex_b.p); }, 240, 80, 240, 1.0);
    draw_case("two_samplers_ps30_2d_cube", c.vs30.p, c.ps30_tex_cube.p, [&] { d->SetTexture(0, c.tex_a.p); d->SetTexture(1, c.cube_b.p); },
              240, 80, 240, 1.0);
    {   // Two-stage fixed function (pretransformed, two texture coordinate sets).
        ...
        d->SetTexture(0, c.tex_a.p); d->SetTexture(1, c.tex_b.p);
        ...
        const FfVertex v[4] = {{-0.5f, -0.5f, 0.5f, 1, 0, 0, 0, 0}, {s, -0.5f, 0.5f, 1, 1, 0, 1, 0},
                               {-0.5f, s, 0.5f, 1, 0, 1, 0, 1}, {s, s, 0.5f, 1, 1, 1, 1, 1}};
        d->BeginScene();
    ...
    {   // StretchRect: RT -> smaller RT (linear), backbuffer -> RT, depth -> depth.
        ...
        HRESULT h = d->CreateRenderTarget(kSize, kSize, D3DFMT_A8R8G8B8, D3DMULTISAMPLE_NONE, 0, FALSE, half.out(), nullptr);
        if (SUCCEEDED(h)) {
        ...
        h = d->CreateDepthStencilSurface(kSize, kSize, D3DFMT_D24S8, D3DMULTISAMPLE_NONE, 0, FALSE, depth_copy.out(), nullptr);
    ...
    {   // Event and occlusion queries.
        ...
        if (SUCCEEDED(h)) {
            ...
            d->SetVertexShader(c.vs30.p); d->SetPixelShader(c.ps30_tex.p);
            d->BeginScene();
            ...
            d->EndScene();
            const double start = now_ms();
            do { h = occlusion->GetData(&pixels, sizeof pixels, D3DGETDATA_FLUSH); } while (h == S_FALSE && now_ms() - start < 5000);
        }
        std::printf("QUERY type=occlusion hr=%08lx pixels=%lu expect=%d\n", (unsigned long)h, (unsigned long)pixels, kSize * kSize);
    ...
    for (const char* depth_format : {"D24X8", "INTZ"}) {
        ...
        const D3DFORMAT format = !std::strcmp(depth_format, "INTZ") ? D3DFORMAT(MAKEFOURCC('I', 'N', 'T', 'Z')) : D3DFMT_D24X8;
        Com<IDirect3DTexture9> snapshot;
        HRESULT h = d->CreateTexture(kSize, kSize, 1, D3DUSAGE_DEPTHSTENCIL, format, D3DPOOL_DEFAULT, snapshot.out(), nullptr);
        HRESULT resz = E_FAIL;
        if (SUCCEEDED(h)) {
            reset_state(c);
        ...
        for (float reference : {0.25f, 0.75f}) {
            ...
            char name[64]; std::snprintf(name, sizeof name, "resz_%s_ref%.2f", depth_format, reference);
            Stats s;
            HRESULT dh = h;
            if (SUCCEEDED(h)) {
                reset_state(c);
                ...
                d->SetVertexShader(c.vs30.p); d->SetPixelShader(c.ps30_depth.p);
                d->BeginScene(); dh = quad(c); d->EndScene();
                s = read_rt(c);
            }
            std::printf("RESZ format=%s create_hr=%08lx resz_hr=%08lx reference=%.2f draw_hr=%08lx read_hr=%08lx mean_r=%.1f\n", depth_format,
                        (unsigned long)h, (unsigned long)resz, reference, (unsigned long)dh, (unsigned long)s.hr, s.r);
        }
    }
    {   // D3DXCreateEffect with the given d3dx9_37 (HLSL compile, Begin/BeginPass, draw).
        Com<ID3DXEffect> effect; Com<ID3DXBuffer> errors;
        HRESULT h = create_effect_fn ? create_effect_fn(d, kEffect, UINT(std::strlen(kEffect)), nullptr, nullptr, 0, nullptr,
                                                        effect.out(), errors.out()) : E_NOINTERFACE;
        std::printf("EFFECT create_hr=%08lx error=%s\n", (unsigned long)h,
                    errors.p ? token(static_cast<const char*>(errors->GetBufferPointer())).substr(0, 200).c_str() : "-");
        Stats s; HRESULT dh = h;
        if (SUCCEEDED(h)) {
            reset_state(c);
            ...
            if (SUCCEEDED(dh)) {
                ...
                if (SUCCEEDED(dh)) { dh = quad(c); effect->EndPass(); }
                effect->End();
                d->EndScene();
            }
            s = read_rt(c);
