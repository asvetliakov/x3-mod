...
int main(){std::setvbuf(stdout,nullptr,_IONBF,0);int exit=1;HWND window=nullptr;
    try{
        ...
        SetEnvironmentVariableW(L"X3M_TELEMETRY",L"1");require(fixture_initialize(GetModuleHandleW(nullptr),fixture_fingerprint(GetModuleHandleW(nullptr))),"install telemetry");
        ...
        require(!fixture_initialize(GetModuleHandleW(nullptr),fixture_fingerprint(GetModuleHandleW(nullptr))),"one shot initialization rejects foreign chain reinstall");require(table[20]==original[0]&&table[27]==original[2]&&table[22]==foreign_pointer,"shutdown restores owned slots preserves foreign hook");
