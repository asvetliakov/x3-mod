...
#define WINAPI
using HRESULT=int;using ULONG=unsigned;using UINT=unsigned;using INT=int;using D3DPRIMITIVETYPE=unsigned;
#define FAILED(x) ((x)<0)
...
static unsigned native_refs=1,pins_acquired=0,pins_released=0;
static bool draw_lock=false,timer_alive=false,drop_application=false,application_dropped=false,owner_destroyed=false;
ULONG release_device(IDirect3DDevice9*);
...
struct PlainHookGuard {explicit PlainHookGuard(frame_timing::Bucket){draw_lock=true;}~PlainHookGuard(){draw_lock=false;}};
struct CallTimer {int backend_ticks=0;template<class T> CallTimer(T&,bool){timer_alive=true;}~CallTimer(){timer_alive=false;}void begin(){}void end(){}};
...
struct Capture {int original_calls=0,effective_calls=0,result_calls=0;bool selected=true,candidate=true;
    bool can_select(const Arguments&)const{return candidate;}
    int original(IDirect3DDevice9*,const Arguments&,std::uint64_t){if(query_depth()!=1||native_refs!=2)std::abort();if(drop_application){release_device(&device);application_dropped=true;}++original_calls;events.push_back(1);fp=88;error=99;return selected?0:-1;}
    template<class Route>void effective(int,IDirect3DDevice9* d,int,GetTarget get,const Route& r){
        if(query_depth()!=1||native_refs!=(application_dropped?1u:2u))std::abort();if(!r.submit)return;++effective_calls;events.push_back(3);void* out=nullptr;get(d,0,&out);fp=188;error=199;
    }
...
struct Motion {bool draw_submission_blocked(){return block;}bool composition_requested(){return false;}
    ...
    Route before_draw(DrawCall){if(query_depth())std::abort();events.push_back(2);return {allow_submit,-77};}
...
static HRESULT original_draw(IDirect3DDevice9*,unsigned t,int b,unsigned m,unsigned n,unsigned s,unsigned c){
    if(query_depth())std::abort();++native_calls;events.push_back(5);args_seen[0]=t;args_seen[1]=unsigned(b);args_seen[2]=m;args_seen[3]=n;args_seen[4]=s;args_seen[5]=c;
    if(fp!=incoming_fp||error!=incoming_error)std::abort();fp=41;error=43;return native_result;
}
static ULONG native_addref(IDirect3DDevice9*){++pins_acquired;fp=788;error=799;return ++native_refs;}
struct Device {unsigned lattice_query_depth=0;int stats=0,caps=0;Motion motion_output;Depth scene_depth;bool capture=false;
    ...
    template<class F> F get(unsigned slot){return reinterpret_cast<F>(slot==1?reinterpret_cast<void*>(&native_addref):slot==38?reinterpret_cast<void*>(&saved_getter):reinterpret_cast<void*>(&original_draw));}
} ctx;
struct Devices {std::shared_ptr<Device> value{&ctx,[](Device*){owner_destroyed=true;}};std::shared_ptr<Device> at(IDirect3DDevice9*){return value;}} devices;
unsigned query_depth(){return ctx.lattice_query_depth;}
ULONG release_device(IDirect3DDevice9*){
    if(query_depth()) {if(!draw_lock||!timer_alive||native_refs!=2)std::abort();return --native_refs;}
    if(draw_lock||timer_alive||native_refs!=(application_dropped?1u:2u))std::abort();
    ++pins_released;fp=888;error=899;
    const auto result=--native_refs;
    if(!result){devices.value.reset();if(owner_destroyed)std::abort();}
    return result;
}
...
#define CHECK(x) do{++checks;if(!(x)){++failures;std::printf("FAIL line=%u\n",unsigned(__LINE__));}}while(0)
    for(unsigned mode=0;mode<4;++mode){
        fp=incoming_fp;error=incoming_error;native_calls=scoped_calls=saved_getter_calls=0;pins_acquired=pins_released=0;events.clear();allow_submit=mode!=3;block=false;
        ...
        CHECK(native_refs==1&&pins_acquired==(mode?1u:0u)&&pins_released==pins_acquired&&query_depth()==0);
        ...
        if(mode==1){CHECK(scoped_calls==3);CHECK(saved_getter_calls==1);CHECK(events==std::vector<int>({1,2,3,4,5,6,7,8}));}
        if(mode==2){CHECK(scoped_calls==2);CHECK(saved_getter_calls==0);CHECK(events==std::vector<int>({1,2,4,5,6,8}));}
        if(mode==3){CHECK(saved_getter_calls==0);CHECK(events==std::vector<int>({1,2,4,6,7,8}));}
    }
    for(unsigned mode=0;mode<4;++mode){
        fp=incoming_fp;error=incoming_error;native_refs=1;native_calls=scoped_calls=0;
        pins_acquired=pins_released=0;events.clear();allow_submit=true;block=mode==1;
        native_result=mode==2?-123:37;drop_application=mode==3;application_dropped=false;owner_destroyed=false;
        ctx.lattice_state=std::make_shared<lattice_state::Capture>();ctx.lattice_state->candidate=mode!=0;
        const auto hr=draw_indexed(&device,4,-2,5,6,7,8);
        CHECK(hr==(block?-77:native_result));CHECK(native_calls==(block?0:1));
        CHECK(fp==(block?incoming_fp:41)&&error==(block?incoming_error:43));
        CHECK(pins_acquired==(mode>=2?1u:0u)&&pins_released==pins_acquired&&query_depth()==0);
        CHECK(native_refs==(drop_application?0u:1u));CHECK(owner_destroyed==drop_application);
        if(mode==0)CHECK(scoped_calls==0);
        if(mode==1)CHECK(events==std::vector<int>({2}));
    }
