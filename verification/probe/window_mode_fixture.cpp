...
static Rect rect_of(const RECT& r) { Rect c; c.left = r.left; c.top = r.top; c.right = r.right; c.bottom = r.bottom; return c; }
static Rect window_rect(HWND h) { RECT r{}; GetWindowRect(h, &r); return rect_of(r); }
...
static MONITORINFO monitor_info(HWND h) { MONITORINFO mi{}; mi.cbSize = sizeof mi; GetMonitorInfoA(MonitorFromWindow(h, MONITOR_DEFAULTTONEAREST), &mi); return mi; }
// The game's creation and placement (0x4dae0d, 0x4daf1b..0x4db040): 640x480 at 0,0, then the work-area origin with the monitor's size.
static HWND create_game_window(DWORD style, bool at_monitor = false) {
    HWND h = CreateWindowExA(0, kClass, kClass, style, 0, 0, 640, 480, nullptr, nullptr, GetModuleHandleA(nullptr), nullptr);
    if (!h) return nullptr;
...
struct ForeignWindow { HANDLE ready, done; HWND hwnd; };
static DWORD WINAPI foreign_thread(LPVOID p) {
    auto* f = static_cast<ForeignWindow*>(p);
    f->hwnd = create_game_window(WS_POPUP);
    SetEvent(f->ready);
    while (WaitForSingleObject(f->done, 10) == WAIT_TIMEOUT) { MSG m; while (PeekMessageA(&m, nullptr, 0, 0, PM_REMOVE)) DispatchMessageA(&m); }
    DestroyWindow(f->hwnd);
    return 0;
}
static void print_rect(const char* name, const Rect& r) { std::printf("%s=%ld,%ld,%ld,%ld", name, long(r.left), long(r.top), long(r.right), long(r.bottom)); }
...
static void refused(const char* label, HWND focus, HWND device, bool windowed, UINT bw, UINT bh, const char* reason) {
    ...
    require(text, r.evaluated && r.decision.action == wm::core::Action::refused && !std::strcmp(r.decision.reason, reason) && window_rect(target) == before);
}
...
int main() {
    ...
    require("config_row", x3m::lines == lines_before + 1 && x3m::last_line.find("window_mode_config requested=1 default=1") == 0);

    WNDCLASSA wc{};
    wc.style = CS_HREDRAW | CS_VREDRAW; wc.lpfnWndProc = DefWindowProcA; wc.hInstance = GetModuleHandleA(nullptr);
    wc.hCursor = nullptr; wc.hbrBackground = CreateSolidBrush(kFill); wc.lpszClassName = kClass;
    ...
    if (!game) { std::printf("RESULT checks=%u failed=%u FAIL\n", checks, failures); return 1; }
    ...
    std::printf("GEOMETRY "); print_rect("monitor", monitor); std::printf(" "); print_rect("work", work);
    std::printf(" offset=%ld offset_x=%ld\n", offset_y, offset_x);
    const UINT bw = UINT(monitor.right - monitor.left), bh = UINT(monitor.bottom - monitor.top);
    Rect placement; placement.left = work.left; placement.top = work.top; placement.right = work.left + LONG(bw); placement.bottom = work.top + LONG(bh);
    pump(200);
    const Rect placed = window_rect(game);
    std::printf("PLACED "); print_rect("rect", placed); std::printf("\n");
    ...
    if (menu_bar_offset) {
        require("moved_to_monitor_rect", moved.evaluated && moved.decision.action == wm::core::Action::move && moved.set_result && moved.after == monitor
    ...
    RECT client{}; GetClientRect(game, &client);
    ...
    pump(1200);
    {
        ...
        BITMAPINFO bi{}; bi.bmiHeader.biSize = sizeof bi.bmiHeader; bi.bmiHeader.biWidth = width; bi.bmiHeader.biHeight = -rows;
        ...
        if (blt && bits) {
            ...
            for (int y = 0; y < rows; ++y) {
                ...
                for (int x = 0; x < width; x += 16) { const DWORD p = pixels[y * width + x] & 0xffffffu; ++samples; match += p == fill; dark += p == 0; }
                if (match * 10 >= samples * 9) { ++fill_rows; if (y < offset_y) ++fill_rows_top; }
                if (dark * 10 >= samples * 9) ++black_rows;
            }
        }
        std::printf("SCREEN available=%d dib=%d screen_dc=%d error=%lu rows=%d fill_rows=%d fill_rows_top=%d black_rows=%d menu_rows=%ld foreground=%d\n", int(blt),
                    int(dib != nullptr), int(screen != nullptr), static_cast<unsigned long>(blt_error), rows, fill_rows, fill_rows_top, black_rows, offset_y,
                    int(GetForegroundWindow() == game));
    ...
    require("window_at_monitor_untouched", at_before == monitor && untouched.decision.action == wm::core::Action::noop && window_rect(at_monitor) == monitor);

    HWND spare = create_game_window(WS_POPUP);
    if (menu_bar_offset) require("spare_at_work_origin", window_rect(spare) == placement);
    else require("spare_at_work_origin", true);
    ...
    ForeignWindow foreign{CreateEventA(nullptr, TRUE, FALSE, nullptr), CreateEventA(nullptr, TRUE, FALSE, nullptr), nullptr};
    HANDLE thread = CreateThread(nullptr, 0, foreign_thread, &foreign, 0, nullptr);
    WaitForSingleObject(foreign.ready, 10000);
    if (foreign.hwnd) refused("foreign_thread", foreign.hwnd, nullptr, true, bw, bh, "foreign_thread");
    else require("refused_foreign_thread", false);
    SetEvent(foreign.done); WaitForSingleObject(thread, 10000); CloseHandle(thread);
    ...
    require("option_off_untouched", !wm::enabled() && !off.evaluated && x3m::lines == lines_off && window_rect(spare) == spare_before);

    DestroyWindow(caption); DestroyWindow(decorated); DestroyWindow(spare); DestroyWindow(at_monitor); DestroyWindow(game);
