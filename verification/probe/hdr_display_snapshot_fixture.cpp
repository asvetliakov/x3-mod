...
HRESULT begin_scene(IDirect3DDevice9*) {
    active->record(1);
    return active->scenario == Case::BeginScene ? E_FAIL : S_OK;
}
HRESULT end_scene(IDirect3DDevice9*) {
    active->record(2);
    return active->scenario == Case::EndScene ? E_FAIL : S_OK;
}
HRESULT stretch(IDirect3DDevice9*, IDirect3DSurface9*, const RECT*, IDirect3DSurface9*, const RECT*,
...
bool cleared(const HdrDisplaySnapshot& s) {
    ...
    return !s.valid && !s.resolved && !s.width && !s.height && s.sharpen == 0.f && s.decode == defaults.decode &&
           !std::memcmp(&s.agx, &defaults.agx, sizeof(s.agx)) &&
...
std::uint64_t HdrPass::stamp(bool) const noexcept {
    return 0;
}
HRESULT HdrPass::bind(IDirect3DSurface9*, std::uint64_t*) noexcept {
    active->record(4);
    return fault(HdrFault::Bind) ? E_FAIL : S_OK;
}
HRESULT HdrPass::copy_draw(IDirect3DSurface9*, IDirect3DTexture9* texture, IDirect3DSurface9*, UINT, UINT,
                           IDirect3DSurface9*, HRESULT* restoration, HRESULT injected_draw, bool injected_restore,
                           Program* program) noexcept {
    active->record(5);
    ++active->draws;
    active->sampled = texture;
    if (program && program->constants) std::memcpy(&active->constants, program->constants, sizeof(active->constants));
    if (program && program->sharpen) std::memcpy(&active->sharpen, program->sharpen, sizeof(active->sharpen));
...
Run run(Case scenario, x3::temporal::AgxDecode decode, bool resolved, bool sharp, bool output, bool scene_open) {
    ...
    snapshot.height = 777;
    snapshot.sharpen = .9f;
    snapshot.agx.exposure[0] = 999.f;
    snapshot.sharpen_constants.values[0] = 999.f;
    ...
    pass.native_ = native;
    pass.target_ = &target;
    pass.shader_ = &identity;
    pass.tonemap_shader_ = &tonemap;
    pass.sharpen_shader_ = &sharpen;
    pass.tonemap_sharpen_shader_ = &tonemap_sharpen;
    ...
    check(x3::temporal::prepare(pass.agx_, .78125f, 17.5f, decode, x3::temporal::AgxLook::golden),
    ...
    switch (scenario) {
    case Case::SharpenFallback:
    case Case::IdentityFallback:
    case Case::BothFallback: pass.set_fault(HdrFault::TonemapDraw, 1); break;
    ...
    case Case::Bind:
        write = false;
        pass.set_fault(HdrFault::Bind, 1);
        break;
    ...
    if (output)
        run.result = pass.write_back(main_arg, final_arg, scene_open, write, false, source, &snapshot);
    else
        run.result = pass.write_back(main_arg, final_arg, scene_open, write, false, source); // default-null ABI
    ...
    if (output) {
        const bool valid = scenario == Case::Normal || scenario == Case::SharpenFallback || scenario == Case::Meter ||
                           scenario == Case::SharpenDisabled || scenario == Case::Manual || scenario == Case::NoRing ||
                           scenario == Case::MutatedConfig;
        check(snapshot.valid == valid, "scenario validity");
        if (!valid)
            check(cleared(snapshot), "invalid snapshot clears every field");
        else {
            check(!std::memcmp(&snapshot.agx, &active->constants, sizeof(exact)) &&
                      !std::memcmp(&snapshot.agx, &exact, sizeof(exact)),
                  "bit-exact draw constants");
            ...
            const bool sharpened = sharp && resolved && scenario != Case::SharpenFallback &&
                                   scenario != Case::SharpenDisabled;
            check(snapshot.sharpen == (sharpened ? .75f : 0.f), "effective original sharpen strength");
            if (sharpened)
                check(!std::memcmp(&snapshot.sharpen_constants, &active->sharpen, sizeof(active->sharpen)),
                      "exact c23");
            else
                check(snapshot.sharpen_constants.values[0] == 1.f && snapshot.sharpen_constants.values[1] == 0.f,
    ...
    if (scenario == Case::BothFallback)
        check(run.result.sharpen_fallback && run.result.fallback && !run.result.tonemap && active->draws == 3 &&
                  active->meters == 1,
              "both fallbacks still submit meter once");
    if (scenario == Case::Manual || scenario == Case::NoRing)
        check(active->meters == 0, "manual/missing-ring skips meter");
    if (scenario == Case::Meter)
        check(run.result.meter == E_FAIL && run.result.draw == S_OK, "meter failure does not spoil display snapshot");
    active->output = nullptr;
    active->sampled = nullptr; // no stack pointer escapes the fixture
    return run;
}
...
PassBytes pass_bytes(const HdrPass& pass) {
    PassBytes result{};
    std::memcpy(result.data(), &pass, sizeof pass);
    return result;
}
struct ExchangeFixture {
    ...
    ExchangeFixture() {
        ...
        old_texture.level = &old_target;
        candidate_texture.level = &candidate;
        pass.device_ = &device;
        pass.native_ = native;
        pass.caps_.enabled = true;
        pass.caps_.tonemap = pass.caps_.meter = true;
        pass.target_ = &old_target;
        pass.width_ = pass.last_width_ = 17;
        pass.height_ = pass.last_height_ = 11;
        pass.shader_ = &identity;
        pass.tonemap_shader_ = &tonemap;
        pass.meter_level0_shader_ = &meter0;
        pass.meter_reduce_shader_ = &reduce;
        pass.quad_vs_ = &quad;
        pass.quad_declaration_ = &declaration;
        pass.chain_[0] = &chain;
        pass.chain_width_[0] = 5;
        pass.chain_height_[0] = 3;
        pass.chain_count_ = 1;
        pass.tile_width_ = 2;
        pass.tile_height_ = 1;
        pass.chain_texel_bytes_ = 8;
        pass.chain_slot_ = 1;
        for (unsigned i = 0; i < 2; ++i) {
            pass.chain_ring_[i] = &ring[i];
            pass.chain_readback_[i] = &readback[i];
            pass.chain_pending_[i] = true;
        }
        pass.tile_mean_ = std::make_unique<float[]>(2);
        pass.tile_mean_[0] = 0.25f;
        pass.tile_mean_[1] = 0.75f;
        pass.tile_capacity_ = 2;
        pass.latch_ticks_ = 123456;
        MeterStatistics meter{};
        meter.tiles = meter.lit = 1;
        meter.neutral = false;
        meter.lit_fraction = meter.lit_weight = 1.f;
        meter.lit_median_log = meter.avg_log_l = -2.f;
...
enum class ExchangeCase {
    ...
    NullOwner,
    ContainerFail,
    ContainerFailedOutput,
    NullContainer,
    Mips,
    TextureDescFail,
    TextureDescMismatch,
    TextureOwnerFail,
    TextureOwnerFailedOutput,
    TextureWrongDevice,
    TextureNullOwner,
    LevelFail,
    LevelFailedOutput,
...
unsigned exchange_cases() {
    ...
    for (unsigned i = 0; i <= unsigned(ExchangeCase::WrongLevel); ++i) {
        ...
        case ExchangeCase::OwnerFailedOutput:
            f.candidate.device_hr = E_FAIL;
            f.candidate.output_on_failure = true;
            break;
        ...
        case ExchangeCase::ContainerFailedOutput:
            f.candidate.container_hr = E_FAIL;
            f.candidate.output_on_failure = true;
            break;
        ...
        case ExchangeCase::TextureOwnerFailedOutput:
            f.candidate_texture.device_hr = E_FAIL;
            f.candidate_texture.output_on_failure = true;
            break;
        ...
        case ExchangeCase::LevelFailedOutput:
            f.candidate_texture.level_hr = E_FAIL;
            f.candidate_texture.output_on_failure = true;
            break;
        ...
        auto* old_target = f.pass.target();
        const auto before = pass_bytes(f.pass);
        const unsigned references = f.pass.references();
        ...
        if (which == ExchangeCase::Good) {
            check(f.pass.target() == old_candidate && candidate == old_target,
                  "exchange transfers both owned references");
            check(f.non_target_bytes_equal(before), "exchange preserves all non-target owner/meter/exposure bytes");
        } else
            check(f.pass.target() == old_target && candidate == old_candidate && pass_bytes(f.pass) == before,
    ...
    for (unsigned alias_case = 0; alias_case < 4; ++alias_case) {
        ...
        if (alias_case == 0) {
            ...
            check(result == S_OK && f.pass.target() == supplied && candidate == &f.old_target &&
                      f.non_target_bytes_equal(before),
                  "canonical device and surface aliases exchange successfully");
        f.balanced_queries();
    }
    ...
    for (unsigned position = 0; position < 8; ++position)
        for (unsigned fault = 0; fault < 3; ++fault) {
            ...
            const auto before = pass_bytes(f.pass);
            const HRESULT result = f.pass.exchange_target(candidate);
            check(result == (fault == 2 ? E_NOINTERFACE : E_FAIL),
                  "identity query failure remains distinct from inequality");
            check(candidate == &f.candidate && pass_bytes(f.pass) == before,
                  "failed identity query never mutates ownership or meter state");
            f.balanced_queries();
        }
    ...
    const auto steps = f.pass.exposure().steps();
    const auto ev = f.pass.exposure().ev();
    candidate->Release();
    candidate = nullptr; // caller retires its old target before native Reset
    f.pass.before_reset();
    check(!f.pass.target() && !f.pass.width() && !f.pass.height(), "Reset retires exchanged target");
    check(f.candidate.refs == 0 && f.old_target.refs == 0 && f.chain.refs == 0 && !f.ring[0].refs && !f.ring[1].refs &&
              !f.readback[0].refs && !f.readback[1].refs,
    ...
    f.pass.shutdown();
    check(!f.pass.device_ && !f.pass.native_ && !f.pass.enabled() && f.pass.references() == 0 && f.identity.refs == 0 &&
              f.tonemap.refs == 0 && f.meter0.refs == 0 && f.reduce.refs == 0 && f.quad.refs == 0 &&
              f.declaration.refs == 0 && f.pass.exposure().steps() == 0,
...
int main() {
    unsigned scenarios = 0;
    auto paired = [&](Case scenario, x3::temporal::AgxDecode decode, bool resolved, bool sharp,
    ...
    for (auto decode :
         {x3::temporal::AgxDecode::gamma22, x3::temporal::AgxDecode::srgb, x3::temporal::AgxDecode::none}) {
        for (bool resolved : {false, true})
            for (bool sharp : {false, true})
                for (bool scene_open : {false, true}) paired(Case::Normal, decode, resolved, sharp, scene_open);
