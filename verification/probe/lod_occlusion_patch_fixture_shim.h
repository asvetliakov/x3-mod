...
// src/proxy/lod_occlusion.cpp by build_lod_occlusion_patch.py, with
// -DX3M_LOD_OCCLUSION_SHIM, and nowhere else. It routes the four calls the
// patch's CodeOps makes (VirtualProtect, FlushInstructionCache,
// engine_patch::read_code, engine_patch::write_code) through pass-through
// wrappers defined in lod_occlusion_patch_fixture.cpp, which call the real
// function unless the fixture armed a one-shot fault for that call. The
// production source and the production build are untouched.
#include <windows.h>
#include <cstdint>
...
#ifdef X3M_LOD_OCCLUSION_SHIM
#define VirtualProtect x3m::lod_occlusion_fixture::virtual_protect
#define FlushInstructionCache x3m::lod_occlusion_fixture::flush_instruction_cache
