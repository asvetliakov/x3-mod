...
int main(){
    ...
    for(const auto& fixed:captured){
        ...
        for(unsigned j=0;j<3;++j)for(unsigned i=0;i<3;++i)translation[j]-=camera_world[i]*r[3*i+j];
        assert(fog_world_basis(r,translation,sun,b)&&b.valid);
        for(unsigned row=0;row<3;++row)for(unsigned col=0;col<3;++col){
            double v=0;for(unsigned k=0;k<3;++k)v+=r[3*row+k]*b.inverse_columns[3*col+k];
            assert(std::abs(v-(row==col?1:0))<1e-7);
        }
        assert(b.origin_mod[0]==32765&&b.origin_mod[1]==2&&b.origin_mod[2]==32767.75f);
        double length2=0;for(float v:b.sun_world)length2+=double(v)*v;
        assert(std::abs(length2-1)<1e-7);
    }
    double edge[9]={std::sqrt(1+.000999),0,0,0,1,0,0,0,1};
    assert(fog_world_basis(edge,t,sun,b));
    edge[0]=std::sqrt(1+.001001);assert(!fog_world_basis(edge,t,sun,b)&&!b.valid);
    const double shear[9]={1,.002,0,0,1,0,0,0,1};assert(!fog_world_basis(shear,t,sun,b)&&!b.valid);
    const double scaled[9]={1.01,0,0,0,1,0,0,0,1};assert(!fog_world_basis(scaled,t,sun,b));
    const double singular[9]={1,0,0,1,0,0,0,0,1};assert(!fog_world_basis(singular,t,sun,b));
