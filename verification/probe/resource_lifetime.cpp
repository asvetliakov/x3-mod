...
static bool check(const char* name, HRESULT hr) {
    ...
    expect(name, SUCCEEDED(hr)); return SUCCEEDED(hr);
}
...
struct Owner final : IUnknown {
    ...
    explicit Owner(IDirect3DTexture9* value) : texture(value) { texture->AddRef(); }
    ...
    ULONG WINAPI Release() override {
        ...
        if (!remaining) { ownerDestroyed = true; texture->Release(); delete this; }
        return remaining;
    }
};
...
int main() {
    ...
    RegisterClassA(&cls);
    HWND window = CreateWindowA(cls.lpszClassName, "X3 lifetime probe", WS_OVERLAPPEDWINDOW,
    ...
    for (unsigned kind = 0; kind < 2; ++kind) {
        ...
        if (!check("CreateDevice", api->CreateDevice(0, D3DDEVTYPE_HAL, window,
              D3DCREATE_HARDWARE_VERTEXPROCESSING | D3DCREATE_PUREDEVICE, &pp, &device))) break;
        IDirect3DTexture9* texture = nullptr;
        if (!check("CreateTexture FP16", device->CreateTexture(64, 64, 1, D3DUSAGE_RENDERTARGET,
              D3DFMT_A16B16G16R16F, D3DPOOL_DEFAULT, &texture, nullptr))) { device->Release(); break; }
        ...
        if (kind) {
            ...
            if (!check("CreateDepthStencilSurface D24X8", device->CreateDepthStencilSurface(64, 64,
                  D3DFMT_D24X8, D3DMULTISAMPLE_NONE, 0, TRUE, &depth, nullptr))) {
                texture->Release(); device->Release(); break;
            }
            ...
            std::printf("source surface Release returns %lu; private owner destroyed=%u\n",
                        (unsigned long)surfaceRefs, ownerDestroyed);
