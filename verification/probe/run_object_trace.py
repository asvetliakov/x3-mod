...
def main():
    ...
    inputs=['src/proxy/object_trace.cpp','src/proxy/object_trace.h','verification/probe/object_trace.cpp','verification/probe/build_object_trace.sh','verification/probe/run_object_trace.py']
    def digest(path):return hashlib.sha256(path.read_bytes()).hexdigest()
    def sources():return {name:digest(root/name) for name in inputs}
    results=root/'verification/results';results.mkdir(exist_ok=True)
    exe=root/'verification/probe/build/object_trace.exe'
    report=results/'object-trace.txt';wine_log=results/'object-trace-wine.log'
    data=dict(started_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),game_launched=False,fresh_build=True,sources_before=sources())
    ...
    if build.returncode==0 and data['sources_before']==data['sources_after_build']:
        ...
        match=re.search(r'RESULT (PASS|FAIL) checks=(\d+) failures=(\d+) backend_calls=(\d+)',report.read_text())
        if match:data.update(checks=int(match[2]),failures=int(match[3]),backend_calls=int(match[4]))
        data['passed']=bool(data['exit_code']==0 and match and match[1]=='PASS' and data['failures']==0 and data['sources_before']==data['sources_after_run'] and data['executable_sha256']==data['executable_sha256_after_run'])
