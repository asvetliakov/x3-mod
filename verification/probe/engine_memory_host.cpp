...
void decommit(unsigned i) {
    table[i].state = MEM_RESERVE;
    table[i].protect = 0;
}
void recommit(unsigned i) {
    table[i].state = MEM_COMMIT;
    table[i].protect = PAGE_READWRITE;
}
std::uintptr_t at(unsigned region, unsigned offset = 16) {
    return reinterpret_cast<std::uintptr_t>(arena[region]) + offset;
}
...
int main() {
    ...
    // outside the route): cached within 100 ms, re-queried after it.
    {
        ...
        now = 150;
        check(read_ok(at(0)), "pre-frame cached read");
        check(vq_calls == q + 1, "pre-frame reads share one query within the age bound");
        now = 50000;
        check(read_ok(at(0)), "pre-frame read after the age bound");
    ...
    // is decommitted ~0.9 s after that Present. The read must be refused.
    {
        ...
        now = 100010;
        check(read_ok(at(1)), "cached read in the same frame");
    ...
    // frame. Exactly one VirtualQuery per distinct region per frame.
    {
        ...
        check(s1.stalled_reads == s0.stalled_reads && s1.strict_reads == s0.strict_reads,
              "no stalled or strict read while frames advance");
        ...
        std::printf("HOTPATH frames=%u reads=%u queries=%llu ticks=%llu host_ns_per_read=%.1f\n", frames,
                    frames * per_frame, vq_calls - q, tick_calls - t, ns);
    }
    ...
    // validation (one query per region per tick step, not per read).
    {
        ...
        for (unsigned i = 0; i < 10; ++i) {
            now = 400000 + i * 9;
            check(read_ok(at(0)), "young frame read");
        }
        ...
        now = 400305;
        check(read_ok(at(0)), "stalled read 5 ms after validation");
        check(vq_calls == q + 2, "stalled: still trusted at 5 ms");
        now = 400306;
        check(read_ok(at(0)), "stalled read 6 ms after validation");
    ...
    // LastError survives a failed query, the next frame advance ends it.
    {
        ...
        check(s1.shutdown_signals == s0.shutdown_signals + 1 && s1.shutdown_source &&
                  !std::strcmp(s1.shutdown_source, "host_test"),
              "signal source kept");
