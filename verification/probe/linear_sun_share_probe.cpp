...
void require(bool ok) {
    ++checks;
    if (!ok) throw std::runtime_error("check " + std::to_string(checks));
}
...
struct Fixture {
    Words original, converted;
    Structure source, output;
    std::vector<SunSite> sites;
    unsigned seed = 0, final = 0;
};
Fixture fixture(bool branch = false, unsigned mutation = 0) {
    Fixture f;
    f.original = {0xffff0300u};
    ...
    if (branch) {
        emit(f.original, 42, {});
        emit(f.original, mov, {dst(temp, 0), src(temp, 4)});
        emit(f.original, 43, {});
    }
    ...
    require(
        structure(f.original.data(), f.original.size(), false, f.source, true, default_abi, 7, false, false, branch));
    f.converted = {0xffff0300u};
    definitions(f.converted, false, {});
    for (const auto& i : f.source.instructions) {
        const auto at = f.converted.size();
        f.sites.push_back({i.at, at});
    ...
    require(structure(f.converted.data(), f.converted.size(), false, f.output, false, default_abi, 7, false, false,
                      branch));
    return f;
}
int main(int argc, char** argv) {
    try {
        ...
        for (const bool branch : {false, true}) {
            auto f = fixture(branch);
            bool extracted = false;
            require(sun_share_variant(f.original.data(), f.source, f.converted, f.output, f.sites, {f.seed, 0}, f.final,
                                      extracted));
            require(extracted);
            write(std::string(argv[1]) + (branch ? "/branch.bin" : "/straight.bin"), f.converted);
        }
        for (unsigned mutation : {1u, 2u}) {
            auto f = fixture(false, mutation);
            bool extracted = true;
            require(sun_share_variant(f.original.data(), f.source, f.converted, f.output, f.sites, {f.seed, 0}, f.final,
                                      extracted));
            require(!extracted);
            write(std::string(argv[1]) + "/refused-" + std::to_string(mutation) + ".bin", f.converted);
        }
        auto f = fixture();
        ...
        for (unsigned r = 16; r <= 23; ++r) {
            auto c = f.converted;
            const auto at = c.size() - 1;
            c.pop_back();
            emit(c, mov, {dst(temp, r), src(temp, 0)});
            c.push_back(end_token);
            auto s = f.output;
            s.instructions.push_back({at, mov, 2});
        ...
        for (unsigned form = 0; form < 3; ++form) {
            ...
            auto s = f.output;
            s.instructions.push_back({at, form == 0 ? def : form == 1 ? mov : dcl, form == 0 ? 5u : 2u});
            c.push_back(end_token);
            require(!sun_resources_free(c, s));
        }
        Words reduction{0xffff0300u};
        definitions(reduction, false, {});
        emit(reduction, def, {dst(constant, 221, xyzw), bits(.2126f), bits(.7152f), bits(.0722f), bits(0x1p-20f)});
        sun_reduction(reduction);
        emit(reduction, mov, {dst(color_output, 2, 2), lane(temp, 23, 3)});
        reduction.push_back(end_token);
