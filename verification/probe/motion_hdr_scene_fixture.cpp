...
void callback(void* context, const MotionHdrScene& ready) noexcept {
    check(context == active, "caller context");
    ++active->callbacks;
    ...
    check(ready.main == active->main && motion.hdr_main_ == ready.main &&
              motion.hdr_state_ == MotionOutput::HdrState::Active,
    ...
    check(!std::memcmp(&ready.display.agx, &active->consumed.agx, sizeof(ready.display.agx)) &&
              ...
              !std::memcmp(&ready.display.sharpen_constants, &active->consumed.sharpen_constants,
                           sizeof(ready.display.sharpen_constants)),
          "exact display payload forwarded");
    active->display = ready.display; // caller-owned copy; no reference escapes
    ready.scene->AddRef();
    active->retained_scene = ready.scene;
    ready.main->AddRef();
    active->retained_main = ready.main;
}
}

namespace x3m::renderer {
HdrPass::~HdrPass() = default;
HdrWriteback HdrPass::write_back(IDirect3DSurface9* main, IDirect3DSurface9* final_rt, bool, bool write, bool,
                                 IDirect3DTexture9* source, HdrDisplaySnapshot* display) noexcept {
    active->calls.push_back(2);
    ++active->writes;
    ...
    if (active->scenario == Case::AgxFailure || !target_) {
        ...
        result.unwind = true;
        result.restore = E_FAIL;
    } else if (active->scenario == Case::Identity) {
        result.tonemap = false;
        result.fallback = true;
        result.unwind = true;
    } else if (active->scenario == Case::MeterFailure)
        result.meter = E_FAIL;
...
template <class T> void release(T*& p) noexcept {
    if (T* old = p) {
        p = nullptr;
        old->Release();
    }
...
const char* hdr_source_name(unsigned) {
    return "fixture";
}
MotionOutput::MotionOutput() noexcept = default;
MotionOutput::~MotionOutput() = default;
void MotionOutput::restore_bindings() noexcept {
    active->calls.push_back(5);
}
void MotionOutput::finish_cut_detector() noexcept {
    active->calls.push_back(6);
    cut_finished_ = true;
}
void MotionOutput::invalidate_taa(TaaInvalidateSite) noexcept {
    active->calls.push_back(7);
}
bool MotionOutput::ensure_taa() noexcept {
    active->calls.push_back(8);
    return active->scenario != Case::TaaInitialize;
}
...
renderer::ShadowReplayPass::~ShadowReplayPass() = default; // the depth-replay pass is never constructed here; only the
                                                           // member destructor is needed
renderer::SunShadowApplyPass::~SunShadowApplyPass() = default; // likewise: only the unique_ptr member destructor is
                                                               // needed
...
void MotionOutput::run_volumetric_fog() noexcept {} // the hook's call is behind fog_requested_ (default false): never
                                                    // reached here
bool renderer::LinearEmissionPass::reference_accounting_busy() const noexcept {
    return false;
}
bool renderer::LinearEmissionPass::coverage_valid() const noexcept {
    return true;
}
...
HRESULT MotionOutput::readback_surface(IDirect3DSurface9*, D3DFORMAT, unsigned, const wchar_t*, const wchar_t*,
                                       const char*, const char*, UINT, UINT) noexcept {
    active->calls.push_back(10);
    return S_OK;
}
HRESULT MotionOutput::resolve(IDirect3DSurface9*, IDirect3DTexture9* scene) noexcept {
    active->calls.push_back(11);
    ++active->resolves;
...
Run run(Case scenario, bool taa, bool supplied) {
    ...
    void* native[39]{};
    native[38] = reinterpret_cast<void*>(&get_rt);
    motion.device_ = &device;
    motion.native_ = native;
    motion.id_ = 37;
    motion.frame_ = 43;
    motion.generation_ = 5;
    motion.enabled_ = motion.hdr_enabled_ = motion.scene_open_ = true;
    motion.taa_enabled_ = taa;
    motion.jitter_active_ = true;
    motion.counters_.filled = true;
    motion.target_surface_ = &motion_target;
    motion.depth_surface_ = &depth;
    motion.hdr_ = std::make_unique<renderer::HdrPass>();
    motion.hdr_->target_ = &target;
    motion.hdr_->width_ = 17;
    motion.hdr_->height_ = 11;
    motion.hdr_main_ = &main;
    motion.hdr_state_ = MotionOutput::HdrState::Active;
    motion.hdr_dirty_ = true;
    motion.selector_.state_ = renderer::BoundaryState::Scene;
    motion.main_ = {true, 1, 0, 17, 11, unsigned(D3DFMT_A8R8G8B8), 0};
    observed.consumed.valid = true;
    observed.consumed.width = 17;
    observed.consumed.height = 11;
    observed.consumed.sharpen = taa && scenario != Case::UnsharpenedRetry ? .75f : 0.f;
    x3::temporal::prepare(observed.consumed.agx, .78125f, 19.f, x3::temporal::AgxDecode::srgb,
                          x3::temporal::AgxLook::golden);
    ...
    case Case::PriorAttempt:
    case Case::PriorOtherAttempt:
        motion.counters_.taa.attempted = motion.counters_.taa.resolved = motion.counters_.taa.hdr = true;
        motion.counters_.taa.source = unsigned(scenario == Case::PriorAttempt ? SceneEndSource::Hook
                                                                              : SceneEndSource::StretchRect);
        motion.hdr_resolved_ = &resolved;
        break;
    case Case::HdrOff:
        motion.hdr_state_ = MotionOutput::HdrState::Off;
        motion.hdr_main_ = nullptr;
        main.refs = 1;
        break;
    ...
    case Case::ContainerFailureOutput:
        target.container_hr = E_FAIL;
        target.failure_output = true;
        break;
    default: break;
    }
    check(motion.reference_accounting_busy() == (scenario == Case::Releasing || scenario == Case::TaaBusy),
          "reference accounting accessor");
    if (supplied)
        motion.scene_end_hook(&callback, &observed);
    else
        motion.scene_end_hook(); // unchanged public call compiles and executes
    const bool accepted = scenario == Case::Normal || scenario == Case::MeterFailure ||
                          scenario == Case::UnsharpenedRetry;
    ...
    if (observed.callbacks) {
        check(motion.hdr_state_ == MotionOutput::HdrState::Off && !motion.hdr_main_ && !motion.hdr_resolved_,
              "redirect cleaned after callback");
        ...
        observed.retained_scene->Release();
        observed.retained_main->Release();
    }
    for (auto unavailable : {Case::Disabled, Case::HdrDisabled, Case::NoOpenScene, Case::Query, Case::Recording,
        ...
        if (scenario == unavailable)
            check(!motion.bloom_boundary_available(), "CPU admission rechecks current failure/activity state");
    if (scenario == Case::ContainerFailureOutput)
        check(original.refs == 1, "failed GetContainer non-null output released");
    const unsigned writes = observed.writes, resolves = observed.resolves, callbacks = observed.callbacks;
    if (supplied)
        motion.scene_end_hook(&callback, &observed);
    else
        motion.scene_end_hook();
    ...
    check(original.refs == 1 && resolved.refs == 1 && target.refs == 1,
          "all ordinary-path temporary scene refs balanced");
...
int main() {
    ...
    auto paired = [&](Case scenario, bool taa) {
        ...
        check(on.counters.hdr.writebacks == off.counters.hdr.writebacks &&
                  on.counters.taa.attempted == off.counters.taa.attempted &&
                  on.counters.taa.resolved == off.counters.taa.resolved &&
                  on.counters.hdr.end == off.counters.hdr.end && on.counters.hdr.unwind == off.counters.hdr.unwind &&
                  on.counters.restore_failures == off.counters.restore_failures,
    ...
    for (bool taa : {false, true}) {
        for (auto scenario :
             {Case::Normal,          Case::Disabled,     Case::HdrDisabled,     Case::OutsideScene,
              Case::Duplicate,       Case::NoOpenScene,  Case::Query,           Case::Recording,
              Case::Blocked,         Case::PriorUnwind,  Case::PriorRestore,    Case::Releasing,
              Case::TaaBusy,         Case::Msaa,         Case::HdrOff,          Case::HdrSuspended,
              Case::NoHdr,           Case::NoMain,       Case::NoTarget,        Case::AgxFailure,
              Case::AgxRestore,      Case::Identity,     Case::InvalidSnapshot, Case::SnapshotDimensions,
              Case::MainDimensions,  Case::MainIdentity, Case::SnapshotSource,  Case::MeterFailure,
              Case::UnsharpenedRetry})
            paired(scenario, taa);
    }
    for (auto scenario :
         {Case::NoJitter, Case::NotFilled, Case::TaaInitialize, Case::TaaTarget, Case::TaaContainer, Case::TaaFailure,
          Case::TaaRestore, Case::TaaNoTexture, Case::PriorAttempt, Case::PriorOtherAttempt})
        paired(scenario, true);
    for (auto scenario : {Case::RebindOnly, Case::ContainerFailure, Case::ContainerNull, Case::ContainerFailureOutput})
        paired(scenario, false);
