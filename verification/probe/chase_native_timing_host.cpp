...
static Token token(std::uint32_t thread = 1, std::uint64_t generation = 7, std::uint64_t update = 11,
                   std::uint32_t cockpit = 0x1000) {
...
static void four_phases_and_solver_false() {
    ...
    for (unsigned phase = 0; phase < phase_count; ++phase) {
        ...
        end_token.mode = 300 + phase;
        end_token.target = 0x3000 + phase;
    ...
    for (unsigned phase = 0; phase < phase_count; ++phase) {
        ...
        check(metric.failures == (phase == 1 ? 1u : 0u), "only false solver result increments failure count");
    }
...
static void unmatched_common_end() {
    ...
    check(state.window.unmatched == 1 && state.window.metrics[0].calls == 0, "common end without start is unmatched");
}
...
static void nested_begin_abandons() {
    ...
    check(state.window.abandoned == 1 && live_starts(state) == 1, "nested begin of same phase abandons prior start");
    auto next = first;
    ++next.update;
...
static void explicit_revoke_paths() {
    ...
    check(state.window.abandoned == 1 && live_starts(state) == 1, "cockpit destructor revokes only its matching span");
...
static void invalid_and_backwards_clock() {
    ...
    check(live_starts(state) == 0 && state.window.unmatched == 0, "invalid phase/token/frame/QPC inputs are ignored");
...
static void bounded_thread_capacity() {
    ++scenarios;
    State state;
    for (std::uint32_t thread = 1; thread <= thread_count; ++thread) state.begin(token(thread), 0, thread, 10 + thread);
...
static void slow_sample_ring() {
    ...
    check(state.window.slow == 10 && state.window.first_used == 4 && state.window.last_used == 4 &&
              state.window.slow - 8 == 2,
