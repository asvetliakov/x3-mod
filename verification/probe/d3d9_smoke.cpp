...
static void draw(IDirect3DDevice9* device) {
    const Vertex vertices[] = {
        {10, 10, .5f, 1, 0xffff0000}, {160, 180, .5f, 1, 0xff00ff00}, {310, 10, .5f, 1, 0xff0000ff}};
...
int main() {
    ...
    if (!api) {
        std::puts("Create9 null");
        return 1;
    }
    IUnknown* apiIdentity = nullptr;
    check("D3D9 QueryInterface IUnknown", api->QueryInterface(IID_PPV_ARGS(&apiIdentity)));
    expect("D3D9 COM identity", apiIdentity == static_cast<IUnknown*>(api));
    if (apiIdentity) apiIdentity->Release();
    WNDCLASSA cls = {};
    cls.lpfnWndProc = DefWindowProcA;
    cls.hInstance = GetModuleHandleA(nullptr);
    cls.lpszClassName = "X3Smoke";
    RegisterClassA(&cls);
    HWND window = CreateWindowA(cls.lpszClassName, "X3 D3D9 proxy smoke test", WS_OVERLAPPEDWINDOW, 80, 80, 340, 240,
    ...
    for (int iteration = 0; iteration < 2; ++iteration) {
        ...
        check("GetDirect3D", device->GetDirect3D(&parent));
        expect("GetDirect3D identity", parent == api);
        if (parent) parent->Release();
        IUnknown* identity = nullptr;
        check("Device QueryInterface IUnknown", device->QueryInterface(IID_PPV_ARGS(&identity)));
        expect("Device COM identity", identity == static_cast<IUnknown*>(device));
        if (identity) identity->Release();
        IDirect3DTexture9* texture = nullptr;
        if (check("CreateTexture managed",
                  device->CreateTexture(16, 16, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &texture, nullptr))) {
            IDirect3DDevice9* owner = nullptr;
            check("Texture GetDevice", texture->GetDevice(&owner));
            expect("Texture GetDevice identity", owner == device);
            if (owner) owner->Release();
            check("SetTexture", device->SetTexture(0, texture));
            check("UnsetTexture", device->SetTexture(0, nullptr));
            texture->Release();
        }
        ...
        if (check("CreateStateBlock", device->CreateStateBlock(D3DSBT_ALL, &state))) {
            check("StateBlock Capture", state->Capture());
            check("Change captured state", device->SetRenderState(D3DRS_ALPHABLENDENABLE, TRUE));
            check("StateBlock Apply", state->Apply());
            DWORD value = 1;
            check("GetRenderState after Apply", device->GetRenderState(D3DRS_ALPHABLENDENABLE, &value));
            expect("StateBlock restored value", value == FALSE);
            state->Release();
        }
        check("BeginStateBlock", device->BeginStateBlock());
        check("Record state", device->SetRenderState(D3DRS_ALPHABLENDENABLE, FALSE));
        state = nullptr;
        check("EndStateBlock", device->EndStateBlock(&state));
        if (state) {
            check("Recorded state Apply", state->Apply());
            state->Release();
        }
        ...
        if (check("CreatePixelShader", device->CreatePixelShader(bytecode, &shader))) {
            DWORD copy[5] = {};
            UINT size = sizeof copy;
            check("PixelShader GetFunction", shader->GetFunction(copy, &size));
            expect("PixelShader bytecode preserved",
                   size == sizeof bytecode && std::memcmp(copy, bytecode, sizeof bytecode) == 0);
            IDirect3DDevice9* owner = nullptr;
            check("PixelShader GetDevice", shader->GetDevice(&owner));
            expect("PixelShader owner identity", owner == device);
            if (owner) owner->Release();
            check("SetPixelShader", device->SetPixelShader(shader));
            const float color[] = {.2f, .6f, .9f, 1};
            check("SetPixelShaderConstantF", device->SetPixelShaderConstantF(0, color, 1));
        }
        draw(device);
        draw(device);
        check("UnsetPixelShader", device->SetPixelShader(nullptr));
        if (shader) shader->Release();
        check("Reset", device->Reset(&pp));
        draw(device);
        expect("Device final Release reaches zero", device->Release() == 0);
    }
    expect("D3D9 final Release reaches zero", api->Release() == 0);
