...
// as the "sunapply" mode does: identity light rows (the map is the draws' own
// x, y and depth z), one managed vertex buffer of four quads under a
...
void run_shadow_alpha(Fixture& f) {
    ...
    // The draws: identity light rows (c0-c3), the documented GREATEREQUAL ref 1 threshold of the game's passes.
    ...
    for (unsigned i = 0; i < 4; ++i) {
        ...
        for (unsigned k = 0; k < 4; ++k) d.light_rows[k * 4 + k] = 1.f;
    ...
    const auto transaction = [&](ShadowReplayPass& pass, const ShadowReplayDraw* list, unsigned count, const char* label, ShadowReplayResult& out) -> HRESULT {
        ...
        const HRESULT hr = pass.execute(list, count, true, false, &out);
    ...
    api(a.attach(f.d.p, nullptr, caps, D3DFMT_X8R8G8B8, alpha_map_size), "attach alpha pass");
    api(b.attach(f.d.p, nullptr, caps, D3DFMT_X8R8G8B8, alpha_map_size), "attach depth-only pass");
    ...
    // Per issue five calls; H adds the pair (2), sampler 0 (8), its texture and the threshold;
    // Z its texture (same threshold); O2 the depth-only pair back (2).
    std::printf("ALPHA_CALLS state_calls=%u expected=%u\n", out.state_calls_map[0], 4u * 5u + 12u + 1u + 2u);
    check(out.state_calls_map[0] == 4u * 5u + 12u + 1u + 2u, "the issue path's native calls (no redundant alpha state)");
    ...
    check(out.state_calls_map[0] == 2u * 5u, "no alpha state for opaque draws");
    ...
    { api(f.d->BeginScene(), "BeginScene refusal"); hostile_state();
      const Snapshot before = f.snapshot();
      const HRESULT hr = b.execute(draws, 4, true, false, &out);
    ...
    // A refused alpha program: the pass stays enabled for the depth-only casters, holding nothing more.
    {
        ...
        api(r.attach(f.d.p, table, caps, D3DFMT_X8R8G8B8, alpha_map_size), "attach with the alpha program refused");
