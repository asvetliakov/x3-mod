...
void* operator new(std::size_t n) {
    if (refuse_allocation) std::abort();
...
struct Run {
    ...
    void stamp(unsigned index, std::uint64_t now) {
        pass.stamp(index, now, frame.submission_ticks_at(now), frame.submit_begin);
    }
    void material(std::uint64_t now) {
        residual.material(now, pass.end_clock, pass.begin_clock, pass.begin_armed, frame.submission_ticks_at(now),
                          pass.end_submission, pass.begin_submission, frame.submit_begin != 0);
    }
    void draw(std::uint64_t now) {
        for (unsigned i = 0; i < 4; ++i) stamp(i, now + 10 * i);
    }
};
int main() {
    ...
    for (const auto gap : {100ull, 100000ull}) {
        ...
        r.material(220);
        r.draw(230);
        r.frame.site(7, 300 + gap);
        r.frame.site(8, 310 + gap);
        r.material(320 + gap);
        r.draw(330 + gap);
        r.frame.site(9, 370 + gap);
        x3m::residual_phases::detail::Sample rs;
        r.residual.take(1, 1000000, 1000 + gap, 20, 170, 2, r.pass.passes, r.pass.begin_clock, r.pass.begin_armed, rs,
                        r.pass.begin_submission);
        ...
        check(!r.pass.orphans && !r.pass.clock_errors && !r.pass.clock_failures && !r.pass.scope_errors &&
              !r.pass.complement_underflow);
        check(!r.residual.scope_errors && !r.residual.clock_errors && !r.residual.clock_failures &&
              !r.residual.other_underflow);
    ...
    Run cross;
    cross.frame.restart(1);
    cross.frame.site(7, 10);
    cross.frame.site(8, 20);
    cross.stamp(0, 30);
    cross.stamp(1, 40);
    cross.frame.site(9, 50);
    cross.stamp(2, 60);
    cross.stamp(3, 70);
    check(cross.pass.scoped_ticks == 20 && cross.pass.outside_ticks == 20 && cross.pass.crossing_passes == 1);
    cross.pass.stamp(3, 0);
    check(!cross.pass.end_clock && cross.pass.clock_failures == 1);
    Run envelop;
    envelop.frame.restart(1);
    envelop.stamp(0, 10);
    envelop.frame.site(7, 20);
    envelop.frame.site(8, 30);
    envelop.stamp(1, 40);
    envelop.frame.site(9, 50);
    envelop.stamp(2, 60);
    envelop.stamp(3, 70);
    ...
    for (unsigned i = 0; i < 3; ++i) {
        x3m::pass_phases::detail::Accumulator a;
        a.scoped_ticks = (i == 1 ? 100 : 10);
        x3m::pass_phases::detail::Sample s;
        a.take(i, 1000000, i == 0 ? 100 : 110, s);
        w.add(s);
    }
    x3m::pass_phases::detail::Summary summary;
    check(w.close(summary));
    ...
    check(!off.clock_errors && !off.total());
