...
int main() {
    ...
    x3m::FpsOverlay::format(text, sizeof text, 61.28, 16.66, 638);
    CHECK(!std::strcmp(text, "FPS 61.3  16.7 MS  DRAWS 638"));
    x3m::FpsOverlay::format(text, sizeof text, 60.0, 16.6666, 0);
    CHECK(!std::strcmp(text, "FPS 60.0  16.7 MS  DRAWS 0"));
    x3m::FpsOverlay::format(text, sizeof text, -1.0, -5.0, 1);
    CHECK(!std::strcmp(text, "FPS 0.0  0.0 MS  DRAWS 1"));
    x3m::FpsOverlay::format(text, sizeof text, 1e9, 1e9, 1u << 31);
    CHECK(!std::strcmp(text, "FPS 9999.9  9999.9 MS  DRAWS 999999"));
    CHECK(std::strlen(text) <= 36); // the notice's column count
    x3m::FpsOverlay::format(text, sizeof text, 0.0 / 0.0, 0.0 / 0.0, 3);
    CHECK(!std::strcmp(text, "FPS 0.0  0.0 MS  DRAWS 3"));
    x3m::FpsOverlay overlay;
    CHECK(!overlay.requested() && !overlay.visible()); // unrequested: inert
    overlay.configure(false, hz);
    CHECK(!overlay.visible());
    for (unsigned i = 0; i < 3000; ++i)
        CHECK(!overlay.frame(std::uint64_t(i) * 16667, 600)); // never refreshes while unrequested
    CHECK(overlay.line()[0] == '\0');
    overlay.configure(true, hz);
    CHECK(overlay.requested() && overlay.visible() && overlay.line()[0] == '\0');
    ...
    for (unsigned i = 0; i < 600; ++i) {
        now += 16667;
        if (overlay.frame(now, 600)) {
            ++refreshes;
            if (!first_refresh_frame) first_refresh_frame = i;
        }
    }
    ...
    unsigned frames_to_30 = 0;
    bool saw_blend = false;
    for (unsigned i = 0; i < 120; ++i) {
        now += 33333;
        if (overlay.frame(now, 300)) {
            const double fps = std::atof(overlay.line() + 4);
            if (fps > 30.5 && fps < 60.0) saw_blend = true;
            if (!frames_to_30 && fps < 30.5) frames_to_30 = i + 1;
        }
    }
    CHECK(saw_blend && frames_to_30 > 0 && frames_to_30 <= 38); // 38 frames at 33.3 ms = 1.27 s: the last 60-fps bucket
                                                                // has left the window
    CHECK(!std::strcmp(overlay.line(), "FPS 30.0  33.3 MS  DRAWS 300"));
    // Draws average over the window, rounded to nearest.
    for (unsigned i = 0; i < 120; ++i) {
        now += 33333;
        overlay.frame(now, i % 2 ? 301 : 300);
    }
    ...
    CHECK(overlay.visible() && overlay.line()[0] == '\0');
    now += hz;
    CHECK(!overlay.frame(now, 1));
    for (unsigned i = 0; i < 14; ++i) {
        now += 16667;
        CHECK(!overlay.frame(now, 1));
    }
    now += 16667;
    CHECK(overlay.frame(now, 1) && !std::strcmp(overlay.line(), "FPS 60.0  16.7 MS  DRAWS 1"));
    ...
    overlay.reset();
    CHECK(overlay.fog(1)); // Reset forgets the written state
    ...
    for (unsigned i = 0; i < 100; ++i)
        CHECK(!overlay.draw_outcome(true) && overlay.visible()); // retried, not re-logged
    CHECK(!overlay.draw_outcome(false) && !overlay.draw_failed());
    CHECK(overlay.draw_outcome(true)); // a new episode after a success logs again
    overlay.reset();
    CHECK(!overlay.draw_failed() && overlay.draw_outcome(true));
