...
struct PinNames {
    ...
    const char* types;
    const char* type_next;
    const char* selection;
};
static constexpr PinNames source_names{
    "source",           "source_enum_pins",      "source_pin_next", "source_direction", "source_enum_types",
    "source_type_next", "source_audio_selection"};
static constexpr PinNames sink_names{
    "sink",           "sink_enum_pins",      "sink_pin_next", "sink_direction", "sink_enum_types",
    "sink_type_next", "sink_audio_selection"};
static constexpr PinNames input_names{"decoder_input",          "decoder_in_enum_pins",  "decoder_in_pin_next",
                                      "decoder_in_direction",   "decoder_in_enum_types", "decoder_in_type_next",
                                      "decoder_input_selection"};
static constexpr PinNames output_names{"decoder_output",          "decoder_out_enum_pins",  "decoder_out_pin_next",
                                       "decoder_out_direction",   "decoder_out_enum_types", "decoder_out_type_next",
                                       "decoder_output_selection"};
...
static bool audio_pin(Trace& t, IBaseFilter* filter, const PinNames& names, PIN_DIRECTION wanted, IPin*& chosen,
    ...
    for (unsigned pi = 0;; ++pi) {
        IPin* pin{};
        hr = t.call(names.next, pi + 1, true, [&] { return pins->Next(1, &pin, nullptr); });
        if (hr == S_FALSE) break;
        if (hr != S_OK || pi == 16) {
            if (pin) pin->Release();
            failure = FAILED(hr) ? hr : E_UNEXPECTED;
            t.need(names.next, failure);
            break;
        }
            ...
            if (SUCCEEDED(hr)) {
                for (unsigned ti = 0;; ++ti) {
                    ...
                    if (hr != S_OK || ti == 16 || !mt) {
                        free_type(mt);
                        failure = FAILED(hr) ? hr : E_UNEXPECTED;
                        t.need(names.type_next, failure);
                        break;
                    }
                    char major[40], sub[40], format[40];
                    guid_text(mt->majortype, major);
                    guid_text(mt->subtype, sub);
                    guid_text(mt->formattype, format);
                    WAVEFORMATEX wave{};
                    ...
                    t.prefix("ASF_TYPE");
                    std::printf(
                        " role=%s pin=%u type=%u major=%s subtype=%s format=%s format_bytes=%lu tag=%u channels=%u rate=%lu bits=%u align=%u avg=%lu extra=%u\n",
                        names.role, pi, ti, major, sub, format, static_cast<unsigned long>(mt->cbFormat),
                        wave.wFormatTag, wave.nChannels, static_cast<unsigned long>(wave.nSamplesPerSec),
                        wave.wBitsPerSample, wave.nBlockAlign, static_cast<unsigned long>(wave.nAvgBytesPerSec),
                        wave.cbSize);
                ...
                failure = hr;
                t.need(names.types, hr);
            }
        }
        t.prefix("ASF_PIN");
        std::printf(" role=%s pin=%u direction=%d audio=%d\n", names.role, pi, int(direction), audio);
        if (audio) {
            ++candidates;
            if (!chosen) {
                chosen = pin;
    ...
    pins->Release();
    t.prefix("ASF_SELECTION");
    std::printf(" role=%s candidates=%u\n", names.role, candidates);
...
static bool negotiated(Trace& t, IPin* pin, const char* role, const char* name) {
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (mt.formattype == FORMAT_WaveFormatEx && mt.pbFormat && mt.cbFormat >= 16)
            std::memcpy(&wave, mt.pbFormat, std::min<std::size_t>(sizeof(wave), mt.cbFormat));
        t.prefix("ASF_NEGOTIATED");
        std::printf(
            " role=%s major=%s subtype=%s format=%s format_bytes=%lu tag=%u channels=%u rate=%lu bits=%u align=%u avg=%lu extra=%u\n",
            role, major, sub, format, static_cast<unsigned long>(mt.cbFormat), wave.wFormatTag, wave.nChannels,
...
static bool explicit_create(Graph& g, const wchar_t* path, IDirectSound8* sound, int mode, State& state) {
    ...
    AM_MEDIA_TYPE* sink_type{};
    AM_MEDIA_TYPE* decoder_in_type{};
    AM_MEDIA_TYPE* decoder_out_type{};
    ...
    if (mode == 1) {
        ...
        HRESULT hr = t.call("wma_wrapper_activate", 1, false, [&] {
            return CoCreateInstance(CLSID_DMOWrapperFilter, nullptr, CLSCTX_INPROC_SERVER, IID_IBaseFilter,
                                    reinterpret_cast<void**>(&g.wrapper));
        });
        if (SUCCEEDED(hr))
            hr = t.call("wma_wrapper_qi", 1, false, [&] {
                return g.wrapper->QueryInterface(IID_IDMOWrapperFilter, reinterpret_cast<void**>(&dmo));
            });
        if (SUCCEEDED(hr))
            hr = t.call("wma_decoder_init", 1, false,
                        [&] { return dmo->Init(CLSID_CWMADecMediaObject, DMOCATEGORY_AUDIO_DECODER); });
        state.decoder_hr = hr;
        state.decoder_available = SUCCEEDED(hr);
        t.release("release_asf_dmo_interface", dmo);
        t.prefix("ASF_DECODER");
        std::printf(" available=%d hr=%08lx\n", state.decoder_available, static_cast<unsigned long>(hr));
    }
    auto work = [&]() -> bool {
#define NEED(name, expr)                                                                                               \
    do {                                                                                                               \
        ...
        NEED("activate_stream", CoCreateInstance(CLSID_AMMultiMediaStream, nullptr, CLSCTX_INPROC_SERVER,
                                                 IID_IAMMultiMediaStream, reinterpret_cast<void**>(&g.multi)));
        ...
        NEED("set_pcm", g.audio->SetFormat(&wanted));
        NEED("get_graph", g.multi->GetFilterGraph(&g.graph));
        NEED("asf_activate", CoCreateInstance(CLSID_WMAsfReader, nullptr, CLSCTX_INPROC_SERVER, IID_IBaseFilter,
                                              reinterpret_cast<void**>(&g.source)));
        ...
        NEED("asf_load", file->Load(path, nullptr));
        state.loaded = true;
        ...
        if (mode == 0) {
            ...
            if (!negotiated(t, out, "source", "source_connected_type") ||
                !negotiated(t, in, "sink", "sink_connected_type"))
                return false;
        } else {
            ...
            if (!negotiated(t, out, "source", "source_connected_type") ||
                !negotiated(t, decoder_in, "decoder_input", "decoder_input_connected_type"))
                return false;
            ...
            if (!negotiated(t, decoder_out, "decoder_output", "decoder_output_connected_type") ||
                !negotiated(t, in, "sink", "sink_connected_type"))
                return false;
        }
        state.connected = true;
        // Preserve the original constructor's post-open sample/buffer contract.
        t.release("release_audio_pre", g.audio);
        t.release("release_media_pre", g.media);
        ...
        std::printf(" tag=%u rate=%lu channels=%u bits=%u align=%u avg=%lu extra=%u\n", f.wFormatTag,
                    static_cast<unsigned long>(f.nSamplesPerSec), f.nChannels, f.wBitsPerSample, f.nBlockAlign,
                    static_cast<unsigned long>(f.nAvgBytesPerSec), f.cbSize);
        if (f.wFormatTag != WAVE_FORMAT_PCM || f.wBitsPerSample != 16 || !f.nChannels ||
            f.nBlockAlign != 2 * f.nChannels || !f.nSamplesPerSec || !f.nAvgBytesPerSec ||
            f.nAvgBytesPerSec > 1000000 || std::uint64_t(f.nSamplesPerSec) * f.nBlockAlign != f.nAvgBytesPerSec)
        ...
        NEED("activate_audio_data", CoCreateInstance(CLSID_AMAudioData, nullptr, CLSCTX_INPROC_SERVER, IID_IAudioData,
                                                     reinterpret_cast<void**>(&g.data)));
        NEED("set_buffer_native", g.data->SetBuffer(static_cast<DWORD>(g.pcm.size()), g.pcm.data(), 0));
        NEED("data_format", g.data->SetFormat(&f));
        NEED("create_sample", g.audio->CreateSample(g.data, 0, &g.sample));
        DSBUFFERDESC desc{};
        desc.dwSize = sizeof(desc);
        desc.dwFlags = 0x10088;
        desc.dwBufferBytes = f.nAvgBytesPerSec * 5;
        desc.lpwfxFormat = &g.format;
        ...
        NEED("stream_run", g.multi->SetState(STREAMSTATE_RUN));
        NEED("control_pause", g.control->Pause());
        g.constructed = true;
        return true;
#undef NEED
    };
    bool result = work();
    t.release("release_explicit_file", file);
    t.release("release_explicit_manual_filter", sink);
    t.release("release_source_pin", out);
    t.release("release_sink_pin", in);
    t.release("release_decoder_input_pin", decoder_in);
    t.release("release_decoder_output_pin", decoder_out);
    free_type(source_type);
    free_type(sink_type);
    free_type(decoder_in_type);
    free_type(decoder_out_type);
    return result;
}
int wmain(int argc, wchar_t** argv) {
    ...
    if (FAILED(init)) {
        std::printf("ASF_ABORT stage=co_initialize hr=%08lx\n", static_cast<unsigned long>(init));
    ...
    wc.lpszClassName = L"X3ExplicitAsfProbe";
    RegisterClassW(&wc);
    HWND window = CreateWindowW(wc.lpszClassName, L"X3 silent ASF probe", WS_OVERLAPPEDWINDOW, 0, 0, 64, 64, nullptr,
    ...
    for (int source = 0; source < 2; ++source)
        for (int mode = 0; mode < 2; ++mode)
            for (int repeat = 0; repeat < 2; ++repeat) {
                ...
                Graph g(t);
                State state;
                bool made = explicit_create(g, argv[1 + source], sound, mode, state);
                topology(t, g.graph);
                bool decoded = made && g.decode();
                g.cleanup();
                t.prefix("ASF_CASE");
                std::printf(
                    " source_available=%d loaded=%d pins_selected=%d decoder_attempted=%d decoder_available=%d decoder_hr=%08lx connected=%d constructed=%d decoded=%d fatal=%s hr=%08lx cleanup=1\n",
                    state.source_available, state.loaded, state.pins_selected, state.decoder_attempted,
                    state.decoder_available, static_cast<unsigned long>(state.decoder_hr), state.connected, made,
                    decoded, t.fatal, static_cast<unsigned long>(t.fatal_hr));
            }
    t.source = 0;
    t.route = 0;
    t.wrapper = 0;
