...
Update current_update(std::uint32_t value) {
    return value == cockpit ? update : Update{};
}
std::uint64_t generation(std::uint32_t value) {
    return value == cockpit ? update.generation : 0;
}
} // namespace chase_transition
...
static void AcquireSRWLockExclusive(SRWLOCK* value) {
    value->mutex.lock();
}
static void ReleaseSRWLockExclusive(SRWLOCK* value) {
    value->mutex.unlock();
}
...
static std::uint32_t GetCurrentThreadId() {
    return host_thread;
}
...
static bool QueryPerformanceCounter(LARGE_INTEGER* value) {
    value->QuadPart = static_cast<long long>(++host_qpc);
    return true;
}
bool timing = false;
enum Reason {
...
static core::Pending* ticket_for_frame(std::uint32_t frame) {
    for (auto& slot : pending)
        if (slot.frame == frame) return &slot.ticket;
    return nullptr;
}
...
struct World {
    ...
    World() { reset(); }
    void reset() {
        engine_memory::bytes.clear();
        engine_memory::unreadable.clear();
        writable_addresses.clear();
        ...
        hide_calls = 0;
        hidden_entry = 0;
        hide_native_wrong_node = false;
        enabled = true;
        hud_enabled = true;
        native_timing_enabled = false;
        hud_anchor_forward = false;
        ...
        for (const unsigned offset : {0x118u, 0x140u, 0x12cu}) engine_memory::put(overlay + offset, std::uint32_t(0));
        ...
        initial.screen[0] = 1280;
        initial.screen[1] = 720;
        ...
        allow_write(marker + 0x30, 8);
        allow_write(overlay + 0x340, 8);
        allow_write(overlay + 0x3c, 8);
...
static void geometry_and_late_fov() {
    ...
    core::Projection late = world.initial;
    late.position[0] = 80;
    late.fov = 0x1800;
    ...
    auto final_state = world.final_regs();
    final_fov(final_state.data());
    check(reasons[Finalized] == 1 && expected.x != early_x, "late camera position and FOV change recomputes lead");
...
static void native_passthrough() {
    ++scenarios;
    World world;
    auto regs = world.gate_regs(false);
    gate(regs.data());
...
static void nested_serial_and_skipped_publication() {
    ...
    auto skipped = world.publish_regs();
    skipped[4] = 0xdead;
...
static void ownership_and_scene_guards(bool wrong_marker, bool wrong_scene) {
    ...
    check(publication[0] == 88 && publication[1] == 77, "foreign ownership leaves native coordinates untouched");
}
...
static void owner_thread_callbacks() {
    ...
    auto refused = world.gate_regs();
    handle(0, refused.data());
...
static void projection_failure_hides(ProjectionFault fault) {
    ...
    auto publication = world.publish_regs();
    publish(publication.data());
    check(reasons[Projection] == 1 && pending_empty(), "invalid FOV, behind point, or overflow refuses publication");
    check(hide_calls == 1 && hidden_entry == World::overlay + 0x3c,
          "projection failure invokes native hide only for owned overlay");
    check(get<std::uint32_t>(World::overlay + 0x3c) == 0 && get<std::uint32_t>(World::marker + 0x1c) == 0,
          "native hide stub detaches marker scene link and active flag");
    check(get<std::int32_t>(World::overlay + 0x340) == -1 && publication[0] == UINT32_MAX &&
              publication[1] == UINT32_MAX,
...
static void inactive_entry_is_not_reshown() {
    ...
    World world;
    core::Identity owner{};
    check(scope(pose, owner) == Ready, "inactive fixture owner scope");
    hide(owner);
    check(hide_calls == 1 && get<std::int32_t>(World::overlay + 0x340) == -1, "first owned hide invalidates marker");
    hide(owner);
...
static void slot_capacity() {
    ...
    auto overflow = world.gate_regs();
    overflow[2] = 208;
    gate(overflow.data());
    const unsigned occupied = unsigned(
        std::count_if(std::begin(pending), std::end(pending), [](const Slot& slot) { return slot.frame != 0; }));
    check((overflow[8] & 0x40) && occupied == 8, "ninth concurrent frame preserves native gate at bounded capacity");
    check(reasons[Ready] == 8 && reasons[Lifetime] == 1, "slot overflow records one bounded lifetime refusal");
}
...
static void joint_fallback_planes() {
    ...
    auto gate_state = world.gate_regs();
    gate(gate_state.data());
    check(!(gate_state[8] & 0x40) && ticket_for_frame(World::frame), "joint fallback plane remains eligible at gate");
}

static void nested_frames_preserve_parent() {
    ++scenarios;
    World world;
    auto parent = world.gate_regs();
    gate(parent.data());
    pose.serial = ++chase_transition::update.serial;
    auto inner = world.gate_regs();
    inner[2] = World::frame + 1;
    gate(inner.data());
    auto inner_publication = world.publish_regs();
    inner_publication[2] = World::frame + 1;
    publish(inner_publication.data());
    auto final_state = world.final_regs();
    final_fov(final_state.data());
    check(reasons[Committed] == 1 && reasons[Skipped] == 1, "inner serial frame publishes and finalizes independently");
    ...
    auto parent_publication = world.publish_regs();
    publish(parent_publication.data());
    check(reasons[Lifetime] == 1 && hide_calls == 1 && pending_empty(),
...
static void fractional_pixels_truncate_toward_zero() {
    ...
    core::Pixel pixel{};
    check(core::project(world.initial, point, pixel), "fractional signed projection remains finite");
    // The unconverted coordinates are approximately +1.545 and -0.869.
    // Rounding would produce (+2,-1); native signed IDIV produces (+1,0).
    check(pixel.x == 1 && pixel.y == 0, "positive and negative fractional pixels truncate toward zero");
}
...
static void marker_destination_overflow_refuses() {
    ...
    check(!writable_at(marker, 0x30, 8), "marker destination offset rejects before wrapped writable address");
...
static void sample_retention_counts() {
    ...
    for (unsigned index = 0; index < 4; ++index) {
        sample.x = std::int32_t(index);
        note(Committed, &sample);
    }
    check(counts.samples == 4 && counts.dropped == 0, "four samples are retained without omission");
    sample.x = 4;
    note(Committed, &sample);
...
static void central_guard(HudGuard guard) {
    ...
    case HudGuard::WrongCockpit:
        ++regs[7];
        expected = NoPose;
        break;
    case HudGuard::WrongOverlay:
        ++regs[4];
        expected = NoPose;
        break;
    case HudGuard::WrongThread:
        host_thread = 2;
        expected = Lifetime;
        break;
    case HudGuard::Generation:
        ++chase_transition::update.generation;
        expected = Lifetime;
        break;
    case HudGuard::Serial:
        ++chase_transition::update.serial;
        expected = Lifetime;
        break;
    case HudGuard::InactiveCockpit:
        engine_memory::put(World::link + 8, std::uint32_t(0));
        expected = Identity;
        break;
    case HudGuard::Refs:
        engine_memory::put(World::cockpit + 0xc, std::uint32_t(0));
        expected = Identity;
        break;
    ...
    case HudGuard::Camera:
        engine_memory::put(World::cockpit + 0x58, std::uint32_t(0));
        expected = Identity;
        break;
    case HudGuard::ShipId:
        engine_memory::put(World::ship + 8, std::uint32_t(999));
        expected = Identity;
        break;
    ...
    case HudGuard::Back:
        engine_memory::put(World::overlay + 0x324, std::uint32_t(0));
        expected = Identity;
        break;
    case HudGuard::ReadFailure:
        engine_memory::unreadable.insert(World::cockpit + 0x234);
        expected = Read;
        break;
    }
    central_hud(regs.data());
...
static void central_native_cleanup_flags() {
    for (const unsigned offset : {0x118u, 0x140u, 0x12cu}) {
        ...
        auto first = world.central_regs(0x840);
        central_hud(first.data());
        ...
        auto next = world.central_regs(0x840);
        central_hud(next.data());
...
static void central_texture_guard(TextureGuard guard) {
    ...
    host_thread = 3;
    switch (guard) {
    case TextureGuard::CachedSurface: engine_memory::put(World::textures + 15 * 0x10 + 8, std::uint32_t(0)); break;
    case TextureGuard::SignedTotal:
        engine_memory::put(0x6069b0, std::int16_t(-1));
        engine_memory::put(0x6069b4, std::int16_t(16));
        break;
    case TextureGuard::RequiredMetadata:
        engine_memory::put(0x608dac, std::int16_t(16));
        engine_memory::put(World::texture_rows + 15 * 0x3c + 0xc, std::int16_t(0));
        break;
...
static void hud_anchor_projection_geometry() {
    ...
    for (const auto expected : {std::pair<double, std::int32_t>{0.5, -4}, {13, -118}, {5, -44}, {21, -196}}) {
        ...
        if (expected.first == 13) {
            ...
            core::Pixel returned{};
            check(core::project_direction(p, roundtrip, returned) && std::abs(returned.x - direction.x) <= 1 &&
                      std::abs(returned.y - direction.y) <= 1,
...
static void hud_anchor_uses_changed_final_fov() {
    ++scenarios;
    World world;
    prepare_forward_anchor(world);
    auto central = world.central_regs(0x840);
    central_hud(central.data());
    ...
    auto final = world.final_regs();
    final_fov(final.data());
...
static void interrupted_updates_reclaim_anchor_tickets() {
    ...
    for (unsigned i = 0; i < 8; ++i) {
        auto interrupted = world.central_regs(0x840);
        central_hud(interrupted.data());
        ++chase_transition::update.serial;
        invalidate_pose();
        camera_context(World::cockpit, World::ship, World::camera, true, &camera_basis, &view_rel);
    }
    check(occupied_hud_anchor_slots() == 0, "eight interrupted updates reclaim every revoked anchor ticket");
    auto recovery = world.central_regs(0x840);
    central_hud(recovery.data());
...
static void hud_anchor_group_write() {
    ...
    check(hud_anchor_counts.applied == 1 && hud_anchor_counts.refused == 0 && hud_anchor_counts.last_x == 0 &&
              hud_anchor_counts.last_y == -118,
...
static void native_end_precedes_hud_refusal() {
    ++scenarios;
    World world;
    host_thread = owner_thread = 3;
    native_timing_enabled = true;
    auto begin = world.gate_regs();
    native_hook(0, begin.data());
    engine_memory::put(World::overlay + 0x118, std::uint32_t(1));
    auto central = world.central_regs(0x840);
    central[2] = World::frame;
    handle(3, central.data());
    check(native_times.window.metrics[0].calls == 1, "common native lead span ends before central HUD policy");
    ...
    for (const auto& slot : native_times.slots) central_started |= slot.starts[2].stamp != 0;
...
static void native_hook_complete_sequence() {
    ...
    host_thread = owner_thread = 3;
    native_timing_enabled = true;
    auto lead = world.gate_regs();
    native_hook(0, lead.data());
    auto central = world.central_regs();
    central[2] = World::frame;
    native_hook(3, central.data());
    auto solver = world.central_regs();
    solver[2] = World::frame;
    native_hook(4, solver.data());
    solver[7] = 0;
    native_hook(5, solver.data());
    auto distance = world.gate_regs();
    native_hook(6, distance.data());
    native_hook(7, distance.data());
    ...
    check(native_times.window.metrics[1].failures == 1 && native_times.window.metrics[0].failures == 0 &&
              native_times.window.metrics[2].failures == 0 && native_times.window.metrics[3].failures == 0,
          "saved false solver EAX is the only phase failure");
    check(native_times.window.metrics[0].ticks == 1 && native_times.window.metrics[1].ticks == 1 &&
              native_times.window.metrics[2].ticks == 5 && native_times.window.metrics[3].ticks == 1,
...
int main() {
    ...
    for (auto guard : {HudGuard::Disabled,    HudGuard::NativePath, HudGuard::WrongCockpit, HudGuard::WrongOverlay,
                       HudGuard::WrongThread, HudGuard::Generation, HudGuard::Serial,       HudGuard::InactiveCockpit,
                       HudGuard::Refs,        HudGuard::Mode,       HudGuard::Connect,      HudGuard::AppliedFlags,
                       HudGuard::Gun,         HudGuard::Camera,     HudGuard::ShipId,       HudGuard::ShipType,
                       HudGuard::OwnerType,   HudGuard::Scene,      HudGuard::Draw2d,       HudGuard::Display,
                       HudGuard::Bodies,      HudGuard::Back,       HudGuard::ReadFailure})
        central_guard(guard);
    central_native_cleanup_flags();
    for (auto guard : {TextureGuard::CachedSurface, TextureGuard::SignedTotal, TextureGuard::RequiredMetadata,
                       TextureGuard::MissingRead})
