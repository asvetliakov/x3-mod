...
def validate(text, trace, work, case, no_map=False):
    ...
    for i, row in enumerate(rows):
        ...
        available = lane and not (i == 2 and (late or case in ('untracked', 'cutout_pair', 'shadow_apply', 'unregistered_fault', 'unregistered_mid') or failed_coverage)) and case != 'original_share_refused'
    ...
    for row in publications:
        ...
        assert int(row['stamp_refused']) == (3 if case in ('unregistered_fault', 'unregistered_mid') and i == 2 else 0), (case, row)
        assert int(row['stamped_prims']) == (3 if case == 'unregistered' and i == 2 else 0), (case, row)
    ...
    elif case in ('unregistered_fault', 'unregistered_mid'):
    ...
    try:
        for case in selected:
            ...
            env.update(X3M_MOTION_OUTPUT='1', X3M_TAA='1', X3M_HDR='1', X3M_HDR_TONEMAP='agx', X3M_HDR_DECODE='gamma2.2',
                       ...
                       X3M_SUN_SHADOW_LANE='1', X3M_FIXTURE_TAA_SENTINEL='1', X3M_TAA_SHARPEN='0', X3M_TAA_MIP_BIAS='0',
                       X3M_SCENE_HOOK='0', X3M_OWNERSHIP='0', X3M_TELEMETRY='1', X3M_MOTION_FRAME_LOG='1',
