...
static void renderAndRead(IDirect3DDevice9* device, Compiler compiler, D3DFORMAT format, unsigned generation) {
    const char* label = format == D3DFMT_A8R8G8B8 ? "RGBA8" : format == D3DFMT_R32F ? "R32F" : "RGBA16F";
    ...
    check("CreateTexture resolve destination",
          device->CreateTexture(size, size, 1, D3DUSAGE_DEPTHSTENCIL, destinationFormat, D3DPOOL_DEFAULT,
                                &depthTexture.p, nullptr));
    check("GetSurfaceLevel resolve destination", depthTexture->GetSurfaceLevel(0, &destinationDepth.p));
    check(
        "CreateDepthStencilSurface ordinary source",
        device->CreateDepthStencilSurface(size, size, sourceFormat, D3DMULTISAMPLE_NONE, 0, FALSE, &depth.p, nullptr));
    check("CreateTexture sentinel",
          device->CreateTexture(2, 2, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &sentinel.p, nullptr));
    ...
    const Vertex triangles[] = {vertex(4, 4, .25f), vertex(28, 4, .25f), vertex(4, 28, .25f), vertex(36, 4, .75f),
                                ...
                                vertex(4, 4, .875f), vertex(28, 4, .875f), vertex(4, 28, .875f), vertex(4, 36, .9999f),
                                vertex(28, 36, .9999f), vertex(4, 60, .9999f), vertex(36, 36, .99999f),
                                vertex(60, 36, .99999f), vertex(60, 60, .99999f)};
    check("DrawPrimitiveUP five depth triangles",
          device->DrawPrimitiveUP(D3DPT_TRIANGLELIST, 5, triangles, sizeof(Vertex)));
    ...
    DWORD originalPointBits = 0;
    std::memcpy(&originalPointBits, &pointSize, 4);
    ...
    D3DSURFACE_DESC observedDesc{};
    check("GetDesc original depth", observedDepth->GetDesc(&observedDesc));
    ...
    std::printf("STATE generation=%u output=%s original_depth_identity=%u original_depth_format=%u "
                ...
                generation, label, observedDepth.p == depth.p, unsigned(observedDesc.Format),
                observedTexture.p == sentinel.p, originalPointBits, observedPointBits, stateOK ? "PASS" : "FAIL");
    ...
    if (shadowProbe) {
        float ref[4] = {shadowReference, 0, 0, 0};
        check("Set shadow reference", device->SetPixelShaderConstantF(0, ref, 1));
    }
    ...
    for (const auto& point :
         {Point{8, 8, .25f, "near_with_far_rejected"}, Point{56, 8, .75f, "far"}, Point{32, 48, 1, "clear"},
          Point{24, 24, 1, "outside_near_triangle"}, Point{8, 40, .9999f, "far_precision_9999"},
        ...
        const float tolerance = format == D3DFMT_A8R8G8B8 ? 1.5f / 255 : format == D3DFMT_R32F ? 1.2e-7f : .001f;
        bool pass = true;
        ...
        std::printf("SAMPLE generation=%u output=%s name=%s xy=%u,%u expected=%.9f "
                    ...
                    generation, label, point.name, point.x, point.y, expectedSample, actual[0], actual[1], actual[2],
...
int main(int argc, char** argv) {
    ...
    RegisterClassA(&cls);
    HWND window = CreateWindowA(cls.lpszClassName, "X3 disposable depth resolve probe", WS_OVERLAPPEDWINDOW, 100, 100,
    ...
    try {
        ...
        if (argc < 2 || argc > 3)
            throw std::runtime_error(
                "usage: depth_resolve.exe <D3DX path> [D24X8_INTZ|D24S8_INTZ|D24X8_DF24|D24X8_D24X8|D24X8_SHADOW|D24S8_FOURCC|D24X8_SHADOW_NODUMMY]");
        const char* variant = argc == 3 ? argv[2] : "D24X8_INTZ";
        if (!std::strcmp(variant, "D24S8_INTZ"))
            sourceFormat = D3DFMT_D24S8;
        else if (!std::strcmp(variant, "D24X8_DF24"))
            destinationFormat = D3DFORMAT(MAKEFOURCC('D', 'F', '2', '4'));
        else if (!std::strcmp(variant, "D24X8_D24X8"))
            destinationFormat = D3DFMT_D24X8;
        else if (!std::strcmp(variant, "D24X8_SHADOW")) {
            destinationFormat = D3DFMT_D24X8;
            shadowProbe = true;
        } else if (!std::strcmp(variant, "D24S8_FOURCC")) {
            sourceFormat = D3DFMT_D24S8;
            resolveTrigger = MAKEFOURCC('R', 'E', 'S', 'Z');
        } else if (!std::strcmp(variant, "D24X8_SHADOW_NODUMMY")) {
            destinationFormat = D3DFMT_D24X8;
            shadowProbe = true;
            dummyDraw = false;
        } else if (std::strcmp(variant, "D24X8_INTZ"))
            throw std::runtime_error("unknown format variant");
        ...
        for (D3DFORMAT color : {D3DFMT_A8R8G8B8, D3DFMT_A16B16G16R16F, D3DFMT_R32F}) {
            ...
            check("CheckDepthStencilMatch original source",
                  api->CheckDepthStencilMatch(0, D3DDEVTYPE_HAL, mode.Format, color, sourceFormat));
        }
