...
static_assert(sites::placeholder_va - engine_page == 0x5c6,
              "the stub's placeholder bind sits at the engine's page offset");
...
bool vanilla(Branch b) {
    return b.lod0 == 1 && b.lod1 == 2;
}
bool patched_path(Branch b) {
    return b.lod0 == 1 && b.lod1 == 1;
}
...
bool page_is(const unsigned char* page, const unsigned char* reference, const unsigned char span[4]) {
    ...
    return !std::memcmp(now, reference, s) && !std::memcmp(now + s, span, 4) &&
           !std::memcmp(now + s + 4, reference + s + 4, page_size - s - 4);
}
...
void set_mode(const wchar_t* value) {
    SetEnvironmentVariableW(L"X3M_LOD_OCCLUSION", value);
}
...
std::string install_row(const char* status, const char* reason, const char* mode, const char* setting,
                        const char* write, bool defaulted = false) {
    char text[200];
    std::snprintf(
        text, sizeof text, "lod_occlusion site=%08lx status=%s reason=%s mode=%s setting=%s write=%s default=%u",
...
int main() {
    ...
    check(reinterpret_cast<std::uintptr_t>(&occl_placeholder_global) == placeholder_global_va &&
              occl_placeholder_global == 0x5a5a1234u,
          "placeholder_global_at_engine_va");
    if (reinterpret_cast<std::uintptr_t>(engine) != engine_page ||
        reinterpret_cast<std::uintptr_t>(&occl_placeholder_global) != placeholder_global_va) {
        std::printf("RESULT checks=%u failures=%u\n", checks, failures);
        return 1;
    }
    ...
    check(((reinterpret_cast<std::uintptr_t>(engine) + write_at) & 7u) == 1u &&
    ...
    const Refusal refusals[] = {
        {nullptr, true, "default_unset_off", unset_row.c_str(), nullptr},
    ...
    for (const Refusal& r : refusals) {
        ...
        check(!applied && !occl::patched() && last_log() == r.row && g.protects == 0 && g.writes == 0 &&
    ...
    check(!r && occl::state() == std::string("bytes_mismatch") && g.protects == 0 && g.writes == 0 &&
              ...
              last_log() == install_row("refused", "bytes_mismatch", "all", "all", "none"),
    ...
    arm();
    check(!initialize_checked("refuse_already_patched") && occl::state() == std::string("bytes_mismatch") &&
    ...
    check(!r && !occl::patched() && g.writes == 0 && g.flushes == 0 && page_is(engine, reference, original) &&
              ...
              last_log() == install_row("refused", "protect_failed", "all", "all", "none"),
    ...
    check(applied && occl::patched() && occl::state() == std::string("ok") &&
              occl::write_path() == std::string("atomic") &&
              last_log() == install_row("patched", "ok", "all", "all", "atomic"),
    ...
    check(span_is(engine, patched) && page_is(engine, reference, patched),
          "install_readback_00000000_rest_of_page_unchanged");
    ...
    arm();
    check(!occl::install_at(sites::window_va) && occl::patched() && occl::state() == std::string("already_installed") &&
              g.protects == 0 && g.writes == 0 && span_is(engine, patched),
          "second_install_refused_already_installed");
    check(initialize_checked("initialize_again") && occl::patched() && g.writes == 0,
    ...
    check(clean && !occl::patched() && occl::state() == std::string("restored") &&
              one_row(rows, "lod_occlusion_restore site=004c34f7 status=restored found=00000000 registered=0"),
          "restore_row", rows.c_str());
    check(g.protects == 2 && g.writes == 1 && g.atomic_writes == 1 && g.flushes == 1 && span_is(engine, original) &&
              page_is(engine, reference, original) && protection(engine) == PAGE_EXECUTE_READ,
          "restore_readback_c9000000_protection");
    ...
    check(!r && !occl::patched() && occl::state() == std::string("patch_rolled_back") &&
              last_log() == install_row("refused", "patch_rolled_back", "all", "all", "atomic") && g.writes == 2 &&
    ...
    f.write_drop = 1;
    arm(f);
    check(!initialize_checked("rollback_dropped_write") && !occl::patched() &&
              occl::state() == std::string("patch_rolled_back") && page_is(engine, reference, original) &&
              protection(engine) == PAGE_EXECUTE_READ,
    ...
    f.protect_fail = 2 | 4;
    arm(f);
    const bool unprotected = !initialize_checked("rollback_unprotected") && !occl::patched() &&
                             occl::state() == std::string("rollback_unprotected") &&
    ...
    check(!r && occl::patched() && occl::state() == std::string("rollback_failed") &&
              last_log() == install_row("patched_unverified", "rollback_failed", "all", "all", "atomic") &&
              span_is(engine, corrupt) && protection(engine) == PAGE_EXECUTE_READ,
    ...
    check(occl::patched() && occl::state() == std::string("restore_not_owned") &&
              one_row(rows, "status=restore_not_owned found=c8000000 registered=1") && g.writes == 0 &&
              g.protects == 0 && g.flushes == 0 && span_is(engine, corrupt) && protection(engine) == PAGE_EXECUTE_READ,
    ...
    check(occl::patched() && occl::state() == std::string("restore_failed") &&
              one_row(rows, "status=restore_failed found=00000000 registered=1") && span_is(engine, patched) &&
              protection(engine) == PAGE_EXECUTE_READ,
    ...
    check(occl::patched() && one_row(rows, "status=restore_failed found=00000000 registered=1") && g.writes == 0,
          "restore_protect_failed_registered", rows.c_str());
    arm();
    rows = shutdown_rows("restore_after_failures");
    check(!occl::patched() && one_row(rows, "status=restored found=00000000 registered=0") &&
              page_is(engine, reference, original) && protection(engine) == PAGE_EXECUTE_READ,
          "restore_after_failures_c9000000", rows.c_str());
    ...
    arm();
    check(initialize_checked("install_2") && poke(engine + write_at, original, 4),
          "setup_install_then_external_restore");
    arm();
    rows = shutdown_rows("restore_already_original");
    check(!occl::patched() && one_row(rows, "status=restored found=c9000000 registered=0") && g.writes == 0 &&
              g.protects == 0,
    ...
    check(occl::patched() && one_row(rows, "status=restore_not_owned found=-- registered=1") && g.writes == 0 &&
              g.protects == 0 && page_is(engine, reference, patched),
          "restore_unreadable_found_unread_no_write", rows.c_str());
    arm();
    rows = shutdown_rows("restore_after_unreadable");
    check(!occl::patched() && one_row(rows, "status=restored found=00000000 registered=0") &&
              page_is(engine, reference, original),
          "restore_after_unreadable_c9000000", rows.c_str());
    ...
    check(!occl::patched() && one_row(rows, "status=restored found=00000000 registered=0") && span_is(engine, original),
          "restore_no_flush_bytes_c9000000", rows.c_str());
    ...
    std::printf(
        "FLUSH patch_without_flush_lod1=%d (1 = new rel32 executed) restore_without_flush_lod1=%d (2 = old rel32 executed)\n",
    ...
    arm();
    check(occl::install_at(reinterpret_cast<std::uintptr_t>(priv + window_offset)) &&
              occl::write_path() == std::string("atomic") && span_is(priv, patched) &&
              page_is(priv, reference, patched),
    ...
    if (writer && roview) {
        ...
        if (!raised) {
            arm();
            check(!occl::install_at(reinterpret_cast<std::uintptr_t>(roview + window_offset)) && !occl::patched() &&
                      occl::state() == std::string("protect_failed") && occl::write_path() == std::string("none") &&
    ...
    check(!r && last_log() == install_row("refused", "late_claim", "all", "all", "none") && g.protects == 0 &&
              g.writes == 0 && page_is(engine, reference, original),
          "late_initialize_refused", last_log().c_str());
    arm();
    check(!occl::install_at(reinterpret_cast<std::uintptr_t>(priv + window_offset)) &&
              occl::state() == std::string("late_claim") && g.protects == 0 && page_is(priv, reference, original),
