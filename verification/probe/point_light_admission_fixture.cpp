...
static char frame_lines[8][512];
static unsigned frame_line_count = 0;
static char node_lines[80][512];
static unsigned node_line_count = 0;
namespace x3m {
void log(const char* format, ...) {
    ...
    va_end(a);
    if (!std::strncmp(text, "point_light_admission_frame ", 28)) {
        if (frame_line_count < 8) std::strcpy(frame_lines[frame_line_count++], text);
        return;
    }
    if (!std::strncmp(text, "point_light_node ", 17)) {
        if (node_line_count < 80) std::strcpy(node_lines[node_line_count++], text);
        return;
    }
...
std::uint32_t fixture_site = 0;
void fixture_run(Frame* frame);
}
static_assert(offsetof(Frame, out) == 32 && offsetof(Frame, entry_esp) == 68 && offsetof(Frame, exit_esp) == 72 &&
                  offsetof(Frame, locals) == 76 && offsetof(Frame, x87env) == 172,
              "frame layout");
asm(".text\n.globl _fixture_run\n_fixture_run:\n"
    "pushl %ebp\n movl %esp,%ebp\n pushl %ebx\n pushl %esi\n pushl %edi\n"
...
static std::uint32_t site_va() {
    return std::uint32_t(reinterpret_cast<std::uintptr_t>(code) + core::site_offset);
}
...
static bool site_bytes_are(const unsigned char* expected, unsigned n) {
    return !std::memcmp(code + core::site_offset, expected, n);
}
static bool window_untouched_around_site() {
    return !std::memcmp(code, core::expected_window, core::site_offset) &&
           !std::memcmp(code + core::site_offset + core::site_length,
                        core::expected_window + core::site_offset + core::site_length,
                        core::window_length - core::site_offset - core::site_length);
}
...
static void put(Node& n, unsigned off, std::uint32_t v) {
    std::memcpy(n.bytes + off, &v, 4);
}
static void node_set(Node& n, std::uint32_t parent, std::int32_t scale, std::int32_t x, std::int32_t y,
                     std::int32_t z) {
    std::memset(n.bytes, 0, sizeof n.bytes);
    put(n, core::parent_offset, parent);
    put(n, core::scale_offset, std::uint32_t(scale));
    put(n, core::position_offset, std::uint32_t(x));
    put(n, core::position_offset + 4, std::uint32_t(y));
    put(n, core::position_offset + 8, std::uint32_t(z));
}
static void light_set(Node& l, std::int32_t range, std::int32_t x, std::int32_t y, std::int32_t z,
                      std::uint32_t record) {
    node_set(l, 0, 0, x, y, z);
    put(l, core::range_offset, std::uint32_t(range));
    put(l, 0x16c, record);
}
constexpr std::uint32_t k_ebx = 0x0b0b0b0b, k_edi = 0x0d0d0d0d, k_ecx = 0xcccccccc, k_edx = 0xdddddddd,
                        k_record = 0x5ec0d0ad;
enum Path : std::uint32_t { admit = 1, reject = 2 };
struct Result {
    std::uint32_t path, eax, esi;
...
static Result run(std::int32_t distance, Node& node, Node& light, std::uint32_t flags = 0x246, bool new_frame = true) {
    ...
    f.eax = std::uint32_t(distance);
    f.ecx = k_ecx;
    f.edx = k_edx;
    f.ebx = k_ebx;
    f.ebp = addr(frame_block);
    f.esi = addr(&light);
    f.edi = k_edi;
    f.flags = flags;
    fixture_site = addr(code);
    fixture_run(&f);
    Result r{};
    r.path = f.out[6];
    r.eax = f.out[7];
    r.esi = f.out[1];
    bool locals_ok = true;
    for (std::uint32_t w : f.locals) locals_ok = locals_ok && w == 0xa5a5a5a5u;
    // PUSHAD stores the ESP value before PUSHAD, i.e. after the PUSHFD word: entry_esp - 96 (locals) - 4.
    r.preserved = f.out[0] == k_edi && f.out[2] == addr(frame_block) && f.out[4] == k_ebx &&
                  f.out[3] == f.entry_esp - 100 && f.exit_esp == f.entry_esp && locals_ok;
    r.x87_empty = ((f.x87env[1] >> 11) & 7) == 0 && (f.x87env[2] & 0xffff) == 0xffff; // TOP = 0, all tags empty
    if (!r.preserved || !r.x87_empty)
        std::printf(
            "DETAIL edi=%08lx ebp=%08lx ebx=%08lx esp=%08lx entry_esp=%08lx exit_esp=%08lx locals_ok=%u status=%04lx tag=%04lx\n",
            (unsigned long)f.out[0], (unsigned long)f.out[2], (unsigned long)f.out[4], (unsigned long)f.out[3],
            (unsigned long)f.entry_esp, (unsigned long)f.exit_esp, locals_ok ? 1u : 0u,
...
static void check_path(const Result& r, Path expected, Node& light, const char* label) {
    ...
    if (expected == admit)
        check(r.eax == addr(&light) && r.esi == k_record, "admit path: eax=light, esi=[light+0x16c]");
    else
        check(r.eax == 0xa5a5a5a5u && r.esi == addr(&light), "reject path: eax=[esp+0x5c], esi=light");
}
static bool same(const Result& a, const Result& b) {
    return a.path == b.path && a.eax == b.eax && a.esi == b.esi && a.preserved == b.preserved &&
           a.x87_empty == b.x87_empty;
}
...
static double bench_us(std::int32_t distance, Node& node, Node& light, bool new_frame = true) {
    ...
    for (unsigned i = 0; i < loops; ++i) run(distance, node, light, 0x246, new_frame);
...
int main() {
    ...
    check(!pla::initialize() && !std::strcmp(pla::state(), "disabled") &&
              site_bytes_are(core::expected_site, core::site_length),
    ...
    check(!pla::initialize() && !std::strcmp(pla::state(), "bytes_mismatch") && pla::detour_address() == 0,
    ...
    check(!pla::install_at(site_va()) && !std::strcmp(pla::state(), "bytes_mismatch"),
          "changed site byte: bytes_mismatch");
    code[core::site_offset + 1] ^= 1;
    unsigned char* reject_at = code + core::site_offset + core::site_length + core::site_rel32;
    reject_at[2] ^= 1;
    check(!pla::install_at(site_va()) && !std::strcmp(pla::state(), "bytes_mismatch") &&
              site_bytes_are(core::expected_site, core::site_length),
          "changed reject-target byte: bytes_mismatch, site untouched");
    reject_at[2] ^= 1;

    // install
    check(pla::install_at(site_va()) && !std::strcmp(pla::state(), "ok"), "install_at synthetic site");
    if (!pla::detour_address()) {
        std::printf("FAIL install state=%s\nPOINT LIGHT ADMISSION CPU checks=%u failures=%u\n", pla::state(), checks,
                    failures + 1);
        return 1;
    }
    unsigned char patched[core::site_length];
    core::encode_site_patch(site_va(), std::uint32_t(pla::detour_address()), patched);
    check(site_bytes_are(patched, core::site_length) && patched[0] == 0xe9 && patched[5] == 0x90 &&
              window_untouched_around_site(),
          "site bytes are jmp detour; nop and the rest of the window is untouched");
    check(!std::strcmp(pla::write_path(), "plain") || !std::strcmp(pla::write_path(), "atomic"), "write path reported");
    unsigned char detour_expected[core::detour_length];
    core::encode_detour(std::uint32_t(pla::detour_address()),
                        addr(reinterpret_cast<const void*>(&x3m_point_light_root_admits)), site_va() + 6,
                        site_va() + 6 + core::site_rel32, addr(const_cast<std::uint32_t*>(&x3m_point_light_fast_admit)),
                        detour_expected);
    check(!std::memcmp(reinterpret_cast<const void*>(pla::detour_address()), detour_expected, core::detour_length),
          "detour bytes as encoded");
    ...
    // chain depth: 7 ancestors resolve, 8 do not
    {
        ...
        for (unsigned depth = 1; depth <= 8; ++depth) {
            ...
            check_path(r, depth <= core::max_hops - 1 ? admit : reject, light,
                       depth <= core::max_hops - 1 ? "patched: chain within the hop bound admits"
                                                   : "patched: chain beyond the hop bound rejects");
        }
    }
    // cycles
    node_set(mid, addr(&node), 200, 5000, 0, 0);
    node_set(node, addr(&mid), 200, 1232, 0, 0);
    ...
    node_set(mid, addr(&mid), 200, 5000, 0, 0);
    node_set(node, addr(&mid), 200, 1232, 0, 0);
    ...
    check(GetLastError() == 0x5150, "LastError preserved across a failed validation");
    {
        ...
        auto* edge = static_cast<unsigned char*>(page) + 0x1000 - 0x40; // +0x18 readable, +0x70 and +0xb0 are on the
                                                                        // uncommitted page
    ...
    std::printf(
        "POINT LIGHT ADMISSION OUTCOMES admitted=%lu node_is_root=%lu chain_unreadable=%lu chain_too_deep=%lu chain_cycle=%lu root_unreadable=%lu light_unreadable=%lu reach_negative=%lu root_rejected=%lu\n",
        (unsigned long)s.outcomes[0], (unsigned long)s.outcomes[1], (unsigned long)s.outcomes[2],
        (unsigned long)s.outcomes[3], (unsigned long)s.outcomes[4], (unsigned long)s.outcomes[5],
        (unsigned long)s.outcomes[6], (unsigned long)s.outcomes[7], (unsigned long)s.outcomes[8]);
    check(s.outcomes[0] == 11 && s.outcomes[1] == 1 && s.outcomes[2] == 2 && s.outcomes[3] == 1 && s.outcomes[4] == 3 &&
              s.outcomes[5] == 1 && s.outcomes[6] == 0 && s.outcomes[7] == 1 && s.outcomes[8] == 3,
          "outcome counters");
    ...
    {
        Node& other = pool[13];
        node_set(other, addr(&root), 200, 1232, 0, 0);
        ...
        check_path(run(1300, node, light, 0x246, false), admit, light,
                   "memo: same frame, different distance, same verdict");
        ...
        now = pla::stats();
        check(now.walks == before.walks + 1 && now.memo_hits == before.memo_hits + 2, "memo: keyed by node");
        Node& light2 = pool[14];
        light_set(light2, 1000, 0, 0, 0, k_record);
        check_path(run(1232, node, light2, 0x246, false), admit, light2, "memo: another light in the same frame walks");
        now = pla::stats();
        check(now.walks == before.walks + 2, "memo: keyed by light");
        node_set(root, 0, 500, 81192, 0, 0); // the root moves out of range: the same frame still answers the memoised
                                             // admit
        ...
        now = pla::stats();
        check(now.walks == before.walks + 3 && now.memo_hits == before.memo_hits + 3, "memo: new frame walked");
        ...
        now = pla::stats();
        check(now.walks == before.walks + 4 && now.memo_hits == before.memo_hits + 4, "memo: counters");
    ...
    // frame telemetry: counters since the last present(), the frame line's sums, the capture-frame samples, the reset
    {
        ...
        check(frame_line_count == 1 && node_line_count == 0,
              "present logs one frame line, no samples on a plain frame");
        frame_line_count = 0;
        pla::Stats z = pla::stats();
        check(z.tests == 0 && z.fast_admit == 0 && z.walks == 0 && z.memo_hits == 0 && z.outcomes[0] == 0 &&
                  z.outcomes[8] == 0 && z.samples == 0,
              "present resets the counters");
        pla::begin_frame(true); // capture frame: sample the walked nodes
        Node& other = pool[13];
        node_set(other, addr(&root), 200, 1232, 0, 0);
        Node& lonely = pool[15];
        node_set(lonely, 0, 300, 5000, 0, 0);
        ...
        z = pla::stats();
        check(z.tests == 7 && z.fast_admit == 3 && z.reject == 4 && z.walks == 3 && z.memo_hits == 1 && z.samples == 3,
        ...
        unsigned long device = 0, frame = 0, tests = 0, fast = 0, rej = 0, walks = 0, hits = 0, ra = 0, rr = 0, cu = 0,
                      cd = 0, cc = 0, nr = 0, ru = 0, lu = 0, rn = 0, smp = 0;
        const int fields = std::sscanf(
            frame_lines[0],
            "point_light_admission_frame device=%lu frame=%lu tests=%lu fast_admit=%lu reject=%lu walks=%lu memo_hits=%lu root_admit=%lu root_reject=%lu chain_unreadable=%lu chain_too_deep=%lu chain_cycle=%lu node_is_root=%lu root_unreadable=%lu light_unreadable=%lu reach_negative=%lu samples=%lu",
            &device, &frame, &tests, &fast, &rej, &walks, &hits, &ra, &rr, &cu, &cd, &cc, &nr, &ru, &lu, &rn, &smp);
        check(fields == 17 && device == 7 && frame == 42, "frame line parses");
        check(tests == fast + rej && rej == walks + hits && walks == ra + rr + cu + cd + cc + nr + ru + lu + rn,
              "frame line sums");
        check(tests == 7 && fast == 3 && rej == 4 && walks == 3 && hits == 1 && ra == 2 && nr == 1 && rr == 0 &&
                  smp == 3,
              "frame line values");
        unsigned long n_node = 0, n_root = 0;
        unsigned depth = 0;
        long dist = 0, reach = 0, rdist = 0, rreach = 0, nscale = 0, rscale = 0;
        char verdict[32] = {};
        const int nf = std::sscanf(
            node_lines[0],
            "point_light_node device=%lu frame=%lu node=%lx root=%lx depth=%u dist=%ld reach=%ld root_dist=%ld root_reach=%ld verdict=%31s node_scale=%ld root_scale=%ld",
            &device, &frame, &n_node, &n_root, &depth, &dist, &reach, &rdist, &rreach, verdict, &nscale, &rscale);
        check(nf == 12 && n_node == addr(&node) && n_root == addr(&root) && depth == 1 && dist == 1232 &&
                  reach == 1200 && rdist == 3461 && rreach == 20536 && !std::strcmp(verdict, "root_admit") &&
                  nscale == 200 && rscale == 19536,
              "node line 1: the walked clamp with its root");
        const int nf3 = std::sscanf(
            node_lines[2],
            "point_light_node device=%lu frame=%lu node=%lx root=%lx depth=%u dist=%ld reach=%ld root_dist=%ld root_reach=%ld verdict=%31s node_scale=%ld root_scale=%ld",
            &device, &frame, &n_node, &n_root, &depth, &dist, &reach, &rdist, &rreach, verdict, &nscale, &rscale);
        check(nf3 == 12 && n_node == addr(&lonely) && n_root == 0 && depth == 0 && dist == 5000 && reach == 1300 &&
                  rdist == 0 && rreach == 0 && !std::strcmp(verdict, "node_is_root") && nscale == 300 && rscale == 0,
              "node line 3: a root node reports no root");
        ...
        for (unsigned i = 0; i < 70; ++i) {
            node_set(many[i], addr(&root), 200, 1232, 0, 0);
            run(1232, many[i], light, 0x246, false);
        }
        z = pla::stats();
        check(z.walks == 70 && z.samples == 64, "sample bounded at 64 of 70 walks");
        pla::present(7, 43, true);
        check(frame_line_count == 1 && node_line_count == 64, "64 node lines");
        frame_line_count = node_line_count = 0;
        pla::begin_frame(true);
        pla::present(7, 44, false); // capture flagged then not captured at present: nothing logged for nodes
    ...
    const double patched_admit = bench_us(900, node, light), patched_root = bench_us(1232, node, light),
                 patched_memo = bench_us(1232, node, light, false);
    {
    ...
    check(site_bytes_are(core::expected_site, core::site_length) &&
              !std::memcmp(code, core::expected_window, core::window_length),
          "rollback bytes exact");
    check(same(run(1232, node, light), native_reject) && same(run(900, node, light), native_admit),
          "after rollback the native decisions are back");
    ...
    std::printf(
        "POINT LIGHT ADMISSION BENCH native_admit_us=%.4f native_reject_us=%.4f patched_admit_us=%.4f patched_root_walk_us=%.4f patched_memo_hit_us=%.4f harness=fixture_run_included game_fps=unmeasured\n",
    ...
    check(pla::install_at(site_va()) && pla::shutdown() && site_bytes_are(core::expected_site, core::site_length),
          "re-install and restore");
    x3m::engine_patch::close_install_window("fixture");
    check(!pla::install_at(site_va()) && !std::strcmp(pla::state(), "late_claim") &&
              site_bytes_are(core::expected_site, core::site_length),
