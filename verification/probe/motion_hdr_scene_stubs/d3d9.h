...
constexpr HRESULT S_OK = 0, S_FALSE = 1, E_FAIL = -1, E_ABORT = -2, E_NOINTERFACE = -3, D3DERR_DEVICELOST = -4,
                  D3DERR_DEVICENOTRESET = -5, D3DERR_NOTAVAILABLE = -6, D3DERR_INVALIDCALL = -7, D3DERR_NOTFOUND = -8;
...
enum D3DCULL {
    ...
    D3DCULL_CW = 2,
    D3DCULL_CCW = 3
}; // documented D3D9 values; the replay records default to no culling
...
struct IDirect3DSurface9 {
    ...
    HRESULT GetContainer(int, void** out) {
        ++containers;
        *out = nullptr;
        if ((SUCCEEDED(container_hr) || failure_output) && texture) {
            texture->AddRef();
            *out = texture;
        }
        return container_hr;
