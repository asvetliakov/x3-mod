...
disassembled and audited. Sites 6, 7 and 8: `inc dword [abs]`, the displaced
`mov eax,[hits]` (site 7) or `sub esp,0x34; push ebx; push edi` (site 8) and
one `jmp`, nothing else. Site 5: the exact
...
def audit_stubs():
    ...
    n8 = mnemonics(sites.encode_n8_stub(0x10000400, 0x20000018, sites.N8_NEXT))
    ...
    if any(m.startswith('f') or m == 'emms' or re.search(r'\b(mm|st)\d?\b', o) for m, o in n5 + n6 + n7 + n8):
    ...
    if n8 != [('inc', n8[0][1]), ('sub', 'esp,0x34'), ('push', 'ebx'), ('push', 'edi'), ('jmp', n8[4][1])] or len(n8) != 5:
        raise RuntimeError(f'site-8 stub: unexpected instructions {n8}')
    return {'n5': {'instructions': len(n5), 'sequence_exact': True, 'bracketed_handler_calls': 2}, 'n6': [m for m, _ in n6], 'n7': [m for m, _ in n7], 'n8': [m for m, _ in n8], 'no_x87_mmx': True}
