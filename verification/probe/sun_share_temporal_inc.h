...
struct SunCopyGuard {
    ...
    static HRESULT WINAPI copy(IDirect3DDevice9* d,IDirect3DSurface9* a,const RECT* ar,IDirect3DSurface9* b,const RECT* br,D3DTEXTUREFILTERTYPE filter){
        D3DSURFACE_DESC x{},y{};if(a&&b&&SUCCEEDED(a->GetDesc(&x))&&SUCCEEDED(b->GetDesc(&y))&&x.Format==D3DFMT_G32R32F&&y.Format==D3DFMT_R32F){++cross_format;return D3DERR_INVALIDCALL;}
...
std::vector<unsigned char> sun_read(IDirect3DDevice9* d,IDirect3DTexture9* input){
    D3DSURFACE_DESC desc{};check("sun read desc",input->GetLevelDesc(0,&desc));const unsigned stride=desc.Format==D3DFMT_R32F?4:8;
    ...
    for(unsigned generation=0;generation<2;++generation){
        {
            ...
            for(const UINT width:{W,W/2}){
                ...
                check("sun enhanced depth",d->CreateTexture(width,H,1,D3DUSAGE_RENDERTARGET,D3DFMT_G32R32F,D3DPOOL_DEFAULT,&enhanced.p,nullptr));
                ...
                check("sun enhanced staging",d->CreateOffscreenPlainSurface(width,H,D3DFMT_G32R32F,D3DPOOL_SYSTEMMEM,&enhanced_upload.p,nullptr));
                ...
                for(UINT y=0;y<H;++y)for(UINT x=0;x<width;++x){const float r=depths[(x+y)%6],g=shares[(2*x+y)%6];expected[y*width+x]=r;
                    std::memcpy(static_cast<char*>(a.pBits)+y*a.Pitch+x*4,&r,4);const float rg[]={r,g};std::memcpy(static_cast<char*>(b.pBits)+y*b.Pitch+x*8,rg,8);
                ...
                auto run=[&](TemporalPass& pass,FrameInputs in,bool enhanced_input,unsigned fail_draw=0){
                    ...
                    {SunCopyGuard guard(d);Fault fault(d,fail_draw);hr=pass.run(in,&out);
                     require(SunCopyGuard::cross_format==0,"no G32R32F to R32F StretchRect");
