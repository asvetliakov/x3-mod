...
static own::CopyDepthView copy_view(IDirect3DDevice9* device) {
    own::CopyDepthView view{};ok("copy view",own::get_copy_depth_view(device,&view));return view;
}
...
static void exercise_copy(IDirect3D9* api,HWND window,Compiler compiler,DWORD flags) {
    ...
    Com<IDirect3DDevice9> app;ok("create ordinary auto-depth device",api->CreateDevice(0,D3DDEVTYPE_HAL,window,flags,&pp,&app.p));
    auto* native=own::borrowed_native_device(app.p);expect("native seam",native!=nullptr);
    ...
    for(unsigned generation=0;generation<2;++generation) {
        ...
        expect("copy storage ready with no stale content",view.available&&view.texture&&view.source_bound&&!view.copy_valid);
        expect("copy generation advances",view.generation>previous_generation);previous_generation=view.generation;
        ...
        D3DSURFACE_DESC desc{},physical_desc{};ok("logical descriptor",logical->GetDesc(&desc));ok("native descriptor",physical->GetDesc(&physical_desc));
        expect("native depth remains original D24X8",desc.Format==D3DFMT_D24X8&&physical_desc.Format==D3DFMT_D24X8&&desc.Width==pp.BackBufferWidth);
        ok("other depth allocation",app->CreateDepthStencilSurface(desc.Width,desc.Height,D3DFMT_D24X8,D3DMULTISAMPLE_NONE,0,TRUE,&other.p,nullptr));
        ...
        ok("rebind original",app->SetDepthStencilSurface(logical.p));other.reset();
        ...
        draw_scene(app.p);
        Com<IDirect3DTexture9> sentinel;ok("sentinel texture",app->CreateTexture(4,4,1,0,D3DFMT_A8R8G8B8,D3DPOOL_MANAGED,&sentinel.p,nullptr));
        ok("bind sentinel texture",app->SetTexture(0,sentinel.p));
        float point=4.25f;DWORD point_bits;std::memcpy(&point_bits,&point,4);ok("sentinel pointsize",app->SetRenderState(D3DRS_POINTSIZE,point_bits));
        ...
        Com<IDirect3DStateBlock9> block;ok("end application stateblock",app->EndStateBlock(&block.p));block.reset();
        ok("copy original depth outside scene",own::copy_auto_depth(app.p));
        view=copy_view(app.p);expect("valid copy epoch",view.copy_valid&&view.copy_epoch==view.source_epoch);
        Com<IDirect3DBaseTexture9> restored;ok("restored sampler getter",app->GetTexture(0,&restored.p));
        DWORD restored_point=0;ok("restored pointsize getter",app->GetRenderState(D3DRS_POINTSIZE,&restored_point));
        expect("copy restores exact sampler and pointsize",restored.p==sentinel.p&&restored_point==point_bits);restored.reset();
        ok("copy keeps source bound",app->GetDepthStencilSurface(&again.p));expect("source logical identity unchanged",again.p==logical.p);again.reset();
        ...
        physical_after.reset();physical.reset();
        const auto copied_epoch=view.copy_epoch;
        ok("overlay destroys original depth",app->Clear(0,nullptr,D3DCLEAR_ZBUFFER,0,1,0));
        auto after=copy_view(app.p);expect("saved copy survives source clear",after.copy_valid&&after.copy_epoch==copied_epoch&&after.source_epoch==copied_epoch+1);
        {
            Snapshot saved(native,desc.Width,desc.Height);
            saved.capture(native,view.texture,compiler);saved.verify(native,"preserved-original-scene-depth",false);
            ...
            saved.capture(native,view.texture,compiler);saved.verify(native,"fresh-copy-after-source-clear",true);
        }
        ok("unbind sentinel",app->SetTexture(0,nullptr));sentinel.reset();
        if(generation==0) {
            ...
            auto lost=copy_view(app.p);expect("failed Reset retires copied storage",!lost.available&&!lost.copy_valid&&!lost.texture);
            logical.reset();pp.BackBufferWidth=80;pp.BackBufferHeight=48;ok("copy reset retry",app->Reset(&pp));
        }
    }
    expect("copy device reaches final release",app.p->Release()==0);app.p=nullptr;
}
int main(int argc,char** argv) {
    ...
    try {
        ...
        std::ifstream shader(argv[2]);std::ostringstream contents;contents<<shader.rdbuf();decode_source=contents.str();expect("production depth decoder loaded",!decode_source.empty());
        WNDCLASSA wc{};wc.lpfnWndProc=DefWindowProcA;wc.hInstance=GetModuleHandleA(nullptr);wc.lpszClassName="X3CopiedDepthFixture";
        RegisterClassA(&wc);window=CreateWindowA(wc.lpszClassName,"X3 copied depth fixture",WS_OVERLAPPEDWINDOW,0,0,128,128,nullptr,nullptr,wc.hInstance,nullptr);
        ...
        auto* backend=create(D3D_SDK_VERSION);expect("native factory",backend!=nullptr);
        Com<IDirect3D9> api;own::Options options;options.capture_auto_depth=true;
        HRESULT hr=own::wrap_factory(backend,&api.p,options);if(FAILED(hr))backend->Release();ok("wrap factory copy enabled",hr);
