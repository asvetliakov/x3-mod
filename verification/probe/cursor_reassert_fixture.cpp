...
struct Recorder {
    ...
    void* arrow() {
        order += 'L';
        return reinterpret_cast<void*>(0xa0);
    }
    void* set_cursor(void* h) {
        order += 'S';
        void* p = handle;
        handle = h;
        return p;
...
static cr::core::Gates all_pass() {
    cr::core::Gates g;
    g.same_thread = g.foreground = g.visible = g.cursor_hidden = g.pointer_in_client = true;
    return g;
}
struct CursorState {
    BOOL ok;
...
static CursorState cursor_state() {
    ...
    ci.cbSize = sizeof ci;
    CursorState s{};
    s.ok = GetCursorInfo(&ci);
    s.flags = ci.flags;
    s.cursor = ci.hCursor;
    GetCursorPos(&s.pos);
    return s;
}
static bool same(const CursorState& a, const CursorState& b) {
    return a.ok && b.ok && a.flags == b.flags && a.cursor == b.cursor && a.pos.x == b.pos.x && a.pos.y == b.pos.y;
}
// The thread's show count, read with a balanced pair (+1 then -1).
static int show_count() {
    const int up = ShowCursor(TRUE);
    ShowCursor(FALSE);
    return up - 1;
}
static bool ascending(const CursorEvent* e, unsigned n, std::uint32_t last) {
    for (unsigned i = 0; i < n; ++i)
        if (e[i].seq != last - (n - 1) + i) return false;
    return true;
}
static void cursor_ring_cases() {
    light::initialize();
    light::set_original(static_cast<unsigned>(x3m::loading_trace::Operation::CursorSet),
                        reinterpret_cast<PVOID>(&SetCursor));
    light::set_original(static_cast<unsigned>(x3m::loading_trace::Operation::CursorPosition),
                        reinterpret_cast<PVOID>(&SetCursorPos));
    ...
    for (int i = 0; i < 10; ++i) {
        light::cursor_set(i & 1 ? nullptr : arrow);
        if (i == 4) {
    ...
    for (unsigned i = 0; i < n; ++i)
        alternating = alternating && events[i].op == 0 &&
                      events[i].a == (i & 1 ? 0u : std::uint32_t(reinterpret_cast<uintptr_t>(arrow)));
    std::printf("DRAIN case=ten n=%u dropped=%u set=%u newest=%u\n", n, counts.dropped, counts.set, newest);
    require("drain_ten_in_order", n == 10 && counts.dropped == 0 && counts.set == 13 && newest == seen + 10 &&
                                      ascending(events, n, newest) && alternating);
    seen = newest;
    // One position event at the current pointer position (no visible warp), then an identical repeat.
    POINT p{};
    GetCursorPos(&p);
    const BOOL moved = light::cursor_position(p.x, p.y);
    light::cursor_position(p.x, p.y);
    n = light::cursor_drain(seen, events, 64, &newest, &counts);
    require("drain_position_event", moved && n == 1 && counts.pos == 2 && counts.set == 0 && events[0].op == 1 &&
                                        int(events[0].a) == p.x && int(events[0].b) == p.y && events[0].result == 1 &&
                                        events[0].seq == newest);
    seen = newest;
    // 100 changes between two drains: the newest 64 in order, 36 dropped.
    for (int i = 0; i < 100; ++i)
        light::cursor_set(i & 1 ? nullptr : arrow); // the previous step left NULL: 100 changes, ending on NULL
    n = light::cursor_drain(seen, events, 64, &newest, &counts);
    std::printf("DRAIN case=past_capacity n=%u dropped=%u set=%u first=%u newest=%u\n", n, counts.dropped, counts.set,
                n ? events[0].seq : 0, newest);
    require("drain_past_capacity_dropped", n == 64 && counts.dropped == 36 && counts.set == 100 &&
                                               newest == seen + 100 && ascending(events, n, newest));
    seen = newest;
    SetLastError(0x2468);
    light::cursor_set(arrow);
...
int main() {
    ...
    // 1. Sequence order and counts with the recording stand-in.
    {
        ...
        require("sequence_order", r.order == "LS+-S" && s.previous == nullptr && r.handle == nullptr &&
                                      s.arrow == reinterpret_cast<void*>(0xa0));
        require("sequence_counts_hidden",
                s.up == 0 && s.down == -1 && r.count == -1 && cr::core::expected_counts(s.up, s.down));
        Recorder z{"", 0, reinterpret_cast<void*>(0x55)};
        const auto t = cr::core::run_sequence(z);
        require("sequence_counts_shown", t.up == 1 && t.down == 0 && z.count == 0 &&
                                             z.handle == reinterpret_cast<void*>(0x55) &&
                                             cr::core::expected_counts(t.up, t.down));
        Recorder d{"", -2, nullptr};
        const auto u = cr::core::run_sequence(d);
        require("sequence_counts_deep_rejected",
                u.up == -1 && u.down == -2 && !cr::core::expected_counts(u.up, u.down));
    }
    cursor_ring_cases();
    ...
    wc.lpszClassName = "X3MCursorFixture";
    RegisterClassA(&wc);
    HWND hwnd = CreateWindowExA(0, wc.lpszClassName, wc.lpszClassName, WS_POPUP, 0, 0, 320, 200, nullptr, nullptr,
    ...
    // 2. Hooks.
    wt::attach(hwnd, 1);
    require("hooks_installed", wt::observed().installed &&
                                   logged(0, "window_trace_hooks",
                                          {"installed=1", "trace=1", "reassert=1", "call=1 ret=1 get=1", "pinned=1"}));
    {
        const size_t before_second = x3m::lines.size();
        wt::attach(hwnd, 2);
        require("second_device_logged_already_hooked",
                wt::observed().installed &&
                    logged(before_second, "window_trace_hooks",
                           {"device=2", "installed=0", "reason=already_hooked", "hooked_device=1"}));
    }
    ...
    require("callwndprocret_sees_result", magic == kMagic && seen.rets == rets0 + 1 && seen.calls == calls0 + 1 &&
                                              seen.last_message == kProbe && seen.last_result == kMagic);
    ...
    require("launch_armed_at_attach",
            cr::machine().arms == 1 && cr::machine().state == cr::core::State::armed &&
                cr::machine().armed_by == cr::core::arm_launch_source &&
                logged(0, "window_trace_hooks", {"device=1", "installed=1", "launch_arm=1"}) &&
    ...
    require("first_present_flush_and_snapshot",
            logged(before_first, "window_trace_flush", {"reason=first_present"}) &&
                logged(before_first, "window_msg ", {"name=WM_SETCURSOR", "hit=2"}) &&
                logged(before_first, "cursor_snapshot", {"burst=0"}) &&
                logged(before_first, "cursor_reassert_arm", {"armed_by=launch", "message=none", "arms=1"}) &&
                cr::machine().state == cr::core::State::armed);
    cr::core::Gates waiting = all_pass();
    waiting.foreground = false;
    {
        const size_t before = x3m::lines.size();
        unsigned waits = 0;
        cr::core::Step last = cr::core::Step::none;
        for (unsigned i = 0; i < cr::core::window_frames && cr::machine().state == cr::core::State::armed; ++i) {
            last = cr::present_with(hwnd, ++frame, waiting).step;
            waits += last == cr::core::Step::wait;
        }
        require(
            "launch_not_foreground_refused",
            last == cr::core::Step::refuse && waits == cr::core::window_frames - 2 && cr::machine().fires == 0 &&
        ...
        require("launch_fires_once_balanced",
                fired.step == cr::core::Step::fire && cr::machine().arms == 2 && cr::machine().fires == 1 &&
                    ...
                    logged(before_fire, "cursor_reassert frame=",
                           {"action=fired", "armed_by=launch", " up=0 down=-1 ", "balanced=1", "disabled=0",
                            "message=none"}));
        require("launch_no_second_fire",
                cr::present_with(hwnd, ++frame, all_pass()).step == cr::core::Step::none && cr::machine().fires == 1);
    }
    ...
    require("activate_replaces_pending_launch_arm",
            left_launch == cr::core::window_frames - 1 && cr::machine().arms == arms_base + 2 &&
                cr::machine().state == cr::core::State::armed && cr::machine().armed_by == WM_ACTIVATE &&
                cr::machine().frames_left == cr::core::window_frames);
    SendMessageA(hwnd, WM_ACTIVATEAPP, TRUE, 0);
    require("activate_arms_once", cr::machine().arms == arms_base + 2 &&
                                      cr::machine().state == cr::core::State::armed &&
                                      cr::machine().armed_by == WM_ACTIVATE);
    ...
    bool waited = true;
    for (int i = 0; i < 5; ++i)
        waited = waited && cr::present_with(hwnd, ++frame, waiting).step == cr::core::Step::wait;
    require("waits_while_gate_fails",
            waited && cr::machine().fires == 1 && cr::machine().state == cr::core::State::armed &&
    ...
    const CursorState end = cursor_state();
    require("fires_once_balanced", fired.step == cr::core::Step::fire && cr::machine().fires == 2 &&
                                       cr::machine().state == cr::core::State::idle &&
                                       logged(before_fire, "cursor_reassert frame=",
                                              {"action=fired", "armed_by=activate", " up=0 down=-1 ", "balanced=1",
                                               "disabled=0", "message=WM_ACTIVATE"}));
    require("win32_end_state_equals_start", same(start, end) && fire_error == 0x1357);
    require("no_second_fire",
            cr::present_with(hwnd, ++frame, all_pass()).step == cr::core::Step::none && cr::machine().fires == 2);
    std::printf("CURSOR start_flags=%lu start_cursor=%p end_flags=%lu end_cursor=%p x=%ld y=%ld\n",
                static_cast<unsigned long>(start.flags), static_cast<void*>(start.cursor),
                static_cast<unsigned long>(end.flags), static_cast<void*>(end.cursor), start.pos.x, start.pos.y);
    ...
    unsigned waits = 0;
    cr::core::Step last = cr::core::Step::none;
    for (unsigned i = 0; i < cr::core::window_frames; ++i) {
        last = cr::present_with(hwnd, ++frame, waiting).step;
        waits += last == cr::core::Step::wait;
    }
    require("refuses_after_window", cr::machine().arms == arms_base + 3 && waits == cr::core::window_frames - 1 &&
                                        last == cr::core::Step::refuse && cr::machine().refusals == 2 &&
                                        cr::machine().state == cr::core::State::idle &&
                                        logged(before_refuse, "cursor_reassert frame=",
                                               {"action=refused", "reason=foreground", "armed_by=activate"}));
    // 8. The real gate collection with this window shown under the pointer and brought to the foreground.
    {
        ...
        CURSORINFO ci{};
        ci.cbSize = sizeof ci;
        const BOOL ci_ok = GetCursorInfo(&ci);
        ...
        std::printf(
            "REAL_GATES same_thread=%d foreground=%d visible=%d cursor_ok=%d cursor_flags=%lu show_count=%d cursor_hidden=%d pointer_in_client=%d clip_is_client=%d "
            "armed_by_show=%u step=%s state=%s\n",
            int(gates.same_thread), int(gates.foreground), int(gates.visible), int(ci_ok),
            static_cast<unsigned long>(ci.flags), count, int(gates.cursor_hidden), int(gates.pointer_in_client),
            int(gates.clip_is_client), cr::machine().arms - arms_before, fired ? "fired" : "not_fired",
            cr::machine().state == cr::core::State::armed  ? "armed"
            : cr::machine().state == cr::core::State::idle ? "idle"
                                                           : "disabled");
        require("real_gates_collected", gates.same_thread && count == -1);
        require("real_present_end_state_unchanged",
                same(before, after) && count_after == count && cr::machine().state != cr::core::State::disabled);
    ...
    require("mismatch_disables",
            deeper == -2 && mismatch.step == cr::core::Step::fire && cr::machine().state == cr::core::State::disabled &&
                !cr::enabled() &&
    ...
    SendMessageA(hwnd, WM_ACTIVATE, WA_ACTIVE, 0);
    require("disabled_never_rearms", arms_disabled <= arms_before_mismatch + 1 && cr::machine().arms == arms_disabled &&
                                         cr::machine().state == cr::core::State::disabled);
    ShowCursor(TRUE);
    ShowCursor(TRUE); // back to 0
    ...
    require("flush_on_transition", logged(before_flush, "window_trace_flush", {"reason=transition"}) &&
                                       logged(before_flush, "window_msg ", {"name=WM_ACTIVATE", "result=00000000"}) &&
                                       logged(before_flush, "window_msg ", {"name=WM_ACTIVATEAPP"}) &&
                                       logged(before_flush, "cursor_snapshot", {"burst=0"}));
    for (int i = 0; i < 3; ++i) PostMessageA(hwnd, WM_MOUSEMOVE, 0, MAKELPARAM(10 + i, 10));
    pump();
    for (int i = 0; i < 5; ++i)
        SendMessageA(hwnd, WM_SETCURSOR, reinterpret_cast<WPARAM>(hwnd), MAKELPARAM(HTCLIENT, WM_MOUSEMOVE));
    ...
    require("setcursor_summary_and_frame_counts",
            logged(before_second, "window_msg ", {"name=WM_SETCURSOR", "hit=1", "trigger=0200", "result=00000001"}) &&
                logged(before_second, "window_msg_frame", {"mousemove=3", "setcursor=5"}) &&
                logged(before_second, "window_msg ", {"name=WM_SIZE"}) &&
