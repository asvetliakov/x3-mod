#!/usr/bin/env python3
"""Build and audit the point-light root-admission CPU fixture. Never runs Wine.
...
BUILD = ROOT / 'build/verification/point-light-admission'
...
def audit_module(path):
    ...
    if '_x3m_point_light_root_admits' not in listing:
        raise RuntimeError('handler symbol missing')
    return {'functions': functions, 'no_x87_mmx_xmm': True, 'no_exception_runtime_symbols': True}


def build():
    BUILD.mkdir(parents=True, exist_ok=True)
    objects = []
    for source, stem, extra in [('verification/probe/point_light_admission_fixture.cpp', 'fixture', []),
                                ('src/proxy/point_light_admission.cpp', 'module', NO_SSE),
    ...
    exe = BUILD / 'point_light_admission_fixture.exe'
