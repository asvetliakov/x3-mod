...
void native_timing_invalidate(std::uintptr_t cockpit, std::uint32_t thread) noexcept {
    native.revoke(std::uint32_t(cockpit), thread);
}
}
static bool pending(std::uint32_t thread) {
    for (const auto& slot : x3m::chase_lead::native.slots)
        if (slot.thread == thread)
            for (const auto& start : slot.starts)
                if (start.stamp) return true;
    return false;
}
...
static void setup() {
    ...
    for (unsigned i = 0; i < 3; ++i) {
        auto g = state.construct(cockpits[i], threads[i]);
        state.complete(cockpits[i], threads[i]);
...
int main() {
    ...
    for (unsigned kind : {0u, 2u}) {
        setup();
        read_ok = false;
        auto previous = state.generation(own_cockpit);
        handle(kind, regs);
        check(argument_reads == 1, "failed argument read attempted once");
        check(!pending(owner_thread) && pending(other_thread) && pending(unrelated_thread),
              "failed read revokes only callback thread");
        ...
        check(!pending(owner_thread) && !pending(other_thread) && pending(unrelated_thread),
              "decoded cockpit revokes other thread without touching unrelated owner");
    ...
    for (unsigned kind : {3u, 4u, 5u}) {
        ...
        read_ok = false;
        handle(kind, regs);
        check(!pending(owner_thread) && pending(other_thread),
              "update entry and both exits retire only current thread spans");
    }
