...
int main() {
    ...
    for (const auto& size : sizes)
        for (unsigned dense = 0; dense != 2; ++dense) {
            ...
            auto run = [&] {
                return meter_statistics(means.data(), maxima.data(), weights.data(), count, scratch.data(), params);
            };
            const MeterStatistics expected = run();
            if (expected.tiles != count || expected.lit != expected_lit || expected.neutral ||
                !std::isfinite(expected.lit_median_log) || !std::isfinite(expected.p99_max_log))
                return 2;
            float witness = 0;
            ...
            for (unsigned batch = 0; batch != batches; ++batch) {
                ...
                for (unsigned i = 0; i != repetitions; ++i) {
                    ...
                    if (result.lit != expected_lit || result.lit_median_log != expected.lit_median_log ||
                        result.p99_max_log != expected.p99_max_log)
                        return 3;
            ...
            std::printf(
                "CASE width=%u height=%u dense=%u tiles=%u lit=%u batches=%u repetitions=%u warmup=%u median_us=%.6f p95_us=%.6f min_us=%.6f witness=%.6f\n",
                size[0], size[1], dense, count, expected_lit, batches, repetitions, warmup, samples[batches / 2],
                samples[(batches * 95) / 100], samples.front(), double(witness));
        }
