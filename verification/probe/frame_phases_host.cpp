...
static bool scripted_frame(std::uint64_t frame, const std::uint64_t* gap, unsigned views, std::uint64_t present, Sample& out) {
    for (unsigned k = 0; k < core_sites; ++k) {
        at(gap[k]); tracker.site(k, clock_ticks);
        if (k == 3) for (unsigned v = 0; v < views; ++v) {
            at(1); tracker.site(7, clock_ticks); at(3); tracker.site(8, clock_ticks); at(7); tracker.site(9, clock_ticks);
        }
    }
    at(gap[core_sites]); tracker.present_begin(clock_ticks);
    at(present); tracker.present_end(clock_ticks);
...
int main() {
    ...
    tracker.site(0, clock_ticks); tracker.site(7, clock_ticks);
    ...
    std::uint64_t sum = 0; for (unsigned i = 0; i < phase_count; ++i) sum += s.phase_us[i];
    ...
    at(5); tracker.site(0, clock_ticks); at(6); tracker.site(1, clock_ticks); at(7); tracker.site(2, clock_ticks);
    at(80); tracker.site(5, clock_ticks); at(9); tracker.site(6, clock_ticks);
    at(1); tracker.present_begin(clock_ticks); at(2); tracker.present_end(clock_ticks);
    ...
    at(5); tracker.site(0, clock_ticks); at(5); tracker.site(3, clock_ticks); at(5); tracker.site(2, clock_ticks);
    check(tracker.order_errors == 1 && tracker.dropped == 1 && !tracker.live);
    at(5); tracker.site(6, clock_ticks); // unmatched while dropped
    check(tracker.unmatched == 3);
    tracker.present_begin(clock_ticks); tracker.present_end(clock_ticks);
    check(!tracker.take(3, 1000000, s) && tracker.live);
    at(5); tracker.site(0, clock_ticks); at(5); tracker.site(0, clock_ticks);
    ...
    tracker.present_begin(clock_ticks); tracker.present_begin(clock_ticks);
    check(tracker.order_errors == 3 && tracker.dropped == 3);
    tracker.present_end(clock_ticks); check(tracker.live);
    ...
    at(1); tracker.site(9, clock_ticks); at(1); tracker.site(8, clock_ticks);
    ...
    for (unsigned i = 0; i < window_frames; ++i) {
        Sample f{};
        f.frame = i; f.complete = i % 3 != 0; f.views = 3;
        for (unsigned p = 0; p < phase_count; ++p) { f.phase_us[p] = (i + 1) * (p + 1); f.dt_us += f.phase_us[p]; }
        f.view_setup_us = i; f.view_submit_us = 2 * i;
        reduction.add(f);
    }
    ...
    for (unsigned i = 0; i < 5; ++i) { Sample f{}; f.frame = i; f.dt_us = (i * 7) % 5 + 1; f.complete = true; reduction.add(f); }
    check(reduction.close(summary) && summary.frames == 5 && summary.incomplete == 0 && summary.dt_p95 == 5);
    refuse_allocation = true;
    for (unsigned i = 0; i < window_frames + 40; ++i) { Sample f{}; f.frame = i; f.dt_us = 9; reduction.add(f); }
    refuse_allocation = false;
    check(reduction.count() == window_frames && reduction.close(summary) && summary.frames == 300 && summary.frame == 339);
    std::printf("frame_phases_host checks=%u failures=0 tracker_bytes=%zu window_bytes=%zu\n", checks, sizeof(Tracker), sizeof(Window));
    return 0;
}
