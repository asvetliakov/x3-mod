...
static void report(const Clock& c, int result, const media_owned::Update& u = {}) {
    ...
    std::cout << "{\"result\":" << result << ",\"generation\":" << c.generation()
        << ",\"operation\":" << c.operation() << ",\"rate\":" << c.rate_numerator()
        << ",\"armed\":" << c.armed() << ",\"intent\":" << c.intent()
        << ",\"paused\":" << c.paused() << ",\"queued\":" << c.queued()
        << ",\"end\":" << int(c.end()) << ",\"selected\":" << u.selected
        << ",\"token\":" << u.frame.token << ",\"start\":" << u.frame.start
        << ",\"stop\":" << u.frame.end << ",\"consumed\":" << u.consumed
        << ",\"late\":" << u.late << ",\"superseded\":" << u.superseded
        << ",\"ms\":" << ms.truncated << ",\"in_range\":" << ms.in_range
        << ",\"seconds_bits\":" << seconds_bits << ",\"scaled_bits\":" << scaled_bits
        << ",\"numerator\":\"" << std::hex << std::setfill('0');
...
int main(int argc, char**) {
    if (argc > 1) {
        ...
        std::cout << "{\"iterations\":" << iterations << ",\"consumed\":" << accepted
                  << ",\"nanoseconds\":" << ns << ",\"core_bytes\":" << sizeof c << "}\n";
    ...
    while (std::cin >> command) {
        ...
        if (command == "begin") {
            ...
            std::cin >> start >> end >> now; result = c.seek(start, end, now);
        } else if (command == "rate") {
            std::cin >> rate >> now; result = c.set_rate(rate, now);
        } else if (command == "frame") {
            media_owned::Frame f;
            std::cin >> f.generation >> f.token >> f.start >> f.end;
            result = int(c.submit(f));
        } else if (command == "eof") {
            std::cin >> generation; result = c.provider_eof(generation);
        } else {
            std::cin >> now;
            if (command == "update") { update = c.update(now); result = update.accepted; }
            else if (command == "pause") result = c.pause(now);
            else if (command == "resume") result = c.resume(now);
            else if (command == "stop") result = c.stop(now);
            else if (command == "loop") result = c.restart_loop(now);
            else return 2;
        }
        if (!std::cin) return 2;
