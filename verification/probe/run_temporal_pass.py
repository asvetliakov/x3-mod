...
try:
    ...
    assert run.returncode==0 and match and tuple(map(int,match.groups()))==(44,40,2) and report['samples']==44 and 'RESET PASS' in text and 'FAIL' not in text,text[-1500:]
    assert report['source_unchanged'] and report['binary_unchanged'],'Provenance changed during run'
    report['passed']=True
finally:
