...
try:
    ...
    report['thin_region_camera']={'static':fields('THIN_REGION_CAMERA_STATIC '),'pan_oracle':fields('THIN_REGION_PAN '),'pan':fields('THIN_REGION_CAMERA '),'forward':fields('THIN_REGION_CAMERA_FORWARD '),'stale':fields('THIN_REGION_STALE '),'box_domain':fields('THIN_REGION_BOX_DOMAIN '),'bad_motion':fields('THIN_REGION_BAD_MOTION '),'timing':fields('LINE_TIMING_CAMERA ')}
    assert [m['kind'] for m in report['thin_region_camera']['bad_motion']]==['overflow_1e30','nan'] and report['thin_region_camera']['bad_motion'][0]['camera_gate_max_within_8px']=='0.0000' and len(report['thin_region_camera']['box_domain'])==1 and len(report['thin_region_camera']['forward'])==1,report['thin_region_camera']
    ...
    # 28 / 9 are the run-139 lattice cases (FLICKER_BASE); the line-filter cases add 45 numerical and 2 state checks (LINE_BASE), the far-stabiliser cases 127 and 2 (FAR_BASE), the thin-region cases 119 and 4 (42 and 2 of them the camera gate, taa-lattice-crawl.md sections 32.1 and 32.3).
    # 28 / 9 are the run-139 lattice cases (LATTICE_BASE); the flicker cases add 182 numerical and 4 state checks.
    assert lattice.returncode==0 and 'LATTICE_BASE numerical=28 state_restorations=9' in lattice_text and 'FLICKER_BASE numerical=210 state_restorations=13' in lattice_text and 'LINE_BASE numerical=255 state_restorations=15' in lattice_text and 'RESULT PASS numerical=501 state_restorations=21 lattice=1' in lattice_text and 'FAIL' not in lattice_text,lattice_text[-1500:]
