...
try:
    ...
    # 28 / 9 are the run-139 lattice cases (FLICKER_BASE); the line-filter cases add 45 numerical and 2 state checks.
    # 28 / 9 are the run-139 lattice cases (LATTICE_BASE); the flicker cases add 182 numerical and 4 state checks.
    assert lattice.returncode==0 and 'LATTICE_BASE numerical=28 state_restorations=9' in lattice_text and 'FLICKER_BASE numerical=210 state_restorations=13' in lattice_text and 'RESULT PASS numerical=255 state_restorations=15 lattice=1' in lattice_text and 'FAIL' not in lattice_text,lattice_text[-1500:]
