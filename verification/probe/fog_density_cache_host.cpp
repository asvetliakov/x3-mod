...
struct Gpu {
    ...
    Gpu() {
        for (auto& a : atlas) a.assign(kAtlasBytes, 0xcd);
    }
    void lose() {
        for (auto& a : atlas) a.assign(kAtlasBytes, 0xcd);
    }
    ...
    unsigned frame(DensityCache& cache, std::size_t budget = kDefaultUploadBudget, bool fail = false) {
        ...
        max_rects = std::max<std::size_t>(max_rects, n);
        frames_with_upload += n != 0;
...
unsigned settle(DensityCache& cache, Gpu& gpu, const double camera[3], std::uint64_t& frame,
                FrameState* last = nullptr) {
    unsigned frames = 0;
...
void planning() {
    ...
    for (int level = 0; level < kLevelCount; ++level) {
        ...
        require("retarget_at_threshold",
                retarget_needed(level, origin, moved) && window.contains(need_box(level, moved)));
        ...
        bool cells = true, sorted = true;
        for (std::size_t i = 0; i < count; ++i) {
            const NodeBox& b = jobs[i].box;
            nodes += b.nodes();
            for (int a = 0; a < 3; ++a) {
                const std::int64_t cell = a == 2 ? kLanes : kBrickTexels;
                cells = cells && (b.lo[a] & ~(cell - 1)) == (b.hi[a] & ~(cell - 1));
            }
        ...
        bool disjoint = true;
        for (std::size_t i = 0; i < count && disjoint; ++i)
            for (std::size_t j = i + 1; j < count; ++j)
                if (!intersect(jobs[i].box, jobs[j].box).empty()) {
                    disjoint = false;
                    break;
                }
        ...
        NodeBox have = intersect(window, grow(need, kFirstFillSlack[level])), slab;
        NodeBox urgent_need = have;
        urgent_need.hi[1] += 1;
        require("urgent_side_first", next_slab(have, window, urgent_need, slab) && slab.lo[1] == have.hi[1] + 1 &&
                                         slab.hi[1] == window.hi[1] && slab.lo[0] == have.lo[0] &&
                                         slab.hi[2] == have.hi[2]);
        std::uint64_t grown = have.nodes();
        unsigned steps = 0;
        while (next_slab(have, window, need, slab) && steps < 8) {
            ++steps;
            grown += slab.nodes();
            require("slab_disjoint_from_box", intersect(have, slab).empty());
            for (int a = 0; a < 3; ++a) {
                have.lo[a] = std::min(have.lo[a], slab.lo[a]);
                have.hi[a] = std::max(have.hi[a], slab.hi[a]);
            }
        }
        require("slabs_reach_window_exactly", have == window && grown == window.nodes() && steps <= 6);
    }
    require("box_algebra", intersect(empty_box(), universe_box()).empty() && universe_box().contains(empty_box()) &&
                               !empty_box().contains(universe_box()) &&
                               empty_box() == intersect(window_box({0, 0, 0}), window_box({200, 0, 0})));
}
...
void logic() {
    ...
    cache.gpu_reset();
    {
        ...
        require("origin_storage_126",
                storage_index(o.x) == 126 && storage_index(o.y) == 126 && storage_index(o.z) == 126);
    }
    { // has_work() is false between slabs although the windows are far from complete: only idle() means quiescent,
        ...
        require("has_work_false_mid_fill_is_not_idle",
                !cache.has_work() && !cache.idle() && cache.stats().nodes_generated < 2ull * 128 * 128 * 128);
        require("mid_fill_resident_box_is_exactly_the_uploaded_slab",
                s.resident[1] && !s.resident[0] && cache.gpu_box(0).empty() &&
                    cache.gpu_box(1) ==
                        intersect(window_box(cache.worker_origin(1)), grow(need_box(1, camera), kFirstFillSlack[1])) &&
                    cache.gpu_box(1).nodes() == cache.stats().nodes_generated);
    ...
    unsigned far_ready_frame = 0, fine_ready_frame = 0, frames = 0;
    bool monotone = true, bounded = true;
    float previous[2] = {0, cache.step(camera, frame).ready[1]};
    for (bool busy = true; (busy || previous[0] < 1 || previous[1] < 1) && frames < 100000; ++frames) {
        const FrameState s = cache.step(camera, ++frame);
        for (int l = 0; l < kLevelCount; ++l) {
            monotone = monotone && s.ready[l] >= previous[l];
            bounded = bounded && s.ready[l] - previous[l] <= 1.f / kReadinessRampFrames + 1e-6f;
            previous[l] = s.ready[l];
        }
    ...
    std::printf(
        "FILL frames=%u far_resident_frame=%u fine_resident_frame=%u nodes=%llu jobs=%llu slabs=%llu max_upload_bytes=%zu max_rects=%zu upload_bytes=%llu\n",
        frames, far_ready_frame, fine_ready_frame, (unsigned long long)filled.nodes_generated,
        (unsigned long long)filled.jobs, (unsigned long long)filled.slabs, gpu.max_bytes, gpu.max_rects,
        (unsigned long long)filled.upload_bytes);
    ...
    require("fill_generates_each_node_once",
            filled.nodes_generated == 2ull * kWindowNodes * kWindowNodes * kWindowNodes && filled.first_fills == 2);
    require("upload_budget_enforced_including_the_first_rectangle",
            !gpu.over_budget && gpu.max_bytes <= kDefaultUploadBudget && gpu.max_bytes > 0);
    ...
    { // Steady state: no lock, no work.
        const std::uint64_t missed = cache.stats().missed_locks;
        const double start = now_us();
        const int n = 200000;
        for (int i = 0; i < n; ++i) cache.step(camera, ++frame);
        std::printf("STEP_COST steady_ns_per_frame=%.1f\n", (now_us() - start) * 1e3 / n);
        require("steady_state_no_uploads_no_misses", !cache.has_work() && cache.stats().missed_locks == missed &&
                                                         cache.stats().nodes_generated == filled.nodes_generated);
    }
    // Recentre across the seam: 2 fine nodes per move on all three axes, five moves (storage 126..135 -> wraps).
    bool recentre = true;
    std::uint64_t before = cache.stats().nodes_generated;
    const std::vector<std::uint8_t> untouched_far = gpu.atlas[1];
    for (int move = 0; move < 5; ++move) {
        ...
        std::printf("RECENTRE move=%d nodes=%llu expected=%llu fine_ready=%.3f\n", move,
                    (unsigned long long)(after - before), (unsigned long long)expect, s.ready[0]);
        recentre = recentre && after - before == expect && s.ready[0] == 1 && s.ready[1] == 1 &&
                   equal_static(gpu, cache, identity.offset, "recentre");
        before = after;
    }
    ...
    { // Back across the seam in the negative direction, far level included (9 far nodes).
        ...
        FrameState s;
        settle(cache, gpu, camera, frame, &s);
        require("negative_recentre_both_levels_equals_from_scratch",
                equal_static(gpu, cache, identity.offset, "negative"));
    ...
    { // Stalled worker: guard violation ramps down, need violation drops at once.
        ...
        double away[3] = {camera[0] + 5 * kFineDelta, camera[1], camera[2]}; // need hi = window hi: inside, guard
                                                                             // outside
        ...
        require("guard_violation_ramps_down",
                soft && s.ready[0] < 1 && s.ready[0] >= 1 - 1.5f / kReadinessRampFrames && s.ready[1] == 1);
        away[0] += 2 * kFineDelta;
        s = cache.step(away, ++frame);
        require("need_violation_drops_fine_only", s.ready[0] == 0 && !s.resident[0] && s.ready[1] == 1 &&
                                                      !cache.covers(0, away) && cache.covers(1, away));
        settle(cache, gpu, away, frame, &s);
        for (int a = 0; a < 3; ++a) camera[a] = away[a];
        require("fine_recovers_and_ramps",
                s.resident[0] && s.ready[0] > 0 && equal_static(gpu, cache, identity.offset, "stall"));
    }
    { // Reset mid-recentre and a failed upload: everything is uploaded again, nothing regenerated twice.
        ...
        settle(cache, gpu, camera, frame, &s);
        require("reset_mid_fill_recovers_from_cpu_cache",
                equal_static(gpu, cache, identity.offset, "reset") && s.resident[0] && s.resident[1]);
        require("reset_regenerates_only_the_pending_slabs",
                cache.stats().nodes_generated - generated_before <= 128ull * 128 * 128 - 126ull * 126 * 126);
    }
    { // Identity change: stale data dropped, new offset generated, ramps restart.
        CacheIdentity other = identity;
        other.sector_key = 0x77;
        other.offset.x += 4096. * 100;
        cache.configure(other);
        FrameState s = cache.step(camera, ++frame);
        require("identity_change_invalidates", s.ready[0] == 0 && s.ready[1] == 0 && cache.gpu_box(1).empty());
        settle(cache, gpu, camera, frame, &s);
        require("new_identity_equals_static_with_its_offset",
                equal_static(gpu, cache, other.offset, "identity") && cache.stats().first_fills == 4);
        cache.configure(other);
    ...
    { // One-tile budget: the enforced cap is one tile, never one tile more.
        ...
        unsigned frames_used = 0;
        while (small.frame(tight, 1) && frames_used < 100000) ++frames_used; // a 1-byte budget is raised to one tile
        std::printf("TIGHT_BUDGET frames=%u max_upload_bytes=%zu\n", frames_used, small.max_bytes);
        require("tight_budget_never_exceeds_one_tile_and_still_drains",
                !small.over_budget && small.max_bytes <= kTileBytes && small.max_bytes > 0 && !tight.has_work());
    }
    { // Admission boundary: node keys near +-2^31, where lo + hi of a job box leaves int32.
        ...
        bool sane = true;
        for (int level = 0; level < kLevelCount; ++level) {
            const std::size_t count = plan_jobs(window_box(window_origin(kLevelDelta[level], c[0], c[1], c[2])), level,
                                                c, jobs.get(), kMaxJobs);
            sane = sane && count > 0 && jobs[0].distance2 < 3 * 32. * 32. && jobs[count - 1].distance2 < 3 * 80. * 80.;
            for (std::size_t i = 0; i < count; ++i)
                sane = sane && jobs[i].distance2 >= 0 && jobs[i].distance2 < 3 * 80. * 80.;
        }
        require("job_distances_at_the_camera_limit_do_not_overflow", sane);
        FrameState s;
        settle(edge, far_gpu, c, f, &s);
        require("camera_limit_fills_and_equals_static",
                s.resident[0] && s.resident[1] && edge.idle() && equal_static(far_gpu, edge, kNoOffset, "limit"));
        const double beyond[3] = {std::nextafter(kCameraLimit, 2e12), 0, 0};
        s = edge.step(beyond, ++f);
        require("camera_beyond_the_limit_is_refused",
                !s.resident[0] && !s.resident[1] && s.ready[1] == 0 && !edge.covers(1, beyond) && edge.covers(1, c));
    }
}
...
void threaded() {
    double camera[3] = {95576., 97323., 82698.};
    {
        ...
        double far_ready = 0, fine_ready = 0, worst_step = 0, worst_frame = 0;
        FrameState s{};
        ...
        while ((s.ready[0] < 1 || s.ready[1] < 1 || !cache.idle()) && now_us() - begin < 120e6) {
            ...
            worst_step = std::max(worst_step, t1 - t0);
            worst_frame = std::max(worst_frame, t2 - t1);
        ...
        std::printf(
            "THREADED far_resident_ms=%.1f fine_resident_ms=%.1f frames=%llu nodes=%llu worker_busy_ms=%.1f nodes_per_second=%.0f worst_step_us=%.1f worst_upload_frame_us=%.1f max_upload_bytes=%zu max_rects=%zu missed_locks=%llu\n",
            far_ready / 1e3, fine_ready / 1e3, (unsigned long long)frame, (unsigned long long)st.nodes_generated,
            st.worker_busy_us / 1e3, st.worker_busy_us ? st.nodes_generated * 1e6 / st.worker_busy_us : 0., worst_step,
            worst_frame, gpu.max_bytes, gpu.max_rects, (unsigned long long)st.missed_locks);
        require("threaded_fill_equals_static_atlas",
                s.ready[0] == 1 && s.ready[1] == 1 && equal_static(gpu, cache, kNoOffset, "threaded"));
        ...
        for (int move = 0; move < 40; ++move) {
            camera[move % 3] += kFineDelta * (move % 5 == 4 ? -1 : 1);
            for (int i = 0; i < 3; ++i) {
                cache.step(camera, ++frame);
                gpu.frame(cache);
                std::this_thread::sleep_for(std::chrono::milliseconds(1));
        ...
        for (;;) {
            cache.step(camera, ++frame);
            gpu.frame(cache);
            if (cache.idle() || now_us() - settle_begin > 60e6) break;
            std::this_thread::sleep_for(std::chrono::milliseconds(1));
        }
        ...
        bool centred = true;
        for (int l = 0; l < kLevelCount; ++l)
            centred = centred && cache.gpu_box(l) == window_box(cache.worker_origin(l)) &&
                      !retarget_needed(l, cache.worker_origin(l), camera);
        require("threaded_recentre_equals_static_atlas",
                quiet && centred && equal_static(gpu, cache, kNoOffset, "threaded_recentre"));
        require("stop_is_idempotent", !cache.running());
        cache.stop();
    }
    { // Shutdown while generating, invalidation storms, restart: no deadlock, prompt join.
        ...
        for (int round = 0; round < 24; ++round) {
            ...
            for (int i = 0; i < 4 + round; ++i) {
                if (i % 3 == 1) {
                    CacheIdentity id;
                    id.sector_key = std::uint64_t(round) * 16 + i;
                    id.offset.x = 4096. * i;
                    cache.configure(id);
                }
            ...
            if (round % 2) cache.stop(); // odd rounds: explicit stop, even: destructor joins
    ...
    { // Invalidate under a live worker converges to the new identity.
        ...
        CacheIdentity id;
        id.sector_key = 9;
        id.offset = {4096. * 3, 0, -4096. * 7};
        cache.configure(id);
        const double begin = now_us();
        FrameState s{};
        while ((!s.resident[0] || !s.resident[1] || !cache.idle()) && now_us() - begin < 120e6) {
            s = cache.step(camera, ++frame);
            gpu.frame(cache);
            std::this_thread::sleep_for(std::chrono::milliseconds(1));
        }
...
void lifetime() {
    ...
    { // stop() cannot take the mutex for 250 ms while the worker is alive (starved, not dead):
        ...
        DensityCache* cache = new DensityCache;
        Gpu gpu;
        std::uint64_t frame = 0;
        bool ok = cache->start();
        cache->gpu_reset();
        ...
        std::thread holder([&] {
            std::unique_lock<std::mutex> held(cache->test_mutex());
            phase.store(1);
        ...
        phase.store(2);
        holder.join();
        std::this_thread::sleep_for(std::chrono::milliseconds(200)); // the live worker takes the freed-or-not mutex,
                                                                     // sees the stop flag and leaves
        ...
        require("held_lock_abandons_after_250ms_without_free",
                ok && waited >= 250e3 && waited < 5e6 && after >= before && cache->cache_bytes(0) != nullptr);
    }
    { // Process-exit form: abandon, then the owner's release. No join, no notify, no free; prompt.
        ...
        cache->abandon();
        DensityCache::retire(cache);
        const double took = now_us() - t0;
        std::printf("ABANDON retire_ms=%.3f\n", took / 1e3);
        require("abandon_then_retire_is_prompt_and_leaks",
                ok && cache->abandoned() && took < 50e3 && cache->cache_bytes(1) != nullptr);
