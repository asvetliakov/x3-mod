...
existing JSON without running Wine (no lock needed). Never launches the game.
"""
...
def row(case, errors):
    ...
    return '| {stage} | {kind} | {n}{loop} | {compile} | {slots} | {create} | {draw} | {executed} | {ms} |'.format(
...
def finish(record, out_path, err_path, out_json):
    ...
    md += ['', '## Cases', '',
           ...
           'Slots = D3DXDisassembleShader "approximately N instruction slots used".', '',
