...
int main(int argc, char** argv) {
    try {
        ...
        for (unsigned index = 0; index < 9; ++index) {
            ...
            require(fingerprint(vertex.data(), vertex.size()) == pair.vertex && vertex[0] == 0xfffe0101u,
                    "native VS identity");
            const auto original = read(std::string(argv[1]) + "/ps_" + hex(pair.pixel) + ".bin");
            const auto saved = original;
            require(fingerprint(original.data(), original.size()) == pair.pixel && original[0] == 0xffff0101u,
                    "native PS identity");
            const Profile* profile = nullptr;
            for (const auto& p : profiles)
                if (p.hash == pair.pixel) profile = &p;
            ...
            require(linear_emission_sm1_pixel_variant(original.data(), original.size(),
                                                      {1.f, LinearEmissionSm1Outputs::Native, false},
                                                      native) == LinearEmissionResult::Applied,
                    "native reference");
            ...
            for (unsigned g = 0; g < 4; ++g) {
                ...
                if (gains[g] == 1.f) {
                    ...
                    const auto status = linear_emission_sm1_pixel_variant(original.data(), original.size(), config,
                                                                          output);
                    ns += std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::steady_clock::now() - start)
                              .count();
                    if (status != LinearEmissionResult::Applied)
                        std::cerr << hex(pair.pixel) << " status=" << static_cast<int>(status) << '\n';
                    ...
                    require(linear_emission_sm1_pixel_variant(output.data(), output.size(), config, again) !=
                                    LinearEmissionResult::Applied &&
                                again == sentinel,
            ...
            Words output = {91, 92};
            const Words sentinel = output;
            for (float invalid : {0.f, 0.5f, 0.999f, 8.001f, 16.f, -1.f, std::numeric_limits<float>::infinity(),
                                  std::numeric_limits<float>::quiet_NaN()}) {
                require(linear_emission_sm1_pixel_variant(original.data(), original.size(),
                                                          {invalid, LinearEmissionSm1Outputs::AdditiveGain, false},
                                                          output) == LinearEmissionResult::InvalidConfig &&
                            output == sentinel,
                        "invalid additive gain");
                auto alias = original;
                require(linear_emission_sm1_pixel_variant(alias.data(), alias.size(),
                                                          {invalid, LinearEmissionSm1Outputs::AdditiveGain, false},
                                                          alias) == LinearEmissionResult::InvalidConfig &&
                            alias == saved,
                        "invalid additive gain alias rollback");
            }
            require(linear_emission_sm1_pixel_variant(original.data(), original.size(),
                                                      {2.f, LinearEmissionSm1Outputs::AdditiveGain, true},
                                                      output) == LinearEmissionResult::InvalidConfig &&
                        output == sentinel,
                    "additive partial precision refused");
            require(linear_emission_sm1_pixel_variant(nullptr, original.size(),
                                                      {2.f, LinearEmissionSm1Outputs::AdditiveGain, false},
                                                      output) == LinearEmissionResult::InvalidInput &&
                        output == sentinel,
                    "null rollback");
            for (std::size_t n = 0; n < original.size(); ++n) {
                require(linear_emission_sm1_pixel_variant(original.data(), n,
                                                          {2.f, LinearEmissionSm1Outputs::AdditiveGain, false},
                                                          output) != LinearEmissionResult::Applied &&
                            output == sentinel,
                        "truncation rollback");
            }
            rows += std::string(rows.empty() ? "" : ",") + "{\"vertex\":\"" + hex(pair.vertex) + "\",\"pixel\":\"" +
                    hex(pair.pixel) + "\",\"scalar\":" + (profile->scalar ? "true" : "false") +
                    ",\"original_words\":" + std::to_string(original.size()) + "}";
        }
        // A PS2 program of the live emission route is outside this registry.
        {
            auto w = read(std::string(argv[1]) + "/ps_8360f422de08b5bd.bin");
            Words result = {7};
            const Words sentinel = result;
            require(linear_emission_sm1_pixel_variant(w.data(), w.size(),
                                                      {2.f, LinearEmissionSm1Outputs::AdditiveGain, false},
                                                      result) == LinearEmissionResult::UnsupportedShader &&
                        result == sentinel,
