...
constexpr bool SUCCEEDED(HRESULT h) {
    return h >= 0;
}
constexpr bool FAILED(HRESULT h) {
    return h < 0;
}
...
constexpr unsigned D3DRS_ZENABLE = 7, D3DRS_ZWRITEENABLE = 14, D3DRS_ALPHATESTENABLE = 15, D3DRS_ALPHABLENDENABLE = 27,
                   D3DRS_COLORWRITEENABLE = 168, D3DRS_SRGBWRITEENABLE = 194, D3DRS_COLORWRITEENABLE1 = 190,
                   D3DRS_COLORWRITEENABLE2 = 191, D3DRS_WRAP0 = 128, D3DRS_WRAP1 = 129, D3DRS_WRAP2 = 130,
                   D3DRS_WRAP3 = 131, D3DRS_WRAP4 = 132, D3DRS_WRAP5 = 133, D3DRS_WRAP6 = 134, D3DRS_WRAP7 = 135,
                   D3DRS_WRAP8 = 198, D3DRS_WRAP9 = 199, D3DRS_WRAP10 = 200, D3DRS_WRAP11 = 201, D3DRS_WRAP12 = 202,
                   D3DRS_WRAP13 = 203, D3DRS_WRAP14 = 204, D3DRS_WRAP15 = 205;
constexpr unsigned D3DRS_ALPHAFUNC = 25, D3DRS_ALPHAREF = 24, D3DRS_ZFUNC = 23, D3DRS_FOGENABLE = 28,
                   D3DRS_DITHERENABLE = 26, D3DRS_STENCILENABLE = 52, D3DRS_CULLMODE = 22, D3DRS_FILLMODE = 8,
...
struct MotionOutputProfile {
    std::uint8_t texcoord_index = 4, depth_texcoord_index = 7;
};
...
using IDirect3DVertexShader9 = Shader;
using IDirect3DPixelShader9 = Shader;
...
struct MotionRoute {
    ...
    bool depth = false, linear_material = false, write2_set = false, rt2_set = false, write_set = false, rt_set = false,
         ps_set = false, vs_set = false, vs_constants_set = false, ps_constants_set = false;
    DWORD saved_write1 = 15, saved_write2 = 15, saved_wrap[6]{};
    ...
    bool hull_lightmap_widen = false, alpha_tested = false; // hull emissive widening: the widened PS selected; the
                                                            // draw's alpha test (never here)
    bool widen_filter_set = false;
...
#define CHECK(x)                                                                                                       \
    do {                                                                                                               \
        ...
        if (!(x)) {                                                                                                    \
            ++failures;                                                                                                \
            std::printf("FAIL line=%u: %s\n", __LINE__, #x);                                                           \
        }                                                                                                              \
    } while (0)
struct Device {
    std::array<DWORD, 256> states{};
    unsigned gets = 0, sets = 0, vs_sets = 0, ps_sets = 0;
    std::array<unsigned, 16> fail_get{}, fail_set{};
    std::array<HRESULT, 64> set_result{};
    bool mutation_before_failure = false;
    Shader *fail_shader = nullptr, *bound_vs = nullptr, *bound_ps = nullptr;
    unsigned shader_gets = 0;
    std::array<unsigned, 4> fail_shader_get{}; // nth GetVertexShader/GetPixelShader call fails
...
constexpr unsigned D3DSAMP_MINFILTER = 6;
using SetSamplerStateFn = HRESULT (*)(Device*, DWORD, D3DSAMPLERSTATETYPE, DWORD);
using GetSamplerStateFn = HRESULT (*)(Device*, DWORD, D3DSAMPLERSTATETYPE, DWORD*);
using SetPsFn = HRESULT (*)(Device*, Shader*);
using SetVsFn = SetPsFn;
using GetVsFn = HRESULT (*)(Device*, Shader**);
using GetPsFn = GetVsFn;
...
HRESULT set_sampler(Device*, DWORD, D3DSAMPLERSTATETYPE, DWORD) {
    return E_FAIL;
} // never reached: the widening is off in this seam
HRESULT get_sampler(Device*, DWORD, D3DSAMPLERSTATETYPE, DWORD*) {
    return E_FAIL;
}
HRESULT get_state(Device* d, unsigned s, DWORD* v) {
    ++d->gets;
    for (auto n : d->fail_get)
        if (n == d->gets) return E_FAIL;
    *v = d->states[s];
    return S_OK;
}
HRESULT set_state(Device* d, unsigned s, DWORD v) {
    ++d->sets;
    d->writes[(d->sets - 1) % d->writes.size()] = {s, v};
    HRESULT result = d->set_result[d->sets];
    if (SUCCEEDED(result))
        for (auto n : d->fail_set)
            if (n == d->sets) {
                result = E_FAIL;
                break;
            }
    if (SUCCEEDED(result) || d->mutation_before_failure) d->states[s] = v;
    return result;
}
HRESULT set_vs(Device* d, Shader* p) {
    ++d->vs_sets;
    if (p && p == d->fail_shader) return E_FAIL;
    d->bound_vs = p;
    return S_OK;
}
HRESULT set_ps(Device* d, Shader* p) {
    ++d->ps_sets;
    if (p && p == d->fail_shader) return E_FAIL;
    d->bound_ps = p;
    return S_OK;
...
// writes nothing.
HRESULT get_vs(Device* d, Shader** p) {
    ++d->shader_gets;
    for (auto n : d->fail_shader_get)
        if (n == d->shader_gets) return E_FAIL;
    *p = d->bound_vs;
    if (d->bound_vs) d->bound_vs->AddRef();
    return S_OK;
}
HRESULT get_ps(Device* d, Shader** p) {
    ++d->shader_gets;
    for (auto n : d->fail_shader_get)
        if (n == d->shader_gets) return E_FAIL;
    *p = d->bound_ps;
    if (d->bound_ps) d->bound_ps->AddRef();
    return S_OK;
}
HRESULT set_constants(Device*, unsigned, const float*, unsigned) {
    return S_OK;
}
...
public:
    Device* device_;
    bool enabled_ = true, state_shadow_ = true, state_hooks_ = true, motion_state_lost_ = false,
         scene_open_ = false; // state_hooks_: hybrid unhook mirror (hooks on here)
    HRESULT motion_state_error_ = D3DERR_INVALIDCALL;
    unsigned id_ = 1, generation_ = 0, frame_ = 0, taa_references_ = 0, logged_failures_ = 0, taa_invalidations = 0,
             frames = 0;
    Pass* taa_ = nullptr;
    ...
    std::uint64_t sun_apply_frame_ = ~std::uint64_t(0), depth_replayed_frame_ = ~std::uint64_t(0); // per-frame markers
                                                                                                   // cleared by
                                                                                                   // after_reset
    struct {
        unsigned rs_queries = 0, rs_hits = 0, rs_gets = 0, rs_resyncs = 0, restore_failures = 0, draws = 0,
                 sb_resyncs = 0, material_bind_failures = 0, restore_getters = 0, restore_declines = 0,
                 fade_owner_masked = 0;
    } counters_;
    struct {
        ...
        /* sized for the production composition_blend_states table (asserted below) */ DWORD composition_blend[8]{};
        bool composition_blend_known[8]{};
        DWORD fill_mode = 0;
        bool fill_mode_known = false;
        bool vs_reserved_written = false, ps_reserved_written = false;
        Shader *vs = nullptr, *ps = nullptr, *vs_variant = nullptr, *ps_variant = nullptr,
               *vs_material_variant = nullptr, *ps_material_variant = nullptr;
        bool original_fill_pair = false;
        Shader* ps_original_fill_variant = nullptr;
        bool xt_default_pair = false, xt_default_ready = false;
        Shader *vs_xt_default_linear = nullptr, *vs_xt_default_ordinary = nullptr, *ps_xt_default_ordinary = nullptr;
        Shader *ps_sun_motion = nullptr, *ps_sun_material = nullptr, *ps_sun_xt = nullptr;
        bool ps_sun_extraction = false;
        Shader* ps_sun_original = nullptr;
        bool original_share_pair = false, original_share_refused = false; // original share variant
                                                                          // (legacy-sun-application.md 4.1)
        Shader *ps_sun_original_lightmap = nullptr, *ps_hull_lightmap_variant = nullptr;
        bool hull_lightmap_pair = false; // hull light-map gain variants; inert here
        Shader *ps_hull_lightmap_widen = nullptr, *ps_sun_original_lightmap_widen = nullptr;
        std::uint8_t hull_lightmap_stage = 0; // hull emissive widening variants; inert here
        float vs_reserved[16]{}, ps_reserved[8]{};
        renderer::LinearMaterialPairContract material_contract{};
    } shadow_;
    explicit MotionOutput(Device& d)
        : device_(&d) {}
    template <class F, class... A> HRESULT direct_call(unsigned n, A... a) {
        return native<F>(n)(device_, a...);
    } // the route's value-only entry
    template <class F> F native(unsigned n) {
        switch (n) {
    ...
    bool cutout_reset_pending_ = false;
    void probe_cutout_caps(bool) {}
    ...
    unsigned sun_original_refused_draws_ = 0; // read by the bind path's original-share gate
    bool thin_vote_upload_ = false; // X3M_TAA_THIN_VOTE: off in this seam (the extracted undo restores c216-c217 only)
    bool fade_rt2_owner_ = false;
    bool upload_c218() const { return thin_vote_upload_ || fade_rt2_owner_; } // X3M_FADE_RT2_OWNER: off in this seam
    bool hull_gain_enabled_ = true, hull_lightmap_enabled_ = true;
    std::uint32_t hull_lightmap_draws_ = 0; // F6 guide-light flag, F4 light-map flag and light-map draw counter read by
                                            // the bind/after-draw paths
    float lightmap_widen_draw_scale_[2] = {0.f, 0.f};
    struct {
        DWORD levels = 0;
    } samplers_[16]; // hull emissive widening: this draw's footprint lanes (0 = off) and the sampler shadow's level
                     // counts read by the bind path
    unsigned sun_qualifications_ = 0;
    void qualify_sun_lane() { ++sun_qualifications_; }
    struct {
        bool failed = false, published = false, available = false, coverage_required = false;
        unsigned receivers = 0, covered = 0, untracked = 0;
    } sun_frame_;
...
unsigned wrap(unsigned i) {
    return i < 8 ? D3DRS_WRAP0 + i : D3DRS_WRAP8 + i - 8;
}
renderer::LinearMaterialPairContract material(std::uint8_t count, renderer::LinearMaterialScalarTransport a = {},
                                              renderer::LinearMaterialScalarTransport b = {}) {
    renderer::LinearMaterialPairContract result{};
    result.sampler_mask = 31;
    result.bump = true;
    result.scalar_transport = {a, b};
    result.scalar_transport_count = count;
    return result;
}
void app_state(Device& d, MotionOutput& m, unsigned index, DWORD value) {
    d.states[wrap(index)] = value;
    m.set_render_state(wrap(index), value);
}
        ...
        for (bool depth : {false, true})
            for (unsigned a = 0; a < 16; ++a)
                for (unsigned b = 0; b < 16; ++b) {
                    ...
                    Device d;
                    MotionOutput m(d);
                    m.state_shadow_ = cached;
                    for (unsigned i = 0; i < 16; ++i) {
                        app_state(d, m, i, i + 1);
                        CHECK(shadow_index(wrap(i)) == i + 8);
                    }
                    m.shadow_.material_contract = material(2, {6, 0, 1, 3}, {6, 1, 2, 3});
                    const auto before = d.states;
                    MotionRoute r;
                    r.depth = depth;
                    renderer::MotionOutputProfile p{std::uint8_t(a), std::uint8_t(b)};
                    CHECK(m.apply_wrap_states(r, p) == S_OK);
                    for (unsigned i = 0; i < 16; ++i)
                        CHECK(d.states[wrap(i)] == ((i == a || (depth && i == b)) ? 0 : i + 1));
                    CHECK(m.undo(r) == S_OK && d.states == before && !m.motion_state_lost_);
                    CHECK(d.sets == (depth ? 4u : 2u));
                    CHECK(d.gets == (cached ? 0u : depth ? 2u : 1u));
                }
}
...
void actual_material_contracts() {
    ...
    for (bool depth : {false, true})
        for (DWORD source = 0; source < 4; ++source) {
            Device d;
            MotionOutput m(d);
            m.shadow_.material_contract = boron_base;
            app_state(d, m, 7, 15);
            app_state(d, m, 8, 10);
            app_state(d, m, 6, source);
            app_state(d, m, 1, (source & 1) ? 0 : 15);
            app_state(d, m, 2, (source & 2) ? 0 : 15);
            const auto before = d.states;
            MotionRoute r;
            r.depth = depth;
            r.linear_material = true;
    ...
    auto single = [&](const renderer::LinearMaterialPairContract& contract, const renderer::MotionOutputProfile& row,
                      unsigned source_index) {
        for (bool depth : {false, true})
            for (DWORD source : {0u, 1u}) {
                Device d;
                MotionOutput m(d);
                m.shadow_.material_contract = contract;
                app_state(d, m, row.texcoord_index, 9);
                app_state(d, m, row.depth_texcoord_index, 10);
                app_state(d, m, source_index, source);
                app_state(d, m, 1, 6 | (source ? 0u : 8u));
                const auto before = d.states;
                MotionRoute r;
                r.depth = depth;
                r.linear_material = true;
                ...
                CHECK(d.states[wrap(row.texcoord_index)] == 0 &&
                      d.states[wrap(row.depth_texcoord_index)] == (depth ? 0u : 10u) &&
                      d.states[wrap(source_index)] == source);
                CHECK(d.states[wrap(1)] == ((before[wrap(1)] & ~8u) | (source ? 8u : 0u)));
                CHECK(m.undo(r) == S_OK && d.states == before);
            }
    };
    single(boron_single, boron_row, 6);
    single(paranid, paranid_row, 7);
}

void fallback_and_alias_cases() {
    const renderer::MotionOutputProfile p{4, 5};
    const auto boron = material(2, {6, 0, 1, 3}, {6, 1, 2, 3});
    ...
    for (bool fail_vs : {true, false}) {
        Device d;
        MotionOutput m(d);
        Shader vs, ps, mvs, mps, app_vs, app_ps;
        m.shadow_.vs = &vs;
        m.shadow_.ps = &ps;
        m.shadow_.vs_variant = &vs;
        m.shadow_.ps_variant = &ps;
        m.shadow_.vs_material_variant = &mvs;
        m.shadow_.ps_material_variant = &mps;
        m.shadow_.material_contract = boron;
        d.bound_vs = &app_vs;
        d.bound_ps = &app_ps; // the application's programs, owned across the routed draw
        ...
        const auto before = d.states;
        MotionRoute r;
        CHECK(m.bind_variant_pair(r, true) == S_OK && !r.linear_material && m.counters_.material_bind_failures == 1);
        ...
        CHECK(m.apply_wrap_states(r, p) == S_OK && r.wrap_count == 1 && d.states[wrap(4)] == 0);
        CHECK(d.states[wrap(1)] == before[wrap(1)] && d.states[wrap(2)] == before[wrap(2)] &&
              d.states[wrap(6)] == before[wrap(6)]);
        CHECK(m.undo(r) == S_OK && d.states == before);
        CHECK(d.bound_vs == &app_vs && d.bound_ps == &app_ps);
        m.release_restore(r);
        CHECK(!r.restore_held && !r.restore_vs && !r.restore_ps);
        CHECK(app_vs.refs == 1 && app_ps.refs == 1 && app_vs.add_refs == app_vs.releases &&
              app_ps.add_refs == app_ps.releases);
    }
    ...
    // original snapshot, rather than the first mapping's new value.
    {
        Device d;
        MotionOutput m(d);
        m.shadow_.material_contract = material(2, {6, 0, 1, 3}, {1, 3, 2, 3});
        app_state(d, m, 4, 1);
        app_state(d, m, 6, 0);
        app_state(d, m, 1, 8 | 5);
        app_state(d, m, 2, 2);
        ...
        r.linear_material = true;
        CHECK(m.apply_wrap_states(r, p) == S_OK);
        CHECK(d.states[wrap(1)] == 5 && d.states[wrap(2)] == 10);
        CHECK(m.undo(r) == S_OK && d.states == before);
    }
    // Distinct destination components share one snapshot and one setter.
    {
        Device d;
        MotionOutput m(d);
        m.shadow_.material_contract = material(2, {6, 0, 1, 3}, {7, 1, 1, 2});
        app_state(d, m, 4, 0);
        app_state(d, m, 6, 1);
        app_state(d, m, 7, 0);
        app_state(d, m, 1, 15);
    ...
    // The two real Boron records alias TEX6 and read it once with shadow disabled.
    {
        ...
        m.shadow_.material_contract = boron;
        d.states[wrap(4)] = d.states[wrap(5)] = 0;
        d.states[wrap(6)] = 3;
        d.states[wrap(1)] = d.states[wrap(2)] = 0;
        MotionRoute r;
        r.depth = true;
        r.linear_material = true;
        CHECK(m.apply_wrap_states(r, p) == S_OK && r.wrap_count == 5 && d.gets == 5);
        CHECK(m.undo(r) == S_OK);
    }
}
...
void restoration_ownership() {
    auto arm = [](MotionOutput& m, Shader& vs, Shader& ps) {
        m.shadow_.vs_variant = &vs;
        m.shadow_.ps_variant = &ps;
    };
    ...
    // and released exactly once: AddRef and Release balance.
    {
        ...
        m.release_restore(r);
        CHECK(app_vs.releases == 1 && app_ps.releases == 1); // idempotent
    }
...
void failure_cases() {
    ...
    for (unsigned zero : {0u, 15u}) {
        Device d;
        d.states[wrap(0)] = 5;
        d.states[wrap(15)] = 9;
        d.states[wrap(zero)] = 0;
        auto before = d.states;
        MotionOutput m(d);
        MotionRoute r;
        r.depth = true;
        CHECK(m.apply_wrap_states(r, basic) == S_OK && d.sets == 1);
        CHECK(m.undo(r) == S_OK && d.sets == 2 && d.states == before);
    }
    for (unsigned bad : {0u, 1u}) {
        Device d;
        MotionOutput m(d);
        MotionRoute r;
        r.depth = true;
        auto invalid = basic;
        if (bad)
            invalid.depth_texcoord_index = 255;
        else
            invalid.texcoord_index = 16;
    ...
    for (unsigned n = 1; n <= 6; ++n) {
        ...
        MotionOutput m(d);
        m.state_shadow_ = false;
        m.shadow_.material_contract = distinct;
        for (unsigned i : {1u, 2u, 4u, 5u, 6u, 7u}) d.states[wrap(i)] = 15;
        const auto before = d.states;
        d.fail_get[0] = n;
        MotionRoute r;
        r.depth = true;
        r.linear_material = true;
        CHECK(FAILED(m.apply_wrap_states(r, full)) && d.gets == n && d.sets == 0);
        CHECK(m.undo(r) == S_OK && d.states == before && !m.motion_state_lost_);
    }
    ...
    for (bool mutation : {false, true})
        for (unsigned n = 1; n <= 4; ++n) {
            ...
            const auto before = d.states;
            d.fail_set[0] = n;
            d.mutation_before_failure = mutation;
            MotionRoute r;
            r.depth = true;
            r.linear_material = true;
            CHECK(FAILED(m.apply_wrap_states(r, full)));
            CHECK(m.undo(r) == S_OK && d.states == before && !m.motion_state_lost_);
            CHECK(d.sets == n * 2);
        }
    ...
    for (bool mutation : {false, true})
        for (unsigned n = 1; n <= 4; ++n) {
            ...
            r.depth = true;
            r.linear_material = true;
            CHECK(m.apply_wrap_states(r, full) == S_OK);
            CHECK(FAILED(m.undo(r)) && m.motion_state_lost_ && m.motion_state_error_ == E_FAIL);
            CHECK(d.sets == 8 && m.counters_.restore_failures == 1 && m.taa_invalidations == 1);
            CHECK(d.writes[4].first == wrap(2) && d.writes[5].first == wrap(1) && d.writes[6].first == wrap(5) &&
                  d.writes[7].first == wrap(4));
    ...
    // The first restoration HRESULT survives later cleanup failures.
    {
        ...
        app_state(d, m, 1, 0);
        app_state(d, m, 2, 15);
        d.set_result[5] = E_FIRST;
        d.set_result[7] = E_LATER;
        MotionRoute r;
        r.depth = true;
        r.linear_material = true;
        CHECK(m.apply_wrap_states(r, full) == S_OK);
        CHECK(m.undo(r) == E_FIRST && m.motion_state_error_ == E_FIRST && m.motion_state_lost_);
        CHECK(d.sets == 8);
    }
}
...
void malformed_contracts() {
    auto refuse = [&](renderer::LinearMaterialPairContract contract,
        ...
        MotionOutput m(d);
        m.shadow_.material_contract = contract;
        for (unsigned i = 0; i < 16; ++i) d.states[wrap(i)] = i;
        const auto before = d.states;
        MotionRoute r;
        r.depth = depth;
        r.linear_material = true;
        CHECK(m.apply_wrap_states(r, row) == D3DERR_INVALIDCALL && d.gets == 0 && d.sets == 0 && d.states == before);
        CHECK(m.undo(r) == S_OK);
    };
    auto valid = material(1, {6, 0, 1, 3});
    {
        auto c = valid;
        c.scalar_transport_count = 3;
        refuse(c);
    }
    {
        auto c = valid;
    ...
    {
        auto c = valid;
        c.scalar_transport[0].source_texcoord = 16;
        refuse(c);
    }
    {
        auto c = valid;
        c.scalar_transport[0].destination_texcoord = 16;
        refuse(c);
    }
    {
        auto c = valid;
        c.scalar_transport[0].source_component = 4;
        refuse(c);
    }
    {
        auto c = valid;
        c.scalar_transport[0].destination_component = 4;
        refuse(c);
    }
    {
        auto c = valid;
        c.scalar_transport[0].source_texcoord = 4;
        refuse(c);
    }
    {
        auto c = valid;
        c.scalar_transport[0].destination_texcoord = 4;
        refuse(c);
    }
    {
        auto c = valid;
        c.scalar_transport[0].source_texcoord = 5;
        refuse(c);
    }
    {
        auto c = valid;
        c.scalar_transport[0].destination_texcoord = 5;
        refuse(c);
    }
    {
        auto c = material(2, {6, 0, 1, 3}, {7, 1, 1, 3});
        refuse(c);
    }
    refuse(valid, {4, 4});
}
...
void shadow_reset_and_performance() {
    ...
    auto gets = d.gets;
    CHECK(m.apply_wrap_states(r, p) == S_OK && m.undo(r) == S_OK && d.gets == gets && d.sets == 0);
    // Recorded writes preserve the logical cache. End/Apply invalidate every slot.
    set_state(&d, wrap(6), 3);
    m.set_render_state(wrap(6), 3);
    m.begin_stateblock();
    m.set_render_state(wrap(6), 0);
    CHECK(m.shadow_.states[14] == 3);
    m.end_stateblock();
    d.states[wrap(1)] = 0;
    d.states[wrap(2)] = 15;
    CHECK(m.apply_wrap_states(r, p) == S_OK && d.states[wrap(1)] == 8 && d.states[wrap(2)] == 15);
    CHECK(m.undo(r) == S_OK);
    d.states[wrap(6)] = 0;
    d.states[wrap(1)] = d.states[wrap(2)] = 15;
    m.stateblock_applied();
    CHECK(m.apply_wrap_states(r, p) == S_OK && d.states[wrap(1)] == 7 && d.states[wrap(2)] == 7);
    CHECK(m.undo(r) == S_OK);
    m.state_shadow_ = false;
    d.states[wrap(6)] = 3;
    d.states[wrap(1)] = d.states[wrap(2)] = 0;
    CHECK(m.apply_wrap_states(r, p) == S_OK && d.states[wrap(1)] == 8 && d.states[wrap(2)] == 8);
    CHECK(m.undo(r) == S_OK);
    // Reset quarantine clears only after all sixteen native states resynchronize.
    m.state_shadow_ = true;
    m.motion_state_lost_ = true;
    m.motion_state_error_ = E_FAIL;
    m.after_reset(E_FAIL);
    CHECK(m.motion_state_lost_);
    d.fail_get[0] = d.gets + 4;
    m.after_reset(S_OK);
    CHECK(m.motion_state_lost_);
    d.fail_get = {};
    d.states = {};
    ...
    const auto alloc = allocations, reads = d.gets, writes = d.sets;
    const auto start = std::chrono::steady_clock::now();
    for (unsigned i = 0; i < 200000; ++i) {
        MotionRoute t;
        t.depth = true;
        t.linear_material = true;
        m.apply_wrap_states(t, p);
        m.undo(t);
    }
    ...
    std::printf(
        "palette_wrap_host_zero iterations=200000 ns_per_transaction=%.2f native_gets=0 native_sets=0 allocations=0 lookups=0\n",
        double(ns) / 200000);
}

int main() {
    roundtrips();
    actual_material_contracts();
    fallback_and_alias_cases();
    restoration_ownership();
    failure_cases();
    malformed_contracts();
    shadow_reset_and_performance();
