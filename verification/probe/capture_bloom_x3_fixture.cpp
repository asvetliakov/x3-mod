...
void check(bool ok, const char* name) {
    ...
    if (!ok) {
        ++failures;
        std::printf("CHECK_FAIL case=%s name=%s\n", case_name, name);
    }
}
...
bool is(const char* name) {
    return std::strcmp(case_name, name) == 0;
}
bool terminal() {
    return is("final_release") || is("thread_final_release");
}
bool resetting() {
    return std::strncmp(case_name, "reset", 5) == 0;
}
void drop_external_resources() {
    ...
    if (main_surface) {
        auto* old = main_surface;
        main_surface = nullptr;
        old->Release();
    }
}
DWORD WINAPI release_worker(void*) {
    native_release = device->Release();
    return 0;
}
void original() {
    ++originals;
    ...
    if (is("get_device")) {
        ...
        if (api(main_surface->GetDevice(&alias), "GetDevice")) {
            check(alias == device, "GetDevice_identity");
            extra_release = alias->Release();
        ...
        check((is("reset_fail") || is("reset_ex_fail")) ? FAILED(reset_hr) : SUCCEEDED(reset_hr),
              "Reset_expected_result");
...
template <class T> bool symbol(HMODULE dll, const char* name, T& out) {
    ...
    static_assert(sizeof(proc) == sizeof(out));
    std::memcpy(&out, &proc, sizeof(out));
    check(out != nullptr, name);
    return out != nullptr;
}
}
...
extern "C" void capture_bloom_outer_caught() {
    ++caught;
}
...
int main() {
    ...
    if (!symbol(dll, "Direct3DCreate9", create9) || !symbol(dll, "Direct3DCreate9Ex", create9ex) ||
        ...
        !symbol(dll, "x3m_bloom_lifetime_fixture_query", query))
        return 2;
    HWND window = CreateWindowExA(0, "STATIC", "Capture bloom lifetime fixture", WS_OVERLAPPEDWINDOW, 0, 0, 64, 64,
                                  nullptr, nullptr, GetModuleHandleA(nullptr), nullptr);
    check(window != nullptr, "CreateWindow");
    if (!window) return 3;
    const char* cases[] = {"normal", "get_device", "final_release", "thread_final_release",
                           "reset",  "reset_fail", "reset_ex",      "reset_ex_fail",
                           "escape", "continue"};
    ...
    for (const char* name : cases) {
        ...
        during_absent = during_expired = 999;
        reset_hr = S_FALSE;
        scene_active = false;
        device = nullptr;
        scene = nullptr;
        ...
        if (use_ex) {
            ...
            hr = static_cast<IDirect3D9Ex*>(d3d)->CreateDeviceEx(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, window, flags,
                                                                 &parameters, nullptr, &ex);
            device = ex;
        } else
            hr = d3d->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, window, flags, &parameters, &device);
        if (!api(hr, "CreateDevice")) return 5;
        if (!api(device->GetRenderTarget(0, &main_surface), "GetRenderTarget") ||
            !api(device->CreateTexture(16, 16, 1, D3DUSAGE_RENDERTARGET, D3DFMT_A16B16G16R16F, D3DPOOL_DEFAULT, &scene,
                                       nullptr),
                 "CreateScene"))
            return 6;
        IDirect3DSurface9* source = nullptr;
        if (!api(scene->GetSurfaceLevel(0, &source), "SceneSurface")) return 7;
        api(device->ColorFill(source, nullptr, D3DCOLOR_XRGB(80, 120, 160)), "FillScene");
        source->Release();
        ...
        scene_active = true;
        void* entry = entry_fixture();
        check(entry != nullptr, "entry");
        if (!entry) return 9;
        void (*target)();
        static_assert(sizeof(target) == sizeof(entry));
        std::memcpy(&target, &entry, sizeof(target));
        capture_bloom_outer_call(target);
        check(originals == 1, "original_once");
        check(query(0) == 1, "pre_once");
        check(query(2) == 1, "cleanup_once");
        check(query(4) == 1, "real_prepare_success");
        check(query(11) == 0, "bridge_quiescent");
        ...
        if (resetting()) {
            check(query(6) == 1, "reset_hook_once");
            check(query(7) == 1, "DEFAULT_refs_gone_before_Reset");
            check(query(10) == 1, "native_pin_kept_before_Reset");
        } else
            check(query(6) == 0, "no_reset");
        if (terminal()) {
            ...
            check(query(8) == 1, "device_map_erased");
            check(query(9) == 1, "CPU_context_expired");
        }
        std::printf(
            "CASE name=%s status=%s pre=%u post=%u cleanup=%u abnormal=%u prepared=%u committed=%u resets=%u defaults_clear=%u map_absent=%u cpu_expired=%u pin_kept=%u active=%u prepare_hr=%08x commit_hr=%08x original=%u caught=%u continued=%u resumed=%u release=%u during_absent=%u during_expired=%u reset_hr=%08lx motion_refs=%u hdr=%u taa=%u\n",
            name, failures == failures_before ? "PASS" : "FAIL", query(0), query(1), query(2), query(3), query(4),
            query(5), query(6), query(7), query(8), query(9), query(10), query(11), query(12), query(13), originals,
            caught, continued, resumed, native_release, during_absent, during_expired, (unsigned long)reset_hr,
            query(14), query(15), query(16));
