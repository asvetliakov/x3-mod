...
#include <cstring>
#ifdef XML_ONLY
extern "C" __declspec(dllexport) void* __cdecl xmlReadMemory(const char* data, int size, const char* url,
                                                             const char* encoding, int options) {
    bool okay = data == reinterpret_cast<const char*>(0x1000) && size == 123 &&
                url == reinterpret_cast<const char*>(0x2000) && encoding == reinterpret_cast<const char*>(0x3000) &&
                options == 0x180;
    SetLastError(okay ? 0x4321 : 0x8765);
    return okay ? reinterpret_cast<void*>(0x4000) : nullptr;
}
#else
extern "C" __declspec(dllexport) void* __cdecl gzopen(const char* path, const char* mode) {
    ...
    SetLastError(okay ? 0x4321 : 0x8765);
    return okay ? reinterpret_cast<void*>(0x3000) : nullptr;
}
...
extern "C" __declspec(dllexport) LONG __cdecl gzseek(void* file, LONG offset, int whence) {
    ...
    SetLastError(okay ? 0x4321 : 0x8765);
    return okay ? static_cast<LONG>(0x76543210) : -1;
}
