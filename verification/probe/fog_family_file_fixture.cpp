...
#endif
namespace {
bool fail_large_allocations = false;
} // switch-time allocation failure witness
...
void put32(std::vector<std::uint8_t>& b, std::size_t at, std::uint32_t v) {
    for (unsigned i = 0; i < 4; ++i) b[at + i] = std::uint8_t(v >> (8 * i));
}
void put64(std::vector<std::uint8_t>& b, std::size_t at, std::uint64_t v) {
    for (unsigned i = 0; i < 8; ++i) b[at + i] = std::uint8_t(v >> (8 * i));
}
void putf(std::vector<std::uint8_t>& b, std::size_t at, float v) {
    std::uint32_t u;
    std::memcpy(&u, &v, 4);
    put32(b, at, u);
}
...
std::uint64_t get64(const std::vector<std::uint8_t>& b, std::size_t at) {
    return get32(b, at) | std::uint64_t(get32(b, at + 4)) << 32;
}
std::uint64_t fnv(const std::uint8_t* p, std::size_t n, std::uint64_t h = fnv_offset) {
    for (std::size_t i = 0; i < n; ++i) h = (h ^ p[i]) * fnv_prime;
    return h;
}
std::uint64_t zeros(std::uint64_t h, std::uint64_t count) {
    auto f = fnv_prime;
    while (count) {
        if (count & 1) h *= f;
        f *= f;
...
Packet make_packet(std::uint32_t profile, std::uint16_t seed, std::uint32_t literal = 4) {
    ...
    auto word = [&](std::uint32_t v) {
        for (unsigned i = 0; i < 4; ++i) payload.push_back(std::uint8_t(v >> (8 * i)));
    };
    word(0x80000000u | literal);
    for (std::uint32_t t = 0; t < literal * 4; ++t) {
        const std::uint16_t half = std::uint16_t(0x3000 + ((seed + t) & 0x3ff));
        payload.push_back(std::uint8_t(half));
        payload.push_back(std::uint8_t(half >> 8));
    }
    ...
    put32(p.bytes, 8, 1);
    put32(p.bytes, 12, 56);
    put32(p.bytes, 16, profile);
    put32(p.bytes, 20, qualified_recipe_id);
    put32(p.bytes, 24, width);
    put32(p.bytes, 28, height);
    put32(p.bytes, 32, texel);
    put32(p.bytes, 36, decoded);
    put32(p.bytes, 40, 2);
    put64(p.bytes, 44, p.checksum);
    put32(p.bytes, 52, 0);
...
struct Family {
    std::string name;
    std::uint32_t packet;
    float sigma = 2.5e-6f, occupancy = .12f;
...
std::vector<std::uint8_t> build(const std::vector<Family>& families, const std::vector<Packet>& packets) {
    ...
    for (std::uint32_t i = 0; i < fc; ++i) {
        ...
        put32(b, r + 32, family_name_id(families[i].name.c_str()));
        put32(b, r + 36, families[i].packet);
        putf(b, r + 40, families[i].sigma);
        putf(b, r + 44, families[i].occupancy);
    ...
    for (std::uint32_t j = 0; j < pc; ++j) {
        ...
        put64(b, r, offset);
        put64(b, r + 8, packets[j].bytes.size());
        put32(b, r + 16, width);
        put32(b, r + 20, height);
        put32(b, r + 24, texel);
        put32(b, r + 28, decoded);
        put64(b, r + 32, packets[j].checksum);
        put32(b, r + 40, packets[j].profile);
        offset += packets[j].bytes.size();
    }
    for (const auto& p : packets) b.insert(b.end(), p.bytes.begin(), p.bytes.end());
    std::memcpy(b.data(), "X3FOGFAM", 8);
    put32(b, 8, family_file_version);
    put32(b, 12, 64);
    put32(b, 16, qualified_recipe_id);
    put32(b, 20, fc);
    put32(b, 24, pc);
    put32(b, 28, family_row_bytes);
    put32(b, 32, family_packet_row_bytes);
    put32(b, 36, 64);
    put32(b, 40, table);
    put32(b, 44, 0);
    put64(b, 48, fnv(b.data() + 64, table));
    put64(b, 56, b.size());
    return b;
}
void refresh(std::vector<std::uint8_t>& b) { // recompute the table checksum and size after an edit
    put64(b, 48, fnv(b.data() + 64, get32(b, 40)));
    put64(b, 56, b.size());
}
...
const char* status(FamilyFileStatus s) {
    switch (s) {
    case FamilyFileStatus::Absent: return "absent";
    case FamilyFileStatus::Disabled: return "disabled";
    case FamilyFileStatus::Loaded: return "loaded";
    case FamilyFileStatus::Rejected: return "rejected";
    default: return "not_loaded";
    }
}
unsigned cases = 0;
void compiled_intact(const FamilyTable& t) {
    ...
    for (const auto& f : family_profiles)
        require(!t.find(f.family), std::string("compiled name matched a file row: ") + f.family);
}
void expect_rejected(const std::string& name, const std::vector<std::uint8_t>& bytes, const char* reason) {
    ...
    require(t->status == FamilyFileStatus::Rejected && !std::strcmp(t->reason, reason) && t->families == 0 &&
                !t->find("zzsynth"),
...
void expect_row(const std::string& name, std::vector<std::uint8_t> bytes, std::uint32_t row, const char* reason,
                bool fresh = true) {
    ...
    require(why && !std::strcmp(why, reason),
            name + ": expected row reason " + reason + ", got " + (why ? why : "none"));
    const std::uint32_t other = row ? 0 : 1;
    require(!t->rows[other].disabled.load() && t->find(t->rows[other].name), name + ": other row disabled");
    std::vector<std::uint16_t> out;
    require(bool(decode_family_packet(*t, native(path).c_str(), Profile(t->rows[other].profile), out)) &&
                out.size() * 2 == decoded,
            name + ": other row decode");
    ProfileInfo info{};
...
void expect_decode_failure(const std::string& name, std::vector<std::uint8_t> bytes, Status expected,
    ...
    require(result.status == expected && out.empty(),
            name + ": expected decode status " + status_name(expected) + ", got " + status_name(result.status));
    require(t->rows[0].disabled.load() && !std::strcmp(t->rows[0].disabled.load(), status_name(expected)) &&
                !t->find(t->rows[0].name),
            name + ": row not disabled");
...
int self_test() {
    ...
    const std::size_t packet0 = std::size_t(get64(valid, prow0));
    {
        ...
        const auto t = load(path);
        require(t->status == FamilyFileStatus::Loaded && !std::strcmp(t->reason, "ok") && t->families == 2 &&
                    t->packets == 2 && t->rows_disabled == 0,
                "valid load");
        for (unsigned i = 0; i < 2; ++i) {
            const auto* row = t->find(i ? b.c_str() : a.c_str());
            require(row && row->profile == family_name_id(row->name) && row->profile >= family_dynamic_id_bit,
                    "valid row");
            ProfileInfo info{};
            require(family_profile_info(*t, Profile(row->profile), info) && info.base_sigma == 2.5e-6f &&
                        info.decoded_bytes == decoded && info.resource_id == 0,
                    "valid info");
            std::vector<std::uint16_t> out;
            const auto r = decode_family_packet(*t, native(path).c_str(), Profile(row->profile), out);
            std::uint64_t h = fnv_offset;
            for (auto w : out) {
                h = (h ^ std::uint8_t(w)) * fnv_prime;
                h = (h ^ std::uint8_t(w >> 8)) * fnv_prime;
            }
            require(r && r.decoded_bytes == decoded && out.size() * 2 == decoded && h == info.decoded_fnv1a &&
                        !row->disabled.load(),
                    "valid decode");
        }
    ...
    { // Two rows sharing one packet (deduplicated by decoded hash): the packet carries the first row's id.
        ...
        for (unsigned i = 0; i < 2; ++i) {
            std::vector<std::uint16_t> out;
            require(bool(decode_family_packet(*t, native(path).c_str(), Profile(t->rows[i].profile), out)),
                    "shared decode");
        }
        ++cases;
        std::printf("CASE shared_packet status=loaded\n");
    }
    { // The empty table (0 families, 0 packets, 64 bytes) the tool installs when the compiled 14
        ...
        ProfileInfo info{};
        require(empty.size() == 64 && t->status == FamilyFileStatus::Loaded && !std::strcmp(t->reason, "ok") &&
                    t->families == 0 && t->packets == 0 && t->rows_disabled == 0 && t->file_bytes == 64 &&
                    !t->find(a.c_str()) && !family_profile_info(*t, Profile(family_name_id(a.c_str())), info),
                std::string("empty table: ") + t->reason);
        compiled_intact(*t);
        ++cases;
        std::printf("CASE empty_table status=loaded families=0 packets=0\n");
    }
    {
        ...
        compiled_intact(*t);
        ++cases;
        std::printf("CASE absent status=absent\n");
        const auto d = load(dir);
        require(d->status == FamilyFileStatus::Rejected && !std::strcmp(d->reason, "open_failed"),
                std::string("directory: ") + d->reason);
        ++cases;
        std::printf("CASE directory status=rejected reason=%s\n", d->reason);
    }
    auto edit = [&](const std::function<void(std::vector<std::uint8_t>&)>& f) {
        auto v = valid;
        f(v);
        return v;
    };
    expect_rejected("zero_length", {}, "truncated_header");
    expect_rejected("truncated_header", std::vector<std::uint8_t>(valid.begin(), valid.begin() + 40),
                    "truncated_header");
    ...
    expect_rejected("table_past_eof", edit([](auto& v) {
                        v.resize(100);
                        put64(v, 56, v.size());
                    }),
                    "table_past_eof");
    ...
    { // A disabled first row still owns its name and id: the later duplicate is disabled too, so
        ...
        require(t->status == FamilyFileStatus::Loaded && !std::strcmp(t->rows[0].disabled.load(), "sigma") &&
                    ...
                    !family_profile_info(*t, Profile(family_name_id(a.c_str())), info),
                "duplicate after a disabled row");
        compiled_intact(*t);
        ++cases;
    ...
    { // A packet header may never carry a compiled id, even when a (disabled) row names it.
        ...
        require(t->status == FamilyFileStatus::Loaded && !std::strcmp(t->rows[0].disabled.load(), "profile_id") &&
                    !std::strcmp(t->rows[1].disabled.load(), "packet_profile") && !t->find(b.c_str()),
                "packet with a compiled id");
        compiled_intact(*t);
        ++cases;
        std::printf("CASE packet_compiled_id status=loaded row=1 reason=packet_profile\n");
    }
    expect_row("name_duplicate", build({{a, 0}, {a, 1}}, {pa, make_packet(family_name_id(a.c_str()), 5)}), 1,
               "name_duplicate");
    expect_row("name_compiled", edit([&](auto& v) {
                   std::memset(&v[row0], 0, 32);
                   std::memcpy(&v[row0], "bluewell", 8);
                   put32(v, row0 + 32, family_name_id("bluewell"));
               }),
               0, "name_compiled");
    ...
    expect_row("id_other_name", edit([&](auto& v) { put32(v, row1 + 32, family_name_id(a.c_str())); }), 1,
               "profile_id");
    ...
    expect_row("sigma_nan", edit([&](auto& v) { putf(v, row0 + 40, std::numeric_limits<float>::quiet_NaN()); }), 0,
               "sigma");
    ...
    expect_row("packet_profile", edit([&](auto& v) { put32(v, prow0 + 40, family_name_id(b.c_str())); }), 0,
               "packet_profile");
    ...
    auto shrink_packet = [&](auto& v) {
        put64(v, prow0 + 8, get64(v, prow0 + 8) - 1);
        refresh(v);
    };
    expect_decode_failure("decode_run_past_end", edit([&](auto& v) { put32(v, literal + 32, width * height); }),
                          Status::InvalidRun);
    expect_decode_failure("decode_short_packet", edit(shrink_packet), Status::Truncated);
    expect_decode_failure("decode_zero_run", edit([&](auto& v) { put32(v, first_run, 0x80000000u); }),
                          Status::InvalidRun);
    expect_decode_failure("decode_overflow_run", edit([&](auto& v) { put32(v, first_run, 0x7fffffffu); }),
                          Status::InvalidRun);
    expect_decode_failure("decode_nonfinite_half", edit([&](auto& v) {
                              v[literal] = 0;
                              v[literal + 1] = 0x7c;
                          }),
                          Status::InvalidHalf);
    ...
    { // A first allocation failure at a switch is retried once; the second disables the rows.
        ...
        require(first.status == Status::AllocationFailed && retained && second.status == Status::AllocationFailed &&
                    t->rows[0].disabled.load() && !std::strcmp(t->rows[0].disabled.load(), "allocation") &&
                    !t->find(a.c_str()) &&
                    bool(decode_family_packet(*t, native(path).c_str(), Profile(t->rows[1].profile), out)),
                "allocation retry");
        ++cases;
        std::printf("CASE allocation_retry status=decode_failed_twice reason=allocation\n");
    }
...
int probe() {
    ...
    std::printf("TABLE status=%s reason=%s families=%u packets=%u rows_disabled=%u bytes=%llu path=%s\n",
                status(t.status), t.reason, t.families, t.packets, t.rows_disabled,
                static_cast<unsigned long long>(t.file_bytes), family_table_path());
    for (std::uint32_t i = 0; i < t.families; ++i) {
        const char* why = t.rows[i].disabled.load();
        std::printf("ROW %u name=%s profile=%u packet=%u disabled=%s\n", i, t.rows[i].name, t.rows[i].profile,
                    t.rows[i].packet, why ? why : "-");
    }
    for (std::uint32_t i = 0; i < t.families; ++i) {
        ...
        std::printf("DECODE %s status=%s bytes=%zu fnv=%016llx match=%u found_after=%u\n", t.rows[i].name,
                    status_name(r.status), out.size() * 2, static_cast<unsigned long long>(h),
                    unsigned(bool(r) && h == info.decoded_fnv1a), unsigned(t.find(t.rows[i].name) != nullptr));
    }
...
int main(int argc, char** argv) try {
    if (argc == 3 && !std::strcmp(argv[1], "--self-test")) {
        dir = argv[2];
        return self_test();
    }
