...
static std::uint64_t test_clock(){
    ++clock_calls;SetLastError(0xbad0);
    if(nested_job&&clock_calls==nested_at){
        Job& j=*nested_job;nested_job=nullptr;
        fx_call(core::caller_va,j.a->node,j.b->node,j.a->body,j.b->body,2,1,0,0,&j.state);
    }
    if(reset_at_clock){reset_at_clock=false;phases::device_reset();}
    if(reenter_at_clock){reenter_at_clock=false;phases::invalidate();}
    if(foreign_at_clock){foreign_at_clock=false;phases::foreign();}
    return fail_clock?0:++tick;
}
static DWORD WINAPI mixed_first_thread(void*){
    query_abi_control=0x037f;_mm_setcsr(0x5f80);
...
static void abi(){
    ...
    for(unsigned mode=0;mode<4;++mode)for(unsigned padding:{0u,4u,8u,12u}){
        const std::uint16_t controls[]={0x037f,0x0b7f,0x0f7f,0x037f};
        query_abi_control=controls[mode];
        std::uint32_t registers[7],args[5],mxcsr;unsigned char xmm[128],fpu[108];
        _mm_setcsr(mode?0x5f80:0x1f80);SetLastError(0x12345678);
        query_abi_call(reinterpret_cast<void*>(&query_abi_target),padding);
        std::memcpy(registers,query_abi_regs,sizeof registers);std::memcpy(args,query_abi_args,sizeof args);
        std::memcpy(xmm,query_abi_xmm,sizeof xmm);std::memcpy(fpu,query_abi_fpu,sizeof fpu);mxcsr=query_abi_mxcsr;const auto rounded=query_abi_round;
        phases::begin();SetLastError(0x12345678);
        ...
        const DWORD error=GetLastError();phases::end();
        check(!std::memcmp(registers,query_abi_regs,sizeof registers),"root wrapper GP registers at every 4-byte alignment");
        check(!std::memcmp(args,query_abi_args,sizeof args)&&args[0]==0x11111111&&args[4]==0x55555555,"root wrapper five args");
        check(!std::memcmp(xmm,query_abi_xmm,sizeof xmm),"root wrapper XMM0-7 outputs");
        check(!std::memcmp(fpu,query_abi_fpu,10)&&!std::memcmp(fpu+28,query_abi_fpu+28,80),"root wrapper live x87 CW/SW/tags/data");
    ...
    query_abi_control=0x037f;_mm_setcsr(0x1f80);x3m_collide_query_descent_target=0x004e2530;
}
static void unpatched_reference(){
    ...
    const auto fix=[](std::uint32_t call,std::uint32_t target){engine<std::int32_t>(call+1)=std::int32_t(target-call-5);};
    fix(reference_va+core::memo_site_va-core::caller_va,dispatch);
    fix(dispatch+0x12,0x52b5d0);fix(dispatch+0x56,query_copy);
    const std::uint32_t calls[][2]={{0x4e27fc,0x4e1ff0},{0x4e281e,0x4e20d0},{0x4e2833,0x4e1ff0},{0x4e2850,0x4e20d0},{0x4e2865,0x4dfd80},{0x4e28ae,0x4e2130},{0x4e28c8,0x4dfd80},{0x4e290f,0x4e2130},{0x4e2956,0x4e2530}};
...
int main(){
    ...
    unpatched_reference();
    unsigned char root[5];std::memcpy(root,reinterpret_cast<void*>(0x4e2956),5);
    ...
    check(!memo::install_at({core::memo_site_va,core::query_va},false)&&pristine()&&!phases::enabled,"memo claim failure rolls root back");
    phases::fixture_fail_restore(true);
    check(!memo::install_at({core::memo_site_va,core::query_va},false)&&!std::strcmp(memo::state(),"rollback_failed")&&!pristine(),"failed root rollback retained and reported");
    ...
    WaitForSingleObject(mixed,INFINITE);CloseHandle(mixed);
    x3m_collide_query_descent_target=0x004e2530;
    phases::begin();phases::end();
    check(phases::fixture_sample().queries==1&&phases::fixture_sample().cpu_mode==1,"first mixed foreign thread cannot steal normal memo owner");
    abi();
    Object a{},b{};double rotation_matrix[9];rotation(rotation_matrix,0);set_rotation(a,rotation_matrix);set_rotation(b,rotation_matrix);
    Model* ma=make_model(64,20),*mb=make_model(64,20);
    place(a,ma,0,0,0,1);place(b,mb,100000,0,0,1);
    const Mode mode{2,1,0,false,0};
    memo::device_reset();phases::fixture_clear();
    const auto calls_before=clock_calls;query(a,b,mode,"first");
    check(clock_calls-calls_before==4,"miss exactly four clocks");
    const auto sampled=phases::fixture_sample();check(sampled.queries==1&&sampled.descents==1&&sampled.query==3&&sampled.descent==1&&sampled.non_descent==2,"same-sample raw query/descent/difference");
    const auto before_hit=clock_calls;query(a,b,mode,"hit");
    check(clock_calls-before_hit==0&&phases::fixture_sample().queries==1,"memo hit and untracked descent have no clocks");
    for(unsigned fault=0;fault<4;++fault){
        ...
        phases::end();fail_clock=false;
        check(phases::fixture_sample().invalid==before.invalid+1,"failed/poisoned clocks discard full query");
    }
    phases::begin();phases::present(1,1);check(phases::fixture_sample().queries==0,"abandoned query discarded at Present");
    phases::device_reset();phases::begin();phases::end();check(phases::fixture_sample().reset==1&&phases::fixture_sample().queries==1,"Reset discards partial frame and rearms");
    const auto before_foreign=clock_calls;
    Job job{&a,&b,{}};HANDLE thread=CreateThread(nullptr,0,foreign_thread_main,&job,0,nullptr);WaitForSingleObject(thread,INFINITE);CloseHandle(thread);
    ...
    place(b,mb,200000,0,0,1);Job nested{&a,&b,{}};
    const auto reentered=memo::counters().reentered;const auto invalid=phases::fixture_sample().invalid;
    nested_job=&nested;nested_at=clock_calls+2;State outer{};
    fx_call(core::caller_va,a.node,b.node,a.body,b.body,2,1,0,0,&outer);
    check(!nested_job&&memo::counters().reentered==reentered+1&&outer.eax==0&&nested.state.eax==0&&phases::fixture_sample().invalid==invalid+1,"nested memo/root bypass preserves outer return and discards timing");
    phases::fixture_clock(nullptr);
    check(memo::shutdown()&&memo::initialize(),"real clock calibration after fault-clock tests");
    memo::device_reset();phases::fixture_clear();
    for(unsigned frame=1;frame<=600;++frame){place(b,mb,100000+frame,0,0,1);query(a,b,mode,"moving");memo::present(1,frame,false);}
    check(phases::fixture_window_size()==0&&tally.differences==0&&tally.register_differences==0,"600 moving frames emit two windows; engine parity");
    // Real recursive/leaf/contact behavior, against the independent query copy.
    unsigned contacts=0,deep=0;
    for(int x=-35;x<=35;++x){place(b,mb,x,1,0,1);query(a,b,mode,"deep_contact");
        contacts+=last_contact;deep+=engine<std::uint32_t>(core::visits_va)>1;}
    check(contacts>0&&deep>0&&tally.differences==0,"recursive descent and contacts match truly unpatched reference");
    check(memo::shutdown()&&memo::install_at({core::memo_site_va,core::memo_target_va},true),"verify-mode diagnostic install");
    place(b,mb,100000,0,0,1);query(a,b,mode,"verify_first");query(a,b,mode,"verify_repeat");
    check(phases::fixture_sample().verifies==1&&memo::counters().verify_mismatches==0&&tally.differences==0,"verify query counted separately with parity");
    ...
    for(unsigned enabled=0;enabled<2;++enabled){
        ...
        double trials[7];State state{};
        for(unsigned trial=0;trial<7;++trial){const auto start=seconds();
            for(unsigned k=0;k<10000;++k){b.node[0xb0/4]=100000+trial*10000+k;fx_call(core::caller_va,a.node,b.node,a.body,b.body,2,1,0,0,&state);}
            trials[trial]=(seconds()-start)*1e9/10000;}
        std::sort(trials,trials+7);cost[enabled]=trials[3];
    }
    ...
    engine_patch::close_install_window("query_fixture");check(!phases::initialize(true)&&pristine(),"late claim refused");
    std::printf("COLLIDE QUERY CPU checks=%u failures=%u queries=%lu differences=%lu\n",checks,failures,tally.queries,tally.differences);
    return failures?1:0;
}
