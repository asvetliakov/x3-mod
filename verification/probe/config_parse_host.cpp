// Host harness of the x3m.ini parser (src/config/config_parse.h, docs/architecture/config-file.md): parses the file named on
// the command line with the DLL's own code and prints what the resolver would hold, one fact per line, for
...
// arguments) with the DLL's own below_environment: one "NAME=value@env|file|default|bare" or "NAME unset" line per query.
static int resolve(int argc, char** argv) {
    ...
    int k = 2;
    const char* env_name[64]; const char* env_value[64]; int env_count = 0;
    for (; k < argc && std::strcmp(argv[k], "--"); ++k) {
        char* eq = std::strchr(argv[k], '=');
        if (!eq || env_count == 64) return 2;
        *eq = 0; env_name[env_count] = argv[k]; env_value[env_count++] = eq + 1;
    }
    const auto env_find = [&](const char* name) { for (int e = 0; e < env_count; ++e) if (!std::strcmp(env_name[e], name)) return e; return -1; };
    for (++k; k < argc; ++k) {
        const char* name = argv[k];
        const int e = env_find(name);
        if (e >= 0) { std::printf("%s=%s@env\n", name, env_value[e]); continue; }
        const int i = parse::find(name);
        if (i < 0) { std::printf("%s unset\n", name); continue; }
        const parse::Layer layer = parse::below_environment(i, bare, [&](int j) { return result.value[j] != nullptr; },
                                                            [&](int j) { return env_find(x3m::config::schema::entries[j].env) >= 0; });
        if (layer == parse::Layer::file) std::printf("%s=%s@file\n", name, result.value[i]);
        else if (layer == parse::Layer::base) std::printf("%s=%s@%s\n", name, parse::base_value(i, bare), bare ? "bare" : "default");
        else std::printf("%s unset\n", name);
    }
    return 0;
}
...
int main(int argc, char** argv) {
    ...
    if (argc != 2) { std::fprintf(stderr, "usage: config_parse_host FILE | find NAME | resolve FILE|- file|none|bare [NAME=VALUE...] -- NAME...\n"); return 2; }
