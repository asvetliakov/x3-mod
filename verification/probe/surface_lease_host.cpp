...
struct Registry {
    ...
    SurfaceLookup lookup(void* device, void* surface, const SurfaceLeaseIdentity* expected, SurfaceLeaseIdentity& out) {
        return detail::lookup_surface(
            mutex, nodes, device, surface, Kind::Device, Kind::Surface,
            [](FakeCom* d, FakeCom* s) {
...
static void validation_and_lifetime() {
    ...
    r.insert(d, Kind::Device);
    r.insert(other_device, Kind::Device);
    r.insert(s, Kind::Surface, &d);
    r.insert(wrong_kind, Kind::Other, &d);
...
static void move_assignment_cleanup() {
    ...
    FakeCom d{}, a{}, b{};
    r.insert(d, Kind::Device);
    r.insert(a, Kind::Surface, &d);
    r.insert(b, Kind::Surface, &d);
    {
        Lease first, second;
        first.adopt_retained(&a);
        second.adopt_retained(&b);
...
static void release_acquire_race() {
    ...
    for (unsigned i = 0; i != 512; ++i) {
        ...
        Lease lease;
        if (result == SurfaceLookup::Ready) {
            lease.adopt_retained(&s);
            ++acquired;
        } else {
            check(result == SurfaceLookup::Invalid);
            ++refused;
        }
        final_release.join();
...
int main() {
    validation_and_lifetime();
    address_reuse_and_exhaustion();
    move_assignment_cleanup();
    release_acquire_race();
}
