...
def build(output):
    ...
    sources=[ROOT/p for p in ['verification/probe/lattice_state_fixture.cpp','src/proxy/lattice_state_capture.cpp','src/proxy/capture_state.cpp']]
    command=[compiler,'-std=c++17','-O2','-Wall','-Wextra','-Werror','-msse2','-mfpmath=sse',
             '-mstackrealign','-mincoming-stack-boundary=2','-ffunction-sections','-fdata-sections',
             '-I',str(ROOT/'src/proxy'),'-I',str(output),*[str(p) for p in sources],
             '-Wl,--gc-sections','-static','-static-libgcc','-static-libstdc++','-ldxguid','-o',str(exe)]
    ...
    dependencies=sources+[ROOT/'src/proxy/lattice_state_capture.h',ROOT/'src/proxy/lattice_state_policy.h',ROOT/'src/proxy/cpu_state.h',ROOT/'src/proxy/capture_state.h',extracted]
    record=dict(command=command,exit_code=run.returncode,seconds=elapsed,inputs={str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in dependencies},
                compiler=subprocess.check_output([compiler,'--version'],text=True).splitlines()[0],native_execution=False)
