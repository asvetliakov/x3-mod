...
def validate_case(output, trace_lines, material, taa):
    ...
    assert int(summary['frames']) == 12 and int(summary['depth_written']) > 0
    assert int(summary['taa_reference_frames']) == (12 if taa else 0)
    ...
    assert set(live) == set(motion) == set(frames) == set(range(12)), 'missing frame evidence'
    for frame, row in frames.items():
        assert int(row['routed']) == int(row['depth_routed']) == 1, (frame, 'motion lost')
        assert int(row['matched']) == (0 if frame in (0, 10) else 1), (frame, 'history changed')
