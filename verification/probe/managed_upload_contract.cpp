...
constexpr unsigned char d3d9_sha[32] = {
    0x58,0xcc,0x36,0xcf,0x74,0x12,0x8a,0xe4,0xb6,0x21,0x11,0x00,0x43,0x0d,0x14,0x6c,
    0x36,0x92,0x80,0x81,0x46,0xd8,0xd2,0x07,0x5e,0x6c,0x5d,0x84,0x61,0x62,0xf8,0xcf};
constexpr unsigned char wined3d_sha[32] = {
    0xf4,0x99,0x7b,0xc0,0x46,0x5d,0xe7,0xe8,0x7b,0xac,0x99,0x21,0xbf,0x02,0x74,0xdb,
    0x00,0xac,0x3b,0x3b,0xa0,0x75,0x4f,0xa0,0x3f,0x1f,0x33,0xe3,0x09,0xa8,0xe8,0x63};
...
bool accessible(const void* pointer, std::uint64_t size, HMODULE owner = nullptr,
    ...
    for (unsigned region = 0; cursor < end && region < 64; ++region) {
        ...
        if (VirtualQuery(reinterpret_cast<const void*>(std::uintptr_t(cursor)), &info, sizeof info) != sizeof info ||
            ...
            (owner && info.AllocationBase != owner)) return false;
        ...
        if ((executable && !rx) || (writable && !rw) ||
            (!executable && !rw && protection != PAGE_READONLY && protection != PAGE_EXECUTE_READ)) return false;
...
bool digest_matches(HMODULE module, const unsigned char expected[32]) noexcept {
    ...
    unsigned char bytes[16384]; DWORD count = 0; std::uint64_t total = 0;
    while (good && total < std::uint64_t(size.QuadPart)) {
        if (!ReadFile(file, bytes, sizeof bytes, &count, nullptr) || !count) { good = false; break; }
        total += count;
    ...
    good = good && total == std::uint64_t(size.QuadPart) &&
        ...
        digest_size == sizeof digest && !std::memcmp(digest, expected, sizeof digest);
...
bool imports_match(HMODULE d3d9, HMODULE wined3d) noexcept {
    ...
    for (unsigned i = 0; i < 6; ++i) {
        ...
        if (!accessible(slot, sizeof(void*), d3d9) || word(slot, 0) != reinterpret_cast<std::uintptr_t>(target) ||
            ...
            !accessible(target, 1, wined3d, true)) return false;
    }
    return true;
}
bool runtime_for(const void* unlock, HMODULE& d3d9, HMODULE& wined3d) noexcept {
    ...
    if (!GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS,
            reinterpret_cast<LPCSTR>(unlock), &candidate)) return false;
    ...
    if (!d3d9_module) {
        ...
        if (good) good = GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS,
        ...
        if (good) good = GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_PIN,
            reinterpret_cast<LPCSTR>(at(wine, 0x7b700)), &pinned_wine) != FALSE;
        if (good) good = GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_PIN,
            reinterpret_cast<LPCSTR>(unlock), &pinned_d3d9) != FALSE;
        if (good) { d3d9_module = pinned_d3d9; wined3d_module = pinned_wine; }
        if (wine) FreeLibrary(wine);
    }
    d3d9 = d3d9_module; wined3d = wined3d_module;
...
template<class Buffer> bool inspect_impl(Buffer* native, BufferContract& result) noexcept {
    ...
    const unsigned endpoints[] = {vertex ? 0x1ae0u : 0x25c0u, vertex ? 0x1c00u : 0x26e0u,
        vertex ? 0x1d10u : 0x27f0u, vertex ? 0x1f90u : 0x2a70u, vertex ? 0x2060u : 0x2b40u,
        vertex ? 0x20c0u : 0x2ba0u};
    for (unsigned i = 0; i < 6; ++i) {
        const void* expected = at(d3d9, endpoints[i]);
        if (word(table, slots[i] * 4) != reinterpret_cast<std::uintptr_t>(expected) ||
            !accessible(expected, 1, d3d9, true)) return false;
    }
    ...
    if (native->GetDesc(&desc) != S_OK || desc.Pool != D3DPOOL_MANAGED || desc.Usage != D3DUSAGE_WRITEONLY ||
        !desc.Size || desc.Size > maximum_buffer_bytes || desc.Type != (vertex ? D3DRTYPE_VERTEXBUFFER : D3DRTYPE_INDEXBUFFER) ||
        (vertex ? desc.Format != D3DFMT_VERTEXDATA : (desc.Format != D3DFMT_INDEX16 && desc.Format != D3DFMT_INDEX32))) return false;
    ...
    if (!accessible(resource, 0xbc) || word(resource, 0x14) != 1 || word(resource, 0x4c) != desc.Size ||
        ...
        word(resource, 0x94) != reinterpret_cast<std::uintptr_t>(at(wined3d, 0x1e7c90))) return false;
...
bool validate_window(const BufferContract& contract, UINT offset, UINT size, DWORD flags,
    ...
    *out = {};
    ...
    if (!same_live_contract(contract, current) || word(current.backend_resource, 8) != 1 ||
        !(word(current.backend_resource, 0xb4) & 2)) return false;
    ...
    if (address >= (std::uint64_t(1) << 32) || pointer != reinterpret_cast<const void*>(std::uintptr_t(address)) ||
        !accessible(pointer, length, nullptr, false, true)) return false;
