...
class SourceAndReplay(unittest.TestCase):
    ...
    def test_production_wiring(self):
        ...
        self.assertIn('accumulator.material(now,pass_link->end_clock,pass_link->begin_clock,pass_link->begin_armed);', handler)
        self.assertIn('accumulator.view(now,frame_link->submit_end);', handler)
        pass_core = (ROOT / 'src/proxy/pass_phases_core.h').read_text()
        self.assertIn('if (index == 0 && begin_armed) { begin_armed = false; begin_clock = now; }', pass_core)
        self.assertIn('if (index == site_count - 1) { ++passes; last = 0; end_clock = now; }', pass_core)
        ...
        self.assertIn('if (!begin_armed && begin_clock >= p_clock) ticks[1] += begin_clock - p_clock;', core)
