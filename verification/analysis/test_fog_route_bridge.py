...
class FogRouteBridgeTests(unittest.TestCase):
    def log(self):
        rows=[f'CHECK {n} PASS' for n in sorted(bridge.REQUIRED) for _ in range(bridge.CAMERA_COUNTS.get(n,1))]
        return '\n'.join([*rows,bridge.CAMERA_SCOPE,SCOPE,f'RESULT fog_route_bridge checks={len(rows)} PASS'])+'\n'
    def test_complete_report_keeps_limits(self):
        value=bridge.validate(self.log());self.assertEqual(value['checks'],sum(bridge.CAMERA_COUNTS.get(n,1) for n in bridge.REQUIRED));self.assertEqual(len(value['limits']),5)
    def test_every_required_witness_is_required(self):
        total=sum(bridge.CAMERA_COUNTS.get(n,1) for n in bridge.REQUIRED)
