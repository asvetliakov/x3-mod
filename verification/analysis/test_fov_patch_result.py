...
the value each executed load store stored (exact-match remap, pass-through, the 65,537-input sweep, registers kept),
...
class FovPatchRecordTests(unittest.TestCase):
    ...
    def test_run_and_bottle(self):
        ...
        self.assertEqual(sorted(audit['sections']), ['.x3mfvc', '.x3mfvd', '.x3mfvf', '.x3mfvl', '.x3mfvr', '.x3mfvs', '.x3mfvx'])
    ...
    def test_pass_counts(self):
        ...
        for name in ('pages_at_engine_vas', 'engine_imm32_offset_4_of_its_qword', 'engine_page_is_mem_image_execute_read', 'default_unset_off_row_and_untouched',
                     ...
                     'load_window_and_caller_bytes_at_engine_vas_site_in_one_qword', 'load_before_install_stores_4000_registers_kept',
                     'refuse_load_site_mismatch_untouched', 'refuse_load_call_mismatch_untouched', 'refuse_load_caller_mismatch_untouched',
                     'install_load_jmp_to_arena_sixth_byte_kept', 'load_vanilla_90_0x4000_to_0x3470', 'load_vanilla_70_0x31c7_to_0x2768',
                     'load_vanilla_100_0x471c_to_0x3b6f', 'load_remapped_90_0x3470_to_0x3470', 'load_remapped_100_0x3b6f_to_0x3b6f',
                     'load_old_constructor_34aa_0x34aa_to_0x34aa', 'load_n45_2000_0x2000_to_0x2000', 'load_n180_8000_0x8000_to_0x8000',
                     'load_non_canonical_4001_0x4001_to_0x4001', 'load_sweep_matches_model_81_changed_registers_kept',
                     'confirm_after_load_row_match', 'confirm_after_second_load_row_3b6f', 'confirm_after_load_registry_absent_row',
                     'restore_load_bytes_back_vanilla_store_4000', 'load_never_claimed_after_setfocus_failure',
                     'load_readback_all_three_sites_rolled_back', 'after_load_rollback_vanilla_setfocus_471c_load_4000',
                     'load_failed_constructor_rollback_failed_registered', 'load_restore_not_owned_foreign_jmp_untouched_registered',
                     'load_restore_after_not_owned_bytes_back', 'install_setting_70_load_90_to_3470', 'install_setting_100_load_90_to_3470',
                     'load_after_late_vanilla_store_4000', 'load_readback_rollback_lasterror_preserved',
                     'load_rollback_setfocus_not_owned_registered_unverified', 'load_rollback_setfocus_foreign_jmp_untouched_at_shutdown',
                     'load_rollback_setfocus_restored_after', 'install_setting_78.5_ctor_0x2ccb', 'install_setting_78.5_load_own_ctor_unchanged',
                     'install_setting_70_load_own_ctor_unchanged', 'load_arena_full_all_three_rolled_back', 'after_arena_full_vanilla_setfocus_load_ctor'):
            self.assertIn(name, names)

    def test_recorded_sequence_and_override(self):
        # Seam reads: reader, store, case window, callee prefix, load window, load caller, window, imm32, INS_SetFocus jmp, load jmp.
        self.assertEqual(self.record['install_sequence'], {'step': 'install', 'protects': 2, 'reads': 10, 'writes': 1, 'atomic': 1, 'flushes': 1})
    ...
    def test_executed_load(self):
        runs = {r['step']: (r['in'], r['out']) for r in self.record['load_runs']}
        self.assertTrue(self.record['load_runs'] and all(r['preserved'] for r in self.record['load_runs']))
        for step, pair in (('before', ('0x4000', '0x4000')), ('patched_vanilla_90', ('0x4000', '0x3470')), ('patched_vanilla_70', ('0x31c7', '0x2768')),
                           ('patched_vanilla_100', ('0x471c', '0x3b6f')), ('patched_remapped_90', ('0x3470', '0x3470')),
                           ('patched_remapped_100', ('0x3b6f', '0x3b6f')), ('patched_old_constructor_34aa', ('0x34aa', '0x34aa')),
                           ('patched_n45_2000', ('0x2000', '0x2000')), ('patched_n180_8000', ('0x8000', '0x8000')),
                           ('patched_non_canonical_4001', ('0x4001', '0x4001')), ('patched_vanilla_n50_floor', ('0x238e', '0x1b6a')),
                           ('patched_vanilla_n130_ceiling', ('0x5c71', '0x52ab')), ('patched_vanilla_n49_passthrough', ('0x22d8', '0x22d8')),
                           ('patched_vanilla_n131_passthrough', ('0x5d27', '0x5d27')), ('patched_below_table', ('0x2333', '0x2333')),
                           ('patched_above_table', ('0x5ccd', '0x5ccd')), ('patched_garbage', ('0xfffffff0', '0xfffffff0')),
                           ('after_restore', ('0x4000', '0x4000')), ('after_setfocus_rollback', ('0x4000', '0x4000')),
                           ('after_load_rollback', ('0x4000', '0x4000')), ('after_load_restore', ('0x4000', '0x4000')), ('after_late', ('0x4000', '0x4000')),
                           ('install_setting_70_load_90_to_3470', ('0x4000', '0x3470')), ('install_setting_100_load_90_to_3470', ('0x4000', '0x3470'))):
            self.assertEqual(runs[step], pair, step)
        # every F 0..0xffff plus 0xffffffff against the model: only the 81 vanilla values of N 50..130 change
        self.assertEqual(self.record['load_sweep'], {'inputs': 65537, 'mismatches': 0, 'changed': 81, 'registers_lost': 0})
        # --fov 78.5: F'(g) 0x2ccc is vanilla N 63, so the constructor stores 0x2ccb and a save of that session loads unchanged
        self.assertEqual((runs['install_setting_78.5_load_own_ctor_unchanged'], self.construction('engine', 'install_setting_78.5_ctor_0x2ccb')),
                         (('0x2ccb', '0x2ccb'), '0x2ccb'))
        # arena_full at the load site: one INS_SetFocus block + claim is 240 B; the fill left less than that plus the load block's 222
        arena = self.record['load_arena_full']
        self.assertEqual(arena['setfocus_bytes'], 240)
        self.assertLess(arena['free_after_fill'], arena['setfocus_bytes'] + 222)
    ...
    def test_log_rows(self):
        ...
        self.assertEqual(counts[('patched', 'ok', 'absent')], 9)
        ...
        self.assertEqual(counts[('refused', 'setfocus_failed', 'skipped')], 2)   # the rollback case and the arena measurement
        self.assertEqual(counts[('patched_unverified', 'setfocus_failed', 'skipped')], 1)
        self.assertEqual(counts[('refused', 'late_claim', 'skipped')], 1)
        self.assertEqual(counts[('refused', 'load_mismatch', 'skipped')], 3)
        self.assertEqual(counts[('refused', 'load_failed', 'skipped')], 2)
        self.assertEqual(counts[('patched_unverified', 'load_failed', 'skipped')], 2)
        # Every live install carries both sites; a refusal before the constructor write never touches the second.
        self.assertTrue(all((row['setfocus'], row['setfocus_write'], row['load'], row['load_write']) == ('active', 'atomic', 'active', 'atomic')
                            for row in installs if row['status'] == 'patched'))
        # A failed load claim takes the other two back; a failed INS_SetFocus claim never reaches the load site.
        self.assertEqual(sorted((row['status'], row['setfocus'], row['load']) for row in installs if row['reason'] == 'load_failed'),
                         [('patched_unverified', 'rollback_failed', 'readback_failed'), ('patched_unverified', 'rolled_back', 'readback_failed'),
                          ('refused', 'rolled_back', 'arena_full'), ('refused', 'rolled_back', 'readback_failed')])
        self.assertTrue(all(row['load'] == 'none' for row in installs if row['reason'] != 'load_failed' and row['status'] != 'patched'))
        self.assertEqual(sorted(row['setfocus'] for row in installs if row['reason'] == 'setfocus_failed'), ['readback_failed'] * 3)
        self.assertEqual({row['value'] for row in installs if row['status'] == 'patched'}, {'0x3470', '0x2768', '0x3b6f', '0x28f8', '0x2ccb'})
        self.assertTrue(all(row['value'] == '0x4000' for row in installs if row['status'] in ('off', 'refused')))
        self.assertEqual([(c['registry'], c['focus'], c['match'], c['after']) for c in self.record['confirm_rows']],
                         [('absent', None, False, None), ('present', '0x3470', True, None), ('present', '0x3470', True, 'save_load_complete'),
                          ('present', '0x3b6f', False, 'save_load_complete'), ('absent', None, False, 'save_load_complete')])
        ...
        self.assertIn(('restored', True, 'restored', 'restore_not_owned'), [(r['status'], r['registered'], r['setfocus'], r['load']) for r in restores])
        self.assertIn(('none', False, 'none', 'restored'), [(r['status'], r['registered'], r['setfocus'], r['load']) for r in restores])
        self.assertIn(('none', True, 'restore_not_owned', 'none'), [(r['status'], r['registered'], r['setfocus'], r['load']) for r in restores])
