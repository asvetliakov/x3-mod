...
class LauncherGate(unittest.TestCase):
    def test_cap_option_removed(self):
        # --shadow-replay-cap went with the single map (2026-09-25): refused as unknown, nothing forwarded.
        with tempfile.TemporaryDirectory() as directory:
            code, output, _ = launch(directory, '--motion-output', '--ownership', '--shadow-replay-candidates')
            self.assertEqual(code, 0); self.assertNotIn('X3M_SHADOW_REPLAY_CAP', json.loads(output)['env'])
            code, _, error = launch(directory, '--motion-output', '--ownership', '--shadow-replay-candidates', '--shadow-replay-cap', '1024')
            self.assertEqual(code, 2); self.assertIn('unrecognized arguments', error)
